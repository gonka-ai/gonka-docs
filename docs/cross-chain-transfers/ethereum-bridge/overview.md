...
!!! tip "Adding WGNK to your Ethereum wallet"
    WGNK is a standard ERC-20 token. [Tangem](https://tangem.com/){target=_blank} lists wrapped GNK on Ethereum natively. In other wallets that support ERC-20 — MetaMask, Trust Wallet, Ledger browser extension, and similar — add it as a **custom token**. Use the bridge contract address above (`0x972a7a92d92796a98801a8818bcf91f1648f2f68`) as the token contract address — it is both the bridge and the WGNK token.
