...
**Confirmation (Random) Proof of Compute** is an auxiliary verification mechanism used to validate the stability of a Host’s declared computational weight outside the primary Proof of Compute Sprint. These checks are designed to confirm that Hosts continue to provide the computational capacity reflected in their most recent PoC results. Confirmation PoC does not replace the standard Proof of Compute process (Sprint).
...
**Governance** in Gonka is a protocol-level mechanism for making collective decisions about network parameters and rules. Governance (voting) power is derived from verifiable compute work, not from passive ownership of GNK. By default, 20% of each Host’s PoC-derived voting power (weight) is activated automatically. The remaining 80% can be activated by locking GNK as collateral, creating a direct link between governance influence, proven compute contribution, and explicit economic commitment. For the first 180 epochs (approximately 6 months), new participants can participate in governance and earn voting power (weight) through PoC alone, without collateral requirements. During this period, the full governance rights are available, while voting weight remains tied to verified compute activity.
...
**Seed Node** is a peer-discovery node that new or restarting nodes briefly connect to in order to obtain a list of other peers and join the network’s P2P layer.

**Upgrade** is a protocol-level coordination event that defines a specific point in the network’s execution timeline at which protocol rules may change. An upgrade establishes a deterministic activation point for new behavior, which may be implemented in different ways:

- by applying governance-approved parameter changes already supported by the current binary,
- by activating logic that was previously shipped but gated by height or configuration,
- by transitioning to a new application binary, optionally accompanied by state migrations.
