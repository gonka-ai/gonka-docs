...
- **B200 owners**: DeepSeek and MiniMax both have B200 node configs. A Kimi coefficient only produces consensus weight if that model group is eligible — check `poc_params` and `confirmation_weight_scales`.
