...
2. You already launched [Quickstart](https://gonka.ai/host/quickstart/) and now want to add more Inference Nodes (go to ["Deploy and register Inference Nodes on a Separate Machine"](https://gonka.ai/host/multi-node-setup-for-review/#deploy-and-register-inference-node-s-on-a-separate-machine)).
