...
As of epoch `308`, `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` has been retired by governance (proposal 78) and `MiniMaxAI/MiniMax-M2.7` is the base model. As of epoch `360` ([proposal 94](../network-updates.md#august-14-2026)), `deepseek-ai/DeepSeek-V4-Flash-0731` is also a PoC model group. As of [proposal 101](../network-updates.md#proposal-101), `zai-org/GLM-5.3-Flash` is a PoC model (`penalty_start_epoch` 394) and `moonshotai/Kimi-K2.6` / `zai-org/GLM-5.2-FP8` are no longer in `poc_params.models`. `poc_params.models` on mainnet contains:

| `model_id` | On-chain notes |
|---|---|
...
| `zai-org/GLM-5.3-Flash` | PoC group since proposal 101. `penalty_start_epoch` 394. Coefficient is in `poc_params`. |
