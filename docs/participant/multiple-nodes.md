...
| `inference_port` | The port where the inference node **accepts inference and training tasks**.    | `5000` (port mapped to `5000` of `mlnode` container)                                                   |
| `poc_port`       | The port which is used for **MLNode management**.   | `8000` (port mapped to `8080` of `mlnode` container)                                                   |
...
!!! note "vLLM performance tuning reference"
    For detailed guidance on selecting optimal deployment configurations and vLLM parameters tailored to your GPU hardware, refer to the [Benchmark to Choose Optimal Deployment Config for LLMs](https://gonka.ai/participant/benchmark-to-choose-optimal-deployment-config-for-llms/) guide.
