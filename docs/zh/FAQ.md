...
您的验证者已被监禁（jailed），因为在最近的 100 个区块中，它签名的区块数量少于 50 个（要求的是这个时间窗口内签名区块的总数，而不是连续签名）。这意味着您的节点会被暂时（约 15 分钟）移出出块流程，以保护网络的稳定性。

导致这种情况可能有以下原因：

- **共识密钥不匹配**. 您的节点使用的共识密钥，可能与链上为您的验证者注册的密钥不一致。请确保节点实际使用的共识密钥与链上注册的密钥一致。
- **网络连接不稳定**. 网络不稳定或中断可能导致节点无法正常参与共识，从而错过签名。请确保节点拥有稳定、低延迟的连接，并避免因为其他进程占用资源而导致性能下降。

**关于奖励**：即使节点被监禁，只要它仍在进行推理或执行其他验证者相关任务，您仍可以继续获得大部分奖励。因此，除非发生推理相关问题，否则奖励不会丢失。

**如何解除监禁（Unjail）**：在问题解决后，您需要使用冷密钥提交 unjail 交易，以恢复节点正常运行：
