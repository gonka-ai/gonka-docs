...
在 Gonka 上锁定 GNK，并在您的以太坊地址收到封装的 GNK（**WGNK**）。
...
验证 `"code": 0` 并提取 base64 `request_id`：
...
!!! tip "桥接纪元更新"
    在提交以太坊铸币交易之前，请确保以太坊桥接合约已同步至 `current_epoch_id`。如果仪表板显示 **桥接需要纪元更新** 或以太坊执行失败并提示 `InvalidEpoch`，请按照[桥接纪元更新](bridge-epoch-update.md)操作。

### C) 在以太坊上提交铸币命令

使用 [mint-wgnk.js](https://github.com/gonka-ai/gonka/blob/gm/contracts/proposals/ethereum-bridge-contact/mint-wgnk.js) 脚本向以太坊上的桥接合约提交铸币命令：
