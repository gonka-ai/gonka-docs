...
本页面面向希望自行展示或集成 Gonka 桥接数据的**仪表板、浏览器和追踪器运营商**。所有桥接状态均可通过公共链 API 读取——无需特殊访问权限。

以下示例使用 `https://node2.gonka.ai:8443/chain-api/...`；你可以将其指向任何你信任的节点。

## 面向终端用户的仪表板

托管的仪表板已为终端用户提供了桥接功能——连接以太坊钱包、推导对应的 `gonka1…` 地址（参见[地址与密钥](addresses-and-keys.md)）、查看封装资产余额，以及发起存入/提现操作：

```text
https://node1.gonka.ai:8443/dashboard/
```

每个合约的 CosmWasm 交易历史记录（可用于审计封装代币）可通过以下地址获取：
...
每条记录包含 `status`（`BRIDGE_PENDING` 或 `BRIDGE_COMPLETED`）、派生的 `ownerAddress`（即 `gonka1…` 接收方）、`amount` 以及验证周期（epoch）——这些信息足以追踪一笔转账从“已看到”到“已完成”的全过程。

## 地址的封装代币余额

列出某个 Gonka 地址持有的所有封装代币（CW-20）余额，包括代币符号、小数位数以及格式化后的可读余额：
...
!!! note
    元数据仅存在于已注册的代币中（参见[注册桥接代币](register-token.md)）。未注册的代币仍然可以进行跨链桥接并显示在余额中，但其元数据为空或为默认值。追踪器应优雅地处理缺失的元数据，并回退到原始合约地址。

## 每个代币对应一个封装合约

每个以太坊代币在 Gonka 上仅对应 **一个** 封装后的 CW-20 合约，以 `(chainId, ethereumContractAddress)` 作为键值。首次存款时会实例化该合约；之后同一代币的所有存款都将复用同一个封装地址。在索引时，应将该封装的 CW-20 合约地址视为该以太坊代币在 Gonka 上的规范身份。
...
去中心化 API 提供了桥接数据接入队列的健康状况，可用于运维监控面板：
