...
本指南描述了通往以太坊的**完整路径**。该路线可在任意完成的步骤后停止：IBC USDT 可保留在 **Keplr 中的 Kava**，转移至 **MetaMask 中的 Kava EVM**，或继续前往 **以太坊**。
...
- Gonka 钱包余额中有**可用的 USDT**
- 一些 **`GNK`** 用于 Gonka 交易手续费
- 一些 **KAVA** 用于 Kava / Kava EVM 手续费
- 一些 **ETH** 用于以太坊手续费
...
Kava 官方帮助文章介绍了如何在 Kava 不同层面之间转移 USDT，相关方向为 **Cosmos → Kava EVM**：[如何一键将 USDt 转移至 Cosmos 链](https://help.app.kava.io/article/44-how-to-transfer-usdt-to-cosmos-chains-with-a-single-click)。

### 如果 USDT 未在 MetaMask 中显示

MetaMask 可能不会自动显示该代币，可能需要手动导入代币。

Kava 帮助文档列出的 **Kava EVM 上的 USDT 合约** 如下：

`0x919C1c267BC06a7039e03fcc2eF738525769109c`

在实际转账前，请再次通过以下渠道验证合约：
...
在某些 UI 中，源网络可能仅显示为 **Kava**。关键是确保与步骤 2 后 USDT 所在的网络一致。
...
如果 **Kava EVM 到以太坊的 USDT** 路线未显示，说明该路线可能已暂停、更改或暂时不可用。
...
如需在另一侧支付 Gas 费用，可使用：[app.kava.io/evm/wkava](https://app.kava.io/evm/wkava)。

Kava 官方分步指南：[在 Kava Cosmos 与 Kava EVM 之间发送 KAVA](https://help.app.kava.io/article/26-send-kava-to-and-from-kava-cosmos-and-kava-evm)。

---

## 高级提示 — 大额转账前验证 Gonka IBC 通道

对于大多数用户而言，小额测试转账**无需验证通道**。
...
当**从 Gonka 发送时**，请使用**Gonka 侧通道**，即 **`channel-5`**。

---

## 最后的安全提醒

在发送全额之前，请确认以下步骤均已成功完成：

- 步骤 1 结束时，USDT 已显示在 **Keplr 中的 Kava 链上**
- 步骤 2 结束时，USDT 已显示在 **MetaMask 中的 Kava EVM 上**
- 步骤 3 在 **Stargate** 中成功实现 **从 Kava EVM 到以太坊的 USDT 转账**
- 小额测试转账已顺利完成

若上述任一检查未通过，请立即停止操作并仔细复查，切勿转移全部资金。

---

请务必反复核对转账路径、地址、资产类型和金额，并始终先进行小额测试转账。
