...
v0.2.15 由于内部 API 重构，意外更改了几个 `/v1` 查询端点的线格式。后续补丁将恢复之前的格式；一旦发布，主机将独立升级：**在过渡期间，以下端点根据您查询的主机不同，可能返回任一格式。** 请同时接受两种格式。这种歧义仅限于此处列出的端点和字段——无需泛化；其他所有内容均未更改且为单一格式。

以下，"v0.2.15 格式" 是当前主机返回的格式；"恢复格式" 是打补丁的主机将返回的 0.2.15 之前的格式。
...
- 来自每主机 API 的一些 `/v1` 响应现在携带 `Deprecation: true` 头部：读取端点正逐步迁移至 edge-api 服务（路径相同，格式相同）。今日无需更改。
