...
- 账户有余额。
- 公钥已上链（CLI 创建账户请使用 `inferenced publish-pubkey`，外部钱包账户可通过任意一笔链上交易完成）。
- `GET /v2/accounts/:address` 能查到你的账户。
- 开发者推理不需要注册 Participant。

完整步骤见 [Developer Quickstart](https://gonka.ai/developer/quickstart/)。
...
将 OpenAI 兼容的 `/chat/completions` 请求负载保存在 `inference-requests` 目录内的文件中。
