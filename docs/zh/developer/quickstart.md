...
本指南解释了如何通过社区代理向 Gonka 发送推理请求。这是今天开始使用网络的最快方式。如果您希望[运行自己的网关](#2-run-your-own-gateway-advanced)而不是通过代理，请参阅本页底部的“运行自己的网关”。

!!! note "如何连接到 Gonka"

    Gonka 推理现在围绕 **devshards** 组织——短暂的会话，持有小额链上存款（托管资金），并离线按请求结算。打开 devshard、签署请求、轮换会话以及提交结算到链上的职责由一种称为 **网关** 的软件执行。

    对于大多数开发者来说，使用 Gonka 最简单的方式是调用 **社区代理**——一个通过网关提供推理访问并暴露标准 OpenAI 兼容 API 的第三方。您只需要从代理获取一个 API 密钥。

    ??? note "Gonka 与传统 AI API 的区别"

    Gonka 不仅仅是一个 AI API。它是一种用于可证明推理的加密协议，旨在使模型执行、计费和结算可独立审计，而不是完全由单一提供商控制。
    ...
    | **抗审查性** | 访问由提供商集中控制。 | 访问正逐步转向透明的协议治理机制。当前生产访问仍受保护，协议级请求验证正在完成中。 |
...
代理是独立运营商，运行 Gonka 网关并向开发者转售推理服务。从您的应用程序角度来看，代理端点的行为类似于任何 OpenAI 兼容的 API：您设置 `base_url`，传递 `Authorization: Bearer <API_KEY>` 头部，并像往常一样调用 `/v1/chat/completions`。
...
??? note "关于此列表"
    这是一个精选的社区代理目录，它们通过公共 Gonka 网关路由推理，并同意公开列示。该列表并非详尽无遗，也不对任何运营商背书。列表在每次页面加载时随机排序，因此每个代理的位置并非排名；请根据各自的优劣评估每个运营商。此目录反映了一个早期启动集。希望独立提供推理服务的新运营商，请参阅[有兴趣运行网关吗？](#3-interested-in-operating-a-gateway)。一些代理提供 **▶ 演示** 链接，指向简短的入门录屏——风格和长度可能有所不同。
...
如果您的应用程序具有高吞吐量或其他要求，您可以自行运行 Gonka 网关，而不是通过经纪人。网关是一个小型程序（以 Docker 容器形式提供），您可以在自己的机器或服务器上运行——绝不能在 Gonka 主机上运行。它暴露与经纪人相同的 OpenAI 兼容 API，但您拥有密钥，并直接在链上为它创建的 devshards 支付 GNK。

!!! warning "自托管网关需要允许列表中的地址"

    目前，只有链上 `devshard_escrow_params.allowed_creator_addresses` 列表中的 Gonka 账户才能打开 devshard。如果您的地址不在该列表中，您的网关无法创建会话，也无法发送推理请求。允许列表仅通过链上治理投票更改。参见下方 [有兴趣操作网关？](#3-interested-in-operating-a-gateway)。
...
**使用公共网关（当前经纪人）。** [§1.1](#11-pick-a-broker) 中的经纪人通过早期推广期间建立的访问安排使用公共 Gonka 网关。这是一个启动步骤，该目录不再主动扩展。

**运行您自己的网关。** 操作您自己的链上 devshard 网关。这要求您的地址在治理控制的允许列表中（`devshard_escrow_params.allowed_creator_addresses`），这是新操作者的推荐路径。完整说明请参见 [网关指南](gateway-developer-quickstart.md)。

要申请链上允许列表准入，请在 [GitHub 问题](https://github.com/gonka-ai/gonka/issues/new?title=Gateway+allowlist+request) 中提交您的操作者名称和联系方式、您计划使用的 `gonka1...` 地址以及您打算提供的模型。是否纳入由链上治理决策决定——任何单一操作者或组织均无权单方面添加地址——表达兴趣并不保证会被审查、纳入或有明确时间表。

---

**需要帮助？** 请参阅 [常见问题页面](https://gonka.ai/FAQ/)，或加入 [Discord 服务器](https://discord.com/invite/RADwCT2U6R) 获取技术问题或安全问题支持。
