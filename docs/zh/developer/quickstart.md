...
本指南介绍如何通过 **社区经纪商（community broker）** 开始使用 Gonka —— 这是目前接入网络最快的方式。它同时涵盖把 Gonka 接入到现有应用或免代码工具（[§1.3](#13)）以及在自己的代码中调用它（[§1.4](#14)）。
...
??? note "Gonka 与传统 AI API 的区别"
    ...
    | **透明的代币经济学** | 价格与资源分配由服务商定义。 | 网络层面的单次推理价格与结算由协议定义、在链上完成，使底层推理经济模型可被检查。经纪商是独立的转售方，会在此之上设定自己的零售价格，因此你实际支付的价格在不同经纪商之间会有所不同。 |
...
如果你不写代码，可以把经纪商凭据填入几乎任何支持 OpenAI 兼容服务商的 AI 软件、聊天界面或 Agent 框架 —— 不需要任何 Gonka 特定的配置。你需要从经纪商那里获取三样东西（见 [§1.2](#12-api-key)）：**Base URL**、**API key** 和 **Model ID**。
...
| Model / Custom Model（模型） | 经纪商支持的准确 Model ID。模型 ID 区分大小写 —— 请严格按原样复制，例如 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。 |
...
更想直接写代码？请继续阅读 [§1.4 发送你的第一个请求](#14)。
