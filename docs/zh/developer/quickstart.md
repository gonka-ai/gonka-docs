...
本指南解释了如何通过社区代理向 Gonka 发送推理请求。这是今天开始使用网络的最快方式。如果您希望[运行自己的网关](#2-run-your-own-gateway-advanced)而非通过代理，请参见本页底部的“运行您的网关”。想自己[作为代理运营](#3-interested-in-operating-a-gateway)？请参见[有兴趣运营网关吗？](#3-interested-in-operating-a-gateway) - 从 [OpenBroker](https://openbroker.gonka.gg) 开始，如果 OpenBroker 无法满足您的需求，可使用 GitHub 白名单问题作为备选方案。

!!! note "如何连接到 Gonka"

    Gonka 推理现在围绕 **devshards** 组织——短暂的会话，持有小额链上存款（托管资金），并离线按请求结算。打开 devshard、签署请求、轮换会话以及向链提交结算的角色由一种称为 **网关** 的软件执行。

    对于大多数开发者而言，使用 Gonka 最简单的方式是调用 **社区代理**——第三方通过网关提供推理访问，并暴露标准的 OpenAI 兼容 API。您只需从代理获取一个 API 密钥即可。

    ??? note "Gonka 与传统 AI API 的区别"

    Gonka 不仅仅是一个 AI API。它是一种用于可证明推理的加密协议，旨在使模型执行、计费和结算独立可审计，而非完全由单一提供商控制。
    ...
    | **抗审查性** | 访问由提供商集中控制。 | 访问正转向透明的协议治理机制。当前生产访问仍受保护，协议级请求验证正在完成中。 |
...
!!! caution "代理不属于核心协议"
    ...
    代理是独立的第三方。定价、支付方式（美元、加密货币、积分）、速率限制、支持的模型、SLA、退款政策和数据处理均由每个代理自行决定。上线前请阅读代理的文档和条款。
...
??? note "关于此列表"
    这是一个精选的社区代理目录，它们通过公共 Gonka 网关路由推理，并同意公开列示。该列表不完整，也不对任何运营者背书。列表在每次页面加载时随机排序，因此每个代理的位置并非排名；请独立评估每个运营者。此目录反映了早期启动集。**想自己成为代理吗？** 使用 [OpenBroker](https://openbroker.gonka.gg)（参见 [§3](#3-interested-in-operating-a-gateway)），如果 OpenBroker 无法满足您的需求，可作为备选方案打开 GitHub 白名单问题。部分代理提供 **▶ 演示** 链接，指向简短的入门录屏——风格和长度可能有所不同。
...
这些仪表板是**社区构建的工具，不属于核心协议的一部分**。数据准确性、方法和可用性由每个仪表板运营商负责。请始终通过您自己的测试验证关键指标。每次页面加载时，列表均随机显示。
...
您需要从代理处获取三项信息（参见 [§1.2](#12-get-an-api-key)）：**基础 URL**、您的 **API 密钥** 和 **模型 ID**。
...
如果您需要自己操作链上 Gonka 网关（您自己的允许列表钱包、您自己的托管金、直接 GNK 结算），您可以在自己的机器或服务器上运行网关软件——绝不能在 Gonka 主机上运行。它暴露与代理相同的 OpenAI 兼容 API，但您拥有密钥，并直接在链上为它创建的 devshards 支付 GNK。

!!! tip "大多数操作者应通过 OpenBroker 成为代理"

    **[OpenBroker](https://openbroker.gonka.gg) 不仅仅是目录中的“另一个代理”。** 它是由可信社区成员创建和运营的基础设施，使您能够**成为代理**——创建代理账户、充值 GNK、颁发 API 密钥并提供推理服务——**无需**链上允许列表的创建者地址，也无需运行您自己的网关。

    除非您明确需要自托管的允许列表网关，否则请优先选择 [§3](#3-interested-in-operating-a-gateway)（OpenBroker）。

!!! warning "自托管网关需要允许列表地址"

    目前，只有链上 `devshard_escrow_params.allowed_creator_addresses` 列表中的 Gonka 账户才能打开 devshards。如果您的地址不在该列表中，您的网关无法创建会话，也无法发送推理请求。允许列表仅通过链上治理投票更改。参见[有兴趣操作网关？](#3-interested-in-operating-a-gateway) —— 从 OpenBroker 开始，或在 OpenBroker 无法满足您的需求时使用 GitHub 允许列表问题作为备选方案。
...
它是由可信社区成员创建和运营的代理控制平面：您注册一个**代理账户**，存入 GNK，生成 API 密钥，并提供 OpenAI 兼容端点。OpenBroker 为您运行允许列表的托管金和 devshard 生命周期——您无需将您的地址列入 `allowed_creator_addresses`。
...
请包含您的操作者名称和联系方式、您打算使用的 `gonka1...` 地址以及您计划提供的模型。是否列入列表由链上治理决定——任何单一操作者或组织均无权单方面添加地址——表达兴趣不保证会被审核、列入或有明确时间表。

---

**需要帮助？** 请参阅 [FAQ 页面](https://gonka.ai/FAQ/)，或加入 [Discord 服务器](https://discord.gg/REcpeYc7P7) 以获取技术问题或安全问题的支持。
