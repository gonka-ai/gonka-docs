...
本指南解释了如何通过社区代理向 Gonka 发送推理请求。这是今天开始使用网络的最快方式。如果您希望[运行自己的网关](#2-run-your-own-gateway-advanced)而非通过代理，请参阅本页底部的“运行自己的网关”。想自己**作为代理运行**？请参阅[有兴趣运行网关吗？](#3-interested-in-operating-a-gateway) —— 从 [OpenBroker](https://openbroker.gonka.gg) 开始，或在 OpenBroker 无法满足您的需求时使用 GitHub 白名单问题作为备选方案。

!!! note "如何连接到 Gonka"

    Gonka 推理现在围绕 **devshards** 组织——短暂的会话，持有小额链上存款（托管资金），并离线结算每次请求的费用。打开 devshard、签署请求、轮换会话以及向链提交结算的任务由一种称为 **网关** 的软件完成。

    对于大多数开发者而言，使用 Gonka 最简单的方式是调用**社区代理**——第三方通过网关提供推理访问，并暴露标准的 OpenAI 兼容 API。您只需从代理获取一个 API 密钥即可。
    ...
    | **抗审查性** | 访问由提供商集中控制。 | 访问正转向透明、协议治理的机制。当前生产环境的访问受到保护，同时协议级别的请求验证正在完善中。 |
    | **可审计性与透明度** | 日志记录、计费和使用跟踪由 API 提供商控制。 | 请求、计费和结算被设计为可签名、带时间戳且可审计。 |
    | **透明的代币经济** | 定价和资源分配由提供商定义。 | 网络的每次推理价格和结算由协议定义并链上进行，使底层推理经济可检查。 |

---

## 使用社区经纪人（推荐）

经纪人是独立运营者，运行Gonka网关并将推理服务转售给开发者。从您的应用程序角度来看，经纪人端点的行为与任何OpenAI兼容的API相同：您设置 `base_url`，传递 `Authorization: Bearer <API_KEY>` 标头，并像往常一样调用 `/v1/chat/completions`。

!!! caution 经纪人不是核心协议的一部分

    经纪人是独立的第三方。定价、支付方式（美元、加密货币、积分）、速率限制、支持的模型、服务等级协议、退款政策和数据处理均由每个经纪人自行决定。在上线前，请阅读经纪人的自有文档和条款。
...
??? note "关于此列表"
    这是一个精选的社区经纪人目录，它们通过公共Gonka网关路由推理，并同意被公开列出。该列表并非详尽无遗，也不对任何运营商进行背书。列表在每次页面加载时随机排序，因此每个经纪人的位置不代表排名；请根据各自的优势评估每个运营商。此目录反映的是早期启动集合。**想自己成为经纪人吗？** 使用 [OpenBroker](https://openbroker.gonka.gg)（参见 [§3](#3-interested-in-operating-a-gateway)），或在OpenBroker无法满足需求时，通过GitHub允许名单问题作为备选方案。部分经纪人提供 **▶ 演示** 链接，指向简短的入门录屏——风格和时长可能有所不同。

??? tip "比较经纪人——社区可观测性仪表板"

社区成员对公共代理端点运行独立的监控探针并发布结果。这些仪表板可帮助您在选择代理前比较正常运行时间、延迟和定价：
...
这些仪表板是**社区构建的工具，不属于核心协议的一部分**。数据准确性、方法论和可用性由每个仪表板运营商负责。请始终通过您自己的测试验证关键指标。每次页面加载时，列表均随机显示。
...
您需要从代理处获取三项内容（参见 [§1.2](#12-get-an-api-key)）：**基础 URL**、您的**API 密钥**和**模型 ID**。
...
Gonka 代理端点与 OpenAI 兼容，因此您可以直接使用官方 OpenAI SDK——无需使用 Gonka 特定客户端。
...
工具调用通过相同的 OpenAI 兼容端点支持。仅 `type: "function"` 受支持 —— Gonka 在底层使用 vLLM，它实现了 OpenAI 聊天补全规范，而非助手 API（`code_interpreter`、`file_search` 不可用）。
...
如果您需要自行操作链上 Gonka 网关（您自己的允许列表钱包、您自己的托管金、直接 GNK 结算），您可以在自己的机器或服务器上运行网关软件——绝不能在 Gonka 主机上运行。它暴露与代理相同的 OpenAI 兼容 API，但您拥有密钥，并直接在链上为它创建的 devshards 支付 GNK。

!!! tip "大多数操作者应通过 OpenBroker 成为代理"

    **[OpenBroker](https://openbroker.gonka.gg) 不仅仅是“目录中的另一个代理”。** 它是由受信任的社区成员创建和运营的基础设施，可让您**成为代理**——创建代理账户、充值 GNK、颁发 API 密钥并提供推理服务——**无需**链上允许列表的创建者地址，也无需运行您自己的网关。

    除非您确实需要自托管、允许列表的网关，否则请优先选择 [§3](#3-interested-in-operating-a-gateway)（OpenBroker）。

!!! warning "自托管网关需要允许列表地址"

    目前，只有链上 `devshard_escrow_params.allowed_creator_addresses` 列表中的 Gonka 账户才能打开 devshards。如果您的地址不在该列表中，您的网关无法创建会话，也无法发送推理请求。允许列表仅可通过链上治理投票更改。参见[有兴趣操作网关？](#3-interested-in-operating-a-gateway) —— 从 OpenBroker 开始，或在 OpenBroker 无法满足您的需求时，使用 GitHub 允许列表问题作为备选方案。
...
推理通过网关进入网络。如果您希望**成为代理/为自己或您的用户操作推理访问**，请选择以下路径。

### 推荐 - 通过 OpenBroker 成为代理

**[OpenBroker](https://openbroker.gonka.gg)** ([openbroker.gonka.gg](https://openbroker.gonka.gg)) 是**成为代理的推荐方式**。

它是由受信任的社区成员创建和运营的代理控制平面：您注册一个**代理账户**，存入 GNK，生成 API 密钥，并提供 OpenAI 兼容端点。OpenBroker 为您运行允许列表的托管金和 devshard 生命周期——您无需您的地址在 `allowed_creator_addresses` 列表中。
...
仅当 OpenBroker 无法满足您的需求时（例如您必须自行持有托管金密钥，或需要自定义链上设置），才运行您自己的 gonka-mainnet 网关。这要求您的地址在治理控制的允许列表中（`devshard_escrow_params.allowed_creator_addresses`）。完整说明请参见[网关指南](gateway-developer-quickstart.md)。
...
请包含您的操作者名称和联系方式、您计划使用的 `gonka1...` 地址，以及您计划提供的模型。加入允许列表是链上治理决策——任何单一操作者或组织均无权单方面添加地址——表达兴趣并不保证会被审核、纳入或提供时间表。
