...
交易通过一个 RPC 端点发送（此处称为 `<NODE_URL>/chain-rpc/`）。如果您未指定 `--node`，CLI 将默认使用 `tcp://localhost:26657`。除非您在本地运行自己的节点，否则请始终提供 `--node <NODE_URL>/chain-rpc/`。

此处支持并推荐使用无序交易，以避免序列竞争问题。（[docs.cosmos.network](https://docs.cosmos.network/sdk/v0.53/learn/beginner/tx-lifecycle)）
...
+ 新增功能）。
   ...
   - `--gas=2000000` → 手动设置 gas 上限（一个足够宽裕的固定值，适用于这些交易）。注意：当使用 `--gas-adjustment` 时，`--gas auto` 仅对估算值进行乘法运算；因此，若与固定值 `--gas` 一起使用，则 `--gas-adjustment` 将被忽略且不会增加额外缓冲。
 
   `--node <NODE_URL>/chain-rpc/` → 除非您运行本地 RPC 节点，否则为必需项。
...
当需要对网络进行影响链上状态的变更时，必须通过治理提案来实现，例如：
...
任何拥有有效治理密钥（冷账户）的用户都可以支付所需费用并创建治理提案。但每个提案仍需通过基于 PoC 加权投票的活跃参与者批准。
...
+ note "备注"
   ...
   **Gas 调优。** 如果模拟执行接近上限，或验证者设置了更高的最低 Gas 价格，请根据网络策略提高 `--gas-adjustment` 或设置 `--gas-prices`。([docs.cosmos.network](https://docs.cosmos.network/sdk/v0.53/learn/beginner/tx-lifecycle))
