...
总质押权重是所有满足 `tokens > 0` 条件的已质押验证者的 `validator.tokens` 之和：
...
对于创世守护者（Genesis Guardians），会首先应用一个额外的算力增强步骤（见下文）。
...
该列表可通过治理进行配置（`genesis_guardian_params.guardian_addresses`），但在引导阶段预计不会发生变化。
...
- 在增益生效的瞬间，三位守护者（Guardian）的总质押份额目标为总质押量的 `multiplier / (1
 + multiplier) = 0.52 / 1.52 ≈ 34%` —— 若共有三位守护者，则每位约为 `11.4%`（合计 `0.52 / (3 × 1.52)`）。
- 此 `≈34%` 是指增益运行期间公式所达到的水平，并非固定的稳态值。守护者的质押权重每纪元设定一次，因此随着网络其余部分的PoC权重增长，守护者总份额会逐渐低于34%。在撰写本文时，主网上实际运行的三位守护者合计质押比例约为 `27%`（每位约 `~9%`）—— 即目前低于 `>33%` 的否决线。请勿假定守护者持续拥有否决权。
- 设计初衷是使守护者接近否决强度（`>33%`），但低于通过强度（`>50%`）：足以阻止恶意提案，但永远不足以单独通过任何提案。
无法单独获取价值或单方面更改共识机制——任何操作均需守护者之间的协同配合。
...
- 高度阈值（`3,000,000`）已被跨越。
- 全网总算力仍远低于`15,000,000`，因此算力加成机制仍处于激活状态。
- 当全网累计的 PoC 权重首次超过`15,000,000`后的下一个纪元切换时，该加成机制将关闭；届时，守护者验证者的`tokens`将与其 PoC 权重相等，与其他所有验证者一致。

上述两个阈值均可通过治理机制进行调整（分别为`network_maturity_threshold`和`network_maturity_min_height`），因此如有需要，可通过成功的治理提案调整激活截止条件。

---

## 治理投票权限委托（冷钥至温钥）

如果持有投票权的密钥并非您日常操作所使用的密钥，则可提前授予治理投票权限。
...
note "您想参与投票，但无法访问持有投票权的密钥。"
    请联络该密钥的所有者，请求其授权您的密钥代表其进行投票。若无此授权，您的密钥将无法为该投票权提交治理投票。

### 由其他密钥代表您投票

请使用以下 `grant` 命令，从持有投票权的密钥发起操作。这将授权被授权密钥代表您提交治理投票。  
此项委托仅限用于治理提案投票，被授权方仍可为其自身密钥投票。授权方可随时撤销此权限。
...
=== "示例响应"
    ...
    `code: 0` 表示投票交易已被接受——这说明您的投票已成功提交。CLI 会返回带有 `txhash` 的广播回执；在交易被打包进区块之前，`height` 为 `0`。
