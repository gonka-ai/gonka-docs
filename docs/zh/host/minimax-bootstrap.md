...
`MiniMaxAI/MiniMax-M2.7`（FP8）已在 `v0.2.13` 升级中作为第三个经治理批准的推理模型加入。本文档说明如何在引导阶段尽可能降低权重被削减的风险，无论该模型在首次尝试中是否获得足够的参与者。

有关当前部署默认配置（包括 `node-config.json`），请参阅[主机快速入门](./quickstart.md)。如需了解多模型 PoC 机制的更广泛背景，请参阅[多模型 PoC](./multi_model_poc.md)。先前模型的引导流程及其机制记录在[Kimi K2.6 引导流程](./kimi-bootstrap.md)中。

!!! note
    引导过程可能持续多个周期（epoch），具体取决于准备就绪的参与者数量。在设定的惩罚周期开始前，只要参与者明确提交了选择，并且计划部署的主机提交了 `PoCIntent`，就不会发生权重削减。

## 时间线

对未参与 MiniMax-M2.7 的惩罚将从 **第 `278` 个周期** 开始。自升级激活后的每个周期，链都会尝试引导该模型：在该周期 PoC 阶段前 500 个区块（即 `DeployWindow`）内生成一个 `BootstrapDelegationSnapshot`，并根据以下条件评估预资格：
- 至少 `V_min = 3` 个直接提交者；
- 网络总权重中至少有 `W_threshold` 比例的部分通过 INTENT
 + DELEGATE 实现了 `>2/3` 的可达性。

若满足预资格条件，则在该周期启动 MiniMax 的 PoC。

当前 `W_threshold` 是一个治理参数——应从链上读取该值，而非硬编码（该值曾由 GIP-48 从 `0.3` 降至 `0.1`，未来可能再次调整）：
...
要计算任意给定评估周期的确切区块编号，应以链的当前周期为锚点进行向前推算。`epoch_shift` 参数不应锚定到创世块（因为它在经历过去周期长度变更后会过时），因此在主网上使用 `epoch_shift
 + N
 * epoch_length` 是错误的——始终应以实时的当前 PoC_start 为锚点：
...
当参与的节点及其委托量在最早的一个epoch中达到门槛时，MiniMax即获得预资格（pre-eligible）。
...
1. **MiniMax 在某个 epoch 的快照中未通过预评估**（并在 PoC 阶段仍不具备资格）：
    ...
    - **在 epoch `278` 之前**：所有未提交任何操作的参与者也保留其全部权重（宽限期内不施加惩罚）
 
    - **从 epoch `278` 开始**：所有未提交任何操作的参与者，将因每轮错过的模型而被扣除 15% 的权重

   => 因此，在 **epoch `278` 之前** 明确发送一笔交易以表明您的意图至关重要

2. **MiniMax 通过预评估，但在 PoC 阶段仍未获得资格**（例如，某个 INTENT 节点未能及时部署）：

 
    - 实际部署了 MiniMax-M2.7 并在该 epoch 提交了 MiniMax PoC 提交记录的节点，保留其现有模型组的全部权重（不受惩罚）
 
    - 所有提交了 `PoCDelegation` / `PoCRefusal` 的参与者保留其全部权重（不受惩罚）
 
    - **从 epoch `278` 开始**：所有未提交任何操作的参与者将被扣除 15% 的权重；而所有提交了 MiniMax 的 `PoCIntent` 但未完成部署并提交 MiniMax PoC 提交记录的参与者，也将被扣除 15% 权重（按 `IntentMissed` 处理）

如果 MiniMax 通过了上述两项检查，则惩罚机制将遵循 [多模型 PoC](./multi_model_poc.md) 中描述的常规情形。

## 硬件资格要求

MiniMax-M2.7（FP8）每个实例**大约需要 320 GB 的总 VRAM** —— 相较于 Kimi K2.6 或 Qwen3-235B（根据[节点快速入门参考配置](./quickstart.md#hardware-and-machines)均需 ≥640 GB 每实例）显著降低了资源需求。实际影响如下：

- **A100 80GB 拥有者**：MiniMax-M2.7 是**首个经治理批准、可适配 A100 80GB 配置的模型**。如果您此前无法运行 Kimi 或 Qwen-235B，现在可通过 MiniMax 获得共识权重。推荐配置：8×A100 80GB，使用 `tp=4`（每台主机运行两个实例）或 `tp=8`（每台主机运行一个实例）。
- **H100 / H200 拥有者**：MiniMax-M2.7 在共识输出方面与 Qwen3-235B 相当（根据工作负载略有上下几个百分点），且在 `v0.2.13` 版本对 Kimi 系数调整后，明显优于 Kimi K2.6。建议从 Kimi 切换至 MiniMax；从 Qwen 切换至 MiniMax 则视运营商偏好决定。
- **B200 / B300 拥有者**：MiniMax-M2.7 运行表现良好，但在旗舰硬件上 Kimi K2.6 仍保持微弱的共识输出领先。若已运行 Kimi，则无需变更。

## 计划部署 MiniMax-M2.7 的节点操作指南

#### 1. 向链上发送 `PoCIntent` 交易
...
MiniMax-M2.7 FP8 权重文件大小约为 **230 GB**，请合理规划磁盘空间和网络带宽。请参考以下指南，使用下方提供的仓库和提交哈希预下载模型权重：[预下载模型权重](https://gonka.ai/host/quickstart/#server-pre-download-model-weights-to-hugging-face-cache-hf_home)：

- `hf_repo`: `MiniMaxAI/MiniMax-M2.7`
- `hf_commit`: `d494266a4affc0d2995ba1fa35c8481cbd84294b`

在引导快照区块之前，请先验证模型能否在您的硬件上成功加载。链上将使用 `Model.ModelArgs` 注册 MiniMax 模型：
...
关键属性是 `pre_eligible`。如果其值为 `true`，则表示该链将在本 epoch 运行 MiniMax PoC，你应做好部署准备。辅助字段显示了通过了哪三项检查：`meets_v_min`（≥ `V_min` 个直接表达意向的提交者）、`meets_weight_threshold`（意向权重 ≥ `total_network_weight` 的 `W_threshold`）以及 `meets_reachability`（意向加上被委托的 `reachable_voting_power` 覆盖了 `>2/3`）。`intent_host_count` 和 `intent_weight` 显示了本 epoch 的直接意向覆盖情况。

#### 4. 如果预符合条件，则切换模型至 MiniMax-M2.7
...
对于 4×B200 / 8×B200 的部署，请根据吞吐量偏好选择使用 `--tensor-parallel-size 2`（每台 8×B200 服务器运行两个实例）或 `--tensor-parallel-size 4`（每台运行一个实例）。`Model.ModelArgs` 配置应保持精简；部署时的参数选项（如 `--tensor-parallel-size`、`--gpu-memory-utilization`、`--max-num-seqs` 等）由运维人员自行决定。

#### 5. 验证您的部署

[`gonka` 仓库](https://github.com/gonka-ai/gonka) 提供了一个名为 `mlnode-validate` 的代理技能，可用于将已部署的 ML Node 与特定模型的预计算诚实 PoC 向量进行比对验证。针对 MiniMax M2.7 模型，仓库中提交的黄金参考数据为 `mlnode/packages/benchmarks/scripts/poc_validation/artifacts/minimaxai-minimax-m2.7.json`（共 200 个向量，记录于 2×H200 环境）。此外，还提供了适用于 `4×A100`、`4×H100`、`2×H200` 和 `2×B200` 的现成 `deploy/join/` 配置文件。详见 [ML Node 部署验证指南](./mlnode-validation.md) 和 [`skills/mlnode-validate/SKILL.md`](https://github.com/gonka-ai/gonka/blob/main/skills/mlnode-validate/SKILL.md)。

## 不打算部署 MiniMax-M2.7 的主机操作说明

#### 1. 检查您是否信任其他已部署 MiniMax 或已发送 `PoCIntent` 的主机
