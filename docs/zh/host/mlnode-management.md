...
如果您从另一台机器调用 Admin API，请将 `localhost` 替换为您的网络节点的私有 IP 或主机名（确保端口 `9200` 可访问并已正确配置防火墙）。
...
!!! note "支持的模型和 vLLM 参数"
    使用 [主机快速入门](./quickstart.md) 中匹配的 `node-config-*.json`。根据 `GET /v1/governance/models` 确认模型 ID。可用性和实时服务与此目录无关。`moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8` 在 [提案 101](../network-updates.md#proposal-101) 后不再是 PoC 模型。参见 [选择 LLM 最优部署配置的基准指南](https://gonka.ai/host/benchmark-to-choose-optimal-deployment-config-for-llms/)。
...
成功时，以 JSON 格式返回带有新注册的 ML 节点配置的 `200 OK`。
如果一个或多个模型无效（未通过治理审批），API 将返回 400 Bad Request 及错误消息。
...
- 如果**所有**节点验证并注册成功：
  - 返回带有已注册节点数组的 `201 Created`。
- 如果**部分**节点验证失败：
  - 返回带有 `nodes`（成功节点）和描述失败的 `errors` 数组的 `206 Partial Content`。
- 如果**所有**节点验证失败：
  - 返回带有 `errors` 数组中详细信息的 `400 Bad Request`。

---

## 更新现有 ML 节点

更新 ML 节点以**upsert**方式实现：
...
!!! note "保持路径和请求体中的 ID 一致"
    为清晰起见并避免混淆，使用 `PUT` 时，请始终将请求体中的 `id` 设置为与 URL 中的 `:id` 一致。
...
- 成功时，返回带有更新后节点配置的 `200 OK`。
- 如果节点无法更新（例如，模型未获治理允许），则返回带有错误消息的 `400 Bad Request`。

---

## 启用 ML 节点

使用此端点**启用**之前被禁用的 ML 节点。此操作不会更改节点的配置，仅更改其管理状态。
...
- 如果节点不存在，则返回带有错误消息的 `404 Not Found`。
...
在协议级别进行端到端验证（计算证明后），您还可以检查当前活跃参与者的列表：
