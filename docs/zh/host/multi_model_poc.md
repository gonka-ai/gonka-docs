...
| `moonshotai/Kimi-K2.6` | 在纪元 328–329 事件后重新 bootstrap 中 |

各模型的 `weight_scale_factor` 与 `penalty_start_epoch` 通过治理变更过于频繁，无法在文档中可靠列出。请始终通过你所使用的链上的实时 `params` 查询读取这些参数：
