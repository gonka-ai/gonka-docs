...
## 先决条件
本节提供有关配置硬件基础设施以参与Gonka网络启动的指导。目标是通过使您的部署符合网络期望来最大化协议奖励。
...
| `moonshotai/Kimi-K2.6` | **Kimi K2.6** — 治理中 / `poc_params`（自v0.2.12起）。目前未提供服务。 |
...
!!! tip "权威模型列表（治理API）"
    ...
    ```
    要仅列出模型ID，请管道输出响应：`jq -r '.models[].id'`。如果`node2.gonka.ai`无法访问，请使用其他参与者的公共API基础URL（协议、主机和端口）。响应还包括网络参数，如`model_args`；后面的`node-config.json`示例展示了常见硬件的典型`args`——请根据您的GPU和基准测试进行调整。

您通常在`node-config.json`中为每个ML节点运行**一个模型**。

治理列表（`GET /v1/governance/models`，`poc_params.models`）是每个模型的批准目录。模型的`weight_scale_factor`只有在该组有资格时才产生共识权重：满足`v_min`和`w_threshold`的投票权。仅对有资格的组适用错过/拒绝处罚。无投票权的列出模型是下一个PoC的启动候选者——仅切换`node-config`无法恢复共识权重。请检查`poc_params`和该轮次的`confirmation_weight_scales`。参见[多模型PoC](./multi_model_poc.md)。

实时服务是单独的检查：`/v1/epochs/current/participants`。`moonshotai/Kimi-K2.6`目前未提供服务。要恢复该组，请参阅[Kimi K2.6启动](./kimi-bootstrap.md)。

!!! note "如果您不运行每个批准的模型"
    多模型PoC按**每个模型**跟踪参与情况。对于您不运行的**有资格**模型，请使用链上**委托**或**拒绝**。仅对有资格的组适用常规的错过/拒绝处罚。无投票权的列出模型是启动候选者；`refuse`不是必需的，也不是启动参与模式。委托/拒绝**不是**上线节点所必需的——您使用与[向ML操作密钥授予权限](#33-local-machine-grant-permissions-to-ml-operational-key)中相同的**账户（冷）密钥**，**在注册和验证之后**。复制粘贴命令在末尾：[可选：PoC委托和拒绝](#optional-poc-delegation-and-refusal)。有关策略和处罚，请阅读[多模型PoC — 主机操作指南](./multi_model_poc.md)。要恢复失去投票权的组，请参阅[Kimi K2.6启动](./kimi-bootstrap.md)。

!!! note "治理与模型分类"
    - 如果获得治理批准，模型可被归类为某一类别。
    - 关于添加或更改支持模型的决定由治理作出。
...
| `moonshotai/Kimi-K2.6` | ≥ 2 | 8× H200 或 8× B200 每个 MLNode（参考配置） | 720 GB |

这是一个参考架构。您可以调整节点数量或硬件分配，但我们建议遵循核心原则：每个节点应支持所有模型层级的多个 ML 节点。

每个模型的 `weight_scale_factor` 值由治理设定——从 `poc_params` 中读取。系数仅在模型组有资格（具有投票权）时产生共识权重。参见 [DeepSeek V4 Flash Bootstrap](./deepseek-bootstrap.md) 和 [系数表](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)。下方 `node-config.json` 示例中包含 vLLM 参数示例。
...
- 已安装并配置 NVIDIA Container Toolkit，CUDA Toolkit 版本在 12.6 至 12.9 之间。您可通过 `nvidia-smi` 检查版本。
...
如果这些端口中任何一个暴露于公共互联网，您的节点将面临风险。第三方可自由发送请求，过载您的 ML 节点，干扰挖矿，或导致您的节点退出一个纪元。
...
=== "情况 2：ML 节点和网络节点位于不同机器上"
    在此设置中，网络节点与 ML 节点之间的所有通信必须通过私有网络进行。严禁使用公共 IP 或公共 DNS 名称进行：

    - ML 节点 API
    - `DAPI_API__POC_CALLBACK_URL`

如果 ML 节点和网络节点容器位于不同机器上，情况 1 中描述的修复方法将无效，保护这些端口的具体方式取决于您的设置。您应通过使用相同的 Docker 网络，或在机器之间设置私有网络、在该网络中暴露端口并关闭公共端口，来建立 ML 节点与网络容器之间的连接。在此情况下，您还应在配置中正确设置 `DAPI_API__POC_CALLBACK_URL` 变量。此 URL 必须指向私有/内部地址，而非公共地址。

## 设置您的节点

快速入门说明旨在将网络节点和推理节点部署在同一台机器上（单服务器设置）。
...
### [本地机器] 安装 CLI 工具
`inferenced` CLI 是本地账户管理和网络操作所必需的。它是一个命令行界面工具，允许您在本地机器上创建和管理 Gonka 账户、注册主机并执行各种网络操作。
...
请确保您使用的是 [版本 0.2.9](https://github.com/gonka-ai/gonka/releases/tag/release/v0.2.9) 或 [更高版本](https://github.com/gonka-ai/gonka/releases) 的 `inferenced` CLI 构建。旧版 CLI 不支持权限授予，可能导致意外行为。

如果您计划提交治理提案，特别是使用新消息类型的提案，请使用最新发布的特定操作系统 CLI 构建。
...
!!! note "macOS 用户"
    在 macOS 上，如果被提示，您可能需要在 `System Settings` → `Privacy & Security` 中允许执行。向下滚动到关于 `inferenced` 的警告并点击 `Allow Anyway`。

    如果二进制文件在 Linux 上启动失败并出现类似 `Error relocating ./inferenced: qsort_r: symbol not found` 的错误，您很可能下载了非 CLI 或仅限升级的工件，而不是特定于操作系统的打包 CLI 构建。请重新下载适用于您的操作系统和架构的正确归档文件。
...
**至关重要**：请将此助记词抄写下来并安全地离线存储。此助记词是**唯一**恢复您的账户密钥的方法。
...
!!! warning "没有域名则无法使用 HTTPS"
    ...
    1. 获取一个域名，并配置 DNS 以指向您服务器的 IP 地址
    2. 点击上方的 **"重置"** 按钮，并在被问及是否拥有域名时选择 **"是"**
...
<div id="quickstart-config-result" style="display: none;">
    ...
    <p style="margin-top: 1rem; font-size: 0.7rem; color: var(--md-default-fg-color--light);">复制上述配置，并按照下面的说明编辑值。</p>
...
如果您的节点无法连接到默认的种子节点，请[参见常见问题解答获取详情。](https://gonka.ai/FAQ/#my-node-cannot-connect-to-the-default-seed-node-specified-in-the-configenv)
### [服务器] 编辑环境变量
...
1) 转到 Microsoft Entra ID → 应用注册 → 新注册。复制应用（客户端）ID 和目录（租户）ID。

2) 转到证书与密钥 → 新客户端密钥。复制密钥值并设置 `AZURE_CLIENT_SECRET`。
...
!!! note "使用环境变量"
    以下各节中的示例将在本地机器命令和服务器命令中引用这些环境变量（例如 `$PUBLIC_URL`、`$ACCOUNT_PUBKEY`、`$SEED_API_URL`）。请确保在每个将执行这些命令的终端会话中运行 `source config.env`。
    <!-- CONDITION END -->
...
!!! note 
    选择与您将要部署的模型和 GPU 类型匹配的标签。确认模型位于 `GET /v1/governance/models`。资格（投票权，`confirmation_weight_scales`）和实时服务（`/v1/epochs/current/participants`）与此目录无关。`moonshotai/Kimi-K2.6` 目前未提供服务。模型的添加或修改由治理机制决定。有关模型治理工作原理及如何提议新模型的详细信息，请参阅 [交易与治理指南](https://gonka.ai/governance/transactions-and-governance/)。

    === "Kimi — 4×B200 / 8×B200（以及 8×H200 参考类别）"

    !!! warning "目前未提供服务"
        `/v1/epochs/current/participants` 中没有主机列出 `moonshotai/Kimi-K2.6`。这些参数是最后已知的部署形态。

        资格是独立的、全网规则：`poc_params` 中的模型不会自动获得资格。资格需要满足 `v_min` / `w_threshold` 的投票权。如果没有，下一个 PoC 会将该模型视为启动候选者——单主机 `node-config` 切换不会恢复共识权重。请参阅 [Kimi K2.6 启动](./kimi-bootstrap.md)。

        对于 Blackwell 上的 **Kimi K2.6**，使用此 vLLM 参数集进行 **4×B200 或 8×B200** 部署，并作为 **8×H200** 在相同布局（`tensor_parallel_size` 4，跨八个 GPU 使用专家并行）下的参考。仅当您的堆栈或基准测试需要时才进行调整。
    ...
    在通过 API 注册或更新节点时，使用相同的 `"models"` 块；有关等效的 `curl` 示例，请参阅 [Kimi K2.6 启动](./kimi-bootstrap.md)。
...
使用此 vLLM 参数集部署 **MiniMax M2.7** 在 **4×A100** 上。A100 无法使用 FP8 FlashInfer MoE 路径，因此此配置使用 `marlin` MoE 后端。您还需要为 `mlnode-308` 服务设置环境变量 `VLLM_USE_FLASHINFER_MOE_FP8=0`（MLNode 3.0.14 自带的 `deploy/join/docker-compose.mlnode.yml` 中已预设）。
...
使用此 vLLM 参数集部署 **MiniMax M2.7** 在 **4×H100** 上。使用 `FLASHINFER` 注意力后端和 FP8 kv-cache。
...
使用此 vLLM 参数集部署 **MiniMax M2.7** 在 **2×H200** 上（MiniMax 的 Hopper 参考类别）。使用 `FLASHINFER` 注意力后端、FP8 kv-cache 和 `tensor_parallel_size=2`。MiniMax M2.7 的 PoC 黄金向量在此确切配置下记录。
...
使用此 vLLM 参数集部署 **MiniMax M2.7** 在 **2×B200** 上（MiniMax 的 Blackwell 参考类别）。使用 `FLASHINFER_TRTLLM` MoE 后端、FP8 kv-cache 和 `tensor_parallel_size=2`。
...
使用此 vLLM 参数集部署 **DeepSeek V4 Flash** 在 **4×H100** 上。DeepSeek 要求 **MLNode 3.0.16**（vLLM 0.25.1）。
...
使用此 vLLM 参数集部署 **DeepSeek V4 Flash** 在 **2×H200** 上。DeepSeek 要求 **MLNode 3.0.16**（vLLM 0.25.1）。
...
使用此 vLLM 参数集部署 **DeepSeek V4 Flash** 在 **2×B200** 上。DeepSeek 要求 **MLNode 3.0.16**（vLLM 0.25.1）。为在 Blackwell 上获得更好性能，请使用 nvfp4 变体 `node-config-deepseekv4flash0731-B200-nvfp4.json`（`model_override` 到 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`）；该格式需要 API `v0.2.15-post5` —— 请参见 [网络更新](../network-updates.md)。
...
使用此 vLLM 参数集部署 **DeepSeek V4 Flash** 在 **1×B300** 上。DeepSeek 要求 **MLNode 3.0.16**（vLLM 0.25.1）。在 B300 上，这是当前系数（0.246）下权重最高的 PoC 选项。为获得更好性能，请使用 nvfp4 变体 `node-config-deepseekv4flash0731-B300-nvfp4.json`（`model_override` 到 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`）；该格式需要 API `v0.2.15-post5` —— 请参见 [网络更新](../network-updates.md)。
...
!!! tip "Validate the deployment"
    [`gonka` 仓库](https://github.com/gonka-ai/gonka) 提供了一个代理技能 `mlnode-validate`，用于根据预计算的诚实 PoC 向量验证 ML Node。使用与您部署的模型匹配的黄金参考（MiniMax-M2.7、DeepSeek V4 Flash 在 [`vllm-0.25.1-upgrade`](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/mlnode/packages/benchmarks/scripts/poc_validation/artifacts) 或 Kimi K2.6）。Qwen3-0.6B / Qwen3-235B 工件为历史版本，**非**主网模型。请参见 [验证 ML Node 部署](./mlnode-validation.md)。
...
`moonshotai/Kimi-K2.6` 目前未提供服务。如需部署（包括恢复组），请下载它。列出但无投票权的模型为引导候选；单个 `node-config` 开关无法恢复共识权重。请参见 [Kimi K2.6 引导](./kimi-bootstrap.md)。
    ...
    模型许可证：参见 [模型许可证](../model-licenses.md)。有关运营说明和链上选项（意图、委托），请参见 [Kimi K2.6 引导](./kimi-bootstrap.md)。
...
=== "MiniMax M2.7"
    ...
    模型许可：参见 [模型许可](../model-licenses.md)。MiniMax M2.7 需要 **MLNode 3.0.14 或更高版本**（镜像 `ghcr.io/gonka-ai/mlnode:3.0.14-cu129`，固定在 `deploy/join/docker-compose.mlnode.yml`）。在 A100 硬件上，请确保为 `mlnode-308` 服务设置了 `VLLM_USE_FLASHINFER_MOE_FP8=0` 环境变量（已预设在随附的 compose 文件中）。
...
=== "DeepSeek V4 Flash"
    ...
    模型许可：参见 [模型许可](../model-licenses.md)。DeepSeek V4 Flash 需要 **MLNode 3.0.16 或更高版本**（镜像 `ghcr.io/gonka-ai/mlnode:3.0.16`，固定在 `deploy/join/docker-compose.mlnode.yml` 分支的 `vllm-0.25.1-upgrade` 上；如果主机仍使用 CUDA 12.9，请使用 `3.0.16-cu129` 标签）。有关操作说明和链上选项（意图、委托），请参阅 [DeepSeek V4 Flash 启动指南](./deepseek-bootstrap.md)。在 Blackwell GPU 上，nvfp4 重新打包 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4` 需要 API `v0.2.15-post5` —— 请参阅 [网络更新](../network-updates.md)。
...
- **`api`** - 此阶段故意排除，因为我们需要在下一步中在其内部创建 ML 操作密钥
...
在 [步骤 3.2.](https://gonka.ai/host/quickstart/#32-server-register-host)（`inferenced register-new-participant`）的注册命令期间，共识密钥将与链上的账户密钥（冷密钥）关联，使您的节点成为网络中的有效参与者。

如果您删除或覆盖 `.tmkms` 文件夹，您的共识密钥将丢失。此密钥是将您的节点与区块链验证者集合关联的关键。一旦 `.tmkms` 被删除，您必须从头开始整个设置，包括重新生成共识密钥（通过 `tmkms`）（参见 FAQ 页面中的 “[我清除了或覆盖了我的共识密钥](https://gonka.ai/FAQ/#i-cleared-or-overwrote-my-consensus-key)”）。
...
使用 `file` 密钥环后端在 `api` 容器内创建温密钥（程序访问必需）。密钥将存储在映射到容器 `/root/.inference` 的持久卷中：
```bash
...
!!! note "重要"
    请勿重复运行此命令。
    ML 操作密钥（温密钥）每个服务器仅生成一次，必须在重启后保留。
...
从同一容器中注册主机——这会将您的 URL、账户密钥和共识密钥（自动获取）链上绑定：
...
**步骤 1.** 仍在第 3.1 步的容器中时，获取您的共识密钥并记录下来：
    ```bash
...
该命令会提示 `confirm transaction before signing and broadcasting [y/N]:` — 输入 `y` 继续。

燃料费从账户密钥的余额中支付。运行此命令前请确保账户有代币。
...
从您的账户密钥向 ML 运营密钥授予权限。这还会创建一个默认支出限额为 **10 GNK** 的 **feegrant**（冷账户 → 热账户），以便 API 可以从冷账户支付有费用的交易。如果该额度后来被耗尽，您无法直接补充 — 必须撤销并重新授予；请参阅 [刷新 feegrant](#refresh-the-feegrant-when-the-10-gnk-allowance-is-used-up)。
```bash
...
!!! note "DNS 挑战"
    Certbot 将暂停并显示您在提供商处需要添加的 **TXT DNS** 记录。验证后，`cert.pem` 和 `private.key` 将出现在 `./secrets/nginx-ssl/` 中。
...
启用 `--profile "ssl"` 标志将启动 `proxy-ssl` 容器，该容器自动管理SSL证书。
<!-- CONDITION END -->
...
您应该在JSON中看到参与者数据（`participant.address`、`participant.inferenceUrl`、`participant.status`）。
...
一旦您的节点在仪表板中可见，您可能还想更新您的公开资料（主机名、网站、头像）。这有助于网络中的其他参与者识别您的节点。您可以在[此处找到说明](https://gonka.ai/host/validator_info/)。

## 为您的账户密钥充值（交易费用） {#fund-account-fees}

**重要：费用由您的冷钱包支付。请在安全的本地机器上执行转账和收费交易。**

从升级 **v0.2.16** 开始，Gonka对一组指定的主机交易收取链上费用。仅注册不足以加入计算证明或一个周期：您的冷账户必须持有足够的流动GNK余额，以便这些消息能被包含在链上。

在下一个PoC窗口前为您的账户密钥地址充值。此余额应与[§5 存入抵押品](#5-local-machine-deposit-collateral)中锁定的抵押品分开——费用从可用余额中扣除；抵押品则存入抵押品模块。
...
| `MsgDeclarePoCIntent` | 在启动窗口期间声明模型意向 |
...
其他协议职责流量（例如网络视为职责的常规推理和验证路径）不在此费用范围内。始终使用 `ngonka` 币种进行余额和费用操作。
...
向该地址发送GNK。建议初始充值**10 GNK**到冷账户——足以覆盖数个周期的操作费用以及您从冷钱包签署的一次性设置交易（权限授权、委托、抵押品存入）。另外，[§3.3 授予权限](#33-local-machine-grant-permissions-to-ml-operational-key)会创建一个**feegrant**，使ML操作密钥可以从冷钱包中支出费用，默认**支出限额为10 GNK**。该限额是feegrant支出的上限，不会因发送更多GNK而自动补充。抵押品是独立的；请参阅[§5 存入抵押品](#5-local-machine-deposit-collateral)。
...
当您的节点运行且**API**容器已启动（[§4 启动完整节点](#4-server-launch-full-node)）后，在**服务器**上运行此命令。管理API仅监听localhost——不会通过公共代理暴露。
...
- **`budget_balance`** — 仅估算**此周期**所需的**ngonka**（主要PoC、可能的确认PoC和硬件差异费用）。这是一个包含余量的上限值，而非精确账单。
- **`spendable_balance`** — 节点当前实际可用于支付费用的金额：冷账户的流动性余额，**受剩余费用授权限额的限制**。
- **`spendable_covers_budget: true`** — 该受限金额足以覆盖本周期的预估费用。
- **`spendable_covers_budget: false`** — 费用余量不足。请检查两方面：
  - 如果冷账户余额较低 → 向冷账户发送更多 GNK。
  - 如果冷账户仍有资金但费用授权限额已耗尽 → 撤销并重新授权（见下文）。仅发送 GNK **不会** 提高费用授权限额。
- **`count_source: "top_participant"`** — 该预估基于上一个 PoC 阶段中最繁忙参与者的存储提交数量。为保守起见，可手动传入自己的提交数量：

```bash
curl -s "http://127.0.0.1:9200/admin/v1/epoch-fee-budget?count=100" | jq
```

此检查**不包括**上表中的抵押品或一次性设置消息——仅涵盖当前周期的重复性 PoC/硬件流量。

### 规划多个周期

该端点一次仅估算**一个周期**。费用在交易确认时支出；未使用的 GNK 仍保留在您的冷账户中，未使用的费用授权余量也保留在授权额度内，直至耗尽。

- **每个周期重新检查** —— 在 PoC 前后，若网络活动发生变化，请再次运行该命令。
- **提前规划** —— 将 `budget_balance` 乘以您希望覆盖的周期数（例如，`budget_balance × 90` 约等于主网上三个月，每个周期约 24 小时）。确保冷账户中保留足够的流动性 GNK，**以及**足够的剩余费用授权限额（默认每个授权为 **10 GNK**）。
- **当 `spendable_covers_budget` 为 false 时** —— 如果余额不足，请补充冷账户；如果费用授权限额已用完，请刷新授权（下一节）。没有“补充费用授权”的交易。

### 当 10 GNK 授权额度用尽时刷新费用授权

`grant-ml-ops-permissions` 创建一个**费用授权**，使 ML 操作密钥（热）能够从您的账户密钥（冷）支付费用，默认支出限额为 **10 GNK**。Cosmos 费用授权没有更新或补充消息：当该授权额度耗尽（或过期）时，即使冷账户仍有余额，API 也无法再提交带费用的交易。您必须**撤销**旧授权，然后**重新授权**以创建新的 10 GNK 限额。

1. 确保冷账户仍持有足够的流动性 GNK（如需请发送更多）。
2. 在您的**本地机器**上，撤销旧授权（`[granter] [grantee]` —— 冷，然后热）。在运行授权前，请等待撤销交易被确认上链。
...
如果已存在授权，`grant-ml-ops-permissions` 将跳过创建新费用授权，并提示您先撤销。成功重新授权后，请使用 `epoch-fee-budget` 重新检查，直到 `spendable_covers_budget` 为 `true`。

## [本地机器] 存入抵押品

**重要：请在您创建账户密钥的安全本地机器上执行此步骤。**

抵押品是锁定的 GNK，用于激活您 PoC 权重中符合抵押资格的部分。若无抵押品，主机仅能获得**基础权重**（默认为 20%），无法获得完整计算证明收益。宽限期已结束，因此必须完成此步骤才能以全权重运行。

> **关于时间的说明**：验证节点状态仅确认您的容器正在运行且参与者已注册。这**不**意味着计算证明已成功——PoC 每约 24 小时运行一次，只有在完成之后，您才能在 `$NODE_URL/v1/epochs/current/participants` 查看实际权重。以下两种选项允许您现在使用预估值存入抵押品，或等待首次 PoC 完成后使用精确数据存入。

无法提前知晓您的 PoC 权重——它由您的硬件、网络当前规模和每模型系数决定。

**选项 A — 现在存入（从周期 1 开始全权重）。** 查看网络当前的权重分布，并存入足够覆盖上限的金额。您的节点在首次 PoC 时即已配备抵押品。
...
> **为什么是 2× 缓冲？** PoC 权重在各纪元之间波动（网络归一化、模型系数、上限、惩罚）。协议**不会**自动补充：如果你的抵押在下一个纪元边界时不足以覆盖实际权重，你将无声地获得较少权重，直到存入更多抵押品——至少损失一个纪元的全额奖励。超额抵押不会丢失：它会存留在模块中，稍后可通过 `withdraw-collateral` 提取。

**选项 B — 等待首次 PoC，然后精确存入（损失一个纪元的 20% 权重）。** 现在跳过此步骤，等待你的首个 PoC 阶段完成（约每 24 小时一次），然后在 `$NODE_URL/v1/epochs/current/participants` 检查你的实际权重，并使用你的实际权重替换上述脚本中的 `MAX_WEIGHT` 重新运行。从第二个纪元起，你的节点将满载运行。
...
存款是累计的——若你的权重增长，可稍后通过另一次 `deposit-collateral` 补充。要释放未使用的抵押品，请使用 `withdraw-collateral`（需经过解绑期，默认为 1 个纪元）。

有关 slashing、提款和参数调优的详细信息，请参阅 [抵押品文档](https://gonka.ai/host/collateral/)。

## 可选：PoC 委托与拒绝 {#optional-poc-delegation-and-refusal}

在你的主机已注册、ML 运营密钥已授权，并且你可以[验证](#verify-node-status)参与状态后使用本节——通常在你的**本地机器**上使用**账户（冷）密钥**（`gonka-account-key`）进行操作。此处操作并非启动容器所必需；当你**不**在自己的 GPU 上运行所有治理批准的模型，而必须**委托** PoC 投票给其他参与者、**拒绝**委托或与 `params` 对比时间时适用。

对于每个 `model_id`，你要么运行该模型（PoC 由你的系统提交），要么在链上发出信号。**委托**是当你信任运行该模型的其他主机时的常见选择；**拒绝**是明确的退出选项。背景：[多模型 PoC —— 主机操作指南](./multi_model_poc.md)。
...
**委托**——将你对该模型 PoC 验证的权重附加到 `DELEGATEE`（他们的 `gonka1…` 地址）。以 MiniMax 为例：
...
以 DeepSeek 为例（例如你仅在 GPU 上运行 MiniMax）：
...
**拒绝**某个模型的委托（链上明确的“否”）。对你不运行的**合格**模型使用此选项（例如，若你仅提供 MiniMax，则拒绝 DeepSeek）。常规拒绝/遗漏惩罚仅适用于合格组。
...
`declare-poc-intent` 主要适用于**新模型引导**窗口；请参阅 [Kimi K2.6 引导](./kimi-bootstrap.md) 和 [DeepSeek V4 Flash 引导](./deepseek-bootstrap.md)。更多命令和边缘情况：[多模型 PoC —— 主机操作指南](./multi_model_poc.md#copy-paste-setup-commands)。

## 停止并清理你的节点

### 如何停止你的节点

检查您当前所处的纪元。打开 URL：[http://node1.gonka.ai:8000/api/v1/epochs/latest](http://node1.gonka.ai:8000/api/v1/epochs/latest)（您也可以使用任何其他活跃参与者的 URL）。
...
```
此区块号表示您可以在该区块之后领取奖励。但请注意，您现在就应该开始禁用每个 ML 节点（无需等待此区块到来再禁用 ML 节点）。
...
```
等待下一个纪元。请勿停止网络节点或 ML 节点。禁用标志仅在下一个纪元开始后生效。
...
验证移除和权重。如果您禁用了所有节点，则您的参与者应不再出现在活跃参与者列表中。如果您仍能看到您的参与者在列表中，这意味着网络仍期望您参与当前周期，如果您继续禁用节点，可能会错过推理，从而影响您的声誉。

确保您位于 `gonka/deploy/join` 文件夹中。要停止所有正在运行的容器：
```
...
1. 要清理缓存并从头开始，请删除本地 `.inference` 和 `.dapi` 文件夹（推理运行时缓存和身份）：
```bash
...
!!! note 
    ...
    **需要帮助？** 请访问 [常见问题页面](https://gonka.ai/FAQ/) 获取答案，或加入 [Discord 服务器](https://discord.gg/REcpeYc7P7) 获取有关一般问题、技术问题或安全问题的帮助。  
