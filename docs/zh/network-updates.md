...
!!! note "关于此页面"
    ...
    此页面不保证内容完整。如需获取最新信息，包括治理投票的启动及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。
...
该提案还向七名受益人发放总计 91,300 USDT 的贡献者奖金：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重达到 25% 的最低投票率、多数赞成票，且反对票（含否决）低于 33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参考有关将治理投票权限从冷密钥授予热密钥的指南。
...
提案获得批准。从社区资金池中拨款15,500 USDT，用于报销Daniil和David Liberman（Gonka协议的联合创建者）参加All-In Summit 2026的两张与会者通行证费用。

峰会于2026年9月13日至15日在洛杉矶举行。标准与会者通行证每人7,500 USDT，门票已预先支付。本次报销涵盖两张门票及提取资金的交易费用。本提案未申请从社区资金池报销机票、住宿及其他费用。
...
提案获得批准。Quant Mesh Limited被正式批准为Gonka-Ledger集成的法律签约方，并从社区资金池拨款350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费75,000 USDT以及350 USDT的处理费用。授权和资金拨付严格限于完整提案中描述的Ledger集成。
...
提案获得批准。`zai-org/GLM-5.3-Flash`现已成为经治理批准的模型并纳入PoC模型集，而`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`已从`poc_params.models`中移除。
...
GLM-5.3-Flash的激活纪元为**394**。从该纪元起，`penalty_start_epoch`生效，因此未明确选择的主办方将面临不参与罚金。如您计划提供该模型，尽早声明意向将最大程度帮助团队形成。
...
提案已获通过，`v4.1`已被添加至批准的devshard版本列表中。`versiond`将下载二进制文件，验证其SHA-256值是否与链上存储的值一致，并启动运行时。无需手动主机操作。

v4.1 现在与现有的 v3 和 v4 运行时并行运行，因此当前流量继续被服务，代理可逐步迁移其网关。
...
`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：其PoC参数和权重保持不变。所有其他链上参数均保持不变。

该模型及其参数由kaitaku.ai团队提出，经vbgd0独立验证，现共同提出。发布输入、测量数据及阈值设定的推理依据详见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
投票将于**2026年9月10日23:17 UTC**结束。提案通过需满足：权重25%的法定人数、多数赞成、且否决票少于33.4%。
...
提案ID 100将`v4.1`添加至批准的devshard版本列表中。

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：Cosmovisor 不参与，链不会暂停，推理服务持续运行。如果获得批准，`versiond` 将下载二进制文件，将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。通过该提案本身不会移动任何流量，因此经纪商可以在之后逐步迁移其网关。
...
投票将于**2026年9月10日22:36（UTC）**结束。提案通过需满足：权重达到25%的出席门槛、多数赞成票，且反对票（含否决票）低于33.4%。

主机无需任何操作。

提案详情及投票信息可通过 `inferenced` 查看。任何活跃节点均可使用：
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅有关将冷钱包的治理投票权限授予热钱包的指南。
...
提案已获批准。将从社区池中拨出100,000 USDT，用于支持Gonka参与2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金将覆盖黄金赞助包及相关后勤支出：差旅、住宿、本地协调、嘉宾接待、活动运营和支援材料。
...
`v4.0.2` devshard二进制文件已可用。主机可使用以下说明手动更新v4二进制文件。暂未安排自动更新。

**变更内容**

重启时，`v4.0.1`会从nonce 1恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond`随后在60秒就绪超时后终止子进程，客户端因此收到HTTP 502错误。
...
提案已获批准。将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提升至0.246。其他所有模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为`PROPOSAL_STATUS_REJECTED`，原因是`failed_reason`：`proposal did not get enough votes to pass`（未达到25%出席门槛）。最终投票结果：赞成15452票，弃权3093票，反对0票，否决0票。

原始的0.214系数源于被高估的nonce/min数值。测量修正详见[gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需任何操作。
...
Devshard升级独立于主区块链更新其运行时。无需通过Cosmovisor协调全节点升级，不影响主网行为，预计不会造成推理服务的停机。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**主要变更**

1. v5的主要目标是高度同步：每个主机和排序器在保证金日志中维护一个经过签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard现在可确凿地判断cPoC是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash`标记已存在于所有先前携带时间戳字段的消息中。后续版本可将`confirmed_at`和`started_at`等墙钟字段替换为高度标记。此举可解决“未来时间”攻击问题，并为协议层处理流超时打开通道。本版本已包含这些标记，但尚未将超时和封印决策切换至其上。

3. 已添加通过所有插槽的往返心跳机制，使静默会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同的同步频率。

**行动项**

请审阅 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。
...
在周期早期，数据尚不足，因此无效推断检查（SPRT）可能仅因少量无效样本就触发，而样本尚未足够大。此提案将其稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本规模下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会被惩罚。真正的违规者仍会被触发（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效样本，且在 500 个样本时，观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期结束统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如能投票，请参与。如果未能在时限内达到紧急阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。如转为常规提案，您需要重新投出您的投票。
...
此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses和Invalid指标将被链上使用。根据当前主网阈值配置，错误率低于10%的主机不会触发任何惩罚或 slashing，但作为协议开发过程的一部分，仍应进行调查。
...
**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，首先从较小的模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi将稍后跟进，因为其数据在负载下目前可靠性较低。
...
在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731`与MiniMax-M2.7和Kimi K2.6一同成为活跃模型组，DeepSeek组中获得的PoC权重现在正以校准系数0.214（后经[提案98](#august-29-2026)提升至0.246）转换为共识权重。

针对DeepSeek的按模型参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（即正在服务Kimi或MiniMax的节点，因为已部署DeepSeek的节点均已使用3.0.16），请在方便时将这些节点迁移至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少数节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。

MLNode镜像在`docker-compose.mlnode.yml`分支的[`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)中固定为3.0.16。

默认MLNode镜像（3.0.16）基于CUDA 13.0构建。如果您的主机仍使用CUDA 12.9，请改用`3.0.16-cu129`标签。这是相同的MLNode 3.0.16，仅针对CUDA 12.9构建。两个标签均位于同一compose文件中。
...
将实时的社区销售合约和所有包装代币实例迁移至新存储的CosmWasm代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 启动快照——在此前请保持委托多样化，以确保快照有效。
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。如果通过，您就合格了。

如果您想提供DeepSeek服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免不参与罚金。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359启动前约500个区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口内切换是安全的，因为其中没有cPoC。
...
区块高度是精确的；时间是估算值，将在接近轮次时进一步确认。[更多关于DeepSeek启动的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
- 需要在vLLM 0.25.1上运行MLNode，并使用发布候选节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
2. 资格预快照：链对意向/委托进行快照，并发出建议性预资格事件。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留当前模型：在第360个纪元之前无需操作；从第360个纪元起，若您未提供该模型，则需为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
该提案已获通过，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换的二进制文件——无需重启主网或手动执行主机操作。
...
该提案已获通过，并于主网区块 5316315 成功执行。同一提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
**Devshard v4 和 v0.2.15 Mainnet 升级提案进入治理阶段**
...
2. 一个**v0.2.15 mainnet 软件升级**，移除了大量过时代码、遗留的交易处理路径及相关组件，这些组件已不再符合当前架构的需求。

尽管这两项变更在一次投票中一并提交，但它们在操作上是独立的：
...
v4 是首个面向多实例高可用的版本：在共享 Postgres 后端，通过 versiond-router 部署 N 个版本化的 /devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，提升了系统的整体稳定性和可维护性，并通过移除过时行为、将交易处理统一到当前支持的架构上，使未来的开发和升级更加容易。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 现在 / 主网升级前 —— 更新您的 API 和桥接器。为在主网升级期间保持以太坊桥接稳定，请提前根据 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和桥接镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接镜像并重启桥接容器。如果您已完成上述两步，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要理解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 请在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全清理 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放审查**

devshard 升级独立于主区块链进行。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是提高 devshard 主机在主机故障和升级时的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布同名但 SHA256 不同的新二进制文件时，versiond 可执行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还修复了错误和安全问题。
...
* 在区块 **5,105,276** 之前声明意向（2026年7月17日，约UTC 12:05）：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在纪元331的PoC开始前约500个区块的窗口期内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
...
这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册至治理模型列表，使其从纪元331开始进入标准引导流程。

同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的内存增长问题 —— 也就是7月15日事件中网络节点OOM的根本原因。

**这标志着v3切换最终完成。** 仍在通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票 —— 紧急窗口期很短。
2. 提供Kimi服务的主机：在投票结束后、区块 **5,105,276**（2026年7月17日，约UTC 12:05）之前立即声明对`moonshotai/Kimi-K2.6`的意向，然后在纪元331的PoC开始前约500个区块的窗口期内（区块 **5,105,776**，2026年7月17日，约UTC 12:50）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**不要委托给守护者节点**；将委托分散至独立的Kimi主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取更新的委托指导。
...
提案详情与投票可通过`inferenced`访问。任何活跃节点均可使用。可用节点包括：
...
- 意向截止时间（纪元330）：区块 **5,105,276** — 2026年7月17日，约UTC 12:05（约05:05 PDT）。
- 纪元331 PoC开始：区块 **5,105,776** — 2026年7月17日 12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6`在纪元328–329中失去了PoC验证多数支持（详情见[此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其以最小停机时间恢复的最快方式 —— 与6月提案78所采用的恢复路径相同。

该提案将在下一次PoC前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将从纪元331开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票 —— 紧急窗口期很短。
2. 提供Kimi服务的主机：保持您的Kimi设置就绪，随时准备在引导PoC时切换您的MLNode。
...
Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者投票权未被触发：当前未被计入投票权的无模型投票权重的守护者会被过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与纪元 306–307 事件类似。纪元 329 开始时的投票确切细节仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个纪元）重新添加：

1. [今天] 一个紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。随后将发布包含提案 ID、投票命令和截止日期的单独公告。
...
* **请勿委托给守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在开发中的修复*** PoC 验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完成迁移后，v1/v2 将被移除且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（公告将随后发布），请对该提案进行投票。
...
提案获批后，v3 现在与现有 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人切换至 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**行动项**

获准的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这使他们能够在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。

**关键变更**

- 已准备代理在 v0.2.14 链升级期间继续提供推理服务，而不依赖已弃用的经典 API 路径。
- 改进了 RAM 利用率。
- 修复了网关运行时行为。
...
v3 运行时准备代理在 v0.2.14 链升级期间继续提供推理服务，而不依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用较早的运行时前缀，直到代理将流量切换至 v3。在此类升级期间，无需进行主网重启或手动主机操作。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
将从该分支单独发布 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换至 `/devshard/v3` 的代理可以在链升级期间继续提供服务。
...
有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一个升级周期中符合社区悬赏资格。
...
4. 经纪人 —— 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 发布版本上线，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供Kimi服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行Kimi，因此不愿运行它的主机可以选择委托给该节点。
...
> **更新（2026年7月15日）：此建议已撤销 —— 请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取更新的委托指引。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此举并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的非参与惩罚）。该系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此举并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2服务的主机需在 `start_poc - deploy_window` 的引导快照前提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 以及 >2/3 可达性）。这使操作员能在配置硬件前确认模型是否具备可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由PoC开始时实际提交PoC存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会激活。
...
* `zai-org/GLM-5.2-FP8` 作为可选PoC模型可用，且无非参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能引导进入第309轮。由于每轮约22.8小时，标准的48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计支持本提案。

**主机需采取的行动**

1. **要提供Kimi服务：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为该窗口内无cPoC。

2. **要部署GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意图并配置您的MLNode。部署是可选的——未选择的主机不会受到任何惩罚。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍使用Qwen或Kimi的主机在本周期内将无法获得cPoC，直到切换为止。
* 计划重新使用Kimi的主机应保持Kimi配置就绪，并在PoC 309时准备好将MLNode切换回Kimi——将在第308周期内进行一次恢复Kimi的投票。

**即将进行**

在第308周期内将进行一次加急投票，合并两项变更，均于第309周期生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导程序。
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，以便主机可选择加入，提前衡量模型需求，不提供服务的主机不会受到惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`当前投票数不足，无法通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一周期。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式。

一项加急提案现已上链，以便在下一周期开始前生效。
链上提案在一次投票中执行以下操作：
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi的投票数不足，无法通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的路径。下一周期将进行一次恢复Kimi的投票。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已提出的阵容调整。它与Kimi情况无关，仅因同为PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 提升为基础模型。**

由于重置必须在第307个周期结束前完成，因此这些变更合并为一次紧急投票。每个周期约为22.9小时，标准的48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创世守护者应支持该提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（若尚未就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需为PoC 309时切换回Kimi做好准备——第308周期将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 第308周期将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情及投票可通过`inferenced`访问：任何活跃节点均可使用：
...
这是首次仅针对 devshard 的运行时升级。它独立于全链软件升级，无需链二进制升级。

提案通过后，v2 现在可与现有的 v1 devshard 运行时并行运行。新进程在 `/devshard/v2` 前缀下提供服务，而现有的 v1 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 会自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于这种仅 devshard 的运行时升级，无需重启节点容器或手动执行主机步骤。

**主要变更**

- 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在 RAM 或状态中。
...
- 将每个推理的验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每个槽的总计数。
...
本提案涵盖 devshard v2 发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对 devshard 的升级。它独立于全链软件升级。若获批准，v2 将与现有的 v1 devshard 运行时并行运行。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在 RAM 或状态中
...
4. 将每个推理的验证计数器存储在状态根之外的 SQLite/Postgres 中，并在推理修剪后通过 devshard 统计端点暴露每个槽的总计数
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获得批准，`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
...
v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax组中获得的PoC权重现在将以校准系数0.3024转换为共识权重。

针对MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
关于 [升级提案 v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **通过**。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从第 278 个纪元起，未采取任何操作的主机会对其全部权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基线。此调整**仅适用于您共识权重中源自 Kimi 的部分**；您源自 Qwen 的权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新提交，此次提交包含若干更新。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。如果守护者弃权，非守护者需在剩余的 75% 中实现约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将 devshard 随机数上限从 `20_000` 提升至 `1_000_000`。
- 将每个纪元的最大 devshards 数量从 `100` 提升至 `500_000`。
...
**在第 278 个周期前对 `MiniMaxAI/MiniMax-M2.7` 的参与选择（从该周期开始处罚）**
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 第一个 MiniMax PoC 阶段在升级后的下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。
...
- 该提案将有意跳过从升级高度到升级纪元结束（10000 块宽限期）的确认 PoC。若获得批准，此跳过为预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最后 3 个纪元）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上无任何变更，升级将不会发生。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何措施。
...
在应用额外保护措施期间，推理服务可能暂时不可用。访问预计会逐步恢复，首先通过若干社区代理和经纪人端点。

**发生了什么**

在第266轮中，网络的活跃权重显著下降。

该问题已被追踪到使用非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。
...
在相关保护措施应用期间，推理服务可能暂时不可用。访问预计会逐步恢复，首先通过若干社区代理和经纪人端点。
...
2. 监控下几轮的PoC参与情况。确保您的节点如预期般进入PoC，并且活跃权重正确反映。
...
似乎PoC投票未能收集到本轮所需的投票数。确切原因尚未确认，社区正在积极调查此情况。
...
- 提案在升级后增加了宽限期，升级后3000个区块内不会惩罚主机。
...
- 将创世守护者调整后的投票权减少至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不投票的情况下，这使得剩余 75% 投票权（`0.25 / 0.75 = 0.334`）的有效法定人数为 1/3。
...
- 默认在端口 `9400` 启用 `NodeManagerGrpcPort`。
- 为 devshard 状态添加 Postgres 支持。
- 为 SQLite 和 Postgres devshard 数据库添加数据修剪功能。
- 添加状态快照以加快 devshard 启动和恢复速度。
- 修复 OpenAI 兼容 API 响应解析问题。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。此时，请先确认您运行的是当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊侧合约激活，所有主机需验证其桥接容器已部署、运行最新版本并同步正确。
...
本次仅为拉取请求审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天启动。
...
- Devshard 结算使用了硬编码的 `20_000` 非nonce 限制。现在限制为 `DevshardEscrowParams.MaxNonce`，v0.2.13 升级将其设置为 `1_000_000`。升级还将 `MaxEscrowsPerEpoch` 提升至 `500_000`。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能延迟启动。这延迟了 api 激活，导致部分矿工因延迟跳过了确认 PoC。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实际中难以达成。为避免Kimi-K2.6在未来纪元中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
提交了PoCIntent的主机应在PoC于区块 `3874496` 开始前，至少将一个MLNode从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。

预评估与PoC开始之间有500个区块的窗口期。在此期间，无CPoC任务，因此声明了意向的主机可安全地将其模型节点切换至 `Kimi K2.6`。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保留您的MLNode使用 `Qwen235B`，并于明天提交下一纪元的意向。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：该模型组将在升级后两个纪元激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
鼓励主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/948) 上审阅提案并参与投票。
...
鼓励主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/948) 上审阅提案并投票。
...
每个 DAPI 会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此 `/admin/v1/nodes` 看起来是干净的，即使持久化配置仍包含额外模型。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个带有清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。剩余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需要手动修复。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成其自身的本地 PoC 权重，然后通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固。** 对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间验证。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
请主机从 `gonka/deploy/join` 目录运行以下命令以更新仪表板/浏览器：
```
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml pull explorer
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
...
如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 经销商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于接近 PoC 启动的节点，稳定性有所提升。

更新修复了在 PoC 启动附近的一个边缘情况，此前在特定条件下可能导致性能下降。
...
这些性能改进可使每个区块的推理次数最多提升 100 倍，具体取决于工作负载和网络条件。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获得批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 的性能。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限的生产测试，并非因其已完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能同时启动剪枝活动，造成不必要的运营压力。
...
针对此次 PR 审查的有意义贡献，可能在下一次升级中提出奖励。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被认为完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作至关重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间大规模启动剪枝活动，造成不必要的运营压力。
...
如果您无权访问持有投票权的冷钱包密钥，则建议提前安排投票委托。请与该密钥的所有者联系，请求授权您代为投票。若无此授权，其他账户无法提交投票。
...
API二进制文件的新版本已发布。该版本更新了连接超时处理，并在PoC验证管道中引入了额外的检查。

1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格5分钟超时，而某些请求可能需要更长时间。新版本的API将此值恢复，不再强制执行严格限制。
2. 以前的请求重试系统即使因处理超时（而非TLS超时）失败也会重试推理。
服务器端对长请求的重试通常无效，因为这会导致相同的超时情况。同时，客户端可能收到不一致的输出。新版本的API在这些情况下不再重试推理。
3. 当前被保留且不参与PoC生成的MLNode仍被用于PoC验证。这可能导致推理遗漏。新版本将这些节点排除在PoC验证之外。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍有活跃请求而导致潜在权重下降的风险。
...
- 确保[抵押](https://gonka.ai/host/collateral/)配置正确，否则从第180个纪元起，PoC所得奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若无替代方案获批，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各纪元间可能波动。较小的权重可能经历相对更大的波动。为避免在纪元边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选定。如所实施，有效的PoC参考窗口比先前名义假设长约5个区块。
...
由于有效归一化窗口比早期名义模型长约5个区块，每个纪元重新计算的权重成比例降低。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此次升级通过实际 PoC 持续时间对 PoC 参与者权重进行标准化，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入了一个为期3000个区块的宽限期，期间无确认 PoC，并对升级当期的错过率和无效率阈值采取较宽松的标准。
...
- 每1单位算力对应0.032 GNK（约每H100对应10 GNK）
- 错过率或封禁惩罚为0.01%
- 无效推理惩罚为0.5%

这意味着，在单个周期内，即使被惩罚，矿工最多只会损失其抵押品的0.5%。而所需抵押品仅占日奖励的约24%。

**警告：** 抵押参数将不受投票结果影响而生效。若本提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。

投票结束后、第180周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其奖励将被削减5倍。
...
下一版本 v0.2.10 的链上软件升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认关闭），并实现基于实时的权重标准化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级当期降低了错过率和无效率的阈值。
...
- 在升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用大量磁盘空间，可应用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理技术。
...
正如AMA期间讨论并基于社区成员提出的论点，建议从较低的抵押要求和最小的惩罚开始。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的0.5%。所需抵押物仅占日收益的约24%。

提案提交投票后将另行公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世时定义的抵押参数将在第180个纪元自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果稳定性得到证明，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
对于对本次 PR 审查的有意义贡献，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审查请求，并非正式投票的开始。治理投票流程将在审查期结束后开始。
...
2. 社区审核该PR后，预计将从该分支创建v0.2.10版本，并提交该版本的链上升级提案，以启动正式的治理投票流程。
3. 如果链上提案通过，该PR将在升级在链上执行后被合并。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此次升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串而非数字编码，int64/uint64值现在也以字符串编码。

## 2026年2月4日

**CLI更新提醒**

为授予在v0.2.9升级后创建的热密钥权限，请使用[CLI版本v0.2.9](https://github.com/gonka-ai/gonka/releases/tag/release%2Fv0.2.9)。
...
- 在 PoC 外部使用标准 vLLM 基于相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与 PoC v2 中观察到的相同 GPU 类型间的相对性能差异。
...
针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并已在主网第2451000个区块成功执行升级。此次升级实现了PoC v2的权重分配机制，并完成了向传统PoC机制的过渡。

**请注意**

- 下一个PoC周期（从第158个纪元过渡到第159个纪元）至关重要。请确保在线，以便在需要时及时执行后续步骤或缓解措施。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的ML节点才有资格进入下一个（第159个）纪元并参与PoC v2权重分配。运行其他模型的ML节点将不会被纳入即将来临纪元的参与集合中。
...
有关将ML节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、升级ML节点镜像以及移除其他模型的指导，请参阅[常见问题解答](https://gonka.ai/FAQ/#how-to-switch-to-qwenqwen3-235b-a22b-instruct-2507-fp8-upgrade-ml-nodes-and-remove-other-models)。

**当前生效的关键变更****PoC v2激活**

- PoC v2作为权重分配的活动机制
- 确认PoC（V2追踪）作为结果的权威来源
- 传统PoC逻辑不再用于权重计算
...
- 提供其他模型的ML节点不参与PoC v2权重分配。在支持的情况下，系统可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在因cPoC惩罚而减少或排除奖励的情况下，未计入的部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
- 服务其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换到`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型
...
在上一个周期中，大量ML节点未获得PoC权重。
分析表明，这是由于PoC验证参与不足所致。在多个案例中，参与者发布了nonce，但未执行验证，或验证水平远低于协议要求。
下表显示了上一个周期具有权重、在当前周期提交了PoC nonce，但未参与或未充分参与PoC验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)

他们的总权重约为36%。加上完全未参与PoC的参与者，PoC验证参与不足或无参与的总权重达到约48%，这一比例过高。
如果您的节点在表中 `validated` 列显示为0，请检查您的PoC验证日志和配置，确保验证正常运行。
...
当前支持的模型集已受限。除以下模型外，所有此前支持的模型均已从活跃集合中移除：
...
!!! note 
    一旦网络过渡到单模型配置，未满足上述两项条件的节点将失去参与PoC v2的资格。PoC v2的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明了主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未满足两项条件的节点在网络切换为单模型配置后将不再符合条件。
    - ML节点升级必须在迁移完成、PoC v2通过v0.2.8升级后单独的治理提案激活之前完成。
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估是否具备过渡到 PoC v2 的条件。一旦在活跃 Host 中观察到足够水平的采用率（约 50%），可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
由于变更量较大，此处仅列出部分重点内容。完整更新与修复列表请参见 [GitHub 拉取请求。](https://github.com/gonka-ai/gonka/pull/539)
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不具备参与 PoC v2 的资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
`allowlist` 具有固定过期时间，不会延续至延长的稳定期之后。一旦 `allowlist` 在区块 2443558 过期：
...
所有参与逻辑保持不变。本提案仅解决现有过滤逻辑中的一个小问题。
...
`Allowlist`过期：在区块 2,222,222 自动过期。
...
该提案已获得批准。此提案定义了一个临时 `allowlist`，反映在最近几轮中行为保持一致的参与者。

**当前生效的关键变更**

1) 网络将运行一个 `allowlist`，其参与者在多个轮次中：
...
2) 之前表现出偏离这些模式的参与者将被排除在 `allowlist` 之外，直到在区块 2,222,222 结束的稳定窗口期结束。

**执行特性**

- `allowlist` 从下一个轮次（第135轮）开始生效
- 激活发生在第135轮的首次PoC期间
- 无需软件升级
...
此投票定义了稳定窗口期内参与者的 `allowlist` ([https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv))，反映那些行为始终符合网络预期的参与者集合。

**投票范围**

若获得批准，网络将临时运行一个 `allowlist`，包含在先前轮次中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个轮次中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式集进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，且
- 观察到的PoC权重低于使用可比硬件的其他参与者所表现的权重的150%。
之前持续偏离这些模式的参与者在稳定窗口期结束于区块 2222222 之前不属于 `allowlist`。
...
- `Allowlist`生效：在下一次PoC执行区块2089140后。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备变更，使未来可通过治理机制控制参与者加入和推理执行，而无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得等于1.083倍第117轮奖励的 payout，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
此升级将此前通过手动 API 更新分发并已在网络中使用的重大修复正式化。这些修复包括：
...
- 主机级注册：新主机的注册将在区块 2,222,222（约两周后）之前暂停。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期内，新开发者地址的注册将被暂停。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本升级包含的预备性变更，为未来通过治理控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第117轮奖励分发**

本提案涵盖与链暂停（第117轮）相关的两项奖励分发：
