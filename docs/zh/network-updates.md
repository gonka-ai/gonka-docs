...
**Kimi-K2.6 事件（第328-329轮）：事件经过、恢复计划及委托指导变更**

在第328轮中，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数票。在第329轮开始时，运行 Kimi 的主机被移出组。其余网络正常运行。

**事件经过**

两台由守护者运营的运行 Kimi 的服务器同时发生故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因供应商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障导致这些投票立即消失。
* 第二台服务器因内存不足而失去网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此此次故障足以使其群体低于多数。守护者投票决胜机制未启动：目前没有模型投票权重的守护者会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与第306-307轮事件类似。第329轮开始时的投票具体细节仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约1.5轮）重新添加：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意向、部署窗口、PoC 存储提交。意向截止前将发布相关说明。

**委托指导变更 — 请阅读**

先前的指导（包括2026年6月27日的引导公告）建议将守护者节点作为委托目标。此建议现被撤回：

* **请勿委托至守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主机制与备用机制绑定在同一硬件上——本次事件正是这种故障模式。目前正在讨论协议层面的限制。
**避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
**如果您能直接运行 Kimi —— 这是最有帮助的。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已在进行中的修复*** 守护者参与 PoC 验证（守护者始终可投票）—— v0.2.14。

自委托问题——v0.2.14。
Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
网络节点内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**
一旦紧急 Kimi 提案上链（后续公告），请对其投票。

如果您计划提供 Kimi：关注引导说明，并在意向截止前准备好声明意向。
如果您为 Kimi 委托：选择非守护者的委托目标；尽可能将权重分散到多个独立主机上。
经纪人：立即迁移 devshard 流量至 `/devshard/v3`。这将解决内存增长问题，并在移除 v1/v2 前必须完成。
检查网络节点的内存，并设置容器内存限制（`api`、`versiond`）。
...
这是一个仅限 devshard 的运行时升级。它独立于全链软件升级，无需链二进制升级。

提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人切换流量至 v3。

发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需重启 mainnet 或手动执行主机步骤。

**操作项**

已列入白名单的 devshard 创建者应在 mainnet v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这使他们能够在链升级运行时继续提供推理服务，而无需依赖已弃用的经典 API 路径。

**关键变更**
- 已准备代理，以便在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。
改进了 RAM 利用率。
修复了网关运行时行为。
...
v3 运行时准备代理，以便在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。如果链上提案获得批准，`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期运行时前缀，直到代理将流量切换到 v3。在此类升级期间，无需重启 mainnet 或手动执行主机步骤。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何从冷密钥向热密钥授予治理投票权限。
...
mainnet 链/API 工作重点包括：PoC 重复工件保护、早期共享检测、经典推理 API 废弃（在 mainnet 上禁用 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收方路由以及升级时的安全修复。
...
在此分支上将单独提出并部署一个 devshard v3 发布版本，以在 mainnet 链升级之前完成。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级运行期间继续提供推理服务。

**建议流程**

1. 活跃主机请在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查此提案。
2. 提议在主网链升级前发布并部署devshard v3。
...
本次仅呼吁审查该Pull Request，不启动正式投票。治理投票流程将在审查期结束后开始。

**Devshard v3治理投票**

提议在主网链升级前发布并部署devshard v3，以便经纪人可以在升级运行前将推理流量迁移到`/devshard/v3`。

**主机的操作事项**

1. 现在——审查PR。请阅读GitHub上的PR #1267，并对任何发现、问题、改进建议、边缘情况或漏洞留下评论。
2. 现在/主网升级前——更新您的API和网桥。为在主网升级期间保持以太坊网桥的稳定性，请提前根据指南将`ap`i二进制文件和网桥镜像更新至0.2.14。如果您的`api`二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非PoC或cPoC期间执行此步骤。
3. 对devshard v3进行投票。
4. 经纪人——将推理流量切换到`/devshard/v3`。一旦devshard v3发布，将推理流量移至`/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者——准备好调整指标的统计方式。详细说明将在`devshard v3`投票启动并接近完成时发布。

## 2026年7月6日

**安全更新：PoC-v2权重验证强化——更新您的API容器**

上周，在HackerOne上报告了PoC-v2权重路径中的一个漏洞。参与者可夸大其计算权重，从而影响区块奖励、共识投票权和治理投票权。

根据历史数据，此前未观察到此类攻击。但在当前纪元中，已检测到主机`gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3`使用了该攻击。

针对即将发布的v0.2.14链升级，已准备好了永久修复此问题的补丁。由于攻击已在使用中，因此在升级前提供了一个仅更新API的热修复方案——可异步部署，通过强化PoC-v2验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可以选择委托给该节点。
...
> **更新（2026年7月15日）：此建议已撤回 —— 请勿委托给守护节点。** 守护节点是 PoC 验证的后备机制，必须与委托保持独立。请选择运行该模型的非守护节点，并避免选择已是主要委托目标的主机。有关最新委托指南，请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定使得在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相当。此设定并非为了偏向任何硬件——MiniMax 已具备相同的权重。运行 Kimi 而非 MiniMax 的8xB300主机将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（此举实际上取消了该模型的不参与惩罚）。该系数设定使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。此设定并非为了偏向任何硬件。运行 GLM-5.2 而非 MiniMax 的8xB300主机将获得约2%的收益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 以及 >2/3 可达性）。这使操作者能在配置硬件前确认模型是否具备可行性。
...
从第309轮开始（PoC 开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是每个模型均满足启动资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能启动进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计将支持本提案。

**主机需采取的操作**

1. **若要提供 Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 中将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，期间切换是安全的，因为该窗口内无 cPoC。

2. **要提供GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意图并 provision 您的MLNode。提供它是可选的——未选择参与的主机不会受到任何惩罚。

3. **要继续使用MiniMax：** 继续提供`MiniMaxAI/MiniMax-M2.7`——无需任何操作。

4. **在截止日期前对提案79进行投票。** 加急窗口很短。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`获取。任何活跃节点均可使用：
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍运行Qwen或Kimi的主机在本周期内将无法获得cPoC，直到切换为止。
* 有意再次提供Kimi的主机应保留其Kimi配置并准备好在PoC 309时将MLNode切换回Kimi——在第308周期内将进行一次恢复Kimi的投票。
...
`moonshotai/Kimi-K2.6`目前缺乏足够的投票用于PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——被移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复路径，停机时间最短。下一周期将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已提出的阵容变更。它与Kimi的情况无关，仅因同样是PoC阵容变更而一并包含在此处。
...
3. 计划再次提供Kimi服务的主机仍应现在就将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308周期将进行恢复Kimi的投票。


**下一步计划*** **恢复Kimi K2.6** —— 第308周期将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**不设不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。
...
4) 将每个推理的验证计数器移出状态根，放入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个槽的总计数。
5) 在周期变更时修剪旧的周期存储，将SQLite/Postgres模式设置移出热路径，并强制每个进程仅选择一个存储后端。
...
**[仅针对devshard的升级PR](https://github.com/gonka-ai/gonka/pull/1289) 现已开放审查。**
...
MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续正常工作。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅适用于您共识权重中 Kimi 相关部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新提交，本次提交包含若干更新。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。若守护者弃权，非守护者需在剩余 75% 中实现约三分之一的参与率才能满足法定人数要求（参见 inference-chain 部分）。
...
若获批准，二进制版本将通过链上升级提案更新。有关升级过程的更多信息，请参阅 [/docs/upgrades.md.](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.13/docs/upgrades.md)
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期其余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后的下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关安全删除 `.inference` 目录中旧备份的指南，请参阅 [文档。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
...
- 该提案将有意在升级高度到升级周期结束（10000 块宽限期）之间跳过 Confirmation PoC。若获批准，此跳过为预期行为，并非故障；新的快照逻辑将在下一个周期开始。
...
2. 监控下几轮的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重被正确反映。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
...
如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。
...
- 该提案将有意从升级高度到升级纪元结束（3000区块的宽限期）跳过确认 PoC。如果获得批准，此跳过是预期行为，而非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个纪元）。
...
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级会回填该权限，以便它们能响应经销商投诉。
...
- Devshard 现在支持更大规模部署的共享存储选项。它现在可以使用 Postgres 作为主存储，SQLite 仍作为本地后备。
...
审查者可在以下链接找到完整的升级提案、迁移细节、测试摘要和建议流程：
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能延迟启动。这延迟了 api 激活，导致部分矿工跳过确认 PoC。
...
在Kimi-K2.6引导过程中，主机观察到30%的最低直接参与阈值在实践中难以达到。为避免Kimi-K2.6在未来的轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。

安全模型保持不变：PoC验证本身未改变，仍需超多数验证算力才能接受结果。

为使变更能在下一轮PoC前生效，该提案将加速处理。投票将持续12小时。
...
预评估与PoC开始之间有500个区块的窗口期。在此期间没有CPoC任务，因此已声明意向的主机可安全地将其模型节点切换至 `Kimi K2.6`。
...
今天的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保持您的MLNode使用 `Qwen235B`，并于明天提交下一轮的意向。
...
- **多模型PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将单一固定模型的计算证明（PoC）过渡为按模型分组的PoC。每个经治理批准的模型生成其自身的本地PoC权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托PoC投票权重）。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，将遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 仍显示为干净。版本 0.2.12 停止了这种裁剪，直接加载持久化配置。
...
- **共识层交易费用与自动迁移**（[#937](https://github.com/gonka-ai/gonka/pull/937)，[#981](https://github.com/gonka-ai/gonka/pull/981)）。引入由治理控制的Gas价格。协议职责消息（PoC、验证、推理、BLS DKG）通过`NetworkDutyFeeBypassDecorator`免收费用。`MsgPoCV2StoreCommit`采用双组件费用（基础验证 + 计数线性）作为主要Sybil防御机制。详情请参见[docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard独立运行时**（[#1045](https://github.com/gonka-ai/gonka/pull/1045)）。将devshard发布与DAPI/主网发布周期解耦。
...
- **协议加固**。实现更强的PoC v2 RNG（256位全熵，此前为32位），将通过单独的治理投票激活。其他更新包括：将`mlnode`版本传播至链上`HardwareNode`，修复DKG经销商共识，使传统验证者惩罚与所需抵押品语义对齐，确保devshard托管资金的原子性，以及在`inference_finished`事件解析中添加零时间戳容差。
...
- 确保冷钱包持有足够金额（例如100 GNK）以覆盖自动授予的费用限额支出。
- 在治理批准新模型后，部署、委托或明确拒绝每个治理批准的模型（包含的模型将在升级后3个周期激活）
...
更新修复了在PoC启动附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
目前，通过每个推理的链上交易处理推理会限制吞吐量。此设计将推理执行和验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这故意是一个早期且受限的版本。提出此版本并非因其已完善，而是因为此类系统需要尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，这可使每个区块容纳最多 100 倍的推理数量，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中可能同时出现大量剪枝活动，造成不必要的运维压力。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间启动剪枝活动，造成可避免的操作压力。
...
1. 升级 v0.2.10 引入了 Executor → MLNode 连接的严格 5 分钟超时，而某些请求可能需要更长时间。新版本的 API 将恢复该值，而不是强制执行严格限制。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 启动时中断正在进行的推理请求，以降低因 PoC 开始时仍处于活动状态的请求导致潜在权重下降的风险。
...
- 确保 [抵押](https://gonka.ai/host/collateral/) 配置正确，否则从第 180 个周期起，PoC 衍生奖励将减少 5 倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若无替代方案获批，则默认规则适用。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重可能在各周期间波动。较小的权重可能经历相对更大的波动。为避免在周期边界处出现临时抵押不足，建议在抵押水平相对较低时，存入高达计算最低要求 2 倍的金额。这可提供操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为将 PoC 权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的 PoC 参考窗口比先前的名义假设长约 5 个区块。
...
由于有效归一化窗口比先前的名义模型长约 5 个区块，因此每个周期重新计算的权重成比例降低。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入了 3000 个区块的宽限期，期间无 Confirmation PoC，并对升级所在周期的错过率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的 0.5%。所需抵押物仅占日收益的约 24%。

**警告：** 抵押参数将无视投票结果而生效。若本提案未通过，创世时定义的抵押参数将在第 180 周期自动激活，而非上述参数。
...
下一链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获得批准，该提案将引入一项对 PoC 验证的重大优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且对升级所在纪元的错过率和无效率阈值放宽。
...
这意味着即使在一个纪元内被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押仅占每日奖励的约24%。

提案提交投票后将另行公告。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有充分理由时适用。如果证明稳定性良好，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请审查GitHub上的v0.2.10升级PR并提供您的反馈。有意义的审查贡献可能在下一次升级中获得奖励。
...
此升级通过实际PoC耗时对参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
2. 社区审核该PR后，预计将从该分支创建v0.2.10版本，并提交该版本的链上升级提案，启动正式治理投票流程。
...
在更新后的权重分配生效后，追踪器（仪表板）维护人员可能希望审查第159期及之后的系数，以确保与当前PoC v2权重分配保持一致。
...
- 认证绕过：将 `epochId` 绑定到签名，以验证是否属于正确的轮次。
...
本指南说明主机应如何根据v0.2.8模型可用性变更和即将推出的PoC v2更新来升级其ML节点。ML节点配置对PoC v2的合规性从第155个周期开始生效。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。所有先前支持的模型均从活跃集合中移除，除非：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估过渡到 PoC v2 的准备情况。一旦观察到活跃 Host 中约 50% 的足够采用率，可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性，不会触发任何自动网络变更。

在下一次网络步骤经治理批准后，网络将仅暂时支持 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
- 身份验证绕过与重放：将 epochId 绑定至签名，并针对正确的 epoch 验证授权。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而不升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不再被视为有资格参与 PoC v2。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个纪元中逐步执行更新。
...
`allowlist` 具有固定的过期时间，不会在延长的稳定期后继续有效。一旦 `allowlist` 在区块 2443558 过期：
...
- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的精确列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），且
...
- 从该点开始，`allowlist`将持续有效，直至并包括区块2222222。
- 如有进一步调整，仍需通过治理决定。
...
本升级中包含的预备性变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得等于第117轮奖励1.083倍的支付，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意试图破坏或压垮系统所导致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内处于激活状态。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并激活一些额外的保护措施。
...
此升级将此前通过手动 API 更新分发并已在网络上使用的重大修复正式化。这些修复包括：
...
- 开发者地址注册：在稳定期间，暂停新开发者地址的注册。一个预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本升级中包含的准备工作，为未来通过治理控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
## 2026年1月5日
当前网络上观察到高于正常的推理遗漏率。
在许多情况下，这是由于一个缺陷：未支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
