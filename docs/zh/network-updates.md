...
将正在运行的社区销售合约和所有封装代币实例迁移至新存储的 CosmWasm 代码，并为未来的实例化注册该封装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中识别出的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 和 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 启动快照——在此前请保持委托的多样化，以确保快照有效。
...
在区块 **5,551,615** 之前，提交委托（有助于启动可达性）或拒绝。从 epoch **360** 起，DeepSeek `penalty_start_epoch` 将生效——未明确选择的主机将面临不参与处罚。
...
以该格式运行模型需要新的API版本v0.2.15-post5。安装方法如下：
```bash
...
建议逐步将节点切换到新API，而不是一次性更新整个网络。

**3) 使用mlnode-validate验证**

该技能现已支持DeepSeek。它位于独立的`./skills`文件夹中，因此无需任何代理特定设置——将其软链接到`.claude/skills`并运行[`mlnode-validate`技能](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/skills/mlnode-validate)：
```
/mlnode-validate how to use
/mlnode-validate <your-node-ip:port> @deploy/join/node-config-deepseekv4flash0731-B300.json
```
该技能会部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则表示一切正常。

如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免非参与罚金。

注意：明天将发布另一个带有更好日志的MLNode版本，因此请期待后续简短更新。

## 2026年8月12日

**DeepSeek V4 Flash启动（第359轮）**
...
1. 声明意向。在提案94通过后、在引导快照区块5,536,224（约2026年8月13日02:40 UTC / 8月12日19:40 PDT）之前，提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。此窗口内切换是安全的，因为其中没有cPoC。
注意：在Blackwell显卡上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4的模型（而非fp8 + fp4），例如`MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在引导前发布并共享更新的API二进制文件。
3. 如果您进行委托。请勿委托给守护节点；请将委托分散至独立的DeepSeek主机。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 如需保留当前模型。在第360轮前无需任何操作。从第360轮起，若您未提供DeepSeek服务，请提交`PoCDelegation`或`PoCRefusal`，否则将适用15%的非参与罚金。

区块高度是精确的；时间仅为估算值，将在接近轮次时确定。[更多关于DeepSeek启动的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。现有参数或模型均未修改，唯一变更即新增模型条目。
...
- 8xH200 在运行 MiniMax M2.7 时最优产出 1.46x 权重
- 8xB200 在运行 Kimi K2.6 时最优产出 2.96x 权重
- 8xB300 在运行 DeepSeek V4 Flash 时最优产出 3.37x 权重
...
在符合引导资格的前提下，从第 359 个周期（PoC 开始时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的 PoC 模型。现有模型保持不变。DeepSeek 的不参与惩罚从第 `360` 个周期（`penalty_start_epoch = 360`）开始。

**主机所需操作**

1. 要提供服务：投票结束后声明意向，然后部署您的 MLNode（vLLM 0.25.1 + release-candidate 配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
...
- PoC种子稳定性。在周期边界延迟后恢复遗漏的种子提交，并在临时故障（如签名者不可用）后重试本地种子恢复，以防止验证者被归零。
- API。
    - `/v1/versions` 现在返回增强的DAPI版本信息，包括ML节点详情
    - 默认的Node Manager gRPC端口现已设为9400，以确保与自定义配置的兼容性
...
这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更包括：周期跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作线程的内存清理现在与周期修剪保持一致。

**升级计划**

devshard运行时通过链上参数提案进行升级，而非完整链软件升级。若获得批准，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动执行主机操作。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅指南，了解如何将治理投票权限从冷密钥授予热密钥。
...
该提案已获批准，并于主网区块 5316315 成功执行升级。该提案还批准了 devshard v4 运行时，该运行时现已与现有的 v3 运行时并行运行。
...
2. **v0.2.15 主网软件升级**，移除大量过时代码、遗留交易处理路径及相关组件，这些组件已不再符合当前架构需求。

尽管这两项变更通过单一投票提交审议，但它们在操作上保持独立：
...
v4 是首个面向多实例高可用的版本：在共享 Postgres 后端部署 N 个 versiond/devshardd 副本，由 versiond-router 使用粘性会话路由和验证租约独占机制进行管理。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。通过移除过时行为并将交易处理统一到当前支持的架构上，也为未来的开发和升级奠定了更清晰的基础。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的 Cosmovisor 备份 `application.db` 通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
devshard 部分准备 v3 运行时，以便在链升级期间代理无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升内存利用率并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和网桥。为确保主网上线升级期间以太坊网桥稳定，请提前根据 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像升级至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两步，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护人员 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，将继续正常运行。您需要了解一项语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
devshard 升级独立于主区块链进行。它们不需要通过 Cosmovisor 协调的全节点升级，不影响主网行为，且预计不会导致推理服务中断。若通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**主要变更**

- v4 的主要目标是在主机故障和升级时实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐个替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一轮升级周期的社区奖励。
...
```
* 在第331轮PoC开始前的约500个区块窗口内，将您的MLNode切换至Kimi（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT）。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。这消除了旧运行时容器长期存在的RAM增长问题 —— 这正是7月15日网络节点OOM事件的根源。
...
2. 提供Kimi服务的主机：在投票结束后、区块 **5,105,276**（2026年7月17日，约12:05 UTC）之前立即声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日，约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**不要委托给守护者节点**；将委托分散至独立的Kimi主机。请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取更新的委托指引。
4. 经纪人 / devshard创建者：如果您任何网关流量仍在使用v1/v2或`/v1/devshard` —— 请在投票结束前切换至`/devshard/v3`。
...
`moonshotai/Kimi-K2.6`在第328–329轮中失去了PoC验证多数支持（详情见 [此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是恢复其服务且停机时间最短的方式 —— 与6月提案78所用的恢复路径相同。

该提案将在下一轮PoC前将`moonshotai/Kimi-K2.6`从PoC参数中移除，随后立即重新添加，Kimi将在第331轮开始进入标准引导流程。
...
Kimi 的验证投票一直接近 2/3 阈值，因此此次故障足以使该组低于多数。守护者决胜机制未触发：当前没有每模型投票权重的守护者被过滤出决胜机制。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与时段 306–307 的事件类似。时段 329 开始时的投票确切细节仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个时段）重新添加：

1. [今天] 一个紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。随后将发布包含提案 ID、投票命令和截止日期的独立公告。
...
* **请勿委托给守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主机制与备用机制绑定到同一硬件——此次事件正是这种故障模式。协议层面的限制正在讨论中。
...
* 网络节点内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完全迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（公告随后发布），请立即投票。
2. 如果您计划提供 Kimi：关注引导说明，并在意图截止前准备好声明意图。
3. 如果您为 Kimi 委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机。
4. 经纪人：请立即迁移 devshard 流量至 `/devshard/v3`。这将修复内存增长问题，并在移除 v1/v2 前必须完成。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，且不依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
在此分支上将单独发布 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供推理服务。
...
2. 现在 / 主网升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据指南将 `api` 二进制文件和桥接器镜像更新至 0.2.14。如果您已更新 `api` 二进制文件，则只需更新桥接器镜像并重启桥接容器。如果您已完成以上两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复程序 —— 可异步部署，通过强化 PoC-v2 验证阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可以选择委托给它。
...
> **更新（2026年7月15日）：此建议已撤销——请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择运行该模型的非守护节点，并避免选择已是主要委托目标的主机。请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指导。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定为：在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 基本相当。此设定并非偏向任何硬件——MiniMax 已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的增益。
...
常规引导流程从第309轮开始。由于投票结束时间距PoC仅约1小时，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多引导信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的非参与惩罚）。该系数设定为：在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 基本相当。此设定并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的增益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作者可在部署前确认模型是否具备可行性。
...
从第309轮开始（PoC开始约6月26日15:25 UTC；生效约16:00 UTC），前提是各模型满足引导资格条件：
...
提案必须在第308轮结束前完成，以便两个模型能顺利引导进入第309轮。由于每轮约22.8小时，标准的48小时投票周期无法容纳在一个轮次内；而12小时的紧急投票周期可以。创世守护节点预计支持本提案。

**主机需采取的操作**

1. **要提供 Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口期，期间切换是安全的，因为该窗口期内无cPoC。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换后才可获得本周期的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308周期内将进行一次投票以恢复Kimi。

**即将进行**

在第308周期内将进行一次加急投票，合并两项变更，均将于第309周期生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导程序。
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，以便主机可选择性参与，提前衡量模型需求，未选择提供该模型的主机不会受到惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`当前投票不足，无法通过PoC验证。因此，正在提供Kimi的参与者已被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi的投票不足无法通过PoC验证，将其移除并重新引导是使其尽快回归且停机时间最短的途径。下一周期将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi的情况无关，仅因同样是PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 提升为基础模型。**

由于重置必须在epoch 307结束前完成，因此这些更改被合并为一次紧急投票。每个epoch约为22.9小时，标准的48小时投票周期无法容纳在一个epoch内；而12小时的紧急周期可以。Genesis守护者应支持本提案。
...
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供服务的主机不会受到任何惩罚。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情及投票可通过`inferenced`访问：任何活跃节点均可使用：
...
- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以避免长运行会话将所有已服务的推理保留在RAM或状态中。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以避免长运行会话将所有已服务的推理保留在RAM或状态中
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。若链上提案获批，`versiond`将自动下载该二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。
...
MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止日期为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会对其全部权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基准。此调整**仅适用于您共识权重中源自 Kimi 的部分**；您的 Qwen 源权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降低至约 25%，并设置链上法定人数为 0.25。若守护者弃权，非守护者需在剩余 75% 中实现约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 区块时（约提前 43 分钟）→ 首次 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 该提案将有意跳过从升级高度到升级周期结束（10000区块宽限期）的确认 PoC。若获批准，此跳过属预期行为，并非故障；新快照逻辑将在下一个周期开始。
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 仍将作为默认选项，并自动清理（保留最近3个周期）。
- 若提案未通过（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上无任何变化，升级将不会发生。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何操作。
...
社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更多清晰信息，将及时发布更新。
...
- 包括：对Kimi（`0.78`）的权重重新校准、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复和以太坊桥接主网激活。
...
- 将创世守护者调整后的投票权减少至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束，确认 PoC 触发将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用 v0.2.10 的宽限期原语。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口的服务器可能启动延迟。这延迟了 api 激活，导致部分矿工跳过确认 PoC。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保持您的MLNode使用 `Qwen235B`，并在明天为下一个纪元提交您的意图。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个纪元激活。其系数为Qwen235B的3.51倍，基于相同硬件（8xH200，8xB200）上的模型计算复杂度。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
有关这些变更的更多详情请参见治理文档：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
此清理过程**必须在升级发生前完成**。若在升级时您的节点配置包含不受支持的模型，**节点将被拒绝并离线**。
...
运行脚本后等待 60 秒，确保更改已持久化，然后再触发升级。然后验证配置：
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将随机抽样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型概念验证（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的PoC。每个经治理批准的模型生成其自身的本地PoC权重，并通过模型特定系数聚合为总共识权重
- **共识层交易费用与自动迁移** ([#937](https://github.com/gonka-ai/gonka/pull/937), [#981](https://github.com/gonka-ai/gonka/pull/981))。引入由治理控制的gas价格。协议职责消息（PoC、验证、推理、BLS DKG）通过`NetworkDutyFeeBypassDecorator`豁免。`MsgPoCV2StoreCommit`采用两部分费用（基础验证 + 计数线性）作为主要Sybil防御机制。详情请参见[docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将devshard发布与DAPI/主网发布周期解耦。
- **Certik审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。所有已知的审计发现均已修复。
- **协议加固**。实现更强的PoC v2 RNG（256位全熵，此前为32位），将通过单独的治理投票激活。其他更新包括将`mlnode`版本传播至链上`HardwareNode`、修复DKG经销商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保devshard托管资金的原子性，以及在`inference_finished`事件解析中添加零时间戳容差。
...
- 确保冷钱包账户持有足够的金额（例如100 GNK），以覆盖自动授予的费用限额支出。
...
新版本mlnode现已发布：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
更新修复了在PoC启动附近的一个边缘情况，此前在特定条件下可能导致性能下降。
...
下一版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著改善 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理扩展性。

目前，通过每个推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中将同时启动剪枝活动，造成不必要的运营压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作至关重要。如果许多节点推迟到升级后才进行剪枝，网络中将几乎同时启动剪枝活动，造成不必要的运营压力。
...
对于已经使用代理层的团队，这可能是简化堆栈、转而依赖原生行为的好机会。实际上，这应能带来更清晰的集成模式和更易维护的系统。
...
抵押参数更新提案未能达到法定人数，因此根据当前治理规则被否决。这意味着更新后的参数将不会生效。
...
- 确保[抵押](https://gonka.ai/host/collateral/)设置正确，否则从第180个周期开始，PoC收益将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能发生波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选定。如实施所示，有效的PoC参考窗口比先前名义假设长约5个区块。
...
!!! note "兼容性说明"
    ...
    **基于实时的PoC权重归一化**
    ...
    此升级通过实际PoC耗时对PoC参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
**宽限期**：升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且对升级当期的错过率和无效率阈值要求更宽松。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押品的0.5%。所需抵押品仅占日奖励的约24%。

**警告**：无论投票结果如何，抵押参数都将生效。如果本提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。
...
下一个链上软件版本v0.2.10的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的PoC验证优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并且在升级当期降低了错过率和无效率的阈值。
...
- 在升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用了大量磁盘空间，可应用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理技术。
- 升级后，Postgres可作为本地负载存储的选项。
...
正如AMA期间讨论并基于社区成员提出的论点，建议初始抵押要求较低，并且惩罚最小化。
...
这意味着即使在一个周期内被惩罚，矿工最多也只能损失其抵押物的0.5%。而所需抵押物仅占日收益的约24%。

提案提交投票后将另行公告。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世时期定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。
...
在当前纪元中观察到cPoC（确认PoC）与PoC之间的重叠。直至纪元最后一个区块，`is_confirmation_poc_active`被观察为`true`。
...
请保持在线，以便在补丁发布时能及时应用。更多详情和补丁说明将在准备就绪后发布。
...
此升级通过实际PoC耗时对参与者权重进行标准化，以减少区块时间漂移效应，确保权重结果与真实执行时长一致。
...
2. 社区审查该PR后，预计从此分支创建v0.2.10版本，并可提交此版本的链上升级提案，启动正式的治理投票流程。
...
在PoC v2激活后，权重分配现在基于当前模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`的实测推理性能。因此，GPU的中位权重以及不同GPU类型之间的相对权重比例均已调整。
...
- 观察到的变化表明，GPU权重差异现在反映的是模型特定的推理吞吐量，而非名义上的硬件规格。例如，H100 PCIe权重的下降幅度大于H100 HBM3权重，这与`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`的观察推理行为一致。
- 在当前模型配置下，基于观察到的推理轨迹，B200 GPU的推理性能并未高于H100类GPU。
- 如果未来周期通过治理引入更大或更苛刻的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准vLLM推理对同一模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`进行的控制推理基准测试，显示出与PoC v2中观察到的GPU类型间相同的相对性能差异。
...
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不包含在即将到来的纪元的参与者集合中。
...
他们的总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或参与极低的总权重达到约 48%，这一比例严重偏高。
...
- 双模式迁移：支持 V1（常规 PoC）和 V2（确认 PoC）的跟踪。
...
!!! note 
    一旦网络过渡到单模型配置，未能满足上述两项条件的节点将无法参与 PoC v2。PoC v2 的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置在第155个Epoch起需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个Epoch后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
- 将 PoC 直接集成到 vLLM 中，无需卸载模型或加载单独的 PoC 模型，即可立即从推理切换到 PoC。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML 节点版本，可评估向 PoC v2 过渡的准备情况。一旦观察到活跃主机中达到足够的采用率（约 50%），可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而不升级 ML 节点不足以参与 PoC v2。一旦网络切换为单模型配置，未满足两项条件的节点将不被视为 PoC v2 参与资格。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机建议在多个纪元中逐步执行更新。
...
- 表现出的PoC权重不超过同类硬件观察到的权重的150%

2) 之前表现出持续偏离这些模式的参与者将被排除在 `allowlist` 之外，直到区块 2,222,222 的稳定窗口结束。
...
如果获得批准，网络将临时使用一个 `allowlist`，包含在先前纪元中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：
...
- 观察到的PoC权重保持在使用同类硬件的其他参与者所表现权重的150%以下。
之前持续偏离这些模式的参与者在稳定窗口结束于区块 2222222 之前不包含在 `allowlist` 中。
...
此次升级将此前通过手动API更新分发且已在网络中使用的关键修复正式化。这些修复包括：
...
本次升级包含的预备变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的 payouts，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内处于激活状态。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并激活一些额外的保护措施。
...
通过将其纳入此处，该行为将成为跨网络一致应用的协议级规则。

**临时参与和执行限制**

- 主机级注册：在区块2,222,222之前（约两周后）暂停新主机注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2,294,222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者入网和推理执行提供了支持，而无需额外的软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
