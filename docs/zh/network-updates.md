...
!!! note "关于本页面"
    ...
    本页面的内容不保证完整。如需获取最新信息，包括治理投票的启动及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。

## 2026年7月15日

**紧急治理投票（提案87）：移除Kimi-K2.6以快速重新引导**

`moonshotai/Kimi-K2.6`在第328–329个周期失去了PoC验证的多数支持（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在从活跃集合中移除Kimi并重新引导，是使其在最小停机时间内恢复的最快方式——与6月提案78的恢复路径相同。

该提案将在下一个PoC之前将`moonshotai/Kimi-K2.6`从PoC参数中移除，随后立即重新添加，Kimi将在第331个周期开始标准引导流程。

**主机需采取的必要操作**

1. 在截止日期前对提案87进行投票——紧急窗口时间很短。
2. 运行Kimi的主机：请保持您的Kimi设置处于暂存状态，并准备好在引导PoC时切换回您的MLNode。
3. 在引导期间委托时：请勿委托给守护节点；请将委托分散到独立的Kimi主机上。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
**截止日期*** 投票时间：2026年7月15日17:01 ~ 2026年7月16日05:01（PDT）/ 2026年7月16日00:01 ~ 2026年7月16日12:01（UTC）（紧急，0.667通过阈值；参与率重要，请尽快投票）。
* 引导意向截止日期和周期时间将在引导说明中跟进。

## 2026年7月15日

**Kimi-K2.6事件（第328–329周期）：事件经过、恢复计划及委托指导变更**

在第328周期，`moonshotai/Kimi-K2.6`失去了PoC验证的多数支持。在第329周期开始时，运行Kimi的主机被移出组。网络其余部分正常运行。

**事件经过**

两台由守护节点运营的运行Kimi的服务器同时故障：

* 其中一台服务器上的Kimi MLNode因供应商端问题崩溃。大量Kimi委托集中在这台主机上，其故障导致这些投票立即消失。
* 第二台服务器因内存不足导致网络节点中断。

Kimi的验证投票一直接近2/3阈值，因此这次故障足以使其低于多数支持。守护节点的平票机制未触发：目前，没有模型级投票权重的守护节点已被排除在平票机制之外。这是一个已知问题，将在即将发布的v0.2.14升级中修复。

这种情况与第306–307周期的事件类似。第329周期开始时的投票确切细节仍在确认中，此摘要可能存在轻微修正。

**恢复计划**

恢复Kimi最安全的方式是通过全新引导（约1.5个周期）重新添加：

1. [今天] 一个紧急提案将在下一个PoC之前移除Kimi，并立即重新添加。随后将发布单独公告，包含提案ID、投票命令和截止日期。
2. [明天] Kimi将再次经历标准引导流程：声明意向、部署窗口、PoC存储提交。意向截止日期前将发布详细说明。

**委托指导变更——请仔细阅读**

之前的指导（包括2026年6月27日的引导公告）建议将守护节点作为委托目标。此建议现被撤销：

* **请勿委托给守护节点。** 守护节点是PoC验证的备用机制。将委托集中于它们会将主机制与备用机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中到任何单个主机上。** 在支持基于百分比的多主机委托之前，请将委托分散到多个运行模型的独立主机上。如果您运行多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**修复已在处理中*** PoC验证中的守护者投票（守护者始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
...
3. 如果您为Kimi委托：选择非守护者委托方；尽可能将权重分散到独立主机上。
4. 经纪人：立即把devshard流量迁移到`/devshard/v3`。这将修复RAM增长问题，并在v1/v2移除前必须完成。
...
提案通过后，v3现在与现有devshard运行时并行运行。新进程在`/devshard/v3`前缀下提供服务，而现有devshard流量可继续使用早期运行时前缀，直到经纪人将流量切换至v3。

此版本将`devshardd`二进制文件作为Gonka发布工件发布。`versiond`会自动下载该二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动一个额外的`devshardd`进程。

此类仅限devshard的运行时升级无需重启主网或手动执行主机步骤。

**操作项**

已获白名单的devshard创建者应在主网v0.2.14链升级前，将推理流量切换至`/devshard/v3`。这将使他们在链升级运行期间仍能继续提供推理服务，而不依赖已弃用的经典API路径。
...
v3运行时为经纪人做好准备，使其在v0.2.14链升级期间无需依赖已弃用的经典API路径即可继续提供推理服务。同时改进了RAM利用率，修复了网关运行时行为，并启用了SQLite与Postgres存储之间的安全切换。
...
此版本将`devshardd`二进制文件作为Gonka发布工件发布。如果链上提案获得批准，`versiond`将自动下载该二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动一个额外的`devshardd`进程。
新进程在`/devshard/v3`前缀下提供服务。现有devshard流量可继续使用早期运行时前缀，直到经纪人将流量切换至v3。此类升级期间无需重启主网或手动执行主机步骤。
...
下一个链上软件升级 v0.2.14 的[拉取请求](https://github.com/gonka-ai/gonka/pull/PR)现已开放供审查。

主网链/API 工作重点包括：PoC 重复工件防护、早期份额检测、经典推理 API 废弃（禁用主网 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收方路由，以及升级时的安全修复。
...
将从本分支单独发布 devshard v3 版本，并在主网链升级前部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级运行期间继续提供推理服务。
...
1. 现在 — 审查 PR。阅读 GitHub 上的 PR #1267，并对您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器稳定，请提前根据指南将 `ap`i 二进制文件和桥接器镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，您只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
3. 对 devshard v3 进行投票。
4. 代理 — 将推理流量切换至 `/devshard/v3`。一旦 devshard v3 版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 — 准备调整指标统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近尾声后另行发布。
...
根据历史数据，此前未发现此类攻击。然而，在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 的使用行为。

针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复程序 —— 可异步部署，通过强化 PoC-v2 验证以阻止复制计算通过采样检查，从而阻止攻击。
...
> **更新（2026年7月15日）：此建议已撤回 —— 请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择运行该模型的非守护主机，并避免选择已是主要委托目标的主机。有关最新委托指南，请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定使得在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相当。此设定并非偏向任何硬件——MiniMax 上已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。此设定并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员可在配置资源前确认模型是否具备可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在PoC开始时实际提交PoC存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会激活。
...
1. **要提供 Kimi：** 在投票**结束后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间没有 cPoC。
...
* 打算再次提供Kimi的主机应保持Kimi配置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308个纪元将进行一次投票以恢复Kimi。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，以便主机可选择加入，提前衡量模型需求，不提供该模型的主机不会受到惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以实现快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前投票不足，无法通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一纪元。现在将其从活跃集合中移除并重新引导，是最快恢复Kimi且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi投票不足，无法通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的方式。下一纪元将进行一次投票以恢复Kimi。
* **Qwen3-235B——退役（独立、无关变更）。** 退役旧版Qwen3-235B是独立的、此前已请求的模型调整。此变更与Kimi情况无关，仅因同属PoC模型调整而一并包含在此处。
...
从epoch 308开始（~UTC 6月25日 17:25），`MiniMaxAI/MiniMax-M2.7`将成为唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6将不再活跃。恢复Kimi将在下一个epoch的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如尚未就绪），以避免在epoch边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需为PoC 309时切换回Kimi做好准备——epoch 308将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 在epoch 308中进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设置不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情及投票可通过`inferenced`访问：
...
发布版本将 `devshardd` 二进制文件作为Gonka发布工件发布。`versiond` 自动下载二进制文件，验证sha256哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
...
4) 将每个推理的验证计数器移出状态根，进入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个插槽的总计数。
...
发布版本将 `devshardd` 二进制文件作为Gonka发布工件发布。如果链上提案获得批准，`versiond` 将自动下载二进制文件，验证sha256哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
...
v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7与Qwen3-235B和Kimi K2.6一同成为活跃模型组，MiniMax组中获得的PoC权重现在将以校准系数0.3024转换为共识权重。

MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需任何额外操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
针对 [升级提案 v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **批准**。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。如果守护者弃权，非守护者需在剩余 75% 中达到约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将 `MiniMaxAI/MiniMax-M2.7` 作为治理批准的模型和 PoC 模型加入。
- 更新推理验证阈值：
    - Qwen 235B：`0.940`
    - Kimi K2.6：`0.900`
    - MiniMax-M2.7：`0.922`
...
**在第 278 个纪元前对 `MiniMaxAI/MiniMax-M2.7` 的参与选择（届时开始处罚）**
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级纪元其余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个纪元边界开始 → MiniMax 处罚在链纪元 278 执行。
...
- 该提案将有意在升级高度到升级纪元结束（10000区块宽限期）之间跳过 Confirmation PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个纪元）。
...
第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击加上若干相关条件，导致许多参与者未能进入第266轮。
...
由于现有协议规则：一个模型的总权重不能超过上一轮所有模型总权重的75%，Kimi在第267轮中的活跃权重较低。
...
2. 监控下一阶段的PoC参与情况。检查您的节点是否如预期般进入PoC，且活跃权重是否正确反映。
...
社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更多清晰认识，将分享更新。
...
- 包括：Kimi（`0.78`）的重新校准权重、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复和以太坊桥接主网激活。
...
要求所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再执行进一步操作。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效载荷存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近3个周期）。
...
- Devshard 结算使用了硬编码的 `20_000` 非 nonce 限制。限制现在为 `DevshardEscrowParams.MaxNonce`，v0.2.13 升级将其设置为 `1_000_000`。升级还将 `MaxEscrowsPerEpoch` 提升至 `500_000`。
...
- Devshard 需要为更大规模部署提供共享存储选项。现在可以使用 Postgres 作为主存储，SQLite 仍作为本地后备存储。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口的服务器可能启动延迟。这延迟了 api 激活，导致一些矿工因延迟而跳过 Confirmation PoC。
...
在上一轮训练中发现Kimi-K2.6解析某些响应时存在一个轻微bug。
...
我们建议更换api容器的二进制文件。除修复外，新版本还启用了devshard数据库的修剪功能，并为devshard状态增加了Postgres支持。
...
传输代理 `node1`、`node2` 和 `node3` 已禁用。所有主网推理现在均通过 `node4` 路由，该代理采用新的 `devshard` 计费方案。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型转变为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固。** 对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间验证。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
更新修复了在 PoC 开始附近的一个边缘情况，此前在特定条件下可能导致性能下降。
...
升级提案 v0.2.11 的链上治理投票已结束。该提案已获批准，并已在主网上成功执行升级。
...
下一代链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中。部分问题仅通过本地测试难以发现。当前实现已设计为不影响矿工奖励。
...
在类似主网的条件下，这使每个区块可容纳最多 100 倍的推理量，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
在类似主网的条件下，这使得每个区块可容纳的推理数量最多增加100倍，具体取决于工作负载和网络条件。  ￼
...
对于已使用代理层的团队，这可能是简化堆栈、转而依赖原生行为的好机会。实际上，这将带来更清晰的集成模式和更易维护的系统。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低PoC开始时仍处于活动状态的请求导致潜在权重下降的风险。
...
抵押参数更新提案未能达到法定人数，因此根据当前治理规则被拒绝。这意味着更新后的参数将不会被激活。

如前所述，第180轮次的抵押激活与此投票无关。

由于提案未通过，创世区块中定义的抵押参数将在第180轮次自动生效。

参与者应：

- 审查创世区块中定义的抵押参数。
- 在第180轮次前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)已正确设置，否则从第180轮次开始，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各轮次间可能波动。较小的权重可能会经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平仍相对较低时，存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化PoC权重**
    ...
    此升级根据实际PoC经过时间标准化PoC参与者的权重，以减少区块时间漂移影响，并使权重结果与实际执行持续时间保持一致。
...
这意味着，在单个周期内，即使被惩罚，矿工最多只会损失其抵押品的0.5%。而所需抵押品仅占每日奖励的约24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果该提案未通过，创世时定义的抵押参数将在第180个周期自动激活，而非上述参数。
...
下一个链上软件版本v0.2.10的升级提案现已上链并开放投票。若获批准，该提案将引入一项对PoC验证的重大优化（默认禁用），并实现基于实时的权重标准化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并对升级当期的错过率和无效率阈值采取较宽松的标准。
...
任何未来的抵押品增加都将通过单独的投票提案。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果稳定性得到验证，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
链上推理访问目前开放，不再仅限开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
此升级通过实际 PoC 持续时间归一化参与者权重，以减少区块时间漂移效应，并使权重结果与实际执行时长保持一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带有锁仓的流式支付转移
...
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与PoC v2中观测到的GPU类型间相对性能差异一致的结果。

**跟踪器（仪表板）维护人员的操作**

在更新的权重分配生效后，跟踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。
...
在最近一个周期中，大量ML节点未获得PoC权重。
分析表明，这是由于PoC验证参与不足所致。在多个案例中，参与者发布了nonce，但未执行验证，或验证级别远低于协议要求。
下表显示了上一周期具有权重、在当前周期提交了PoC nonce，但未参与或未充分参与PoC验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估向 PoC v2 过渡的准备情况。一旦观察到约 50% 的活跃 Host 足够采用，可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。在网络切换至单模型配置后，未满足两项条件的节点将不被视为 PoC v2 参与资格合格。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
该提案将当前的稳定期延长约两周。延长的期限旨在为即将进行的 PoC 变更及相关网络升级提供额外的测试时间。有关新 PoC 开发进展的更多详情，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

该延长也为主机提供了时间，以准备其设备以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步执行更新。
...
- 报告的硬件特征已与预定义的常见硬件配置模式进行比对，以识别偏差和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，并且
...
- 临时重新激活创世验证者增强——一种此前使用过、有时间限制的防御机制，拟重新启用。
...
- 开发者地址注册：在稳定期内暂停新开发者地址注册。一个预定义的 `allowlist` 开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第 117 期奖励分配**

本提案涵盖与链停顿（第 117 期）相关的两项奖励分配：

- 在第 117 期活跃但未收到奖励的节点，将补发该期遗漏的奖励。
- 所有在第 117 期活跃的节点，将获得额外发放，金额为第 117 期奖励的 1.083 倍，统一适用于所有符合条件的节点，包括已收到原始奖励的节点。
