...
下一次链上软件升级 v0.2.16 的拉取请求已开放审查 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)。有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能在下一个升级周期中获得社区奖励。

主网链/API 工作重点包括可信权重、动态系数、交易费用组和 PoC 挑战。如果提案获得批准，治理、BLS 和 PoC 验证权将仅限于上一个纪元确认的算力，而新的算力仍可立即获得奖励。治理可为每个模型设置目标份额和系数范围。升级时这些范围保持关闭，因此系数和奖励不会变动。新增了消息组和气体规则，每个组默认禁用，因此协议仍不收取费用。经许可的挑战者可将一个活跃主机从推理任务移至全容量 PoC，直至下一次常规 PoC。

**关键变更**

1. **可信权重。** 在 v0.2.16 之前，声称的算力突然增加会立即提升参与者在治理、BLS 和 PoC 验证中的权力。这使得未经确认的算力影响共识及对其他参与者的验证。此次升级将该权力限制为上一个纪元已确认的算力。新加入或回归的参与者从 0 开始，若确认失败，则下一个基线设为 0。新的算力仍可立即获得奖励。只有对信任敏感的权力需等待一个确认纪元。
2. **动态系数 v1。** 网络需要为每个模型提供可预测的吞吐量，以便用户依赖其可用性。每个模型的目标吞吐量应符合预期需求。使用固定系数时，整个硬件类别倾向于转向同一模型，导致每模型的容量难以平衡和预测。此次升级允许治理为每个模型设置网络算力的目标百分比和系数范围。协议将动态调整该范围内的系数，使算力趋向目标。超出目标的算力按最低系数评分，以抑制过剩供应。治理最初根据需求估计和数据源（如 OpenRouter）设置目标。后续版本可聚合主机估计，最终使用链上模型使用情况。本版本仅改变激励机制，不会自动切换主机部署的模型。升级时，最小值和最大值均等于当前比例，因此系数和奖励在治理打开范围前保持不变。
3. **费用。** 历史上，协议不收取任何交易费用。因此攻击者可以以极低成本提交高频率消息，而每个验证者却需承担处理和存储成本。此次升级对交易类型进行分组，并添加每消息的气体规则，使每组可单独定价。所有组默认禁用且不收费。治理稍后可启用并定价。升级提案信息也可在升级高度启用这些组。
4. **PoC 挑战。** PoC 和随机确认 PoC 证明主机声称的算力。在纪元其余时间，推理统计数据检查该硬件是否用于协议分配的工作。高比例的遗漏或无效推理可能导致主机被移除。这提供了强有力的持续检查，但其敏感性取决于推理量，可能无法揭示声称与可用算力之间的所有差距。PoC 挑战为此类情况增加了额外的安全层。经许可的挑战者可要求一个活跃主机离开推理，以全容量运行 PoC，直至下一次常规 PoC。为发起挑战，挑战者需锁定目标剩余纪元奖励的一部分作为支付。若目标通过，其获得该支付；若失败，挑战者获得退款，目标将受到与失败确认 PoC 相同的惩罚。挑战期间遗漏的推理不计入目标。仅允许列表中的 devshard 托管创建者可发起挑战。若允许列表为空，则任何人都可发起挑战。
更多详情及其他变更见：[https://github.com/gonka-ai/gonka/pull/1535 ](https://github.com/gonka-ai/gonka/pull/1535 )

**行动项**

1. **所有人。** 现在 —— 审查拉取请求。阅读 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535) 并对任何发现、问题、改进建议、边界情况或漏洞留下评论。
2. **主机。** 投票前无需任何操作。不要更新容器、重新授予权限或为此升级移动模型。
3. **仪表板维护者。** 如果治理投票通过，治理、BLS 和验证投票将遵循上限权重。奖励仍基于真实权重。
...
主要协议新增功能为高度同步（height-sync），为 DevShard 会话提供经过签名、可重放的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立协议级逻辑时间。此版本还包括高可用性改进、热切换与会话恢复、网关更新、验证与结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机需执行的操作**

无需操作。`versiond` 下载新运行时，验证其 SHA256 并与链上存储的值比对，然后启动运行。`v3`、`v4` 和 `v4.1` 仍为批准版本，因此现有会话和路由前缀不受影响。这是一个运行时版本，而非网关发布：任何网关迁移将另行公告并提供独立指南。
...
这是仅针对 devshard 的变更。Cosmovisor 不涉及，链不会停止，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，验证其 SHA-256 值与链上存储值一致后启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能为高度同步（height-sync），为 devshard 会话提供经过签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级 QoS 建立协议级逻辑时间。此版本还包括高可用性改进、热切换与会话恢复、网关更新、验证与结算修复、安全加固、可观测性改进以及扩展的端到端测试。
...
投票将于 **2026 年 9 月 18 日 16:47 UTC** 结束。提案通过需满足：权重达到 25% 的最低投票率、多数赞成，且反对并行使否决票的比例低于 33.4%。

请投票。此前的尝试因参与率不足而失败，无人反对，因此参与是该提案获批的唯一障碍。
...
**提案被否决：批准 devshard v5 及赏金发放**

提案 ID 为 104 的链上治理投票已结束。该提案被 **否决**，结果为 `failed_reason`：`proposal did not get enough votes to pass`。

最终计票：赞成 25291，反对 0，否决 0，弃权 0。

无人投反对票。参与率约为网络权重的 6%，低于 25% 的最低门槛，因此提案仅因参与不足而失败。
...
自第 394 个纪元起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供服务、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的 15%，并持续至您做出选择为止。
...
v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。提案通过本身不会移动任何流量，因此经纪人可以随后逐步迁移其网关。
...
该提案还支付了 91,300 USDT 的贡献者奖金，分配给七名接收方：
...
**提案已通过：补偿 Daniil 和 David Liberman 参加 All-In Summit 2026 的费用**

提案 ID 103 的链上治理投票已结束。

该提案已获**批准**。从社区金库中拨款 15,500 USDT，用于补偿 Daniil 和 David Liberman（Gonka 协议的联合创建者）参加 All-In Summit 2026 的两张参会通行证费用。

峰会于 2026 年 9 月 13-15 日在洛杉矶举行。标准参会通行证每人 7,500 USDT，门票已预先支付。本次补偿涵盖两张门票及提取资金的交易费用。本提案未申请报销机票、住宿及其他费用。
...
提案已获批准。Quant Mesh Limited被正式批准为Gonka-Ledger集成的法律签约合作方，从社区金库拨付350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费用75,000 USDT以及350 USDT的处理费用。授权与资金拨付严格限于完整提案中描述的Ledger集成。
...
该模型及其参数由kaitaku.ai团队提出，由vbgd0独立验证，并联合提出。测量数据及阈值设定的依据见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash的激活纪元为**394**。自该纪元起，`penalty_start_epoch`生效，因此未明确选择的主机将面临不参与惩罚。若计划提供该模型，尽早声明意向将有助于团队更好地组建。主机指南：[GLM-5.3-Flash启动指南](./host/glm-bootstrap.md)。
...
提案已获批准，`v4.1`已被添加至批准的devshard版本列表中。`versiond`将下载二进制文件，验证其SHA-256与链上存储值是否一致，并启动运行时。主机无需手动操作。

v4.1将与现有的v3和v4运行时并行运行，因此当前流量仍正常服务，经纪人可逐步迁移其网关。
...
投票将于**2026年9月10日23:17 UTC**结束。提案通过需满足：权重25%的出席率、多数赞成，且反对票低于33.4%且无否决权。
...
如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅关于将治理投票权限从冷密钥授予热密钥的指南。

## 2026年9月9日

**devshard v4.1运行时提案已进入治理**

提案ID 100将`v4.1`添加到批准的devshard版本列表中。

这仅是devshard的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond`将下载二进制文件，验证其SHA-256值是否与链上存储的值一致，并启动运行时。

v4.1与现有的v3和v4运行时并行运行。通过提案本身不会迁移任何流量，因此经纪人可在之后逐步迁移其网关。
...
该提案已获批准。它将从社区池中拨出100,000 USDT，用于支持Gonka参加2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金涵盖黄金赞助包及相关后勤费用：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
该提案已获批准。它将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提高至0.246。所有其他模型和链参数保持不变。这是提案97的重新提交。链上提案97最终以`PROPOSAL_STATUS_REJECTED`结束，`failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终票数：赞成15452，弃权3093，反对0，否决0。
...
Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务停机。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**关键变更**

1. v5的主要目标是高度同步：每个主机和序列器在保证金日志中保留一个已签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard现在可以确凿地知道cPoC何时运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height`/`observed_block_hash`标记已存在于所有先前包含时间戳字段的消息中。后续版本可将诸如`confirmed_at`和`started_at`等墙钟字段替换为高度标记。这解决了时间未来攻击问题，并为在协议层面处理流超时铺平了道路。本版本包含这些标记，但尚未将超时和封印决策切换至它们。

3. 已添加通过所有插槽往返的心跳机制，以便安静会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同的周期。

**行动项**

请审阅 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。

有意义的审阅贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一轮升级中获得社区奖励。
...
该提案已被批准。它调整了在周期初期数据不足时无效推断检查（SPRT）的行为，避免过早触发。主机无需执行任何操作。
...
在周期初期，数据尚不足，因此无效推断检查（SPRT）可能仅因少量无效样本就触发。本提案使其更稳定：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会被惩罚。真实违规者仍会触发（约在 360 个样本时为 18%，1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效样本，且在 500 个样本时，观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如有可能，请参与投票。如果未能在时限内达到紧急阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。如转为常规提案，您需重新投票。
...
一个已稳定的网关版本目前正运行在一个网关上，并将用于第一阶段的结算。其统计数据（代表了总网络流量的一部分）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses和Invalid指标将被用于链上。在当前主网阈值配置下，错误率低于10%的主机不会触发任何处罚或罚没，但仍应作为协议开发过程的一部分进行调查。
...
**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，首先从较小的模型开始。此过程将被密切监控，以尽量减少误报记录。Kimi将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 接下来的1-3天。** 将监控结算结果。

**4) 一旦本阶段稳定。** 其他经纪人可以部署已稳定的网关版本，并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至100%。

此 rollout 有意逐步进行，以便在结算扩展到整个网络之前发现并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731`与MiniMax-M2.7和Kimi K2.6一同成为活跃模型组，DeepSeek组中获得的PoC权重现在正以校准系数0.214（后由[提案98](#august-29-2026)提升至0.246）转换为共识权重。

针对DeepSeek的每模型参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需进一步操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（即正在服务Kimi或MiniMax的节点，因为所有已运行DeepSeek的节点均已使用3.0.16），请在方便时将这些节点升级至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。

MLNode镜像已在[`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)的`vllm-0.25.1-upgrade`分支上锁定为3.0.16。

默认MLNode镜像（3.0.16）基于CUDA 13.0构建。如果您的主机仍使用CUDA 12.9，请改用`3.0.16-cu129`标签。这是相同的MLNode 3.0.16，仅针对CUDA 12.9构建。两个标签均在同一个compose文件中。

升级后，请使用`mlnode-validate`进行验证。

## 2026年8月13日

**迁移社区销售和封装代币合约**

将活跃的社区销售合约和所有封装代币实例迁移至新存储的CosmWasm代码，并为未来实例化注册该封装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中识别出的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 启动快照——在此前请保持委托的多样性，以确保快照有效。
...
该技能现已支持 DeepSeek。它位于独立的 `./skills` 文件夹中，无需任何代理特定设置——将其符号链接到 `.claude/skills` 并运行 [`mlnode-validate` 技能](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/skills/mlnode-validate)。
```
...
如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免非参与罚金。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500个区块的安全窗口内（块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口期内切换是安全的，因为该期间无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可使用重新打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署该模型前，将发布并共享更新的API二进制文件。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加至网络。不修改任何现有参数或模型，唯一变更即新增该模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要MLNode运行vLLM 0.25.1并使用发布候选节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
1. 若要提供该模型：在投票结束后声明意向，然后配置您的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第360个周期前无需任何操作；从第360个周期起，若您未提供该模型，请为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更包括：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作线程的内存清理现在与纪元修剪对齐。

**升级计划**

devshard 运行时通过链上参数提案进行升级，而非完整链软件升级。若获通过，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动操作主机。

**如何投票**

如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参考从冷密钥向热密钥授予治理投票权限的指南。
...
该提案已获通过，并于主网区块 5316315 成功执行。该提案同时批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
v4 是首个面向多实例高可用的版本：在共享 Postgres 上，通过 versiond-router 后部署 N 个 versioned / devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
v4 的批准本身并不要求立即切换所有流量。在运行时启动并验证后，可逐步进行迁移。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近3个周期）。
...
- 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者——本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要理解一个语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，请提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
devshard 升级独立于主区块链进行。它们无需通过 Cosmovisor 协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件时（名称不变，仅 sha256 变化），versiond 可进行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含了错误修复和安全补丁。

**操作事项**

请审查 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并对任何发现、疑问、改进建议、边缘情况或潜在漏洞留下评论。
...
```
* 在第331轮PoC开始前约500个区块的窗口期内（区块 **5,105,776** — 2026年7月17日12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
* 委托主机：选择非守护者Kimi主机，并将权重分散至独立目标 — 详见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。

**devshard v1/v2已移除**

`versiond`已停止v1和v2的`devshardd`进程。所有网关流量必须使用`/devshard/v3`。若您的网关在此次变更后停止提供推理服务，说明您仍在通过已被移除的路径路由 — 请遵循v3迁移指南。

## 2026年7月16日

**紧急治理投票（提案88）：重新注册Kimi-K2.6并移除devshard v1/v2运行时**

这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册至治理模型列表，使其从第331轮开始进入标准引导流程。

同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。此举消除了旧运行时容器长期存在的内存增长问题 —— 即7月15日事件中网络节点OOM的根本原因。

**此操作使v3切换最终确定。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，将在提案通过后停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票 —— 紧急窗口期很短。
2. 提供Kimi服务的主机：在投票结束后立即、并在区块 **5,105,276**（2026年7月17日，约12:05 UTC）前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前约500个区块的窗口期内（区块 **5,105,776**，2026年7月17日约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**请勿委托给守护者节点**；将委托分散至独立的Kimi主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取更新的委托指引。
4. 经纪人 / devshard创建者：若您任何网关流量仍在v1/v2或`/v1/devshard`上 —— 请在投票结束前切换至`/devshard/v3`。
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（紧急，0.667通过阈值；投票率重要，请尽快投票）。
...
`moonshotai/Kimi-K2.6`在第328–329轮中失去了PoC验证多数（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现将其从活跃集合中移除并立即重新引导，是最快恢复服务且最小化停机时间的方式 —— 与6月提案78所用恢复路径相同。

该提案将在下一轮PoC前将`moonshotai/Kimi-K2.6`从PoC参数中移除，随后立即重新添加，Kimi将从第331轮开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票 —— 紧急窗口期很短。
2. 提供Kimi服务的主机：请保持Kimi设置就绪，并在引导PoC时准备好切换您的MLNode。
3. 在委托引导时：不要委托给守护节点；将委托分散到独立的Kimi主机上。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
投出您的投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced`版本v0.2.13或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
**Kimi-K2.6 事件（时段 328–329）：发生了什么、恢复计划以及委托指导变更**

在时段 328，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数支持。在时段 329 开始时，运行 Kimi 的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行 Kimi 的服务器同时故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题崩溃。大量 Kimi 委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器的网络节点因内存不足而失效。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者投票决胜机制未触发：目前没有模型级投票权重的守护者会被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与时段 306–307 的事件类似。时段 329 开始时的投票详细信息仍在确认中；本摘要可能存在小幅修正。
...
之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现被撤回：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
...
* 网络节点内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完整迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦加速的 Kimi 提案上链（后续公告），请立即投票。
2. 如果您计划提供 Kimi：关注引导说明，并在意图截止前准备好声明意图。
3. 如果您为 Kimi 委托：选择非守护者的委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量到 `/devshard/v3`。这将解决内存增长问题，且在移除 v1/v2 前必须完成。
...
提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换至 v3。

该版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这将使他们能够在链升级运行时继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期的运行时前缀，直到代理切换到 v3。在此类升级期间，无需主网重启或手动主机操作。
...
devshard 部分准备了 v3 运行时，使代理能够在链升级期间无需依赖已弃用的经典 API 路径提供推理服务，同时改进 RAM 利用率并启用 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
将从该分支单独发布 devshard v3，并在主网链升级前提出并部署。提前将推理流量切换到 `/devshard/v3` 的代理可在链升级运行期间继续提供推理服务。
...
本次仅呼吁对拉取请求进行审查，不启动正式投票。治理投票流程将在审查期结束后开始。
...
1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对任何发现、问题、改进建议、边界情况或漏洞留下评论。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供 Kimi 服务的主机需在第310轮期间声明其意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可以将权重委托给它。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换为 Kimi（PoC 开始时间约6月28日 ~12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定为：在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相当。此设定并非为了偏向任何硬件——MiniMax 上已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的增益。
...
常规启动流程从第309轮开始。由于投票将在 PoC 开始前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于启动的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定为：在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。此设定并非为了偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的增益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员能够在配置资源前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在 PoC 开始时实际提交 PoC 存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮开始（PoC 开始时间约6月26日 15:25 UTC；生效时间约16:00 UTC），前提是各模型满足启动资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能启动进入第309轮。由于每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计将支持本提案。

**主机所需操作**

1. **要提供 Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间没有 cPoC。
...
如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`查看：任何活跃节点均可使用：
...
**提案78已通过：`MiniMaxAI/MiniMax-M2.7`现为唯一PoC模型；Kimi K2.6和Qwen3-235B已被移除**
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍运行Qwen或Kimi的主机，将无法在本周期获得cPoC奖励，直至切换模型。
* 计划再次提供Kimi的主机应保持Kimi部署就绪，并在PoC 309周期随时准备将MLNode切换回Kimi——在第308周期将进行一次恢复Kimi的投票。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，以便主机可选择性参与，提前评估模型需求，不提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B被退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个周期。现在将其从活跃列表中移除并重新引导，是最快恢复其服务且最小化停机时间的方式。

加急提案现已上链，以便在下一个周期开始前生效。
链上提案在单次投票中执行以下操作：
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复服务且最小化停机时间的途径。下个周期将进行一次恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已提出的模型阵容调整。它与Kimi情况无关，仅因同为PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在第307个纪元结束前完成，因此这些变更被合并为一次紧急投票。每个纪元约22.9小时，标准的48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持该提案。

**若获批准的效果**

从第308个纪元开始（~UTC时间6月25日17:25），`MiniMaxAI/MiniMax-M2.7`将成为唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6将不再活跃。恢复Kimi将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如尚未就位），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **请在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi服务的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308纪元将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 在第308纪元进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**不设不参与惩罚**，主机可选择参与以提前测试模型需求，不提供该模型的主机不会受到任何惩罚。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
* 紧急提案需达到0.667的通过阈值；参与率很重要，请尽快投票。
...
- 将每个推理的验证计数器移出状态根，放入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个插槽的总计数。
...
请直接审核PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审核贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一轮升级周期中获得社区奖励。

本次仅为PR审核邀请，不启动正式投票。治理投票流程将在审核期结束后开始。
...
升级将在主网 **区块高度 4267300** 自动执行（约 **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一到两个模型的主机，仍需对其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。未采取任何操作的主机将从第 278 个纪元起，每纪元对其全部权重施加 15% 的惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。请确保 `.inference/data` 有足够的空闲空间用于 cosmovisor 状态备份；如果 `application.db` 较大，请在升级前应用 [cosmovisor 备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基线。此调整**仅适用于您共识权重中 Kimi 相关部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是最高收益选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，此次重新提交时包含若干更新。

- 包括：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专属章节）。
- 该提案将升级后的宽限期延长至 3000 个区块，以便主机在新快照逻辑稳定期间不被惩罚。
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。若守护者弃权，非守护者需在剩余 75% 中实现约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将 devshard 非法次数上限从 `20_000` 提升至 `1_000_000`。
- 将每纪元最大 devshards 数量从 `100` 提升至 `500_000`。
- 修复新模型引导期间确认 PoC 奖励核算。
- 在本升级纪元剩余时间内禁用确认 PoC，以便新快照逻辑从下一纪元干净启动。
...
若获得批准，二进制版本将通过链上升级提案更新。有关升级过程的更多信息，请参阅 [/docs/upgrades.md.](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.13/docs/upgrades.md)

**升级前的必要操作**

若提案获得批准，建议进行以下准备工作。

**在第 278 个周期（处罚自此开始）前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
请所有主机验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表盘/浏览器更新（升级前或升级后）**

请主机更新仪表盘/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先参考加入网络指南。此仪表盘更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约在 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首次 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，请提前确认）。有关安全删除 `.inference` 目录中旧备份的指南，请参阅 [文档。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
...
- 该提案将有意在升级高度到升级周期结束（10000区块宽限期）之间跳过确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过⅓），链上不会发生任何变化，升级将不会进行。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何措施。
...
第266轮的问题是由针对运行Kimi模型的主机的攻击引起的。网络继续运行，但该攻击与若干相关条件共同导致许多参与者无法进入第266轮。
...
2. 监控接下来轮次中的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时推理中断。访问可能不会立即在所有地方恢复，预计会通过社区代理和经纪人端点逐步恢复。
...
似乎PoC投票未能收集到本轮所需的票数。确切原因尚未确认，社区正在积极调查此情况。
...
社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更多明确信息，将及时分享更新。
...
- 将创世守护者调整后的投票权减少至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不参与投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果 `application.db` 占用了大量磁盘空间，可以应用 cosmovisor 备份[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
- 该提案将有意跳过从升级高度到升级纪元结束（3000个区块的宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个纪元）。
...
本次仅为拉取请求的审查请求，不启动正式投票。治理投票流程将在审查期结束后开始，很可能在明天。

**关键变更****inference-chain**

- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集合。在新模型引导期间，这可能导致同时服务了合格模型和未合格模型的诚实矿工被削减。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，并在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断会立即再次使其失效。现在在重新激活和即将晋升时重置该计数器。
- 在 v0.2.12 之前加入的 DAPIs 其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级将回填该权限，以便它们能够响应交易商投诉。
...
- 一些 OpenAI 兼容的上游返回数值型 `stop_reason`。`Choice.StopReason` 现在接受任何 JSON 类型，因此这些响应不再因反序列化失败。
...
- 当前纪元分片统计信息公开了 nonce、版本、组和每主机计数器。
...
在上一个轮次中，发现Kimi-K2.6在解析某些响应时存在一个轻微的bug。
...
我们建议替换api容器的二进制文件。除了修复之外，新版本还启用了对devshard数据库的修剪功能，并为devshard状态添加了Postgres支持。
...
安全模型保持不变：PoC验证本身未作更改，仍需超多数验证算力才能接受结果。
...
`moonshotai/Kimi-K2.6`已完成启动，并加入Gonka网络的PoC参与。
...
预评估与PoC开始之间有500个区块的窗口期。在此期间，没有CPoC任务，因此已声明意向的主机可安全地将其模型节点切换至 `Kimi K2.6`。

请遵循指南完成所需部署步骤：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年5月4日

转移代理 `node1`、`node2` 和 `node3` 已被禁用。所有主网推理现在均通过 `node4` 路由，该代理基于新的 `devshard` 计费方式运行。
...
今天的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保留您的MLNode使用 `Qwen235B`，并于明天为下一轮提交意向。

## 2026年4月30日

**升级已执行：v0.2.12 现已在主网上线**

关于升级提案v0.2.12的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，将遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
每个 DAPI 在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。

版本 0.2.11 通过将运行时视图截断为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种截断，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送带有清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。剩余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需要手动修复。
...
如果您没有直接访问具有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何从冷密钥向热密钥授予治理投票权限。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位全熵，之前为 32 位），将通过独立的治理投票激活。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG dealer 共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
更新修复了 PoC 开始附近的一个边缘情况，该情况此前可能导致在特定条件下性能下降。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过每个推理的链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅处理会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本并非因其已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，这使得每个区块可容纳最多 100 倍的推理次数，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能同时开始大规模剪枝，造成不必要的运营压力。
...
这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被认为完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳多达 100 倍的推理数量，具体取决于工作负载和网络状况。  ￼

**升级前的建议操作****`application.db` 剪枝**

强烈建议主机在升级前根据提供的说明剪枝 `application.db`。
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间大规模启动剪枝活动，造成可避免的操作压力。
...
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代表其投票。若无此授权，其他账户无法提交投票。
...
1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格 5 分钟超时，而某些请求可能需要更长时间。新版本的 API 将恢复此值，而非强制执行严格限制。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍有活动请求而导致潜在权重下降的风险。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时，存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
由于有效归一化窗口比先前的名义模型长约5个区块，因此每个周期重新计算的权重成比例降低。
...
```
过渡到 `mlnode:3.0.12-post4-*` 应在升级中引入的3000区块宽限期内完成。
...
**宽限期：** 升级引入了一个宽限期，在升级后 3000 个区块内无确认 PoC，并且在升级时段内对错过率和无效率阈值要求更宽松。
...
这意味着在单个周期内，即使被惩罚，矿工损失的抵押品也不会超过 0.5%。所需抵押品仅占日收益的约 24%。

**警告：** 抵押参数将不受投票结果影响而生效。如果该提案未通过，创世时定义的抵押参数将在第 180 个周期自动激活，而非上述参数。

投票结束后、第 180 个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 个周期起，其收益将被削减 5 倍。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个为期3000个区块的宽限期，在此期间无确认PoC，并对升级所在纪元放宽了错过率和无效率阈值。
...
- 在升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
根据AMA期间的讨论及社区成员提出的论点，建议从较低的抵押要求和最小的惩罚开始。
...
这意味着即使在一个纪元内被惩罚，矿工最多仅损失其抵押品的0.5%。所需抵押品仅占日收益的约24%。

提案提交投票后将另行发布公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世时定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有充分理由时适用。如果稳定性得到证实，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请在GitHub上审阅v0.2.10升级PR并提供您的反馈。有意义的审查贡献可能在下一次升级中获得奖励。

[https://github.com/gonka-ai/gonka/pull/695](https://github.com/gonka-ai/gonka/pull/695)

## 2026年2月13日

如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止。这是因为节点运行的是与网络不再兼容的旧版二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
在当前周期中观察到cPoC（确认PoC）与PoC的重叠。直到该周期的最后一个区块，`is_confirmation_poc_active`被观察为`true`。
...
链上推理访问目前对所有人开放，不再仅限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
本次仅为对拉取请求的审查请求，而非正式投票的开始。治理投票流程将在审查期结束后启动。
...
此升级通过实际 PoC 已用时间对参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- **[PR #712](https://github.com/gonka-ai/gonka/pull/712)** 要求 proto-go 生成代码与提交代码匹配
...
2. 在社区审查该PR后，预计从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，以启动正式的治理投票流程。
3. 如果链上提案通过，预计在链上升级执行后合并该PR。
...
- 观察到的变化表明，GPU 权重差异现在反映的是模型特定的推理吞吐量，而非名义上的硬件规格。例如，H100 PCIe 权重的下降幅度大于 H100 HBM3 权重，这与 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的观察到的推理行为一致。
- 在当前模型配置下，B200 GPU 的推理性能并未高于 H100 级 GPU，基于观察到的推理轨迹。
- 如果未来轮次通过治理引入更大或更复杂的模型（例如 DeepSeek V3.2），可能会观察到不同的性能特征。
- 在 PoC 外部使用标准 vLLM 基于相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与 PoC v2 中观察到的 GPU 类型间相对性能差异一致的结果。

**追踪器（仪表板）维护者操作**

在更新的权重分配生效后，追踪器（仪表板）维护者可能希望审查第 159 轮及之后的系数，以确保其与当前 PoC v2 权重分配保持一致。

## 2026 年 2 月 2 日

**网络更新 — 补丁可用**

现已提供补丁以解决导致 PoC 周期中区块验证暂停的问题。建议主机尽快应用补丁，以确保正确的 PoC 验证行为并安全恢复区块生成。

**需要采取的操作**

请主机尽快应用补丁，以确保正确的 PoC 验证行为并安全恢复区块生成。
```
# Download Binary
...
根据当前评估，用于处理此情况的机制可能无法正常运行。为防止在不确定或不安全的条件下完成验证器最终确认，网络在验证器选择之前已停止运行。
...
- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时执行后续步骤或缓解措施。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将来临的纪元的参与集合中。
...
在因 cPoC 处罚而减少或排除奖励的情况下，未计入的部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
Upgrade Proposal v0.2.8 的链上治理投票已结束。该提案已获批准并成功在主网上执行。
...
- 身份验证绕过：将 `epochId` 绑定到签名，以验证是否属于正确的周期。
...
!!! note 
    未能同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明了主机应如何根据v0.2.8模型可用性变更和即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8`在迁移期间受支持，但不参与PoC v2就绪或权重分配。参与PoC v2需要提供`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未同时满足两项条件的节点，在网络切换至单模型配置后将不再有资格。
    - ML节点升级必须在迁移完成、并通过v0.2.8升级后独立治理提案激活PoC v2之前完成。
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。所有此前支持的模型均从活跃集合中移除，除了：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
- 授权绕过与重放：将 epochId 绑定到签名，并验证授权是否针对正确的 epoch。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不被认定为 PoC v2 参与资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
- 逐步部署：如果您运行多个 ML 节点，建议在多个纪元中逐步执行这些更新。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步执行更新。
...
`Allowlist`过期时间：自动在区块2443558时过期。
...
2) 之前表现出与这些模式持续偏差的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块 2,222,222 结束。
...
如果获得批准，网络将临时运行一个 `allowlist`，包含在先前周期中未表现出非常规硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已根据预定义的常见硬件配置模式进行评估，用于识别偏差和不一致（非常规配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，且
- 观察到的PoC权重低于使用同类硬件的其他参与者所表现的权重的150%。
之前表现出这些模式持续偏差的参与者，在稳定窗口于区块 2222222 结束前，不包含在 `allowlist` 中。
...
链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强的提案已获批准，并成功部署于主网。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——此前曾使用过的、具有有限持续时间的防御机制，现提议重新启用。
...
此次升级正式固化了此前通过手动API更新分发并已在网络中使用的关键修复。这些修复包括：
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
近期网络增长带来了若干挑战。在过去几天中，网络经历了多次问题，其中一些似乎是由故意试图破坏或施压系统所导致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的韧性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内生效。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
- 主机级注册：在区块2.222.222之前（约两周后）暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。一个预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者准入和推理执行提供了支持，而无需额外的软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第117轮奖励分配**

本提案涵盖与链暂停（第117轮）相关的两项奖励分配：

- 在第117轮期间活跃但未收到该轮奖励的节点，将获得该轮的补发奖励。
- 所有在第117轮期间活跃的节点，将额外获得等于1.083倍第117轮奖励的支付，统一应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个漏洞：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
