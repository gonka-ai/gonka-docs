...
作为协议开发的重要里程碑，链现已准备好逐步启用 devshard 结算，以提升网络的安全性和可靠性。

一个稳定的网关版本目前在单个网关上运行，并将用于第一阶段的结算。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何处罚或 slashing，但仍应作为协议开发过程的一部分进行调查。
...
**下一步：****1) 未来24小时。** 主机应检查其统计数据并解决任何关键错误或配置错误。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上开始手动控制的结算，首先针对较小的模型。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后跟进，因为其数据在负载下目前可靠性较低。

**3) 接下来的1-3天。** 将监控结算结果

**4) 一旦此阶段稳定。** 其他经纪人可部署稳定的网关版本并开始以受控模式进行结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至100%。

此次发布刻意采取渐进方式，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 已在 Gonka 网络上激活**

在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214 转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需做任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版 MLNode（正在服务 Kimi 或 MiniMax 的节点，因为所有已运行 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。

MLNode 镜像在 `vllm-0.25.1-upgrade` 分支的 [`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 中固定为 3.0.16。

默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是相同的 MLNode 3.0.16，仅针对 CUDA 12.9 构建。两个标签均位于同一 compose 文件中。
...
将正在使用的社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时错过预资格。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 引导快照——在此前请保持委托多样化，以确保快照有效。
...
DeepSeek 已通过投票进入引导阶段——如果您计划提供该模型，请立即验证您的设置并提交意向。
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行对比。若通过，则一切正常。

如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免未参与处罚。

提醒：明天将发布另一个具有更好日志的MLNode构建版本，因此请留意后续简短通知。
...
从第359个纪元开始（PoC启动时间约为2026年8月13日03:24 UTC / 8月12日20:24 PDT），前提是模型满足启动资格：

- `deepseek-ai/DeepSeek-V4-Flash-0731` 变为可用的PoC模型。
- `MiniMaxAI/MiniMax-M2.7` 仍然是基础模型。
- `moonshotai/Kimi-K2.6` 仍然可用且未更改。

**主机所需操作**

1. 声明意向。由于提案 94 已通过，且在区块 5,536,224（~8月13日，~02:40 UTC / 8月12日 ~19:40 PDT）的引导快照之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 在PoC 359开始前的约500个区块安全窗口内切换你的MLNode，将其配置为DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate配置），该窗口对应区块5,536,724（约8月13日，03:24 UTC / 8月12日 20:24 PDT）。此窗口内切换是安全的，因为其中没有cPoC。
注意：在Blackwell GPU上，为获得最佳性能，你可以使用重新打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在引导前发布并共享更新的API二进制文件。
3. 如果你委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留你当前的模型。在epoch 360之前无需任何操作。从epoch 360开始，如果你未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的未参与罚金。

区块高度是精确的；时间是估算值，将在接近epoch时进一步确认。[更多关于DeepSeek引导的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。

## 2026年8月10日

**治理投票（提案 94）：添加 DeepSeek V4 Flash**

将 `deepseek-ai/DeepSeek-V4-Flash-0731` 添加到 PoC 模型阵容的提案现已上链并开放投票。
...
- 需要在vLLM 0.25.1上运行并使用release-candidate节点配置的MLNode（`node-config-release-candidate-*.json`适用于B300 / B200 / H200 / H100）
...
根据模型使用情况，社区之后可决定逐步用 DeepSeek 替代 Kimi 或 MiniMax（例如，提高系数以激励将所有 B 系列 GPU 转向 DeepSeek）。
...
在符合引导资格的前提下，从epoch 359（PoC开始时间约为2026年8月13日 03:24 UTC / 8月12日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与罚金从epoch `360`（`penalty_start_epoch = 360`）开始生效。

**主机所需操作**

1. 要提供该模型：投票结束后声明意向，然后配置你的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留你当前的模型：在epoch 360之前无需操作；从epoch 360起，如果你未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`。
3. 在截止日期前对提案94进行投票。

**如何投票**

如果你无法直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
- API。
    - `/v1/versions`现在返回增强的DAPI版本信息，包括ML节点详情
    - 默认的Node Manager gRPC端口现已改为9400，以确保与自定义配置的兼容性
- 部署。默认的加入堆栈配置已更新为使用最新的`v0.2.15-post2` API镜像
...
这只是一个性能和资源管理补丁，除此之外无其他改动。协议、推理、验证和经济行为均保持不变。变更内容：纪元追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。
...
合并的 **devshard v4 和 v0.2.15 主网升级提案** 现已在链上开放投票。
...
v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是将单体架构重构为高可用、容错、可扩展架构的一系列 devshard 和网络节点变更中的首次更新。

v4 是首个专为多实例高可用设计的版本：在共享的 Postgres 上，通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。同时，通过移除过时行为并将交易处理集中于当前支持的架构，使未来的开发和升级更加容易。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间（主网中Cosmovisor对`application.db`的备份通常为数十GB，建议提前确认）。有关如何安全删除`.inference`目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用了大量磁盘空间，可以应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
...
devshard部分为v3运行时做准备，使代理在链升级期间无需依赖已弃用的经典API路径即可提供推理服务，从而提升RAM利用率并支持SQLite与Postgres存储之间的安全切换。
...
- 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像升级至 0.2.14-post3。如果您的 `api` 二进制文件已更新，您只需更新桥接器镜像并重启桥接容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护者 — 此次升级未移除或重塑任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 `application.db` 的 Cosmovisor 备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放供审查**
...
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond / devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
* 在区块 **5,105,276**（2026年7月17日 ~12:05 UTC）前声明意图：
```
...
这是 7 月 15 日宣布的 Kimi 恢复计划的第二步。提案 87 将 `moonshotai/Kimi-K2.6` 从活跃集合中移除；提案 88 将其重新注册到治理模型列表中，使其从周期 331 开始进入标准引导流程。

同一提案还从 `devshard_escrow_params` 中移除了 `approved_versions` v1 和 v2。一旦批准，`versiond` 将停止 v1 和 v2 的 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。这消除了旧运行时容器长期存在的内存增长问题 —— 即 7 月 15 日事件中网络节点 OOM 的根源。

**这标志着 v3 切换的最终完成。** 仍通过 v1/v2 前缀或经典 `/v1/devshard` 路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 进行投票 —— 紧急窗口较短。
2. 提供 Kimi 服务的主机：在投票结束后、区块 **5,105,276**（2026年7月17日 ~12:05 UTC）前立即声明 `moonshotai/Kimi-K2.6` 意图，然后在周期 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776**，2026年7月17日 ~12:50 UTC）切换您的 MLNode。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在进行引导委托时：**请勿委托给守护节点**；将委托分散到独立的 Kimi 主机上。有关最新的委托指南，请参阅 [多模型概念验证指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 经纪人 / devshard 创建者：如果您的网关流量仍使用 v1/v2 或 `/v1/devshard` —— 请在投票结束前切换至 `/devshard/v3`。
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（加速投票，0.667 赞成阈值；参与率重要，请尽快投票）。
...
`moonshotai/Kimi-K2.6` 在第 328 至 329 个周期失去了 PoC 验证多数支持（详见 [此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其在最小停机时间内恢复的最快方式——与六月提案 78 的恢复路径相同。

该提案将在下一个 PoC 之前将 `moonshotai/Kimi-K2.6` 从 PoC 参数中移除，随后立即重新添加，Kimi 将从第 331 个周期开始进入标准引导流程。

**主机所需操作**

1. 请在截止日期前对提案 87 投票——加速窗口很短。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（PDT）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（加速投票，0.667 赞成阈值；参与率重要，请尽快投票）。
* 引导意向截止时间和周期时间请参阅引导说明。

## 2026 年 7 月 15 日

**Kimi-K2.6 事件（第 328–329 个周期）：发生了什么、恢复计划及委托指南变更**

在第 328 个周期，`moonshotai/Kimi-K2.6` 失去了 PoC 验证多数支持。在第 329 个周期开始时，运行 Kimi 的主机被移出组。其余网络正常运行。

**发生了什么**

两台由守护节点运营的运行 Kimi 的服务器同时出现故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而停止。大量 Kimi 委托集中在这台主机上，因此其故障导致这些投票立即消失。
* 第二台服务器因内存耗尽而失去其网络节点。

Kimi 的验证投票一直接近 2/3 的阈值，因此这足以使该组低于多数。守护者决胜机制未启动：当前未赋予每模型投票权重的守护者会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与第 306–307 个周期的事件类似。第 329 个周期开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 的最安全方式是通过全新引导程序重新添加它（约 1.5 个周期）：

1. [今天] 一个加速提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布操作说明。

**委托指南变更——请阅读**

先前的指南（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已撤销：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，并消除单点故障。

**已在开发中的修复*** PoC 验证中的守护者投票（守护者始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期的 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完成全部迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。
...
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这能解决 RAM 增长问题，并在移除 v1/v2 之前必须完成。
...
提案通过后，v3 现在与现有的 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人将流量切换到 v3。

该版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需重启主网或手动执行主机操作。

**操作项**

白名单 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这使他们能在链升级运行期间继续提供推理服务，而不依赖已弃用的经典 API 路径。
...
投出您的票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 v0.2.13 或更高版本的 `inferenced`。
```
export NODE_URL=https://node3.gonka.ai/
...
主链/API 工作重点包括：PoC 重复工件防护、早期共享检测、经典推理 API 废弃（在主网上禁用 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收者路由，以及升级时的安全修复。
...
节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布并部署 devshard v3 版本，再进行主网链升级。提前将推理流量切换到 `/devshard/v3` 的代理，可以在链升级运行期间继续提供推理服务。
...
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为在主网升级期间保持以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两步，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 代理 — 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 — 准备调整指标统计方式。详细说明将在 `devshard v3` 投票启动并接近结束时发布。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证，阻止重复计算通过采样检查，从而阻止攻击。
...
1. 在第 310 个纪元、区块 **4,797,456**（约 6 月 28 日 12:00 UTC）之前声明 PoC 意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若足够多的主机声明意图，请在第 311 个纪元开始前约 500 个区块的窗口期内（PoC 开始于约 6 月 28 日 12:47 UTC）将节点切换为 Kimi。
...
> **更新（2026年7月15日）：此建议已撤销——请勿委托给守护节点。** 守护节点是 PoC 验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。有关更新的委托指南，请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相同。此设计并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相同。此设计并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 服务的主机需在 `start_poc - deploy_window` 的启动快照前为对应模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员能够在配置硬件前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置 MLNode。
   
4. **PoC开始。** 成员资格由在PoC开始时实际提交PoC存储提交的主机决定——而非仅凭意向声明。只有满足资格条件的模型才会被激活。

**若获批准的影响**

从第309轮开始（PoC开始时间约为6月26日15:25 UTC；生效时间约为16:00 UTC），前提是各模型满足引导资格条件：

* `moonshotai/Kimi-K2.6` 再次成为活跃的PoC模型。
* `zai-org/GLM-5.2-FP8` 可作为可选的PoC模型，无非参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计会支持本提案。

**主机需采取的操作**

1. **若要提供Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。

2. **若要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口期间配置您的MLNode。提供GLM-5.2是自愿的——未选择的主机不会受到惩罚。
...
如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何从冷密钥向温密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 访问，任何活跃节点均可使用：
...
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并准备好在PoC 309时将MLNode切换回Kimi——将在第308个纪元进行一次恢复Kimi的投票。
...
* **添加GLM-5.2** —— 添加GLM-5.2 **不设不参与惩罚**，以便主机可选择加入，并提前衡量对该模型的需求，选择不提供该模型的主机不会受到惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重新引导；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快恢复且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6 — 移除以便快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的途径。下一纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立、无关变更）。** 退役较旧的Qwen3-235B是一项独立的、此前要求的阵容变更。它与Kimi情况无关，仅因同样是PoC阵容变更而被包含在此处。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307个纪元结束前完成。每个纪元约22.9小时，标准的48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持该提案。

**若获批准的效果**

从第308个纪元开始（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可安全切换，因为此期间无cPoC。请现在预下载FP8权重（如尚未就绪），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——将在第308个纪元进行一次恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2 **而不施加不参与惩罚**，以便主机可选择加入并提前测试对该模型的需求，选择不提供该模型的主机不会受到任何惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何从冷密钥向热密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
**截止日期*** 提案78投票截止：**2026年6月25日 15:39:53 UTC**（紧急）。
...
devshard v2运行时已通过链上批准并添加到`DevshardEscrowParams.approved_versions`中。
...
提案通过后，v2可与现有的v1 devshard运行时并行运行。新进程在 `/devshard/v2` 前缀下提供服务，而现有v1流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为Gonka发布工件。`versiond` 将自动下载该二进制文件，验证sha256哈希值，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。
...
- 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个槽位的总计数。
...
4. 将每个推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每个槽位的总计数
...
发布版本将 `devshardd` 二进制文件作为Gonka发布工件。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证sha256哈希值，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。
...
**[仅用于devshard升级的PR](https://github.com/gonka-ai/gonka/pull/1289) 现已开放审核。**

这是首次仅针对devshard的升级，因此流程不同于标准链升级。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响主网行为，且不会导致推理服务停机。

如果通过治理流程获得批准，新的devshard版本将与现有的v1运行时并行运行。

请直接审阅PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。
...
MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已**通过**，升级已在主网`4267300`区块成功执行。

## 2026年5月25日

**升级 v0.2.13：提前下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13升级提案（提案ID [54](https://github.com/gonka-ai/gonka/pull/1143)）已通过链上治理，升级已安排。
...
对[Upgrade Proposal v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案ID `54`）的链上治理投票已结束。该提案已**通过**。

升级将在主网**区块高度4267300**（≈ **2026年5月26日周二 14:42 UTC** / **07:42 PDT**）自动执行。

**提醒**

1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT代币元数据以及CW20 `wrapped_token`代码ID `105` 均通过升级处理器注册，因此桥接将在升级高度在主网上激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在提前下载约230 GB的FP8权重。在引导窗口期间，Hugging Face的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6`和`MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元 `278`**。不采取任何行动的主机将从纪元278起，对其全部权重施加15%的每纪元惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。请确保 `.inference/data` 有足够的空闲空间用于cosmovisor状态备份；如果 `application.db` 较大，请在升级前应用[cosmovisor备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13校准将Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映Qwen-on-B200参考的vLLM-0.20.1吞吐量基线。该调整**仅适用于您共识权重中的Kimi部分**；您的Qwen部分权重和Kimi内部PoC分布保持不变。在B200/B300上，Kimi仍是收益最高的选项；在H100/H200上，MiniMax-M2.7成为与Qwen相当、高于Kimi的选项。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过的提案的重新投票，现已提交多个更新。

- 包括：Kimi权重重新校准（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专属部分）。
- 该提案将升级后的宽限期延长至3000个区块，以便在新快照逻辑稳定期间，主机不会受到惩罚。
- 治理：将创世守护者投票权降低至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%的投票权中实现约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 在vLLM 0.20.1发布后，根据Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B：`0.359`（未变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下，Kimi的每轮共识权重减少了约38%）
    - MiniMax-M2.7：`0.3024`
...
- 修复可能导致桥接和流动性池合约调用出现间歇性权限错误的连接问题。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正常。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表盘/浏览器更新（升级前或升级后）**

主机需更新仪表盘/浏览器。请从`gonka/deploy/join`目录运行以下命令。如果您尚未本地克隆`gonka`仓库，请先遵循加入网络指南。此仪表盘更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 运营商时间线：投票截止于5月22日22:12 UTC → 升级高度约5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在 start_poc − 500 块处（约提前43分钟）→ 首个MiniMax PoC阶段在升级后下一个纪元边界开始 → MiniMax惩罚在链纪元278执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中Cosmovisor对 `application.db` 的备份通常为数十GB，建议提前确认）。有关安全删除 `.inference` 目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用大量磁盘空间，可应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获批准，此跳过属预期行为，并非故障；新快照逻辑将在下一个纪元开始。
- 若获批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite仍将为默认选项，并自动清理（保留最近3个纪元）。
- 若提案未通过（未达法定人数或 `no_with_veto` 超过1/3），链上无任何变更，升级不会发生。运营商可能看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
纪元 #267 中PoC验证已成功完成，受影响的主机已恢复至活跃集合。

纪元 #266 的问题由针对运行Kimi模型的主机的攻击引起。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入纪元 #266。
...
由于现有协议规则，Kimi在纪元 #267 中的活跃权重较低：单个模型的总权重不得超过前一纪元所有模型总权重的75%。
...
5. 在社区频道或本帖中分享相关日志或观察结果。如果您的主机在第266个周期受到影响，或在后续周期中行为异常，这一点尤为重要。
...
似乎PoC投票未能收集到本周期所需的票数。确切原因尚未确认，社区正在积极调查此情况。
...
如果您有能帮助调查的相关观察、日志、假设或其他技术背景，请在此分享。
...
- 包括：为Kimi（`0.78`）重新校准权重、新增模型 `MiniMaxAI/MiniMax-M2.7`、更新验证阈值、devshard存储重构，以及多项PoC/奖励修复和Ethereum桥接主网激活。
...
- 修复新模型启动期间确认PoC奖励的核算问题。
- 在本次升级周期剩余时间内禁用确认PoC，以便从下一个周期开始干净地启用新的快照逻辑。
- 当参与者再次激活时，重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
...
**截至第 278 周期的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正常。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意跳过从升级高度到升级周期结束（3000个区块的宽限期）的确认PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊侧合约激活，所有主机需验证其桥接容器已部署、运行最新版本并正确同步。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/1143) 针对下一个链上软件升级 v0.2.13 已开放审查。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一个新产生的不良推断会立即再次使其失效。该计数器现在在重新激活和即将晋升时重置。
- 在v0.2.12之前加入的DAPI在其冷到暖的授权授予中没有`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应交易商投诉。
...
- 由于旧的托管数据保留在一个SQLite存储中，devshard存储可能无限增长。现在存储按纪元范围管理，并在后台清理旧纪元，仅保留最近3个纪元。
- Devshard需要为更大规模部署提供共享存储选项。现在可以使用Postgres作为主存储，SQLite仍作为本地备用存储。
- Postgres数据按`epoch_id`对会话、差异和签名进行分区，以便清理可以干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
- 负载查找固定于托管纪元，并为纪元边界和遗留纪元0请求提供回退机制。
- 当前纪元分片统计信息暴露了nonce、版本、组和每个主机的计数器。

**bridge**

- 桥接器工具处理 Sepolia 标志，并将 Gonka BLS 密钥/签名转换为以太坊合约期望的 EIP-2537 格式。
...
在api容器版本v0.2.11、v0.2.12和v0.2.12-api-post2中存在一个潜在问题。容器重启后，9100、9200和9400端口上的服务器可能启动延迟很长。这延迟了api激活，导致部分矿工跳过确认PoC。
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化更多模型的接入，提议将该阈值降低至10%。

安全模型得以保留：PoC验证本身未发生变化，仍需超多数验证算力才能接受结果。
...
- **协议加固**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。

**主机指南**

- 部署、委托或明确拒绝新的治理批准模型（所包含模型将在升级后两个周期激活）。请参阅[指南。](https://gonka.ai/docs/host/multi_model_poc/)
...
鼓励主机查阅 [GitHub](https://github.com/gonka-ai/gonka/pull/948) 上的提案并参与投票。
...
鼓励主机查阅 [GitHub](https://github.com/gonka-ai/gonka/pull/948) 上的提案并投票。

**升级前必须执行的操作**

随着网络接近升级窗口，主机应提前准备节点，以防提案通过。

此清理过程**必须在升级发生前完成**。如果在升级时您的节点配置包含不支持的模型，**节点将被拒绝并下线。**

版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留此前强制执行的模型和 Kimi。
每个 DAPI 都会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不支持的模型，整个节点将被拒绝，主机将下线。
...
[升级提案](https://github.com/gonka-ai/gonka/pull/948) 的下一个链上软件版本 v0.2.12 已发布至链上，现开放投票。

**主要变更**

- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为每模型 PoC 组。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：模型组将在升级后两个周期激活。该模型的系数为 Qwen235B 的 3.51 倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI / 主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议强化**。保留的节点（`POC_SLOT=true`）将随机采样用于单 PoC / CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、对齐遗留验证者 slashing 与所需抵押品语义、确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
从 `docker-compose.yml` 部署 `versiond` 和 `proxy` 服务的最新版本（使用 tag release/v0.2.12 的仓库）：
```
...
- **协议加固**。实施更强的 PoC v2 RNG（完整 256 位熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- 一旦新模型获得治理批准，部署、委托或明确拒绝每个经治理批准的模型（包含的模型将在升级后3个周期内激活）
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于接近 PoC 开始时间运行的节点，稳定性有所提升。

此次更新修复了在PoC启动附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
v0.2.11 升级提案的链上治理投票已结束。该提案已获批准，升级已在主网上成功执行。

**当前生效的关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此次升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理扩展性。

**[`StartInference` 和 `FinishInference` 性能提升](https://github.com/gonka-ai/gonka/pull/812)**
...
建议主机审阅 [GitHub](https://github.com/gonka-ai/gonka/pull/813) 上的提案并投票。
提前下载二进制文件有助于避免在升级窗口期间依赖 GitHub 的可用性。
...
下一代链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间大规模启动剪枝活动，从而造成可避免的操作压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
如果您无权访问持有投票权的冷钱包密钥，建议提前安排投票委托。请联系该密钥的所有者，请求授权您代为投票。若无此授权，其他账户将无法提交投票。
...
API 二进制文件的新版本已发布。该版本更新了连接超时处理，并在 PoC 验证流水线中引入了额外检查。

1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格5分钟超时，而某些请求可能需要更长时间。新版本的API将此值恢复，而不是强制执行严格限制。
2. 以前的请求重试系统即使因处理超时（而非TLS超时）失败也会重试推理。
服务器端对长请求的重试通常无效，因为会导致相同的超时场景。同时，客户端可能收到不一致的输出。新版本的API在这种情况下不再重试推理。
...
现在提供一个新的 vLLM / mlnode 构建，可在 PoC 开始时中断正在进行的推理请求，以降低在 PoC 启动时仍处于活跃状态的请求可能导致权重下降的风险。
...
抵押参数更新提案已结束，但未达到法定人数。因此，根据当前治理规则，该提案被拒绝。这意味着更新后的参数将不会被激活。

如前所述，第 180 个周期的抵押激活独立于此次投票。

由于提案未通过，创世时定义的抵押参数将在第 180 个周期自动生效。

参与者应：

- 审查创世时定义的抵押参数。
- 在第 180 个周期前准备并存入所需的 GNK。
- 确保 [抵押](https://gonka.ai/host/collateral/) 设置正确，否则从第 180 个周期开始，PoC 奖励将减少 5 倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各周期间可能波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求 2 倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为了将 PoC 权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的 PoC 参考窗口比先前的名义假设长约 5 个区块。
...
由于有效的归一化窗口比早期名义模型长约 5 个区块，每个周期重新计算的权重相应降低。

**观察到的 GPU 权重变化（第 175 周期 → 第 176 周期）**
...
!!! note "兼容性说明"
    ...
    **按实时时间归一化 PoC 权重**
    ...
    此次升级通过实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- 锁仓：支持带有活跃锁仓的 streamvesting 转账（PR #641）。
...
**宽限期：** 升级后引入一个3000个区块的宽限期，期间不进行确认 PoC，并对升级当期的错过率和无效率阈值采取较宽松的标准。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的 0.5%。而所需抵押物仅占日收益的约 24%。

**警告：** 抵押参数将 Regardless of 投票结果生效。如果此提案未通过，创世时定义的抵押参数将在第180个周期自动激活，而非上述参数。

投票结束后、第180个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180个周期开始，其收益将减少5倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获通过，该提案将引入一项对 PoC 验证的重大优化（默认关闭），并实现基于实时的权重归一化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，期间无确认PoC，并在升级时段放宽了漏报率和无效率阈值。
...
如AMA期间讨论并基于社区成员提出的论点，建议从较小的抵押要求和最低惩罚开始。
...
警告：抵押品将无论提案投票结果如何均生效。如果本提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到证实，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止。这是由于节点运行的是与网络不再兼容的旧版本二进制文件。请按照本指南恢复：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
请保持在线，以便在需要时立即应用补丁。更多细节和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已可用**

链上推理访问现已开放，不限于开发者。推理请求可通过上一更新中引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
对于对此次PR审查有实质性贡献的，可能在下一次升级中提出奖励。
...
**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际PoC时间归一化PoC权重**

此升级通过实际PoC经过时间对参与者权重进行归一化，以减少区块时间漂移效应，并使权重结果与实际执行时长保持一致。
...
2. 在社区审查该PR后，预计将从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，以启动正式的治理投票流程。
...
现已提供补丁以解决导致PoC周期中块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为，并安全恢复块生成。

**需采取的操作**

请主机尽快应用补丁，以确保正确的PoC验证行为，并安全恢复块生成。
```
# Download Binary
...
由于在当前的PoC周期中验证阈值可能无法达成的高风险，主机们集体采取了预防性措施，暂停了区块验证。
根据当前评估，用于处理此情况的机制可能无法按预期运行。为防止在不确定或不安全的条件下完成验证器最终确认，网络在验证器选择前已停止运行。
...
- 身份绕过：将 `epochId` 绑定到签名，以验证是否属于正确的周期。
...
本指南说明主机应如何根据 v0.2.8 模型可用性变化及即将推出的 PoC v2 更新来升级其 ML 节点。ML 节点配置对 PoC v2 的合规性自第 155 个周期开始生效。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个周期后安排。迁移阶段结束后，不符合配置要求的 ML 节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8` 在迁移期间受支持，但不参与 PoC v2 就绪或权重分配。参与 PoC v2 需提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
!!! note "重要"
    ...
    - 仅提供正确模型而未升级 ML 节点是不够的。
    - 未同时满足两项条件的节点在网络切换为单模型配置后将失去资格。
    - ML 节点升级必须在迁移完成且 PoC v2 通过 v0.2.8 升级后单独的治理提案激活之前完成。
...
!!! note "治理和 PoC v2 激活说明"
    ...
    **第一阶段：观察（v0.2.8 之后的当前状态）**
    ...
    - 网络将观察采用情况，以评估主机为过渡到 PoC v2 权重做好准备的程度。
...
您的审阅与投票对确保网络的稳定性和未来功能至关重要。

**v0.2.8 的关键变更****PoC v2（核心升级）**

- 将 PoC 直接集成到 vLLM 中，无需卸载模型或加载单独的 PoC 模型，即可立即从推理切换至 PoC。
- 使用 MMR（Merkle 山脉范围）承诺将工件存储移至链下——仅在链上记录 root_hash 和 count。
- 包含双模式迁移策略：V1 用于常规 PoC，V2 用于部署期间的确认 PoC 跟踪。

**模型可用性变更**

作为 v0.2.8 升级的一部分，支持的模型集已更新。所有此前支持的模型均从活跃集合中移除，但以下模型除外：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML 节点版本，可用于评估向 PoC v2 过渡的准备情况。一旦在活跃主机中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML 节点不足以参与 PoC v2。在网络切换至单模型配置后，不满足两项条件的节点将不被视作 PoC v2 参与资格。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。

**如何投票**

您可以使用 `inferenced` 命令获取提案详情并投票。请注意，任何活跃节点均可用于查询或投票。当前可用节点包括：
...
**注意 1：** 请在升级窗口期间保持在线，以便在需要时及时应用后续步骤或缓解指令。
...
该延长也为主机提供了时间来为新的 PoC 要求准备其系统，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步执行更新。
...
`Allowlist`到期时间：自动在区块2443558处到期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[允许列表](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的到期时间与区块2,222,222处的参与者注册截止时间对齐。
...
投票截止：2026年1月12日，UTC时间06:04:14。

`Allowlist`过期时间：自动在区块2,222,222处过期。
...
如果获得批准，网络将临时使用一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已与预定义的常见硬件配置模式进行比对，用于识别偏差和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
...
每位活跃主机的参与对网络向前推进并恢复正常运行至关重要。请保持在线，并在说明发布后立即准备应用更新。
...
- 临时重新激活创世验证者增强——此前曾使用过的、限时的防御机制，现提议重新启用。
...
本升级中包含的预备变更，为未来通过治理方式控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内处于激活状态。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。

**主要变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前使用过的、有时间限制的防御机制，现提议重新激活。
...
- 主机级注册：在区块2,222,222（约两周后）之前暂停新主机注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内暂停新开发者地址注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续到区块2,294,222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理控制参与者准入和推理执行提供了支持，而无需额外的软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外奖励，金额为第117轮奖励的1.083倍，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
