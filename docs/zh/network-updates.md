...
- PoC 种子稳定性。在纪元边界延迟后恢复遗漏的种子提交，并在短暂失败（如签名者不可用）后重试本地种子恢复，以防止验证者被置零。
- API。
...
建议主机查看 [GitHub](https://github.com/gonka-ai/gonka/pull/1497) 上的提案并参与投票。
...
2. 一个 **v0.2.15 主网软件升级**，移除大量过时代码、遗留的交易处理路径及相关组件，这些已不再符合当前架构的需求。
...
v4 是首个支持多实例高可用的版本：在共享的 Postgres 上，由 versiond-router 负责路由，后端部署 N 个带版本的 devshardd 副本，使用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布新二进制文件时（名称不变，仅 sha256 变化），versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
[v0.2.14 提案](https://github.com/gonka-ai/gonka/pull/1267) 已上链并开放投票。

主网链/API工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（禁用主网上的`/v1/chat/completions`计费并从API二进制文件中移除嵌入的`/v1/devshard`）、奖励接收者路由以及升级时的安全修复。

devshard 部分准备了 v3 运行时，使经纪人可以在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率并实现 SQLite 与 Postgres 存储之间的安全切换。

更多详细信息，请参见拉取请求：[https://github.com/gonka-ai/gonka/pull/1267](https://github.com/gonka-ai/gonka/pull/1267)

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新 `api` 或 `node` 容器。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
- 现在/主网升级前——更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前按照[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像升级至 0.2.14-post3。如果您的 `api` 二进制文件已更新，您只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护人员——本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 如果获得批准，devshard 存储在升级后可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并会自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 升级的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放评审**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程获得批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级时确保 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器会逐台替换版本。这是系列更新中的首次更新，该系列旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含了错误修复和安全补丁。

**待办事项**

请审阅 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现的问题、疑问、改进建议、边缘情况或潜在漏洞处留下评论。

有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格在下一次升级周期中获得社区奖励。
...
* 在区块 **5,105,276** 之前（2026年7月17日，约 UTC 12:05）声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第331周期PoC开始前约500个区块的窗口内（区块**5,105,776**——2026年7月17日12:50:53 UTC / 05:50 PDT），将您的MLNode切换为Kimi。
* 委托主机：选择非守护者Kimi主机，并在独立目标间分散权重——参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。

**devshard v1/v2已移除**

`versiond` 已停止v1和v2的`devshardd`进程。所有网关流量必须使用`/devshard/v3`。如果在此更改后您的网关停止提供推理服务，说明您仍在通过已移除的路径路由 — 请遵循v3迁移指南。

## 2026年7月16日

**紧急治理投票（提案88）：重新注册Kimi-K2.6并移除devshard v1/v2运行时**

这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从未激活集合中移除；提案88将其重新注册到治理模型列表中，使其从epoch 331开始进入标准引导流程。

同一提案还将从`devshard_escrow_params`中移除`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的内存增长问题 —— 这正是7月15日事件中网络节点OOM的根本原因。

**这使v3切换成为最终决定。** 仍在通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，将在提案通过后停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票 —— 紧急窗口时间很短。
2. 提供Kimi服务的主机：在投票结束**后**、区块 **5,105,276**（2026年7月17日 ~12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在epoch 331 PoC开始前的~500个区块窗口内（区块 **5,105,776**，2026年7月17日 ~12:50 UTC）将您的MLNode切换回来：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间委托时：**不要委托给守护者节点**；将委托分散到独立的Kimi主机上。请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指南。
4. 经纪人 / devshard创建者：如果您的任何网关流量仍在使用v1/v2或`/v1/devshard` —— 请在投票结束前切换到`/devshard/v3`。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参考 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
- 意向截止日期（epoch 330）：区块 **5,105,276** — 2026年7月17日 ~12:05 UTC（~05:05 PDT）。
...
`moonshotai/Kimi-K2.6`在epoch 328–329中失去了PoC验证多数（详见[此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从未激活集合中移除并重新引导，是使其以最小停机时间恢复的最快方式 —— 与6月提案78所采用的恢复路径相同。

该提案将在下一次PoC前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将从epoch 331开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票——紧急窗口时间很短。
2. 为Kimi提供服务的主机：保持您的Kimi设置就绪，随时准备在引导PoC时重新启用您的MLNode。
3. 在引导期间委托时：不要委托给守护节点；将委托分散到独立的Kimi主机上。
...
Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。监护人投票决胜机制未触发：当前没有每模型投票权重的监护人会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。
...
先前的指导（包括 2026 年 6 月 27 日的引导公告）建议将监护人节点作为委托目标。此建议现被撤销：

* **请勿委托给监护人节点。** 监护人是 PoC 验证的后备机制。将委托集中于它们会将主机制与后备机制绑定到同一硬件上——本次事件正是这种故障模式。目前正在讨论协议层面的限制。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请将委托分配给运行模型的您自己的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重，且无单点故障。

**已在开发中的修复*** 守护者参与 PoC 验证（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。
...
4. 经纪人：立即迁移 devshard 流量到 `/devshard/v3`。这将解决 RAM 增长问题，并在移除 v1/v2 前必需完成。
...
提案获批后，v3 现在与现有的 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有的 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换到 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载该二进制文件，验证 sha256 哈希值，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

已获白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这使他们能够在链升级运行期间继续提供推理服务，而不依赖已弃用的经典 API 路径。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，而不依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获批，`versiond` 将自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换到 v3。此类升级期间无需主网重启或手动主机操作。
...
将从该分支单独发布并部署 devshard v3 版本，再进行主网链升级。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级运行期间继续提供推理服务。
...
本次仅呼吁审查 Pull Request，不启动正式投票。治理投票流程将在审查期结束后开始。
...
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `ap`i 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两步，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
...
针对即将到来的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复版本 —— 可异步部署，通过强化 PoC-v2 验证阻止攻击，使复制的计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第 311 个纪元进行另一次引导尝试。希望提供 Kimi 服务的主机需在第 310 个纪元声明其意向，并在新纪元开始前准备好节点。一个守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第 311 个纪元开始前约 500 个区块的窗口期内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300上运行Kimi时，其共识权重与主模型MiniMax大致相同。此举并非偏向任何硬件 — MiniMax已具备相同的权重。在8xB300主机上运行Kimi而非MiniMax，将获得约1%的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数的选择使得在8xB300上运行GLM-5.2时，其共识权重与MiniMax大致相同。此举并非偏向任何硬件。在8xB300主机上运行GLM-5.2而非MiniMax，将获得约2%的增益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最少成员数 `V_min`、以及>2/3可达性）。这使操作员能够在配置前确认模型是否可行。
...
从第309轮起（PoC开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是各模型满足引导资格：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中启动。由于每轮约22.8小时，标准48小时投票周期无法适应单轮；而12小时的紧急周期可以。创世守护节点预计支持本提案。

**主机需采取的操作**

1. **若要提供Kimi：** 在投票**结束后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。
...
4. **在截止日期前对提案79进行投票。** 紧急窗口较短。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* **添加GLM-5.2** —— 添加GLM-5.2 **且不设不参与惩罚**，以便主机可选择加入，并提前衡量对该模型的需求，不提供该模型的主机不会受到惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快且停机时间最短的恢复方式。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以便快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是恢复它且停机时间最短的最快路径。下一个周期将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立且无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容变更。它与Kimi的情况无关，仅因同样是PoC阵容变更而被包含在此处。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307周期结束前完成。每个周期约22.9小时，标准的48小时投票期无法容纳在一个周期内；而12小时的紧急投票期可以。创世守护者应支持该提案。
...
1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时无cPoC。请现在预下载FP8权重（若尚未准备就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——在第308周期内将进行一次恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** — 在第308个周期中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前测试对该模型的需求，而选择不提供该模型的主机不会受到任何惩罚。
...
* 第308周期将在之后不久形成：PoC开始约16:50 UTC，生效约17:25 UTC。
...
这是首个仅针对devshard的运行时升级。它独立于完整链软件升级，无需链二进制升级。

提案获批后，v2现在可与现有的v1 devshard运行时并行运行。新进程在`/devshard/v2`前缀下提供服务，而现有的v1流量继续在`/devshard/v1`和`/v1/devshard`上运行。

发布版本将`devshardd`二进制文件作为Gonka发布工件。`versiond`会自动下载二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动一个额外的`devshardd`进程。

对于此类仅devshard的运行时升级，无需重启节点容器或手动执行主机操作。

**主要变更**

1) 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中。
2) 通过OpenTelemetry和Prometheus添加了内部devshard追踪和指标。
3) 添加了通过Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor实现的join-stack可观测性。
4) 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每插槽总数。
...
本提案涵盖devshard v2发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首个仅针对devshard的升级。它独立于完整链软件升级。若获批准，v2将与现有的v1 devshard运行时并行运行。
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。如果链上提案获批，`versiond`将自动下载二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动一个额外的`devshardd`进程。

新进程通过`/devshard/v2`前缀提供服务。现有的v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。在此类升级期间，无需重启节点容器或手动执行主机操作。
...
这是首次仅适用于devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，也不预期会导致推理服务停机。
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机被鼓励立即选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获**批准**，并在主网块`4267300`处成功执行升级。
...
对[升级提案v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案ID `54`）的链上治理投票已结束。该提案已**批准**。

升级将在主网上自动在**块高度4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）执行。

**提醒**

1. 确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT代币元数据和CW20 `wrapped_token`代码ID `105`均通过升级处理器注册，因此桥接将在升级高度在主网上激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在预下载约230 GB的FP8权重。在引导窗口期间，Hugging Face的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为**每个**经治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择DELEGATE或REFUSE。MiniMax的截止日期为**链纪元 `278`**。不采取任何行动的主机将从纪元278起，对其全部权重施加每纪元15%的惩罚。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过的提案的重新投票，现重新提交并包含多项更新。
...
- 在vLLM 0.20.1发布后，基于Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B：`0.359`（未变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下，Kimi每轮共识权重减少约38%）
    - MiniMax-M2.7：`0.3024`
...
- 运营商时间表：投票于5月22日22:12 UTC结束 → 升级高度约为5月26日14:42 UTC → 升级周期剩余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax引导快照在 start_poc − 500 块处（约43分钟前）→ 升级后下一个周期边界开始首个MiniMax PoC阶段 → MiniMax惩罚在链周期278处执行。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获批准，此跳过是预期行为，并非故障；新快照逻辑将在下一个周期开始。
- 若获批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认，且会自动清理（保留最近3个周期）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过⅓），链上无任何变化，升级将不会发生。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取行动。
...
在应用额外保护措施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪端点。

**发生了什么**

在周期 #266 中，网络的活跃权重显著下降。

该问题已追溯至具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。

在周期 #267 中，主机成功回归，PoC验证顺利完成。

**推理可用性**

使用受影响的非标准语义的请求将不再被网络接受。

在相关保护措施应用期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪端点。

**周期 #267 中Kimi的权重**

由于现有协议规则，Kimi在周期 #267 中的活跃权重较低：单个模型的总权重不得超过前一周期所有模型总权重的75%。
...
- 提案在升级后延长了宽限期，以避免在升级后3000个区块内惩罚主机。
...
- 将devshard的nonce上限从 `20_000` 提高至 `1_000_000`。
- 将每轮最大devshards数量从 `100` 提高至 `500_000`。
- 修复新模型启动期间确认PoC奖励核算问题。
- 在本次升级轮次剩余时间内禁用确认PoC，以便从下一轮开始干净地启动新快照逻辑。
- 当参与者再次活跃时，重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问问题。
- 将创世守护者的调整投票权降低至约25%，并将链上治理的法定人数设置为 `0.25`。在守护者不投票的情况下，这使得剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，然后再采取进一步操作。
...
- 该提案将有意跳过从升级高度到升级纪元结束（3000个区块的宽限期）的确认PoC。若获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 仍将作为默认选项，并自动清理（保留最近3个纪元）。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推断即可再次使其失效。现在在重新激活和即将升级时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授权中未包含`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应交易商投诉。
...
- 升级为当前纪元安装了一个宽限期条目，扩展了`UpgradeProtectionWindow`（3000个区块）。从升级高度到升级纪元结束期间，确认PoC触发器将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用了v0.2.10的宽限期原语。
...
- Postgres数据按`epoch_id`分区，用于会话、差异和签名，以便清理可以干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
- 有效载荷查找固定在托管纪元，并为纪元边界和遗留纪元0请求提供回退机制。
...
在api容器版本v0.2.11、v0.2.12和v0.2.12-api-post2中存在一个潜在问题。容器重启后，9100、9200和9400端口上的服务器可能启动延迟。这延迟了api激活，导致部分矿工跳过确认PoC。
...
该过程由网络中的主机协调完成：基础设施已准备就绪，意图已提交，委托与拒绝已设置，部署已测试。
...
已提交 PoCIntent 的节点应在 PoC 于区块 `3874496` 启动前，至少将一个 MLNode 从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。

预评估与PoC开始之间有500个区块的窗口期。在此期间，没有CPoC任务，因此已声明意图的主机可安全地将其模型节点切换至 `Kimi K2.6`。

请遵循指南完成所需部署步骤：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年5月4日

传输代理 `node1`、`node2` 和 `node3` 已禁用。所有主网推理现在均通过 `node4` 路由，其采用新的 `devshard` 基础计费方式。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个节点必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固**。保留节点（`POC_SLOT=true`）被随机抽样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易方共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
每个 DAPI 都会本地保存其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此 `/admin/v1/nodes` 看起来是干净的，即使持久化配置中仍包含额外模型。版本 0.2.12 停止了这种裁剪，直接加载持久化配置。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：模型组将在升级后两个周期激活。该模型的系数为 Qwen235B 的 3.51 倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI / 主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议加固**。保留的节点（`POC_SLOT=true`）将随机抽样用于单次 PoC / CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡到按模型分组的 PoC。每个治理批准的模型生成其自身的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
...
- **协议加固**。实施更强的 PoC v2 RNG（完整 256 位熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
此更新修复了在PoC开始附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
目前，通过链上每个推理交易处理推理操作限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链上仅处理会话创建和最终结算。
...
这故意是一个早期且受限的版本。提出此版本供主网审查和有限生产测试，并非因为其已被认为完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，此改进可使每个区块容纳多达100倍的推理数量，具体取决于工作负载和网络状况。  ￼
...
在 `v0.2.10` 中发现潜在漏洞。为降低当前升级前阶段的风险，建议在下一次PoC开始前将api二进制文件升级至 `v0.2.10-post7`。
...
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求授予您代其投票的权限。没有此授权，其他账户无法提交投票。
...
服务器端对长请求的重试通常无效，因为这会导致相同的超时场景。同时，客户端可能收到不一致的输出。
新版本的 API 在此类情况下不再重试推理。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 启动时中断正在进行的推理请求，以降低因 PoC 开始时仍有活跃请求而导致潜在权重下降的风险。
...
如前所述，第 180 轮的抵押激活与此投票无关。

由于提案未通过，创世时定义的抵押参数将在第 180 轮自动生效。

参与者应：

- 审查创世时定义的抵押参数。
- 在第 180 轮前准备并存入所需的 GNK。
- 确保 [抵押](https://gonka.ai/host/collateral/) 正确设置，否则从第 180 轮起，PoC 奖励将减少 5 倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各轮之间可能波动。较小的权重可能经历更大比例的相对波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平仍相对较低时，存入高达计算最低要求 2 倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为使 PoC 权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的 PoC 参考窗口比先前名义假设长约 5 个区块。
...
由于有效归一化窗口比之前的名义模型长约 5 个区块，因此每轮重新计算的权重成比例降低。

**观察到的 GPU 权重变化（第 175 轮 → 第 176 轮）**
...
升级提案v0.2.10的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **按真实时间标准化 PoC 权重**
    ...
    此升级通过实际 PoC 经过时间对 PoC 参与者权重进行标准化，以减少区块时间漂移效应，并使权重结果与实际执行持续时间保持一致。
...
**宽限期：** 升级后引入了 3000 个区块的宽限期，期间无确认 PoC，并且在升级当期 epoch 中放宽了错过率和无效率阈值。
...
这意味着在单个 epoch 内，即使被惩罚，矿工最多损失其抵押物的 0.5%。且所需抵押物仅占日收益的约 24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果此提案未通过，创世时定义的抵押参数将在 Epoch 180 自动激活，而非上述参数。

投票结束后、Epoch 180 之前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从 Epoch 180 开始，其收益将被削减 5 倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项对 PoC 验证的重大优化（默认禁用），并实现基于真实时间的权重标准化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并且在升级当期放宽了错过率和失效率的阈值。
...
任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
本次仅为拉取请求的审查呼吁，而非正式投票的开始。治理投票流程将在审查期结束后启动。
...
此升级通过实际PoC耗时归一化参与者权重，以减少区块时间漂移影响，并确保权重结果与真实执行时长一致。
...
在更新的权重分配生效后，追踪器（仪表板）维护者可能希望审查第159轮及之后的系数，以确保其与当前PoC v2的权重分配保持一致。
...
关于升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并于区块 2451000 成功在主网上执行。此次升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**注意**

- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时执行后续步骤或缓解措施。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入下一纪元的参与集合中。
...
- 仅配置为服务受支持的模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- 镜像已更新为PoC v2兼容版本
...
下表列出了上一轮次具有权重、在当前轮次提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)

其总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或参与极低的总权重达到约 48%，这一比例过高。
...
- SSRF & DoS：验证 `InferenceUrl` 以拒绝内部 IP，并添加超时机制以防止请求挂起。
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置在第155个Epoch起需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个Epoch后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML 节点版本，可评估是否具备过渡到 PoC v2 的条件。一旦在活跃主机中观察到足够比例的采用率（约 50%），可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 但未升级 ML 节点不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不被视作 PoC v2 参与资格。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。
...
若获批准，网络将暂时使用一个 `allowlist`，包含在先前周期中未表现出非常规硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已根据预定义的常见硬件配置模式集进行评估，以识别偏离和不一致情况（非常规配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
此前表现出这些模式持续偏离的参与者，在区块 2222222 的稳定窗口结束前，不包含在 `allowlist` 中。
...
本升级中包含的预备性变更，为未来通过治理方式控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外发放，金额为第117轮奖励的1.083倍，统一适用于所有符合条件的节点，包括已领取原始奖励的节点。
...
创世验证者增强功能最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续有效至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理机制控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第 117 个周期奖励分配**

本提案涵盖与链停顿（第 117 周期）相关的两项奖励分配：

- 在第 117 周期活跃但未收到周期奖励的节点，将补发该周期的遗漏奖励。
- 所有在第 117 周期活跃的节点，将额外获得相当于第 117 周期奖励 1.083 倍的 payouts，均匀分配给所有符合条件的节点，包括已收到原始奖励的节点。
...
此 `API` 版本改进了对失败推理的处理韧性，并减少了推理遗漏计数问题。它还为PoCBatch和PoCValidation交易引入了批处理功能。
