...
!!! note "关于本页面"
    ...
    本页面不保证内容完整。如需最新信息，包括治理投票的启动及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。
...
在 epoch 初期，数据尚不足，因此无效推断检查（SPRT）可能在样本足够大之前，仅因少量无效结果就被触发。本提案对其进行稳定化处理：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本情况下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真实违规者仍会触发该机制（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发该机制：至少需要 32 个被标记的无效结果，且在 500 个样本时，观测到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。epoch 结束时的统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **8 月 21 日 06:04 UTC**（约 12 小时后）结束。请尽可能参与投票。如果未能在规定时间内达到紧急阈值，该提案不会被拒绝，而是转为普通提案，投票将继续进行更长的常规周期。若转为普通提案，您需要重新投出您的选票。
...
此仪表板从网关视角展示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何处罚或罚没，但仍应作为协议开发过程的一部分进行调查。

具有严重错误率的主机应在结算开始前解决这些问题。无显著问题的主机无需采取任何行动。

仪表板还显示了网关内部细节，例如特定主机的请求失败原因或无效上下文长度，有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需采取行动。

**下一步：****1) 接下来的 24 小时。** 主机应检查其统计数据并解决任何关键错误或配置问题。如有需要，贡献者可协助调查问题。

**2) 在此 24 小时窗口之后的第一个 epoch。** 将在当前网关上手动启动结算，首先从较小模型开始。此过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 接下来的 1-3 天。** 将监控结算结果。

**4) 一旦此阶段稳定。** 其他经纪人可部署稳定版网关并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至 100%。

此部署过程刻意缓慢进行，以便在结算扩展至全网之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已上线 Gonka 网络**

在 [提案 94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链 epoch 360 起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214 转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需做任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每 epoch 的惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。

## 2026年8月14日

**MLNode 3.0.16 发布**

如果您的所有节点都已经使用新版本（3.0.16），则无需操作。

如果仍有节点运行旧版 MLNode（即正在服务 Kimi 或 MiniMax 的节点，因为任何已运行 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步发布。请勿一次性切换所有节点。先更新少量节点，确认稳定后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。
...
将线上社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并为未来的实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。
...
**DeepSeek V4 Flash 启动：意向阈值已达成——请在第 360 个纪元快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在第 359 个纪元快照中未满足预资格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 和 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次 **官方** 检查是第 360 个纪元启动快照——在此前请保持多元化的委托，以确保快照有效。
...
```
2. 如果模型已满足预资格条件，请在 PoC 开始前约 500 个区块的部署窗口内（区块 **5,552,115**）部署 DeepSeek。请使用 vLLM 0.25.1 + 发布候选节点配置。

**继续运行其他模型的主机**

在区块 **5,551,615** 之前，提交委托（有助于启动可达性）或拒绝。从第 **360** 个纪元起，DeepSeek `penalty_start_epoch` 将生效——未明确选择的主机将面临不参与惩罚。
...
**关键时间点*** 第 360 个纪元快照（`start_poc − deploy_window`）：区块 **5,551,615** —— 约为 2026年8月14日 01:24 UTC
* 第 360 个纪元 PoC 开始：区块 **5,552,115** —— 约为 2026年8月14日 02:09 UTC
* DeepSeek 不参与惩罚从第 **360** 个纪元开始
...
您所需的一切均在 [`vllm-0.25.1-upgrade` 分支](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/deploy/join) 上。
...
以该格式运行模型需要新版本 API，v0.2.15-post5。要安装它：
```bash
...
```
该技能部署模型，测量 PoC 吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。如果通过，即可正常使用。

如果您希望提供 DeepSeek 服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免非参与罚金。

注意：明天将发布带有更好日志的另一个 MLNode 构建版本，因此请留意后续更新。
...
注意：在 Blackwell GPU 上，为获得最佳性能，您可以使用以 fp8 + nvfp4 重新打包的模型，而非 fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型的精度相同。部署时，将在启动前发布并共享更新的 API 二进制文件。
3. 如果您委托。请勿将委托指向守护节点；请将委托分散到独立的 DeepSeek 主机上。参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在纪元 360 之前无需操作。从纪元 360 开始，如果您未提供 DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用 15% 的非参与罚金。

区块高度是精确的；时间是估算值，将在接近纪元时确定。[更多关于 DeepSeek 启动的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
4. PoC 开始：成员资格由在 PoC 开始时提交 PoC 存储提交的主体决定，而非由声明意向的主体决定。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第 360 纪元前无需操作；从第 360 纪元起，若您未提供服务，请提交 `PoCDelegation` 或 `PoCRefusal` 以支持 DeepSeek。
3. 在截止日期前对提案 94 进行投票。
...
这只是一个性能和资源管理补丁，别无他物。协议、推理、验证和经济行为均保持不变。变更包括：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整链软件升级。若获批准，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动干预主机。
...
v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond / devshardd 副本，使用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。通过移除过时行为并将交易处理统一到当前支持的架构上，也为未来的开发和升级提供了便利。
...
投出您的投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 或更高版本（v0.2.12 及以上）。
```
export NODE_URL=https://node3.gonka.ai/
...
- 投票截止：2026 年 7 月 30 日 07:08 UTC / 2026 年 7 月 30 日 12:08 PDT
- 提议的升级高度：5316315
...
devshard 部分准备了 v3 运行时，以便经纪人在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率，并实现 SQLite 与 Postgres 存储之间的安全切换。
...
- 现在/主网升级前——更新您的API和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将`api`二进制文件和网桥镜像更新至0.2.14-post3。如果您的`api`二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在PoC或cPoC之外执行此步骤。

- 仪表板维护人员——本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的Cosmovisor对`application.db`的备份通常为数十GB，建议提前确认）。有关如何安全清理`.inference`目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
Devshard升级独立于主区块链进行。它们不需要通过Cosmovisor协调完整的节点升级，不影响主网行为，预计不会导致推理服务中断。如果通过治理流程批准，新版本的devshard将与现有的v3运行时并行运行。

**关键变更**

- v4的主要目标是在主机故障或升级时实现devshard主机的高可用性。当一台机器宕机或重启时，它仍能处理新请求；在版本升级时，多台机器逐个替换版本。这是系列更新中的首次，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4是首个面向多实例高可用的版本：在共享Postgres上，通过versiond-router部署N个versioned/devshardd副本，采用粘性会话路由和验证租约独占性。网关仅通过gRPC与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅委托方通过签名聊天绑定。当治理发布具有相同版本名称（名称不变，仅sha256变化）的新二进制文件时，versiond可执行蓝绿切换并排干，以确保正在进行的工作（包括SSE）在旧版本上完成。
- v4还包含错误修复和安全补丁。

**操作项**

请审查PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。
...
* 在区块 **5,105,276**（2026年7月17日，约12:05 UTC）前声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在周期331的PoC开始前约500个区块的窗口期内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换为Kimi。
...
这是7月15日宣布的Kimi恢复计划的第二步。提案87将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册到治理模型列表中，使其从周期331开始进入标准引导流程。

同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2。一旦批准，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）。这消除了旧运行时容器长期存在的RAM增长问题——即7月15日网络节点OOM事件的根源。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间进行委托时：**不要委托给守护节点**；将委托分散到独立的Kimi主机上。有关更新的委托指南，请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`moonshotai/Kimi-K2.6`在epoch 328–329中失去了PoC验证多数（详见[此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在从活跃集合中移除Kimi并重新引导，是使其在最小停机时间内恢复的最快方式——与6月提案78的恢复路径相同。
...
**Kimi-K2.6 事件（时段 328–329）：发生情况、恢复计划及委托指导变更**

在时段 328，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数支持。在时段 329 开始时，运行 Kimi 的主机被移出该组。网络其余部分正常运行。

**发生情况**

两台由监护人运营的运行 Kimi 的服务器同时故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题崩溃。大量 Kimi 委托集中在这台主机上，因此其故障导致这些投票立即消失。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。监护人决胜机制未启动：当前未赋予每模型投票权重的监护人会被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与时段 306–307 的事件类似。时段 329 开始时的投票确切细节仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个时段）重新添加：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的独立公告。
...
* **请勿委托给监护人节点。** 监护人是 PoC 验证的备用机制。将委托集中于它们会将主机制与备用机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托分配给您自己运行模型的节点。
...
1. 一旦紧急 Kimi 提案上链（公告将随后发布），请对此提案进行投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非监护人委托目标；尽可能将权重分散到多个独立主机。
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这将解决内存增长问题，且在移除 v1/v2 前必须完成。
...
提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换至 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前，将推理流量切换到 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
将从该分支单独发布 devshard v3 版本，并在主网链升级前提案并部署。提前将推理流量切换到 `/devshard/v3` 的代理可在链升级期间继续提供推理服务。
...
有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能在下一个升级周期中符合社区奖励资格。

本次仅为对拉取请求的审查请求，不启动正式投票。治理投票流程将在审查期结束后开始。

**Devshard v3 治理投票**

devshard v3 发布版本在主网链升级前提出并部署，以便经纪人能够在升级运行前将推理流量迁移到 `/devshard/v3`。

**主机操作事项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对任何发现、问题、改进建议、边缘情况或漏洞留下评论。
...
5. 仪表板维护者 —— 准备好调整指标统计方式。详细说明将在 `devshard v3` 投票启动并接近完成时另行发布。

## 2026 年 7 月 6 日

**安全更新：PoC-v2 权重验证强化 —— 更新您的 API 容器**

上周，在 HackerOne 上报告了 PoC-v2 权重路径中的一个漏洞。参与者可膨胀其计算权重，从而影响区块奖励、共识投票权和治理投票权。

根据历史数据，此前未观察到此类攻击。然而，在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。

针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已被使用，升级前已提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供Kimi服务的主机需在第310轮期间声明其意向，并在新轮次开始前准备好节点。守护节点也将运行Kimi，因此不愿运行它的主机可将权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，则在第311轮开始前约500个区块的窗口期内将节点切换至Kimi（PoC开始约在6月28日 ~12:47 UTC）。

**将不运行Kimi的主机**

将权重委托给运行Kimi的主机（或发送拒绝）：
```
...
> **更新（2026年7月15日）：此建议已撤销——请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指导。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定为：在8xB300硬件上，Kimi的共识权重与主模型MiniMax大致相当。此举并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
常规启动流程从第309轮开始。由于投票在PoC开始前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于启动的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定为：在8xB300硬件上，GLM-5.2的共识权重与MiniMax大致相当。此举并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员可在配置资源前确认模型的可行性。
...
提案必须在第308轮结束前完成，以便两个模型都能启动至第309轮。由于每轮约22.8小时，标准的48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计支持该提案。

**主机需采取的操作**

1. **要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口期，此期间切换是安全的，因为此期间无cPoC。
...
如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
* 有意再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308周期内将进行一次投票以恢复Kimi。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，因此主机可自愿选择加入，提前衡量对该模型的需求，不提供服务的主机不会受到惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前投票数不足，无法通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其以最小停机时间恢复的最快方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi的投票数不足，无法通过PoC验证，将其移除并重新引导是恢复其服务的最快路径，停机时间最短。下一周期将进行一次投票以恢复Kimi。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已提出的阵容调整。它与Kimi的情况无关，仅因同样是PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在第307个纪元结束前完成，因此这些变更被合并为一次紧急投票。每个纪元约22.9小时，标准的48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持本提案。

**若获批准的效果**

从第308个纪元开始（约UTC时间6月25日17:25），`MiniMaxAI/MiniMax-M2.7`将成为唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6将不再活跃。恢复Kimi将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如尚未预加载），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi服务的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308纪元将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 第308纪元将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设置不参与惩罚**，因此主机可选择加入并提前测试模型需求，而不选择提供该模型的主机不会受到任何惩罚。
...
devshard v2运行时已获链上批准并添加到 `DevshardEscrowParams.approved_versions`。
...
提案通过后，v2现在可与现有的v1 devshard运行时并行运行。新进程在 `/devshard/v2` 前缀下提供服务，而现有v1流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为Gonka发布工件。`versiond` 自动下载二进制文件，验证sha256哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅devshard的运行时升级，无需重启节点容器或手动执行主机步骤。

**关键变更**

- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已提供的推理保留在RAM或状态中。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已提供的推理保留在RAM或状态中
...
发布版本将 `devshardd` 二进制文件作为Gonka发布工件。如果链上提案获得批准，`versiond` 将自动下载二进制文件，验证sha256哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。
...
请直接审阅该PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审阅贡献，包括重要评论、错误发现和安全问题，可能有资格在下一轮升级周期中获得社区奖励。
...
针对MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需采取任何其他操作——现有设置将继续正常工作。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机将对其完整权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基准。此调整**仅适用于您共识权重中由 Kimi 导出的部分**；您的 Qwen 导出权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降至约 25%，并设置链上法定人数为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 根据 vLLM 0.20.1 发布后 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`（不变）
    - Kimi K2.6：`0.78`（从 1.26 下调，约减少 38% 的同等 PoC 权重下的每纪元共识权重）
    - MiniMax-M2.7：`0.3024`
...
- 将 devshard 非负数限制从 `20_000` 提高至 `1_000_000`。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。此时，请先确认您运行的是当前版本，再采取进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期剩余阶段跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约 43 分钟前）→ 首次 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最后 3 个纪元）。
...
第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击加上若干相关条件，导致许多参与者无法进入第266轮。
...
该问题已被追溯到具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
2. 监控下一几轮的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重被正确反映。
...
- 修复桥接和流动性池合约权限检查的 Wasm keeper 访问。
- 将创世守护者调整投票权降低至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不参与投票的情况下，剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊端合约激活，所有主机均需验证其桥接容器已部署、运行最新版本并正确同步。
...
- 当参与者重新变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断可能立即再次使其失效。现在在重新激活和即将晋升时重置计数器。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权授予中缺少 `MsgRespondDealerComplaints`。升级将回填该权限，以便它们能够响应交易商投诉。
...
- 状态快照减少了长期会话的恢复工作量。
- 负载查找固定到托管纪元，并为纪元边界和遗留纪元 0 请求提供回退机制。
...
在上一个周期中，发现Kimi-K2.6解析某些响应时存在一个轻微bug。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来周期中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
`moonshotai/Kimi-K2.6`已通过启动流程并加入Gonka网络的PoC参与。
...
预评估与PoC开始之间有500个区块的窗口期。在此期间无CPoC任务，因此已声明意图的主机可安全地将模型节点切换至 `Kimi K2.6`。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保留您的MLNode使用 `Qwen235B`，并于明天提交下一周期的意图。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个周期激活。其系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，对齐传统验证者惩罚与所需抵押品语义，确保 devshard 保证金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留之前强制实施的模型和 Kimi。
...
- **协议加固。** 对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间验证。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 经销商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 确保冷钱包账户持有足够金额（例如 100 GNK）以覆盖自动授予的费用限额支出。
...
本次更新修复了 PoC 开始附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
v0.2.11 升级提案的链上治理投票已结束。提案已获**批准**，升级已在主网上成功执行。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著改进 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理可扩展性。

目前，通过每笔链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完备，而是因为此类系统需要尽早暴露于真实网络环境中。某些问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，这使得每个区块可容纳高达 100 倍的推理次数，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳的推理数量最多提升至100倍，具体取决于工作负载和网络条件。  ￼

**升级前的建议操作****`application.db` 剪枝**

强烈建议主机在升级前根据提供的说明剪枝 `application.db`。
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时开始大量剪枝活动，造成不必要的运营压力。
...
现在提供一个新的vLLM / mlnode构建，可在PoC开始时中断正在进行的推理请求，以降低PoC启动时仍处于活动状态的请求可能导致权重下降的风险。
...
- 审查创世时定义的抵押参数。
- 在第180轮之前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)设置正确，否则从第180轮起，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各轮之间可能波动。较小的权重可能会经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平相对较低时，存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为了将PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
由于有效归一化窗口比之前的名义模型长约5个区块，因此每轮重新计算的权重成比例降低。
...
从PoC权重推导H100等效容量或奖励预测的追踪器和仪表板，请从第176轮开始验证其转换系数。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级通过实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。
...
**宽限期**：升级后引入了3000个区块的宽限期，在此期间不进行确认 PoC，并对升级当期的错过率和无效率阈值放宽要求。
...
- 每1单位算力对应 0.032 GNK（约每块 H100 对应 10 GNK）
- 错过率或封禁惩罚为 0.01%
- 无效推理惩罚为 0.5%

这意味着在单个周期内，即使被惩罚，矿工损失的抵押品也不会超过其抵押品的 0.5%。所需抵押品仅占每日奖励的约 24%。

**警告**：无论投票结果如何，抵押参数都将生效。如果该提案未通过，创世时定义的抵押参数将在第180个周期自动激活，而非上述参数。

投票结束后、第180个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180个周期起，其奖励将被削减5倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且对升级时段的错过率和无效率阈值进行了放宽。
...
- 如果 `application.db` 占用了大量磁盘空间，可应用此处描述的清理技术 [https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it]。
...
警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提案进行。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在有正当理由时适用。如果证明稳定性良好，逐步将抵押品提高至代币经济白皮书中所述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止。这是因为节点运行的是不再与网络兼容的旧版二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
在当前纪元中观察到cPoC（确认PoC）与PoC之间的重叠。直至纪元最后一个区块，`is_confirmation_poc_active`被观测为`true`。
...
请保持在线，以便在需要时立即应用补丁。更多详情和补丁说明将在准备就绪后发布。
...
此升级引入了一种新的PoC验证机制，通过为每个参与者分配固定的采样验证者集，将复杂度从O(N²)降低至O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)
...
在更新权重分配生效后，追踪器（仪表板）维护者可能希望审查第 159 期及之后的系数，以确保其与当前 PoC v2 权重分配保持一致。
...
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入下一纪元的参与者集合中。
...
- 服务于其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换至`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型
...
其总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或参与度低的总权重达到约 48%，这一比例过高。
...
- 身份绕过：将 `epochId` 绑定到签名，以验证对应正确周期。
...
- Postgres：现在可通过 Postgres 配置本地有效载荷存储，升级后可用。
...
本指南说明主机应如何根据 v0.2.8 模型可用性变更及即将推出的 PoC v2 更新来升级其 ML 节点。ML 节点配置需符合 PoC v2 要求，自第 155 个纪元起生效。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个纪元后安排。迁移阶段结束后，不符合配置要求的 ML 节点的权重将不被计入。
...
!!! note "重要"
    ...
    - 未同时满足两项条件的节点，一旦网络切换为单模型配置，将不再符合条件。
    - ML 节点升级必须在迁移完成且 PoC v2 通过 v0.2.8 升级后的独立治理提案激活之前完成。
...
!!! note "治理和 PoC v2 激活说明"
    ...
    **第一阶段：观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML Node 切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为与 PoC v2 兼容的版本，才能为 PoC v2 权重做出贡献。
...
PoC v2 仅在链上治理提案获得批准后，才成为权重分配的活跃方法。
    在此提案获得批准前：
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。所有先前支持的模型均从活跃集合中移除，除了：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，用于评估向 PoC v2 过渡的准备情况。一旦观察到活跃 Host 中达到足够程度的采用率（约 50%），可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不再被视为 PoC v2 参与资格有效。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步进行更新。
...
`allowlist` 具有固定的过期时间，不会在延长的稳定期结束后继续有效。一旦 `allowlist` 在区块 2443558 过期：
...
`Allowlist`过期：自动在区块 2,222,222 到达时。
...
- 报告的硬件特征已根据预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
...
每位活跃主机的参与对网络推进和恢复正常运行至关重要。请保持在线，并在说明发布后随时准备应用更新。
...
- 临时重新激活创世验证者增强——这是一种此前曾使用过的、限时的防御机制，现提议重新启用。
...
- 开发者地址注册：在稳定期间，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意破坏或压垮系统的行为引起的。本提案旨在通过一系列临时措施增强网络在高负载和不利条件下的弹性。

创世验证者增强最初在网路早期阶段作为临时防御机制引入，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
此次升级正式纳入了此前通过手动 API 更新分发并已在网络上使用的关键修复。这些修复包括：
...
- 主机级别注册：在区块 2.222.222（约两周后）之前暂停新主机注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。一个预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块 2.294.222（约 19 天）。

**治理控制机制**

本升级包含的准备工作为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第 117 期活跃的节点，将获得额外发放，金额为第 117 期奖励的 1.083 倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
