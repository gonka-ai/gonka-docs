...
提案 id 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加到 PoC 模型集中。同时，它将从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：它们的 PoC 参数和权重保持不变。所有其他链上参数均保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，现共同提出。发布输入、测量结果及阈值设定的推理依据详见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
`weight_scale_factor` 经过校准，使得将 PoC 模型切换为 GLM-5.3-Flash 的 B200 主机预计比当前获得的权重高出约 7%。对于其他 GPU，最优模型不变。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成票，且反对票低于 33.4% 且无否决权。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。
...
这是一个仅限devshard的更改。它通过链上参数提案进行，而非完整的链软件升级：Cosmovisor不参与，链不会停止，推理服务持续运行。若获批准，`versiond`将下载二进制文件，将其SHA-256与链上存储的值进行验证，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。通过提案本身并不会迁移任何流量，因此经纪人可以在之后逐步迁移其网关。
...
投票将于 **2026年9月10日22:36 UTC** 结束。提案通过需满足：权重25%的法定人数、多数赞成票，且反对票（含否决）少于33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代表您投票，请参考从冷密钥向热密钥授予治理投票权限的指南。
...
`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：它们的 PoC 参数和权重保持不变。所有其他链参数均保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，现共同提出。发布输入、测量值及阈值设定的推理依据详见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票门槛、多数赞成票，且反对票低于 33.4% 且无否决权。
...
这仅是 devshard 相关变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v4.1 与现有的 v3 和 v4 运行时并行运行。通过该提案本身不会移动任何流量，因此经纪人可以随后逐步迁移其网关。
...
投票将于 **2026 年 9 月 10 日 22:36 UTC** 结束。提案通过需满足：按权重达到 25% 的法定人数、多数赞成，且反对票（含否决）低于 33.4%。
...
**提案通过：支持Gonka参加TOKEN2049新加坡**

提案ID为99的链上治理投票已结束。

提案已获批准。它将从社区资金池拨出100,000 USDT，用于支持Gonka参加2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金将覆盖黄金赞助包及相关后勤开支：差旅、住宿、本地协调、嘉宾接待、活动运营和支援材料。
...
`v4.0.2` devshard 二进制文件现已可用。主机可使用以下说明手动更新v4二进制文件。暂未安排自动更新。

**变更内容**

重启时，`v4.0.1` 从nonce 1恢复了所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 随后在60秒就绪超时后终止子进程，客户端因此看到HTTP 502错误。
...
提案已获批准。它将 `weight_scale_factor` 对 `deepseek-ai/DeepSeek-V4-Flash-0731` 的权重系数从0.214提升至0.246。其他所有模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为 `PROPOSAL_STATUS_REJECTED`，原因是 `failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终投票结果：赞成15452，弃权3093，反对0，否决0。
...
**仅限devshard升级的PR现已开放审核**

Devshard升级独立于主区块链更新devshard运行时。它们无需通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**关键变更**

1. v5 的主要目的是高度同步：每个主机和顺序器在托管日志中维护一个已签名、可重放的主网高度视图，即使在没有推理流量时也是如此。借助该时钟，devshard 现在可以明确知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前携带时间戳字段的消息中。后续版本可以将 `confirmed_at` 和 `started_at` 等墙钟字段替换为高度标记。这解决了时间提前攻击问题，并为在协议层面处理流超时打开了途径。本版本包含这些标记，但尚未将超时和密封决策切换至它们。

3. 已添加通过所有插槽的往返心跳机制，因此安静会话仍能按常规节奏同步高度，而繁忙会话则可通过常规推理标记实现相同的节奏。
...
**提案97开放投票：将DeepSeek V4 Flash的weight_scale_factor提高至0.246**

一项新的链上治理提案（提案编号 97）已开放投票。

**作用**

将`weight_scale_factor`的`deepseek-ai/DeepSeek-V4-Flash-0731`从0.214提高至0.246。所有其他模型和链参数保持不变。
...
该提案已获批准。它调整了小样本下无效推理检查的行为，避免在周期初期数据不足时误触发。主机无需采取任何操作。
...
一项新的链上治理提案（提案编号 96）已开放投票。

**作用**

在周期初期，数据尚不足，因此无效推理检查（SPRT）可能在样本足够大之前仅因少量无效数据而触发。本提案将其稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真实违规者仍会触发它（约 360 个样本时为 18%，约 1650 个样本时为 12%）。
- 小样本无法触发它：至少需要 32 个被标记的无效项，且在 500 个样本时观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

此为加速提案，投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如能请投票。若未能在时限内达到加速阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。若转为常规提案，您需重新投出选票。
...
作为协议开发中的一个重要里程碑，该链已准备好逐步启用devshard结算，以提升网络的安全性和可靠性。

一个稳定的网关版本目前正运行在一个网关上，并将用于第一阶段的结算。其统计信息，代表了部分网络流量，可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于10%的主机不会触发任何处罚或削减，但应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。没有明显问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端的问题。超调度和隔离部分旨在为网关开发提供洞察，主机无需对此采取行动。

**下一步：****1) 接下来的24小时。** 主机应检查其统计信息并解决任何关键错误或配置错误。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上开始手动控制的结算，首先从较小的模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其数据在负载下目前可靠性较低。

**3) 接下来的1-3天。** 将监控结算结果

**4) 一旦此阶段稳定。** 其他经纪人可以部署稳定的网关版本并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至100%。

该 rollout 是有意逐步进行的，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 已在 Gonka 网络上激活**

在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在正以校准系数 0.214（后由 [提案98](#august-29-2026) 提高至 0.246）转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需做任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元的惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。

## 2026年8月14日

**MLNode 3.0.16 发布**

如果您的所有节点均已升级到新版本（3.0.16），则无需任何操作。

如果仍有节点运行旧版 MLNode（即仍在服务 Kimi 或 MiniMax 的节点，因为任何已运行 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少数节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。
...
默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是相同的 MLNode 3.0.16，只是为 CUDA 12.9 构建。两个标签均在同一 compose 文件中。
...
将活跃的社区销售合约和所有封装代币实例迁移至新存储的 CosmWasm 代码，并注册该封装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
- 投票截止：2026年8月14日，UTC 12:01
- 预计迁移时间：2026年8月14日，UTC 12:01

## 2026年8月13日

**DeepSeek V4 Flash 引导：意向阈值已满足 —— 请在 epoch 360 快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预资格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次 **官方** 检查是 epoch 360 引导快照 —— 请在此前保持委托多样化，以确保快照有效。
...
```
2. 如果模型已预资格，请在 PoC 开始（区块 **5,552,115**）前约 500 区块的部署窗口内部署 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。
...
委托主机：选择 **非守护者** 的 DeepSeek 意向主机，并将权重分散到独立目标上 —— 请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。

**关键时间点*** Epoch 360 快照（`start_poc − deploy_window`）：区块 **5,551,615** —— 约为 2026 年 8 月 14 日 01:24 UTC
...
该技能现已支持 DeepSeek。它位于独立的 `./skills` 文件夹中，无需任何代理特定设置 —— 将其软链接到 `.claude/skills` 并运行 [`mlnode-validate` 技能](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/skills/mlnode-validate)：
```
/mlnode-validate how to use
/mlnode-validate <your-node-ip:port> @deploy/join/node-config-deepseekv4flash0731-B300.json
```
该技能将部署模型，测量 PoC 吞吐量，并将输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则说明配置正确。

如果您希望提供 DeepSeek，请使用上述步骤进行设置。如不希望提供，请勿忘记委托，以免遭受不参与惩罚。

提醒：明天将发布日志更完善的 MLNode 新版本，敬请留意后续更新。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 在PoC 359开始前的约500个区块安全窗口内（区块5,536,724，约8月13日，03:24 UTC / 8月12日约20:24 PDT），切换您的MLNode。将您的MLNode配置并切换至DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate配置）。在此窗口内切换是安全的，因为其中没有cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4的模型，而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型的精度相同。部署时，将在引导前发布并共享更新的API二进制文件。
...
区块高度是精确的；时间是估算值，将在接近epoch时确定。[更多关于DeepSeek引导的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。

## 2026年8月10日

**治理投票（提案94）：添加DeepSeek V4 Flash**

将 `deepseek-ai/DeepSeek-V4-Flash-0731` 加入PoC模型阵容的提案现已上链并开放投票。

**该提案的作用**

本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。未修改任何现有参数或模型；唯一的变化是新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在vLLM 0.25.1上运行并使用release-candidate节点配置的MLNode（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
模型通过标准引导流程进入，因此主机可在投入硬件前观察其可行性：
...
在符合引导资格的前提下，从epoch 359（PoC开始时间约为2026年8月13日03:24 UTC / 8月12日20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与惩罚从epoch `360`（`penalty_start_epoch = 360`）开始。

**主机所需操作**

1. 要提供服务：投票结束后声明意向，然后配置您的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留您当前的模型：在epoch 360之前无需操作；从epoch 360起，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`。
3. 在截止日期前对提案94进行投票。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
关于 `v0.2.15-devshard-v4.0.1` 运行时升级提案（提案ID 93）的链上治理投票已结束。

该提案已获批准，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换的二进制文件——无需重启主网或手动操作主机。

**API 二进制更新：`v0.2.15-post3`**

新的 API 二进制文件 `v0.2.15-post3` 已可用。

此版本提高了 PoC 种子的可靠性，并包含多个 API 和部署修复。
...
- API。
    ...
    - 默认的 Node Manager gRPC 端口现已设为 9400，确保与自定义配置的兼容性
...
devshard 运行时通过链上参数提案进行升级，而非完整的链软件升级。若获批准，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动操作主机。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。
...
关于升级提案 v0.2.15（提案ID 92）的链上治理投票已结束。

该提案已获批准，并于区块 5316315 成功在主网上执行。该提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
v4 的主要目标是在主机故障或升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个面向多实例高可用的版本：在共享 Postgres 上，由 versiond-router 负责路由 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占机制。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅授权的托管方通过签名聊天绑定。当治理方发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。

v4 还包含错误修复和安全补丁。

**主网 v0.2.15 变更**

主网部分的升级移除了大量过时代码，包括已不再符合当前架构需求的遗留交易处理路径及相关组件。
...
同一提案也在 `DevshardEscrowParams.approved_versions` 中注册了 devshard v4。
...
投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` v0.2.12 或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的Cosmovisor备份`application.db`通常为数十GB，建议提前确认）。有关如何安全删除`.inference`目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
升级提案v0.2.14（提案ID 89）的链上治理投票已结束。
...
devshard部分为v3运行时做准备，使代理可在链升级期间无需依赖已弃用的经典API路径提供推理服务，从而提高RAM利用率并支持SQLite与Postgres存储之间的安全切换。
...
如果您没有直接访问具有投票权的密钥，或希望由另一个密钥代为投票，请参考[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
投出您的投票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced` v0.2.12或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护人员 — 此次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个纪元）。

## 2026 年 7 月 20 日

**[仅 devshard 升级的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放审核**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整的节点升级，不影响主网行为，预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级时实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还修复了错误和安全问题。

**操作项**

请审核 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现任何问题、疑问、改进建议、边界情况或潜在漏洞时留下评论。

有意义的审核贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一轮升级周期的社区奖金。

本次仅为 PR 审核请求，不启动正式投票。
...
* 在区块 **5,105,276**（2026 年 7 月 17 日，约 12:05 UTC）前声明意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在纪元 331 的 PoC 开始前约 500 个区块的窗口期内（区块 **5,105,776** — 2026 年 7 月 17 日 12:50:53 UTC / 05:50 PDT），将您的 MLNode 切换为 Kimi。
...
同一提案同时从 `devshard_escrow_params` 中移除 `approved_versions` 的 v1 和 v2 版本。一旦批准，`versiond` 将停止 v1 和 v2 的 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。此举消除了旧运行时容器长期存在的内存增长问题 —— 这正是 7 月 15 日事件中网络节点 OOM 的根源。

**这标志着 v3 切换最终完成。** 仍通过 v1/v2 前缀或经典 `/v1/devshard` 路径路由的网关将在提案通过后停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 投票 —— 紧急窗口较短。
2. 提供 Kimi 服务的主机：在投票结束后立即声明 `moonshotai/Kimi-K2.6` 意图，并在纪元 331 的 PoC 开始前约 500 个区块的窗口期内（区块 **5,105,776**，2026 年 7 月 17 日，约 12:50 UTC）切换您的 MLNode。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在进行引导委托时：**不要委托给守护节点**；将委托分散到独立的 Kimi 主机上。请参阅 [多模型概念验证指南](https://gonka.ai/docs/host/multi_model_poc/) 以获取最新的委托指导。
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（紧急，0.667 赞成阈值；出席率重要，请尽快投票）。
- 意向截止时间（第330轮）：区块 **5,105,276** —— 7月17日，约12:05 UTC（约05:05 PDT）。
...
`moonshotai/Kimi-K2.6` 在第328至329轮失去了PoC验证多数票（详见[此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的方式——与六月提案78的恢复路径相同。

该提案将在下一轮PoC前从PoC参数中移除 `moonshotai/Kimi-K2.6`，随后立即重新添加，并让Kimi从第331轮开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票——紧急窗口很短。
2. 运行Kimi的主机：请保持您的Kimi设置处于待命状态，并准备在引导PoC时切换回您的MLNode。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（PDT）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（紧急，0.667 赞成阈值；出席率重要，请尽快投票）。
* 引导意向截止时间和轮次时间请参阅引导说明。

## 2026 年 7 月 15 日

**Kimi-K2.6事件（第328–329轮）：事件经过、恢复计划及委托指导变更**

在第328轮，`moonshotai/Kimi-K2.6` 失去了PoC验证多数票。在第329轮开始时，运行Kimi的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行Kimi的服务器同时出现故障：

* 其中一台服务器上，运行Kimi的MLNode因供应商端问题而崩溃。大量Kimi委托集中在这台主机上，因此其故障导致这些投票立即失效。
* 第二台服务器因内存耗尽而失去了其网络节点。

Kimi 的验证投票一直接近 2/3 的阈值，因此这足以使该组低于多数。守护者投票决胜机制未启动：目前没有每模型投票权重的守护者已被从决胜机制中过滤掉。这是一个已知问题，已在即将发布的 v0.2.14 升级中修复。

这种情况与第 306–307 个周期的事件类似。第 329 个周期开始时的投票确切细节仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导重新添加它（约 1.5 个周期）：

1. [今天] 一个加速提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意向、部署窗口、PoC 存储提交。意向截止前将发布操作说明。

**委托指导变更 — 请阅读**

之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤销：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主要机制和备用机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个运行模型的独立主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已在流程中的修复*** PoC 验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完成全面迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦加速的 Kimi 提案上链，请对其进行投票（公告将随后发布）。
2. 如果您计划提供 Kimi：关注引导说明，并在意向截止前做好声明准备。
3. 如果您为 Kimi 委托：选择非守护者的委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量至 `/devshard/v3`。这将解决 RAM 增长问题，并在移除 v1/v2 之前必须完成。
...
此发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于此类仅限 devshard 的运行时升级，无需重启主网或手动主机操作。

**操作项**

白名单 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们在链升级运行期间继续提供推理服务，而不依赖已弃用的经典 API 路径。

**关键变更**

- 为经纪人做好准备，使其在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。
- 改进了 RAM 利用率。
- 修复了网关运行时行为。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并实现了 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整的链软件升级。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期运行时前缀，直到代理将流量切换到 v3。在此类升级期间，无需重启主网或手动执行主机步骤。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何从冷密钥向热密钥授予治理投票权限。
...
devshard 部分正在准备 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率并实现 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布并部署 devshard v3 版本，然后才进行主网链升级。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级期间继续提供推理服务。
...
5. 请直接审查 PR 代码，并对发现的问题、疑问、改进建议、边缘情况或漏洞留下评论。
...
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为在主网升级期间保持以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。若您已完成以上两步，则无需重复操作。若您有多个节点，请逐个更新，并在 PoC 或 cPoC 外执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复补丁 — 可异步部署，通过强化 PoC-v2 验证阻止攻击，使复制的计算无法通过采样检查。

请更新您的 API 容器。
请确保在 PoC 或 cPoC 外执行此步骤。
部署方式（一次一台机器以降低风险）：
```
...
`moonshotai/Kimi-K2.6` 在第 311 个纪元再次尝试引导。希望提供 Kimi 服务的主机需在第 310 个纪元声明意向，并在新纪元开始前准备好节点。一个守护节点也将运行 Kimi，因此不希望运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 个纪元、区块 **4,797,456**（约 6 月 28 日 12:00 UTC）之前声明 PoC 意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若有足够主机声明意向，请在第 311 个纪元开始前约 500 个区块的窗口内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
> **更新（2026年7月15日）：此建议已撤销——请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。有关更新的委托指南，请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。

**关键时间点*** 意向截止时间（第310轮）：区块 **4,797,456** —— 约6月28日，12:00 UTC。
* 第311轮开始：区块 **4,797,956** —— 约6月28日，12:47 UTC。

有关引导流程的更多内容：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定为：在8xB300硬件上运行Kimi时，其共识权重约等于主模型MiniMax。此设定并非偏向任何硬件——MiniMax已具备相同的权重。在8xB300主机上运行Kimi而非MiniMax，可获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数设定为：在8xB300硬件上运行GLM-5.2时，其共识权重约等于MiniMax。此设定并非偏向任何硬件。在8xB300主机上运行GLM-5.2而非MiniMax，可获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及>2/3可达性）。这使操作员可在配置前确认模型是否具备可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口内为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由在PoC开始时实际提交PoC存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮起（PoC开始时间约为6月26日15:25 UTC；生效时间约为16:00 UTC），前提是各模型满足引导资格：

* `moonshotai/Kimi-K2.6` 再次成为活跃的PoC模型。
* `zai-org/GLM-5.2-FP8` 可作为可选PoC模型，无非参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动引导。每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时紧急周期可以。创世守护节点预计支持本提案。

**主机需执行的操作**

1. **若要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。

2. **若要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口内配置您的MLNode。提供GLM-5.2是自愿的——未选择的主机不会受到惩罚。
...
如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 访问，任何活跃节点均可使用：

* http://node1.gonka.ai:8000
* http://node2.gonka.ai:8000
* https://node3.gonka.ai

投出一票 (`yes`, `no`, `abstain`, `no_with_veto`):
```
    export NODE_URL=https://node3.gonka.ai/
...
**提案78通过：`MiniMaxAI/MiniMax-M2.7` 现为唯一的PoC模型；Kimi K2.6和Qwen3-235B已移除**

提案78的紧急投票已通过。这些变更从第308个纪元起生效。

**当前生效内容**

1. 委托 `initial_model_id` 被设置为 `MiniMaxAI/MiniMax-M2.7` —— MiniMax 为基础模型。
...
* 计划再次提供Kimi的主机应保留Kimi的设置并准备好在PoC 309时将MLNode切换回来——在第308个纪元将进行一次恢复Kimi的投票。

**下一步**

在第308个纪元将进行一次紧急投票，合并两项变更，均在第309个纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6** —— 重新添加Kimi并重启其引导程序。
* **添加GLM-5.2** —— 添加GLM-5.2 **不设不参与惩罚**，以便主机可选择加入，并提前衡量模型需求，对不提供该模型的主机不施加任何惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快且停机时间最短的恢复方式。
...
**每项变更的目的*** **Kimi K2.6 — 移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的路径。下一纪元将进行恢复Kimi的投票。
* **Qwen3-235B — 退役（独立且无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容变更。它与Kimi情况无关，仅因同为PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更合并为一次紧急投票，因为重置必须在第307个纪元结束前完成。每个纪元约22.9小时，标准48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持此提案。

**若获通过的效果**

自第308个纪元起（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时无cPoC。请现在预下载FP8权重（若尚未准备），以避免在纪元边界遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时切换回来——在第308个纪元将进行恢复Kimi的投票。


**接下来的内容*** **恢复 Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前测试模型需求，对选择不提供该模型的主机无任何惩罚。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
devshard v2 运行时已通过链上批准并添加至 `DevshardEscrowParams.approved_versions`。

本提案涵盖 devshard v2 发布版本：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0)

这是首次仅针对 devshard 的运行时升级。它独立于完整链软件升级，无需链二进制升级。

提案通过后，v2 现在可与现有的 v1 devshard 运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有 v1 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于此类仅 devshard 的运行时升级，无需重启节点容器或手动执行主机步骤。

**关键变更**

- 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话在内存或状态中保留所有已服务的推理。
- 通过 OpenTelemetry 和 Prometheus 添加了内部 devshard 跟踪和指标。
- 通过 Grafana、Jaeger、Prometheus、Loki、Promtail 和 cAdvisor 添加了 join-stack 可观测性。
- 将每个推理的验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每个插槽的总数。
- 在纪元变更时修剪旧纪元存储，将 SQLite/Postgres 模式设置移出热路径，并强制每个进程仅选择一个存储后端。

## 2026年6月15日

**v0.2.13-devshard-v2 运行时升级提案已进入治理阶段。**

本提案涵盖 devshard v2 发布版本：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对 devshard 的升级。它独立于完整链软件升级。若获批准，v2 将与现有的 v1 devshard 运行时并行运行。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话在内存或状态中保留所有已服务的推理

2. 通过 OpenTelemetry 和 Prometheus 添加内部 devshard 跟踪和指标

3. 通过 Grafana、Jaeger、Prometheus、Loki、Promtail 和 cAdvisor 添加 join-stack 可观测性

4. 将每个推理的验证计数器存储在状态根之外的 SQLite/Postgres 中，并在推理修剪后通过 devshard 统计端点暴露每个插槽的总数

5. 在纪元变更时修剪旧纪元存储，将 SQLite/Postgres 模式设置移出热路径，并为每个进程选择恰好一个存储后端
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。
...
**[仅适用于devshard的升级PR](https://github.com/gonka-ai/gonka/pull/1289) 现已开放供审查。**

这是首次仅适用于devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。

如果通过治理流程批准，新版本的devshard将与现有的v1运行时并行运行。

请直接审查PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期获得社区奖励。

这仅是呼吁审查PR。它不会启动正式投票。治理投票流程将在审查期结束后开始。

## 2026年5月28日

**MiniMax-M2.7 现已在 Gonka 网络上激活**

v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7加入Qwen3-235B和Kimi K2.6，成为活跃模型组，MiniMax组中获得的PoC权重现在以校准系数0.3024转换为共识权重。

MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获**批准**，并在主网区块 `4267300` 成功执行升级。

## 2026年5月25日

**升级 v0.2.13：预下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13 升级提案（提案编号 [54](https://github.com/gonka-ai/gonka/pull/1143)）已通过链上治理，升级现已安排。
...
升级将在主网上于**区块高度 4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）自动执行。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，也必须为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间是**链纪元 `278`**。不采取任何操作的主机将从纪元278开始，每纪元对其全部权重施加15%的惩罚。
...
5. v0.2.13校准将Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映Qwen-on-B200参考的vLLM-0.20.1吞吐量基准。该调整**仅适用于您共识权重中的Kimi部分**；您的Qwen部分权重和Kimi内部PoC分布保持不变。在B200/B300上，Kimi仍是收益最高的选项；在H100/H200上，MiniMax-M2.7成为与Qwen相当、高于Kimi的选项。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，现提交了若干更新。

- 包括：Kimi（`0.78`）重新校准权重、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复。
- 在主网上激活以太坊桥接（见下文专门章节）。
- 该提案将升级后的宽限期延长至3000个区块，以便在新快照逻辑稳定期间，主机不会受到惩罚。
- 治理：将创世守护者投票权降至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%投票权中实现约1/3的参与率才能满足法定人数（参见inference-chain部分）。
- 所需准备工作：桥接容器检查、MiniMax决策、仪表板更新、投出投票。
- 在提案获得批准之前，链上没有任何变化。
...
- 在vLLM 0.20.1发布后，基于Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B：`0.359`（未变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下，Kimi的每轮共识权重降低约38%）
    - MiniMax-M2.7：`0.3024`
...
- 激活后，桥接将支持Gonka主网与Ethereum之间的跨链转账（包括在Ethereum上包装GNK以及桥接USDC/USDT）。包装/解包脚本和操作员工作流程将另行文档化。
...
所有主机需验证其网桥容器已部署、运行最新版本并同步正确。部分主机可能已部署网桥容器。在这种情况下，请先检查您是否运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

主机需更新仪表板/浏览器。请从`gonka/deploy/join`目录运行以下命令。如果您尚未在本地克隆`gonka`仓库，请先遵循加入网络指南。此仪表板更新仅涉及容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 运营商时间线：投票截止于5月22日22:12 UTC → 升级高度约5月26日14:42 UTC → 升级周期剩余阶段跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在 start_poc − 500 块处（约提前43分钟）→ 首个MiniMax PoC阶段在升级后下一个纪元边界开始 → MiniMax惩罚在链纪元278执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中Cosmovisor对 `application.db` 的备份通常为数十GB，请提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 该提案将有意从升级高度到升级纪元结束（10000区块宽限期）跳过确认PoC。若获批准，此跳过是预期行为而非故障；新快照逻辑将在下一个纪元开始。
- 若获批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储相同的环境变量）。本地SQLite将保持默认，且会自动清理（保留最近3个纪元）。
- 若提案失败（未达到法定人数，或 `no_with_veto` 超过⅓），链上无任何变化，升级将不会发生。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
在应用额外保护措施期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。

**发生了什么**

在纪元 #266 中，网络的活跃权重大幅下降。

该问题已追溯到具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。
...
在相关保护措施实施期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。

**纪元 #267 中Kimi的权重**

由于现有协议规则，Kimi在纪元 #267 中的活跃权重较低：任一模型的总权重不得超过前一纪元所有模型总权重的75%。
...
2. 监控接下来几轮的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重正确反映。
...
社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更清晰的了解，将及时分享更新。
...
- 包括：为Kimi（`0.78`）重新校准权重、新增模型 `MiniMaxAI/MiniMax-M2.7`、更新验证阈值、devshard存储重构，以及多项PoC/奖励修复和以太坊桥接主网激活。
- 提案在升级后延长了宽限期，确保升级发生后3000个区块内不惩罚主机。
...
- 当参与者再次活跃时重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问问题。
- 将创世守护者的调整投票权降低至约25%，并将链上治理门槛设为 `0.25`。在守护者不投票的情况下，这使得剩余75%投票权的有效门槛为1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您是否运行的是当前版本，然后再采取任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 请在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。

## 2026年5月7日

**桥接容器更新/验证要求**

作为即将进行的 v0.2.13 升级准备工作的一部分，可能包括以太坊端合约激活，所有主机需验证其桥接容器已部署、运行最新版本并正确同步。
...
- 确认PoC在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被大幅削减。修复方案是存储一个可确认模型和权重缩放因子的单个纪元快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推理就可能再次使其失效。现在在重新激活和即将晋升时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授予中未包含`MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应交易商投诉。
...
- 升级为当前纪元安装了一个宽限期条目，其中延长了`UpgradeProtectionWindow`（3000个区块）。从升级高度到升级纪元结束期间，将跳过确认PoC触发器，因此新的快照逻辑仅从下一个纪元开始运行。复用了v0.2.10的宽限期原语。
- Wasm守护进程访问在应用连接后解析，因此合约权限检查适用于桥接和流动性池操作。

**decentralized-api**

- 某些OpenAI兼容的上游服务返回数值型`stop_reason`值。`Choice.StopReason`现在接受任何JSON类型，因此这些响应不再因反序列化失败而中断。
- 内部devshard存储迁移不再阻塞dapi启动。在迁移和恢复完成前，devshard路由仍不可用。

**devshard**

- 由于旧的托管数据保留在一个SQLite存储中，devshard存储可能无限增长。现在存储按纪元范围划分，并在后台清理旧纪元，仅保留最近3个纪元。
- Devshard需要为大规模部署提供共享存储选项。现在可以使用Postgres作为主存储，SQLite仍作为本地后备存储。
- Postgres数据按`epoch_id`对会话、差异和签名进行分区，以便清理时能干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
- 有效载荷查找固定于托管纪元，并为纪元边界和遗留纪元0请求提供回退机制。
- 当前纪元分片统计信息暴露了nonce、版本、组和每主机计数器。
...
审查者可在此处查看完整的升级提案、迁移详情、测试摘要和建议流程：
...
请更新api容器的二进制文件。每个PoC开始前有500个区块的无CPoC（`confirmation_poc_safety_window`）窗口，因此这可能是部署最安全的版本。
...
docker start api
```
部署后，请再次确认9100和9200端口上的服务器是否正在运行：
```
...
在上一个纪元中发现Kimi-K2.6某些响应解析存在小bug。
...
安全模型得以保留：PoC验证本身未发生变化，仍需超级多数的验证算力才能接受结果。

该提案已加速处理，以便在下一次PoC之前生效。投票将持续12小时。
...
对于多模型 PoC，这意味着 Kimi 现在作为活跃模型组拥有独立的参与和奖励跟踪机制。
...
今天的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，其权重极低。请将您的 MLNode 保留为 `Qwen235B`，并于明天提交下一个周期的意向。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自身的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个节点必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使旧版验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。

**对节点的指导**

- 部署、委托或明确拒绝新治理批准的模型（所包含的模型将在升级后两个周期激活）。请参阅[指南。](https://gonka.ai/docs/host/multi_model_poc/)

- 请更新仪表盘/浏览器。请从 `gonka/deploy/join` 目录运行以下命令：
...
- 迁移：测试和迁移详情见 [v0.2.12 文档。](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/upgrades.md)
...
每个 DAPI 会在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本会查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个带有清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。剩余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
[升级提案](https://github.com/gonka-ai/gonka/pull/948) 的下一个链上软件版本 v0.2.12 已发布到链上并开放投票。
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议加固**。保留的节点（`POC_SLOT=true`）将随机抽样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、对齐遗留验证者惩罚与所需抵押品语义、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 投票结束：2026 年 4 月 30 日 00:12 UTC
- 升级高度：3834200
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个治理批准的模型生成其自身的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
- **共识层交易费用与自动迁移**（[#937](https://github.com/gonka-ai/gonka/pull/937)，[#981](https://github.com/gonka-ai/gonka/pull/981)）。引入由治理控制的 Gas 价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免费。`MsgPoCV2StoreCommit` 采用双组件费用（基础验证 + 计数线性）作为主要 Sybil 防御机制。详情请参阅 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard 独立运行时**（[#1045](https://github.com/gonka-ai/gonka/pull/1045)）。将 devshard 发布与 DAPI/主网发布周期解耦。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- 确保冷钱包账户持有足够的余额（例如 100 GNK），以覆盖自动授予的费用限额支出。
...
现已推出新版本的 mlnode：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
此版本现已设为主分支中的默认版本：[https://github.com/gonka-ai/gonka/commit/ec8f45573149ce5686e8e5fc29f1a8f49a295689](https://github.com/gonka-ai/gonka/commit/ec8f45573149ce5686e8e5fc29f1a8f49a295689)
...
鼓励主机查阅[GitHub](https://github.com/gonka-ai/gonka/pull/813)上的提案并投票。
提前下载二进制文件有助于避免在升级窗口期间依赖GitHub的可用性。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅通过本地测试难以暴露。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳多达100倍的推理量，具体取决于工作负载和网络条件。  ￼
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间开始大规模剪枝活动，造成不必要的运营压力。
...
本次升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理扩展性。

目前，通过每笔链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这故意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已被认为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时开始剪枝活动，造成不必要的操作压力。
...
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请与该密钥的所有者联系，请求授予您代其投票的权限。没有该授权，其他账户无法提交投票。
...
API二进制文件的新版本已发布。它更新了连接超时处理，并在PoC验证流水线中引入了额外的检查。

1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格5分钟超时，而某些请求可能需要更长时间。新版本的API将此值恢复，不再强制执行严格限制。
2. 之前的请求重试系统即使因处理超时（而非TLS超时）失败也会重试推理。
服务器端对长请求的重试通常无效，因为它会导致相同的超时情况。同时，客户端可能收到不一致的输出。新版本的API在这些情况下不再重试推理。
3. 当前被保留且不参与PoC生成的MLNode仍被用于PoC验证。这可能导致推理遗漏。新版本将此类节点排除在PoC验证之外。
4. PoC验证流水线中增加了额外的保护措施。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低PoC启动时仍处于活跃状态的请求可能导致权重下降的风险。
...
抵押参数更新提案未达到法定人数，因此根据当前治理规则被否决。这意味着更新后的参数将不会生效。

如前所述，第180个周期的抵押激活独立于此次投票。

由于提案未通过，创世时期定义的抵押参数将在第180个周期自动生效。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选择。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
由于有效归一化窗口比早期名义模型长约5个区块，因此每个周期重新计算的权重成比例降低。
...
由于启用了PoC持续时间归一化，且有效的参考窗口现在比先前的名义假设长约5个区块，从第176轮开始的权重值反映了更新的计算模型。
从PoC权重推导H100等效容量或奖励预测的追踪器和仪表板，应从第176轮开始验证其转换系数。
...
针对升级提案v0.2.10的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **当前生效的关键变更****PoC验证采样优化**
    ...
    此次升级引入了一种新的PoC验证机制，通过为每个参与者分配一组固定的采样验证者，将复杂度从O(N^2)降低到O(N x N_SLOTS)。

    **基于实时的PoC权重归一化**

    此次升级根据实际的PoC耗时对PoC参与者的权重进行归一化，以减少区块时间漂移的影响，并确保权重结果与实际执行时长保持一致。

    **启用Qwen235B工具**

    此次升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 添加了工具调用参数（ `--enable-auto-tool-choice` , `--tool-call-parser hermes` ），并设置了验证阈值0.958。
    要启用工具，必须重启MLNode容器内的vLLM。

    **其他协议更新**

- 修复：PoC与CPoC交集错误（PR #752）。
...
**宽限期**：升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并对升级所在周期的错过率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工最多只会损失0.5%的抵押品。所需抵押品仅占日收益的约24%。

**警告**：无论投票结果如何，抵押参数都将生效。如果该提案未通过，创世时定义的抵押参数将在第180周期自动生效，而非上述参数。

投票结束后、第180周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其收益将被削减至五分之一。
...
下一个链上软件版本v0.2.10的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的PoC验证优化（默认关闭）并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并对升级所在周期的漏报率和无效率阈值放宽。

**其他协议更新**

- 修复PoC与CPoC交集的bug（PR #752）
- 将IBC栈升级至v8.7.0。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中旧备份的指导，请参见[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
距离第180纪元不足7天，现在是准备的时候了。

根据AMA讨论及社区成员提出的论点，建议从较低的抵押要求和最小的惩罚开始。
...
这意味着即使在一个周期内被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押品仅占每日奖励的约24%。

提案提交投票后将另行发布公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果稳定性得到证明，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止。这是因为节点运行的是不再与网络兼容的旧版本二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
当前正在评估此重叠的影响。初步观察表明，没有节点记录PoC提交，导致该纪元累积权重为零。

作为预防措施，矿工通过协调行动暂时停止了区块生产。

问题正在定位中。

请保持在线，以便在需要时立即应用补丁。更多详细信息和补丁说明将在准备就绪后发布。
...
本次仅为对拉取请求的审查呼吁，而非正式投票的开始。治理投票流程将在审查期结束后启动。

**关键变更****[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC 验证采样优化**

此升级引入了一种新的 PoC 验证机制，通过为每位参与者分配固定的采样验证者集合，将复杂度从 O(N^2) 降低至 O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际 PoC 时间标准化 PoC 权重**

此升级根据实际 PoC 已用时间标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式发放转账
...
- **[PR #727](https://github.com/gonka-ai/gonka/pull/727)**：对 #634 的后续，将响应写入器传递给 `http.MaxBytesReader` 并对齐测试。
...
- **[PR #545](https://github.com/gonka-ai/gonka/pull/545)**：API 流中的小错误修复。
...
3. 如果链上提案通过，此PR将在链上升级执行后合并。

从[upgrade-v0.2.10](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10)分支创建发布版本（而非`main`）可最大限度减少`main`分支上`/deploy/join/`目录中容器版本与链上二进制版本不匹配的时间，从而为新主机提供更顺畅的接入体验。
...
- 在PoC之外使用标准vLLM推理对同一模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`进行的控制推理基准测试，显示了与PoC v2中观察到的相同GPU类型之间的相对性能差异。

**追踪器（仪表板）维护者操作**

在更新权重分配生效后，追踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新 — 补丁可用**

现已提供补丁以解决导致PoC周期内块验证暂停的问题。建议主机尽快应用该补丁，以确保正确的PoC验证行为并安全恢复块生产。

**需要采取的操作**

请主机尽快应用补丁，以确保正确的PoC验证行为并安全恢复块生产。
```
# Download Binary
...
由于在当前的PoC周期中，验证阈值可能无法达到的高风险，主机们集体采取行动暂停了区块验证。
...
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将来临的纪元的参与者集合中。
...
在因 cPoC 惩罚而减少或排除奖励的情况下，未计入部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
下一个链上软件版本v0.2.9的升级提案现已上链并开放投票。若获得批准，该提案将启用PoC v2进行权重分配，并通过链上治理完成向传统PoC机制的过渡。

**关键变更****PoC v2激活**

- PoC v2用作权重分配的活跃机制
- 确认PoC（V2追踪）用作结果的权威来源
- 传统PoC逻辑不再用于权重计算
...
在因cPoC处罚而减少或排除奖励的情况下，未计入部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
在最新纪元中，大量 ML 节点未获得 PoC 权重。
分析表明，这是由于 PoC 验证参与不足所致。在多个案例中，参与者发布了 nonce，但未执行验证，或验证水平远低于协议要求。
下表列出了上一纪元具有权重、在当前纪元提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
!!! note 
    一旦网络过渡到单模型配置，未能满足上述两项条件的节点将失去 PoC v2 参与资格。PoC v2 的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
PoC v2 仅在链上治理提案获得批准时，才成为权重分配的激活方法。
    在此提案获得批准之前：
...
- 包含双模式迁移策略：V1 用于常规 PoC，V2 用于在 rollout 期间跟踪 Confirmation PoC。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
- 授权绕过与重放：将 epochId 绑定至签名，并验证授权是否针对正确的 epoch。

由于变更量较大，此处仅列出部分重点内容。完整的更新与修复列表请参见 [GitHub 拉取请求。](https://github.com/gonka-ai/gonka/pull/539)
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不被视为 PoC v2 参与资格节点。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
关于稳定期延长的最近治理投票已成功通过。稳定期现已正式延长，以允许进行额外的测试和网络升级。

**主机操作项**

在延长确认后，请利用这段时间为新的 PoC 要求准备您的设备。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步执行更新。
...
未包含在 `allowlist` 中的主机在延长的稳定期内仍将无法参与 PoC，该稳定期将在区块 2443558 结束。
...
当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 处的参与者注册截止时间对齐。
所有参与逻辑保持不变。此提案仅解决了现有过滤逻辑中的一个小问题。

**可复现性与方法论**

`allowlist` 通过公开可观察的链上数据，使用预定义的一组硬件配置模式推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
`Allowlist`过期时间：自动在区块 2,222,222。
...
该提案已获批准。此提案定义了一个临时 `allowlist`，用于反映在最近多个周期中行为保持一致的参与者。
...
2) 之前表现出与这些模式持续偏差的参与者将被排除在 `allowlist` 之外，直到稳定窗口在区块 2,222,222 结束。

**执行特征**

- `allowlist` 将从下一个周期（第135个周期）开始生效
- 激活发生在第135个周期的首次PoC期间
- 无需软件升级
- 从此时起，`allowlist` 将持续有效，直至并包括区块 2,222,222

**可重复性与方法论**

- `allowlist` 仅由公开可观察的链上数据推导得出
...
在成功采用解决PoC相关共识故障的补丁后，一项新的治理投票目前正在生效。

随着区块生产恢复正常，网络即将进入一个短暂的稳定期，为后续增长做准备。

此次投票定义了稳定期内参与者的 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为始终符合网络预期的参与者集合。

**投票范围**

若获批准，网络将临时运行一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的完整列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
- 观察到的PoC权重低于使用同类硬件的其他参与者所表现的权重的150%。
之前表现出这些模式持续偏差的参与者，在稳定窗口于区块 2222222 结束前，不会被纳入 `allowlist`。

**可重复性与方法论**

`allowlist` 基于公开可观察的链上数据，使用预定义的硬件配置模式推导得出。这些模式通过此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
- 无需软件升级。
- 在成功投票后的下一次PoC中，`allowlist`将生效，预计在区块：2089140。
...
- 投票截止：2026年1月10日，06:46:52 UTC。
- `Allowlist`生效：在下一次PoC执行于区块2089140后。
...
链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强的提案已获批准，并成功在主网上部署。
...
本升级中包含的预备变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的 payouts，统一应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
```
当所有命令均无错误完成且显示确认信息时，可认为二进制文件已成功安装。
```
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是故意试图破坏或压垮系统所致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。

**关键变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前使用过、有时间限制的防御机制，现提议重新启用。
...
此升级将此前通过手动API更新分发并已在网络上使用的关键修复正式化。这些修复包括：
...
- 主机级注册：在区块2,222,222之前（约两周后）暂停新主机注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期内暂停新开发者地址注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续到区块2,294,222（约19天）。

**治理控制机制**

本升级包含的预备性变更，为未来通过治理控制参与者入驻和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第 117 期奖励分配**

本提案涵盖与链暂停（第 117 期）相关的两项奖励分配：

- 在第117轮活跃但未收到奖励的节点，将补发该轮的遗漏奖励。
- 所有在第117轮活跃的节点，将额外获得等于1.083倍第117轮奖励的 payouts，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
如果您的 `application.db` 占用大量空间，可以使用 [此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 提供的技术进行清理。
...
此 `API` 版本改进了对失败推理的处理弹性，并减少了推理失败统计问题。它还为 PoCBatch 和 PoCValidation 交易引入了批处理功能。

**升级时间**

在确认 PoC 未激活时应用更新是安全的。
