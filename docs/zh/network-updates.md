...
这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从未激活集合中移除；提案88将其重新注册到治理模型列表中，使其从第331个纪元开始进入标准引导流程。

同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。此举消除了旧运行时容器长期存在的RAM增长问题——这也是7月15日事件中网络节点OOM的根本原因。
...
2. 提供Kimi服务的主机：在投票结束后、区块**5,105,276**（7月17日，约12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331个纪元PoC开始前约500个区块的窗口内（区块**5,105,776**，7月17日，约12:50 UTC）将您的MLNode重新启用：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间委托时：**不要委托给守护节点**；将委托分散到独立的Kimi主机上。有关最新的委托指南，请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
- 意向截止日期（第330纪元）：区块**5,105,276**——7月17日，约12:05 UTC（约05:05 PDT）。
- 第331纪元PoC开始：区块**5,105,776**——7月17日12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6`在第328至329纪元失去了PoC验证多数（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从激活集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式——与6月提案78的恢复路径相同。

该提案将在下一次PoC之前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将从第331纪元开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票——紧急窗口时间很短。
2. 提供Kimi服务的主机：保持您的Kimi设置就绪，并准备好在引导PoC时重新启用您的MLNode。
...
投下您的选票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 v0.2.13 或更高版本的 `inferenced`。
```
export NODE_URL=https://node3.gonka.ai/
...
在纪元 328 中，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数支持。在纪元 329 开始时，运行 Kimi 的主机被移出组。其余网络正常运行。

**发生了什么**

两台由守护者运营的运行 Kimi 的服务器同时故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障导致这些投票立即消失。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者投票决胜机制未触发：当前未被赋予每模型投票权重的守护者会被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。
...
* **请勿委托至守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主机制与后备机制绑定在同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请将委托指向您自己运行该模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已在进行中的修复*** 守护者参与 PoC 验证（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。在完全迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请立即投票。
2. 如果您计划提供 Kimi：关注引导说明，并在意图截止前准备好声明意图。
3. 如果您为 Kimi 委托：请选择非守护者委托目标；尽可能将权重分散到多个独立主机。
...
在提案获批准后，v3 现在与现有的 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换到 v3。

本次发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需重启主网或执行手动主机步骤。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这使他们在链升级运行期间仍能继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
v3 运行时使经纪人能够在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
本次发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。新进程通过 `/devshard/v3` 前缀提供服务。现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换到 v3。在此类升级期间，无需重启主网或执行手动主机步骤。
...
主网链/API 工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（在主网上禁用 `/v1/chat/completions` 计费并从API二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收者路由，以及升级时的安全修复。

devshard部分为v3运行时做准备，以便代理在链升级期间无需依赖已弃用的经典API路径即可提供推理服务，从而提高RAM利用率并实现SQLite与Postgres存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从本分支单独发布并部署devshard v3版本，然后在主网链升级前完成。提前将推理流量切换到 `/devshard/v3` 的代理可在链升级运行期间继续提供推理服务。

**建议流程**

1. 活跃主机在[GitHub](https://github.com/gonka-ai/gonka/pull/PR)上审查本提案。
2. devshard v3版本将在主网链升级前提出并部署。
3. 代理将推理流量切换到 `/devshard/v3`。
4. 如果链上提案获得批准，此PR将在链上升级执行后立即合并。
5. 请直接审查PR代码，并对您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
...
devshard v3版本将在主网链升级前提出并部署，以便代理可在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作事项**

1. 现在 — 审查PR。请阅读GitHub上的PR #1267，并对任何发现、问题、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的API和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `ap`i 二进制文件和网桥镜像更新至0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两步，则无需重复操作。如果您有多个节点，请逐个更新，并在PoC或cPoC之外执行此步骤。
3. 对devshard v3进行投票。
4. 代理 — 将推理流量切换到 `/devshard/v3`。一旦devshard v3版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 — 准备调整指标统计方式。详细说明将在 `devshard v3` 投票启动并接近结束时发布。

## 2026年7月6日

**安全更新：PoC-v2权重验证强化 — 更新您的API容器**

上周，在HackerOne上报告了PoC-v2权重路径中的一个漏洞。参与者可膨胀其计算权重，从而影响区块奖励、共识投票权和治理投票权。

根据历史数据，此前未发现此类攻击。但在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。

针对即将发布的v0.2.14链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅API的热修复版本 — 可异步部署，通过强化PoC-v2验证阻止攻击，使复制的计算无法通过采样检查。
...
**Kimi-K2.6引导：第311纪元的再次尝试**

`moonshotai/Kimi-K2.6` 在第311纪元再次尝试引导。希望提供Kimi服务的主机需在第310纪元声明意图，并在新纪元开始前准备好节点。守护节点也将运行Kimi，因此不愿运行Kimi的主机可将其权重委托给该节点。

**将运行Kimi的主机**

1. 在第310纪元、区块 **4,797,456** 之前（约6月28日 12:00 UTC）声明PoC意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意图，请在第311纪元开始前约500个区块的时间窗口内将节点切换至Kimi（PoC开始约6月28日 12:47 UTC）。

**不运行Kimi的主机**

将您的权重委托给运行Kimi的主机（或发送拒绝声明）：
```
...
> **更新（2026年7月15日）：此建议已撤销——请勿向守护节点委托。** 守护节点是 PoC 验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。请参阅[多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)获取最新的委托指导。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一并恢复。此系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此设计并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此设计并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及>2/3可达性）。这使操作者能在配置硬件前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由在PoC开始时实际提交PoC存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会被激活。

**若获批准的效果**

从第309轮开始（PoC开始约6月26日15:25 UTC；生效约16:00 UTC），前提是各模型满足引导资格：
...
提案必须在第308轮结束前完成，以便两个模型能顺利引导进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计支持本提案。

**主机需执行的操作**

1. **要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，此期间切换是安全的，因为该窗口内无cPoC。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* **添加GLM-5.2** —— 添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入，并提前衡量对该模型的需求，不提供该模型的主机无任何惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重新引导；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出组，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以便快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是最快且停机时间最短的恢复路径。下一个周期将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi情况无关，仅因同属PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307周期结束前完成。每个周期约22.9小时，标准48小时投票期无法容纳在一个周期内；而12小时紧急投票期可以。创世守护者应支持该提案。

**若获批准的效果**

自第308周期起（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个周期的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前约500个区块的窗口期内切换是安全的，因为此时无cPoC。请提前下载FP8权重（如尚未就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急投票窗口很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——第308周期将进行一次恢复Kimi的投票。


**下一步计划*** **恢复 Kimi K2.6** — 在第308个周期进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前测试模型需求，对选择不提供该模型的主机不施加任何惩罚。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
这是首次仅针对 devshard 的运行时升级。它独立于全链软件升级，无需升级链二进制文件。

提案通过后，v2 现在可与现有的 v1 devshard 运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有的 v1 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于此类仅 devshard 的运行时升级，无需重启节点容器或手动执行主机步骤。

**关键变更**

1) 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在内存或状态中。
2) 通过 OpenTelemetry 和 Prometheus 添加了内部 devshard 跟踪和指标。
3) 通过 Grafana、Jaeger、Prometheus、Loki、Promtail 和 cAdvisor 添加了 join-stack 可观测性。
4) 将每个推理的验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每个槽位的总数。
5) 在周期变更时修剪旧周期存储，将 SQLite/Postgres 模式设置移出热路径，并强制每个进程仅选择一个存储后端。
...
本提案涵盖 devshard v2 发布版本：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对 devshard 的升级。它独立于全链软件升级。若获得批准，v2 将与现有的 v1 devshard 运行时并行运行。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在内存或状态中
...
4. 将每个推理的验证计数器存储在状态根之外的 SQLite/Postgres 中，并在推理修剪后通过 devshard 统计端点暴露每个槽位的总数

5. 在周期变更时修剪旧周期存储，将 SQLite/Postgres 模式设置移出热路径，并为每个进程选择恰好一个存储后端
...
新进程通过 `/devshard/v2` 前缀提供服务。现有的 v1 devshard 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。在此类升级过程中，无需重启节点容器或手动执行主机步骤。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。
提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用。可用节点包括：
...
**[仅用于devshard升级的PR](https://github.com/gonka-ai/gonka/pull/1289) 现已开放评审。**

这是首次仅针对devshard的升级，因此流程不同于标准链升级。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务停机。
...
MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需采取任何进一步操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为**每个**经治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一两个模型的主机，也必须为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元 `278`**。未采取任何操作的主机将从纪元278起，对其全部权重施加每纪元15%的惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。请确保 `.inference/data` 有足够空间用于cosmovisor状态备份；如果 `application.db` 较大，请在升级前应用[cosmovisor备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，现已提交多个更新。

- 包括：Kimi（`0.78`）的重新校准权重、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及多项PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专用章节）。
- 该提案将升级后的宽限期延长至3000个区块，以便主机在新快照逻辑稳定期间不会受到惩罚。
...
**`MiniMaxAI/MiniMax-M2.7` 在第278个周期前选择参与方式（届时开始处罚）**

对于每个获得治理批准的模型，多模型PoC要求每个主机明确选择参与方式（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 之后不采取任何操作将导致处罚。在此阶段，您应提前决定首选选项，以便在提案通过且升级成功应用于主网时能够迅速行动。

**桥接容器更新/验证**

所有主机需验证其桥接容器已部署、运行最新版本且同步正常。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再进行任何进一步操作。
...
- 运营商时间表：投票于5月22日22:12 UTC结束 → 升级高度约在5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在start_poc − 500块时（约提前43分钟）→ 升级后下一个纪元边界开始第一阶段MiniMax PoC → MiniMax处罚在链纪元278执行。
...
- 若获批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite仍将为默认选项，并会自动清理（保留最近3个纪元）。
- 若提案失败（未达法定人数或 `no_with_veto` 超过⅓），链上无任何变化，升级不会发生。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
纪元#267中PoC验证已成功完成，受影响的主机已成功返回活跃集合。

纪元#266中的问题由针对运行Kimi模型主机的攻击引起。网络继续运行，但该攻击结合若干相关条件，导致许多参与者无法进入纪元#266。

在应用额外保护措施期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过多个社区代理和经纪人端点。

**发生了什么**

在纪元#266中，网络的活跃权重大幅下降。

该问题已追溯至具有非标准语义的推理请求。该攻击向量影响了运行Kimi模型的主机，并中断了它们的PoC参与。
...
在相关保护措施应用期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过多个社区代理和经纪人端点。

**纪元#267中Kimi的权重**

由于现有协议规则：单一模型的总权重不得超过上一纪元所有模型总权重的75%，因此Kimi在纪元#267中的活跃权重较低。
...
4. 预期推理服务将出现临时中断。访问可能不会立即在所有地方可用，预计会通过社区代理和经纪人端点逐步恢复。
...
- 包括：对Kimi（`0.78`）的重新校准权重、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复、以太坊桥接主网激活。
...
- 将devshard的nonce上限从 `20_000` 提升至 `1_000_000`。
- 将每个周期的最大devshards数量从 `100` 提升至 `500_000`。
- 修复新模型引导期间确认PoC奖励核算问题。
- 在本次升级周期剩余时间内禁用确认PoC，以便从下一个周期开始干净地启动新快照逻辑。
- 当参与者再次活跃时，重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问问题。
- 将创世守护者的调整投票权降低至约25%，并将链上治理最低通过票数设定为 `0.25`。在守护者不参与投票的情况下，这使得剩余75%投票权的有效最低通过票数为1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，然后再采取任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意从升级高度到升级纪元结束（3000个区块的宽限期）跳过确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
...
作为即将进行的 v0.2.13 升级准备工作的一部分，可能涉及以太坊端合约激活，所有主机均需验证其桥接容器已部署、运行最新版本并正确同步。
...
- 确认PoC在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致服务了合格模型和尚未合格模型的诚实矿工被大幅削减。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推断即可立即再次使其失效。现在在重新激活和即将晋升时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授权中未包含`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应经销商投诉。
...
- 升级为当前纪元安装了一个宽限期条目，延长了`UpgradeProtectionWindow`（3000个区块）。从升级高度到升级纪元结束期间，跳过确认PoC触发器，因此新的快照逻辑仅从下一个纪元开始运行。复用了v0.2.10的宽限期原语。
- Wasm守护进程访问在应用布线后解析，因此合约权限检查适用于网桥和流动性池操作。

**decentralized-api**

- 一些OpenAI兼容的上游返回数值型`stop_reason`值。`Choice.StopReason`现在接受任何JSON类型，因此这些响应不再因反序列化失败。
- 内部devshard存储迁移不再阻塞dapi启动。在迁移和恢复完成之前，devshard路由仍不可用。
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化后续模型的接入，建议将该阈值降低至10%。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，其权重极低。请保持您的 MLNode 使用 `Qwen235B`，并于明天提交下一周期的意向。
...
- **协议加固**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者罚没与所需抵押语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
- 迁移：测试和迁移详情请参阅 [v0.2.12 文档。](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/upgrades.md)
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议强化**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单个 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
本次仅为拉取请求审查呼吁，不启动正式投票。治理投票流程将在审查期结束后开始。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
现在可用新版本的 mlnode：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
v0.2.11升级提案的链上治理投票已结束。提案已获批准，并已在主网上成功执行。
...
这些性能提升使每个区块的推理次数最多可提升100倍，具体取决于工作负载和网络条件。
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)

**主机建议**

- **二进制版本**：通过链上升级流程更新。
- **迁移**：测试和迁移详情记录在[v0.2.11文档](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.11/docs/upgrades.md)中。

有关这些变更的更多详情，请参阅治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/)
...
提前下载二进制文件有助于在升级窗口期间避免依赖GitHub的可用性。
...
下一个链上软件版本v0.2.11的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会在同一时间启动剪枝活动，从而造成可避免的操作压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，不是因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
请密切关注并计划参与。投票是支持网络发展的最简单方式之一，有助于确保升级与参与者实际需求保持一致。
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请与该密钥的所有者联系，请求授权您代为投票。若无此授权，其他账户无法提交投票。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 开始时中断正在进行的推理请求，以降低因 PoC 开始时仍有请求处于活动状态而导致的潜在权重下降风险。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致的意外权重降低。协议不会自动补充抵押。
...
为使 PoC 权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的 PoC 参考窗口比先前的名义假设长约5个区块。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此次升级根据实际 PoC 持续时间标准化 PoC 参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
**宽限期：** 升级后引入了3000个区块的宽限期，期间不进行确认 PoC，并对升级时期的错过率和无效率阈值采取较宽松的设定。
...
这意味着在单个周期内，即使被惩罚，矿工最多只会损失其抵押品的 0.5%。所需抵押品仅占日奖励的约 24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果该提案未通过，创世参数中定义的抵押参数将在第180周期自动激活，而非上述参数。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于实时的权重标准化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个为期3000个区块的宽限期，在此期间无确认PoC，并且在升级时段内放宽了错过率和无效率阈值。
...
- 请在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 在升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用大量磁盘空间，可应用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理技术。
...
警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到证明，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
[此拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一个链上软件升级 v0.2.10 已开放审查。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约两天。
...
此升级根据实际 PoC 持续时间标准化参与者权重，以减少区块时间漂移效应，并使权重结果与真实执行时长保持一致。
...
- 在 PoC 之外使用标准 vLLM 基于相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与 PoC v2 中观察到的 GPU 类型间相对性能差异相同。

**追踪器（仪表板）维护人员的操作**

在更新的权重分配生效后，追踪器（仪表板）维护人员可能希望审查第 159 轮及之后的系数，以确保与当前 PoC v2 权重分配保持一致。

## 2026 年 2 月 2 日

**网络更新 — 补丁可用**

现已提供补丁以解决导致 PoC 周期中区块验证暂停的问题。建议主机尽快应用该补丁，以确保正确的 PoC 验证行为并安全恢复区块生产。
...
- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时执行任何后续步骤或缓解措施。
...
- 服务其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换至`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型
...
他们的总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或极低的总权重达到约 48%，这一比例严重偏高。
...
- Postgres：升级后现在可通过Postgres配置本地有效负载存储。
...
本指南说明主机应如何根据v0.2.8模型可用性变化和即将推出的PoC v2更新来更新其ML节点。ML节点配置需符合PoC v2要求，该要求自第155个周期开始生效。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
!!! note 
    ...
    `node-config.json` 仅在首次启动网络节点 API 或本地状态/数据库被删除时使用。若要全新重启，请编辑它。对于现有节点，模型更新应通过管理 API 进行。
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估是否具备向 PoC v2 过渡的条件。一旦观察到活跃 Host 中约 50% 的足够采用率，可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不被视为 PoC v2 参与资格有效。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个纪元中逐步执行更新。

**投票范围**

如果获得批准，网络将在现有 `allowlist`（由未表现出非标准硬件行为的主机组成）下暂时继续运行。
...
- 证明的PoC权重未超过同类硬件观察到的权重的150%

2) 之前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块 2,222,222 结束。
...
如果获得批准，网络将暂时使用一个 `allowlist`，包含在之前纪元中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：

- 报告的硬件特征已根据预定义的常见硬件配置模式进行评估，用于识别偏差和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，以及
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
之前表现出这些模式持续偏离的参与者在稳定窗口于区块2222222结束前不属于 `allowlist`。
...
每位活跃主机的参与对网络向前推进并恢复正常运行至关重要。请保持在线，并在说明发布后准备好应用更新。
...
此次升级正式将此前通过手动API更新分发且已在网络中使用的关键修复纳入协议。这些修复包括：
...
近期网络增长带来了若干挑战。在过去几天中，网络经历了多次问题，其中一些似乎是由故意破坏或施压系统的行为引起的。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。
...
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理控制参与者入驻和推理执行提供了支持，而无需额外的软件升级。本提案中未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第 117 期活跃的节点，将额外获得等于第 117 期奖励 1.083 倍的支付，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
如果您的 `application.db` 占用大量空间，您可以使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中的技术进行清理。
...
## 2026年1月5日
当前网络上观察到高于常态的推理遗漏率。
在许多情况下，这是由一个bug导致的：未被支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
