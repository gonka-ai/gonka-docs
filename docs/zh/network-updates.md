...
下一个链上软件升级 v0.2.16 的拉取请求现已开放审查 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)。有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一轮升级周期中获得社区奖励。

主网链/API 工作聚焦于可信权重、动态系数、交易费用组和 PoC 挑战。如果提案获得批准，治理、BLS 和 PoC 验证权将仅限于上一个纪元确认的算力，而新的算力仍可立即获得奖励。治理可为每个模型设置目标份额和系数范围。升级时这些范围保持关闭，因此系数和奖励不会变动。新增了消息组和气体规则，每个组默认禁用，因此协议仍不收取费用。获准的挑战者可将一个活跃主机从推理任务移至全容量 PoC，直至下一次常规 PoC。

**关键变更**

1. **可信权重。** 在 v0.2.16 之前，声称的算力突然增加会立即提升参与者在治理、BLS 和 PoC 验证中的权力。这使得未经确认的算力影响共识和其他参与者的验证。本次升级将该权力限制为上一个纪元已确认的算力。新加入或重新加入的参与者从 0 开始，若确认失败，则下一个基线设为 0。新的算力仍可立即获得奖励，仅对信任敏感的权力需等待一个确认纪元。
2. **动态系数 v1。** 网络需要每个模型的可预测吞吐量，以便用户能依赖其可用性。每个模型的目标吞吐量应符合预期需求。使用固定系数时，整个硬件类别往往转向同一模型，导致每模型的容量难以平衡和预测。本次升级允许治理为每个模型设置网络算力的目标百分比和系数范围。协议将动态调整该范围内的系数，使算力趋向目标。超过目标的算力按最小系数评分，以抑制供过于求。治理最初根据需求估算和数据源（如 OpenRouter）设置目标。后续版本可聚合主机估算，最终使用链上模型使用数据。此版本仅改变激励机制，不会自动切换主机部署的模型。升级时，最小值和最大值均等于当前比例，因此系数和奖励保持不变，直到治理开启范围。
3. **费用。** 历史上，协议未收取任何交易费用。因此攻击者可以以极低成本提交大量消息，而每个验证者却需承担处理和存储成本。本次升级对交易类型进行分组，并添加每消息的气体规则，使每组可单独定价。所有组默认禁用且不收费。治理稍后可启用并定价。升级提案信息也可在升级高度启用这些组。
4. **PoC 挑战。** PoC 和随机确认 PoC 证明主机声称的算力。在纪元其余时间，推理统计信息检查这些硬件是否用于协议分配的工作。高比例的遗漏或无效推理可导致主机被移除。这提供了强大的持续检查，但其敏感性取决于推理量，可能无法揭示声称算力与可用算力之间的所有差距。PoC 挑战为此类情况增加了一层额外安全。获准的挑战者可要求一个活跃主机离开推理，以全容量运行 PoC，直至下一次常规 PoC。为发起挑战，挑战者需锁定目标剩余纪元奖励的一部分作为付款。若目标通过，将获得该付款；若失败，挑战者获得退款，目标将受到与确认 PoC 失败相同的惩罚。挑战期间遗漏的推理不计入目标。仅允许名单中的 devshard 托管创建者可发起挑战。若允许名单为空，则任何人皆可发起挑战。
更多详细信息及其他变更见此处：[https://github.com/gonka-ai/gonka/pull/1535 ](https://github.com/gonka-ai/gonka/pull/1535 )

**行动项**

1. **所有人。** 现在 —— 审查拉取请求。阅读 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)，对任何发现、问题、改进建议、边缘情况或漏洞留下评论。
2. **主机。** 投票前无需任何操作。不要更新容器、重新授权密钥或为此升级迁移模型。
3. **仪表板维护者。** 若治理投票通过，治理、BLS 和验证投票将遵循上限权重。奖励仍基于实际权重。


## 2026年9月18日

**提案通过：批准 devshard v5 及奖励发放**

提案 ID 为 105 的链上治理投票已结束。

该提案以 123,754 票赞成、反对票数被批准。
...
主要协议新增功能是高度同步（height-sync），为 DevShard 会话提供已签名、可重播的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立协议级逻辑时间。此版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机所需操作**

无需操作。`versiond` 将下载新运行时，验证其 sha256，并与已运行的版本并行启动。`v3`、`v4` 和 `v4.1` 仍为批准版本，因此现有会话和路由前缀不受影响。这是一个运行时版本，而非网关发布：任何网关迁移将另行公告并附有独立指南。

**奖励**

该提案还从社区销售合约向多个开发周期中做出贡献的核心贡献者发放 91,300 USDT：@a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。所有支付均已链上执行。
...
提案 ID 105 将 `v5` 添加至批准的 devshard 版本列表，并从社区销售合约发放 91,300 USDT 的贡献者奖励。

**devshard v5**

这仅是 devshard 的变更。Cosmovisor 不涉及，链不会暂停，推理服务继续运行。若获得批准，`versiond` 将下载二进制文件，将其 SHA-256 与链上存储的值进行验证，并启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能是高度同步（height-sync），为 devshard 会话提供已签名、可重播的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级 QoS 建立协议级逻辑时间。此版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。
...
受益人为活跃于多个开发周期和版本的Gonka核心贡献者：@akup / @a-kuprin, @qdanik, @Ryanchen911, @aikuznetsov, @snevolin, @shd 和 @redstartechno。
...
投票将于 **2026年9月18日16:47 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成票，且反对票（含否决）低于33.4%。

请投票。此前的尝试因参与率不足而失败，无人反对，因此参与是唯一阻碍其通过的障碍。
...
**提案被拒绝：批准devshard v5及赏金发放**

提案ID为104的链上治理投票已结束。该提案因 `failed_reason`: `proposal did not get enough votes to pass` 被拒绝。

最终统计：赞成25291，反对0，否决0，弃权0。

无人投反对票。参与率约为网络权重的6%，低于25%的法定人数，因此提案仅因参与不足而失败。
...
自第394个纪元起，GLM-5.3-Flash的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供服务、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的15%，且该惩罚将在您做出选择前持续生效。
...
提案 ID 104 做了两件事：将 `v5` 添加到 devshard 版本的批准列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖金。

**Devshard v5**

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。提案通过本身不会迁移任何流量，因此经纪人可以随后逐步迁移其网关。
...
该提案还支付了 91,300 USDT 的贡献者奖金，分配给七位接收方：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成票，且反对并行使否决权的票数少于 33.4%。
...
**提案通过：报销 Daniil 和 David Liberman 参与 All-In Summit 2026 的费用**

提案 ID 103 的链上治理投票已结束。

该提案已获**批准**。从社区池中拨款 15,500 USDT，用于报销 Daniil 和 David Liberman（Gonka 协议的联合创始人）参加 All-In Summit 2026 的两张参会通行证费用。

峰会于 2026 年 9 月 13-15 日在洛杉矶举行。标准参会通行证每人 7,500 USDT，门票已预先支付。本次报销涵盖两张门票及提现交易费用，未申请报销航班、住宿及其他费用。
...
提案已获批准。Quant Mesh Limited被正式批准为Gonka-Ledger集成的法律签约方，并从社区金库拨付350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费75,000 USDT以及350 USDT的处理费用。授权和资金用途严格限于完整提案中描述的Ledger集成。
...
**提案通过：添加GLM-5.3-Flash，从PoC模型中移除Kimi-K2.6和GLM-5.2-FP8**
...
该模型及其参数由kaitaku.ai团队提出，经vbgd0独立验证，并联合提出。测量数据及阈值设定的依据见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash的激活纪元为**394**。自该纪元起，`penalty_start_epoch`生效，因此未明确选择的主机将面临不参与惩罚。如您计划提供该模型，尽早声明意向将有助于团队最佳组建。主机指南：[GLM-5.3-Flash启动指南](./host/glm-bootstrap.md)。
...
提案已获批准，`v4.1`已被加入批准的devshard版本列表。`versiond`将下载二进制文件，将其SHA-256与链上存储的值进行校验后启动运行时。主机无需手动操作。
...
最终投票结果：赞成312476票，反对0票，否决0票，弃权0票。

## 2026年9月9日

**提案101：添加GLM-5.3-Flash，从PoC模型中移除Kimi-K2.6和GLM-5.2-FP8**

提案 ID 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加到 PoC 模型集中。同时从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：其 PoC 参数和权重保持不变。所有其他链参数均保持不变。
...
投票将于 **2026 年 9 月 10 日 23:17 UTC** 结束。提案通过需满足：权重 25% 的出席率、多数赞成票，且反对票低于 33.4% 且无否决权。
...
如果您没有直接访问拥有投票权密钥的权限，或希望使用其他密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。
...
这仅是 devshard 相关变更：通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会停止，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并验证其 SHA-256 值是否与链上存储的值一致，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。提案通过本身不会转移任何流量，因此经纪人可在之后逐步迁移其网关。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅关于从冷密钥向热密钥授予治理投票权限的指南。
...
提案已获批准。将从社区金库拨出100,000 USDT，用于支持Gonka参与2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金涵盖黄金赞助套餐及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营及支持材料。
...
重启时，`v4.0.1`会从nonce 1恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond`会在60秒就绪超时后终止子进程，客户端会看到HTTP 502错误。
...
提案已获批准。将 `weight_scale_factor` 对 `deepseek-ai/DeepSeek-V4-Flash-0731` 的权重系数从0.214提升至0.246。其他所有模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为 `PROPOSAL_STATUS_REJECTED`，结果为 `failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终投票统计：赞成15452，弃权3093，反对0，否决0。
...
1. v5的主要目标是高度同步：每个主机和序列器在托管日志中维护一个已签名、可重放的主网高度视图，即使在无推理流量时也如此。借助该时钟，devshard现在可以明确知道cPoC何时运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议添加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前包含时间戳字段的消息中。后续版本可将 `confirmed_at` 和 `started_at` 等壁钟字段替换为高度标记。这解决了时间提前攻击问题，并为协议层面处理流超时铺平了道路。本版本包含这些标记，但尚未将超时和封印决策切换至其上。

3. 已添加通过所有插槽往返的心跳机制，因此安静会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同节奏。

**操作事项**

请审阅PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并在任何发现、问题、改进建议、边缘情况或潜在漏洞处留下评论。

有意义的评审贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一个升级周期获得社区奖励。

这仅是呼吁进行PR评审。不启动正式投票。

## 2026年8月25日

**提案97开放投票：将DeepSeek V4 Flash的weight_scale_factor提升至0.246**

一个新的链上治理提案（提案ID 97）已开放投票。

**功能说明**

将`weight_scale_factor`的`deepseek-ai/DeepSeek-V4-Flash-0731`从0.214提升至0.246。其他所有模型和链参数保持不变。

**投票**

投票将于**UTC时间8月27日03:47**结束。如有可能，请参与投票。
...
在周期初期，数据尚不充分，因此无效推断检查（SPRT）可能在样本足够大之前仅因少量无效项就触发。本提案使其更稳定：

- 将`invalidation_h_threshold`提升至40，并将`bad_participant_invalidation_rate`设为0.18。
- 在大样本情况下，阈值接近10%的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真实违规者仍会被触发（约360个样本时为18%，约1650个样本时为12%）。
- 小样本无法触发：至少需要32个被标记的无效项，且在500个样本时，观察到的无效率必须超过16%。
- 未使用的停机时间SPRT已关闭。周期结束时的统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于**UTC时间8月21日06:04**（约12小时后）结束。如有可能，请参与投票。如果未能在规定时间内达到加速阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。如果转换，您需要在常规提案中重新投票。
...
作为协议开发的重要里程碑，链现已准备逐步启用devshard结算，以提升网络的安全性和可靠性。

一个稳定的网关版本当前已在其中一个网关上运行，并将用于首个结算阶段。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每台主机执行的工作。在启用结算后，缺失和无效指标将用于链上。在当前主网阈值配置下，错误率低于10%不会触发对主机的任何处罚或削减，但仍应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。无显著问题的主机无需采取任何操作。

该仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需对此采取行动。

**下一步：****1) 接下来的24小时。** 主机应检查其统计数据并解决任何关键错误或配置错误。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口之后的第一个周期。** 将在当前网关上手动启动结算，首先从较小的模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后跟进，因为其数据在负载下目前可靠性较低。
...
该部署故意循序渐进，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的引导阶段已完成。自链周期360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 加入 MiniMax-M2.7 和 Kimi K2.6，成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后经 [提案98](#august-29-2026) 提高至 0.246）转换为共识权重。

DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每周期处罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版 MLNode（即仍在服务 Kimi 或 MiniMax 的节点，因为任何已部署 DeepSeek 的节点均已使用 3.0.16），请在方便时将这些节点迁移至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少量节点，确认其稳定后再迁移其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。
...
将正在运行的社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并为未来的实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预资格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 引导快照——在此前请保持委托分散，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始（区块 **5,552,115**）前的约 500 区块部署窗口内配置 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。
...
DeepSeek 已通过投票并进入引导阶段——如果您计划提供服务，请验证您的设置并立即提交意向。
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则您已达标。

如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免不参与罚金。

提醒：明天将上线另一个带有更好日志的MLNode构建版本，因此请留意后续简短通知。
...
- `deepseek-ai/DeepSeek-V4-Flash-0731` 变为可用的PoC模型。
- `MiniMaxAI/MiniMax-M2.7` 保持为基础模型。
- `moonshotai/Kimi-K2.6` 保持可用且不变。

**主机所需操作**

1. 声明意向。在提案94通过后、快照区块5,536,224（约2026年8月13日02:40 UTC / 8月12日19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。此窗口内切换是安全的，因为其间无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用以fp8 + nvfp4重新打包的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。为部署该模型，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；请将委托分散至独立的DeepSeek主机。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第360轮之前无需操作。从第360轮起，若您未提供DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的不参与罚金。

区块高度为精确值；时间仅为估算，将在接近轮次时确认。[更多关于DeepSeek启动的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。不修改任何现有参数或模型；唯一更改是新增该模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 要求MLNode运行vLLM 0.25.1并使用发布候选节点配置（`node-config-release-candidate-*.json`适用于B300 / B200 / H200 / H100）
...
包含系数的完整表格：[Google Sheet](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)
...
2. 资格预检快照：链对意向和委托进行快照，并发出建议性预资格事件。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留您当前的模型：在第360个周期前无需任何操作；从第360个周期起，如果您未提供该模型，则需为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
- API。
    ...
    - 默认 Node Manager gRPC 端口现为 9400，确保与自定义配置兼容。
- 部署。默认加入堆栈配置已更新为使用最新的 `v0.2.15-post2` API 镜像。
...
这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更内容：纪元追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整链软件升级。若获批准，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动执行主机操作。
...
该提案已获批准，并于主网区块 5316315 成功执行。该提案还批准了 devshard v4 运行时，其现在与现有的 v3 运行时并行运行。
...
**Devshard v4 和 v0.2.15 Mainnet 升级提案进入治理**

合并的**devshard v4 和 v0.2.15 mainnet 升级提案**现已上链并开放投票。

本提案包括两部分：

1. 新**devshard v4 运行时**的批准与注册。
2. 一个**v0.2.15 mainnet 软件升级**，移除大量过时代码、遗留的交易处理路径及相关组件，这些组件已不再符合当前架构的需求。

尽管这两项变更作为单一投票提交，但它们在操作上是独立的：

- **devshard v4 运行时**由 `versiond` 通过 devshard 版本管理机制分发和启动。
- **mainnet v0.2.15 二进制文件**通过标准的链上软件升级流程和 Cosmovisor 激活。

**Devshard v4 变更**

v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个为多实例高可用设计的版本：在共享的 Postgres 上，通过 versiond-router 后部署 N 个版本化的 /devshardd 副本，使用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可以执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。

v4 还包含错误修复和安全修复。

**Mainnet v0.2.15 变更**

升级的 mainnet 部分将移除大量过时代码，包括不再符合当前架构需求的遗留交易处理路径及相关组件。
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。通过移除过时行为并围绕当前支持的架构整合交易处理，也为未来的开发和升级提供了便利。


更多详情请参阅拉取请求：[https://github.com/gonka-ai/gonka/pull/1497 ](https://github.com/gonka-ai/gonka/pull/1497 )

**升级计划****Mainnet v0.2.15**
...
同一提案也在 `DevshardEscrowParams.approved_versions` 中注册 devshard v4。
...
devshard 部分正在为 v3 运行时做准备，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升内存利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和网桥。为确保主网上线升级期间以太坊网桥稳定，请提前按照[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护人员 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关如何安全清理 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用大量磁盘空间，可应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 仍为默认选项，并会自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放审查**

devshard 升级独立于主区块链进行。它们无需通过 Cosmovisor 协调完整节点升级，不影响主网行为，预计不会导致推理服务中断。若通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**主要变更**

- v4 的主要目标是提高 devshard 主机在主机故障和升级时的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
* 在区块 **5,105,276** 之前声明意向（2026年7月17日，约12:05 UTC）：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在纪元331的PoC开始前约500个区块的窗口期内（区块 **5,105,776** — 2026年7月17日12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
...
这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册到治理模型列表中，使其从纪元331开始进入标准引导流程。

同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。这消除了旧运行时容器长期存在的RAM增长问题 —— 这正是7月15日事件中网络节点OOM的根本原因。

**这标志着v3切换最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票 —— 紧急窗口期很短。
2. 提供Kimi服务的主机：在投票结束**后**、区块 **5,105,276**（2026年7月17日，约12:05 UTC）前声明对`moonshotai/Kimi-K2.6`的意向，然后在纪元331的PoC开始前约500个区块的窗口期内（区块 **5,105,776**，2026年7月17日约12:50 UTC）将您的MLNode切换回来：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
...
- 意向截止时间（纪元330）：区块 **5,105,276** — 2026年7月17日，约12:05 UTC（约05:05 PDT）。
- 纪元331的PoC开始：区块 **5,105,776** — 2026年7月17日12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6`在纪元328–329中失去了PoC验证多数支持（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的方式 —— 与6月提案78采用的恢复路径相同。

该提案将在下一次PoC前将`moonshotai/Kimi-K2.6`从PoC参数中移除，随后立即重新添加，Kimi将从纪元331开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票 —— 紧急窗口期很短。
2. 提供Kimi服务的主机：保持您的Kimi设置就绪，随时准备在引导PoC时切换回您的MLNode。
...
投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced` v0.2.13或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（紧急投票，0.667 赞成阈值；参与率重要，请尽快投票）。
* 引导意图截止日期和纪元时间将在引导说明中跟进。
...
Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者投票平局机制未触发：当前没有每模型投票权重的守护者被过滤出平局机制。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况类似于纪元 306–307 事件。纪元 329 开始时的投票确切细节仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个纪元）重新添加：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次通过标准引导流程：声明意图、部署窗口、PoC 存储提交。将在意图截止日期前发布说明。

**委托指导变更——请阅读**

先前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤回：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主机制和备用机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请将委托分配给运行模型的您自己的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在开发中的修复*** PoC 验证中的守护者投票（守护者始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链，请投票（公告将随后发布）。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量到 `/devshard/v3`。这将修复内存增长问题，并在移除 v1/v2 之前必须完成。
...
提案批准后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人切换流量至 v3。

此发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

白名单上的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们能够在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。

**关键变更**

- 已准备代理在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
- 改进了 RAM 利用率。
- 修复了网关运行时行为。
...
v3 运行时准备代理在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期运行时前缀，直到代理将流量切换到 v3。在此类升级期间，无需进行主网重启或手动主机操作。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/PR) 针对下一个链上软件升级 v0.2.14 已开放审查。

主网链/API 工作重点包括：PoC 重复工件保护、早期共享检测、经典推理 API 废弃（在主网上禁用 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收方路由，以及升级时的安全修复。

devshard 部分准备了 v3 运行时，以便代理在链升级期间无需依赖已弃用的经典 API 路径即可提供推理服务，同时改进 RAM 利用率并启用 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
将从该分支单独发布 devshard v3，并在主网链升级前提案并部署。提前将推理流量切换至 `/devshard/v3` 的代理可以在链升级期间继续提供推理服务。

**建议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查本提案。
2. devshard v3 发布版本将在主网链升级前被提案并部署。
3. 经纪人将推理流量切换至 `/devshard/v3`。
4. 如果链上提案获得批准，此 PR 将在链上升级执行后立即合并。
5. 请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能在下一次升级周期中获得社区奖励。

本次仅为邀请审查该 Pull Request，不会启动正式投票。治理投票流程将在审查期结束后开始。

**Devshard v3 治理投票**

Devshard v3 发布版本将在主网链升级前推出，以便经纪人能够在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作事项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人 —— 将推理流量切换至 `/devshard/v3`。一旦 devshard v3 发布版本上线，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 —— 准备好调整指标统计方式。详细说明将在 `devshard v3` 投票启动并接近完成之后发布。
...
根据历史数据，此前未发现此类攻击。但在当前周期中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 的使用行为。

针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过加强 PoC-v2 验证阻止攻击，使复制计算无法通过采样检查。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，则在第311轮开始前约500个区块的窗口期内将您的节点切换至 Kimi（PoC 开始约在6月28日 ~12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定为：在8xB300硬件上，Kimi 的共识权重约等于主模型 MiniMax。此设定并非偏向任何硬件——MiniMax 已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
此方案使 Kimi 非常适合8xB200和8xB300服务器。
...
常规引导流程从第309轮开始。由于投票在 PoC 前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多引导信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定为：在8xB300硬件上，GLM-5.2 的共识权重约等于 MiniMax。此设定并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
两种模型均通过标准引导流程进入，主机可在投入硬件前确认其可行性：

1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 服务的主机需在 `start_poc - deploy_window` 的引导快照前提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3 可达性）。这使操作员能在配置资源前确认模型是否具备可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在 PoC 开始时实际提交 PoC 存储提交的主机决定——而非仅由声明意向的主机决定。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮开始（PoC 开始约6月26日15:25 UTC；生效约16:00 UTC），前提是各模型满足引导资格条件：
...
提案必须在第308轮结束前完成，以便两个模型能顺利引导进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计支持本提案。

**主机需采取的行动**

1. **若要提供 Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 中将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，期间切换是安全的，因为此期间无 cPoC。

2. **要部署GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意向并 provision 您的MLNode。部署是可选的——未选择参与的主机不会受到任何惩罚。
...
如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
**提案78已通过：`MiniMaxAI/MiniMax-M2.7` 现为唯一的PoC模型；Kimi K2.6和Qwen3-235B已移除**
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换后才会获得本周期的cPoC奖励。
* 计划再次提供Kimi的主机应保留Kimi的部署环境，并准备好在PoC 309时将MLNode切换回Kimi——在第308周期将进行一次恢复Kimi的投票。

**接下来的内容**

在第308周期将进行一次加急投票，合并两项变更，均将于第309周期生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导程序。
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可选择参与，提前衡量模型需求，未选择提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前没有足够的投票支持以通过PoC验证。因此，正在提供Kimi的参与者已被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快恢复其运行且停机时间最短的方式。

一项加急提案现已上链，以便在下一个周期开始前生效。
链上提案在一次投票中执行以下操作：
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi没有足够的投票支持通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的路径。下一个周期将进行一次恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi的情况无关，仅因同样是PoC阵容变更而被一并包含在此处。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在第307个周期结束前完成，因此这些变更被合并为一次紧急投票。每个周期约22.9小时，标准的48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创世守护者应支持该提案。

**若获批准的效果**

从第308个周期开始（~UTC时间6月25日17:25），`MiniMaxAI/MiniMax-M2.7`将成为唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6将不再活跃。恢复Kimi将在下一个周期的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（若尚未就位），以避免在周期边界时遭遇Hugging Face速率限制。
2. **请在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi服务的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308周期将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 第308周期将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供该模型的主机不会受到任何惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
* 第308个纪元 shortly after：PoC开始约16:50 UTC，生效约17:25 UTC。
* 紧急提案需要0.667的通过阈值；参与率很重要，请及时投票。

## 2026年6月22日

**v0.2.13-devshard-v2运行时升级提案已通过治理**

devshard v2运行时已通过链上批准并添加到 `DevshardEscrowParams.approved_versions`。
...
提案获得批准后，v2现在可以与现有的v1 devshard运行时并行运行。新进程在 `/devshard/v2` 前缀下提供服务，而现有的v1流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。
...
- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长时间运行的会话将所有已服务的推理保留在RAM或状态中。
...
- 在纪元变更时修剪旧纪元存储，将SQLite/Postgres模式设置移出热点路径，并强制每个进程仅选择一个存储后端。

## 2026年6月15日

**v0.2.13-devshard-v2运行时升级提案已进入治理。**

本提案涵盖devshard v2发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对devshard的升级。它独立于完整链软件升级。如果获得批准，v2将与现有的v1 devshard运行时并行运行。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长时间运行的会话将所有已服务的推理保留在RAM或状态中
...
5. 在纪元变更时修剪旧纪元存储，将SQLite/Postgres模式设置移出热点路径，并为每个进程选择恰好一个存储后端
...
新进程在 `/devshard/v2` 前缀下提供服务。现有的v1 devshard流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。此类升级期间无需重启节点容器或手动执行主机步骤。
...
这是首次仅适用于devshard的升级，因此流程不同于标准链升级。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。

如果通过治理流程批准，新devshard版本将与现有的v1运行时并行运行。

请直接审阅PR，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。

有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格在下一轮升级周期中获得社区奖励。

本次仅为PR评审请求，不启动正式投票。治理投票流程将在评审期结束后开始。
...
MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续工作。尚未做出选择的主机建议尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。

## 2026年5月26日

**升级已执行：v0.2.13 现已在主网上线**

Upgrade Proposal v0.2.13（提案 ID 54）的链上治理投票已结束。
该提案已被批准，并于区块 `4267300` 成功在主网上执行。

## 2026年5月25日

**升级 v0.2.13：提前下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13 升级提案（提案 ID [54](https://github.com/gonka-ai/gonka/pull/1143)）已通过链上治理，升级现已安排。
...
针对 [Upgrade Proposal v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **通过**。

升级将在主网 **区块高度 4267300** 自动执行（≈ **2026年5月26日 星期二 14:42 UTC** / **07:42 PDT**）。

**提醒**

1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT 代币元数据以及 CW20 `wrapped_token` 代码 ID `105` 均通过升级处理程序注册，因此桥接将在升级高度于主网激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在预下载约 230 GB 的 FP8 权重。在启动窗口期间，Hugging Face 的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会对其完整权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考基准在 vLLM-0.20.1 之后的吞吐量。此调整**仅适用于您共识权重中由 Kimi 导出的部分**；您的 Qwen 导出权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新提交，现已更新若干内容。
...
- 治理：将创世守护者投票权降至约 25%，并设置链上法定人数为 0.25。若守护者弃权，非守护者需在剩余 75% 中实现约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 更新推理验证阈值：
    ...
    - Kimi K2.6：`0.900`
    - MiniMax-M2.7：`0.922`
- 在 vLLM 0.20.1 发布后，根据 Qwen-on-B200 参考基准重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`（不变）
...
- 将 devshard 随机数限制从 `20_000` 提高至 `1_000_000`。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将启用 Gonka 主网与 Ethereum 之间的跨链转账（包括在 Ethereum 上包装 GNK 以及桥接 USDC/USDT）。包装/解包脚本和操作员工作流程将另行文档化。
...
**在第 278 个周期前对 `MiniMaxAI/MiniMax-M2.7` 的参与选择（届时开始处罚）**
...
请主机更新仪表盘/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先遵循加入网络指南。此仪表盘更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 投票结束：2026 年 5 月 22 日 22:12:25 UTC
- 提议的升级高度：4267300
- 预计升级时间：2026 年 5 月 26 日 14:42:02 UTC
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 区块时（约 43 分钟前）→ 第一个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。

**注意**

- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，请提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用大量磁盘空间，可以应用 cosmovisor 备份 [指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 中描述的清理技术。
- 该提案将有意跳过从升级高度到升级周期结束（10000 区块宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个周期）。
- 如果提案失败（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上无任何变更，升级将不会发生。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何操作。
...
在第267轮中，PoC验证已成功完成，受影响的主机已能够返回活跃集合。

第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击与若干相关条件共同导致许多参与者无法进入第266轮。
...
该问题已追溯至具有非标准语义的推理请求。该攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
随着未来轮次中正常的PoC参与持续进行，权重可能需要数天时间才能稳定。

**主机所需操作**

1. 尽可能保持您的API节点在线并可访问。这有助于保留对主机参与的可见性，并支持后续审查。
2. 监控下一阶段的PoC参与情况。确保您的节点按预期进入PoC，且活跃权重正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方恢复，预计会通过社区代理和经纪人端点逐步恢复。
5. 在社区渠道或本线程中分享相关日志或观察结果。如果您的主机在第266轮受到影响，或在后续轮次中表现异常，这一点尤为重要。
...
- 在本次升级纪元剩余时间内禁用确认 PoC，以便新快照逻辑从下一个纪元开始干净启动。
- 当参与者再次激活时，重置 `ConsecutiveInvalidInferences`。
- 为在 v0.2.12 之前加入的 DAPIs 补全缺失的 `MsgRespondDealerComplaints` 授权授予。
- 修复桥接和流动性池合约权限检查的 Wasm keeper 访问问题。
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不参与投票的情况下，这使剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
**在第 278 纪元前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。如果是，请先确认您正在运行当前版本，再执行任何进一步操作。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊端合约激活，所有主机均需验证其桥接容器已部署、运行最新版本并同步正确。

部分主机可能已部署桥接容器。在这种情况下，请先检查您是否正在运行当前版本，然后再采取进一步操作。
...
- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被惩罚。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，并在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断可能立即再次使其失效。计数器现在在重新激活和即将晋升时重置。
- 在 v0.2.12 之前加入的 DAPIs 其冷到暖的授权授予中未包含 `MsgRespondDealerComplaints`。升级将回填此权限，以便它们能响应交易商投诉。
...
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束，确认 PoC 触发器将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用 v0.2.10 的宽限期原语。
...
- 内部 devshard 存储迁移不再阻塞 dapi 启动。在迁移和恢复完成前，devshard 路由仍不可用。

**devshard**

- 由于旧的托管数据保留在一个 SQLite 存储中，devshard 存储可能无限增长。现在存储按纪元范围管理，并在后台清理旧纪元，仅保留最近的 3 个纪元。
- Devshard 现在支持更大规模部署的共享存储选项。它可以使用 Postgres 作为主存储，SQLite 仍作为本地备用存储。
- Postgres 数据按 `epoch_id` 对会话、差异和签名进行分区，以便清理可干净删除旧纪元数据。
- 状态快照减少了长时间运行会话的恢复工作量。
- 负载查找固定于托管纪元，并为纪元边界和遗留纪元 0 请求提供回退机制。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口的服务器可能延迟启动。这延迟了 api 激活，导致部分矿工跳过确认 PoC。
...
docker start api
```
部署后，请再次确认 9100 和 9200 端口的服务器正在运行：
```
...
我们建议替换api容器的二进制文件。除修复外，新版本还启用了对devshard数据库的修剪功能，并为devshard状态添加了Postgres支持。
...
在Kimi-K2.6引导过程中，主机观察到30%的最低直接参与阈值在实践中难以满足。为避免Kimi-K2.6在未来轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。

安全模型保持不变：PoC验证本身未改变，仍需超级多数验证算力才能接受结果。

该提案被加速处理，以便在下一轮PoC之前生效。投票将持续12小时。
...
对于多模型PoC，这意味着Kimi现在作为独立模型组拥有自己的参与和奖励跟踪。
...
预评估与PoC开始之间有500个区块的窗口期。在此期间，没有CPoC任务，因此已声明意图的主机可安全地将模型节点切换至 `Kimi K2.6`。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：该模型组将在升级后两个轮次激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
有关这些更改的更多详细信息请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
每个 DAPI 在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送带有清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。剩余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
- **协议加固**。保留的节点（`POC_SLOT=true` 将被随机采样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
请从 `gonka/deploy/join` 目录运行以下命令，以更新仪表板/资源管理器：
```
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml pull explorer
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/948) 已开放，供审查下一个链上软件升级 v0.2.12。

请直接审查 PR 代码，并对发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 经销商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
这些性能改进可根据工作负载和网络条件，使每个区块的推理数量最多提升 100 倍。
...
下一代链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获得批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 的性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过每个推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行与验证移至分配的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中，某些问题仅靠本地测试难以发现。当前实现已设计为不会对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能同时出现大规模剪枝活动，造成不必要的运营压力。
...
下一次链上软件升级 v0.2.11 的[拉取请求](https://github.com/gonka-ai/gonka/pull/813)已开放供审查。欢迎提供反馈和改进建议。

对于对本 PR 审查有实质性贡献的，可能在下一次升级中提出奖励。
...
目前，通过每笔推理链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅处理会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能同时开始大量剪枝活动，造成不必要的运营压力。
...
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
审查者可在此处找到完整的升级提案、迁移详情、测试摘要和建议流程：[链接](https://github.com/gonka-ai/gonka/pull/813)。
...
对于已使用代理层的团队，这可能是简化架构、转而依赖原生行为的好机会。实际上，这应能带来更清晰的集成模式和更易维护的系统。

## 2026年3月6日

**注意：v0.2.11 升级预计将于下周初进入审查和治理投票阶段。**

请密切关注并计划参与。投票是支持网络发展、确保升级与参与者实际需求保持一致的最简单方式之一。
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求授权您代为投票。没有该授权，其他账户无法提交投票。
...
API二进制文件的新版本已发布。该版本更新了连接超时处理，并在PoC验证流水线中引入了额外的检查。

1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格5分钟超时，而某些请求可能需要更长时间。新版本的API将恢复该值，而非强制执行严格限制。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 开始时中断正在进行的推理请求，以降低因 PoC 启动时仍处于活跃状态的请求导致潜在权重下降的风险。
...
- 此构建旨在与前一版本向后兼容。
- 它已对少量节点启用，但仍建议在部署前审查变更。

## 2026年2月19日

**抵押参数更新提案——投票结果**

抵押参数更新提案未能达到法定人数，因此根据当前治理规则被否决。这意味着更新的参数将不会激活。

如前所述，第180轮次的抵押激活与此投票无关。

由于提案未通过，创世时定义的抵押参数将在第180轮次自动生效。

参与者应：

- 审查创世时定义的抵押参数。
- 在第180轮次前准备并存入所需的 GNK。
- 确保 [抵押](https://gonka.ai/host/collateral/) 配置正确，否则从第180轮次开始，PoC 奖励将减少 5 倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平相对较低时，存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为将 PoC 权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选择。如实施所示，有效的 PoC 参考窗口比先前名义假设长约5个区块。
...
此前，PoC 权重计算依赖于名义轮次持续时间假设。引入实时归一化后：

- PoC 持续时间与实际区块生产时间对齐
- 权重更准确地反映实际计算时间

由于有效归一化窗口比之前的名义模型长约5个区块，每个轮次重新计算的权重成比例降低。

**观察到的 GPU 权重变化（第175轮次 → 第176轮次）**
...
!!! note "兼容性说明"
    ...
    **启用 Qwen235B 工具**
    ...
    此次升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 添加了工具调用参数（ `--enable-auto-tool-choice` 、 `--tool-call-parser hermes` ），并将验证阈值设置为 0.958。
...
**宽限期：** 升级后引入了3000个区块的宽限期，在此期间无确认 PoC，并对升级时段的错过率和无效率阈值放宽要求。
...
这意味着在单个周期内，即使被惩罚，矿工最多损失其抵押物的0.5%。所需抵押物仅占每日奖励的约24%。

**警告：** 抵押参数将无论投票结果如何均生效。若本提案未通过，创世时定义的抵押参数将在第180个周期自动激活，而非上述参数。

投票结束后、第180个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180个周期起，其奖励将减少5倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获得批准，该提案将引入一项重要的 PoC 验证优化（默认关闭），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且对升级所在纪元的漏报率和无效率阈值放宽。
...
正如AMA期间讨论并基于社区成员提出的论点，本提案建议从较低的抵押要求和最小的惩罚开始。
...
这意味着即使在一个纪元内被惩罚，矿工的抵押损失也不会超过0.5%。所需抵押仅占日收益的约24%。

提案提交投票后将另行公告。

警告：无论提案投票结果如何，抵押品都将生效。如果本提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到验证，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请查阅GitHub上的v0.2.10升级PR并提供您的反馈。有意义的审查贡献可能在下一次升级中获得奖励。
...
在当前周期中观察到cPoC（确认PoC）与PoC之间的重叠。在该周期的最后一个区块之前，`is_confirmation_poc_active`被观察为`true`。

此重叠的影响正在评估中。初步观察表明，没有节点记录了PoC提交，导致该周期累积权重为零。

作为预防措施，矿工通过协调行动暂时停止了区块生产。

问题正在定位中。

请保持在线，以便在补丁发布时能立即应用。更多细节和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已开放**

链上推理访问目前开放，不再仅限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 已开放，供审查下一个链上软件升级 v0.2.10。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约2天。

对于对此次 PR 审查作出有意义贡献的，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审查呼吁，而非正式投票的开始。治理投票流程将在审查期结束后开始。
...
**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际 PoC 时间归一化 PoC 权重**

此升级通过实际 PoC 持续时间归一化参与者权重，以减少区块时间漂移效应，并确保权重结果与实际执行时长一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式转账
...
现有主机无需升级其 `api` 和 `node` 容器。更新的容器版本旨在供在链上升级完成后加入的新主机使用。

**提议流程**

1. 活跃主机请在 GitHub 上审查本提案并留下反馈。
2. 在社区审查该PR后，预计从该分支创建v0.2.10版本，并可提交此版本的链上升级提案，以启动正式的治理投票流程。
3. 如果链上提案通过，预计在链上升级执行后合并此PR。

从[upgrade-v0.2.10](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10)分支创建发布版本（而非`main`）可最小化`main`分支上`/deploy/join/`目录中容器版本与链上二进制版本不匹配的时间，从而确保新主机获得更顺畅的接入体验。

**测试与迁移**

v0.2.10的测试指南和迁移详情请参阅[此处](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10)。请仔细审阅。
...
- 观察到的变化表明，GPU权重差异现在反映的是模型特定的推理吞吐量，而非名义上的硬件规格。例如，H100 PCIe权重的下降幅度大于H100 HBM3权重，这与对 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的观察推理行为一致。
- 在当前模型配置下，基于观察到的推理轨迹，B200 GPU的推理性能并未高于H100类GPU。
- 如果未来通过治理引入更大或更苛刻的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准的vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与PoC v2中观察到的GPU类型间相对性能差异一致的结果。
...
现已提供补丁以解决导致PoC周期中区块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为并安全恢复区块生成。
...
根据当前评估，用于处理此情况的机制可能无法按预期运行。为防止在不确定或不安全的条件下完成验证器最终确认，网络在验证器选择之前已停止运行。
...
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将来临纪元的参与者集合中。
...
下一个链上软件版本v0.2.9的升级提案现已上链并开放投票。若获得批准，该提案将启用PoC v2进行权重分配，并通过链上治理完成对传统PoC机制的过渡。
...
- 运行其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换至`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型

**资格标准**

ML节点要符合PoC v2权重分配资格，必须同时满足以下两个条件：

- 节点仅提供`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型
- 节点运行PoC v2兼容镜像：
    - ghcr.io/product-science/mlnode:3.0.12-post1
    - ghcr.io/product-science/mlnode:3.0.12-post1-blackwell
...
在因cPoC处罚而减少或排除奖励的情况下，未计入部分将转入社区池。此前，此类奖励会重新分配给其他参与者。
...
- 仅配置为提供受支持的`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型
- 镜像已更新为PoC v2兼容版本
...
下表列出了上一周期具有权重、在当前周期提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)

其总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或极低的总权重达到约 48%，这一比例过高。
...
升级提案 v0.2.8 的链上治理投票已结束。该提案已获批准并成功在主网执行。
此次升级实现了 PoC v2 架构，简化了模型支持，并应用了关键的安全与可靠性修复。
...
- 认证绕过：将 `epochId` 绑定到签名，以验证是否属于正确的周期。
...
!!! note 
    未同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去 PoC v2 参与资格。PoC v2 的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明了主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来更新其ML节点。从第155个周期开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
建议预先下载模型权重。为避免启动延迟，请将权重预下载至`HF_HOME`：
```
mkdir -p $HF_HOME
...
PoC v2 仅在链上治理提案获得批准后才成为权重分配的活跃方法。
    在此提案获得批准之前：
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估是否具备向 PoC v2 过渡的条件。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不被视为有资格参与 PoC v2。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个周期内逐步进行更新。
...
- 通过预定义的硬件配置模式，从公开可观察的链上数据推导而来。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
`allowlist` 具有固定的过期时间，不会在延长的稳定窗口之后继续生效。一旦 `allowlist` 在区块 2443558 过期：
...
`Allowlist`过期：在区块2,222,222时自动过期。
...
该提案已获得批准。此提案定义了一个临时 `allowlist`，反映在最近多个周期中行为保持一致的参与者。
...
2) 之前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块2,222,222结束。

**执行特性**

- `allowlist` 从下一个周期（第135周期）开始生效
- 激活发生在第135周期的第一次PoC期间
- 无需软件升级
- 从那时起，`allowlist` 一直有效，直至并包括区块2,222,222
...
此投票定义了稳定期内参与者的 `allowlist` ([https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv))，反映那些行为始终符合网络预期的参与者集合。

**投票范围**

如果获得批准，网络将临时使用一个 `allowlist`，包含在之前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：
...
之前持续偏离这些模式的参与者在稳定窗口于区块2222222结束前不属于 `allowlist`。

**可重复性与方法论**

`allowlist` 使用预定义的硬件配置模式从公开可观察的链上数据中推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
- 无需软件升级。
- 在成功投票后的下一个PoC中，`allowlist`将生效，预计在区块：2089140。
...
- `Allowlist`生效：在下一个PoC执行于区块2089140后。
...
为恢复可靠的共识进展，至少需要67%的活跃网络算力安装该补丁。
...
针对近期共识问题的补丁已就绪，详细说明将很快发布。
每位活跃主机的参与对网络推进和恢复正常运行至关重要。请保持在线，并在说明发布后准备应用更新。
...
链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强的提案已获批准，并已在主网上成功部署。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——此前曾使用过的、限时的防御机制，现提议重新激活。
...
此升级正式固化了此前通过手动API更新分发并已在网络中使用的关键修复。这些修复包括：
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单将立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。

**第117轮奖励分配**

本提案涵盖与链暂停（第117轮）相关的两项奖励分配：

- 在第117轮期间活跃但未收到该轮奖励的节点，将补发该轮的缺失奖励。
- 所有在第117轮期间活跃的节点，将额外获得等于第117轮奖励1.083倍的 payouts，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是故意试图破坏或施压系统所致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的韧性。

创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
此升级将此前通过手动API更新分发且已在网络上使用的关键修复正式化。这些修复包括：
...
- 主机级别注册：在区块2,222,222（约两周后）之前暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续至区块2,294,222（约19天）。

**治理控制机制**

本升级包含的准备工作为未来通过治理控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外奖励，金额为第117轮奖励的1.083倍，统一应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
Cosmovisor在执行更新时会在 `.inference/data` 状态文件夹中创建完整备份，请确保您的磁盘有足够的空间。阅读[此处](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)了解如何安全删除 `.inference` 目录中的旧备份。
如果您的 `application.db` 占用大量空间，可使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)的方法进行清理。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由一个bug引起的：未标记为已完成的不支持格式的推理请求，尽管请求本身已被处理。以下更新将解决此行为。
