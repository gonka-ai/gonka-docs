...
!!! note "关于此页面"
    ...
    此页面不保证完整。如需最新信息，包括治理投票的启动及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。
...
无人投反对票。参与率约为网络权重的 6%，低于 25% 的法定人数门槛，因此该提案因参与不足而失败。
...
自第 394 个纪元起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供服务、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的 15%，并持续适用于每个纪元，直到您做出选择。
...
提案 ID 104 做了两件事：将 `v5` 添加到已批准的 devshard 版本列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖励。

**Devshard v5**

这是一项仅限 devshard 的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会停止，推理服务继续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。通过该提案本身并不会转移任何流量，因此经纪人可在之后逐步迁移其网关。
...
该提案还发放 91,300 USDT 的贡献者奖励，分配给七位受赠者：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票（含否决）少于33.4%。
...
提案已获批准。从社区金库中拨款15,500 USDT，用于补偿Gonka协议的联合创建者Daniil和David Liberman参加All-In Summit 2026的两张参会通行证费用。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人7,500 USDT，门票已先行支付。本次补偿涵盖两张门票及提现交易费用，未申请补偿机票、住宿及其他费用。
...
提案已获批准。Quant Mesh Limited被正式批准为Gonka与Ledger集成的法定合同方，并从社区金库拨款350,350 USDT至 `gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费275,000 USDT、首年年度激活费75,000 USDT以及350 USDT处理费。授权与资金拨付严格限于完整提案中描述的Ledger集成。
...
提案已获批准。`zai-org/GLM-5.3-Flash` 现已成为治理批准的模型并纳入PoC模型集，而 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8` 已从 `poc_params.models` 中移除。这两个ID仍保留在 `GET /v1/governance/models`（推理目录）中，但不再作为PoC模型。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起，`penalty_start_epoch` 生效，因此未明确选择的主机将面临不参与惩罚。如果您计划提供该模型，尽早声明意向将使团队有最佳机会形成。主机指南：[GLM-5.3-Flash 启动](./host/glm-bootstrap.md)。

最终投票结果：赞成 309308，反对 0，反对并否决 0，弃权 3168。
...
该提案已获批准，`v4.1` 已添加至批准的 devshard 版本列表中。`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，并启动运行时。无需手动操作。

v4.1 将与现有的 v3 和 v4 运行时并行运行，因此当前流量将继续被服务，经纪人可以逐步迁移其网关。
...
最终投票结果：赞成 312476，反对 0，反对并否决 0，弃权 0。
...
提案 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加到 PoC 模型集中。同时从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：它们的 PoC 参数和权重保持不变。所有其他链上参数均保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，现联合提出。发布输入、测量数据及阈值设定的推理依据见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
投票将于 **2026年9月10日23:17 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票且具有否决权的比例低于33.4%。
...
这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：Cosmovisor 不参与，链不会停止，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。通过该提案本身不会迁移任何流量，因此经纪人可以在之后逐步迁移其网关。
...
投票将于 **2026年9月10日 22:36 UTC** 结束。提案通过需满足：权重达到 25% 的最低参与率、多数赞成票，且反对并行使否决权的票数少于 33.4%。
...
该提案已获批准。它从社区金库中拨款 100,000 美元，用于支持 Gonka 参与 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金涵盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
重启时，`v4.0.1` 从非零 1 开始恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 随后在 60 秒就绪超时后终止子进程，客户端因此看到 HTTP 502 错误。
...
该提案已获批准。它将 `weight_scale_factor` 对 `deepseek-ai/DeepSeek-V4-Flash-0731` 的权重系数从 0.214 提高至 0.246。所有其他模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，结果为 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的最低参与率）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 系数源于被高估的 nonce/min 数值。测量修正见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需执行任何操作。

感谢所有参与投票的人。

## 2026年8月25日

**仅适用于 devshard 的升级 PR 现已开放审查**
...
1. v5 的主要目标是高度同步：每个主机和序列器在托管日志中维护一个已签名、可重放的主网高度视图，即使在没有推理流量时也是如此。通过这个时钟，devshard 现在可以证明地知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议添加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前携带时间戳字段的消息中。后续版本可以将诸如 `confirmed_at` 和 `started_at` 等墙钟字段替换为高度标记。这解决了时间提前攻击问题，并为在协议层面处理流超时打开了道路。本版本包含这些标记，但尚未将超时和密封决策切换到它们。

3. 通过所有插槽的往返心跳已添加，因此安静会话仍能定期同步高度，而繁忙会话则可通过普通推理标记实现相同的周期。
...
投票将于 **2026年8月27日 03:47 UTC** 结束。如有可能，请参与投票。
...
该提案已获批准。它调整了在小样本情况下无效推理检查的行为，以避免在周期早期数据不足时误触发。主机无需采取任何操作。
...
在周期早期，数据尚不足，因此无效推理检查（SPRT）可能在样本足够大之前仅因少量无效数据而触发。本提案使其稳定：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本情况下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真正的违规者仍会触发它（约 360 个样本时为 18%，约 1650 个样本时为 12%）。
- 小样本无法触发它：至少需要 32 个被标记的无效数据，且在 500 个样本时观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期结束统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于 **2026年8月21日 06:04 UTC**（约12小时后）结束。如能投票，请务必参与。若未能在时限内达到加速阈值，该提案不会被拒绝：它将转为普通提案，投票将继续进行更长的常规周期。若转换成功，您需要在普通提案中重新投出您的票。
...
一个稳定的网关版本目前已在其中一个网关上运行，并将用于第一阶段结算。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关视角显示每个主机执行的工作。Misses 和 Invalid 指标将在结算启用后上链使用。根据当前主网阈值配置，错误率低于10%的主机不会触发任何惩罚或 slashing，但仍应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。没有显著问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需操作。

**下一步：****1) 未来24小时。** 主机应检查其统计数据并解决任何关键错误或配置问题。贡献者可协助调查问题（如需）。

**2) 此24小时窗口后的第一个纪元。** 将在当前网关上手动启动结算，首先针对较小模型。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 接下来的1-3天。** 将监控结算结果

**4) 一旦本阶段稳定。** 其他经纪人可部署稳定版网关，并以受控模式开始结算。
...
在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同作为活跃模型组加入，DeepSeek 组中获得的 PoC 权重现已按校准系数 0.214（后由 [提案98](#august-29-2026) 提升至 0.246）转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需再做任何操作——现有设置将继续生效。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点使用旧版 MLNode（正在服务 Kimi 或 MiniMax 的节点，因为所有已部署 DeepSeek 的节点均已使用 3.0.16），请在方便时将这些节点升级至 3.0.16。
...
默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。若您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是相同的 MLNode 3.0.16，仅针对 CUDA 12.9 构建。两个标签均在同一 compose 文件中。
...
将活跃的社区销售合约及所有包装代币实例迁移至新存储的 CosmWasm 代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中识别出的理论风险，预计不会影响正常运行。
创世守护者预计支持该提案。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查为 epoch 360 启动快照——在此前请保持委托的多样性，以确保快照有效。
...
从该分支获取 MLNode 堆栈——默认镜像已在 [compose 文件](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 中固定。
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行对比。如果通过，您就没问题了。

如果您想提供DeepSeek服务，请使用上述步骤进行设置。如果不，则不要忘记委托，以免遭受不参与惩罚。

提醒：明天将发布另一个带有更好日志的MLNode构建版本，因此请期待后续简短通知。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500个区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口期内切换是安全的，因为期间没有cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用经过重新打包的fp8 + nvfp4模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。为部署此模型，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第360轮之前无需任何操作。从第360轮起，若您未提供DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的不参与惩罚。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。未修改任何现有参数或模型；唯一更改是新增模型条目。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第360个周期前无需操作；从第360个周期起，若您未提供该模型，则需为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 请在截止日期前对提案94进行投票。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
针对 `v0.2.15-devshard-v4.0.1` 运行时升级提案（提案ID 93）的链上治理投票已结束。
...
这是一个非关键性、仅限 devshard 的升级，独立于完整链软件升级运行。它将现有的 devshard v4 运行时替换为 v4.0.1。

这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更内容：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。
...
针对升级提案 v0.2.15（提案ID 92）的链上治理投票已结束。

该提案已获批准，并于主网第5316315区块成功执行升级。同一提案还批准了 devshard v4 运行时，该运行时现已与现有的 v3 运行时并行运行。
...
v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列性 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个为多实例高可用设计的版本：在共享的 Postgres 后端，通过 versiond-router 部署 N 个版本化的 /devshardd 副本，采用粘性会话路由和验证租约独占机制。网关仅通过 gRPC 与链交互。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可以执行蓝绿切换并排空流量，确保正在进行的工作（包括 SSE）在旧版本上完成。

v4 还包含了错误修复和安全补丁。

**主网 v0.2.15 变更**

主网升级部分移除了大量过时代码，包括已不再符合当前架构需求的遗留交易处理路径及相关组件。
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。同时，通过移除过时行为并围绕当前支持的架构整合交易处理流程，使未来的开发和升级更加便捷。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
devshard 部分准备了 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前按照 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您拥有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
...
devshard 升级独立于主区块链进行。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，且不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，使正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还修复了若干漏洞和安全问题。

**操作项**

请审查 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并对任何发现、问题、改进建议、边缘情况或潜在漏洞发表评论。

有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能有资格获得下一升级周期的社区奖励。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦获批，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。此举消除了旧运行时容器长期存在的内存增长问题——即7月15日事件中网络节点OOM的根本原因。
...
2. 提供Kimi服务的主机：在投票结束后立即、并在区块 **5,105,276**（2026年7月17日，约12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前约500个区块的窗口期内（区块 **5,105,776**，2026年7月17日约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**请勿委托给守护者节点**；将委托分散至独立的Kimi主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取最新的委托指导。
...
`moonshotai/Kimi-K2.6`在第328–329轮失去了PoC验证多数支持（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是恢复其服务且最小化停机时间的最快方式——与6月提案78所用的恢复路径相同。
...
2. 提供Kimi服务的主机：请保持Kimi配置就绪，随时准备在引导PoC时切换您的MLNode。
...
**Kimi-K2.6 事件（纪元328–329）：发生了什么、恢复计划以及委托指导的变更**

在纪元328中，`moonshotai/Kimi-K2.6`失去了其PoC验证的多数支持。在纪元329开始时，运行Kimi的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行Kimi的服务器同时失败：

* 其中一台服务器上，运行Kimi的MLNode因提供商端问题而崩溃。大量Kimi委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器因内存不足而丢失了其网络节点。

Kimi的验证投票一直接近2/3阈值，因此这次故障足以使该组低于多数。守护者决胜机制未触发：当前没有每模型投票权重的守护者会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的v0.2.14升级中修复。

这种情况类似于纪元306–307事件。纪元329开始时的投票详细信息仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复Kimi最安全的方式是通过全新引导（约1.5个纪元）重新添加：

1. [今天] 一项紧急提案将在下一次PoC之前移除Kimi，并在之后立即重新添加。随后将发布包含提案ID、投票命令和截止日期的单独公告。
2. [明天] Kimi将再次通过标准引导流程：声明意图、部署窗口、PoC存储提交。在意图截止日期前将发布说明。

**委托指南变更——请阅读**

之前的指导（包括2026年6月27日的引导公告）建议将守护者节点作为委托目标。此建议现被撤回：

* **请勿委托给守护者节点。** 守护者是PoC验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在流程中的修复*** 守护者参与PoC验证（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce重复——v0.2.14（当前由早期API更新处理）。
* 网络节点的RAM增长——已在devshard v3中修复；需要从v1/v2迁移。完全迁移后，v1/v2将被移除且`/v1/devshard`关闭。为`api`和`versiond`容器设置独立的内存限制。

**主机所需操作**

1. 一旦紧急Kimi提案上链（后续公告），请立即投票。
2. 如果您计划提供Kimi：关注引导说明，并准备好声明意图。
3. 如果您为Kimi委托：选择非守护者委托方；尽可能将权重分散到多个独立主机上。
4. 经纪人：现在立即将devshard流量迁移到`/devshard/v3`。这将修复RAM增长问题，且在移除v1/v2之前必须完成。
...
该提案涵盖[devshard v3发布。](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.14/proposals/governance-artifacts/update-v0.2.13-devshard-v3)

这是一个仅限devshard的运行时升级。它独立于全链软件升级运行，无需链二进制升级。

提案通过后，v3现在与现有devshard运行时并行运行。新进程在`/devshard/v3`前缀下提供服务，而现有devshard流量可继续在早期运行时前缀上运行，直到经纪人切换流量至v3。

此发布将`devshardd`二进制文件作为Gonka发布工件发布。`versiond`会自动下载该二进制文件，验证sha256哈希，并在现有`versiond`容器内启动额外的`devshardd`进程。

此类仅限 devshard 的运行时升级无需主网重启或手动主机操作。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这将使他们在链升级运行期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，且不依赖已弃用的经典 API 路径。同时改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期的运行时前缀，直到代理将流量切换到 v3。在此类升级期间，无需进行主网重启或手动主机操作。
...
节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支发布一个独立的 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级期间继续提供服务。
...
本次仅为邀请审查该 Pull Request，不启动正式投票。治理投票流程将在审查期结束后开始。
...
2. 现在 / 主网升级前 —— 更新您的 API 和桥接器。为保持以太坊桥接器在主网升级期间的稳定性，请提前根据指南将 `api` 二进制文件和桥接镜像更新至 0.2.14。如果您已更新 `api` 二进制文件，则只需更新桥接镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人 —— 将推理流量切换至 `/devshard/v3`。一旦 devshard v3 发布版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 —— 准备调整指标统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近结束时发布。

## 2026年7月6日

**安全更新：PoC-v2 权重验证强化 —— 更新您的 API 容器**

上周，HackerOne 上报告了 PoC-v2 权重路径中的一个漏洞。参与者可以夸大其计算权重，从而影响区块奖励、共识投票权和治理投票权。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供Kimi服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。一个守护节点也将运行Kimi，因此不愿运行它的主机可以选择委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换为Kimi（PoC开始约在6月28日 ~12:47 UTC）。
...
> **更新（2026年7月15日）：此建议已撤回 —— 请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指引。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数的选择使得在8xB300硬件上，Kimi的共识权重与主模型MiniMax大致相当。此举并非为了偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上，GLM-5.2的共识权重与MiniMax大致相当。此举并非为了偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2服务的主机需在 `start_poc - deploy_window` 的引导快照前为该模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最少成员数 `V_min`、以及>2/3可达性）。这使操作者能够在配置资源前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由在PoC开始时实际提交PoC存储提交的主机决定——而非仅由声明意向的主机决定。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮开始（PoC开始约6月26日15:25 UTC；生效约16:00 UTC），前提是每个模型均满足引导资格：
...
该提案必须在第308轮结束前完成，以便两个模型都能在第309轮中引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计将支持该提案。

**主机所需操作**

1. **若要提供Kimi：** 在投票**结束后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换为该模型。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。
...
如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`查看：任何活跃节点均可使用：
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍使用Qwen或Kimi的主机，将不会在本纪元获得cPoC，直到切换完毕。
* 计划再次提供Kimi的主机应保留Kimi的部署环境，并准备好在PoC 309时将MLNode切换回Kimi——在第308纪元将进行一次恢复Kimi的投票。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可自愿选择加入，提前衡量模型需求，未选择提供该模型的主机不会受到惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以实现快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前投票不足，无法通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi投票不足，无法通过PoC验证，将其移除并重新引导是最快恢复路径，停机时间最短。下一个纪元将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi情况无关，仅因同属PoC阵容变更而一并包含在此处。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如尚未预加载），以避免在epoch边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划未来再次提供Kimi的主机，现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——epoch 308将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— epoch 308中将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不参与的主机不会受到任何惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
* 第308个纪元 shortly 后形成：PoC开始约16:50 UTC，生效约17:25 UTC。
* 紧急提案要求0.667的通过阈值；参与率很重要，请尽快投票。
...
- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中。
...
这是首次仅针对devshard的升级。它独立于全链软件升级。如果获批，v2将与现有的v1 devshard运行时并行运行。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。如果链上提案获批，`versiond`将自动下载二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。
...
这是首次仅适用于devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。
...
本次仅为PR评审号召，不启动正式投票。治理投票流程将在评审期结束后开始。
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需做任何额外操作——现有设置将继续有效。尚未做出选择的主机建议立即选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需对其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会对其完整权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅适用于您共识权重中 Kimi 相关部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。如果守护者弃权，非守护者需在剩余的 75% 中实现约三分之一的参与率才能满足法定人数要求（参见 inference-chain 部分）。
- 必要准备：桥接容器检查、MiniMax 决策、仪表板更新、投出投票。
- 除非提案获得批准，否则链上无任何变化。
...
- 将每个纪元的最大 devshards 数量从 `100` 提高至 `500_000`。
...
- 激活后，桥接将启用 Gonka 主网与 Ethereum 之间的跨链转移（包括在 Ethereum 上封装 GNK 以及桥接 USDC/USDT）。封装/解封装脚本和操作员工作流程将另行文档化。
...
若获批准，二进制版本将通过链上升级提案进行更新。有关升级流程的更多信息，请参阅 [/docs/upgrades.md.](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.13/docs/upgrades.md)
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再执行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块处（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
在相关保护措施实施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪人端点。
...
2. 监控下几轮的PoC参与情况。确保您的节点如预期般进入PoC，并且活跃权重被正确反映。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，这使剩余 75% 投票权中的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
- 修复长时间启动行为和 devshard 无效化流程的边界情况。
...
请所有主机验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。若如此，请先确认您运行的是当前版本，再执行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
如果您无法直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
- 如果 `application.db` 占用了大量磁盘空间，可以应用 cosmovisor 备份[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 中描述的清理技术。
- 该提案将有意跳过从升级高度到升级周期结束（3000区块宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个周期）。

## 2026年5月7日

**桥接容器更新/验证要求**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊侧合约激活，所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。
...
有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一轮升级周期中符合社区赏金资格。

本次仅为拉取请求审查请求，不启动正式投票。治理投票流程将在审查期结束后开始，很可能明天开始。

**关键变更****inference-chain**

- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集合。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被削减。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，并在所有确认和奖励权重计算中使用该快照。
- 当参与者重新变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新恶意推断会立即再次使其失效。现在在重新激活和即将晋升时重置计数器。
- 在 v0.2.12 之前加入的 DAPIs 在冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级会回填该权限，以便它们能响应交易商投诉。
...
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束，跳过确认 PoC 触发，因此新的快照逻辑仅从下一个纪元开始运行。复用 v0.2.10 的宽限期原语。
...
- 状态快照减少了长期会话的恢复工作量。
- 负载查找固定在托管纪元，对纪元边界和遗留纪元-0 请求提供回退。
...
为使变更能在下一轮PoC前生效，该提案已加速处理。投票将持续12小时。
...
预评估与PoC开始之间有500个区块的窗口期。在此期间无CPoC任务，因此已声明意向的主机可安全地将模型节点切换至 `Kimi K2.6`。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保持您的MLNode使用 `Qwen235B`，并于明天提交下一轮的意向。
...
- **多模型PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将单一固定模型的计算证明（PoC）过渡为按模型分组的PoC。每个经治理批准的模型生成自己的本地PoC权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托PoC投票权重）。
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：该模型组将在升级后两个轮次激活。其系数为Qwen235B的3.51倍，基于相同硬件（8xH200，8xB200）上的模型计算复杂度。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 抵押资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
此清理过程**必须在升级前完成**。若升级时您的节点配置包含不受支持的模型，**节点将被拒绝并离线。**

版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留之前强制执行的模型和 Kimi。
每个 DAPI 会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。若配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图截断为强制模型来掩盖此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 仍显示为干净。版本 0.2.12 停止了此截断，直接加载持久化配置。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接或委托 PoC 投票权重）。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者 slashing 与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 请在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解措施。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型转变为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，之前为 32 位），将通过独立的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
针对升级提案 v0.2.11 的链上治理投票已结束。该提案已获**批准**，升级已在主网上成功执行。
...
目前，通过每个推理的链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因其已完成，而是因为此类系统需要尽早暴露于真实网络环境中，某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这还可使每个区块容纳最多 100 倍的推理量，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中将同时启动剪枝活动，造成不必要的运营压力。
...
本次 PR 审查中具有实质性贡献的，可能在下一次升级中获得奖励。
...
这故意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为它被认为已完成，而是因为此类系统需要尽早暴露在真实网络条件下，某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时启动剪枝活动，造成可避免的操作压力。
...
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
审查者可在此处找到完整的升级提案、迁移详情、测试摘要和提议流程：[链接](https://github.com/gonka-ai/gonka/pull/813)。
...
如果您无权访问持有投票权的冷钱包密钥，建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代为投票。若无此授权，其他账户无法提交投票。
...
现已提供新的 vLLM / mlnode 构建，可在PoC开始时中断正在进行的推理请求，以降低PoC启动时仍处于活跃状态的请求可能导致权重下降的风险。
...
抵押参数更新提案未能达到法定人数，因此根据当前治理规则被拒绝。这意味着更新的参数将不会激活。
...
- 确保 [抵押](https://gonka.ai/host/collateral/) 正确设置，否则从第180个纪元起，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍然是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各纪元间可能会波动。较小的权重可能会经历相对更大的波动。为避免在纪元边界出现临时抵押不足，建议在抵押水平相对较低时，存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
由于有效归一化窗口比之前的名义模型长约5个区块，每个纪元重新计算的权重成比例降低。

**观察到的GPU权重变化（第175纪元 → 第176纪元）**
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此次升级根据实际的 PoC 经过时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并确保权重结果与实际执行时长一致。
...
**宽限期：** 升级后引入了 3000 个区块的宽限期，期间不进行确认 PoC，并对升级所在周期的漏报率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工的抵押金损失也不会超过 0.5%。所需抵押金仅占每日奖励的约 24%。

**警告：** 抵押金将无论投票结果如何均生效。如果该提案未通过，创世时定义的抵押参数将在第 180 周期自动激活，而非上述参数。

投票结束后、第 180 周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 周期起，其奖励将被削减至五分之一。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获得批准，该提案将引入一项对 PoC 验证的重大优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级当期降低了误报率和失效率的阈值。
...
- 在升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中旧备份的指南，请参见[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用了大量磁盘空间，可应用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理技术。
...
任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下实施。如果稳定性得到验证，逐步将抵押品提高至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止运行。这是因为节点运行的是与网络不再兼容的旧版二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
在当前周期中观察到cPoC（确认PoC）与PoC之间的重叠。直至本周期最后一个区块，`is_confirmation_poc_active`被观察为`true`。

此重叠的影响正在评估中。初步观察表明，没有节点记录PoC提交，导致本周期累积权重为零。
...
链上推理访问现已开放，且不限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
此升级根据实际PoC耗时对参与者权重进行标准化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串形式编码，而非数字，且int64/uint64值现在也以字符串形式编码。
...
- 在当前模型配置下，B200 GPU的推理性能并未高于H100类GPU，基于观察到的推理轨迹。
- 如果未来通过治理引入更大或更苛刻的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测量，显示出与PoC v2中观察到的GPU类型之间相同的相对性能差异。

**追踪器（仪表板）维护者的操作**

在更新后的权重分配生效后，追踪器（仪表板）维护者可能希望审查其第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。
...
所有主机必须随时准备安装补丁。
请保持在线并密切留意公告。一旦补丁准备就绪，将立即发布进一步说明。

## 2026年2月1日

**升级已执行：v0.2.9 已在主网上线**

关于升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并已在区块 2451000 成功执行主网升级。此次升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**注意**

- 下一个 PoC 周期（从纪元 158 到 159 的过渡）至关重要。请确保在线，以便在需要时及时应用后续步骤或缓解指令。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将到来的纪元的参与者集合中。
...
在因 cPoC 惩罚而减少或排除奖励的情况下，未计入的部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
下表显示了上一纪元具有权重、在当前纪元提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
!!! note 
    未能满足上述两项条件的节点，在网络过渡到单模型配置后将失去 PoC v2 参与资格。PoC v2 的权重分配过渡仍受观测采用阈值和后续治理的约束。

    **维护与操作**

- Cosmovisor：节点和API二进制文件的更新将自动处理。现有主机无需对运行中的容器执行手动更新。
- 磁盘空间：Cosmovisor会在`.inference/data`目录中创建完整的状态备份。请确保有250 GB以上的可用空间。
- Postgres：现在可以通过Postgres进行本地负载存储配置，升级后即可启用。
...
本指南说明了主机应如何根据v0.2.8版本模型可用性变更及即将推出的PoC v2更新来升级其ML节点。从第155个周期开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，未满足配置要求的ML节点权重将不被计入。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未同时满足两项条件的节点在网络切换至单模型配置后将不再具备资格。
    - ML节点升级必须在迁移完成且PoC v2通过v0.2.8升级后的独立治理提案激活之前完成。
...
确认 ML 节点仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 服务，这是 PoC v2 权重和未来权重分配所使用的唯一模型：
```
...
PoC v2 仅在链上治理提案获得批准时才成为权重分配的激活方法。
    在此提案获得批准前：
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦观察到活跃 Host 中达到足够的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不被视为 PoC v2 参与资格合格。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
**注意：** 升级后，可以将 Postgres 配置为本地有效载荷的存储。
...
该延长也为主机准备新的 PoC 要求提供了时间，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步进行更新。
...
未包含在 `allowlist` 中的主机在延长的稳定期内将无法参与 PoC，该期间将在区块 2443558 结束。
...
`allowlist` 具有固定的过期时间，不会在延长的稳定期结束后继续生效。一旦 `allowlist` 在区块 2443558 过期：
...
`Allowlist`到期时间：在区块2443558自动到期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的到期时间与区块2,222,222的参与者注册截止时间对齐。
...
`Allowlist` 过期时间：自动在区块 2,222,222 到达时生效。
...
2) 之前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块 2,222,222 结束。
...
本次投票定义了稳定窗口期间参与者的 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为始终符合网络预期的参与者集合。
...
建议主机保持在线并监控更新，因为补丁发布后可能需要采取后续行动。
...
- 临时重新激活创世验证者增强——一种此前曾使用过的、具有时间限制的防御机制，现提议重新启用。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理方式控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外发放，金额为第117轮奖励的1.083倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
此次升级将此前通过手动 API 更新分发并已在网络中使用的关键修复正式化。这些修复包括：
...
- 主机级别注册：在区块 2,222,222（约两周后）之前暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本升级中包含的预备变更，为未来通过治理控制参与者入驻和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外支付，金额为第117轮奖励的1.083倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
