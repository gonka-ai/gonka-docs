...
**仅 devshard 升级的 PR 现已开放审查**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调全节点升级，不影响主网行为，且预计不会导致推理服务停机。如果通过治理流程批准，新的 devshard v5 运行时将与现有的 v4 运行时并行运行。

**主要变更**

1. v5 的主要目标是高度同步：每个主机和序列器在保证金日志中保留一个已签名、可重放的主链高度视图，即使在没有推理流量时也是如此。借助该时钟，devshard 现在可以证明地知道 cPoC 正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前包含时间戳字段的消息中。后续版本可以将墙钟字段（如 `confirmed_at` 和 `started_at`）替换为高度标记。这解决了未来时间攻击问题，并为在协议层面处理流超时打开了道路。本次发布包含这些标记，但尚未将超时和密封决策切换到它们。

3. 通过所有插槽的往返心跳机制已添加，因此安静会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同节奏。
...
投票将于 **UTC 时间 8 月 27 日 03:47** 结束。请尽可能参与投票。
...
**提案已通过：稳定小样本中周期内无效 SPRT**

提案编号 96 的链上治理投票已结束。

该提案已被批准。它调整了小样本中无效推理检查的行为，以避免在周期早期数据不足时误触发。主机无需执行任何操作。

感谢所有参与投票的人。

## 2026年8月20日

**提案 96 已开放投票：稳定周期内小样本的无效 SPRT**

一个新的链上治理提案（提案编号 96）已开放投票。

**其内容**

在周期早期，数据尚不足，因此无效推理检查（SPRT）可能在样本足够大之前仅因少量无效数据而触发。本提案对其进行稳定：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本量下，阈值接近10%的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真正的违规者仍会触发它（约360个样本时为18%，约1650个样本时为12%）。
- 小样本量无法触发该机制：至少需要32个被标记为无效的样本，且在500个样本时，观测到的比率必须超过16%。
- 未使用的空闲时间SPRT已关闭。时段结束统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于**UTC时间8月21日06:04**结束（约12小时）。如果可以，请投票。如果未能在时限内达到加速阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。如果转为常规提案，您需要在常规提案上重新投票。
...
一个稳定的网关版本目前正运行在一个网关上，并将用于第一阶段结算。其统计数据，代表了部分网络流量，可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses和Invalid指标将用于链上。根据当前主网阈值配置，错误率低于10%的主机不会触发任何处罚或罚没，但作为协议开发过程的一部分，仍应对其进行调查。
...
**2) 在此24小时窗口之后的第一个时段。** 将在当前网关上手动启动结算，先从小模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi将稍后加入，因为其数据在负载下目前可靠性较低。
...
该部署故意采取渐进方式，以便在结算扩展到整个网络之前识别和解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash现已在Gonka网络上激活**

在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的引导阶段已完成。自链时段360起，`deepseek-ai/DeepSeek-V4-Flash-0731`与MiniMax-M2.7和Kimi K2.6一同成为活跃模型组，DeepSeek组中获得的PoC权重现在以校准系数0.214转换为共识权重。

针对DeepSeek的每模型参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每时段惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（即正在服务Kimi或MiniMax的节点，因为任何已运行DeepSeek的节点均已使用3.0.16），请在方便时将这些节点迁移至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少量节点，确认其稳定后再迁移其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。

MLNode镜像在`vllm-0.25.1-upgrade`分支的[`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)中固定为3.0.16。

默认MLNode镜像（3.0.16）基于CUDA 13.0构建。如果您的主机仍使用CUDA 12.9，请改用`3.0.16-cu129`标签。这是相同的MLNode 3.0.16，只是为CUDA 12.9构建。两个标签均在同一个compose文件中。
...
将活跃的社区销售合约和所有包装代币实例迁移至新存储的CosmWasm代码，并为未来的实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中识别出的理论风险，预计不会影响正常操作。
创世守护者应支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时错过预资格。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 和 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 启动快照——在此前请保持委托分散，以确保快照有效。
...
如果您希望提供DeepSeek服务，请使用上述步骤进行设置。如果不，则不要忘记委托，以避免非参与罚金。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口期内切换是安全的，因为期间无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用以fp8 + nvfp4重打包的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在启动前发布并共享更新的API二进制文件。
...
区块高度是精确的；时间是估算值，将在接近轮次时确认。[更多关于DeepSeek启动的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案在网络上新增DeepSeek V4 Flash作为新的PoC模型。不修改任何现有参数或模型；唯一的变化是新增模型条目。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留您当前的模型：在第360个纪元之前无需操作；从第360个纪元起，若您未提供该模型，则需为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
devshard 运行时通过链上参数提案升级，而非完整链软件升级。若获通过，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动执行主机操作。
...
该提案已获通过，并于主网区块 5316315 成功执行。该提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
v4 是首个面向多实例高可用的版本：在共享的 Postgres 上，通过 versiond-router 部署 N 个 versioned / devshardd 副本，采用粘性会话路由和验证租约独占机制。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十GB，建议提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。
...
devshard 部分正在为 v3 运行时做准备，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和桥接器。为确保主网上线升级期间以太坊桥接器稳定，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接器容器。如果您已完成以上两个步骤，则无需重复操作。若您拥有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含了错误修复和安全补丁。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的RAM增长问题 —— 这正是7月15日网络节点OOM事件的根本原因。
...
`moonshotai/Kimi-K2.6`在第328-329轮中失去了PoC验证多数支持（详情见[此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是恢复其服务且最小化停机时间的最快方式 —— 与6月提案78所采用的恢复路径相同。

该提案将在下一轮PoC前将`moonshotai/Kimi-K2.6`从PoC参数中移除，随后立即重新添加，Kimi将在第331轮开始时进入标准引导流程。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（加速投票，0.667 赞成阈值；出勤率重要，请尽快投票）。
* 引导意图截止日期和纪元时间详见引导说明。

## 2026年7月15日

**Kimi-K2.6 事件（纪元328–329）：发生了什么、恢复计划以及委托指导变更**

在纪元328中，`moonshotai/Kimi-K2.6`失去了其PoC验证的多数支持。在纪元329开始时，运行Kimi的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行Kimi的服务器同时失效：

* 其中一台服务器上，运行Kimi的MLNode因提供商端问题崩溃。大量Kimi委托集中在这台主机上，因此其失效导致这些投票立即消失。
* 第二台服务器因内存不足导致网络节点中断。

Kimi的验证投票一直接近2/3阈值，因此这次失效足以使该组低于多数。守护者投票权未触发：当前未被赋予模型投票权重的守护者会被排除在投票权机制之外。这是一个已知问题，将在即将发布的v0.2.14升级中修复。

这种情况与纪元306–307事件类似。纪元329开始时的投票确切细节仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复Kimi最安全的方式是通过全新引导（约1.5个纪元）重新添加：

1. [今天] 一个加速提案将在下一次PoC之前移除Kimi，并在之后立即重新添加。随后将发布包含提案ID、投票命令和截止日期的独立公告。
2. [明天] Kimi将再次经历标准引导流程：声明意图、部署窗口、PoC存储提交。在意图截止日期前将发布说明。

**委托指导变更——请阅读**

之前的指导（包括2026年6月27日的引导公告）建议将守护者节点作为委托目标。此建议现被撤回：

* **请勿委托给守护者节点。** 守护者是PoC验证的后备机制。将委托集中于它们会将主机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议级限制正在讨论中。
...
* 网络节点内存增长——已在devshard v3中修复；需从v1/v2迁移。完整迁移后，v1/v2将被移除且`/v1/devshard`关闭。为`api`和`versiond`容器设置独立内存限制。

**主机所需操作**

1. 一旦加速的Kimi提案上链，请立即投票（公告随后发布）。
2. 如果您计划提供Kimi：关注引导说明，并在意图截止前准备好声明意图。
3. 如果您为Kimi委托：请选择非守护者委托目标；尽可能将权重分散到多个独立主机。
4. 经纪人：立即迁移devshard流量至`/devshard/v3`。这将解决内存增长问题，且在移除v1/v2前必须完成。
...
提案通过后，v3现在与现有devshard运行时并行运行。新进程通过`/devshard/v3`前缀提供服务，而现有devshard流量可继续使用早期运行时前缀，直到经纪人切换至v3。

此发布将`devshardd`二进制文件作为Gonka发布工件。`versiond`会自动下载该二进制文件，验证sha256哈希，并在现有`versiond`容器内启动额外的`devshardd`进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

白名单上的 devshard 创建者应在主网 v0.2.14 链升级前，将推理流量切换至 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
在此分支上将单独提出并部署 devshard v3 发布版本，且在主网链升级前完成。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供服务。
...
本次仅为请求审查该 Pull Request，不会启动正式投票。治理投票流程将在审查期结束后开始。
...
5. 仪表板维护者 —— 准备好调整指标统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近结束时另行发布。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机必须在第310轮期间声明意向，并在新轮次开始前准备好节点。一个守护节点也将运行 Kimi，因此不希望运行它的主机可以将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换至 Kimi（PoC 开始约在6月28日 ~12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数的选择使得在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相当。此举并非为了偏向任何硬件——同样的权重在 MiniMax 上已可用。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
此方案使 Kimi 成为8xB200和8xB300服务器的理想选择。
...
常规引导流程从第309轮开始。由于投票将在 PoC 开始前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于引导的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。此举并非为了偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
此方案使其成为8xB300服务器的理想选择。

按硬件类型估算的共识权重：
[https://docs.google.com/spreadsheets/d/1yQ-8UHnHC5pvqd6uDB1dRPzHJ2qfJoQrXssXIGi8QMg/edit?usp=sharing](https://docs.google.com/spreadsheets/d/1yQ-8UHnHC5pvqd6uDB1dRPzHJ2qfJoQrXssXIGi8QMg/edit?usp=sharing)

MLNode 版本和说明将在投票结束后发布。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员能够在配置资源前确认模型是否具备可行性。
...
从第309轮开始（PoC 开始约6月26日 15:25 UTC；生效约16:00 UTC），前提是每个模型均满足引导资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动引导。由于每轮约22.8小时，标准48小时投票周期无法适应单轮；而12小时的紧急周期可以。创世守护节点预计支持该提案。

**主机所需操作**

1. **要提供 Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，期间切换是安全的，因为该窗口内无 cPoC。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍使用Qwen或Kimi的主机，除非切换，否则本周期将无法获得cPoC奖励。
* 计划再次提供Kimi的主机应保留Kimi的配置并准备好在PoC 309时将MLNode切换回来——在第308周期将进行一次投票以恢复Kimi。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可选择性参与，提前衡量对该模型的需求，不提供服务的主机不会受罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出组，Kimi无法进入下一周期。现在将其从活跃集合中移除并重新引导，是最快恢复且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的路径。下一周期将进行一次投票以恢复Kimi。
* **Qwen3-235B——退役（独立无关变更）。** 退役旧版Qwen3-235B是一项独立的、此前已提出的阵容调整。它与Kimi的情况无关，仅因同为PoC阵容变更而一并列入。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如尚未准备就绪），以避免在epoch边界时遭遇Hugging Face速率限制。
...
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供该模型的主机也不会受到惩罚。
...
- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长会话将所有已服务的推理保留在RAM或状态中。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长会话将所有已服务的推理保留在RAM或状态中
...
新进程通过`/devshard/v2`前缀提供服务。现有的v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。此类升级期间无需重启节点容器或执行手动主机步骤。
...
这是首次仅适用于devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响主网行为，且不会导致推理服务中断。
...
v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax 组中获得的 PoC 权重现在按校准系数 0.3024 转换为共识权重。

MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需额外操作——现有设置将继续正常工作。尚未做出选择的主机请尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机将对其全部权重施加每纪元 15% 的惩罚。
...
- 治理：将创世守护者投票权降低至约 25%，并将链上法定人数设定为 0.25。如果守护者弃权，非守护者需在剩余 75% 中达到约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
所有主机请验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约 43 分钟前）→ 第一个 MiniMax PoC 阶段在升级后的下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最后 3 个纪元）。
...
第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入第266轮。
...
这可能有助于补偿委员会收集PoC参与的证据，并更准确地计算受影响的贡献。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先检查您是否运行的是当前版本，再进行任何进一步操作。
...
- 如果 `application.db` 占用了大量磁盘空间，可以应用 cosmovisor 备份[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
- 该提案将有意跳过从升级高度到升级纪元结束（3000个区块的宽限期）之间的确认PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作（可能包括以太坊端合约激活），所有主机请验证其桥接容器是否已部署、运行最新版本并正确同步。
...
- 当参与者重新变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推理可能立即再次使其失效。计数器现在在重新激活和即将晋升时重置。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权授予中缺少 `MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应交易商投诉。
...
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束期间，确认 PoC 触发将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用了 v0.2.10 的宽限期原语。
...
- Devshard 现在支持更大部署的共享存储选项。它现在可以使用 Postgres 作为主存储，SQLite 仍作为本地后备。
...
在Kimi-K2.6引导过程中，主机观察到30%的最低直接参与阈值在实际中难以达成。为避免Kimi-K2.6在未来的轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
今天的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保留您的MLNode使用 `Qwen235B`，并于明天提交下一轮的意图。

## 2026年4月30日

**升级已执行：v0.2.12 现已在主网上线**

关于升级提案v0.2.12的链上治理投票已结束。该提案已获得批准，升级已在主网上成功执行。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：该模型组将在升级后两个轮次中激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200，8xB200）上的模型计算复杂度。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
有关这些变更的更多详细信息，请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
每个 DAPI 都会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为每模型 PoC 组。每个经治理批准的模型生成其自身的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接或委托 PoC 投票权重）。
...
- **协议加固。** 为单个 PoC/CPoC 时间随机采样保留的节点（`POC_SLOT=true`）。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易方共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/948) 对下一个链上软件升级 v0.2.12 开放审查。
...
- **多模型PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型转变为按模型分组的PoC。每个经治理批准的模型生成自己的本地PoC权重，并通过模型特定系数聚合为总共识权重
- **共识层交易费用与自动迁移** ([#937](https://github.com/gonka-ai/gonka/pull/937), [#981](https://github.com/gonka-ai/gonka/pull/981))。引入由治理控制的gas价格。协议职责消息（PoC、验证、推理、BLS DKG）通过`NetworkDutyFeeBypassDecorator`免收费用。`MsgPoCV2StoreCommit`采用两部分费用（基础验证 + 计数线性）作为主要Sybil防御机制。详情请见[docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将devshard发布与DAPI/主网发布周期解耦。
...
- **协议加固**。实施更强的PoC v2 RNG（256位全熵，此前为32位），将通过单独的治理投票激活。其他更新包括将`mlnode`版本传播至链上`HardwareNode`、修复DKG经销商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保devshard托管资金的原子性，以及在`inference_finished`事件解析中添加零时间戳容差。
...
- 确保冷钱包持有足够的金额（例如100 GNK），以覆盖自动授予的费用配额上限。
...
更新修复了在PoC开始附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
目前，通过每个推理的链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链仅处理会话创建和最终结算。

这故意是一个早期且受限的版本。提出此版本是为了在主网上进行审查和有限的生产测试，而非因为它被认为是已完成的，而是因为此类系统需要尽早暴露于真实网络环境中。某些问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这还能使每个区块容纳的推理次数最多提升 100 倍，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能同时开始剪枝活动，从而造成不必要的运维压力。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时开始大量剪枝活动，造成不必要的操作压力。
...
**提醒：v0.2.11 升级预计将于下周初进入审核和治理投票阶段。**

请留意并计划参与。投票是支持网络发展的最简单方式之一，也能确保升级与参与者实际需求保持一致。
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代为投票。若无此授权，其他账户无法提交投票。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC启动时仍有活跃请求而导致的潜在权重下降风险。
...
- 确保[抵押](https://gonka.ai/host/collateral/)设置正确，否则从第180个纪元开始，PoC产生的奖励将减少5倍。
...
!!! note "兼容性说明"
    ...
    **基于实时的PoC权重归一化**
    ...
    此升级根据实际的PoC耗时对PoC参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
**宽限期：** 升级后引入3000个区块的宽限期，期间无确认PoC，并对升级时段的错过率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工的抵押品损失也不会超过0.5%。所需抵押品仅占日奖励的约24%。

**警告：** 抵押参数将无视投票结果而生效。如果该提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。
...
下一个链上软件版本v0.2.10的升级提案现已上链并开放投票。若获批准，该提案将引入一项对PoC验证的重大优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个为期3000个区块的宽限期，在此期间不进行确认PoC，并在升级当期放宽了漏报率和无效率阈值。
...
任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下适用。如果稳定性得到证实，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
本次仅为对拉取请求的审查请求，而非正式投票的开始。治理投票流程将在审查期结束后开始。
...
此升级通过实际PoC耗时标准化参与者权重，以减少区块时间漂移效应，并使权重结果与真实执行持续时间保持一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带有锁仓的流式发放转账
...
2. 社区审核该PR后，预计从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，启动正式治理投票流程。
...
在更新的权重分配生效后，追踪器（仪表板）维护人员可能希望审查其第159轮及之后的系数，以确保与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新——补丁可用**

现已提供补丁以解决导致PoC周期内区块验证暂停的问题。建议主机尽快应用该补丁，以确保正确的PoC验证行为并安全恢复区块生成。
...
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将来临的纪元的参与者集合中。
...
- 运行其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，将自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
他们的总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或为零的总权重达到约 48%，这一比例过高。
...
本指南说明主机应如何根据 v0.2.8 模型可用性变化及即将推出的 PoC v2 更新来升级其 ML 节点。ML 节点配置在第 155 个周期开始符合 PoC v2 要求。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个周期后安排。迁移阶段结束后，不符合配置要求的 ML 节点的权重将不被计入。
...
PoC v2 仅在链上治理提案获得批准时，才成为权重分配的活动方法。
    在此提案获得批准前：
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦观察到约 50% 的活跃 Host 足够采用，可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
当前有一个新的治理投票正在进行。它通过将若干地址添加到 [允许列表](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv) 来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 处的参与者注册截止时间对齐。
所有参与逻辑保持不变。此提案仅解决了现有过滤逻辑中的一个小问题。

**可重复性与方法论**

`allowlist` 由公开可观察的链上数据使用预定义的硬件配置模式推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
如果获得批准，网络将临时使用一个 `allowlist`，包含在先前周期中未表现出非常规硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已根据预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（确切的非常规配置字符串列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示的权重的150%。
之前持续偏离这些模式的参与者在稳定窗口结束于区块2222222之前，不包含在 `allowlist` 中。

**可重复性与方法论**

`allowlist` 使用预定义的硬件配置模式从公开可观察的链上数据推导得出。这些模式通过此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
- 临时重新激活创世验证者增强——这是一种此前曾使用过的、具有有限持续时间的防御机制，现提议重新激活。
...
创世验证者增强最初在网路早期阶段作为临时防御机制引入，并在运行的前两个月内处于激活状态。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
本次升级将此前通过手动API更新分发并已在网络上使用的关键修复正式化。这些修复包括：
...
- 开发者地址注册：在稳定期内，将暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表将立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续至区块2,294,222（约19天）。

**治理控制机制**

本次升级包含的预备变更，使未来可通过治理机制控制参与者准入和推理执行，而无需额外的软件升级。本提案不启用任何此类治理激活的约束，需另行投票决定。
...
此 `API` 版本改进了对失败推理的处理韧性，并减少了推理遗漏计数问题。同时引入了对 PoCBatch 和 PoCValidation 交易的批量处理。
