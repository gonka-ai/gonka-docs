...
该提案已获批准。它将从社区金库中拨出 100,000 USDT，用于支持 Gonka 参与 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金涵盖黄金赞助包及相关后勤费用：差旅、住宿、本地协调、嘉宾接待、活动运营和支援材料。
...
重启时，`v4.0.1` 从 nonce 1 恢复了所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 随后在 60 秒就绪超时后终止子进程，导致客户端出现 HTTP 502 错误。

`v4.0.2` 加载最新快照并仅恢复其后的差异。此版本与之前的 v4 版本完全兼容。

发布版本：[devshard/v4.0.2](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v4.0.2)

**更新说明**

请一次更新一个节点，并在继续下一个节点前确认其已恢复正常。
...
**提案通过：将 DeepSeek V4 的 weight_scale_factor 提高至 0.246**

提案 ID 为 98 的链上治理投票已结束。

该提案已获批准。它将 `weight_scale_factor` 的 `deepseek-ai/DeepSeek-V4-Flash-0731` 从 0.214 提高至 0.246。其他所有模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，结果为 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的最低投票门槛）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 因子源于被夸大了的 nonce/min 数值。测量修正见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需执行任何操作。
...
Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，预计不会导致推理服务中断。若通过治理流程批准，新的 devshard v5 运行时将与现有的 v4 运行时并行运行。

**主要变更**

1. v5 的主要目标是高度同步：每个主机和排序器在托管日志中维护一个已签名、可重放的主网高度视图，即使在没有推理流量时也是如此。借助该时钟，devshard 现在可以明确知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前包含时间戳字段的消息中。后续版本可将诸如 `confirmed_at` 和 `started_at` 等壁钟字段替换为高度标记。这解决了未来时间攻击问题，并为在协议层面处理流超时打开了道路。本版本包含这些标记，但尚未将超时和封印决策切换至其上。

3. 已添加通过所有插槽的往返心跳机制，因此安静会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同频率。

**操作事项**

请审查 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并在任何发现、问题、改进建议、边缘情况或潜在漏洞上留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期中获得社区奖励。

本次仅为 PR 审查请求。尚未启动正式投票。
...
在周期初期，数据尚不足，因此无效推断检查（SPRT）可能仅因少量无效样本就触发。本提案将其稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真正的违规者仍会被触发（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效样本，且在 500 个样本时观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如有可能，请参与投票。如果未能在时限内达到紧急阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。若转为常规提案，您需要重新投出您的投票。
...
作为协议开发的重要里程碑，链现已准备逐步启用 devshard 结算，以提升网络的安全性和可靠性。

一个稳定的网关版本目前正运行在一个网关上，并将用于第一阶段结算。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何惩罚或罚没，但仍应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决其问题。无重大问题的主机无需采取任何操作。

仪表板还显示网关内部详细信息，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无法直接操作。

**下一步：****1）未来24小时内。** 主机应检查其统计信息并解决任何关键错误或配置错误。如有需要，贡献者可协助调查问题。

**2）在此24小时窗口后的第一个纪元。** 将在当前网关上开始手动控制结算，优先从较小模型开始。该过程将密切监控，以尽量减少误报记录。Kimi 将稍后跟进，因为其数据在负载下目前可靠性较低。

**3）随后的1-3天内。** 将监控结算结果

**4）一旦此阶段稳定。** 其他经纪人可以部署已稳定的网关版本，并以受控模式开始结算。

**5）在所有经纪人成功验证后。** 结算覆盖率将逐步提升至100%。

推广计划有意逐步进行，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后由[提案98](#august-29-2026)提升至 0.246）转换为共识权重。

针对 DeepSeek 的按模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需额外操作——现有设置将继续有效。尚未做出选择的主机请尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
一个请求：请逐步推广。请勿一次性切换所有节点。请先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。
...
将实时社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。
...
如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请勿忘记委托，以免遭受不参与惩罚。

提醒：明天将上线另一个带有更好日志的MLNode构建版本，敬请期待后续简短通知。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359启动前约500个区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。此窗口内切换是安全的，因为其中无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用经重新打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；请将委托分散至独立的DeepSeek主机。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 若要保留当前模型。在第360轮之前无需操作。从第360轮起，若您未提供DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的不参与惩罚。

区块高度为精确值；时间仅为估算，将在接近轮次时最终确定。[更多关于DeepSeek启动的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案在网络上新增DeepSeek V4 Flash作为新的PoC模型。未修改任何现有参数或模型，唯一变更仅为新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需使用vLLM 0.25.1及发布候选节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
- 候选链参数：PoC `weight_scale_factor = 0.214`，推理 `validation_threshold = 0.90`（处理后的logprobs），`penalty_start_epoch = 360`

DeepSeek的0.214系数为B300主机提供轻微优势。对于其他硬件，产生最高PoC权重的模型保持不变。
...
2. 资格预检快照：链上快照意向和委托，并发出建议性预资格事件。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留当前模型：在第360个周期前无需操作；从第360个周期起，如果您未提供该模型，则需为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
该提案已获通过，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换的二进制文件——无需重启主网或手动操作主机。
...
这只是一个性能和资源管理补丁，仅此而已。协议、推理、验证和经济行为均保持不变。变更包括：纪元追踪从每区块链查询改为事件驱动更新，主机会话和验证工作线程的内存清理现在与纪元修剪对齐。

**升级计划**

devshard 运行时通过链上参数提案进行升级，而非完整链软件升级。若获通过，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动操作主机。
...
该提案已获通过，并于主网区块 5316315 成功执行升级。同一提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
2. 一个**v0.2.15 主网软件升级**，移除大量过时代码、遗留交易处理路径及相关组件，这些已不再符合当前架构的需求。
...
v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是将单体架构重构为高可用、容错、可扩展架构系列变更中的首次更新。

v4 是首个专为多实例高可用设计的版本：在共享 Postgres 后端，通过 versiond-router 部署 N 个版本化的 /devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
- 投票截止：2026年7月30日 07:08 UTC / 2026年7月30日 00:08 PDT
- 提议的升级高度：5316315
- 预计升级时间：2026年7月30日 15:28 UTC / 2026年7月30日 08:28 PDT
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和网桥。为确保主网上线升级期间以太坊网桥稳定，请提前根据 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，将继续正常运行。您需要了解一项语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 仍为默认选项，并会自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放评审**

devshard 升级独立于主区块链进行。它们不需要通过 Cosmovisor 协调的全节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是提高 devshard 主机在主机故障和升级时的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级期间，多台机器可逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同版本名的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可进行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
```
* 在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。此举消除了旧运行时容器长期存在的内存增长问题 —— 这正是7月15日事件中网络节点OOM的根本原因。

**这标志着v3切换最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止时间前对提案88进行投票 —— 紧急窗口时间很短。
2. 提供Kimi服务的主机：在投票结束后、区块 **5,105,276**（2026年7月17日，约UTC 12:05）前立即声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日，约UTC 12:50）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**请勿委托给守护者节点**；将委托分散至独立的Kimi主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取更新的委托指导。
4. 经纪人 / devshard创建者：若您的任何网关流量仍在使用v1/v2或`/v1/devshard` —— 请在投票结束前切换至`/devshard/v3`。
...
提案详情及投票可通过`inferenced`查看。任何活跃节点均可使用。可用节点包括：
...
- 意向截止时间（第330轮）：区块 **5,105,276** — 2026年7月17日，约UTC 12:05（约05:05 PDT）。
...
`moonshotai/Kimi-K2.6`在第328–329轮中失去了PoC验证多数支持（详见[事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式 —— 与6月提案78所用的恢复路径相同。

该提案将在下一轮PoC前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将从第331轮开始进入标准引导流程。

**主机所需操作**

1. 在截止时间前对提案87进行投票 —— 紧急窗口时间很短。
...
投出您的投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 或更高版本（v0.2.13+）。
```
export NODE_URL=https://node3.gonka.ai/
...
**Kimi-K2.6 事件（纪元 328–329）：发生情况、恢复计划及委托指导变更**
...
* 其中一台服务器上运行 Kimi 的 MLNode 因供应商端问题崩溃。大量 Kimi 委托集中在这台主机上，因此其失效立即移除了这些投票。
* 第二台服务器因内存不足导致网络节点断开。

Kimi 的验证投票一直接近 2/3 阈值，因此此次失效足以使该组低于多数。守护者投票决胜机制未启动：当前未赋予每模型投票权重的守护者会被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。
...
1. [今天] 一个紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的独立公告。
2. [明天] Kimi 将再次通过标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布说明。

**委托指导变更——请阅读**

此前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤回：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单台主机。** 在支持基于百分比的多主机委托前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重，且无单点故障。

**已在进行中的修复*** 守护者参与 PoC 验证（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完成迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请立即投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：请选择非守护者委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：请立即迁移 devshard 流量至 `/devshard/v3`。这将修复 RAM 增长问题，且在移除 v1/v2 前必须完成。
...
这是一次仅针对 devshard 的运行时升级。它独立于完整链软件升级，无需链二进制升级。

提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直至经纪人切换至 v3。
...
获准的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这使他们能够在链升级运行期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
devshard 部分准备了 v3 运行时，使代理能够在链升级期间无需依赖已弃用的经典 API 路径即可提供推理服务，同时改进 RAM 利用率并启用 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
将从该分支单独发布 devshard v3 版本，并在主网链升级前提案并部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供服务。
...
devshard v3 发布版本将在主网链升级之前提出并部署，以便经纪人可以在升级运行前将推理流量迁移到 `/devshard/v3`。

**主机操作项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对任何发现、问题、改进建议、边界情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据指南将 `api` 二进制文件和桥接镜像更新至 0.2.14。如果您已更新 `api` 二进制文件，则只需更新桥接镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
根据历史数据，此前未观察到此类攻击。然而，在当前周期中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不希望运行它的主机可以将其权重委托给该节点。
...
> **更新（2026年7月15日）：此建议已撤回 —— 请勿委托给守护节点。** 守护节点是 PoC 验证的后备机制，必须与委托保持独立。请选择运行该模型的非守护节点，并避免选择已是主要委托目标的节点。请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指引。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定使得在8xB300硬件上运行 Kimi 时，其共识权重与主模型 MiniMax 基本相当。此设定并非偏向任何硬件——MiniMax 上已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
...
常规启动流程从第309轮开始。由于投票在 PoC 开始前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于启动的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定使得在8xB300硬件上运行 GLM-5.2 时，其共识权重与 MiniMax 基本相当。此设定并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3 可达性）。这使操作者能够在配置硬件前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由 PoC 开始时实际提交 PoC 存储提交的节点决定——而非仅凭声明意向。只有满足资格条件的模型才会激活。
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动。由于每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急投票周期可以。创世守护节点预计支持本提案。

**主机需采取的操作**

1. **要提供 Kimi 服务：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口期，此期间切换是安全的，因为此期间无 cPoC。
...
* 加急提案要求0.667的通过阈值；参与率至关重要，请尽快投票。

## 2026年6月25日

**提案78已通过：`MiniMaxAI/MiniMax-M2.7`成为唯一的PoC模型；Kimi K2.6和Qwen3-235B已移除**
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换后才会获得本纪元的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308个纪元将进行一次投票以恢复Kimi。

**接下来的内容**

在第308个纪元将进行一次加急投票，合并两项变更，均将于第309个纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导程序。
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，因此主机可选择参与，提前测量对该模型的需求，不提供服务的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前没有足够的投票支持以通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快恢复且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi没有足够的投票支持通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的方式。下一个纪元将进行一次投票以恢复Kimi。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已提出的阵容调整。它与Kimi的情况无关，仅因同样是PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在第307个周期结束前完成，因此这些变更被合并为一次紧急投票。每个周期约22.9小时，标准的48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创始守护者应支持该提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请提前下载FP8权重（若尚未准备就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **请在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划未来再次提供Kimi的主机，现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回——第308周期将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 第308周期将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与，提前测试模型需求，而不参与的主机不会受到任何惩罚。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
提案获批后，v2现在可与现有的v1 devshard运行时并行运行。新进程在`/devshard/v2`前缀下提供服务，而现有v1流量继续在`/devshard/v1`和`/v1/devshard`上运行。
...
- 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中
...
新进程在`/devshard/v2`前缀下提供服务。现有v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。此类升级期间无需重启节点容器或手动执行主机步骤。
...
v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax 组中获得的 PoC 权重现在将按校准系数 0.3024 转换为共识权重。

MiniMax 的按模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续正常工作。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
关于 [升级提案 v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **获得批准**。

升级将在主网上自动在 **区块高度 4267300**（约 **2026年5月26日 14:42 UTC** / **07:42 PDT**）执行。
...
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从第 278 个纪元起，未采取任何操作的主机会对其完整权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基准。该调整**仅适用于您共识权重中 Kimi 相关的部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。如果守护者弃权，非守护者需在剩余 75% 中实现约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将 devshard 非 nonce 限制从 `20_000` 提高到 `1_000_000`。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不参与投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 该提案将有意在升级高度到升级纪元结束（10000区块宽限期）期间跳过确认 PoC。若获批准，此跳过为预期行为，并非故障；新快照逻辑将在下一个纪元开始。
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个纪元）。
...
该问题已追溯至使用非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
在相关防护措施实施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪人端点。
...
- 将每个周期的最大 devshards 数量从 `100` 提高到 `500_000`。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理门槛设为 `0.25`。在守护者不投票的情况下，剩余 75% 投票权的有效门槛为 1/3（`0.25 / 0.75 = 0.334`）。
...
**第 278 周期的 `MiniMaxAI/MiniMax-M2.7` 参与选择**

对于每个治理批准的模型，多模型 PoC 要求每个主机明确选择参与方式（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 之后不采取任何操作将导致惩罚。在此阶段，您应提前决定首选选项，以便在提案通过且升级成功应用于主网时能迅速行动。

**桥接容器更新/验证**

请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。若如此，请先确认您运行的是当前版本，再执行后续操作。
...
- 该提案将有意跳过从升级高度到升级纪元结束（3000个区块的宽限期）期间的确认 PoC。若获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作（可能包括以太坊侧合约激活），请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。
...
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新产生的错误推断会立即再次使其失效。现在在重新激活和即将晋升时重置计数器。
- 在 v0.2.12 之前加入的 DAPIs 其冷到暖授权中未包含 `MsgRespondDealerComplaints`。升级将补全该权限，以便它们能响应交易商投诉。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能延迟启动。这延迟了 api 激活，导致部分矿工因延迟跳过确认 PoC。

修复方案通过并行加载 devshards 并从快照恢复现有 devshard 会话来移除该阻塞。

[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)

请更新 api 容器的二进制文件。在每次 PoC 开始前有 500 个区块的无 CPoC（`confirmation_poc_safety_window`）窗口，因此这可能是部署最安全的版本。
...
在Kimi-K2.6引导过程中，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来的轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
预评估与PoC启动之间有500个区块的窗口期。在此期间，无CPoC任务，因此已声明意图的主机可安全地将其模型节点切换至`Kimi K2.6`。
...
今天的预资格验证未通过，对于`PoCIntent`权重低于30%的主机。请保留您的MLNode使用`Qwen235B`，并于明天为下一轮次提交您的意图。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
有关这些更改的更多详情请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
如果您没有直接访问具有投票权的密钥，或希望使用其他密钥代您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
这故意是一个早期且受限的版本。提出此版本并非因其已完成，而是因为此类系统需要尽早暴露于真实网络环境中，以发现某些仅靠本地测试难以暴露的问题。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时开始大规模剪枝，造成可避免的操作压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能同时开始大量剪枝活动，造成可避免的操作压力。
...
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
审查者可在此处找到完整的升级提案、迁移详情、测试摘要和提议流程：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)。
...
如果您没有访问持有投票权的冷密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求他们授权您代为投票。没有此授权，其他账户无法提交投票。
...
**建议（可选）：vLLM / mlnode 构建在 PoC 启动时中断正在进行的请求**

现已提供新的 vLLM / mlnode 构建，可在 PoC 启动时中断正在进行的推理请求，以降低因 PoC 开始时仍有活动请求而导致潜在权重下降的风险。
...
抵押参数更新提案已结束，但未达到法定人数。因此，根据当前治理规则，该提案被否决。这意味着更新的参数将不会被激活。
...
- 确保 [抵押](https://gonka.ai/host/collateral/) 正确设置，否则从第180轮次起，PoC 奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重可能在各轮次间波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平仍相对较低时，存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为使 PoC 权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选择。如实施所示，有效的 PoC 参考窗口比先前名义假设长约5个区块。
...
由于有效归一化窗口比之前的名义模型长约5个区块，因此每轮次重新计算的权重成比例降低。

**观测到的 GPU 权重变化（第175轮次 → 第176轮次）**
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际 PoC 经过时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并使权重结果与实际执行持续时间保持一致。
...
**宽限期：** 升级引入了一个宽限期，在升级后 3000 个区块内不进行确认 PoC，并对升级时期的错过率和无效率阈值采取较宽松的标准。
...
**警告：** 抵押参数将 Regardless of 投票结果生效。如果此提案未通过，创世时定义的抵押参数将在第 180 个周期自动激活，而非上述参数。

投票结束后、第 180 个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 个周期起，其奖励将被削减至五分之一。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并降低了升级时段的错过率和无效率阈值。
...
这意味着，在单个周期内，即使被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押品仅占日收益的约24%。

提案提交投票后将另行公告。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世时期定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。
...
即将进行的软件升级v0.2.10的链上投票期预计将于周日晚上（洛杉矶时间）/周一上午（UTC）开始。
...
请审阅GitHub上的v0.2.10升级PR并提供您的反馈。有意义的审查贡献可能在下一次升级中获得奖励。
...
在当前周期中观察到cPoC（确认PoC）与PoC之间的重叠。直至该周期的最后一个区块，`is_confirmation_poc_active`被观察为`true`。
...
链上推理访问当前已开放，且不限于开发者。推理请求可通过上一版本引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
此升级通过实际PoC经过时间对PoC参与者的权重进行标准化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- **[PR #634](https://github.com/gonka-ai/gonka/pull/634)**：添加请求主体大小限制以降低DoS风险。
...
现有主机无需升级其 `api` 和 `node` 容器。更新后的容器版本旨在供在链上升级完成后加入的新主机使用。
...
在PoC v2启用后，权重分配现在基于当前模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`的实测推理性能。因此，中位GPU权重以及GPU类型间的相对权重比例均已调整。
...
- 在 PoC 外部使用标准 vLLM 基础推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与 PoC v2 中观察到的相同 GPU 类型间的相对性能差异。

**追踪器（仪表板）维护人员的操作**

在更新的权重分配生效后，追踪器（仪表板）维护人员可能希望审查第 159 期及之后的系数，以确保与当前 PoC v2 权重分配保持一致。

## 2026 年 2 月 2 日

**网络更新——补丁可用**

现已提供补丁以解决导致 PoC 周期中区块验证暂停的问题。建议主机尽快应用补丁，以确保正确的 PoC 验证行为并安全恢复区块生产。

**需要采取的操作**

请主机尽快应用补丁，以确保正确的 PoC 验证行为并安全恢复区块生产。
```
# Download Binary
...
所有主机必须随时准备在短时间内安装补丁。
请保持在线并密切关注公告。一旦补丁准备就绪，将立即分享进一步的说明。
...
他们的总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或参与极低的总权重达到约 48%，这一比例过高。
...
- 身份验证绕过：将 `epochId` 绑定到签名，以验证对应正确的周期。
...
本指南说明了主机应如何根据v0.2.8模型可用性变化和即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。

**1. 背景：模型可用性变化（升级v0.2.8）**
...
!!! note "治理和 PoC v2 激活说明"
    ...
    **阶段 1. 观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML 节点切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为兼容 PoC v2 的版本，才能为 PoC v2 权重做贡献。
...
仅当链上治理提案获得批准时，PoC v2 才成为权重分配的活动方法。
    在此提案获得批准之前：
...
作为 v0.2.8 升级的一部分，支持的模型集已更新。所有先前支持的模型均从活跃集中移除，除：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估向 PoC v2 过渡的准备情况。一旦在活跃主机中观察到足够程度的采用率（约 50%），可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
由于变更量较大，此处仅列出部分重点内容。更多更新和修复的完整列表请参见 [GitHub 拉取请求](https://github.com/gonka-ai/gonka/pull/539)。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不被视为 PoC v2 参与资格合格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
- 逐步部署：如果您运行多个 ML 节点，建议在多个周期内逐步进行这些更新。
...
该提案将当前的稳定期延长约两周。延长的期限旨在为即将到来的 PoC 变更及相关网络升级进行额外测试。有关新 PoC 开发进展的更多详情，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。
...
如果获得批准，网络将暂时在现有的 `allowlist`（由未表现出非标准硬件行为的主机组成）下继续运行。
...
`Allowlist`过期时间：自动在区块2443558处过期。
...
1) 网络将使用一个 `allowlist` 运行，该 `allowlist` 由在多个纪元中满足以下条件的参与者组成：
...
2) 此前表现出偏离这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块2,222,222处结束。
...
此次投票定义了稳定窗口期间的参与者 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为一直符合网络预期的参与者集合。

**投票范围**

若获得批准，网络将临时使用一个 `allowlist`，包含在先前纪元中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))
- 观察到的PoC权重低于使用同类硬件的其他参与者所表现的权重的150%。
此前持续偏离这些模式的参与者在稳定窗口于区块2222222结束前不属于 `allowlist`。
...
- 临时重新激活创世验证者增强——这是一种此前曾使用过的、有时间限制的防御机制，现提议重新激活。
...
此次升级正式将此前通过手动API更新分发并已在网络中使用的关键修复纳入协议。这些修复包括：
...
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。一个预定义的`allowlist`开发者地址白名单立即生效。白名单内的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理机制控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得等于第117轮奖励1.083倍的发放，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
此升级将此前通过手动 API 更新分发且已在网络中使用的关键修复正式化。这些修复包括：
...
- 主机级注册：新主机的注册将在区块 2,222,222（约两周后）之前暂停。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的约束，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外支付，金额为第117轮奖励的1.083倍，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于常态的推理遗漏率。
在许多情况下，这是由一个错误导致的：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
