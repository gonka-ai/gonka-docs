...
上周，HackerOne 上报告了 PoC-v2 权重路径中的一个漏洞。参与者可以膨胀其计算权重，从而影响区块奖励、共识投票权和治理投票权。

根据历史数据，此前未发现此类攻击。但在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。

针对即将发布的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供了一个仅限 API 的热修复程序——它可以异步部署，并通过强化 PoC-v2 验证来阻止攻击，使复制的计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第 311 纪元进行另一次引导尝试。希望提供 Kimi 服务的主机需在第 310 纪元声明其意向，并在新纪元开始前准备好节点。守护节点也将运行 Kimi，因此不希望运行 Kimi 的主机可将其权重委托给该节点。
...
```
2. 如果足够多的主机声明意向，请在第 311 纪元开始前约 500 个区块的窗口期内将您的节点切换为 Kimi（PoC 开始于约 6 月 28 日 ~12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使 8xB300 上的 Kimi 能获得与主模型 MiniMax 相近的共识权重。此举并非偏向任何硬件——MiniMax 上已存在相同的权重。对于运行 Kimi 而非 MiniMax 的 8xB300 主机，将获得约 1% 的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数的选择使 8xB300 上的 GLM-5.2 能获得与 MiniMax 相近的共识权重。此举并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的 8xB300 主机，将获得约 2% 的增益。
...
1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 服务的主机需在引导快照 `start_poc - deploy_window` 之前提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 和 >2/3 可达性）。这使操作员可在配置前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机将在部署窗口期内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在 PoC 开始时实际提交 PoC 存储提交的节点决定——而非由声明意向的节点决定。只有满足资格条件的模型才会变为活跃状态。

**批准后的效果**

从第 309 个周期开始（PoC 开始时间约为 UTC 时间 6 月 26 日 15:25；生效时间约为 16:00 UTC），前提是每个模型均满足启动资格：

* `moonshotai/Kimi-K2.6` 再次成为活跃的 PoC 模型。
* `zai-org/GLM-5.2-FP8` 可作为可选的 PoC 模型，无未参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 保持为基础模型。

**为何加速**

该提案必须在第 308 个周期结束前完成，以便两个模型都能在第 309 个周期启动。每个周期约 22.8 小时，标准的 48 小时投票周期无法容纳在一个周期内；而 12 小时的加速周期可以。创世守护者应支持该提案。

**主机所需操作**

1. **要提供 Kimi 服务：** 在投票结束**后立即**声明对 `moonshotai/Kimi-K2.6` 的意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约 500 个区块的安全窗口，期间切换是安全的，因为此期间无 cPoC。

2. **要提供 GLM-5.2 服务：** 在部署窗口期间声明对 `zai-org/GLM-5.2-FP8` 的意向并配置您的 MLNode。提供该模型是自愿的——未选择参与的主机不会受到惩罚。

3. **要继续使用 MiniMax：** 继续提供 `MiniMaxAI/MiniMax-M2.7`——无需操作。

4. **在截止日期前对提案 79 进行投票。** 加速窗口很短。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过 `inferenced` 查看：任意活跃节点均可使用：
...
```
**截止日期*** 提案 79 投票结束：*2026 年 6 月 26 日 14:18:58 UTC*（加速）。
* 加速提案需要 0.667 的赞成阈值；参与率很重要，请尽快投票。

## 2026 年 6 月 25 日

**提案 78 通过：`MiniMaxAI/MiniMax-M2.7` 现为唯一的 PoC 模型；Kimi K2.6 和 Qwen3-235B 已移除**

提案 78 的加速投票已通过。这些变更从第 308 个纪元起生效。
...
自第 308 个纪元起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃 PoC 模型。Qwen3-235B 和 Kimi K2.6 不再活跃。

**主机所需操作*** 请确保您的 MLNode 正在提供 `MiniMaxAI/MiniMax-M2.7`。任何仍使用 Qwen 或 Kimi 的主机在本纪元将无法获得 cPoC，直到切换为止。
* 计划再次提供 Kimi 的主机应保留其 Kimi 设置并准备好在 PoC 309 时将 MLNode 切换回来——将在第 308 个纪元进行一次恢复 Kimi 的投票。

**接下来的内容**

在第 308 个纪元将进行一次加速投票，合并两项变更，均在第 309 个纪元生效（PoC 开始时间约为 2026 年 6 月 26 日 15:25 UTC）：

* **恢复 Kimi K2.6** —— 重新添加 Kimi 并重启其引导程序。
* **添加 GLM-5.2** —— 添加 GLM-5.2 **不设不参与惩罚**，以便主机可选择加入并提前衡量对该模型的需求，不提供该模型的主机不会受到惩罚。

## 2026 年 6 月 25 日

**加速治理投票（提案 78）：MiniMax-M2.7 成为唯一的 PoC 模型；Kimi K2.6 被移除以便快速重启；Qwen3-235B 已退役**

`moonshotai/Kimi-K2.6` 目前没有足够的投票支持以通过 PoC 验证。因此，提供 Kimi 的参与者被移出组别，Kimi 无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。

一项加速提案现已上链，以便在下一个纪元开始前生效。
链上提案在单次投票中执行以下操作：
1. 将委托 `initial_model_id` 设置为 `MiniMaxAI/MiniMax-M2.7`，使 MiniMax 成为基础模型。
2. 仅在 PoC 参数中保留 `MiniMaxAI/MiniMax-M2.7`。
...
**每项变更的目的*** **Kimi K2.6 — 移除以便快速恢复。** 由于 Kimi 没有足够的投票支持通过 PoC 验证，将其移除并重新引导是恢复它且停机时间最短的最快路径。下一纪元将进行一次恢复 Kimi 的投票。
* **Qwen3-235B — 退役（独立且无关的变更）。** 退役较旧的 Qwen3-235B 是一项独立的、此前要求的阵容变更。它与 Kimi 情况无关，仅因同样是 PoC 阵容变更而被包含在此处。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次加速投票，因为重置必须在第 307 个纪元结束前完成。每个纪元约 22.9 小时，标准的 48 小时投票期无法容纳在一个纪元内；而 12 小时的加速期可以。创世守护者应支持该提案。

**若获通过的效果**

从第 308 个纪元（约 2026 年 6 月 25 日 17:25 UTC）起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃 PoC 模型。Qwen3-235B 和 Kimi K2.6 不再活跃。Kimi 的恢复将在下一纪元的后续投票中处理。

**主机所需操作**

1. **在 PoC 308 开始前将您的 MLNode 切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用 Qwen 或 Kimi 的主机——都必须将 MLNode 切换至 MiniMax。在 PoC 开始前有约 500 个区块的窗口期可以安全切换，因为此时没有 cPoC。请现在预下载 FP8 权重（若尚未准备好），以避免在纪元边界时遭遇 Hugging Face 速率限制。
2. **在截止日期前对提案 78 投票。** 加速窗口很短。
3. 计划再次提供 Kimi 的主机现在仍应将 MLNode 切换至 MiniMax，但需准备好在 PoC 309 时将其切换回来——第 308 个纪元将进行一次恢复 Kimi 的投票。


**接下来的内容*** **恢复 Kimi K2.6** —— 第 308 个纪元中的一次后续投票，以重新添加 Kimi 并重启其引导程序。
* **GLM-5.2** — 一个后续提案将添加 GLM-5.2 **而不设不参与惩罚**，以便主机可以选择加入并提前测试模型需求，对于选择不提供该模型的主机不会有任何惩罚。
...
**截止日期*** 提案 78 投票结束：**2026 年 6 月 25 日 15:39:53 UTC**（加速处理）。
* 第 308 个纪元将在稍后形成：PoC 开始于约 16:50 UTC，生效于约 17:25 UTC。
* 加速提案要求 0.667 的通过阈值；参与率很重要，请尽快投票。
...
3) 添加了通过 Grafana、Jaeger、Prometheus、Loki、Promtail 和 cAdvisor 实现的 join-stack 可观测性。
4) 将每次推理的验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每个槽的总数。
5) 在纪元变更时修剪旧纪元存储，将 SQLite/Postgres 模式设置移出热路径，并强制每个进程仅选择一个存储后端。
...
4. 将每次推理的验证计数器存储在状态根之外的 SQLite/Postgres 中，并在推理修剪后通过 devshard 统计端点暴露每个槽的总数
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获批，`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

新进程通过 `/devshard/v2` 前缀提供服务。现有的 v1 devshard 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。在此类升级期间，无需重启节点容器或手动执行主机操作。
...
这是首次仅针对devshard的升级，因此流程不同于标准链升级。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，也不预期会导致推理服务中断。
...
本次仅为PR评审请求。它不启动正式投票。治理投票流程将在评审期结束后开始。
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需做任何其他操作——现有设置继续有效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
升级将在主网**区块高度4267300**（≈ **2026年5月26日周二14:42 UTC** / **07:42 PDT**）自动执行。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6`和`MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一到两个模型的主机，也必须为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元`278`**。未采取任何操作的主机将从纪元278起，每纪元对其全部权重施加15%的惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。请确保`.inference/data`有足够的空闲空间用于cosmovisor状态备份；如果`application.db`较大，建议在升级前应用[cosmovisor备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13校准将Kimi K2.6 `WeightScaleFactor`从`1.26`调整为`0.78`，以反映Qwen-on-B200参考的vLLM-0.20.1吞吐量基线。此调整**仅适用于您共识权重中的Kimi部分**；您的Qwen部分权重和Kimi内部PoC分配保持不变。在B200/B300上，Kimi仍是收益最高的选项；在H100/H200上，MiniMax-M2.7成为与Qwen相当、高于Kimi的选项。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新投票，现提交了多项更新。

- 包括：Kimi的重新校准权重（`0.78`）、新模型`MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专用部分）。
- 该提案将升级后的宽限期延长至3000个区块，以便主机在新快照逻辑稳定期间不被惩罚。
- 治理：将创世守护者投票权降低至约25%，并将链上法定人数设为0.25。若守护者弃权，非守护者需在剩余75%中实现约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 在vLLM 0.20.1发布后，基于Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B: `0.359`（不变）
    - Kimi K2.6: `0.78`（从1.26下调，同等PoC权重下每轮共识权重约降低38%）
    - MiniMax-M2.7: `0.3024`
...
主机需更新仪表盘/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先参考加入网络指南。此仪表盘更新仅涉及容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 运营商时间线：投票于5月22日22:12 UTC结束 → 升级高度约在5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在start_poc − 500区块处（约提前43分钟）→ 首个MiniMax PoC阶段在升级后下一个纪元边界开始 → MiniMax惩罚在链纪元278执行。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获批准，此跳过是预期行为而非故障；新快照逻辑将在下一个纪元开始。
- 若获批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认设置并自动清理（保留最近3个纪元）。
- 若提案未通过（未达到法定人数，或 `no_with_veto` 超过⅓），链上不会发生任何变化，升级将不会执行。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
纪元#266中的问题由针对运行Kimi模型的主机的攻击引起。网络继续运行，但该攻击结合若干相关条件，导致许多参与者无法进入纪元#266。
...
该问题被追溯到具有非标准语义的推理请求。该攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。
...
由于现有协议规则，Kimi在纪元#267中的活跃权重较低：单个模型的总权重不得超过上一纪元所有模型总权重的75%。
...
2. 监控接下来几个周期的PoC参与情况。检查您的节点是否如预期进入PoC，且活跃权重是否正确反映。
...
- 在本次升级周期剩余时间内禁用确认PoC，以便新快照逻辑从下一周期干净启动。
...
- 将创世守护者的调整投票权降低至约25%，并将链上治理的最低通过票数设定为 `0.25`。在守护者不参与投票的情况下，这使得剩余75%投票权的有效通过票数为1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再进行任何进一步操作。
...
作为即将进行的 v0.2.13 升级准备工作的一部分，可能包括以太坊侧合约激活，所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。
...
本次仅为拉取请求的审查呼吁，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天启动。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推理就可能再次使其失效。现在在重新激活和即将升级时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授予中未包含`MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应经销商投诉。
- Devshard结算使用了硬编码的`20_000`_nonce限制。现在限制为`DevshardEscrowParams.MaxNonce`，v0.2.13升级将其设置为`1_000_000`。升级还将`MaxEscrowsPerEpoch`提升至`500_000`。
- 升级为当前纪元安装了一个宽限期条目，延长了`UpgradeProtectionWindow`（3000个区块）。从升级高度到升级纪元结束期间，确认PoC触发器将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用了v0.2.10的宽限期原语。
- Wasm守护进程访问在应用连接后解析，因此合约权限检查适用于网桥和流动性池操作。
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实践中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化更多模型的上线流程，建议将该阈值降低至10%。
...
在预评估与 PoC 开始之间有 500 个区块的窗口期。在此期间无 CPoC 任务，因此已声明意向的节点可安全地将其模型节点切换至 `Kimi K2.6`。
...
- 投票结束：2026年4月30日，UTC 00:12
- 升级高度：3834200
- 预计升级时间：2026年4月30日，UTC 6:00
...
本次仅为拉取请求审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始。
...
- **协议加固**。实现更强的 PoC v2 RNG（完整 256 位熵，此前为 32 位），将通过独立治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使传统验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于靠近PoC启动的节点，稳定性有所提升。

此次更新修复了PoC启动附近的一个边缘情况，此前在特定条件下可能导致性能下降。
...
下一版链上软件v0.2.11的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每笔推理交易处理推理操作限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅处理会话创建和最终结算。

这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以暴露。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时开始大量剪枝活动，从而造成不必要的运营压力。
...
这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被认为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
已发现 `v0.2.10` 中存在潜在漏洞。为降低当前升级前时期的潜在风险，建议在下一次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
如果您无权访问持有投票权的冷钱包密钥，则建议提前安排投票委托。请与该密钥的所有者联系，请求授权您代为投票。若无此授权，其他账户将无法提交投票。
...
1. 升级 v0.2.10 引入了Executor → MLNode连接的严格5分钟超时，而某些请求可能需要更长时间。新版本的API将恢复该值，而不是强制执行严格限制。
...
现已提供新的 vLLM / mlnode 构建，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍有活跃请求而导致潜在权重下降的风险。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能发生波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
针对升级提案v0.2.10的链上治理投票已结束。该提案已获批准，升级已在主网成功执行。此次升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此次升级根据实际 PoC 持续时间标准化 PoC 参与者权重，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。
...
这意味着在单个周期内，即使被惩罚，矿工的抵押金损失也不会超过 0.5%。所需抵押金仅占每日奖励的约 24%。

**警告**：无论投票结果如何，抵押金都将生效。如果此提案未通过，创世时定义的抵押参数将在第 180 周期自动激活，而非上述参数。

投票结束后、第 180 周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 周期起，其奖励将减少 5 倍。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级时段内放宽了错过率和无效率阈值。
...
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
这意味着即使在一个周期内被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押仅占日奖励的约24%。

提案提交投票后将另行发布公告。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有充分理由时适用。如果证明稳定性良好，则逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK），将支持网络的长期成功。
...
请在GitHub上审阅v0.2.10升级PR并留下您的反馈。有意义的审查贡献可能在下一次升级中提出奖励。
...
本次仅为对拉取请求的审阅呼吁，而非正式投票的开始。治理投票流程将在审阅期结束后启动。

**关键变更****[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC验证采样优化**

此升级引入了一种新的PoC验证机制，通过为每个参与者分配固定的采样验证者集，将复杂度从O(N^2)降低至O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际PoC时间标准化PoC权重**

此升级通过实际PoC经过时间对参与者权重进行标准化，以减少区块时间漂移影响，并使权重结果与真实执行时长保持一致。
...
- **[PR #712](https://github.com/gonka-ai/gonka/pull/712)** 要求proto-go生成与提交代码匹配
...
- 在当前模型配置下，基于观测到的推理轨迹，B200 GPU的推理性能并未高于H100级GPU。
- 如果未来通过治理引入更大或更复杂的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与PoC v2中观察到的相同GPU类型间的相对性能差异。
...
根据当前评估，旨在处理此情况的机制可能无法正常运行。为防止在不确定或不安全的条件下完成验证者最终确认，网络在验证者选择前已停止运行。
...
下表显示了上一周期具有权重、在当前周期提交了 PoC nonce，但未能参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
Upgrade Proposal v0.2.8 的链上治理投票已结束。该提案已获得批准并在主网上成功执行。
...
!!! note 
    一旦网络过渡到单模型配置，未能满足上述两项条件的节点将失去 PoC v2 参与资格。PoC v2 权重分配的过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明了主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦观察到活跃 Host 中约 50% 的足够采用率，可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
- 身份验证绕过与重放攻击：将 epochId 绑定至签名，并验证授权是否针对正确的 epoch。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不被视作 PoC v2 的合格参与者。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间来为新的 PoC 要求准备其设置，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个周期内逐步进行更新。
...
如果获得批准，网络将暂时使用一个 `allowlist`，包含在先前纪元中未表现出非常规硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：

- 报告的硬件特征已与预定义的常见硬件配置模式进行比对，以识别偏差和不一致（确切的非常规配置字符串列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，以及
- 观测到的PoC权重低于使用同类硬件的其他参与者所表现的权重的150%。
...
每位活跃主机的参与对网络向前推进并恢复正常运行至关重要。请保持在线，一旦说明发布，请立即准备应用更新。
...
本升级中包含的预备性变更，为未来通过治理机制控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 开发者地址注册：在稳定期间暂停新开发者地址注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本升级包含的预备变更，使未来可通过治理机制控制参与者入网和推理执行，而无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
如果您的 `application.db` 占用大量空间，可以使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)的方法进行清理。
