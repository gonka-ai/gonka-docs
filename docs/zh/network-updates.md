...
!!! note "关于此页面"
    ...
    此页面的内容不保证完整。如需获取最新信息，包括治理投票的启动及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。

## 2026年6月27日

**Kimi-K2.6 启动：第311个周期的再次尝试**

`moonshotai/Kimi-K2.6` 在第311个周期再次尝试启动。希望提供 Kimi 服务的主机需在第310个周期内声明意向，并在新周期开始前准备好节点。一个守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第310个周期内、区块 **4,797,456** 之前（约6月28日，12:00 UTC）声明PoC意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311个周期开始前约500个区块的窗口期内将您的节点切换至 Kimi（PoC开始时间约为6月28日，12:47 UTC）。
...
**关键时间点*** 意向截止时间（第310周期）：区块 **4,797,456** —— 约6月28日，12:00 UTC。
* 第311周期开始：区块 **4,797,956** —— 约6月28日，12:47 UTC。

有关启动机制的更多详情：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****主要变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数的选择使得在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相当。此举并非偏向任何硬件——同样的权重在 MiniMax 上已可用。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
...
常规启动流程从第309周期开始。由于投票将在PoC开始前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于启动的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。此举并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员能够在配置资源前确认模型是否可行。
...
从第309周期开始（PoC开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是每个模型均满足启动资格条件：
...
提案必须在epoch 308结束前完成，以便两个模型都能启动进入epoch 309。由于每个epoch约22.8小时，标准的48小时投票期无法容纳在一个epoch内；而12小时的紧急投票期可以。创世守护者预计会支持该提案。

**主机所需操作**

1. **为了服务Kimi**：在投票结束**后立即**声明对`moonshotai/Kimi-K2.6`的意向。然后，在PoC 309中将您的MLNode切换至它。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间没有cPoC。

2. **为了服务GLM-5.2**：在部署窗口期间声明对`zai-org/GLM-5.2-FP8`的意向并配置您的MLNode。服务GLM-5.2是自愿的——未选择参与的主机不会受到任何惩罚。

3. **为了继续使用MiniMax**：继续服务`MiniMaxAI/MiniMax-M2.7`——无需任何操作。

4. **在截止日期前对提案79进行投票。** 紧急窗口很短。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
提案78的紧急投票已通过。这些变更自epoch 308起生效。

**当前生效内容**

1. 委托`initial_model_id`被设置为`MiniMaxAI/MiniMax-M2.7`——MiniMax为基础模型。
...
自epoch 308起，`MiniMaxAI/MiniMax-M2.7`是唯一活跃的PoC模型。Qwen3-235B和Kimi K2.6已不再活跃。

**主机所需操作*** 请确保您的MLNode正在服务`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，在切换前将无法获得本epoch的cPoC奖励。
* 计划再次服务Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好切换回Kimi——在epoch 308期间将进行一次恢复Kimi的投票。

**接下来的内容**

在epoch 308期间将进行一次紧急投票，合并两项变更，均于epoch 309生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其启动流程。
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可自愿选择参与，提前衡量模型需求，不服务的主机不会受到任何惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以实现快速重启；Qwen3-235B被退役**

`moonshotai/Kimi-K2.6`目前没有足够的投票支持用于PoC验证。因此，服务Kimi的参与者被移出组，Kimi无法进入下一个epoch。现在将其从活跃集合中移除并重新启动，是让它以最小停机时间回归的最快方式。

一项紧急提案现已上链，以便这些变更能在下一个epoch开始前生效。
链上提案在一次投票中执行以下操作：
...
**每个更改的目的*** **Kimi K2.6 — 已移除以快速恢复**。由于 Kimi 没有足够的投票支持 PoC 验证，将其移除并重新引导是恢复服务且停机时间最短的最快路径。下个纪元将进行投票以恢复 Kimi。
* **Qwen3-235B — 已退役（独立且无关的更改）**。退役较旧的 Qwen3-235B 是一项独立的、此前已请求的阵容调整。这与 Kimi 情况无关，仅因同样属于 PoC 阵容变更而一并列出。
* **MiniMax-M2.7 — 提升为基础模型**。

这些更改被合并为一次紧急投票，因为重置必须在纪元 307 结束前完成。每个纪元约 22.9 小时，标准的 48 小时投票周期无法容纳在一个纪元内；而 12 小时的紧急周期可以。Genesis 守护者应支持该提案。

**若获批准的效果**

从纪元 308（约 UTC 时间 6 月 25 日 17:25）开始，`MiniMaxAI/MiniMax-M2.7` 是唯一活跃的 PoC 模型。Qwen3-235B 和 Kimi K2.6 不再活跃。Kimi 的恢复将在下个纪元的后续投票中处理。

**主机所需操作**

1. **在 PoC 308 开始前将您的 MLNode 切换至 `MiniMaxAI/MiniMax-M2.7`**。所有主机——包括当前使用 Qwen 或 Kimi 的主机——都必须将 MLNode 切换至 MiniMax。在 PoC 开始前有约 500 个区块的窗口期可以安全切换，因为此时没有 cPoC。请现在预下载 FP8 权重（若尚未就位），以避免在纪元边界时遭遇 Hugging Face 速率限制。
2. **在截止日期前对提案 78 投票**。紧急窗口期很短。
3. 计划再次服务 Kimi 的主机仍应现在将 MLNode 切换至 MiniMax，但需准备好在 PoC 309 时切换回来——纪元 308 将进行恢复 Kimi 的投票。


**接下来的内容*** **恢复 Kimi K2.6** —— 在纪元 308 中进行后续投票，以重新添加 Kimi 并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加 GLM-5.2，**不设不参与惩罚**，因此主机可选择加入以提前测试模型需求，而不选择服务该模型的主机不会受到惩罚。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过 `inferenced` 进行：
...
1) 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以避免长时间运行的会话将所有已提供的推理保留在RAM或状态中。
2) 通过OpenTelemetry和Prometheus添加了内部devshard跟踪和指标。
3) 添加了使用Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor的join-stack可观测性。
4) 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个槽的总计数。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以避免长时间运行的会话将所有已提供的推理保留在RAM或状态中
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅影响您共识权重中源自 Kimi 的部分**；您的 Qwen 源权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降至约 25%，并设置链上法定人数为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约 ⅓ 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
**在第 278 个周期前对 `MiniMaxAI/MiniMax-M2.7` 的参与选择（处罚将从此时开始）**
...
所有主机需验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。此时，请先确认您正在运行当前版本，再采取进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约在 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 区块时（约 43 分钟前）→ 第一阶段 MiniMax PoC 在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。

**注意**

- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关安全删除 `.inference` 目录中旧备份的指导，请参阅 [文档。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个周期）。
...
社区成员目前正在审查第266轮期间发生的情况。一旦对根本原因有更多明确信息，将分享更新。
...
- 将创世守护者调整投票权减少至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，这使得其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
有意义的审阅贡献，包括重要评论、错误发现和安全问题，可能在下一轮升级周期中获得社区奖励。

本次仅为拉取请求的审阅邀请，不启动正式投票。治理投票流程将在审阅期结束后开始，很可能在明天。
...
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应交易商投诉。
...
- Devshard 现在支持更大规模部署的共享存储选项，可以使用 Postgres 作为主存储，SQLite 仍作为本地后备。
...
修复方案通过并行加载 devshards 并从快照恢复现有 devshard 会话移除了该阻塞点。
...
为使变更在下一轮PoC前生效，该提案已加速处理。投票将持续12小时。
...
该过程由网络各主机协同完成：基础设施已准备就绪，意向已提交，委托与拒绝已设置，部署已测试。
...
已提交PoCIntent的主机应在PoC于区块 `3874496` 开始前，至少将一个MLNode从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保留您的MLNode使用 `Qwen235B`，并于明天为下一轮提交意向。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
有关这些更改的更多详情请参见治理文档：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，之前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
- 确保冷钱包持有足够的金额（例如 100 GNK），以覆盖自动授予的费用限额支出。
...
此更新修复了 PoC 开始附近的一个边缘情况，该情况此前可能导致在特定条件下性能下降。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。提出此版本并非因其已完善，而是因为此类系统需要尽早暴露于真实网络环境中，某些问题仅靠本地测试难以发现。当前实现已设计为不会对矿工奖励产生负面影响。
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中可能同时出现大规模剪枝活动，造成不必要的运维压力。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
对于已经使用代理层的团队，这可能是简化架构、转而依赖原生行为的好机会。实践中，这将带来更清晰的集成模式和更易维护的系统。
...
现已提供新的vLLM / mlnode构建，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍有活跃请求而导致潜在权重下降的风险。
...
- 审查创世时定义的抵押参数。
- 在第180轮次前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)正确设置，否则从第180轮次起，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平仍相对较低时，存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
由于有效归一化窗口比先前名义模型长约5个区块，重新计算的每轮权重相应降低。

**观测到的GPU权重变化（第175轮 → 第176轮）**
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际的 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并使权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入了一个3000个区块的宽限期，在此期间无确认 PoC，并且对升级当期的错过率和无效率阈值放宽。
...
这意味着，在单个周期内，即使被惩罚，矿工的抵押金损失也不会超过 0.5%。而所需的抵押金仅占每日奖励的约 24%。

**警告：** 抵押参数将不受投票结果影响而生效。如果该提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。

投票结束后、第180周期之前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其奖励将被削减至五分之一。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获得批准，该提案将引入一项对 PoC 验证的重大优化（默认关闭），并实现基于实时的权重归一化，以提升网络公平性与可扩展性。
...
根据AMA期间的讨论以及社区成员提出的论点，建议初始抵押要求较低，惩罚也最小化。
...
警告：抵押品将无论提案投票结果如何均生效。如果本提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，则逐步将抵押品提高至代币经济白皮书中描述的水平（例如，每块H100约100 GNK），将支持网络的长期成功。
...
请在GitHub上审阅v0.2.10升级PR并提供反馈。有意义的审查贡献可能在下一次升级中提出奖励。
...
请主机尽快应用此补丁，以确保正确的PoC验证行为，并安全恢复区块生产。
```
# Download Binary
...
此升级根据实际 PoC 经过时间对 PoC 参与者权重进行标准化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在本次升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串而非数字编码，int64/uint64值现在也以字符串编码。
...
升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并已在区块 2451000 成功在主网上执行升级。此次升级实现了PoC v2的权重分配，并完成了向传统PoC机制的过渡。

**注意**

- 下一个PoC周期（从第158纪元过渡到第159纪元）至关重要。请确保在线，以便在需要时能够及时应用后续步骤或缓解措施。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的ML节点有资格进入下一个（159）纪元并参与PoC v2权重分配。运行其他模型的ML节点将不包含在即将到来的纪元的参与者集合中。
...
- 提供其他模型的ML节点不参与PoC v2权重分配。在支持的情况下，可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在因cPoC处罚而导致奖励减少或排除的情况下，未计入的部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
- 提供其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换到`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`模型
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置在Epoch 155起需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在Epoch 155后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估是否具备过渡至 PoC v2 的条件。一旦观察到约 50% 的活跃 Host 达成足够采用率，可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
- 身份验证绕过与重放：将 epochId 绑定至签名，并验证授权是否匹配正确的 epoch。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。在网络切换至单模型配置后，未满足两项条件的节点将不具备参与 PoC v2 的资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个周期内逐步执行更新。
...
1) 网络将运行一个 `allowlist`，其参与者在多个纪元中：
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2.294.222（约19天）。
...
与创世验证者增强功能相关的链上治理提案已发布，现开放投票。

近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意干扰或压力测试引起的。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强功能最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。
...
本升级中包含的准备工作，使未来可通过治理机制控制参与者准入和推理执行，而无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
如果您的 `application.db` 占用大量空间，您可以使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)提到的技术进行清理。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个bug导致不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
