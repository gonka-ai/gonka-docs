...
devshard 部分准备了 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提高内存利用率并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和网桥。为确保主网上线升级期间以太坊网桥的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和网桥镜像升级到 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，将继续正常工作。您需要理解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
**[仅 devshard 的升级 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放审核**

devshard 升级独立于主区块链进行更新。它们不需要通过 Cosmovisor 协调完整的节点升级，不影响主网行为，预计不会导致推理服务中断。如果通过治理流程批准，新版本的 devshard 将与现有的 v3 运行时并行运行。

**主要变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 也包含错误修复和安全补丁。

**行动项**

请审阅 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并对任何发现的问题、疑问、改进建议、边界情况或潜在漏洞留下评论。
...
`versiond` 已停止 v1 和 v2 `devshardd` 进程。所有网关流量必须使用 `/devshard/v3`。如果您的网关在此次变更后停止提供推理服务，说明您仍在通过已移除的路径路由流量 —— 请遵循 v3 迁移指南。
...
同一提案还从 `devshard_escrow_params` 中移除了 `approved_versions` 的 v1 和 v2 版本。一旦获批，`versiond` 将停止 v1 和 v2 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）继续运行。这消除了旧运行时容器长期存在的内存增长问题 —— 这正是 7 月 15 日事件中网络节点 OOM 的根源。

**这使 v3 切换最终确定。** 仍通过 v1/v2 前缀或经典 `/v1/devshard` 路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 进行投票 —— 紧急窗口期很短。
...
4. 经纪人 / devshard 创建者：如果您的任何网关流量仍使用 v1/v2 或 `/v1/devshard` —— 请在投票结束前切换到 `/devshard/v3`。
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（紧急，0.667 赞成阈值；出勤率重要，请尽快投票）。
...
`moonshotai/Kimi-K2.6`在第328–329轮失去了PoC验证多数票（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将Kimi从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的方式——与6月提案78的恢复路径相同。
...
2. 运行Kimi的主机：请保持Kimi设置就绪，并准备好在引导PoC时切换回您的MLNode。
3. 在为引导进行委托时：请勿委托给守护节点；请将委托分散到独立的Kimi主机上。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（PDT）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（紧急，0.667赞成阈值；出勤率重要，请尽快投票）。
* 引导意向截止时间和轮次时间详见引导说明。

## 2026年7月15日

**Kimi-K2.6事件（第328–329轮）：发生情况、恢复计划及委托指导变更**
...
* 其中一台服务器上运行Kimi的MLNode因提供商端问题而崩溃。大量Kimi委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器因内存不足而失去网络节点。

Kimi的验证投票一直接近2/3阈值，因此此次故障足以使该组低于多数。守护者投票权未介入：当前未赋予模型投票权重的守护者会被从决胜投票中过滤掉。这是一个已知问题，将在即将发布的v0.2.14升级中修复。

这种情况与第306–307轮事件类似。第329轮开始时的投票确切细节仍在确认中，本摘要可能存在小幅修正。

**恢复计划**

恢复Kimi最安全的方式是通过全新引导（约1.5轮）重新添加：

1. [今天] 一项紧急提案将在下一轮PoC之前移除Kimi，并立即重新添加。后续将发布包含提案ID、投票命令和截止日期的单独公告。
2. [明天] Kimi将再次经历标准引导流程：声明意向、部署窗口、PoC存储提交。意向截止前将发布详细说明。

**委托指导变更——请阅读**

先前的指导（包括2026年6月27日的引导公告）建议将守护节点作为委托目标。此建议现已被撤销：

* **不要将委托委托给守护节点。** 守护节点是PoC验证的后备机制。将委托集中于它们会将主要机制和后备机制绑定到同一硬件上——此次事件正是这种故障模式。目前正在讨论协议层面的限制。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个运行模型的独立主机上。如果您运行多个节点，请将委托给运行模型的您自己的节点。
* **如果您能直接运行Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**修复已在途中*** PoC验证中的守护投票（守护节点始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
...
该发布将`devshardd`二进制文件作为Gonka发布工件发布。`versiond`自动下载该二进制文件，验证sha256哈希值，并在现有`versiond`容器内启动额外的`devshardd`进程。

此类仅限devshard的运行时升级无需重启主网或手动主机操作。

**操作项**

已列入白名单的devshard创建者应在主网v0.2.14链升级前将推理流量切换至`/devshard/v3`。这将使他们在链升级运行期间仍能继续提供推理服务，而不依赖已弃用的经典API路径。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有 devshard 流量可以继续使用早期运行时前缀，直到经纪人将流量切换到 v3。此类升级期间无需重启主网或手动执行主机步骤。
...
devshard 部分准备 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提高 RAM 利用率并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布并部署 devshard v3 版本，然后再进行主网链升级。提前将推理流量切换至 `/devshard/v3` 的代理，可以在链升级运行期间继续提供推理服务。

**提议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查本提案。
2. 在主网链升级之前，提出并部署 devshard v3 版本。
...
devshard v3 版本将在主网链升级前提出并部署，以便代理能够在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作事项**

1. 现在 —— 审查 PR。阅读 GitHub 上的 PR #1267，并对任何发现、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `ap`i 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
...
上周，在 HackerOne 上报告了 PoC-v2 权重路径中的一个漏洞。参与者可以夸大其计算权重，从而影响区块奖励、共识投票权和治理投票权。

根据历史数据，此前未发现此类攻击。但在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。

针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已被使用，因此在升级前提供仅 API 的热修复程序——可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制的计算无法通过采样检查。
...
> **更新（2026年7月15日）：此建议已撤回——请勿向守护节点委托。** 守护节点是PoC验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取最新的委托指导。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一并恢复。此系数的选择使得在8xB300上运行Kimi时，其共识权重与主模型MiniMax大致相同。此举并非为了偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数的选择使得在8xB300上运行GLM-5.2时，其共识权重与MiniMax大致相同。此举并非为了偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的增益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使运营商可在配置前确认模型是否可行。
...
1. **要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何从冷密钥向热密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 计划再次提供Kimi的主机应保留Kimi的配置暂存，并准备好在PoC 309时将MLNode切换回Kimi——将在第308个纪元中进行一次恢复Kimi的投票。

**接下来的内容**

在第308个纪元中将进行一次紧急投票，合并两项更改，均将于第309个纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：
...
**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重新引导；Qwen3-235B被退役**

`moonshotai/Kimi-K2.6` 目前没有足够的投票支持以通过PoC验证。因此，提供Kimi的参与者被移出组别，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
**每项更改的目的*** **Kimi K2.6 —— 被移除以便快速恢复。** 由于Kimi没有足够的投票支持通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的路径。下一个纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B —— 被退役（独立、无关的更改）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi的情况无关，仅因同为PoC阵容调整而一并包含在此处。
* **MiniMax-M2.7 —— 升级为基础模型。**

这些更改被合并为一次紧急投票，因为重置必须在第307个纪元结束前完成。每个纪元约22.9小时，标准的48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持此提案。

**若获通过的效果**

自第308个纪元开始（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如尚未暂存），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回Kimi——将在第308个纪元中进行一次恢复Kimi的投票。


**接下来的内容*** **恢复 Kimi K2.6** — 在第308个周期进行的后续投票，以重新添加Kimi并重启其引导过程。
...
**截止时间*** 提案78投票截止：**2026年6月25日 15:39:53 UTC**（紧急）。
...
提案通过后，v2 现可与现有的 v1 devshard 运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有的 v1 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

该发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于此类仅 devshard 的运行时升级，无需重启节点容器或手动执行主机操作。

**关键变更**

1) 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在 RAM 或状态中。
...
4) 将每个推理的验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每个插槽的总数。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在 RAM 或状态中
...
5. 在周期变更时修剪旧周期存储，将 SQLite/Postgres 模式设置移出热路径，并为每个进程仅选择一个存储后端
...
新进程通过 `/devshard/v2` 前缀提供服务。现有的 v1 devshard 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。此类升级期间无需重启节点容器或手动执行主机操作。
...
**[仅 devshard 升级的 PR 现已开放审核](https://github.com/gonka-ai/gonka/pull/1289)。**

这是首次仅 devshard 升级，因此流程与标准链升级不同。Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。

如果通过治理流程获得批准，新的 devshard 版本将与现有 v1 运行时并行运行。

请直接审核该 PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审核贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期获得社区奖励。

本次仅为请求审核 PR。尚未开始正式投票。治理投票流程将在审核期结束后启动。

## 2026年5月28日

**MiniMax-M2.7 现已在 Gonka 网络上线**

v0.2.13 中宣布的引导阶段已完成。自链纪元 278 起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax 组中获得的 PoC 权重现在以校准系数 0.3024 转换为共识权重。

MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行其他操作——现有设置将继续生效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获**批准**，升级已于主网 `4267300` 块成功执行。

## 2026年5月25日

**升级 v0.2.13：提前下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13 升级提案（提案 ID (https://github.com/gonka-ai/gonka/pull/1143)54）已通过链上治理，升级现已安排。
...
对 [v0.2.13 升级提案](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已**批准**。

升级将在主网**区块高度 4267300**（≈ **2026年5月26日周二 14:42 UTC** / **07:42 PDT**）自动执行。

**提醒**

1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT 代币元数据和 CW20 `wrapped_token` 代码 ID `105` 均通过升级处理程序注册，因此桥接将在升级高度在主网激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在提前下载约 230 GB 的 FP8 权重。在引导窗口期间，Hugging Face 的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 的截止时间为**链纪元 `278`**。未采取任何操作的主机将从纪元 278 起，对其全部权重施加每纪元 15% 的惩罚。
4. 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。确保 `.inference/data` 有足够的空闲空间用于 cosmovisor 状态备份；如果 `application.db` 较大，请在升级前应用 cosmovisor 备份指南中的[清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基线。此调整**仅适用于您共识权重中的 Kimi 部分**；您的 Qwen 部分权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，现已提交多个更新。

- 包括：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专节）。
- 该提案将升级后的宽限期延长至 3000 个区块，以便主机在新快照逻辑稳定期间不会受到惩罚。
...
对于每个经过治理批准的模型，多模型PoC要求每个主机明确选择参与方式（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 之后不采取任何操作将导致处罚。在此阶段，您应提前决定首选选项，以便在提案通过且升级成功应用于主网时能够迅速行动。
...
- 运营商时间线：投票于5月22日22:12 UTC结束 → 升级高度约为5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax引导快照在 start_poc − 500 块处（约43分钟前）→ 升级后下一个周期边界开始第一阶段MiniMax PoC → MiniMax惩罚在链周期278执行。
...
- 如果 `application.db` 占用大量磁盘空间，可应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获批准，此跳过属预期行为，并非故障；新快照逻辑将在下一个周期开始。
- 若获批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储相同的环境变量）。本地SQLite仍将为默认选项，并自动清理（保留最近3个周期）。
...
在周期 #267 中，主机成功回归，PoC验证已顺利完成。

**推理可用性**

网络将不再接受使用受影响的非标准语义的请求。

在相关保护措施应用期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。
...
看起来，PoC投票未能收集到本周期所需的票数。确切原因尚未确认，社区正在积极调查此事。
...
- 包括：对Kimi（`0.78`）的重新校准权重、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及多项PoC/奖励修复和以太坊桥接主网激活。
...
- 修复新模型启动期间确认PoC奖励的核算。
- 在本次升级周期剩余时间内禁用确认PoC，以便从下一个周期开始干净地启动新的快照逻辑。
- 当参与者再次活跃时，重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问。
- 将创世守护者调整后的投票权降低至约25%，并将链上治理门槛设置为 `0.25`。在守护者不参与投票的情况下，这使得剩余75%投票权的有效门槛为1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再采取进一步操作。
...
- 该提案将有意跳过从升级高度到升级纪元结束（3000区块宽限期）的确认 PoC。如果获得批准，此跳过属预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作，该升级可能包括以太坊端合约激活，所有主机均需验证其桥接容器是否已部署、运行最新版本并正确同步。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推断就可能再次使其失效。现在在重新激活和即将晋升时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授予中未包含`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应经销商投诉。
...
在上一个纪元中发现Kimi-K2.6某些响应解析存在一个小bug。
...
传输代理 `node1`、`node2` 和 `node3` 已禁用。所有主网推理现在均通过 `node4` 路由，其采用新的 `devshard` 计费方式。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，权重极低。请保留您的 MLNode 使用 `Qwen235B`，并于明天提交下一周期的意向。
...
- **协议强化**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易方共识，使遗留验证者罚没与所需抵押语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
每个 DAPI 会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不支持的模型，整个节点将被拒绝，主机将下线。
...
[升级提案](https://github.com/gonka-ai/gonka/pull/948) 已发布至链上，现开放投票，用于下一个链上软件版本 v0.2.12。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将随机采样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（完整 256 位熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者 slashing 与所需抵押品语义对齐、确保 devshard 保证金基金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 一旦新模型获得治理批准，部署、委托或明确拒绝每个治理批准的模型（包含的模型将在升级后3个周期内激活）
...
下一代链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获得批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每项推理交易处理推理操作限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅负责会话创建和最终结算。

这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳多达100倍的推理数量，具体取决于工作负载和网络条件。  ￼
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时开始剪枝活动，从而造成不必要的运营压力。
...
对于对本 PR 审查有实质性贡献者，可在下一次升级中提出奖励。
...
目前，通过每次推理的链上交易处理推理会话会限制吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅处理会话创建和最终结算。

这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
API 二进制文件的新版本已发布。该版本更新了连接超时处理，并在 PoC 验证流水线中引入了额外的检查。

1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格 5 分钟超时，但某些请求可能需要更长时间。新版本将恢复此值，而非强制执行严格限制。
2. 之前的请求重试系统在因处理超时（而非 TLS 超时）导致失败时仍会重试推理。
服务器端对长请求的重试通常无效，因为会导致相同的超时场景，同时客户端可能收到不一致的输出。新版本在这些情况下不再重试推理。
3. 当前被保留且未参与 PoC 生成的 MLNode 仍被用于 PoC 验证，这可能导致推理遗漏。新版本将排除此类节点参与 PoC 验证。
4. PoC 验证流水线中增加了额外的安全措施。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 开始时中断正在进行的推理请求，以降低因 PoC 开始时仍有请求活跃而导致的潜在权重下降风险。
...
- 确保 [抵押](https://gonka.ai/host/collateral/) 设置正确，否则从第180轮次开始，PoC 奖励将减少 5 倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若无替代方案获批，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
在最近升级后，由于 PoC 持续时间归一化，节点权重已调整。
为使 PoC 权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选定。如实施所示，有效的 PoC 参考窗口比先前名义假设长约5个区块。
...
由于有效归一化窗口比先前名义模型长约5个区块，因此每轮次重新计算的权重成比例降低。
...
**宽限期：** 升级后引入了一个3000个区块的宽限期，期间不进行确认 PoC，并且对升级时期的错过率和无效率阈值放宽。
...
这意味着，在单个纪元内，即使被惩罚，矿工最多只会损失其抵押品的 0.5%。所需抵押品仅占每日奖励的约 24%。

**警告：** 抵押参数将无视投票结果生效。如果该提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。

投票结束后、第180纪元前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180纪元起，其奖励将被削减至原来的五分之一。
...
此升级通过实际PoC耗时标准化参与者权重，以减少区块时间漂移影响，并使权重结果与真实执行时长保持一致。

**启用Qwen235B工具**

此升级为`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`添加了工具调用参数（`--enable-auto-tool-choice`、`--tool-call-parser hermes`），并设置验证阈值`0.958`。
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并为升级所在纪元放宽了错过率和无效率阈值。
...
若提案获得批准并执行升级，必须重启ML Node容器以触发模型重新部署。运行：
```
docker restart join-mlnode-1
```
...
正如AMA期间讨论并根据社区成员提出的论点，建议初始抵押要求较低，并且 slashing 极小。

将提交给社区投票的参数：

- 每1单位算力需0.032 GNK（约每H100为10 GNK）
- 错过率或封禁的 slashing 为0.01%
- 无效推理的 slashing 为0.5%
...
任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下实施。如果证明稳定性良好，则逐步将抵押品提高至代币经济白皮书中描述的水平（例如，每块H100约100 GNK），将支持网络的长期成功。
...
请在GitHub上审阅v0.2.10升级PR并提供反馈。有意义的审查贡献可能会在下一次升级中获得奖励。

[https://github.com/gonka-ai/gonka/pull/695](https://github.com/gonka-ai/gonka/pull/695)

## 2026年2月13日

如果您的节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止运行。这是因为节点运行的是与网络不再兼容的旧版二进制文件。为恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
下一次链上软件升级 v0.2.10 的[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 已开放供审查。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约2天。
...
此升级根据实际PoC经过时间标准化PoC参与者的权重，以减少区块时间漂移效应，并使权重结果与真实执行持续时间保持一致。
...
在更新的权重分配生效后，追踪器（仪表板）维护者可能希望审查其第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。
...
本指南说明主机应如何根据v0.2.8模型可用性变化和即将推出的PoC v2更新来更新其ML节点。从第155个周期开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML 节点版本，可评估向 PoC v2 过渡的准备情况。一旦观察到约 50% 的活跃主机采用该配置，可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML 节点不足以参与 PoC v2。一旦网络切换至单模型配置，未同时满足上述两项条件的节点将不具备参与 PoC v2 的资格。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。
...
`Allowlist`过期时间：自动在区块 2443558 到达时过期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被错误过滤。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 的参与者注册截止时间对齐。
...
该提案已获批准。此提案定义了一个临时 `allowlist`，反映那些在近期多个周期中行为保持一致的参与者。
...
2) 之前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直到区块 2,222,222 结束稳定窗口。
...
在成功采用解决PoC相关共识故障的补丁后，新的治理投票目前正在进行中。
...
若获批准，网络将临时使用一个 `allowlist`，包含那些在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：
...
之前持续偏离这些模式的参与者在稳定窗口结束于区块2222222之前不属于 `allowlist`。
...
建议主机保持在线并监控更新，补丁发布后可能需要采取后续操作。

## 2026年1月8日

**v0.2.7升级提案：主网上线的创世验证者增强**

链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强方案已获批准，并成功部署于主网。
...
本升级中包含的预备性变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是故意试图破坏或压垮系统所致。本提案旨在通过一系列临时措施增强网络在高负载和不利条件下的弹性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并激活一些额外的保护措施。

**关键变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用、有时间限制的防御机制，拟重新激活。
- 在网络增长期间提供共识保护。在其先前运行期间：
...
本次升级正式将此前通过手动 API 更新分发且已在网络中使用的若干关键修复纳入协议。这些修复包括：
...
- 主机级注册：在区块 2,222,222（约两周后）之前，暂停新主机注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约19天）。
...
如果您的 `application.db` 占用大量空间，可以使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)提到的技术进行清理。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个错误：即使推理请求已被处理，但以不支持的格式发出的请求未被标记为已完成。以下更新将解决此行为。
