...
!!! note "关于本页面"
    ...
    此页面不保证内容完整。如需最新信息，包括治理投票的启动及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。

## 2026年9月18日

**提案通过：批准 devshard v5 和奖励发放**

提案 ID 105 的链上治理投票已结束。

该提案以 123,754 票赞成、反对票数较少获得通过。

**变更内容**

DevShard v5 已添加到链上批准的 devshard 版本列表中：
...
主要协议新增功能是 height-sync，它为 DevShard 会话提供了一个已签名、可重放的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立了协议级逻辑时间。此版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机所需操作**

无需操作。`versiond` 将下载新的运行时，验证其 sha256，并与已运行的版本并行启动。`v3`、`v4` 和 `v4.1` 仍为已批准版本，因此现有会话和路由前缀不受影响。这是一个运行时版本，而非网关发布：任何网关变更将单独公告并附带独立指南。

**奖励发放**

该提案还从社区销售合约向多个开发周期中作出贡献的核心贡献者发放 91,300 USDT 奖励：@a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。所有奖励均已链上执行。

技术范围、个人贡献、PR 引用及完整奖励明细：[PR 1584](https://github.com/gonka-ai/gonka/pull/1584)

## 2026年9月16日

**devshard v5 提案重新提交至治理（提案 104 的重提）**

提案 ID 105 将 `v5` 添加到批准的 devshard 版本列表中，并从社区销售合约发放 91,300 USDT 的贡献者奖励。

**devshard v5**

这仅是 devshard 的变更。Cosmovisor 不参与，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，并启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能是 height-sync，它为 devshard 会话提供了一个已签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级 QoS 建立了协议级逻辑时间。此版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。

该版本已由核心测试团队和外部测试实验室进行测试。
...
完整说明：[update-v0.2.15-devshard-v5/README.md](https://github.com/gonka-ai/gonka/blob/devshard-0.2.15-v5/proposals/governance-artifacts/update-v0.2.15-devshard-v5/README.md) (sha256 `7bba17188e4c3d15476f2c6330281b3908fc124e9eb26950acd6f9f927d89dc4`)

**奖励发放**

受赠者为在多个开发周期和版本中活跃的 Gonka 核心贡献者：@akup / @a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。
...
投票将于 **2026年9月18日16:47 UTC** 结束。该提案通过需满足：权重25%的法定人数、多数赞成票，且反对票（含否决）少于33.4%。

请投票。之前的尝试因参与率不足而失败，无人反对，因此参与是唯一阻碍其通过的因素。
...
```
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅有关将治理投票权限从冷钱包授予热钱包的指南。

## 提案被拒绝：批准 devshard v5 及奖励发放

提案 ID 为 104 的链上治理投票已结束。该提案因 `failed_reason`：`proposal did not get enough votes to pass` 被拒绝。

最终投票结果：赞成 25291，反对 0，否决 0，弃权 0。

无人投反对票。参与率仅为网络权重的约6%，低于25%的法定人数，因此提案仅因参与不足而失败。

## 2026年9月16日

**GLM-5.3-Flash 已上线**

自第395个纪元起，`zai-org/GLM-5.3-Flash` 成为活跃模型组，并在主网上提供推理服务。

如果您希望托管它，欢迎加入。在部署窗口期间声明对该模型的意向并配置您的MLNode，该窗口在下一次PoC开始前500个区块开启。

**请勿忽略您的参与选择**

GLM-5.3-Flash 的不参与惩罚自第394个纪元起生效。对于每个经治理批准的模型，您必须明确选择：自行提供、委托或拒绝。不采取任何操作将导致每纪元扣除您全部权重的15%，并持续至您做出选择为止。
...
**devshard v5 提案已进入治理流程**

提案ID 104 包含两项内容：将 `v5` 添加到已批准的 devshard 版本列表中，并从社区销售合约分配91,300 USDT作为贡献者奖励。

**Devshard v5**

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证后启动运行时。

v5 与现有的 v3、v4 和 v4.1 运行时并行运行。提案通过本身不会转移任何流量，因此经纪人可随后逐步迁移其网关。
...
该提案还发放91,300 USDT的贡献者奖励，分配给七名受赠方：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成票，且反对票（含否决）少于33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅有关将冷钱包的治理投票权限授予热钱包的指南。

## 2026年9月12日

**提案通过：补偿Daniil和David Liberman参加All-In峰会2026的费用**

提案ID为103的链上治理投票已结束。

提案已获批准。从社区金库拨款15,500 USDT，用于补偿Daniil和David Liberman（Gonka协议的联合创建者）参加All-In峰会2026的两张参会通行证费用。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人7,500 USDT，门票已支付。本次补偿涵盖两张门票及提取资金的交易费用。飞行、住宿及其他费用未在本提案中申请社区金库支付。

最终投票结果：赞成184762票，反对0票，反对（含否决）0票，弃权0票。

主办方无需采取任何操作。

## 2026年9月11日

**提案通过：批准Quant Mesh Limited为账本集成合作方并拨付初始账本集成款项**

提案ID为102的链上治理投票已结束。

提案已获批准。Quant Mesh Limited被正式批准为Gonka与Ledger集成的法律合同合作方，并从社区金库拨款350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费75,000 USDT以及350 USDT处理费用。授权与资金拨付严格限于完整提案中描述的Ledger集成。

转账已在链上执行。

最终投票结果：赞成146903票，反对0票，反对（含否决）0票，弃权5568票。
...
提案已获批准。`zai-org/GLM-5.3-Flash`现已成为经治理批准的模型并纳入PoC模型集合，而`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`已从`poc_params.models`中移除。这两个ID仍保留在`GET /v1/governance/models`（推理目录）中，但不再作为PoC模型。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：它们的 PoC 参数和权重保持不变。

该模型及其参数由 kaitaku.ai 团队提出，由 vbgd0 独立验证，并共同提出。测量结果及阈值设定的推理见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起 `penalty_start_epoch` 生效，因此未明确选择的宿主将面临不参与惩罚。如果您计划提供该模型，尽早声明意向将为团队形成提供最佳机会。宿主指南：[GLM-5.3-Flash 启动](./host/glm-bootstrap.md)。

最终投票结果：赞成 309308，反对 0，带否决的反对 0，弃权 3168。
...
该提案已获批准，`v4.1` 已加入 approved 的 devshard 版本列表。`versiond` 将下载二进制文件，并将其 SHA-256 与链上存储的值进行验证，然后启动运行时。无需手动操作宿主。

v4.1 现在与现有的 v3 和 v4 运行时并行运行，因此当前流量继续被服务，经纪人可以逐步迁移其网关。
...
最终统计：赞成 312476，反对 0，带否决权反对 0，弃权 0。

感谢所有投票的人。

## 2026年9月9日

**提案101：添加GLM-5.3-Flash，从PoC模型中移除Kimi-K2.6和GLM-5.2-FP8**

提案 ID 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加到 PoC 模型集合中。同时将其从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：它们的PoC参数和权重保持不变。所有其他链上参数均保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，现共同提出。发布输入、测量数据及阈值设定的推理依据详见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票率、多数赞成，且反对票（带否决权）少于 33.4%。
...
投出您的票（`yes`，`no`，`abstain`，`no_with_veto`）：
...
如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅有关从冷密钥向温密钥授予治理投票权限的指南。

## 2026年9月9日

**devshard v4.1 运行时提案已进入治理**

提案 ID 100 将 `v4.1` 添加到已批准的 devshard 版本列表中。

这仅是 devshard 相关变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储值一致，并启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。通过提案本身不会迁移任何流量，因此经纪人可在之后逐步迁移其网关。
...
投票将于 **2026年9月10日 22:36 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成票，且反对票（含否决）少于 33.4%。
...
提案已获批准。将从社区金库拨款100,000 USDT，用于支持Gonka参与2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金将覆盖黄金赞助包及相关后勤开支：差旅、住宿、本地协调、嘉宾接待、活动运营及支持材料。
...
重启时，`v4.0.1`会从nonce 1恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond`随后在60秒就绪超时后终止子进程，导致客户端出现HTTP 502错误。

`v4.0.2`加载最新快照并仅恢复其后的差异。此版本与之前的v4版本完全兼容。

发布版本：[devshard/v4.0.2](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v4.0.2)

**更新说明**

请一次更新一个节点，并在继续下一个节点前确认当前节点已恢复正常。
...
提案已获批准。将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提升至0.246。其他所有模型和链参数保持不变。这是提案97的重新提交。链上提案97最终以`PROPOSAL_STATUS_REJECTED`结束，原因是`failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终投票结果：赞成15452，弃权3093，反对0，否决0。

原始的0.214系数源于被高估的nonce/min数值。测量修正详见[gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需操作。

感谢所有参与投票的用户。

## 2026年8月25日

**仅devshard升级的PR现已开放评审**

Devshard升级独立于主区块链更新devshard运行时。它们无需通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会造成推理服务的停机。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**关键变更**

1. v5的主要目标是高度同步：每个主机和序列器在托管日志中维护一个已签名、可重放的主网高度视图，即使在无推理流量时也如此。借助该时钟，devshard现在可以确凿地判断cPoC何时运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height`/`observed_block_hash`标记已存在于所有先前包含时间戳字段的消息中。后续版本可将诸如`confirmed_at`和`started_at`等墙钟字段替换为高度标记。这解决了未来时间攻击问题，并为在协议层面处理流超时开辟了道路。本次发布包含这些标记，但尚未将超时和封印决策切换至它们。

3. 新增了通过所有插槽往返的心跳机制，因此安静的会话仍能定期同步高度，而繁忙的会话则可通过常规推理标记实现相同节奏。

**行动项**

请审查 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584) 并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一次升级周期中获得社区奖励。
...
**提案通过：稳定小样本中周期内无效SPRT**

提案编号 96 的链上治理投票已结束。

该提案已获批准。它调整了在周期初期数据不足时无效推断检查（SPRT）的行为，避免因样本过小而误触发。主机无需采取任何操作。

感谢所有参与投票的人。

## 2026年8月20日

**提案96开放投票：稳定小样本中周期内无效SPRT**

一个新的链上治理提案（提案编号 96）已开放投票。

**作用**

在周期初期，数据尚不足，因此无效推断检查（SPRT）可能在样本量足够前仅因少量无效结果而触发。本提案使其更稳定：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于此阈值的诚实参与者不会被惩罚。真实违规者仍会触发它（约 360 个样本时为 18%，约 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效结果，且在 500 个样本时观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期结束时的统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如能投票，请参与。若未能及时达到加速阈值，提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。若转为常规提案，您需重新投出您的投票。
...
作为协议开发中的一个重要里程碑，该链已准备好逐步启用 devshard 结算，以提升网络的安全性和可靠性。

一个稳定的网关版本目前已在单个网关上运行，并将用于第一阶段的结算。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何惩罚或削减，但仍应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。没有明显问题的主机无需采取任何操作。
...
**2) 在此24小时窗口之后的第一个周期。** 将在当前网关上手动启动结算，首先从较小的模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其数据在负载下目前可靠性较低。

**3）接下来1-3天。** 将监控结算结果

**4) 一旦此阶段稳定。** 其他经纪人可以部署稳定的网关版本，并以受控模式开始结算。

**5）在所有经纪人成功验证后。** 结算覆盖率将逐步向100%推进。

此 rollout 有意逐步进行，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash现已在Gonka网络上激活**

在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链周期360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 加入 MiniMax-M2.7 和 Kimi K2.6，成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后由 [提案98](#august-29-2026) 提高至 0.246）转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需再做任何操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每周期惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。

## 2026年8月14日

**MLNode 3.0.16部署**

如果您的所有节点均已升级至新版本（3.0.16），则无需操作。

如果仍有节点运行旧版 MLNode（即正在服务 Kimi 或 MiniMax 的节点，因为已部署 DeepSeek 的节点均已使用 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少量节点，确认其稳定性后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。
...
默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍在使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。它同样是 MLNode 3.0.16，只是为 CUDA 12.9 构建。两个标签均在同一个 compose 文件中。
...
将正在运行的社区销售合约和所有封装代币实例迁移至新存储的 CosmWasm 代码，并为未来的实例化注册该封装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。
...
投出您的选票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要v0.2.12或更高版本的`inferenced`。
```
export NODE_URL=https://node3.gonka.ai/
...
**DeepSeek V4 Flash 引导：意向阈值已满足——请在 epoch 360 快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 引导快照——在此前请保持委托分散，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始前（区块 **5,552,115**）的约 500 区块部署窗口内部署 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。

**继续运行其他模型的主机**

在区块 **5,551,615** 之前，提交委托（有助于引导可达性）或拒绝。从 epoch **360** 开始，DeepSeek `penalty_start_epoch` 将激活——未明确选择的主机将面临不参与惩罚。
...
**关键时间点*** 第360轮快照（`start_poc − deploy_window`）：区块**5,551,615**——约2026年8月14日 01:24 UTC
* 第360轮PoC启动：区块**5,552,115**——约2026年8月14日 02:09 UTC
* DeepSeek不参与惩罚从第360轮开始
...
DeepSeek 已通过投票进入引导阶段——如果您计划提供服务，请验证您的设置并立即提交意向。
...
为了在 B* 卡上获得最佳性能，您可以使用从 fp8 + fp4 重新打包为 fp8 + nvfp4 的模型：
...
建议逐步将节点切换到新 API，而非一次性全 fleet 更新。
...
```
该技能将部署模型，测量 PoC 吞吐量，并将输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则表示准备就绪。

如果您打算提供 DeepSeek，请按上述步骤设置。若不打算提供，请勿忘记委托，以免遭受不参与惩罚。

提醒：明天将发布带有更好日志的另一个 MLNode 版本，敬请关注后续更新。

## 2026 年 8 月 12 日

**DeepSeek V4 Flash 启动（第 359 个周期）**

提案 94 已通过——DeepSeek V4 Flash 已获批准。计划提供该模型的主机应立即声明意向，以便模型能在第 359 个周期启动。

[完整系数表](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)

**效果**

从第 359 个周期开始（PoC 开始时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），前提是模型满足启动资格：

- `deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的 PoC 模型。
- `MiniMaxAI/MiniMax-M2.7` 保持为基础模型。
- `moonshotai/Kimi-K2.6` 保持可用且不变。

**主机所需操作**

1. 声明意向。鉴于提案 94 已通过，且在引导快照区块 5,536,224（约 8 月 13 日 02:40 UTC / 8 月 12 日 19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 在PoC 359开始前的约500个区块安全窗口内切换您的MLNode，将其配置为DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate配置），该窗口对应区块5,536,724（约8月13日 03:24 UTC / 8月12日 20:24 PDT）。此窗口内切换是安全的，因为其中没有cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在引导前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留您当前的模型。在epoch 360之前无需操作。从epoch 360开始，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的未参与罚金。

区块高度是精确的；时间是估算值，将在接近周期时最终确定。[更多关于 DeepSeek 启动的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。

## 2026 年 8 月 10 日

**治理投票（提案 94）：添加 DeepSeek V4 Flash**

将 `deepseek-ai/DeepSeek-V4-Flash-0731` 加入PoC模型列表的提案现已上链并开放投票。

**提案内容**

本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。不修改任何现有参数或模型，唯一变更就是新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在vLLM 0.25.1上使用release-candidate节点配置的MLNode（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
- 候选链参数：PoC `weight_scale_factor = 0.214`，推理 `validation_threshold = 0.90`（处理过的logprobs），`penalty_start_epoch = 360`

DeepSeek 的 0.214 系数为在 B300 上托管 DeepSeek 提供了轻微优势。对于其他硬件，产生最高 PoC 权重的模型保持不变。

与运行 MiniMax M2.7 的 8xH100 集群相比：

- 8xH200 最优可产生 1.46 倍权重运行 MiniMax M2.7
- 8xB200 最优可产生 2.96 倍权重运行 Kimi K2.6
- 8xB300 最优可产生 3.37 倍权重运行 DeepSeek V4 Flash

根据模型使用情况，社区日后可决定逐步用 DeepSeek 替代 Kimi 或 MiniMax（例如，提高系数以激励所有 B 系列 GPU 转向 DeepSeek）。

完整系数表：[Google 表格](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)

**引导工作原理**

模型通过标准引导流程进入，主机可在投入硬件前评估其可行性：

1. 声明意向：在 `start_poc - deploy_window` 快照前提交 `MsgDeclarePoCIntent`。
2. 预资格快照：链对意向和委托进行快照，并发出建议性预资格事件。
3. 部署窗口：声明意向的主机配置其MLNode。
4. PoC启动：成员资格由在PoC启动时提交PoC存储提交的主机决定，而非仅由声明意向的主机决定。

最终的 MLNode 版本和设置说明将在投票结束后发布。

更多关于引导工作原理：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)

**若获批准的影响**

在符合引导资格的前提下，从epoch 359（PoC启动时间约为2026年8月13日 03:24 UTC / 8月12日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与罚金从epoch `360`（`penalty_start_epoch = 360`）开始生效。

**主机所需操作**

1. 若要提供该模型：投票结束后声明意向，然后配置您的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第 360 周期前无需操作；从第 360 周期起，若您未提供服务，请提交 `PoCDelegation` 或 `PoCRefusal` 以支持 DeepSeek。
3. 在截止日期前对提案 94 进行投票。
...
投票持续48小时；投票于2026年8月12日16:05 UTC / 2026年8月12日09:05 PDT结束。

## 2026 年 8 月 3 日

**升级已执行：devshard 运行时 v4.0.1 已上线**

针对 `v0.2.15-devshard-v4.0.1` 运行时升级提案（提案编号 93）的链上治理投票已结束。

该提案已获批准，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换二进制文件——无需重启主网或手动操作主机。

**API 二进制更新：`v0.2.15-post3`**

新的 API 二进制文件 `v0.2.15-post3` 已发布。

此版本提高了 PoC 种子的可靠性，并修复了若干 API 和部署问题。

**变更内容**

- PoC 种子稳定性。在周期边界延迟后恢复遗漏的种子提交，并在临时故障（如签名者不可用）后重试本地种子恢复，以防止验证者被清零。
- API。
    - `/v1/versions` 现在返回增强的 DAPI 版本信息，包括 ML 节点详情
    - 默认的 Node Manager gRPC 端口现在为 9400，确保与自定义配置兼容
...
这只是一个性能和资源管理补丁，除此之外无其他改动。协议、推理、验证和经济行为均保持不变。变更包括：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。

**升级计划**

devshard 运行时通过链上参数提案进行升级，而非完整的链软件升级。若获得批准，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动操作主机。
...
**升级已执行：v0.2.15 现已在主网上线**

针对升级提案 v0.2.15（提案编号 92）的链上治理投票已结束。

该提案已获批准，并于主网区块 5316315 成功执行。该提案同时批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
2. 一个 **v0.2.15 主网软件升级**，移除大量过时代码、遗留交易处理路径及相关组件，这些组件已不再符合当前架构需求。

尽管这两项变更以单次投票形式提交，但它们在操作上是独立的：

- **devshard v4 运行时** 通过 devshard 版本管理机制由 `versiond` 分发并启动。
- **主网 v0.2.15 二进制文件** 通过标准链上软件升级流程和 Cosmovisor 激活。

**Devshard v4 变更**

v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级期间，多台机器逐台替换版本。这是系列性 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个面向多实例高可用的版本：在共享 Postgres 后端，通过 versiond-router 部署 N 个 versioned/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，提升了系统的整体稳定性和可维护性，并通过移除过时行为、将交易处理统一到当前支持的架构上，使未来的开发和升级更加便捷。
...
如果获得批准，二进制版本将通过链上升级提案进行更新。有关升级流程的更多信息，请参阅 [/docs/upgrades.md.](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.15/docs/upgrades.md)。

**Devshard v4**

该提案还将在 `DevshardEscrowParams.approved_versions` 中注册 devshard v4。
...
4. 通过相应的 devshard 路由配置提供该运行时。

Devshard v4 将与现有的 v3 运行时并行运行。在经纪人和主机操作员准备和验证其 v4 部署期间，现有 v3 流量可继续运行。

v4 的批准本身并不要求所有流量立即切换。在运行时启动并验证后，可逐步进行迁移。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)以了解如何将治理投票权限从冷密钥授予热密钥。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间（主网上的Cosmovisor备份`application.db`通常为数十GB，需提前确认）。有关如何安全删除`.inference`目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认设置，并自动清理（保留最近3个周期）。
...
- 投票结束：2026 年 7 月 23 日 00:02 UTC
- 升级高度：5195700
- 预计升级时间：2026年7月23日 约03:45 UTC

鼓励主机查看[GitHub](https://github.com/gonka-ai/gonka/pull/1267)上的提案并参与投票。
...
主网链/API工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（在主网上禁用`/v1/chat/completions`计费并从API二进制文件中移除嵌入的`/v1/devshard`）、奖励接收方路由以及升级时的安全修复。

devshard 部分准备了 v3 运行时，以便经纪人可以在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率，并实现 SQLite 与 Postgres 存储之间的安全切换。

如需更多详情，请参见拉取请求：[https://github.com/gonka-ai/gonka/pull/1267](https://github.com/gonka-ai/gonka/pull/1267)

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需手动更新其 `api` 或 `node` 容器以完成升级。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参考[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
投出您的投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` v0.2.12 或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像升级至 0.2.14-post3。如果您的 `api` 二进制文件已更新，您只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护者 — 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放评审**

Devshard升级独立于主区块链更新其运行时。它们不需要通过Cosmovisor协调完整的节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程批准，新版本的devshard将与现有的v3运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级时实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名但 sha256 不同的新二进制文件时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一轮升级周期的社区奖励。

本次仅为 PR 评审邀请，不启动正式投票。

## 2026年7月16日

**提案88已通过：Kimi-K2.6重新注册，devshard v1/v2被移除**

紧急提案 88 已链上通过。

**Kimi-K2.6 启动（周期 331）**

`moonshotai/Kimi-K2.6` 已重新列入治理模型列表，并进入标准启动流程。

* 在区块 **5,105,276**（2026年7月17日，约12:05 UTC）前声明意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在周期 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的 MLNode 切换为 Kimi。
...
这是 7 月 15 日宣布的 Kimi 恢复计划的第二步。提案 87 将 `moonshotai/Kimi-K2.6` 从活跃集合中移除；提案 88 将其重新注册到治理模型列表中，使其从周期 331 开始进入标准启动流程。

同一提案从 `devshard_escrow_params` 中移除 `approved_versions` 的 v1 和 v2。一旦批准，`versiond` 将停止 v1 和 v2 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。这消除了旧运行时容器长期存在的内存增长问题 —— 这正是 7 月 15 日网络节点 OOM 事件的根源。

**这标志着v3切换最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 投票 —— 紧急窗口较短。
2. 提供 Kimi 服务的主机：在投票结束后立即声明 `moonshotai/Kimi-K2.6` 意图，并在周期 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776**，2026年7月17日，约12:50 UTC）将您的 MLNode 切换回来。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在进行引导委托时：**不要委托给守护节点**；将委托分散到独立的 Kimi 主机上。有关最新的委托指南，请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 经纪人 / devshard 创建者：如果您的网关流量仍使用 v1/v2 或 `/v1/devshard`——请在投票结束前切换到 `/devshard/v3`。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
投出您的选票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced` v0.2.13或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（加速，0.667 赞成阈值；出席率重要，请尽快投票）。
- 意向截止时间（第 330 个周期）：区块 **5,105,276** — 7 月 17 日，约 12:05 UTC（约 05:05 PDT）。
- 第 331 个周期的 PoC 开始：区块 **5,105,776** — 7 月 17 日 12:50 UTC（05:50 PDT）。

有关引导机制的更多信息：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026 年 7 月 15 日

**加速治理投票（提案 87）：移除 Kimi-K2.6 以实现快速重新引导**

`moonshotai/Kimi-K2.6` 在第 328–329 个周期失去了 PoC 验证多数票（详见 [此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的最快方式——与六月提案 78 的恢复路径相同。

该提案将在下一个 PoC 之前从 PoC 参数中移除 `moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi 将从第 331 个周期开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案 87 投票——加速窗口很短。
2. 运行 Kimi 的主机：请保持您的 Kimi 设置处于待命状态，并准备好在引导 PoC 时切换回您的 MLNode。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（PDT）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（加速，0.667 赞成阈值；出席率重要，请尽快投票）。
* 引导意向截止时间和周期时间请参阅引导说明。

## 2026 年 7 月 15 日

**Kimi-K2.6 事件（第 328–329 个周期）：事件经过、恢复计划及委托指南变更**

在第 328 个周期，`moonshotai/Kimi-K2.6` 失去了 PoC 验证多数票。在第 329 个周期开始时，运行 Kimi 的主机被移出群体。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行 Kimi 的服务器同时故障：

* 其中一台服务器上，运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器因内存不足而失去了其网络节点。

Kimi 的验证投票一直接近 2/3 的阈值，因此这一情况足以使该组低于多数。守护者投票决胜机制未启动：目前未分配每模型投票权重的守护者已被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与第 306–307 个纪元的事件类似。第 329 个纪元开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导程序重新添加它（约 1.5 个纪元）：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次通过标准引导流程：声明意向、部署窗口、PoC 存储提交。意向截止日期前将发布相关说明。

**委托指导变更 — 请阅读**

先前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤销：

* **请勿将委托指向守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主机制与备用机制绑定到同一硬件上——本次事件正是这种故障模式。目前正在讨论协议层面的限制。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个运行模型的独立主机上。如果您运行多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已在进行中的修复*** PoC 验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期的 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。在完成全面迁移后，v1/v2 将被移除，且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请对其投票。
2. 如果您计划提供 Kimi：关注引导说明，并在意向截止前准备好声明意向。
3. 如果您为 Kimi 委托：选择非守护者的委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这将解决 RAM 增长问题，并在移除 v1/v2 之前必须完成。
5. 检查网络节点的 RAM 并设置容器内存限制（`api`、`versiond`）。

## 2026年7月11日

**v0.2.13-devshard-v3 运行时升级提案已通过治理**

devshard v3 运行时已通过链上治理并添加至 `DevshardEscrowParams.approved_versions`。
...
提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人切换流量至 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 自动下载二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

此类仅限 devshard 的运行时升级无需主网重启或手动主机操作。

**操作项**

白名单 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这使他们在链升级期间仍能继续提供推理服务，而不依赖已弃用的经典 API 路径。

**关键变更**

- 使经纪人能够在 v0.2.14 链升级期间继续提供推理服务，而不依赖已弃用的经典 API 路径。
- 改进了 RAM 利用率。
- 修复了网关运行时行为。
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换到 v3。此类升级期间无需重启主网或手动执行主机步骤。
...
主链/API 工作重点包括：PoC 重复工件防护、早期共享检测、经典推理 API 废弃（禁用主网上的 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收者路由，以及升级时的安全修复。

devshard 部分正在准备 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布并部署 devshard v3 版本，再进行主网链升级。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供推理服务。
...
4. 若链上提案获得批准，此 PR 将在链上升级执行后立即合并。
5. 请直接审查 PR 代码，并对发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格在下一轮升级周期中获得社区奖励。

本次仅为邀请审查 Pull Request，不启动正式投票。治理投票流程将在审查期结束后开始。

**Devshard v3 治理投票**

devshard v3 版本将在主网链升级前提出并部署，以便代理可在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作项**

1. 现在 — 审查 PR。请阅读 GitHub 上的 PR #1267，并对发现的问题、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。若您已完成上述两步，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证以阻止复制计算通过采样检查，从而阻止攻击。
...
**Kimi-K2.6 启动：第 311 纪元的再次尝试**

`moonshotai/Kimi-K2.6` 将在第 311 纪元再次尝试启动。希望提供 Kimi 服务的主机需在第 310 纪元声明其意向，并在新纪元开始前准备好节点。守护节点也将运行 Kimi，因此不希望运行 Kimi 的主机可将其权重委托给该节点。

**将运行Kimi的主机**

1. 在第 310 纪元、区块 **4,797,456**（约 6 月 28 日 12:00 UTC）之前声明 PoC 意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若足够多的主机声明意向，请在第 311 纪元开始前约 500 个区块的窗口期内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。

**将不运行Kimi的主机**

将您的权重委托给运行Kimi的主机（或发送拒绝）：
```
...
> **更新（2026年7月15日）：此建议已撤销——请勿委托给守护节点。** 守护节点是 PoC 验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指引。
...
**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****主要变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一并恢复。该系数设定使得在8xB300硬件上运行Kimi的共识权重约等于主模型MiniMax。此设定并非为了偏袒任何硬件——MiniMax上已具备相同的权重。在8xB300主机上运行Kimi而非MiniMax，可获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这将有效取消该模型的不参与惩罚）。该系数设定使得在8xB300硬件上运行GLM-5.2的共识权重约等于MiniMax。此设定并非为了偏袒任何硬件。在8xB300主机上运行GLM-5.2而非MiniMax，可获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2的主机，需在 `start_poc - deploy_window` 的引导快照前提交 `MsgDeclarePoCIntent` 以声明对该模型的意向。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及>2/3可达性）。这使操作者可在配置硬件前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由在PoC开始时实际提交PoC存储提交的节点决定——而非仅由声明意向的节点决定。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮起（PoC开始约6月26日15:25 UTC；生效约16:00 UTC），前提是每个模型均满足引导资格：

* `moonshotai/Kimi-K2.6` 再次成为活跃的PoC模型。
* `zai-org/GLM-5.2-FP8` 作为可选的PoC模型提供，且无非参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能顺利引导进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计支持该提案。

**主机需采取的操作**

1. **若要提供Kimi：** 在投票结束**后立即**声明 `moonshotai/Kimi-K2.6` 意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，此期间切换是安全的，因为此期间无cPoC。

2. **若要提供GLM-5.2：** 声明 `zai-org/GLM-5.2-FP8` 意向，并在部署窗口期间配置您的MLNode。提供GLM-5.2是自愿的——未选择的主机不会受到任何惩罚。

3. **继续使用MiniMax：** 继续提供`MiniMaxAI/MiniMax-M2.7`——无需任何操作。

4. **在截止日期前对提案79进行投票。** 紧急窗口很短。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何从冷密钥向热密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 打算再次提供Kimi的主机应保留其Kimi设置并准备在PoC 309时将MLNode切换回Kimi——在第308个纪元将进行一项恢复Kimi的投票。

**接下来的内容**

在第308个纪元将进行一次紧急投票，合并两项变更，均于第309个纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6** —— 重新添加Kimi并重启其引导程序。
* **添加GLM-5.2** —— 添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入，并提前衡量对模型的需求，选择不提供该模型的主机不会受到惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以快速重启；Qwen3-235B被退役**

`moonshotai/Kimi-K2.6` 目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。

一项紧急提案现已上链，以便在下一个纪元开始前生效。
链上提案在单次投票中执行以下操作：
...
**每项变更的目的*** **Kimi K2.6 — 被移除以快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的路径。下一纪元将进行恢复Kimi的投票。
* **Qwen3-235B — 被退役（独立、无关变更）。** 退役较旧的Qwen3-235B是一项独立的、此前要求的阵容变更。它与Kimi情况无关，仅因同为PoC阵容变更而被一并包含。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307个纪元结束前完成。每个纪元约22.9小时，标准的48小时投票期无法容纳在一个纪元内；而12小时的紧急投票期可以。创世守护者应支持此提案。

**若获批准的效果**

自第308个纪元（约2026年6月25日17:25 UTC）起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换为 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时无cPoC。请现在预下载FP8权重（如尚未准备就绪），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 打算再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——在第308个纪元将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2 **而不施加不参与惩罚**，以便主机可选择加入并提前测试模型需求，对于选择不提供该模型的主机无任何惩罚。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何从冷密钥向热密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
* 第308纪元 shortly after：PoC开始约16:50 UTC，生效约17:25 UTC。
* 紧急提案要求0.667的赞成阈值；参与率很重要，请尽快投票。
...
提案获批后，v2现在可与现有的v1 devshard运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有的v1流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为Gonka发布工件。`versiond` 会自动下载该二进制文件，验证sha256哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于此类仅devshard的运行时升级，无需重启节点容器或手动执行主机操作。

**关键变更**

- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以避免长时间运行的会话将所有已服务的推理保留在RAM或状态中。
- 通过OpenTelemetry和Prometheus添加了内部devshard追踪和指标。
- 通过Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor添加了join-stack可观测性。
- 将每次推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个插槽的总计数。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以避免长时间运行的会话将所有已服务的推理保留在RAM或状态中

2. 通过OpenTelemetry和Prometheus添加内部devshard跟踪和指标

3. 通过Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor添加join-stack可观测性

4. 将每次推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每个插槽的总计数

5. 在纪元变更时修剪旧纪元存储，将SQLite/Postgres模式设置移出热路径，并为每个进程选择恰好一个存储后端

**升级计划**

devshard运行时通过链上参数提案升级，而非完整的链软件升级。

该提案在 `DevshardEscrowParams.approved_versions` 中注册一个新条目。

发布版本将 `devshardd` 二进制文件作为Gonka发布工件。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证sha256哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

新流程在 `/devshard/v2` 前缀下提供服务。现有的 v1 devshard 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。此类升级期间无需重启节点容器或手动执行主机步骤。

**如何投票**

如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
提案详情和投票可通过`inferenced`查看。任何活跃节点均可使用。可用节点包括：
...
**[仅用于devshard的升级PR](https://github.com/gonka-ai/gonka/pull/1289) 现已开放评审。**

这是首次仅针对devshard的升级，因此流程不同于标准链升级。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。

如果通过治理流程获得批准，新版本的devshard将与现有的v1运行时并行运行。

请直接审查PR，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。

有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期中获得社区奖励。

本次仅为PR评审邀请，不启动正式投票。治理投票流程将在评审期结束后开始。
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已**批准**，升级已在主网块`4267300`成功执行。

## 2026年5月25日

**升级v0.2.13：预下载二进制文件和MiniMax-M2.7权重**
...
链上治理投票已结束，[v0.2.13升级提案](https://github.com/gonka-ai/gonka/pull/1143)（提案ID `54`）已**批准**。

升级将在主网**块高度4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）自动执行。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6`和`MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一两个模型的主机，仍需为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元`278`**。未采取任何操作的主机将从纪元278起，对其全部权重施加每纪元15%的惩罚。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过的提案的重新投票，现已提交多个更新。

- 包括：Kimi（`0.78`）的重新校准权重、新模型`MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专用章节）。
- 该提案将升级后的宽限期延长至3000个区块，以便在新快照逻辑稳定期间主机不被惩罚。
- 治理：将创世守护者投票权降低至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%的投票权中达到约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 在 vLLM 0.20.1 发布后，根据 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`（不变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下Kimi的每轮共识权重减少约38%）
    - MiniMax-M2.7：`0.3024`
...
- 为在v0.2.12之前加入的DAPIs补全缺失的`MsgRespondDealerComplaints`授权。
- 修复可能导致桥接和流动性池合约调用中出现间歇性权限错误的连接问题。
- 将创世守护者调整后的投票权降低至约25%，并将链上治理法定人数设置为`0.25`。若守护者不投票，则剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
- 将4个早期主机和经纪人添加至`allowed_creator_addresses`。

**Ethereum 桥接主网激活**

- 通过升级处理器激活Ethereum主网桥接设置。
- 注册Ethereum桥接合约地址`0x972a7a92d92796a98801a8818bcf91f1648f2f68`、USDC和USDT代币元数据、桥接交易授权以及CW20`wrapped_token`代码ID`105`。
- 激活后，桥接将支持Gonka主网与Ethereum之间的跨链传输（包括在Ethereum上封装GNK以及桥接USDC/USDT）。封装/解封脚本和操作员工作流程将另行文档化。
...
**在第278个周期前选择`MiniMaxAI/MiniMax-M2.7`参与方式（届时开始处罚）**

对于每个经治理批准的模型，多模型 PoC 要求每个主机明确选择参与方式（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 之后不采取任何操作将导致处罚。在此阶段，您应提前决定首选选项，以便在提案通过且升级成功应用于主网时能够迅速行动。

**桥接容器更新/验证**

所有主机需验证其桥接容器已部署、运行最新版本且同步正常。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再进行其他操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表盘/浏览器更新（升级前或升级后）**

主机需更新仪表盘/浏览器。请从`gonka/deploy/join`目录运行以下命令。如果您尚未在本地克隆`gonka`仓库，请先参考加入网络指南。此仪表盘更新仅涉及容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
投出您的选票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 或更高版本（v0.2.12）。
...
- 运营商时间表：投票于5月22日22:12 UTC结束 → 升级高度约在5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax引导快照在 start_poc − 500 块时（约提前43分钟）→ 升级后下一个纪元边界启动首个MiniMax PoC阶段 → MiniMax处罚在链纪元278执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中Cosmovisor对 `application.db` 的备份通常为数十GB，请提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获得批准，此跳过为预期行为，并非故障；新快照逻辑将在下一个纪元开始。
- 若获得批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储相同的环境变量）。本地SQLite将保持默认设置，并自动清理（保留最近3个纪元）。
- 如果提案未通过（未达到法定人数或 `no_with_veto` 超过⅓），链上无任何变化，升级不会发生。运营商可能看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
问题已追溯至具有非标准语义的推理请求。该攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。

在纪元 #267 中，主机得以恢复，PoC验证已成功完成。

**推理可用性**

使用受影响的非标准语义的请求将不再被网络接受。

在相关保护措施应用期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。

**纪元 #267 中Kimi的权重**

由于现有协议规则，Kimi在纪元 #267 中的活跃权重较低：单个模型的总权重不得超过上一纪元所有模型总权重的75%。
...
2. 监控接下来几轮的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重被正确反映。
...
在当前轮次（#266）中，网络观察到活跃权重显著下降。
似乎PoC投票未能收集到本轮所需的投票。确切原因尚未确认，社区正在积极调查此事。

**受影响参与者**

如果您的节点未能进入本轮，请尽可能保持您的API节点在线并可访问。
这有助于补偿委员会收集PoC参与的证据，并更准确地核算受影响的贡献。

**调查进行中**

社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更清晰的认识，将及时更新信息。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 现已在链上并开放投票。

- 包括：对Kimi（`0.78`）重新校准权重、新增模型 `MiniMaxAI/MiniMax-M2.7`、更新验证阈值、devshard存储重构，以及若干PoC/奖励修复和Ethereum桥接主网激活。
- 提案在升级后延长了宽限期，升级后3000个区块内不再惩罚主机。
- 所需准备：桥接容器检查、MiniMax决策、仪表板更新、投票。
...
- 将devshard nonce限制从 `20_000` 提高到 `1_000_000`。
- 将每轮最大devshards数量从 `100` 提高到 `500_000`。
- 修复新模型启动期间确认PoC奖励核算。
- 在本次升级轮次剩余时间内禁用确认PoC，以便新快照逻辑从下一轮开始干净启动。
- 当参与者再次激活时重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问。
- 将创世守护者调整后的投票权降低至约25%，并将链上治理门槛设为 `0.25`。在守护者不投票的情况下，这使得剩余75%投票权的有效门槛为1/3（`0.25 / 0.75 = 0.334`）。
...
- 为 SQLite 和 Postgres devshard 数据库添加数据清理功能。
...
**第 278 纪元前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您是否正在运行当前版本，然后再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意跳过从升级高度到升级周期结束（3000区块宽限期）的确认PoC。若获得批准，此跳过属预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作，该升级可能包括以太坊端合约激活，所有主机需验证其桥接容器已部署、运行最新版本并正确同步。
...
请直接审查 PR 代码，并就您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一个升级周期中获得社区奖励。

本次仅为拉取请求的审查呼吁，不启动正式投票。治理投票流程将在审查期结束后开始，很可能在明天。

**关键变更****inference-chain**

- 确认PoC在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致服务了合格模型和尚未合格模型的诚实矿工被大幅削减。修复方案是存储一个可确认模型和权重缩放因子的纪元快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推断就会立即再次使其失效。现在在重新激活和即将晋升时会重置该计数器。
- 在v0.2.12之前加入的DAPI在其冷到暖的授权授予中未包含`MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应交易商投诉。
- Devshard结算使用了硬编码的`20_000` nonce限制。现在该限制为`DevshardEscrowParams.MaxNonce`，v0.2.13升级将其设为`1_000_000`。升级还将`MaxEscrowsPerEpoch`提升至`500_000`。
- 升级为当前纪元安装了一个宽限期条目，延长了`UpgradeProtectionWindow`（3000个区块）。从升级高度到升级纪元结束期间，将跳过确认PoC触发器，因此新的快照逻辑仅从下一个纪元开始运行。复用了v0.2.10的宽限期原语。
- Wasm守护进程访问在应用布线后解析，因此合约权限检查适用于桥接和流动性池操作。

**decentralized-api**

- 一些OpenAI兼容的上游返回数值型`stop_reason`值。`Choice.StopReason`现在接受任何JSON类型，因此这些响应不再因反序列化失败。
- 内部devshard存储迁移不再阻塞dapi启动。在迁移和恢复完成之前，devshard路由保持不可用。

**devshard**

- 由于旧的托管数据保留在一个SQLite存储中，devshard存储可能无限增长。现在存储按纪元范围划分，并在后台清理旧纪元，仅保留最新的3个纪元。
- Devshard需要为大规模部署提供共享存储选项。现在它可以使用Postgres作为主存储，SQLite仍作为本地后备存储。
- Postgres数据按`epoch_id`对会话、差异和签名进行分区，以便清理可以干净地删除旧纪元数据。
- 状态快照减少了长时间运行会话的恢复工作量。
- 负载查找被绑定到托管纪元，并为纪元边界和遗留纪元0请求提供回退机制。
- 当前纪元分片统计信息暴露了nonce、版本、组和每个主机的计数器。
...
评审者可以在此处查看完整的升级提案、迁移详情、测试摘要和建议流程：
...
修复方案通过并行加载devshards并从快照恢复现有devshard会话来移除该阻塞点。
...
docker start api
```
部署后，请再次确认9100和9200端口上的服务器是否正常运行：
```
...
在上一个轮次中，发现Kimi-K2.6解析某些响应时存在一个轻微bug。
...
我们建议替换api容器的二进制文件。除修复外，新版本还启用了devshard数据库的清理功能，并为devshard状态添加了Postgres支持。
...
安全模型保持不变：PoC验证本身未改变，仍需超多数验证算力才能接受结果。
...
对于多模型 PoC，这意味着 Kimi 现在作为活跃模型组拥有自己的参与和奖励追踪。
...
已提交 PoCIntent 的节点应在 PoC 于区块 `3874496` 开始前，至少将一个 MLNode 从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。

预评估与PoC启动之间有500个区块的窗口期。在此期间无CPoC任务，因此已声明意向的主机可安全地将模型节点切换至`Kimi K2.6`。

请遵循指南完成所需部署步骤：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年5月4日

传输代理 `node1`、`node2` 和 `node3` 已禁用。所有主网推理现在均通过 `node4` 路由，该代理基于新的 `devshard` 计费方式运行。

这标志着网络的重要里程碑：`devshard` 已上线并具备生产就绪能力。`node4` 将作为未来推荐的公共网关。

**操作要求**：将您的端点更新为 `node4`。

## 2026年5月2日

今日的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，其权重极低。请保留您的 MLNode 使用 `Qwen235B`，并于明天提交下一个周期的意向。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成其独立的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个节点必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固**。保留节点（`POC_SLOT=true`）被随机抽样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易方共识，使遗留验证者惩罚与所需抵押语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。

**主机指南**

- 部署、委托或明确拒绝新批准的治理模型（包含的模型将在升级后 2 个周期激活）。请参阅[指南。](https://gonka.ai/docs/host/multi_model_poc/)
...
- 迁移：测试和迁移详情详见 [v0.2.12 文档。](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/upgrades.md)
...
版本0.2.12将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留之前强制执行的模型和Kimi。
每个DAPI在其本地持久化MLNode配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。

版本0.2.11通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes`也显示为干净。版本0.2.12停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找`/admin/v1/config`中包含额外模型的每个节点，并向`/admin/v1/nodes/<id>`发送一个包含清理后配置的`PUT`请求。这些更改将在60秒内持久化。其余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
[升级提案](https://github.com/gonka-ai/gonka/pull/948)的下一个链上软件版本v0.2.12现已上链并开放投票。

**主要变更**

- **多模型PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为每模型PoC组。每个经治理批准的模型生成自己的本地PoC权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接或通过委托PoC投票权重）。
- **`moonshotai/Kimi-K2.6`作为第二个模型引入**：模型组将在升级后两个周期激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200，8xB200）上的模型计算复杂度。
- **Devshard独立运行时**（[#1045](https://github.com/gonka-ai/gonka/pull/1045)）。将devshard发布与DAPI/主网发布周期解耦。
- **Certik审计修复**（[#1020](https://github.com/gonka-ai/gonka/pull/1020)，[#1021](https://github.com/gonka-ai/gonka/pull/1021)，[#1022](https://github.com/gonka-ai/gonka/pull/1022)，[#987](https://github.com/gonka-ai/gonka/pull/987)，[#949](https://github.com/gonka-ai/gonka/pull/949)，[#988](https://github.com/gonka-ai/gonka/pull/988)，[#825](https://github.com/gonka-ai/gonka/pull/825)，[#1011](https://github.com/gonka-ai/gonka/pull/1011)，[#1029](https://github.com/gonka-ai/gonka/pull/1029)，[#789](https://github.com/gonka-ai/gonka/pull/789)）。已解决审计发现的问题。
- **协议加固**。保留的节点（`POC_SLOT=true`）将随机采样用于单次PoC/CPoC时间。其他更新包括将`mlnode`版本传播到链上`HardwareNode`，修复DKG经销商共识，对齐传统验证者 slashing 与所需抵押品语义，确保devshard托管资金的原子性，以及在`inference_finished`事件解析中添加零时间戳容差。
...
从`docker-compose.yml`部署`versiond`和`proxy`服务的最新版本（使用tag为release/v0.2.12的仓库）：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/948) 针对下一次链上软件升级 v0.2.12 已开放审查。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型转变为按模型分组的 PoC。每个治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
- **共识级交易费用与自动迁移** ([#937](https://github.com/gonka-ai/gonka/pull/937), [#981](https://github.com/gonka-ai/gonka/pull/981))。引入由治理控制的 Gas 价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免费。`MsgPoCV2StoreCommit` 采用双组件费用（基础验证 + 计数线性）作为主要 Sybil 防御机制。详情请参见 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位完整熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
- 确保冷账户持有足够的金额（例如 100 GNK），以覆盖自动授予的费用限额支出。
- 一旦治理批准新模型，即部署、委托或明确拒绝每个治理批准的模型（包含的模型将在升级后3个周期内激活）
...
现在有新版本的 mlnode 可用：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于接近PoC开始运行的节点，稳定性有所提升。

此次更新修复了在PoC开始附近的一个边缘情况，该情况此前可能导致在特定条件下性能下降。
...
针对升级提案 v0.2.11 的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。
...
有关这些变更的更多详情，请参见治理文档：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/)
...
下一代链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每项推理交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的设计版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳多达 100 倍的推理量，具体取决于工作负载和网络状况。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时启动大量剪枝活动，造成不必要的运营压力。
...
- 请在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
针对此 PR 审查的有意义贡献，可在下一次升级中提出奖励。

本次仅为拉取请求的审查请求，并非正式投票的开始。治理投票流程将在审查期结束后启动。
...
目前，通过每次推理的链上交易处理推理会话会限制吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，不是因为其已被认为完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时开始大量剪枝活动，造成可避免的操作压力。
剪枝说明详见[此处](https://gonka.ai/FAQ/#__tabbed_7_4)。
...
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
审查者可在此处查看完整的升级提案、迁移详情、测试摘要和建议流程：[链接](https://github.com/gonka-ai/gonka/pull/813)。

## 2026年3月16日

**API 二进制文件 `v0.2.10-post7` 已可用**

已发现 `v0.2.10` 中存在潜在漏洞。为降低当前升级前阶段的风险，建议在下一次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
对于已使用代理层的团队，这可能是简化架构、转而依赖原生行为的好机会。实际上，这将带来更清晰的集成模式和更易维护的系统。

## 2026年3月6日

**注意：v0.2.11 升级预计将于下周初进入审查和治理投票阶段。**

请密切关注并计划参与。投票是支持网络发展、确保升级符合参与者实际需求的最简单方式之一。
如果您无权访问持有投票权的冷钱包密钥，则建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代为投票。若无此授权，其他账户将无法提交投票。

在此设置中：

- 授予人 = 拥有投票权的账户（冷钱包密钥）
- 被授予人 = 代表授予人提交投票的账户（热钱包密钥）

被授予人仍可为自己账户投票。授予人可随时撤销此授权。
...
1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格 5 分钟超时，而某些请求可能需要更长时间。新版本的 API 将此值恢复，而不是强制执行严格限制。
2. 之前的请求重试系统即使因处理超时（而非 TLS 超时）失败也会重试推理。
服务器端对长请求的重试通常无效，因为这会导致相同的超时场景。同时，客户端可能收到不一致的输出。新版本的 API 在此类情况下不再重试推理。
3. 当前被保留且不参与 PoC 生成的 MLNode 仍被用于 PoC 验证。这可能导致推理遗漏。新版本将这些节点排除在 PoC 验证之外。
4. PoC 验证管道中增加了额外的保护措施。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低PoC启动时仍处于活跃状态的请求导致潜在权重下降的风险。
...
- 此构建版本旨在与前一版本向后兼容。
- 它已针对少量节点启用，但仍建议在部署前审查变更内容。

## 2026年2月19日

**抵押参数更新提案——投票结果**

抵押参数更新提案未能达到法定人数，因此根据当前治理规则被否决。这意味着更新后的参数将不会激活。

如前所述，Epoch 180时的抵押激活独立于本次投票。

由于提案未通过，Genesis中定义的抵押参数将在Epoch 180时自动生效。

参与者应：

- 审查Genesis中定义的抵押参数。
- 在Epoch 180之前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)设置正确，否则从Epoch 180起，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若无替代方案获批准，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各纪元间可能波动。较小的权重可能经历相对更大的波动。为避免在纪元边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
从PoC权重推导H100等效容量或奖励预测的追踪器和仪表板应从第176轮起验证其转换系数。
...
!!! note "兼容性说明"
    ...
    **当前生效的关键变更****PoC 验证采样优化**
    ...
    此次升级引入了一种新的 PoC 验证机制，通过为每个参与者分配一组固定的采样验证者，将复杂度从 O(N^2) 降低到 O(N x N_SLOTS)。

    **按实时时间归一化 PoC 权重**

    此次升级根据实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入一个3000个区块的宽限期，期间不进行确认 PoC，并对升级所在纪元的错过率和无效率阈值放宽要求。
...
这意味着在单个纪元内，即使被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押品仅占每日奖励的约24%。

**警告：** 抵押参数将 Regardless of 投票结果生效。如果此提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。

投票结束后、第180纪元前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180纪元起，其奖励将被削减5倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获得批准，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并降低了升级当期的错过率和无效率阈值。
...
- 投票结束：2026年2月18日，UTC 09:26:26
- 升级高度：2712600
- 预计升级时间：2026年2月18日，UTC 15:30:00
...
距离第180个周期不足7天——是时候做好准备了。

正如AMA期间讨论并基于社区成员提出的论点，本提案建议从较小的抵押要求和最低限度的惩罚开始。
...
这意味着即使在一个周期内被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押仅占日收益的约24%。

提案提交投票后将另行发布公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到证明，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。

## 2026年2月13日

**即将进行的v0.2.10升级投票与执行时间表**

即将进行的软件升级v0.2.10的链上投票期预计将于周日晚上（洛杉矶时间）/周一早上（UTC）开始。
...
请保持在线，以便在需要时迅速应用补丁。更多详细信息和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已开放**

链上推理访问目前开放，不限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 已开放供审查，用于下一次链上软件升级 v0.2.10。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约2天。

对于对此次PR审查做出有意义贡献的，可能在下一次升级中提出奖励。

本次仅为拉取请求的审查请求，而非正式投票的开始。治理投票流程将在审查期结束后开始。

**关键变更****[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC验证采样优化**

此升级引入了一种新的PoC验证机制，通过为每位参与者分配固定的采样验证者集合，将复杂度从O(N²)降低至O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际PoC时间标准化PoC权重**

此升级根据实际PoC耗时标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- **[PR #712](https://github.com/gonka-ai/gonka/pull/712)** 要求proto-go生成代码与提交代码匹配
- **[PR #711](https://github.com/gonka-ai/gonka/pull/711)** 从链状态获取PoC测试参数
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式兑付转移
...
- **[PR #644](https://github.com/gonka-ai/gonka/pull/644)**：避免在每次启动时重写静态配置。
...
- **[PR #545](https://github.com/gonka-ai/gonka/pull/545)**：API流程中的小修复。
...
现有主机无需升级其 `api` 和 `node` 容器。更新后的容器版本专为在链上升级完成后加入的新主机设计。

**建议流程**

1. 活跃主机请在GitHub上审查本提案并留下反馈。
2. 在社区审查该PR后，预计将从该分支创建v0.2.10版本，并可提交此版本的链上升级提案，以启动正式的治理投票流程。
...
在更新后的权重分配生效后，追踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保其与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新——补丁已发布**

现已发布补丁，以解决导致PoC周期中块验证近期暂停的问题。建议主机尽快应用此补丁，以确保正确的PoC验证行为，并安全地恢复块生成。

**需要采取的操作**

请主机尽快应用补丁，以确保正确的PoC验证行为，并安全地恢复块生成。
```
# Download Binary
...
由于在当前的PoC周期中，验证阈值可能无法满足的高风险，主机们集体采取了预防性措施，暂停了区块验证。
根据当前评估，用于处理此情况的机制可能无法按预期运行。为防止在不确定或不安全的条件下完成验证者最终确认，网络在验证者选择之前已停止运行。
...
针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并于区块 2451000 成功在主网上执行了升级。此次升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**请注意**

- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时执行后续步骤或缓解指令。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入下一纪元的参与节点集合。
...
在因 cPoC 惩罚而减少或排除奖励的情况下，未计入的部分将转入社区池。此前，此类奖励会在其他参与者之间重新分配。
...
- 节点仅服务于`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- 节点运行PoC v2兼容镜像：
    - ghcr.io/product-science/mlnode:3.0.12-post1
    - ghcr.io/product-science/mlnode:3.0.12-post1-blackwell
...
- 仅配置为服务于支持的模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- 镜像已更新至PoC v2兼容版本
...
- MMR 承诺：工件存储通过默克尔山峰范围承诺迁移到链下；仅 `root_hash` 和 `count` 记录在链上。
- 双模式迁移：支持 V1（常规 PoC）和 V2（确认 PoC）追踪。
...
- SSRF 和 DoS：验证 `InferenceUrl` 以拒绝内部 IP，并添加超时以防止请求挂起。
- 投票翻转：拒绝重复的 PoC 验证以防止覆盖。
- 认证绕过：将 `epochId` 绑定到签名，以验证对应正确周期。

**PoC v2 参与资格的主机要求**

要具备参与 PoC v2 的资格，主机必须完成以下事项：
...
!!! note 
    未能同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 权重分配的过渡仍受观察性采用阈值和后续治理的约束。

    **维护与操作**

- Cosmovisor：节点和API二进制文件的更新将自动处理。现有主机无需对运行中的容器执行手动更新。
- 磁盘空间：Cosmovisor会在`.inference/data`目录中创建完整状态备份。请确保有250 GB以上的可用空间。
- Postgres：升级后现在可通过Postgres配置本地负载存储。
...
本指南说明主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置在第155个周期开始符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8`在迁移期间受支持，但不贡献于PoC v2就绪性或权重分配。参与PoC v2需要提供`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
- 您的所有ML节点均提供Qwen/Qwen3-235B-A22B-Instruct-2507-FP8。这是唯一贡献于PoC v2权重的模型。
...
!!! note 
    ...
    `node-config.json` 仅在 Network Node API 首次启动或本地状态/数据库被删除时使用。如需全新重启，请编辑它。对于现有节点，模型更新应通过管理 API 进行。

    **5. 升级 ML Node 镜像（PoC v2 所需）**

    编辑 `docker-compose.mlnode.yml` 并更新 ML Node 镜像：
...
确认 ML Node 仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 服务，这是 PoC v2 权重和未来权重分配使用的唯一模型：
```
...
!!! note "治理和 PoC v2 激活说明"
    ...
    **阶段 1. 观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML Node 切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为兼容 PoC v2 的版本，才能为 PoC v2 权重做出贡献。
    - 网络将观察采用情况，以评估主机为迁移到 PoC v2 权重做好准备的程度。
...
PoC v2 仅在链上治理提案获得批准时才成为权重分配的活动方法。
    在此提案获得批准之前：
...
- 从配置中移除所有其他模型
- ML Node 镜像为 3.0.12（或 3.0.12-blackwell）
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。所有此前支持的模型均从活跃集合中移除，除：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估是否具备过渡至 PoC v2 的条件。一旦观察到约 50% 的活跃 Host 达成足够采用率，可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
由于变更量较大，此处仅突出显示部分项目。完整的更新与修复列表请参见 [GitHub 拉取请求](https://github.com/gonka-ai/gonka/pull/539)。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未同时满足两项条件的节点将不被视为 PoC v2 参与资格节点。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
- 升级提案在区块 2387000 提出，该区块的预计时间是 2026 年 1 月 29 日 06:30:00（UTC）。
...
在延长确认后，请利用这段时间为新的 PoC 要求准备您的设置。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机建议在多个周期内逐步进行更新。
...
`allowlist` 具有固定的过期时间，不会延续至延长的稳定窗口之外。一旦 `allowlist` 在区块 2443558 过期：
...
`Allowlist`过期：自动在区块2443558处过期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块2,222,222处的参与者注册截止时间对齐。
...
投票截止：2026年1月12日，06:04:14 UTC。

`Allowlist`过期：自动在区块 2,222,222。

## 2026年1月10日

**已批准临时参与者 `allowlist`。将在第135个周期激活**

关于临时参与者 `allowlist` 在稳定期的链上治理投票已结束。

该提案已获批准。此提案定义了一个临时 `allowlist`，反映在最近多个轮次中行为保持一致的参与者。

**当前生效的关键变更**

1) 网络将运行一个 `allowlist`，由在多个轮次中满足以下条件的参与者组成：

- 报告的硬件特征与常见观察到的配置模式匹配（过滤后的非标准配置字符串列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))
- 证明的PoC权重不超过可比硬件观察到的权重的150%

2) 之前表现出偏离这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块 2,222,222 结束。
...
- 激活发生在第135轮的第一次PoC期间
- 无需软件升级
- 从此时起，`allowlist` 将持续有效，直至并包括区块 2,222,222
...
若获得批准，网络将暂时使用一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：
...
- 观察到的PoC权重低于使用可比硬件的其他参与者所展示的权重的150%。
之前持续偏离这些模式的参与者在稳定窗口于区块 2222222 结束前不属于 `allowlist`。
...
为恢复可靠的共识进展，至少需要67%的网络活跃算力安装该补丁。
...
链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强已获批准，并在主网上成功部署。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——此前曾使用过的、具有时限的防御机制，现提议重新启用。
...
此升级将此前通过手动API更新分发并已在网络中使用的关键修复正式化。这些修复包括：
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级中包含的预备性变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得等于第117轮奖励1.083倍的 payout，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
- 主机无需执行任何进一步操作。
- Cosmovisor在每次更新时都会在`.inference`状态文件夹中创建完整备份。为安全执行更新，建议保留250 GB以上的可用磁盘空间。[点击此处](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory) 了解如何安全删除`.inference`目录中的旧备份。
...
这可以减少磁盘使用量和升级时间。但是，如果升级失败，将无法恢复到先前状态。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意试图破坏或施压系统所导致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。

**关键变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用、有时间限制的防御机制，现提议重新激活。
...
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续到区块 2,294,222（约 19 天）。

**治理控制机制**

本升级包含的预备性变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第 117 期奖励分配**

本提案涵盖与链暂停（第 117 期）相关的两项奖励分配：

- 在第 117 期活跃但未收到该期奖励的节点，将获得该期的补发奖励。
- 所有在第 117 期活跃的节点，将获得额外支付，金额为第 117 期奖励的 1.083 倍，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
Cosmovisor在执行更新时会在 `.inference/data` 状态文件夹中创建完整备份，请确保您的磁盘有足够的空间。请阅读[此处](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)了解如何安全地从 `.inference` 目录中删除旧备份。
如果您的 `application.db` 占用大量空间，您可以使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中的方法进行清理。
...
## 2026年1月5日
当前网络上观察到高于正常的推理遗漏率。
在许多情况下，这是由于一个漏洞，导致不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。

参考：[https://github.com/gonka-ai/gonka/pull/517](https://github.com/gonka-ai/gonka/pull/517)

此 `API` 版本改进了对失败推理的处理弹性，并减少了推理遗漏计数问题。它还为PoCBatch和PoCValidation交易引入了批处理功能。
