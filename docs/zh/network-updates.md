...
[拉取请求](https://github.com/gonka-ai/gonka/pull/PR) 已开放，供审查下一个链上软件升级 v0.2.14。
...
devshard 部分准备 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而改善 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支发布一个独立的 devshard v3 版本，并在主网链升级前推出。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级运行期间继续提供推理服务。
...
devshard v3 运行时发布的治理投票将于 2026 年 7 月 8 日约 18:30 PDT（2026 年 7 月 9 日 01:30 UTC） shortly 开始。devshard v3 发布将在主网链升级前推出，以便代理在升级运行前将推理流量移至 `/devshard/v3`。投票详情将在提案进入治理时公布。

**主机操作事项**

1. 现在 — 审查 PR。阅读 [GitHub 上的 PR #1267](https://github.com/gonka-ai/gonka/pull/PR)，并对您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器稳定，请提前将 `ap`i 二进制文件和桥接器镜像更新至 0.2.14，具体请参阅[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接器容器。如果您已完成这两步操作，则无需重复执行。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
3. ~2026年7月8日18:30 PDT / 2026年7月9日01:30 UTC — 对 devshard v3 进行投票。devshard v3 运行时的治理投票开放。提案上线后请立即投票；投票详情将在其进入治理时公布。
4. 代理 — 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 发布，将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 — 准备调整指标统计方式。详细说明将在 `devshard v3` 投票启动并接近完成之后发布。
...
针对即将到来的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复程序 —— 可异步部署，通过强化 PoC-v2 验证阻止攻击，使复制的计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第 311 个纪元进行另一次引导尝试。希望提供 Kimi 服务的主机需在第 310 个纪元声明其意图，并在新纪元开始前准备好节点。一个守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可将其委托给该节点。
...
```
2. 如果足够多的主机声明意图，请在第 311 个纪元开始前的约 500 个区块窗口内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
有关引导流程的更多详情：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定为使8xB300上运行Kimi时的共识权重与主模型MiniMax大致相当。该设定并非偏向任何硬件——MiniMax上已具备相同的权重。运行Kimi而非MiniMax的8xB300主机将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数设定为使8xB300上运行GLM-5.2时的共识权重与MiniMax大致相当。该设定并非偏向任何硬件。运行GLM-5.2而非MiniMax的8xB300主机将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预合格快照。** 在 `start_poc - deploy_window`，链将快照意向与委托，并发出建议性预合格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3可达性）。这使运营商可在配置前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由PoC开始时实际提交PoC存储提交的主机决定——而非仅凭声明意向。模型只有在满足合格条件时才会激活。

**若获批准的效果**

从第309轮开始（PoC开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是各模型满足引导合格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护者应支持本提案。

**主机所需操作**

1. **若要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 计划再次提供Kimi的主机应保留Kimi的配置并准备好在PoC 309时将MLNode切换回Kimi——在第308周期将进行一次恢复Kimi的投票。
...
* **添加GLM-5.2** —— 添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前衡量对该模型的需求，选择不提供该模型的主机不会受到惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快且停机时间最短的恢复方式。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以便快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是恢复其服务的最快路径，停机时间最短。下一个周期将进行恢复Kimi的投票。
* **Qwen3-235B — 退役（独立且无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi的情况无关，仅因同属PoC阵容变更而一并包含在此处。
...
1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前约有500个区块的窗口期可以安全切换，因为此时无cPoC。请现在预先下载FP8权重（如尚未准备就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——在第308周期将进行一次恢复Kimi的投票。


**下一步计划*** **恢复Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2 **而不设置不参与惩罚**，以便主机可以选择加入并提前测试模型需求，对于选择不提供该模型的主机不设任何惩罚。
...
* 第308个纪元随后形成：PoC开始约16:50 UTC，生效约17:25 UTC。
* 紧急提案需达到0.667的赞成阈值；参与率至关重要，请尽快投票。
...
4) 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每槽总计数。
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。如果链上提案获批，`versiond`将自动下载该二进制文件，验证sha256哈希，并在现有`versiond`容器内启动额外的`devshardd`进程。
...
这是首次仅针对devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且不会导致推理服务停机。
...
**MiniMax-M2.7现已在Gonka网络上线**

v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7与Qwen3-235B和Kimi K2.6一同成为活跃模型组，MiniMax组中获得的PoC权重现在以校准系数0.3024转换为共识权重。

MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续正常工作。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。

## 2026年5月26日

**升级已执行：v0.2.13现已在主网上线**

升级提案v0.2.13（提案ID 54）的链上治理投票已结束。
该提案已获**批准**，升级已在主网区块 `4267300` 成功执行。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，仍需为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元 `278`**。未采取任何操作的主机将从纪元278起，对其全部权重施加每纪元15%的惩罚。
...
- 治理：将创世守护者投票权降低至约25%，并设置链上法定人数为0.25。如果守护者弃权，非守护者需要在剩余的75%中达到约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 将创世守护者的调整投票权降低至约25%，并将链上治理法定人数设置为`0.25`。在守护者不投票的情况下，这使剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将启用Gonka主网与Ethereum之间的跨链传输（包括在Ethereum上包装GNK以及桥接USDC/USDT）。包装/解包脚本和操作员工作流程将另行文档化。
...
所有主机需验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再进行任何进一步操作。
...
- 运营商时间表：投票于5月22日22:12 UTC结束 → 升级高度约在5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax引导快照在start_poc − 500块时（约提前43分钟）→ 升级后下一个纪元边界进入首个MiniMax PoC阶段 → MiniMax惩罚在链纪元278时执行。
...
- 若获得批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认，并自动清理（保留最近3个纪元）。
- 若提案失败（未达到法定人数或 `no_with_veto` 超过⅓），链上不会发生任何变化，升级不会执行。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
建议在PoC/确认PoC阶段之外更新容器。要检查是否存在确认PoC：
```
...
第267纪元中PoC验证已成功完成，受影响的主机已成功返回活跃集合。

第266纪元的问题是由针对运行Kimi模型的主机的攻击引起的。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入第266纪元。
...
该问题已追踪到具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。

在第267纪元，主机成功恢复，PoC验证顺利完成。
...
由于现有协议规则，Kimi在第267纪元的活跃权重较低：单个模型的总权重不得超过前一纪元所有模型总权重的75%。
...
2. 监控下一轮次的PoC参与情况。检查您的节点是否按预期进入PoC，且活跃权重是否正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方可用，并预计通过社区代理和经纪人端点逐步恢复。
5. 在社区频道或此线程中分享相关日志或观察结果。如果您的主机在第266轮受到影响或在后续轮次中行为异常，这一点尤为重要。
...
看起来，PoC投票未能收集到本轮所需的投票数。确切原因尚未确认，社区正在积极调查此事。
...
- 包括：对Kimi（`0.78`）重新校准权重、新增模型 `MiniMaxAI/MiniMax-M2.7`、更新验证阈值、devshard存储重构，以及若干PoC/奖励修复和以太坊桥接主网激活。
...
**在第 278 个周期内 `MiniMaxAI/MiniMax-M2.7` 的参与选择**
...
要求所有主机验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您是否运行的是当前版本，然后再采取进一步操作。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一个新产生的错误推理会立即再次使其失效。该计数器现在在重新激活和即将晋升时重置。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授权中未包含`MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应经销商投诉。
...
评审人员可以在此处找到完整的升级提案、迁移细节、测试摘要和建议流程：
...
在api容器版本v0.2.11、v0.2.12和v0.2.12-api-post2中存在潜在问题。在容器重启后，9100、9200和9400端口上的服务器可能启动延迟很长。这延迟了api激活，导致部分矿工跳过确认PoC。
...
传输代理 `node1`、`node2` 和 `node3` 已被禁用。所有主网推理现在均通过 `node4` 路由，其采用新的 `devshard` 计费方式。
...
- **协议加固**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。

**对节点的指导**

- 部署、委托或明确拒绝新治理批准的模型（所含模型将在升级后两个周期激活）。请参阅[指南。](https://gonka.ai/docs/host/multi_model_poc/)
...
- 迁移：测试与迁移详情记录在 [v0.2.12 文档中。](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/upgrades.md)
...
每个 DAPI 会将 MLNode 配置本地持久化。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议强化。** 对保留节点（`POC_SLOT=true`）进行随机抽样以进行单次 PoC/CPoC 时间验证。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何从冷密钥向热密钥授予治理投票权限。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为按模型的 PoC 组。每个治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位完整熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使传统验证者 slashing 与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 一旦治理批准新模型，需部署、委托或明确拒绝每个治理批准的模型（包含的模型将在升级后3个周期内激活）
...
下一个链上软件版本v0.2.11的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时开始大量剪枝活动，造成不必要的运营压力。
...
对于对本次 PR 审查有实质性贡献者，可在下一次升级中提出奖励。
...
这故意是一个早期且受限的版本。提出此版本供主网审查和有限生产测试，并非因为它被认为是已完成的，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将几乎同时开始剪枝活动，从而造成可避免的操作压力。
...
对于已经使用代理层的团队，这可能是简化架构、转而依赖原生行为的好机会。实际上，这应能带来更清晰的集成模式和更易维护的系统。
...
如果您无权访问持有投票权的冷密钥，则建议提前安排投票委托。请联系该密钥的所有者，请求授权您代为投票。若无此授权，其他账户无法提交投票。
...
现在提供一个新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍处于活跃状态的请求导致潜在权重下降的风险。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此升级根据实际 PoC 经过时间对 PoC 参与者权重进行标准化，以减少区块时间漂移效应，并使权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入一个3000个区块的宽限期，在此期间无确认 PoC，且对升级所在周期的错过率和无效率阈值放宽。
...
**警告：** 抵押参数将 Regardless of 投票结果生效。若此提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了3000个区块的宽限期，在此期间无确认PoC，并降低了升级时段的错过率和无效率阈值。
...
正如AMA期间讨论并基于社区成员提出的论点，建议初始抵押要求较低，惩罚机制也最小化。
...
这意味着即使在一个周期内被惩罚，矿工最多只会损失其抵押品的0.5%。所需抵押品仅占每日奖励的约24%。

提案提交投票后，将另行发布公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押品参数将在第180个周期自动激活，而非上述列出的参数。
...
当前正在评估此重叠的影响。初步观察表明，没有节点记录PoC提交，导致该周期累积权重为零。
...
此升级根据实际 PoC 经过时间标准化参与者权重，以减少区块时间漂移效应，并使权重结果与实际执行时长保持一致。
...
在更新后的权重分配生效后，跟踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保其与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新——补丁可用**

现已提供补丁以解决导致PoC周期中区块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为，并安全恢复区块生成。

**需要采取的操作**

请主机尽快应用补丁，以确保正确的PoC验证行为，并安全恢复区块生成。
```
# Download Binary
...
根据当前评估，旨在处理此情况的机制可能无法正常运行。为防止在不确定或不安全的条件下完成验证者最终确认，网络在验证者选择之前已停止运行。
...
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不包含在即将来临的纪元的参与集合中。
...
- 身份绕过：将 `epochId` 绑定到签名，以验证对应正确的周期。
...
!!! note 
    未能同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去 PoC v2 参与资格。PoC v2 的权重分配过渡仍受观测采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8模型可用性变化和即将推出的PoC v2更新来升级其ML节点。ML节点配置在第155个纪元开始符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。所有此前支持的模型均从活跃集合中移除，除非：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估向 PoC v2 过渡的准备情况。一旦在活跃 Host 中观察到足够的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性，不会触发任何自动网络变更。
...
该延长也为主机提供了时间，以准备符合新 PoC 要求的设置，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步执行更新。
...
1) 网络将运行一个 `allowlist`，由在多个纪元中满足以下条件的参与者组成：
...
2) 此前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块2,222,222处结束。
...
若获批准，网络将临时运行一个 `allowlist`，包含在先前纪元中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))
- 观测到的PoC权重低于使用同类硬件的其他参与者所表现的权重的150%。
此前表现出持续偏离这些模式的参与者在稳定窗口于区块2222222结束前不属于 `allowlist`。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。
...
- 开发者地址注册：在稳定期间，暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续有效至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备性变更，使未来可通过治理机制控制参与者入网和推理执行，而无需额外的软件升级。本提案不启用任何此类治理激活的限制，需经额外的治理投票决定。

**第 117 期奖励分配**

本提案涵盖与链暂停（第 117 期）相关的两项奖励分配：

- 在第 117 期活跃但未收到该期奖励的节点，将补发该期的遗漏奖励。
- 所有在第 117 期活跃的节点，将获得额外支付，金额为第 117 期奖励的 1.083 倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个漏洞，导致不支持格式的推理请求即使已被处理，也没有被标记为完成。以下更新将解决此行为。
