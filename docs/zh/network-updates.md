...
!!! note "关于本页面"
    ...
    本页面的内容不保证完整。如需获取最新信息，包括治理投票的启动及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。

## 2026年9月11日

**提案通过：批准 Quant Mesh Limited 作为账本集成合作方并拨付初始账本集成款项**

提案 ID 为 102 的链上治理投票已结束。

该提案已获通过。Quant Mesh Limited 被正式批准为 Gonka-Ledger 集成的法律签约方，并从社区资金池中拨付 350,350 USDT 至 `gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该笔拨款涵盖第一阶段启动费用 275,000 USDT、第一年年度激活费用 75,000 USDT 以及 350 USDT 的处理成本。授权与资金拨付严格限于完整提案中描述的账本集成。
...
提案 ID 为 101 的链上治理投票已结束。

该提案已获通过。`zai-org/GLM-5.3-Flash` 现已获得治理批准，成为 PoC 模型集的一部分，而 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8` 已从 `poc_params.models` 中移除。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：其 PoC 参数和权重保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，并联合提出。测量数据及阈值设定的依据详见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起 `penalty_start_epoch` 生效，因此未明确选择的主机将面临不参与惩罚。若您计划提供该模型，尽早声明意向将有助于团队更好地组建。

最终投票结果：赞成 309308，反对 0，否决 0，弃权 3168。
...
提案已获批准，`v4.1`已添加至批准的devshard版本列表中。`versiond`将下载二进制文件，将其SHA-256与链上存储的值进行验证，并启动运行时。无需手动主机操作。
...
`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：其 PoC 参数和权重保持不变。所有其他链上参数均保持不变。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起，`penalty_start_epoch` 生效，因此未明确选择的主机将面临不参与罚金。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票参与率、多数赞成票，且带否决的反对票少于 33.4%。
...
提案 ID 100 将 `v4.1` 添加至批准的 devshard 版本列表中。

这仅是 devshard 的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，将其 SHA-256 与链上存储的值进行验证，并启动运行时。
...
投票将于 **2026 年 9 月 10 日 22:36 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成，且反对票（含否决）低于 33.4%。
...
如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。

## 2026 年 9 月 4 日

**提案通过：支持 Gonka 参与 TOKEN2049 新加坡**

提案 ID 为 99 的链上治理投票已结束。

提案已获批准。它将从社区池中拨出 100,000 USDT，用于支持 Gonka 参与 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金涵盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
重启时，`v4.0.1` 从 nonce 1 恢复了所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 在 60 秒就绪超时后终止了子进程，导致客户端出现 HTTP 502 错误。
...
提案已获批准。它将 `deepseek-ai/DeepSeek-V4-Flash-0731` 的 `weight_scale_factor` 从 0.214 提高至 0.246。所有其他模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，结果为 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的法定人数）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。
...
**仅 devshard 升级的 PR 现已开放评审**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，且预计不会造成推理服务的停机。若通过治理流程批准，新的 devshard v5 运行时将与现有的 v4 运行时并行运行。

**主要变更**

1. v5 的主要目标是高度同步：每个主机和排序器在托管日志中维护一个经过签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard 现在可以确凿地知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 高度时间戳为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 时间戳已存在于所有先前携带时间戳字段的消息中。后续版本可以用高度时间戳替换墙钟字段，如 `confirmed_at` 和 `started_at`。这解决了时间未来攻击问题，并为在协议层面处理流超时打开了途径。本版本携带了这些时间戳，但尚未将超时和封印决策切换至它们。

3. 通过所有插槽的往返心跳机制已添加，因此安静会话仍能定期同步高度，而繁忙会话则可通过普通推断时间戳实现相同的节奏。

**行动项**

请审阅 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。

有意义的审阅贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一次升级周期中获得社区奖励。

本次仅为请求 PR 审阅，不启动正式投票。
...
提案详情和投票可通过 `inferenced` 进行，任何活跃节点均可使用：
...
该提案已获批准。它调整了在样本量较小时无效推断检查（SPRT）的行为，避免在周期初期数据不足时误触发。主机无需采取任何操作。
...
在周期初期，数据量尚不足，因此无效推断检查（SPRT）可能在样本量足够前仅因少量无效结果就触发。本提案使其更稳定：

- 将 `invalidation_h_threshold` 提高至40，并将 `bad_participant_invalidation_rate` 设为0.18。
- 在大样本量下，阈值接近10%的无效率，因此低于该阈值的诚实参与者不会被惩罚。真正的违规者仍会触发它（约360个样本时为18%，约1650个样本时为12%）。
- 小样本无法触发：至少需要32个被标记的无效结果，且在500个样本时观察到的比率必须超过16%。
- 未使用的停机时间SPRT已关闭。周期结束统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于**2026年8月21日06:04 UTC**（约12小时后）结束。如能投票，请务必参与。如果未能及时达到紧急阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。如果转为常规提案，您需要重新投票。
...
一个已稳定的网关版本目前正运行在一个网关上，并将用于第一阶段结算。其统计信息，代表了部分网络流量，可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关视角显示每个主机执行的工作。在结算启用后，Misses和Invalid指标将用于链上。根据当前主网阈值配置，错误率低于10%的主机不会触发任何惩罚或削减，但作为协议开发过程的一部分，仍应进行调查。

错误率较高的主机应在结算开始前解决这些问题。无重大问题的主机无需采取任何操作。

仪表板还显示了网关内部细节，例如特定主机的请求失败原因或无效上下文长度，有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需操作。

**下一步：****1) 接下来的24小时。** 主机应检查其统计信息并解决任何关键错误或配置问题。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口后的第一个周期。** 将在当前网关上手动启动结算，首先针对较小模型。该过程将被密切监控，以尽量减少误报记录。Kimi将稍后加入，因为其数据在负载下目前可靠性较低。

**3) 接下来的1-3天。** 将监控结算结果

**4) 一旦此阶段稳定。** 其他经纪人可部署已稳定的网关版本并开始以受控模式进行结算。

**5) 在所有经纪人成功验证后。** 结算覆盖范围将逐步提升至100%。

该 rollout 有意逐步进行，以便在结算扩展到整个网络之前识别和解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的启动阶段已完成。自链周期360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后由[提案98](#august-29-2026)提升至 0.246）转换为共识权重。

现在对DeepSeek实施按模型参与强制执行。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需任何操作——现有设置将继续生效。尚未做出选择的主机请尽快选择，以避免每轮惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（即正在服务Kimi或MiniMax的节点，因为所有已运行DeepSeek的节点均已升级至3.0.16），请在方便时将这些节点升级至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。先更新少数节点，确认其稳定后再逐步更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。

MLNode镜像已在[`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)的`vllm-0.25.1-upgrade`分支中锁定为3.0.16。
...
将活跃的社区销售合约和所有包装代币实例迁移至新存储的CosmWasm代码，并为未来的实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。
...
**DeepSeek V4 Flash引导：意向阈值已达成——请在第360轮快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731`在第359轮快照中未能满足预资格条件。此后，实时意向和委托权重已满足`V_min`、`W_threshold`以及**>2/3可达性**（DeepSeek意向权重加上委托给这些主机的权重）。下一次**官方**检查是第360轮引导快照——请在此前保持多样化的委托，以确保快照有效。

**将运行DeepSeek的主机**

1. 请在区块**5,551,615**前保留或提交`MsgDeclarePoCIntent`：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 如果模型已满足预资格，请在PoC开始前约500个区块的部署窗口内（区块**5,552,115**）部署DeepSeek。使用vLLM 0.25.1 + 发布候选节点配置。

**继续运行其他模型的主机**

在区块**5,551,615**前，提交委托（有助于引导可达性）或拒绝。从第**360**轮起，DeepSeek `penalty_start_epoch`生效——未明确选择的主机将面临不参与惩罚。
...
委托主机：选择**非守护者**的DeepSeek意向主机，并将权重分散至独立目标——参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。

**关键时间点*** 第360轮快照（`start_poc − deploy_window`）：区块**5,551,615**——约2026年8月14日 01:24 UTC
* 第360轮PoC开始：区块**5,552,115**——约2026年8月14日 02:09 UTC
* DeepSeek不参与惩罚从第**360**轮开始
...
运行该格式的模型需要新的 API 版本 v0.2.15-post5。要安装它：
```bash
...
```
该技能部署模型，测量 PoC 吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。如果通过，则表示一切正常。

如果您希望提供 DeepSeek 服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免非参与罚金。

注意：明天将发布另一个带有更好日志的 MLNode 版本，因此请留意后续更新。
...
- `deepseek-ai/DeepSeek-V4-Flash-0731` 成为可用的 PoC 模型。
- `MiniMaxAI/MiniMax-M2.7` 保持为基础模型。
- `moonshotai/Kimi-K2.6` 保持可用且不变。

**主机所需操作**

1. 声明意向。提案 94 已通过，在引导快照前（区块 5,536,224，约 2026 年 8 月 13 日 02:40 UTC / 8 月 12 日 19:40 PDT），请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的 MLNode。在 PoC 359 开始前的约 500 区块安全窗口内（区块 5,536,724，约 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），配置并切换您的 MLNode 至 DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。此窗口内切换是安全的，因为其中没有 cPoC。
注意：在 Blackwell GPU 上，为获得最佳性能，您可以使用重新打包为 fp8 + nvfp4 而非 fp8 + fp4 的模型，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在启动前发布并共享更新的 API 二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的 DeepSeek 主机上。参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 若要保留当前模型。在第 360 个周期前无需操作。从第 360 个周期起，若您未提供 DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用 15% 的非参与罚金。

区块高度是精确的；时间是估算值，将在接近周期时确认。[更多关于 DeepSeek 启动的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案将 DeepSeek V4 Flash 作为新的 PoC 模型添加到网络中。现有参数或模型均未修改，唯一更改是新增模型条目。
...
DeepSeek 的 0.214 系数为在 B300 上托管 DeepSeek 提供轻微优势。对于其他硬件，产生最高 PoC 权重的模型保持不变。
...
3. 部署窗口：声明意向的主机为其 MLNode 进行配置。
4. PoC 开始：成员资格由在 PoC 开始时提交 PoC 存储提交的人决定，而非由声明意向的人决定。

最终的 MLNode 版本和设置说明将在投票结束后发布。

更多关于引导工作原理的内容：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)

**若获批准的影响**

在符合引导资格的前提下，从第 359 个纪元（PoC 开始时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的 PoC 模型。现有模型保持不变。DeepSeek 的不参与惩罚从第 `360` 个纪元（`penalty_start_epoch = 360`）开始。

**主机所需操作**

1. 若要提供该模型：投票结束后声明意向，然后配置您的 MLNode（vLLM 0.25.1 + release-candidate 配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第 360 个纪元前无需操作；从第 360 个纪元起，若您未提供该模型，请提交 `PoCDelegation` 或 `PoCRefusal` 以支持 DeepSeek。
3. 在截止日期前对提案 94 进行投票。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
**升级已执行：devshard 运行时 v4.0.1 现已上线**

针对 `v0.2.15-devshard-v4.0.1` 运行时升级提案（提案编号 93）的链上治理投票已结束。

该提案已获批准，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换的二进制文件——无需重启主网或主机手动操作。
...
这只是一个性能和资源管理补丁，仅此而已。协议、推理、验证和经济行为均保持不变。更改内容：时段跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与时段修剪保持一致。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整的链软件升级。若获批准，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动操作主机。
...
该提案已获批准，并于主网第 5316315 块成功执行升级。该提案还批准了 devshard v4 运行时，它现在与现有的 v3 运行时并行运行。
...
v4 是首个支持多实例高可用的版本：在共享 Postgres 上，通过 versiond-router 后部署 N 个 versioned / devshardd 副本，采用粘性会话路由和验证租约独占机制。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，提升了系统的整体稳定性和可维护性，并通过移除过时行为、将交易处理集中于当前支持的架构，使未来的开发和升级更加便捷。
...
1. 下载已批准的二进制文件。
2. 将其 SHA-256 哈希值与链上存储的值进行比对。
3. 启动 v4 `devshardd` 运行时。
4. 通过相应的 devshard 路由配置提供该运行时。

Devshard v4 将与现有的 v3 运行时并行运行。在经纪人和主机运营商准备并验证其 v4 部署期间，现有的 v3 流量可以继续运行。

v4 的批准本身并不要求所有流量立即切换。在运行时启动并验证后，可以逐步进行迁移。
...
- 投票截止：2026 年 7 月 30 日 07:08 UTC / 2026 年 7 月 30 日 00:08 PDT
- 提议升级高度：5316315
...
- 投票截止：2026 年 7 月 23 日 00:02 UTC
- 升级高度：5195700
- 预计升级时间：2026 年 7 月 23 日约 03:45 UTC
...
devshard 部分准备了 v3 运行时，使经纪人可以在链升级期间无需依赖已废弃的经典 API 路径提供推理服务，从而提高 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。

如需更多详情，请参见拉取请求：[https://github.com/gonka-ai/gonka/pull/1267](https://github.com/gonka-ai/gonka/pull/1267)

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需手动更新其 `api` 或 `node` 容器作为升级的一部分。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
- 现在/主网升级前——更新您的API和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将`api`二进制文件和桥接器镜像更新至0.2.14-post3。如果您的`api`二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非PoC或cPoC期间执行此步骤。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间（主网上Cosmovisor对`application.db`的备份通常为数十GB，需提前确认）。有关如何安全删除`.inference`目录中的旧备份，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite仍将作为默认选项，并自动清理（保留最近3个周期）。

## 2026年7月20日

**[仅针对devshard的PR](https://github.com/gonka-ai/gonka/pull/1482)现已开放评审**

Devshard升级独立于主区块链更新其运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程获得批准，新版本的devshard将与现有的v3运行时并行运行。

**关键变更**

- v4的主要目标是在主机故障和升级期间实现devshard主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4是首个专为多实例高可用设计的版本：在共享Postgres后端，通过versiond-router部署多个versioned/devshardd副本，采用粘性会话路由和验证租约独占机制。网关仅通过gRPC与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅委托方通过签名聊天绑定。当治理发布同版本名的新二进制文件（名称不变，仅sha256变化）时，versiond可执行蓝绿切换并排水，确保正在进行的工作（包括SSE）在旧版本上完成。
- v4还修复了错误和安全问题。

**操作项**

请审查PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现任何问题、疑问、改进建议、边界情况或潜在漏洞时留下评论。
...
* 在区块 **5,105,276** 之前（2026年7月17日 ~12:05 UTC）声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第331个周期PoC开始前约500个区块的窗口内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换为Kimi。
...
同一提案从`devshard_escrow_params`中移除`approved_versions`的v1和v2版本。一旦批准，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。此举消除了旧运行时容器长期存在的内存增长问题——这也是7月15日网络节点OOM事件的根源。

**这标志着v3切换最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关将在提案通过后停止提供推理服务。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间进行委托时：**不要委托给守护节点**；将委托分散到独立的Kimi主机上。有关最新委托指南，请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（紧急，0.667通过阈值；参与率重要，请及时投票）。
...
`moonshotai/Kimi-K2.6`在epoch 328–329中失去了PoC验证多数（详见[此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其以最小停机时间恢复的最快方式——与6月提案78的恢复路径相同。

该提案将在下次PoC前将`moonshotai/Kimi-K2.6`从PoC参数中移除，随后立即重新添加，Kimi将在epoch 331开始标准引导流程。
...
* 其中一台服务器上运行 Kimi 的 MLNode 因供应商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障立即移除了这些投票。
* 第二台服务器因内存不足而丢失了其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。监护人决胜机制未启动：目前没有模型投票权重的监护人被过滤出决胜机制。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与纪元 306–307 事件类似。纪元 329 开始时的投票详细信息仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个纪元）重新添加：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。随后将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次通过标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布说明。

**委托指导变更——请阅读**

之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将监护人节点作为委托目标。此建议现已被撤销：

* **请勿委托给监护人节点。** 监护人是 PoC 验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。目前正在讨论协议级限制。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托给运行该模型的您自己的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在开发中的修复*** PoC 验证中的监护人投票（监护人始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。在完全迁移后，v1/v2 将被移除且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（将发布公告），请对其进行投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非监护人委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量至 `/devshard/v3`。这将修复内存增长问题，并在移除 v1/v2 前必需完成。
...
这是一个仅针对 devshard 的运行时升级。它独立于完整链软件升级，不需要链二进制升级。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换至 v3。在此类升级期间，无需进行主网重启或手动主机操作。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/PR) 针对下一次链上软件升级 v0.2.14 已开放供审查。

主网链/API 工作重点包括：PoC 重复工件保护、早期共享检测、经典推理 API 废弃（禁用主网 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收者路由以及升级时的安全修复。

devshard 部分准备了 v3 运行时，使代理在链升级期间无需依赖已弃用的经典 API 路径即可提供推理服务，同时提升 RAM 利用率并支持 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布 devshard v3，并在主网链升级前提出并部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供服务。
...
根据历史数据，此前未观察到此类攻击。然而，在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 正在使用此攻击。

针对即将发布的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过加强 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供 Kimi 服务的主机必须在第310轮期间声明其意向，并在新轮次开始前准备好节点。一个守护节点也将运行 Kimi，因此不希望运行它的主机可以将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换至 Kimi（PoC 开始时间约6月28日 ~12:47 UTC）。
...
有关启动机制的更多内容：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行 Kimi 时，其共识权重与主模型 MiniMax 大致相同。该设置并非为了偏向任何硬件——同样的权重已在 MiniMax 上可用。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的增益。
...
常规启动流程从第309轮开始。由于投票将在 PoC 前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于启动的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上运行 GLM-5.2 时，其共识权重与 MiniMax 大致相同。该设置并非为了偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的增益。
...
1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 服务的主机需在 `start_poc - deploy_window` 的启动快照前为该模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3 可达性）。这使操作者能在配置硬件前确认模型是否可行。
...
从第309轮开始（PoC 开始时间约6月26日 15:25 UTC；生效时间约16:00 UTC），前提是每个模型均满足启动资格：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动。由于每轮约22.8小时，标准的48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计会支持本提案。

**主机需采取的行动**

1. **要提供 Kimi：** 在投票结束后的**立即**声明 `moonshotai/Kimi-K2.6` 意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，期间切换是安全的，因为该窗口内无 cPoC。
...
```
**截止日期*** 提案79投票结束：*2026年6月26日14:18:58 UTC*（加速）。
* 加速提案要求0.667的通过阈值；参与率很重要，请尽快投票。

## 2026年6月25日

**提案78已通过：`MiniMaxAI/MiniMax-M2.7`现为唯一PoC模型；Kimi K2.6和Qwen3-235B已移除**

提案78的加速投票已通过。这些变更从第308个纪元起生效。
...
自第308个纪元起，`MiniMaxAI/MiniMax-M2.7`是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6已不再活跃。

**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换后才能获得本纪元的cPoC奖励。
* 打算再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308纪元期间将进行一次投票以恢复Kimi。

**接下来的内容**

在第308纪元期间将进行一次加速投票，合并两项变更，均于第309纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导过程。
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，以便主机可自愿选择参与，提前衡量模型需求，不提供服务的主机不会受到惩罚。

## 2026年6月25日

**加速治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出组，Kimi无法进入下一纪元。现在将其从活跃集合中移除并重新引导，是最快恢复且停机时间最短的方式。

一项加速提案现已上链，以便在下一纪元开始前生效。
链上提案在一次投票中执行以下操作：
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的方式。下一纪元将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立无关变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的模型阵容调整。它与Kimi的情况无关，仅因同为PoC阵容变更而一并包含在此处。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如果尚未就位），以避免在epoch边界时遭遇Hugging Face速率限制。
...
* **GLM-5.2** —— 后续提案将添加GLM-5.2，且**不设不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不为未提供该模型的主机施加惩罚。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
* 第308个纪元将在不久后形成：PoC开始时间约为16:50 UTC，生效时间约为17:25 UTC。
...
- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长期运行的会话将所有已服务的推理保留在RAM或状态中。
- 通过OpenTelemetry和Prometheus添加了内部devshard追踪和指标。
- 通过Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor添加了join-stack可观测性。
- 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个插槽的总数。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长期运行的会话将所有已服务的推理保留在RAM或状态中

2. 通过OpenTelemetry和Prometheus添加内部devshard追踪和指标

3. 通过Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor添加join-stack可观测性

4. 将每个推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每个插槽的总数

5. 在纪元变更时修剪旧纪元存储，将SQLite/Postgres模式设置移出热路径，并为每个进程选择恰好一个存储后端

**升级计划**

devshard运行时通过链上参数提案进行升级，而非完整链软件升级。

该提案在`DevshardEscrowParams.approved_versions`中注册一个新条目。

发布版本将`devshardd`二进制文件作为Gonka发布工件。如果链上提案获批，`versiond`将自动下载该二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动一个额外的`devshardd`进程。

新进程通过`/devshard/v2`前缀提供服务。现有的v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。在此类升级期间，无需重启节点容器或手动执行主机步骤。
...
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从第 278 个纪元起，未采取任何操作的主机会对其全部权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 的权重从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考基准在 vLLM-0.20.1 之后的吞吐量。此调整 **仅适用于您共识权重中由 Kimi 导出的部分**；您的 Qwen 导出权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过的提案的重新提交，现包含若干更新。

- 包含：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（见下方专节）。
- 该提案将升级后的宽限期延长至 3000 个区块，以便主机在新快照逻辑稳定期间不被惩罚。
- 治理：将创世守护者投票权降低至约 25%，并将链上投票通过门槛设为 0.25。若守护者弃权，非守护者需在剩余的 75% 中达到约 ⅓ 的参与率才能满足门槛（参见 inference-chain 部分）。
...
- 将 devshard 非ce 限制从 `20_000` 提高到 `1_000_000`。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
**在第 278 个周期（处罚从此开始）前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
请所有主机验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再执行进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

请主机更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先参考加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
如果您无法直接访问拥有投票权的密钥，或希望使用其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何从冷密钥向热密钥授予治理投票权限。
提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用。可用节点包括：
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期剩余阶段跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约 43 分钟前）→ 第一个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 该提案将有意在升级高度到升级纪元结束（10000 块宽限期）之间跳过确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最后 3 个纪元）。
- 如果提案失败（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上无任何变化，升级将不会发生。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何操作。
...
第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击加上若干相关条件，阻止了许多参与者进入第266轮。
...
该问题已追溯到具有非标准语义的推理请求。该攻击向量影响了运行Kimi模型的主机，并干扰了其中许多主机的PoC参与。
...
2. 监控下一轮的PoC参与情况。检查您的节点是否如预期进入PoC，且活跃权重是否正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方恢复，预计将通过社区代理和经纪人端点逐步恢复。
5. 在社区渠道或此线程中分享相关日志或观察结果。如果您的主机在第266轮受到影响，或在后续轮次中行为异常，这一点尤为重要。
...
这可能有助于补偿委员会收集PoC参与的证据，并更准确地核算受影响的贡献。

**调查进行中**

社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更多明确信息，将发布更新。
...
- 必要准备：桥接容器检查、MiniMax决策、仪表板更新、投票。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理最低通过票数设为 `0.25`。在守护者不参与投票的情况下，其余 75% 投票权的有效最低通过票数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器，请先确认您运行的是当前版本，再进行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效载荷存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个周期）。
...
部分主机可能已部署桥接容器。在这种情况下，请先检查您运行的是否为当前版本，再采取进一步操作。
...
本次仅为拉取请求审查请求，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天启动。

**关键变更****inference-chain**

- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集合。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被削减。修复方案存储了一个纪元的可确认模型快照及权重缩放因子，并在所有确认和奖励权重计算中使用该快照。
...
- Devshard 现在支持更大规模部署的共享存储选项，可使用 Postgres 作为主存储，SQLite 仍作为本地备用。
...
- 桥接工具处理 Sepolia 标志，并将 Gonka BLS 密钥/签名转换为以太坊合约预期的 EIP-2537 格式。
- 添加了用于 GNK 和包装代币桥接操作的脚本。

审查者可在以下链接找到完整的升级提案、迁移详情、测试摘要和建议流程：
...
在上一轮训练中，发现Kimi-K2.6在解析某些响应时存在一个轻微的bug。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实践中难以满足。为避免Kimi-K2.6在未来的轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。

安全模型保持不变：PoC验证本身未改变，仍需超多数验证算力才能接受结果。

为使该变更能在下一轮PoC前生效，提案已加速处理。投票将持续12小时。
...
该过程由网络各主机协同完成：基础设施已准备就绪，意图已提交，委托与拒绝已设置，部署已测试。
...
提交了PoCIntent的主机应在PoC于块`3874496`启动前，至少将一个MLNode从`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`切换至`moonshotai/Kimi-K2.6`。
...
这标志着网络的重要里程碑：`devshard`已上线并具备生产就绪能力。`node4`是今后推荐的公共网关。

**需执行操作**：请将您的端点更新为`node4`。

## 2026年5月2日

今日的预资格验证未通过，对于`PoCIntent`权重低于30%的主机，请保持MLNode使用`Qwen235B`，并于明天提交下一轮的意图。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
此清理过程**必须在升级前完成**。如果在升级时您的节点配置包含不受支持的模型，**节点将被拒绝并离线。**

版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留之前强制启用的模型和 Kimi。
每个 DAPI 在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此 `/admin/v1/nodes` 即使持久化配置中仍包含额外模型，也显示为干净。版本 0.2.12 停止了这种裁剪，直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个包含清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。其余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需要手动修复。
...
- **协议加固。** 对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，之前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG dealer 共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 确保冷钱包账户持有足够的金额（例如 100 GNK），以覆盖自动授予的费用配额上限。
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于接近 PoC 开始运行的节点，稳定性有所提升。

此更新修复了在 PoC 开始附近的一个边缘情况，该情况过去可能导致在某些条件下性能下降。
...
建议主机查看 [GitHub](https://github.com/gonka-ai/gonka/pull/813) 上的提案并投票。
提前下载二进制文件有助于避免在升级窗口期间依赖 GitHub 的可用性。
...
目前，通过每条推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本并非因其已完善，而是因为此类系统需要尽早暴露于真实网络条件下，部分问题仅靠本地测试难以发现。当前实现已设计为不会对矿工奖励造成负面影响。
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中将同时出现大量剪枝活动，造成不必要的运维压力。
...
目前，通过每笔链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已被认为完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时开始剪枝活动，造成可避免的操作压力。
...
对于已经使用代理层的团队，这可能是简化架构、转而依赖原生行为的好机会。实际上，这将带来更清晰的集成模式和更易维护的系统。
...
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求他们授权您代表其投票。没有此授权，其他账户无法提交投票。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 开始时中断正在进行的推理请求，以降低 PoC 启动时仍处于活跃状态的请求导致权重可能下降的风险。
...
- 确保 [抵押](https://gonka.ai/host/collateral/) 配置正确，否则从第180个周期开始，PoC 奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍是调整参数的机制，但若无替代方案获批，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各周期间可能发生波动。较小的权重可能经历相对更大的波动。为避免在周期边界处出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为将 PoC 权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选定。如实施所示，有效的 PoC 参考窗口比先前的名义假设长约5个区块。
...
此前，PoC 权重计算依赖于名义的周期持续时间假设。引入实时归一化后：

- PoC 持续时间与实际区块生产时间对齐
- 权重更准确地反映实际计算时间

由于有效的归一化窗口比之前的名义模型长约5个区块，因此每个周期重新计算的权重成比例降低。
...
!!! note "兼容性说明"
    ...
    **按真实时间标准化 PoC 权重**
    ...
    此次升级根据实际 PoC 经过时间标准化 PoC 参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。

    **启用 Qwen235B 工具**

    此次升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 添加了工具调用参数 ( `--enable-auto-tool-choice` , `--tool-call-parser hermes` )，并将验证阈值设为 0.958。
...
**宽限期：** 升级后引入了一个3000区块的宽限期，在此期间不进行确认 PoC，并对升级当期的错过率和无效率阈值采取较宽松的标准。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的 0.5%。所需抵押物仅占每日奖励的约 24%。

**警告：** 抵押参数将 Regardless of 投票结果生效。如果本提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项对 PoC 验证的重大优化（默认关闭），并实现基于真实时间的权重标准化，以提升网络公平性和可扩展性。
...
此升级通过实际PoC耗时标准化参与者权重，以减少区块时间漂移影响，并使权重结果与实际执行时长保持一致。

**启用Qwen235B工具**

此升级为`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`添加了工具调用参数（`--enable-auto-tool-choice`、`--tool-call-parser hermes`），并设置验证阈值`0.958`。
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并降低了升级时段的错过率和无效率阈值。
...
- 请检查任何解析 `inferenced` CLI 输出的脚本。由于IBC栈升级至v8.7.0，枚举和int64/uint64值现在以字符串形式编码。
- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中的旧备份的指南，请参见[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用了大量磁盘空间，可应用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理技术。
- 升级后，Postgres 可作为本地负载存储的选项。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的0.5%。所需抵押物仅占日奖励的约24%。

提案提交投票后将另行公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到验证，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止运行。这是因为节点运行的是不再与网络兼容的旧版二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一个链上软件升级 v0.2.10 已开放审查。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约两天。
...
此升级根据实际 PoC 持续时间标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
2. 社区审核该PR后，预计从此分支创建v0.2.10版本，并提交此版本的链上升级提案，以启动正式的治理投票流程。
3. 如果链上提案通过，此PR将在升级在链上执行后合并。
...
在更新权重分配生效后，追踪器（仪表板）维护人员可能希望审查第 159 期及之后的系数，以确保与当前 PoC v2 权重分配保持一致。
...
根据当前评估，旨在处理此情况的机制可能无法按预期运行。为防止在不确定或不安全的条件下完成验证器最终确认，网络在验证器选择前已停止运行。
...
- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时能够及时执行后续步骤或缓解措施。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不包含在即将到来的纪元的参与集合中。
...
下一个链上软件版本v0.2.9的升级提案现已上链并开放投票。若获得批准，该提案将启用PoC v2用于权重分配，并通过链上治理完成向传统PoC机制的过渡。
...
- 服务其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换到`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在因cPoC惩罚而减少或排除奖励的情况下，未计入部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
下表显示了上一纪元具有权重、在当前纪元提交了 PoC nonce，但未能参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
本指南说明主机应如何根据 v0.2.8 模型可用性变化及即将推出的 PoC v2 更新来升级其 ML 节点。从第 155 个周期开始，ML 节点配置需符合 PoC v2 要求。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个周期后安排。迁移阶段结束后，不符合配置要求的 ML 节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8` 在迁移期间受支持，但不参与 PoC v2 就绪或权重分配。参与 PoC v2 需要提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
仅当链上治理提案获得批准时，PoC v2 才成为权重分配的活跃方法。
    在此提案获得批准之前：
...
- 包含双模式迁移策略：V1 用于常规 PoC，V2 用于在部署期间跟踪 Confirmation PoC。

**模型可用性变更**

作为 v0.2.8 升级的一部分，支持的模型集已更新。所有先前支持的模型均从活跃集合中移除，但以下模型除外：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估是否具备过渡到 PoC v2 的条件。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。在网络切换至单模型配置后，未同时满足两项条件的节点将不被视为符合 PoC v2 参与资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
关于稳定期延长的最近治理投票已成功通过。稳定期现已正式延长，以便进行额外的测试和网络升级。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步进行更新。

**投票范围**

如果获得批准，网络将暂时继续在现有的 `allowlist`（包含未表现出非标准硬件行为的主机）下运行。

开发人员 `allowlist` 将以相同的偏移量延长，并持续生效至区块 2459375。
...
`allowlist` 具有固定过期时间，不会延续至延长的稳定期之外。一旦 `allowlist` 在区块 2443558 过期：
...
`Allowlist`到期：自动在区块 2,222,222 到期。
...
2) 之前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直到区块 2,222,222 的稳定窗口结束。

**执行特征**

- `allowlist` 从下一个纪元（第135纪元）开始生效
...
随着区块生产恢复正常，网络即将进入一个短暂的稳定期，为下一步增长做准备。

此次投票定义了稳定期内参与者的 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为一直符合网络预期的参与者集合。

**投票范围**

若获批准，网络将暂时使用一个 `allowlist`，包含在先前纪元中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式集进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），以及
- 观察到的PoC权重低于使用类似硬件的其他参与者所表现的权重的150%。
之前持续偏离这些模式的参与者在稳定窗口结束于区块 2222222 之前不属于 `allowlist`。
...
- 开发者地址注册：在稳定期内暂停新开发者地址注册。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。

**第117轮奖励分配**

本提案涵盖与链暂停（第117轮）相关的两项奖励分配：

- 在第117轮期间活跃但未收到奖励的节点，将补发该轮的缺失奖励。
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的发放，统一应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意破坏或施压系统的行为引起的。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的韧性。
...
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续至区块2,294,222（约19天）。

**治理控制机制**

本升级包含的准备工作为未来通过治理控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的支付，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于常态的推理遗漏率。
在许多情况下，这是由一个缺陷引起的：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
