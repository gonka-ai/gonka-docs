...
提案获批后，v3 现在与现有的 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换至 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需重启主网或手动执行主机步骤。

**操作事项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这使他们能够在链升级运行时继续提供推理服务，而无需依赖已弃用的经典 API 路径。

**关键变更**

- 准备经纪人，在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。
- 改进了 RAM 利用率。
- 修复了网关运行时行为。
...
这是一个仅限 devshard 的升级。它独立于全链软件升级运行。一旦获批，v3 将与现有的 devshard 运行时并行运行。

v3 运行时为经纪人做好准备，使其在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。如果链上提案获批，`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
新进程通过 `/devshard/v3` 前缀提供服务。现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换至 v3。在此类升级期间，无需重启主网或手动执行主机步骤。
...
将从该分支单独发布并部署 devshard v3 版本，然后才进行主网链升级。提前将推理流量切换至 `/devshard/v3` 的代理，可在链升级运行期间继续提供推理服务。
...
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `ap`i 二进制文件和网桥镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。若已完成上述两步，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
针对即将到来的 v0.2.14 链升级，已准备好永久修复此问题的补丁。由于攻击已在使用中，因此在升级前提供仅 API 的热修复版本 —— 可异步部署，通过强化 PoC-v2 验证以阻止复制计算通过采样检查，从而阻止攻击。
...
`moonshotai/Kimi-K2.6` 在第 311 个纪元再次尝试引导。希望提供 Kimi 服务的主机需在第 310 个纪元声明意图，并在新纪元开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可委托给该节点。
...
关于引导流程的更多详情：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一并恢复。此系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此举并非为了偏向任何硬件——MiniMax上已具备相同的权重。运行Kimi而非MiniMax的8xB300主机将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一并添加（此举实际上取消了该模型的非参与惩罚）。此系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此举并非为了偏向任何硬件。运行GLM-5.2而非MiniMax的8xB300主机将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及>2/3可达性）。这使运营商能够在配置硬件前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机将在部署窗口期间为其模型配置MLNode。
...
该提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动引导。由于每轮约22.8小时，标准的48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护者应支持本提案。

**主机所需操作**

1. **要提供Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，此期间切换是安全的，因为该窗口内无cPoC。

2. **要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口期间配置您的MLNode。提供GLM-5.2是自愿的——未选择的主机不会受到任何惩罚。
...
如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 打算再次提供Kimi的主机应保留Kimi的设置并准备好在PoC 309时将MLNode切换回Kimi——将在第308纪元进行一次恢复Kimi的投票。
...
`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出组别，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以便快速恢复。** 由于Kimi未获得足够的PoC验证票数，将其移除并重新引导是最快恢复且停机时间最短的路径。下一个纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi的情况无关，仅因同属PoC阵容变更而一并列入。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更合并为一次紧急投票，因为重置必须在第307纪元结束前完成。每个纪元约22.9小时，标准的48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持该提案。

**若获批准的效果**

从第308纪元（约2026年6月25日17:25 UTC）起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如尚未准备就绪），以避免在纪元边界遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——第308纪元将进行一次恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** — 在第308个周期中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，因此主机可选择加入并提前测试对该模型的需求，选择不提供该模型的主机不会受到任何惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
1) 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已提供的推理保留在RAM或状态中。
2) 通过OpenTelemetry和Prometheus添加了内部devshard追踪和指标。
3) 添加了使用Grafana、Jaeger、Prometheus、Loki、Promtail和cAdvisor的join-stack可观测性。
4) 将每个推理的验证计数器移出状态根，放入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个插槽的总数。
...
新进程在`/devshard/v2`前缀下提供服务。现有的v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。在此类升级期间，无需重启节点容器或执行手动主机步骤。
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需采取任何其他操作——现有设置将继续正常工作。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基线。此调整**仅适用于您共识权重中源自 Kimi 的部分**；您源自 Qwen 的权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新投票，现已提交多个更新。

- 包括：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（参见下方专用部分）。
- 该提案将升级后的宽限期延长至 3000 个区块，以便主机在新快照逻辑稳定期间不被惩罚。
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。如果守护者弃权，非守护者需在剩余 75% 中达到约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再采取进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期其余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 区块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 该提案将有意跳过从升级高度到升级周期结束（10000区块宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
...
由于现有协议规则：单个模型的总权重不得超过上一轮所有模型总权重的75%，因此Kimi在第267轮中的活跃权重较低。
...
2. 监控下一几轮的PoC参与情况。确保您的节点如预期般进入PoC，并且活跃权重正确反映。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。若如此，请先确认您正在运行当前版本，再进行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一次升级周期中符合社区赏金资格。

本次仅为拉取请求的审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天启动。

**关键变更****inference-chain**

- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和未合格模型的诚实矿工被削减。修复方案存储了一个纪元的可确认模型快照及权重缩放因子，并在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一次新的错误推理即可立即再次使其失效。计数器现在在重新激活和即将晋升时重置。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级将补全该权限，以便它们能够响应交易商投诉。
- Devshard 结算使用了硬编码的 `20_000` 非nonce 限制。现在限制为 `DevshardEscrowParams.MaxNonce`，v0.2.13 升级将其设为 `1_000_000`。升级还将 `MaxEscrowsPerEpoch` 提升至 `500_000`。
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束期间，跳过确认 PoC 触发，因此新的快照逻辑仅从下一个纪元开始运行。复用了 v0.2.10 的宽限期原语。
...
- 状态快照减少了长期会话的恢复工作量。
- 负载查找固定在托管纪元，同时为纪元边界和遗留纪元 0 请求提供回退机制。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保持您的MLNode使用 `Qwen235B`，并于明天提交下一周期的意图。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，再通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接或通过委托 PoC 投票权重）。
...
- **协议加固。** 为单个 PoC/CPoC 时间随机采样保留节点（`POC_SLOT=true`）。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立治理投票激活。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者 slashing 与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
更新修复了在 PoC 开始附近的一个边缘情况，该情况此前可能导致在特定条件下性能下降。
...
这些性能改进可根据工作负载和网络条件，使每个区块的推理数量提升高达100倍。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理可扩展性。

目前，通过每个推理的链上交易处理推理操作限制了吞吐量。此设计将推理执行与验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这有意设计为该架构的早期和受限版本。提出此版本供主网审查和有限生产测试，并非因其已完成，而是因为此类系统需尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间开始大量剪枝活动，造成不必要的运营压力。
...
3. 当前被保留且不参与 PoC 生成的 MLNodes 仍被用于 PoC 验证。这可能导致推理丢失。新版本将此类节点排除在 PoC 验证之外。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各纪元间可能会波动。较小的权重可能经历相对更大的波动。为避免在纪元边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为了将PoC权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选定。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
由于有效归一化窗口比早期名义模型长约5个区块，因此每个纪元重新计算的权重成比例降低。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级通过实际 PoC 经过时间对 PoC 参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
**宽限期**：升级引入了一个宽限期，在升级后的3000个区块内不进行确认 PoC，并对升级所在纪元的错过率和无效率阈值放宽要求。
...
这意味着，在单个纪元内，即使被惩罚，矿工的抵押品损失也不会超过 0.5%。所需抵押品仅占每日奖励的约 24%。

**警告**：无论投票结果如何，抵押参数都将生效。如果此提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。

投票结束后、第180纪元前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180纪元起，其奖励将被削减至五分之一。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级时段内放宽了错过率和无效率的阈值。
...
链上推理访问当前已开放，不再仅限于开发者。推理请求可通过上一版本引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一次链上软件升级 v0.2.10 已开放审核。欢迎提供反馈和改进建议。当前计划是将审核窗口保持约两天。

对于对本次 PR 审核的有意义贡献，可能在下一次升级中提出奖励。

本次仅为拉取请求的审核请求，而非正式投票的开始。治理投票流程将在审核期结束后启动。
...
此升级通过实际 PoC 持续时间标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与PoC v2中观察到的GPU类型之间相同的相对性能差异。
...
现已提供补丁以解决导致PoC周期内块验证暂停的问题。建议主机尽快应用此补丁，以确保正确的PoC验证行为并安全恢复块生产。
...
所有主机必须随时准备在短时间内安装补丁。
请保持在线并密切留意公告。一旦补丁准备就绪，将立即分享进一步的说明。
...
- 下一个 PoC 周期（从第 158 轮到第 159 轮的过渡）至关重要。请确保在线，以便在需要时能够及时执行后续步骤或缓解措施。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）轮次并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将来临的轮次的参与集合中。
...
- 提供其他模型的 ML 节点不会被纳入 PoC v2 权重分配。在支持的情况下，可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
下一代链上软件版本v0.2.9的升级提案现已上链并开放投票。若获得批准，该提案将启用PoC v2用于权重分配，并通过链上治理完成对传统PoC机制的过渡。
...
本指南说明了主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置需符合PoC v2要求，自第155个纪元起生效。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
!!! note 
    ...
    `node-config.json` 仅在 Network Node API 首次启动或本地状态/数据库被删除时使用。若要重新启动，请编辑它。对于现有节点，模型更新应通过管理 API 进行。
...
- 包含双模式迁移策略：V1 用于常规 PoC，V2 用于在部署期间跟踪确认 PoC。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而不升级 ML 节点不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不被视为符合 PoC v2 参与资格。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。
...
该提案将当前的稳定期延长约两周，以便为即将到来的 PoC 变更及相关网络升级进行额外测试。有关新 PoC 开发进展的更多详情，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

该延长也为主机提供了时间来为新的 PoC 要求准备其设置，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步执行更新。
...
`allowlist` 具有固定的过期时间，不会在延长的稳定窗口之后继续存在。一旦 `allowlist` 在区块 2443558 过期：
...
当前有一个新的治理投票正在进行。该投票通过将若干地址添加到 [白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv) 来修正一个过滤边缘情况，这些地址此前因硬件名称为空但 ML 节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 的参与者注册截止时间对齐。
...
1) 网络将使用一个 `allowlist` 运行，其中的参与者在多个轮次中：
...
2) 之前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直到区块2,222,222的稳定窗口结束。
...
若获批准，网络将临时使用一个 `allowlist`，包含在先前轮次中未表现出非常规硬件行为的参与者。实际上，`allowlist` 对应于在多个轮次中满足以下条件的参与者：

- 报告的硬件特征已与预定义的常见硬件配置模式进行比对，用于识别偏差和不一致（非常规配置字符串的完整列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），以及
- 观察到的PoC权重低于使用同类硬件的其他参与者所表现权重的150%。
之前表现出对这些模式持续偏离的参与者在稳定窗口于区块2222222结束前不属于 `allowlist`。
...
- 临时重新激活创世验证者增强——一种此前曾使用过的、有时间限制的防御机制，现提议重新激活。
...
- 开发者地址注册：在稳定期间，暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块 2,294,222（约 19 天）。

**治理控制机制**

本升级包含的预备性变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。

**第 117 期奖励分发**

本提案涵盖与链停摆（第 117 期）相关的两项奖励分发：

- 在第 117 期活跃但未收到该期奖励的节点，将补发该期遗漏的奖励。
- 所有在第 117 期活跃的节点，将额外获得相当于第 117 期奖励 1.083 倍的 payouts，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
此 `API` 版本改进了对失败推理的处理弹性，并减少了推理遗漏计数问题。同时引入了对 PoCBatch 和 PoCValidation 交易的批量处理。
