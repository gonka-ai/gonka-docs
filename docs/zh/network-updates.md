...
**紧急治理投票（提案78）：MiniMax-M2.7 成为唯一PoC模型；Kimi K2.6 被移除以快速重新启动；Qwen3-235B 被退役**

`moonshotai/Kimi-K2.6` 目前没有足够的投票用于PoC验证。因此，为Kimi服务的参与者被移出小组，Kimi无法进入下一个周期。现在将Kimi从活跃集合中移除并重新启动，是使其以最小停机时间回归的最快方式。

一项紧急提案现已上链，以便在下一个周期开始前生效。
链上，该提案在一次投票中执行以下操作：
1. 将委托 `initial_model_id` 设为 `MiniMaxAI/MiniMax-M2.7`，使MiniMax成为基础模型。
...
**每一项更改的目的*** **Kimi K2.6 — 被移除以快速恢复。** 由于Kimi没有足够的投票用于PoC验证，将其移除并重新启动是恢复其服务的最快路径，且停机时间最短。下一个周期将进行一项投票以恢复Kimi。

* **Qwen3-235B — 退役（独立、无关的更改）。** 退役较旧的Qwen3-235B是独立的、此前已请求的模型阵容调整。它与Kimi的情况无关，仅因同样属于PoC阵容调整而一并列出。
**MiniMax-M2.7 — 升级为基础模型。**

这些更改被捆绑在一个紧急投票中，因为重置必须在第307周期结束前完成。每个周期约22.9小时，标准的48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创世守护者预计将支持该提案。

**若获批准的效果**

从第308周期开始（约UTC时间6月25日17:25），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B 和 Kimi K2.6 不再活跃。Kimi的恢复将在下一个周期的后续投票中处理。

**主机所需操作**
**在PoC 308开始前将您的MLNode切换为 `MiniMaxAI/MiniMax-M2.7`。** 每位主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换为MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（若尚未就位），以避免在周期边界时遭遇Hugging Face速率限制。

**在截止日期前对提案78进行投票。** 紧急窗口期很短。
计划再次服务Kimi的主机仍应现在将MLNode切换为MiniMax，但需准备好在PoC 309时切换回去——下一个周期将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 在第308周期进行后续投票，以重新添加Kimi并重启其启动流程。
**GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择加入并提前测试模型需求，对选择不服务该模型的主机无惩罚。


**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 第308周期即将形成：PoC开始约UTC时间16:50，生效约17:25 UTC。
...
提案获批准后，v2 现在可与现有的 v1 devshard 运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有的 v1 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

此类仅 devshard 的运行时升级无需重启节点容器或手动执行主机步骤。

**关键变更**

1) 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以避免长期运行的会话将所有已服务的推理保留在 RAM 或状态中。
...
4) 将每推理验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每插槽总数。
...
本提案涵盖 devshard v2 发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅 devshard 升级。它独立于全链软件升级。若获批准，v2 将与现有的 v1 devshard 运行时并行运行。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以避免长期运行的会话将所有已服务的推理保留在 RAM 或状态中
...
4. 将每推理验证计数器存储在状态根之外的 SQLite/Postgres 中，并在推理修剪后通过 devshard 统计端点暴露每插槽总数
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。若链上提案获批准，`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

新进程通过 `/devshard/v2` 前缀提供服务。现有的 v1 devshard 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。此类升级期间无需重启节点容器或手动执行主机步骤。
...
**[仅限 devshard 的升级 PR](https://github.com/gonka-ai/gonka/pull/1289) 现已开放审核。**

这是首次仅限 devshard 的升级，因此流程与标准链升级不同。Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调全节点升级，不影响主网行为，也不预期会导致推理服务中断。

如果通过治理流程批准，新 devshard 版本将与现有的 v1 运行时并行运行。

请直接审核该 PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审核贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期获得社区奖励。

本次仅为 PR 审核呼吁。不启动正式投票。治理投票流程将在审核期结束后开始。
...
MiniMax 的按模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
提案已获**批准**，升级已在主网 `4267300` 块处成功执行。

## 2026年5月25日

**升级 v0.2.13：预下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13 升级提案（提案 ID (https://github.com/gonka-ai/gonka/pull/1143)54）已通过链上治理，升级现已排期。
...
升级将在主网 **高度 4267300**（≈ **2026年5月26日周二 14:42 UTC** / **07:42 PDT**）自动执行。
...
3. 升级完成后，每个主机都需要为**每个**经治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 的截止日期为**链纪元 `278`**。未采取任何操作的主机将从纪元 278 起，对其全部权重施加每纪元 15% 的惩罚。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过的提案的重新投票，现已提交更新版本。

- 包括：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专属章节）。
- 该提案将升级后的宽限期延长至 3000 个区块，以避免主机在新快照逻辑稳定期间受到惩罚。
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
- 所需准备：桥接容器检查、最小最大决策、仪表板更新、投出选票。
- 除非提案获得批准，否则链上无任何更改。
...
- 在 vLLM 0.20.1 发布后，根据 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`（不变）
    - Kimi K2.6：`0.78`（从 1.26 下调，相当于在相同 PoC 权重下，Kimi 的每轮共识权重减少了约 38%）
    - MiniMax-M2.7：`0.3024`
...
- 将 devshard 非nonce 限制从 `20_000` 提高到 `1_000_000`。
- 将每轮最大 devshards 数量从 `100` 提高到 `500_000`。
- 修复新模型引导期间确认 PoC 奖励核算问题。
- 在本次升级轮次剩余时间内禁用确认 PoC，以便新快照逻辑从下一轮干净启动。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
**`MiniMaxAI/MiniMax-M2.7` 在第278个周期前的选择（从该周期开始处罚）**
...
- 运营商时间线：投票于5月22日22:12 UTC结束 → 升级高度约为5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在start_poc − 500块时（约提前43分钟）→ 升级后下一个纪元边界启动首个MiniMax PoC阶段 → MiniMax惩罚在链纪元278执行。
...
纪元#267中PoC验证已成功完成，受影响的主机已能重新加入活跃集合。

纪元#266的问题由针对运行Kimi模型的主机的攻击引起。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入纪元#266。

在应用额外保护措施期间，推理可能暂时不可用。访问预计会逐步恢复，首先通过若干社区代理和经纪人端点。

**发生了什么**

在纪元#266中，网络的活跃权重大幅下降。

问题已追溯至具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
在相关保护措施实施期间，推理可能暂时不可用。访问预计会逐步恢复，首先通过若干社区代理和经纪人端点。

**纪元#267中Kimi的权重**

由于现有协议规则，Kimi在纪元#267中的活跃权重较低：单个模型的总权重不得超过前一纪元所有模型总权重的75%。
...
似乎PoC投票未能收集到本周期所需的票数。确切原因尚未确认，社区正在积极调查此事。
...
社区成员目前正在审查第266个周期发生的情况。一旦对根本原因有更清晰的了解，将发布更新。
...
**epoch 278 之前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
- 该提案将有意从升级高度到升级纪元结束（3000区块宽限期）跳过确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
...
- 确认PoC在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被削减。修复方案是存储一个可确认模型和权重缩放因子的单个纪元快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一个新产生的不良推断会立即再次使其失效。现在在重新激活和即将晋升时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权中未包含`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应交易商投诉。
...
- Postgres数据按`epoch_id`分区，用于会话、差异和签名，以便清理时能干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
...
docker start api
```
部署后，请再次确认9100和9200端口的服务器是否正常运行：
```
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化后续模型的接入，提议将该阈值降低至10%。
...
已提交 PoCIntent 的节点现在应在 PoC 于区块 `3874496` 启动前，将至少一个 MLNode 从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议强化**。保留的节点（`POC_SLOT=true`）将被随机采样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为按模型划分的 PoC 组。每个治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
- **共识级交易费用与自动迁移**（[#937](https://github.com/gonka-ai/gonka/pull/937)，[#981](https://github.com/gonka-ai/gonka/pull/981)）。引入由治理控制的汽油价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免费。`MsgPoCV2StoreCommit` 采用双组件费用（基础验证 + 计数线性）作为主要 Sybil 防御机制。详情请参见 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard 独立运行时**（[#1045](https://github.com/gonka-ai/gonka/pull/1045)）。将 devshard 发布与 DAPI/主网发布周期解耦。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位完整熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
下一版本v0.2.11的链上升级提案现已发布并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著改善 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每个推理交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅处理会话创建和最终结算。

这故意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间开始大量剪枝活动，从而造成可避免的操作压力。
...
对于此次 PR 审查的有意义贡献，可能在下一次升级中提出奖励。
...
本次升级引入了基于 `devshards` 的推理会话的初始版本，旨在提高推理可扩展性。

目前，通过每笔推理链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链仅处理会话创建和最终结算。

这故意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时启动剪枝活动，造成不必要的操作压力。
...
**建议（可选）：在PoC开始时中断正在进行的请求，使用vLLM / mlnode新版本构建**

现已发布新的vLLM / mlnode版本，可在PoC开始时中断正在进行的推理请求，以降低PoC启动时仍处于活跃状态的请求可能导致的权重下降风险。
...
- 此版本旨在与前一版本保持向后兼容。
- 该版本已为少量节点启用，但仍建议在部署前审查变更内容。

## 2026年2月19日

**抵押参数更新提案——投票结果**

抵押参数更新提案未能达到法定人数，因此根据当前治理规则被拒绝。这意味着更新后的参数将不会生效。
...
- 确保[抵押](https://gonka.ai/host/collateral/)设置正确，否则从第180个纪元开始，PoC收益将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各纪元间可能发生波动。较小的权重可能经历相对更大的波动。为避免在纪元边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
由于有效归一化窗口比先前名义模型长约5个区块，每个纪元重新计算的权重成比例降低。

**观测到的GPU权重变化（第175纪元 → 第176纪元）**

| GPU类型 | 第175纪元 | 第176纪元 | 更改 |
|---------------------|----------:|----------:|--------:|
...
由于采用了PoC持续时间归一化，且有效参考窗口现在比之前的名义假设长约5个区块，从第176个周期开始的权重值反映了更新的计算模型。
从PoC权重推导H100等效容量或奖励预测的跟踪器和仪表板，应从第176个周期起验证其转换系数。
...
针对升级提案v0.2.10的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络公平性和可扩展性。

**注意**

ML节点容器必须重启以触发模型的重新部署。运行：
```
docker restart join-mlnode-1
```
在升级中引入的3000个区块宽限期结束前，应完成向`mlnode:3.0.12-post4-*`的过渡。

!!! note "兼容性说明"
    此升级包含迁移至IBC堆栈v8.7.0。请检查任何解析`inferenced` CLI输出的脚本。枚举和int64/uint64值现在以字符串形式编码。
    ...
    **基于实时的PoC权重归一化**
    ...
    此升级通过实际PoC耗时对PoC参与者的权重进行归一化，以减少区块时间漂移的影响，并确保权重结果与实际执行持续时间一致。
...
**宽限期：** 此升级引入了一个宽限期，在升级后 3000 个区块内无确认 PoC，并且在升级时段内对错过率和无效率阈值放宽要求。

有关这些更改的更多详细信息，请参见治理工件：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md)
...
这意味着，在单个纪元内，即使被罚没，矿工损失的抵押品也不会超过 0.5%。所需抵押品仅占日收益的约 24%。

**警告：** 抵押参数将 Regardless of 投票结果生效。如果此提案未通过，创世时定义的抵押参数将在第 180 纪元自动激活，而非上述参数。

投票结束后、第 180 纪元前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 纪元起，其收益将减少 5 倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将显著优化 PoC 验证（默认关闭）并实现实时权重归一化，以提升网络公平性与可扩展性。

**关键变更****PoC 验证采样优化**

此升级引入了一种新的 PoC 验证机制，通过为每个参与者分配固定的验证者采样集，将复杂度从 O(N^2) 降低至 O(N x N_SLOTS)。

**基于实时的 PoC 权重归一化**

此升级根据实际 PoC 经过时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，确保权重结果与实际执行时长一致。

**为 Qwen235B 启用工具**

此升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 添加了工具调用参数（`--enable-auto-tool-choice` , `--tool-call-parser hermes`），并设置了验证阈值 `0.958`。
要启用工具，必须重启 MLNode 容器内的 vLLM。此升级引入了一个宽限期，在升级后 3000 个区块内无确认 PoC，并且在升级时段内对错过率和无效率阈值放宽要求。

**其他协议更新**

- 修复 PoC 与 CPoC 交集错误（PR #752）
- 将 IBC 栈升级到 v8.7.0。
- 惩罚阈值现在由链上数据派生（PR #688）
- 支持带有活跃锁仓的流式锁仓转账（PR #641）
- 更可靠的 MLNode 容器版本 `ghcr.io/product-science/mlnode:3.0.12-post4` / `ghcr.io/product-science/mlnode:3.0.12-post4-blackwell`。

有关这些及其他变更的更多详细信息，请参见治理工件 [https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md ](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md)

**升级执行后所需主机操作**

如果提案获得批准并执行升级，必须重启 ML Node 容器以触发模型重新部署。运行：
```
docker restart join-mlnode-1
```
过渡到`mlnode:3.0.12-post4-*`应在升级中引入的3000个区块宽限期内完成。
...
- 检查任何解析 `inferenced` CLI 输出的脚本。由于 IBC 栈升级到 v8.7.0，枚举和 int64/uint64 值现在以字符串形式编码。
- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
距离 Epoch 180 不足 7 天——现在是准备的时候了。

正如在 AMA 中讨论并根据社区成员提出的论点，建议从较小的抵押要求和最低的惩罚开始。
...
这意味着，在单个 Epoch 内，即使被惩罚，矿工损失的抵押品也不会超过 0.5%。而所需的抵押品仅占日奖励的约 24%。

一旦提案提交投票，将另行发布公告。

警告：无论提案投票结果如何，抵押都将生效。如果该提案未通过，Genesis 中定义的抵押参数将在 Epoch 180 自动激活，而非上述参数。

任何未来的抵押增加都将通过单独的投票提出。目标是观察网络稳定性，确保不合理的惩罚罕见且仅在合理原因下实施。如果稳定性得到验证，将逐步增加抵押至 Tokenomics 白皮书中描述的水平（例如，每 H100 约 100 GNK），以支持网络的长期成功。
...
请审阅 GitHub 上的 v0.2.10 升级 PR 并提供反馈。有意义的审查贡献可能在下一次升级中获得奖励。

[https://github.com/gonka-ai/gonka/pull/695](https://github.com/gonka-ai/gonka/pull/695)

## 2026年2月13日

如果您的节点未能及时应用最新升级，可能在区块 2628371 处因共识失败而停止。这是因为节点运行的是不再与网络兼容的旧二进制文件。要恢复，请遵循本指南 [https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)

## 2026年2月12日

**网络更新：补丁可用（PoC / cPoC 重叠）**

现已提供补丁以解决当前 Epoch（169/170）中观察到的事件。
...
**网络事件：cPoC / PoC 重叠（区块生产暂停）**

在当前纪元中观察到 cPoC（确认 PoC）与 PoC 之间的重叠。直到纪元的最后一个区块，`is_confirmation_poc_active` 被观察为 `true`。

此重叠的影响正在评估中。初步观察表明，没有节点记录 PoC 提交，导致该纪元累积权重为零。

作为预防措施，矿工通过协调行动暂时停止了区块生产。

问题正在定位中。

请保持在线，以便在需要紧急应用补丁时及时响应。一旦准备好，将提供更多信息和补丁说明。
...
```
新版本库已发布：[https://gonka.ai/developer/quickstart/#3-inference-using-modified-openai-sdk](https://gonka.ai/developer/quickstart/#3-inference-using-modified-openai-sdk)
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 已开放审核，用于下一次链上软件升级 v0.2.10。欢迎提供反馈和改进建议。当前计划将审核窗口保持约2天。

对于对此次PR审核的有意义贡献，可在下一次升级中提议赏金。

本次仅为拉取请求的审核请求，并非正式投票的开始。治理投票流程将在审核期结束后启动。
...
此升级通过实际PoC耗时标准化参与者权重，以减少区块时间漂移效应，并确保权重结果与真实执行时长一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的Streamvesting转账
...
1. 活跃主机在GitHub上审核此提案并留下反馈。
2. 在社区审核PR后，预计从此分支创建v0.2.10版本，并提交针对此版本的链上升级提案，以启动正式的治理投票流程。
3. 如果链上提案通过，预计在链上升级执行后合并此PR。

从 [upgrade-v0.2.10](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10) 分支（而非 `main`）创建发布版本，可最小化 `main` 分支上 `/deploy/join/` 目录中容器版本与链上二进制版本不匹配的时间，从而为新主机提供更顺畅的接入体验。

**测试与迁移**

v0.2.10 的测试指南和迁移详情请参见 [此处](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10)。请仔细审阅。

**兼容性说明**

如果您有任何解析 `inferenced` CLI JSON 输出的脚本，请在此升级后重新检查。由于 ibc-go 升级至 v8.7.0，枚举现在以字符串而非数字编码，int64/uint64 值也现在以字符串编码。

## 2026年2月4日

**CLI 更新提醒**

为授予在 v0.2.9 升级后创建的热密钥权限，应使用 [CLI 版本 v0.2.9](https://github.com/gonka-ai/gonka/releases/tag/release%2Fv0.2.9)。

## 2026年2月3日

**PoC v2 基于推理的权重调整**

在 PoC v2 启用后，权重分配现在基于当前模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的实测推理性能。因此，中位数 GPU 权重以及不同 GPU 类型之间的相对权重比例均已调整。

**观察到的 GPU 权重变化（第158轮 → 第159轮）**

| GPU 类型 | 第158轮 | 第159轮 | 变化 |
|------------------|-----------|-----------|--------|
...
- 在当前模型配置下，根据观察到的推理轨迹，B200 GPU 的推理性能并未高于 H100 级 GPU。
- 如果未来通过治理引入更大或更复杂的模型（例如 DeepSeek V3.2），可能会观察到不同的性能特征。
- 在 PoC 之外使用标准 vLLM 基于相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与 PoC v2 中观察到的相同 GPU 类型间的相对性能差异。

**追踪器（仪表板）维护者操作**

在更新的权重分配生效后，追踪器（仪表板）维护者可能希望审查第 159 期及之后的系数，以确保与当前 PoC v2 权重分配保持一致。
...
本指南说明了主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
作为 v0.2.8 升级的一部分，支持的模型集已更新。除以下模型外，所有先前支持的模型均已从活跃集中移除：
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。在网络切换至单模型配置后，不满足上述两项条件的节点将不再被视为 PoC v2 参与资格节点。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
`allowlist` 通过公开可观察的链上数据，使用一组预定义的硬件配置模式推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
1) 网络将运行一个 `allowlist`，由以下参与者组成，这些参与者在多个周期中：
...
本次升级包含的预备性变更，为未来通过治理方式控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 主机级别注册：在区块 2,222,222（约两周后）之前，暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期间，暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本升级包含的预备性变更，为未来通过治理控制参与者加入和推理执行提供了支持，而无需额外的软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。
