...
!!! note "关于本页面"
    ...
    本页面不保证内容完整。有关最新信息，包括治理投票的发布及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。
...
该提案已获批准。它将从社区资金池拨款 100,000 USDT，用于支持 Gonka 参与 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金用于覆盖黄金赞助套餐及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
重启时，`v4.0.1` 从 nonce 1 恢复了所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 在 60 秒就绪超时后终止子进程，客户端因此看到 HTTP 502 错误。

`v4.0.2` 加载最新快照，仅恢复其后的差异。此版本与之前的 v4 版本完全兼容。
...
该提案已获批准。它将 `deepseek-ai/DeepSeek-V4-Flash-0731` 的 `weight_scale_factor` 从 0.214 提高至 0.246。所有其他模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终以 `PROPOSAL_STATUS_REJECTED` 结束，原因是 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的法定人数）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 因子源于被高估的 nonce/min 数值。测量修复见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需任何操作。
...
Devshard 升级独立于主区块链更新 devshard 运行时。它们无需通过 Cosmovisor 协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。若通过治理流程批准，新的 devshard v5 运行时将与现有的 v4 运行时并行运行。

**主要变更**

1. v5 的主要目标是高度同步：每个主机和序列器在保证金日志中维护一个经过签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard 现在可以明确知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 高度戳在协议中添加了逻辑时间。`observed_height` / `observed_block_hash`戳已存在于所有先前携带时间戳字段的消息中。后续版本可以将墙钟字段（如`confirmed_at`和`started_at`）替换为高度戳。这解决了时间未来攻击问题，并为在协议层面处理流超时打开了道路。本版本携带了这些戳，但尚未将超时和密封决策切换至它们。

3. 已添加通过所有插槽进行往返的心跳机制，因此安静的会话仍能定期同步高度，而繁忙的会话则可通过常规推理戳实现相同的节奏。

**行动项**

请审阅PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。

有意义的审阅贡献，包括重要评论、错误发现和安全问题，可能有资格在下一升级周期获得社区奖励。
...
该提案已获批准。它调整了在周期早期数据不足时无效推断检查的行为，避免因样本过小而过早触发。主机无需采取任何操作。
...
一个新的链上治理提案（提案编号96）已开放投票。

**作用**

在周期初期，数据尚不足，因此无效推断检查（SPRT）可能仅因少量无效样本就触发。本提案使其稳定：

- 将`invalidation_h_threshold`提高至40，并将`bad_participant_invalidation_rate`设为0.18。
- 在大样本情况下，阈值接近10%的无效率，因此低于该阈值的诚实参与者不会被惩罚。真实违规者仍会被触发（约360个样本时为18%，约1650个样本时为12%）。
- 小样本无法触发：至少需要32个被标记的无效样本，且在500个样本时观测到的比率必须超过16%。
- 未使用的停机时间SPRT已被关闭。周期末统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于**UTC时间8月21日06:04**（约12小时后）结束。如有可能，请参与投票。若未能在时限内达到加速阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。若转为常规提案，您需重新投出您的选票。
...
作为协议开发的重要里程碑，该链已准备好逐步启用devshard结算，以提升网络的安全性和可靠性。
...
此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将被用于链上。在当前主网阈值配置下，错误率低于10%的主机不会触发任何处罚或罚没，但仍应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。没有明显问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端的问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需对此采取行动。

**下一步：****1) 接下来的24小时。** 主机应检查其统计信息并解决任何关键错误或配置问题。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上开始手动控制的结算，首先从较小的模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后跟进，因为其数据在负载下目前可靠性较低。
...
该部署故意采取渐进方式，以便在结算扩展到整个网络之前识别并解决任何意外问题。
...
针对DeepSeek的每模型参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需再做任何操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（正在服务Kimi或MiniMax的节点，因为任何已运行DeepSeek的节点均已升级至3.0.16），请在方便时将这些节点迁移至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新几个节点，确认其稳定性后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。
...
将实时的社区销售合约和所有包装代币实例迁移至新存储的CosmWasm代码，并为未来实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预资格。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 和 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查为 epoch 360 引导快照——在此期间请保持委托分散，以确保快照有效。
...
委托主机：选择**非守护者** DeepSeek 意向主机，并将权重分散到独立目标上——参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
```
该技能将部署模型，测量 PoC 吞吐量，并将输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则您的设置正确。
...
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留您当前的模型。在epoch 360之前无需任何操作。从epoch 360开始，如果您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的未参与惩罚。
...
在符合引导资格的前提下，从epoch 359（PoC开始时间约为2026年8月13日03:24 UTC / 8月12日20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与惩罚从epoch `360`（`penalty_start_epoch = 360`）开始。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留您当前的模型：在epoch 360之前无需操作；从epoch 360开始，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`。
3. 在截止日期前对提案94进行投票。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
这只是一个性能和资源管理补丁，除此之外无其他变更。协议、推理、验证和经济行为均保持不变。变更内容：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作线程的内存清理现在与纪元修剪保持一致。
...
v4 的主要目标是在主机故障或升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是将单体架构重构为高可用、容错、可扩展架构的一系列 devshard 和网络节点变更中的首次更新。

v4 是首个面向多实例高可用的版本：在共享 Postgres 后端部署 N 个 versiond / devshardd 副本，通过 versiond-router 实现粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅通过签名聊天绑定的托管方可访问。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
...
同一提案还将 devshard v4 注册至 `DevshardEscrowParams.approved_versions`。
...
Devshard v4 将与现有的 v3 运行时并行运行。在经纪人和主机操作员准备并验证其 v4 部署期间，现有 v3 流量可继续运行。

v4 的批准本身并不要求立即切换所有流量。在运行时启动并验证后，可逐步进行迁移。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上`application.db`的Cosmovisor备份通常为数十GB，需提前确认）。有关如何安全删除`.inference`目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例支持（与有效负载存储使用相同的环境变量）。本地SQLite将保持默认设置，并自动清理（保留最近3个周期）。
...
主网链/API工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（在主网上禁用`/v1/chat/completions`计费并从API二进制文件中移除嵌入的`/v1/devshard`）、奖励接收者路由以及升级时的安全修复。

devshard部分为v3运行时做准备，以便代理在链升级期间无需依赖已弃用的经典API路径即可提供推理服务，从而提升RAM利用率并支持SQLite与Postgres存储之间的安全切换。
...
- 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，您只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护人员 — 此次升级未移除或重塑任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上 `application.db` 的 Cosmovisor 备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放评审**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整的节点升级，不影响主网行为，也不预期导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond / devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅受信所有者通过签名聊天绑定。当治理发布同版本名的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含了错误修复和安全补丁。

**操作事项**

请审阅 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，对任何发现、疑问、改进建议、边缘情况或潜在漏洞留下评论。
...
**Kimi-K2.6 启动（第 331 个周期）**

`moonshotai/Kimi-K2.6` 已重新列入治理模型列表，并进入标准启动流程。

* 在区块 **5,105,276**（2026年7月17日，约 12:05 UTC）之前声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第 331 个周期 PoC 开始前约 500 个区块的窗口期内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的 MLNode 切换至 Kimi。
...
这是 7 月 15 日宣布的 Kimi 恢复计划的第二步。提案 87 已将 `moonshotai/Kimi-K2.6` 从活跃集合中移除；提案 88 将其重新注册到治理模型列表中，使其从第 331 个周期开始进入标准启动流程。

同一提案同时从 `devshard_escrow_params` 中移除 `approved_versions` 的 v1 和 v2。一旦批准，`versiond` 将停止 v1 和 v2 的 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。此举消除了旧运行时容器长期存在的内存增长问题 —— 这正是 7 月 15 日事件中网络节点 OOM 的根源。
...
2. 提供 Kimi 服务的主机：在投票结束后立即、并在区块 **5,105,276**（2026年7月17日，约 12:05 UTC）之前声明对 `moonshotai/Kimi-K2.6` 的意向，然后在第 331 个周期 PoC 开始前约 500 个区块的窗口期内（区块 **5,105,776**，2026年7月17日，约 12:50 UTC）将您的 MLNode 切换回来。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在进行引导委托时：**请勿委托给守护节点**；将委托分散到独立的 Kimi 主机上。请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/) 以获取最新的委托指引。
...
投出您的投票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` v0.2.13 或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（PDT）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（加速，0.667 赞成阈值；参与率重要，请尽快投票）。
...
`moonshotai/Kimi-K2.6` 在第 328 至 329 个周期失去了 PoC 验证多数票（详见 [此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的方式——与六月提案 78 的恢复路径相同。

该提案将在下一个 PoC 之前将 `moonshotai/Kimi-K2.6` 从 PoC 参数中移除，随后立即重新添加，Kimi 将从第 331 个周期开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案 87 进行投票——加速窗口很短。
2. 运行 Kimi 的主机：请保持您的 Kimi 设置处于待命状态，并准备好在引导 PoC 时切换回您的 MLNode。
3. 在进行引导委托时：请勿委托给守护节点；将委托分散到独立的 Kimi 主机上。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（PDT）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（加速，0.667 赞成阈值；参与率重要，请尽快投票）。
* 引导意向截止时间和周期时间详见引导说明。

## 2026 年 7 月 15 日

**Kimi-K2.6 事件（第 328–329 个周期）：事件经过、恢复计划及委托指引变更**

在第 328 个周期，`moonshotai/Kimi-K2.6` 失去了 PoC 验证多数票。在第 329 个周期开始时，运行 Kimi 的主机被移出组。网络其余部分正常运行。

**事件经过**

两台由守护者运营的运行 Kimi 的服务器同时发生故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障导致这些投票瞬间消失。
* 第二台服务器因内存不足而失去了其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这足以使该组低于多数。守护者投票决胜机制未启动：当前未分配每模型投票权重的守护者已被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与第 306–307 个周期的事件类似。第 329 个周期开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个周期）重新添加它：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意向、部署窗口、PoC 存储提交。在意向截止日期前将发布操作说明。
...
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行模型的主机上。如果您运行多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已在开发中的修复*** PoC 验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完成全面迁移后，v1/v2 将被移除，且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链，立即投票（公告将随后发布）。
2. 如果您计划提供 Kimi：关注引导说明，并在意向截止前准备好声明意向。
3. 如果您为 Kimi 委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量至 `/devshard/v3`。这将修复 RAM 增长问题，并在移除 v1/v2 前必须完成。
...
该发布将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于此类仅限 devshard 的运行时升级，无需重启主网或手动主机操作。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们能够在链升级期间继续提供推理服务，而不依赖已弃用的经典 API 路径。

**关键变更**

- 使经纪人能够在 v0.2.14 链升级期间继续提供推理服务，且不依赖已弃用的经典 API 路径。
- 改进了 RAM 利用率。
- 修复了网关运行时行为。
...
节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支发布独立的 devshard v3 版本，并在主网链升级前推出。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级运行时继续提供推理服务。

**提议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查此提案。
2. 在主网链升级前，提议并推出 devshard v3 版本。
3. 代理将推理流量切换至 `/devshard/v3`。
4. 如果链上提案获得批准，此 PR 将在链上升级执行后立即合并。
5. 请直接审查 PR 代码，并就您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
...
devshard v3 版本将在主网链升级前提议并推出，以便代理在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作项**

1. 现在 — 审查 PR。阅读 GitHub 上的 PR #1267，并就您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两步，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 代理 — 将推理流量切换至 `/devshard/v3`。一旦 devshard v3 版本推出，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复程序 — 可异步部署，通过强化 PoC-v2 验证以阻止复制计算通过采样检查，从而阻断攻击。
...
`moonshotai/Kimi-K2.6` 在第 311 个纪元进行另一次引导尝试。希望提供 Kimi 服务的主机需在第 310 个纪元声明意图，并在新纪元开始前准备好节点。一个守护节点也将运行 Kimi，因此不希望运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 个纪元、区块 **4,797,456** 之前（约 6 月 28 日 12:00 UTC）声明 PoC 意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意图，请在第 311 个纪元开始前约 500 个区块的窗口期内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
> **更新（2026年7月15日）：此建议已撤销——请勿向守护节点委托。** 守护节点是PoC验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。有关最新委托指南，请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行Kimi的共识权重与主模型MiniMax大致相当。此举并非偏向任何硬件——MiniMax上已具备相同的权重。在8xB300主机上运行Kimi而非MiniMax，可获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的未参与惩罚）。此系数的选择使得在8xB300硬件上运行GLM-5.2的共识权重与MiniMax大致相当。此举并非偏向任何硬件。在8xB300主机上运行GLM-5.2而非MiniMax，可获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2的主机需在 `start_poc - deploy_window` 的引导快照前为该模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 和 >2/3 可达性）。这使操作者可在配置资源前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置MLNode。
   
4. **PoC启动。** 成员资格由在PoC启动时实际提交PoC存储提交的主机决定——而非由声明意向的主机决定。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮开始（PoC启动时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是各模型满足引导资格条件：

* `moonshotai/Kimi-K2.6` 将再次成为活跃的PoC模型。
* `zai-org/GLM-5.2-FP8` 将作为可选的PoC模型提供，且无未参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍将作为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能在第309轮中完成引导。每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计会支持该提案。

**主机所需操作**

1. **若要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC启动前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。

2. **若要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口期间配置您的MLNode。提供GLM-5.2是可选的——未选择的主机不会受到任何惩罚。

3. **若要继续使用MiniMax：** 继续提供 `MiniMaxAI/MiniMax-M2.7` —— 无需任何操作。

4. **在截止时间前对提案79进行投票。** 紧急窗口很短。
...
* 紧急提案需达到0.667的赞成阈值；参与率很重要，请尽快投票。

## 2026年6月25日

**提案78通过：`MiniMaxAI/MiniMax-M2.7` 现为唯一的PoC模型；Kimi K2.6 和 Qwen3-235B 已移除**

提案78的紧急投票已通过。这些变更从第308个周期起生效。
...
自第308个周期起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B 和 Kimi K2.6 不再活跃。

**主机所需操作*** 请确保您的MLNode正在提供 `MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，在切换前将无法获得本周期的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并准备好在PoC 309时将MLNode切换回Kimi——在第308周期将进行一次恢复Kimi的投票。

**下一步**

在第308周期将进行一次紧急投票，合并两项变更，均于第309周期生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6** —— 重新添加Kimi并重启其引导程序。
* **添加GLM-5.2** —— 添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入，并提前衡量模型需求，不提供服务的主机不会受到惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出组，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
* **Qwen3-235B —— 退役（独立、无关变更）。** 退役旧版Qwen3-235B是独立的、此前已请求的阵容变更。它与Kimi情况无关，仅因同属PoC阵容变更而一并列出。
* **MiniMax-M2.7 —— 升级为基础模型。**

这些变更合并为一次紧急投票，因为重置必须在第307周期结束前完成。每个周期约22.9小时，标准48小时投票期无法容纳在一个周期内；而12小时的紧急投票期可以。创世守护者应支持该提案。

**若获通过的效果**

自第308周期起（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B 和 Kimi K2.6 不再活跃。Kimi的恢复将在下一周期的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此期间无cPoC。请现在预先下载FP8权重（如尚未就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——第308周期将进行恢复Kimi的投票。


**下一步*** **恢复Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前测试对该模型的需求，而不选择提供该模型的主机不会受到任何惩罚。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过`inferenced`查看。任何活跃节点均可使用：
...
* 第308个纪元将在之后不久形成：PoC开始约16:50 UTC，生效约17:25 UTC。
...
提案通过后，v2现在可与现有的v1 devshard运行时并行运行。新进程通过`/devshard/v2`前缀提供服务，而现有v1流量继续在`/devshard/v1`和`/v1/devshard`上运行。
...
这是首次仅针对devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。
...
MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。

## 2026年5月26日

**升级已执行：v0.2.13 现已在主网上线**

Upgrade Proposal v0.2.13（提案编号 54）的链上治理投票已结束。
该提案已获**批准**，升级已在主网块 `4267300` 成功执行。
...
提前预下载二进制文件和权重有助于避免在升级窗口期间依赖 GitHub / Hugging Face 的可用性。
```
# 1. Create Directories
...
对 [v0.2.13 升级提案](https://github.com/gonka-ai/gonka/pull/1143)（提案编号 `54`）的链上治理投票已结束。该提案已**批准**。

升级将在主网**块高度 4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）自动执行。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 的截止日期为**链纪元 `278`**。未采取任何操作的主机将从纪元 278 开始，对其全部权重施加 15% 的每纪元惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。请确保 `.inference/data` 有足够的空闲空间用于 cosmovisor 状态备份；如果 `application.db` 较大，建议在升级前应用 cosmovisor 备份指南中的[清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基线。此调整**仅适用于您共识权重中来自 Kimi 的部分**；您的 Qwen 权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降低至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%的投票权中实现约三分之一的参与率才能满足法定人数（参见inference-chain部分）。
...
- 将创世守护者调整后的投票权降低至约25%，并将链上治理法定人数设为`0.25`。若守护者不投票，则剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将启用Gonka主网与Ethereum之间的跨链转账（包括在Ethereum上包装GNK以及桥接USDC/USDT）。包装/解包脚本和操作员工作流程将另行文档化。

**decentralized-api & devshard**

- 默认在端口`9400`启用`NodeManagerGrpcPort`。
- 为devshard状态添加Postgres支持。
- 为SQLite和Postgres devshard数据库添加清理功能。
...
所有主机需验证其网桥容器已部署、运行最新版本且同步正确。部分主机可能已部署网桥容器。在这种情况下，请先确认您正在运行当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表盘/浏览器更新（升级前或升级后）**

主机需更新仪表盘/浏览器。请从`gonka/deploy/join`目录运行以下命令。如果您尚未本地克隆`gonka`仓库，请先参考加入网络指南。此仪表盘更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 运营商时间表：投票截止于5月22日22:12 UTC → 升级高度约为5月26日14:42 UTC → 升级周期剩余阶段跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在 start_poc − 500 块处（约43分钟前）→ 升级后下一个纪元边界开始第一阶段MiniMax PoC → MiniMax惩罚在链纪元278执行。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获得批准，此跳过属预期行为而非故障；新快照逻辑将在下一个纪元开始。
- 若获得批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认，且会自动清理（保留最近3个纪元）。
- 若提案失败（未达到法定人数，或 `no_with_veto` 超过1/3），链上无任何变化，升级将不会发生。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取行动。
...
纪元#267中PoC验证已成功完成，受影响的主机已能够重新加入活跃集合。

纪元#266的问题由针对运行Kimi模型的主机的攻击引起。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入纪元#266。

在应用额外防护措施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪人端点。

**发生了什么**

在纪元#266中，网络的活跃权重大幅下降。

该问题已追溯至具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。

在纪元#267中，主机得以恢复，PoC验证成功完成。

**推理可用性**

网络将不再接受使用受影响的非标准语义的请求。

在相关防护措施应用期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪人端点。

**纪元#267中Kimi的权重**

由于现有协议规则，Kimi在纪元#267中的活跃权重较低：单一模型的总权重不得超过上一纪元所有模型总权重的75%。

由于第266个周期的总活跃权重显著降低，此规则也限制了Kimi在第267个周期的权重。

随着未来周期中正常的PoC参与持续进行，权重可能需要数天才能稳定下来。

**主机所需操作**

1. 尽可能保持您的API节点在线并可访问。这有助于保留对主机参与情况的可见性，并支持后续审查。
2. 监控接下来几个周期的PoC参与情况。检查您的节点是否按预期进入PoC，且活跃权重是否正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方可用，并预计通过社区代理和经纪人端点逐步恢复。
5. 在社区渠道或本线程中分享相关日志或观察结果。如果您的主机在第266个周期受到影响，或在后续周期中行为异常，这一点尤为重要。

## 2026年5月16日

**第266个周期：PoC权重下降调查**

在当前周期（#266）中，网络的活跃权重出现显著下降。
...
社区成员目前正在审查第266个周期发生的情况。一旦对根本原因有更清晰的了解，将及时更新信息。
...
- 将devshard nonce上限从 `20_000` 提高至 `1_000_000`。
- 将每个周期的最大devshards数量从 `100` 提高至 `500_000`。
- 修复新模型启动期间确认PoC奖励核算问题。
- 在本升级周期剩余时间内禁用确认PoC，以便新快照逻辑从下一周期干净启动。
- 当参与者再次激活时重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问问题。
- 将创世守护者调整后的投票权降低至约25%，并将链上治理门槛设置为 `0.25`。在守护者不投票的情况下，这使得剩余75%投票权的有效门槛为1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再采取进一步操作。
...
- 该提案将有意从升级高度到升级纪元结束（3000个区块的宽限期）跳过确认PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证要求**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊侧合约激活，所有主机均需验证其桥接容器已部署、运行最新版本并正确同步。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/1143) 针对下一个链上软件升级 v0.2.13 已开放供审查。

请直接审查 PR 代码，并就您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一个新产生的错误推理会立即再次使其失效。该计数器现在在重新激活和即将升级时重置。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授权中未包含`MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应交易商投诉。
...
- Wasm守护进程访问在应用布线后解析，因此合约权限检查适用于网桥和流动性池操作。
...
- Devshard需要为更大规模部署提供共享存储选项。现在可以使用Postgres作为主存储，SQLite仍作为本地后备存储。
- Postgres数据按`epoch_id`对会话、差异和签名进行分区，以便清理可干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
- 负载查找被固定到托管纪元，对纪元边界和遗留纪元0请求提供回退支持。
...
评审者可在此处查看完整的升级提案、迁移详情、测试摘要和建议流程：
...
在api容器版本v0.2.11、v0.2.12和v0.2.12-api-post2中存在一个潜在问题。容器重启后，9100、9200和9400端口的服务器可能启动延迟。这延迟了api激活，部分矿工因此跳过了确认PoC。
...
在上一个纪元中，发现Kimi-K2.6的某些响应解析存在一个小错误。
...
我们建议替换api容器的二进制文件。除修复外，新版本还启用了devshard数据库的清理功能，并为devshard状态增加了Postgres支持。
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化更多模型的接入，建议将该阈值降低至10%。
...
预评估与 PoC 开始之间有 500 个区块的窗口期。在此窗口期内，无 CPoC 任务，因此已声明意向的节点可安全地将其模型节点切换至 `Kimi K2.6`。
...
这标志着网络的一个里程碑：`devshard` 已上线并具备生产就绪能力。`node4` 将作为未来的推荐公共网关。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：该模型组将在升级后两个周期激活。该模型的系数为 Qwen235B 的 3.51 倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议加固**。保留节点（`POC_SLOT=true`）被随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易方共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留此前强制实施的模型和Kimi。
每个DAPI都会在本地持久化其MLNode配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送带有清理后配置的 `PUT` 请求。这些更改将在60秒内持久化。其余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需要手动修复。

将以下脚本粘贴到主机的shell中。默认情况下，它将应用更改。若要预览更改而不应用，请将 `APPLY=dry` 设置为任意非 `--apply` 的值。
...
- **多模型PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的PoC。每个经治理批准的模型生成自己的本地PoC权重，然后通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托PoC投票权重）。
- **`moonshotai/Kimi-K2.6` 作为第二个模型被引入**：模型组将在升级后两个周期激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次PoC/CPoC时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复DKG经销商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保devshard托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡到按模型分组的 PoC。每个治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金基金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于靠近PoC开始运行的节点，稳定性有所提升。

此更新修复了在PoC开始附近的一个边缘情况，该情况此前可能导致在特定条件下性能下降。
...
v0.2.11升级提案的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。
...
有关这些变更的更多详细信息，请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/)
...
下一个链上软件版本v0.2.11的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每笔推理交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。我们提议将其用于主网审查和有限的生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时开始大规模剪枝活动，从而造成可避免的操作压力。
...
本次仅为拉取请求的审查邀请，而非正式投票的开始。治理投票流程将在审查期结束后开始。
...
目前，通过每个推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链仅处理会话创建和最终结算。

这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
对于已使用代理层的团队，这可能是简化堆栈、转而依赖原生行为的好机会。实践中，这应能带来更清晰的集成模式和更易维护的架构。

## 2026年3月6日

**提醒：v0.2.11 升级预计将于下周初进入审查和治理投票阶段。**

请留意并计划参与。投票是支持网络发展、确保升级符合参与者实际需求的最简单方式之一。
如果您无权访问持有投票权的冷密钥，则建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代为投票。若无此授权，其他账户无法提交投票。

在此设置中：

- 授权方 = 拥有投票权的账户（冷密钥）
- 被授权方 = 代表授权方提交投票的账户（热密钥）

被授权方仍可为其自身账户投票。授权方可随时撤销此权限。
...
服务器端对长请求的重试通常无效，因为这会导致相同的超时情况。同时，客户端可能收到不一致的输出。新版本的 API 在此类情况下不再重试推理。
...
抵押参数更新提案未达到法定人数，因此根据当前治理规则被拒绝。这意味着更新的参数将不会被激活。

如前所述，第180轮次的抵押激活独立于此次投票。
...
- 审查创世时定义的抵押参数。
- 在第180轮次前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)已正确设置，否则从第180轮次起，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
此前，PoC权重计算依赖于名义的轮次持续时间假设。引入实时归一化后：

- PoC持续时间与实际区块生成时间对齐
- 权重更准确地反映实际计算时间

由于有效归一化窗口比早期名义模型长约5个区块，因此每个轮次重新计算的权重成比例降低。

**观察到的GPU权重变化（第175轮 → 第176轮）**
...
!!! note "兼容性说明"
    ...
    **基于真实时间的PoC权重归一化**
    ...
    此次升级通过实际PoC耗时对PoC参与者权重进行归一化，以减少区块时间漂移效应，确保权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入3000个区块的宽限期，期间无确认PoC，并对升级时期的错过率和无效率阈值放宽要求。
...
这意味着在单个纪元内，即使被惩罚，矿工最多损失其抵押品的0.5%。所需抵押品仅占日奖励的约24%。

**警告：** 抵押参数将无论投票结果如何均生效。若此提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。

投票结束后、第180纪元前，所有矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，自第180纪元起，其奖励将减少5倍。
...
下一个链上软件版本v0.2.10的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的PoC验证优化（默认禁用），并实现基于真实时间的权重归一化，以提升网络公平性和可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并对升级所在纪元放宽了错过率和失效率的阈值。
...
这意味着，在单个纪元内，即使被惩罚，矿工最多只会损失其抵押品的0.5%。而所需抵押品仅占日奖励的约24%。

提案提交投票后，将另行发布通知。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到验证，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请保持在线，以便在补丁发布时能立即应用。更多详细信息和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已开放**

链上推理访问目前开放，不限于开发者。推理请求可通过上一版本引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
本次仅为对拉取请求的审查呼吁，并非正式投票的开始。治理投票流程将在审查期结束后启动。
...
此升级根据实际 PoC 经过时间标准化 PoC 参与者权重，以减少区块时间漂移效应，并使权重结果与真实执行时长保持一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式兑付转账
- **[PR #659](https://github.com/gonka-ai/gonka/pull/659)** 模型分配检查上一轮奖励
- **[PR #716](https://github.com/gonka-ai/gonka/pull/716)** 重命名 PoC 权重函数以提高清晰度与正确性。
...
现有主机无需升级其 `api` 和 `node` 容器。更新后的容器版本旨在供链上升级完成后加入的新主机使用。
...
- 在PoC之外使用标准的vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与PoC v2中观察到的GPU类型间相同的相对性能差异。

**追踪器（仪表板）维护者操作**

在更新权重分配生效后，追踪器（仪表板）维护者可能希望审查第159期及之后的系数，以确保其与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新 — 补丁可用**

现已提供补丁以解决导致PoC周期中块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为并安全恢复块生产。
...
由于在当前的PoC周期中验证阈值可能无法满足的高风险，主机们集体采取行动暂停了区块验证。
...
在因 cPoC 处罚而导致奖励减少或排除的情况下，未计入部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
- MMR 承诺：工件存储通过默克尔山峰承诺迁移到链下；仅 `root_hash` 和 `count` 记录在链上。
...
本指南说明主机应如何根据v0.2.8模型可用性变化和即将推出的PoC v2更新来更新其ML节点。ML节点配置在第155个纪元开始符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。除以下模型外，所有先前支持的模型均从活跃集合中移除：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估是否具备过渡到 PoC v2 的条件。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
- 逐步部署：如果您运行多个 ML 节点，建议您在多个纪元中逐步进行这些更新。
...
该提案将当前的稳定期延长约两周。延长的期间旨在为即将进行的 PoC 变更及相关网络升级提供额外的测试时间。有关新 PoC 开发进展的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

该延长也为主机提供了时间来为新的 PoC 要求准备设备，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机建议在多个纪元中逐步进行更新。

**投票范围**

如果获得批准，网络将暂时在现有的 `allowlist`（包括未表现出非标准硬件行为的主机）下继续运行。

开发人员 `allowlist` 将以相同的偏移量延长，并将持续到区块 2459375。
...
`Allowlist`到期：自动在区块2443558处到期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的到期时间与区块2,222,222处的参与者注册截止时间对齐。
...
该提案已获得批准。此提案定义了一个临时 `allowlist`，用于反映在最近多个周期中行为保持一致的参与者。
...
2) 之前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直到区块2,222,222的稳定窗口结束。
...
- 报告的硬件特征已与预定义的常见硬件配置模式进行评估，以识别偏差和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
之前持续偏离这些模式的参与者在稳定窗口结束（区块2222222）前不包含在 `allowlist` 中。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备变更，为未来通过治理方式控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
- 主机无需执行任何进一步操作。
- Cosmovisor在执行更新时，会在`.inference`状态文件夹中创建完整备份。为安全执行更新，建议保留250 GB以上的可用磁盘空间。[点击查看](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory) 如何安全删除`.inference`目录中的旧备份。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意干扰或压力测试引起的。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强功能最初是在网络早期阶段作为临时防御机制引入的，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并激活一些额外的保护措施。

**关键变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用、具有时间限制的防御机制，现提议重新激活。
- 在网络增长期间提供共识保护。在其先前运行期间：
...
本次升级将此前通过手动 API 更新分发并已在网络上使用的关键修复正式化。这些修复包括：
...
- 主机级注册：在区块 2,222,222（约两周后）之前暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第 117 期活跃的节点，将额外获得相当于第 117 期奖励 1.083 倍的发放，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
此 `API` 版本改进了对失败推理的处理弹性，并减少了推理遗漏统计问题。它还为 PoCBatch 和 PoCValidation 交易引入了批处理功能。
