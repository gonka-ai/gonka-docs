...
**提案通过：稳定小样本中 epoch 内的无效 SPRT**

提案 ID 为 96 的链上治理投票已结束。

该提案已获批准。它调整了在样本量较小时无效推断检查（SPRT）的行为，避免在 epoch 初期数据不足时误触发。主机无需采取任何操作。

感谢所有参与投票的人。

## 2026年8月20日

**提案 96 开始投票：稳定小样本中 epoch 内的无效 SPRT**

一个新的链上治理提案（提案 ID 96）已开放投票。

**其作用**

在 epoch 初期，数据量尚不足，因此无效推断检查（SPRT）可能在样本量足够前仅因少量无效数据就触发。此提案将其稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真实违规者仍会触发它（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发它：至少需要 32 个被标记的无效项，且在 500 个样本时观测到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。epoch 结束时的统计测试不受影响。

**投票**

这是一个快速提案，因此投票将于 **UTC 时间 8 月 21 日 06:04** 结束（约 12 小时）。如有可能，请尽快投票。如果未能在时限内达到快速提案的阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。若转为常规提案，您需重新投票。
...
一个稳定的网关版本目前正运行在一个网关上，并将用于第一阶段的结算。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何处罚或 slashing，但仍应作为协议开发过程的一部分进行调查。

存在严重错误率的主机应在结算开始前解决这些问题。无显著问题的主机无需采取任何操作。
...
**2）在此 24 小时窗口之后的第一个 epoch。** 将在当前网关上手动启动结算，先从小模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后跟进，因为其数据在负载下目前可靠性较低。

**3）随后的 1-3 天内。** 将监控结算结果

**4）一旦此阶段稳定。** 其他经纪人可部署稳定的网关版本并以受控模式开始结算。

**5）在所有经纪人成功验证后。** 结算覆盖率将逐步提升至 100%。

推广是故意逐步进行的，以便在将结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 已在 Gonka 网络上激活**

在 [提案 94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的引导阶段已完成。自链纪元 360 起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214 转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行其他操作——现有设置将继续正常工作。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版 MLNode（正在服务 Kimi 或 MiniMax 的节点，因为已运行 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点迁移至 3.0.16。

一个请求：请逐步推广。请勿一次性切换所有节点。请先更新少数节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。
...
默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是同一 MLNode 3.0.16，仅针对 CUDA 12.9 构建。两个标签均在同一个 compose 文件中。
...
将在线社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。
...
**DeepSeek V4 Flash 引导：意向阈值已达成——请在纪元 360 快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在纪元 359 快照中未满足预资格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次 **官方** 检查是纪元 360 引导快照——请在此前保持多样化的委托，以确保快照有效。

**将运行 DeepSeek 的主机**

1. 在区块 **5,551,615** 之前保持或提交 `MsgDeclarePoCIntent`：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 如果模型已预合格，请在 PoC 开始前约 500 区块的部署窗口内部署 DeepSeek（区块 **5,552,115**）。请使用 vLLM 0.25.1 + 发布候选节点配置。

**保留其他模型的主机**

在区块 **5,551,615** 之前，提交委托（有助于引导可达性）或拒绝。自纪元 **360** 起，DeepSeek `penalty_start_epoch` 将激活——未明确选择的主机将面临不参与惩罚。
...
委托主机：选择 **非守护者** 的 DeepSeek 意向主机，并将权重分散至独立目标——请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。

**关键时间点*** 纪元 360 快照（`start_poc − deploy_window`）：区块 **5,551,615** —— 约 2026 年 8 月 14 日 01:24 UTC
* 纪元 360 PoC 开始：区块 **5,552,115** —— 约 2026 年 8 月 14 日 02:09 UTC
* DeepSeek 不参与惩罚自纪元 **360** 起生效

有关引导过程的更多内容：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)

## 2026年8月13日

**测试 DeepSeek V4 Flash**

DeepSeek 已通过投票并进入引导阶段——如果您计划部署它，请立即验证您的设置并提交意向。
...
以该格式运行模型需要新的 API 版本 v0.2.15-post5。要安装它：
```bash
...
```
该技能部署模型，测量 PoC 吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比较。如果通过，您就准备好了。

如果您希望提供 DeepSeek，请使用上述步骤进行设置。否则，请不要忘记委托，以避免非参与惩罚。

提醒：明天将发布另一个带有更好日志的 MLNode 构建版本，因此请期待后续简短更新。
...
从第359轮开始（PoC 开始于 ~2026年8月13日 03:24 UTC / 8月12日 20:24 PDT），前提是模型满足启动资格：
...
1. 声明意向。提案94已通过，在引导快照（区块 5,536,224，~2026年8月13日 ~02:40 UTC / 8月12日 ~19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的 MLNode。在 PoC 359 启动前约500区块的安全窗口内（区块 5,536,724，~2026年8月13日 ~03:24 UTC / 8月12日 ~20:24 PDT），将您的 MLNode 配置为 DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate 配置）。在此窗口内切换是安全的，因为该窗口内没有 cPoC。
注意：在 Blackwell GPU 上，为获得最佳性能，您可以使用以 fp8 + nvfp4 重新打包的模型，而不是 fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在引导前发布并共享更新的 API 二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的 DeepSeek 主机上。参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第360轮之前无需操作。从第360轮开始，如果您未提供 DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的非参与惩罚。

区块高度是精确的；时间是估算值，将在接近轮次时确定。[更多关于 DeepSeek 启动的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。

## 2026年8月10日

**治理投票（提案94）：添加 DeepSeek V4 Flash**

将 `deepseek-ai/DeepSeek-V4-Flash-0731` 添加到 PoC 模型列表的提案现已上链并开放投票。

**提案内容**

本提案将 DeepSeek V4 Flash 作为新的 PoC 模型添加到网络中。现有参数或模型均未修改，唯一变化是新增了该模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在 vLLM 0.25.1 上运行的 MLNode，并使用 release-candidate 节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
根据模型使用情况，社区之后可决定逐步用 DeepSeek 替代 Kimi 或 MiniMax（例如，提高系数以激励所有 B 系列 GPU 转向 DeepSeek）。
...
在符合引导资格的前提下，从第 359 个纪元（PoC 开始时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 成为可用的 PoC 模型。现有模型保持不变。DeepSeek 的不参与惩罚从第 `360` 纪元（`penalty_start_epoch = 360`）开始。

**主机所需操作**

1. 若要提供该模型：投票结束后声明意向，然后部署您的 MLNode（vLLM 0.25.1 + release-candidate 配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第 360 纪元前无需操作；从第 360 纪元起，若您未提供该模型，请提交 `PoCDelegation` 或 `PoCRefusal` 以支持 DeepSeek。
3. 在截止日期前对提案 94 进行投票。
...
投票持续 48 小时；投票于 2026 年 8 月 12 日 16:05 UTC / 8 月 12 日 09:05 PDT 结束。
...
- API。
    ...
    - 默认的 Node Manager gRPC 端口现已为 9400，确保与自定义配置的兼容性
...
这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更包括：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。
...
该提案已获批准，并于主网第 5316315 块成功执行升级。该提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
尽管这两项更改在一次投票中提交，但它们在操作上是独立的：

- **devshard v4 运行时** 通过 devshard 版本管理机制由 `versiond` 分发和启动。
- **主网 v0.2.15 二进制文件** 通过标准的链上软件升级流程和 Cosmovisor 激活。

**Devshard v4 变更**

v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个专为多实例高可用设计的版本：在共享 Postgres 后端，通过 versiond-router 部署 N 个 versiond/devshardd 副本，使用粘性会话路由和验证租约独占机制。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
投出您的投票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的 Cosmovisor 备份 `application.db` 通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用大量磁盘空间，可应用 [指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 中描述的 cosmovisor 备份清理技术。
...
devshard 部分准备了 v3 运行时，使经纪人可在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提高 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。
...
如果您没有直接访问具有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。
...
- 现在/主网升级前——更新您的API和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将`api`二进制文件和网桥镜像更新至0.2.14-post3版本。如果您的`api`二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非PoC或cPoC期间执行此步骤。
...
- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间（主网上Cosmovisor对`application.db`的备份通常为数十GB，需提前确认）。有关如何安全删除`.inference`目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
Devshard升级独立于主区块链进行。它们无需通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程批准，新版本的devshard将与现有的v3运行时并行运行。

**关键变更**

- v4的主要目标是提高devshard主机在主机故障和升级时的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4是首个面向多实例高可用的版本：在共享Postgres后端，通过versiond-router部署N个versioned/devshardd副本，采用粘性会话路由和验证租约独占机制。网关仅通过gRPC与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅授权所有者通过签名聊天绑定。当治理发布同版本名的新二进制文件（名称不变，仅sha256变更）时，versiond可执行蓝绿切换并排水，确保正在进行的工作（包括SSE）在旧版本上完成。
- v4还包含错误修复和安全补丁。

**操作项**

请审阅PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，对任何发现、疑问、改进建议、边缘情况或潜在漏洞提出评论。

有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一个升级周期的社区奖励。
...
`moonshotai/Kimi-K2.6`重新列入治理模型列表，并进入标准引导流程。

* 在区块**5,105,276**（2026年7月17日约12:05 UTC）前声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第331周期PoC开始前约500个区块窗口内（区块**5,105,776**——2026年7月17日12:50:53 UTC / 05:50 PDT），将您的MLNode切换为Kimi。
* 委托主机：选择非守护者Kimi主机，并在独立目标间分散权重——参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
这是7月15日宣布的Kimi恢复计划的第二步。提案87将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册到治理模型列表中，使其从第331周期开始进入标准引导流程。

同一提案从`devshard_escrow_params`中移除`approved_versions`的v1和v2版本。一旦批准，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）。这消除了旧运行时容器长期存在的RAM增长问题——即7月15日网络节点OOM事件的根本原因。

**这标志着v3切换的最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票——紧急窗口时间很短。
2. 为Kimi提供服务的主机：在投票结束**之后**、区块**5,105,276**（7月17日，约12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在Epoch 331 PoC开始前的约500个区块窗口内（区块**5,105,776**，7月17日，约12:50 UTC）重新启用您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间进行委托时：**不要委托给守护节点**；将委托分散到独立的Kimi主机上。有关最新的委托指南，请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 经纪人/ devshard创建者：如果您的网关流量仍在使用v1/v2或`/v1/devshard`——请在投票结束前切换到`/devshard/v3`。
...
- 意向截止日期（Epoch 330）：区块**5,105,276**——7月17日，约12:05 UTC（约05:05 PDT）。
...
`moonshotai/Kimi-K2.6`在Epoch 328–329中失去了PoC验证多数（详见[此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其在最小停机时间内恢复的最快方式——与6月提案78的恢复路径相同。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（加速投票，0.667 是票阈值；出勤率重要，请及时投票）。
...
两台运行 Kimi 的监护人操作服务器同时故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障立即移除了这些投票。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。监护人决胜机制未启动：目前没有每模型投票权重的监护人被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与纪元 306–307 事件类似。纪元 329 开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。
...
之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将监护人节点作为委托目标。此建议现被撤回：

* **请勿委托给监护人节点。** 监护人是 PoC 验证的备用机制。将委托集中于它们会将主要机制与备用机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请将委托给运行模型的您自己的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重，且无单点故障。

**已在进行中的修复*** PoC 验证中的监护人投票（监护人始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦加速的 Kimi 提案上链（将发布公告），请对其进行投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非监护人委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量到 `/devshard/v3`。这将解决 RAM 增长问题，并在移除 v1/v2 前必须完成。
...
提案批准后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人将流量切换到 v3。
...
已获白名单的 devshard 创建者应在主网 v0.2.14 链升级前，将推理流量切换至 `/devshard/v3`。这将使他们能够在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
在此分支上将单独提出并部署 devshard v3 发布版本，且在主网链升级之前完成。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供服务。

**提议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查此提案。
2. devshard v3 发布版本将在主网链升级之前提出并部署。
...
Devshard v3 发布版本将在主网链升级前推出，以便经纪人能够在升级运行前将推理流量迁移到 `/devshard/v3`。

**主机操作事项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对发现的问题、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人 —— 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 发布版本上线，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 —— 准备好调整指标统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近结束时发布。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。一个守护节点也将运行 Kimi，因此不愿运行它的主机可以将权重委托给该节点。
...
> **更新（2026年7月15日）：此建议已撤回——请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。有关最新委托指南，请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定使得在8xB300硬件上运行Kimi的共识权重约等于主模型MiniMax。此设定并非偏向任何硬件——MiniMax上已存在相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定使得在8xB300硬件上运行GLM-5.2的共识权重约等于MiniMax。此设定并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的增益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 和 >2/3 可达性）。这使操作者能在配置硬件前确认模型是否具备可行性。
...
从第309轮开始（PoC开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是各模型满足引导资格：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中启动引导。由于每轮约22.8小时，标准的48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计支持该提案。

**主机需采取的行动**

1. **要提供Kimi服务：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间没有cPoC。

2. **要部署GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意图并配置您的MLNode。部署是可选的——未选择参与的主机不会受到任何惩罚。
...
如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`查看：任何活跃节点均可使用：
...
在第308纪元期间将进行一次加急投票，合并两项变更，均将于第309纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导流程。
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可选择参与并提前衡量对该模型的需求，未选择提供该模型的主机不会受到任何惩罚。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复服务且最小化停机时间的路径。下一纪元将进行一次投票以恢复Kimi。
* **Qwen3-235B——退役（独立、无关变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容变更。它与Kimi的情况无关，仅因同样是PoC阵容变更而被一并纳入。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（若尚未准备就绪），以避免在epoch边界时遭遇Hugging Face速率限制。
...
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不参与的主机也不会受到惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：
...
对于这种仅针对devshard的运行时升级，无需重启节点容器或执行手动主机步骤。

**关键变更**

- 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪有效载荷，以避免长运行会话将所有已服务的推理保留在RAM或状态中。
...
本提案涵盖devshard v2发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对devshard的升级。它独立于全链软件升级。如果获得批准，v2将与现有的v1 devshard运行时并行运行。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪有效载荷，以避免长运行会话将所有已服务的推理保留在RAM或状态中
...
新进程在`/devshard/v2`前缀下提供服务。现有的v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。此类升级期间无需重启节点容器或执行手动主机步骤。
...
MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需采取任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获批准，并在区块 `4267300` 成功执行了主网升级。
...
针对 [升级提案 v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **通过**。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从第 278 个纪元起，未采取任何操作的主机会遭受每纪元 15% 的全权重惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅适用于您共识权重中源自 Kimi 的部分**；您源自 Qwen 的权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，此次重新提交包含多项更新。
...
- 将 devshard 非ces 限制从 `20_000` 提升至 `1_000_000`。
...
请所有主机验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再进行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约在 5 月 26 日 14:42 UTC → 升级周期剩余阶段跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 该提案将有意跳过从升级高度到升级纪元结束（10000 块宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最后 3 个纪元）。
...
2. 监控下一阶段的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重正确反映。
...
- 在 vLLM 0.20.1 发布后，使用 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`
    - Kimi K2.6：`0.78`
    - MiniMax-M2.7：`0.3024`
    - 参考数据：[https://docs.google.com/spreadsheets/d/1dHHlbhW1_hVgd7Q6MtmcVSOpmnl7NnynoTzPHJ1oU-g/edit?gid=0#gid=0](https://docs.google.com/spreadsheets/d/1dHHlbhW1_hVgd7Q6MtmcVSOpmnl7NnynoTzPHJ1oU-g/edit?gid=0#gid=0)
...
- 将创世守护者调整投票权减少至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不参与投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。若如此，请先确认您正在运行当前版本，再进行后续操作。
...
如果您无法直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个纪元）。
...
- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被削减。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断会立即再次使其失效。计数器现在在重新激活和即将升级时重置。
- 在 v0.2.12 之前加入的 DAPI 没有在冷到暖的授权中包含 `MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应交易商投诉。
...
- 状态快照减少了长期运行会话的恢复工作量。
- 负载查找固定于托管纪元，对纪元边界和遗留纪元 0 请求提供回退。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能延迟启动。这延迟了 api 激活，导致部分矿工跳过了确认 PoC。
...
预评估与PoC启动之间有500个区块的窗口期。在此期间，没有CPoC任务，因此已声明意向的主机可安全地将其模型节点切换至 `Kimi K2.6`。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机，请保留您的MLNode使用 `Qwen235B`，并于明天提交下一个周期的意向。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个周期激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200，8xB200）上的模型计算复杂度。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
有关这些更改的更多详细信息，请参阅治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
- **协议加固。** 对保留节点（`POC_SLOT=true`）进行随机抽样以进行单次 PoC/CPoC 时间验证。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者 slashing 与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位完整熵值，替代之前的 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者罚没与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于接近 PoC 启动时间运行的节点，稳定性有所提升。

更新修复了 PoC 启动附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
这些性能改进可根据工作负载和网络状况，使每个区块的推理数量提升高达 100 倍。
...
目前，通过每个推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，这使得每个区块可容纳的推理数量最多提升 100 倍，具体取决于工作负载和网络状况。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能同时启动大量剪枝活动，造成不必要的运营压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
对于已经使用代理层的团队，这可能是简化架构、依赖原生行为的好机会。实际上，这应能带来更清晰的集成模式和更易维护的系统。
...
如果您没有访问持有投票权的冷密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求他们授权您代表其投票。没有该授权，其他账户无法提交投票。
...
3. 当前被保留且不参与 PoC 生成的 MLNodes 仍被用于 PoC 验证。这可能导致推理遗漏。新版本将此类节点排除在 PoC 验证之外。
...
现在提供一个新的 vLLM / mlnode 构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍有活跃请求而导致潜在权重下降的风险。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能会波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
由于有效归一化窗口比早期名义模型长约5个区块，每个周期重新计算的权重成比例降低。
...
由于启用了PoC持续时间归一化，且有效的参考窗口现在比之前的名义假设长约5个区块，从第176轮开始的权重值反映了更新的计算模型。
...
关于升级提案 v0.2.10 的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
**宽限期：** 升级后引入了一个3000区块的宽限期，在此期间不进行确认 PoC，并且对升级当期的错过率和无效率阈值要求更宽松。
...
- 每1单位算力 0.032 GNK（约每H100为10 GNK）
- 错过率或关押罚没 0.01%
- 无效推断罚没 0.5%

这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的0.5%。而所需抵押物仅占每日奖励的约24%。

**警告：** 抵押参数将 Regardless of 投票结果生效。如果该提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。

投票结束后、第180周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其奖励将被削减至五分之一。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个为期3000个区块的宽限期，期间无确认PoC，并在升级时段放宽了错过率和无效率的阈值。
...
- 每1单位算力0.032 GNK（约每块H100为10 GNK）
- 错过率或封禁惩罚为0.01%
- 无效推理惩罚为0.5%

这意味着，在单个周期内，即使被惩罚，矿工最多也只能损失其抵押物的0.5%。所需抵押物仅占日收益的约24%。

提案提交投票后，将另行发布公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果证明稳定性良好，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
目前正评估此重叠的影响。初步观察表明，没有节点记录PoC提交，导致该周期累计权重为零。
...
链上推理访问当前已开放，不再仅限开发者。推理请求可通过上一更新中引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 已开放供审查，用于下一次链上软件升级 v0.2.10。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约两天。
...
此升级通过实际 PoC 已用时间标准化参与者权重，以减少区块时间漂移影响，并使权重结果与实际执行时长保持一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式转让
...
2. 社区审核该PR后，预计从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，以启动正式的治理投票流程。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此次升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串形式编码而非数字，int64/uint64值也改为以字符串形式编码。
...
现已提供补丁以解决导致PoC周期中区块验证暂停的问题。建议主机尽快应用该补丁，以确保正确的PoC验证行为并安全恢复区块生产。
...
- 下一个 PoC 周期（从纪元 158 到 159 的过渡）至关重要。请确保在线，以便在需要时能够及时应用后续步骤或缓解指令。
...
- 服务于其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可自动切换至`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在因cPoC惩罚而减少或排除奖励的情况下，未计入部分将转入社区池。此前，此类奖励会在其他参与者间重新分配。
...
此笔记本展示了构建上述表格所用的过程：[https://github.com/gonka-ai/gonka/blob/gm/debug-155-1/debug-validation.ipynb](https://github.com/gonka-ai/gonka/blob/gm/debug-155-1/debug-validation.ipynb)。
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置需符合PoC v2要求，该要求自第155个纪元起生效。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
- 您的所有ML节点均提供Qwen/Qwen3-235B-A22B-Instruct-2507-FP8。这是唯一为PoC v2权重做出贡献的模型。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未同时满足两项条件的节点，在网络切换为单模型配置后将不再符合资格。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦在活跃 Host 中观察到足够水平的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。在网络切换至单模型配置后，未同时满足两项条件的节点将不被视为 PoC v2 参与资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
该提案将当前的稳定期延长约两周。延长的期间旨在为即将到来的 PoC 变更及相关网络升级进行额外测试。有关新 PoC 开发进展的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

该延长也为主机提供了时间来为新的 PoC 要求准备其设置，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个纪元中逐步进行更新。
...
当前有一个新的治理投票正在进行。它通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块2,222,222处的参与者注册截止时间对齐。
...
1) 网络将使用一个 `allowlist` 运行，其参与者在多个周期中：
...
在成功采用解决PoC相关共识故障的补丁后，新的治理投票目前正在生效。
...
若获批准，网络将临时使用一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的精确列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），以及
...
每个活跃主机的参与对网络推进和恢复正常运行至关重要。请保持在线，并在说明发布后准备应用更新。
...
- 临时重新激活创世验证者增强——一种此前曾使用过的、有时间限制的防御机制，现提议重新启用。
...
本次升级包含的预备性变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得等于第117轮奖励1.083倍的 payouts，统一应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意破坏或施压系统的行为引起的。本提案旨在通过一系列临时措施增强网络在高负载和不利条件下的韧性。

创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并激活一些额外的保护措施。

**关键变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前使用过的、有时间限制的防御机制，拟重新激活。
...
此次升级正式将此前通过手动 API 更新分发并已在网络中使用的重大修复纳入协议。这些修复包括：
...
- 主机级注册：在区块 2,222,222（约两周后）之前暂停新主机注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期内暂停新开发者地址注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备变更，使未来可通过治理机制控制参与者入驻和推理执行，而无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第 117 期活跃的节点，将获得额外 payouts，金额为第 117 期奖励的 1.083 倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
如果您的 `application.db` 占用大量空间，可参考 [此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 中的技术进行清理。

**参考**

Genesis 验证者增强的完整技术细节请见：
[https://github.com/gonka-ai/gonka/tree/main/proposals/early-network-protection](https://github.com/gonka-ai/gonka/tree/main/proposals/early-network-protection)
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个错误：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。

参考：[https://github.com/gonka-ai/gonka/pull/517](https://github.com/gonka-ai/gonka/pull/517)

此 `API` 版本改进了对失败推理的处理韧性，并减少了推理遗漏计数问题。同时引入了对 PoCBatch 和 PoCValidation 交易的批处理功能。
