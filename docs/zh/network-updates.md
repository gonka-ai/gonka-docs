...
下一次链上软件升级 v0.2.16 的拉取请求现已开放审查 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)。有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能在下一个升级周期中获得社区奖励。

主网链/API 工作重点在于可信权重、动态系数、交易费分组和 PoC 挑战。如果提案获得批准，治理、BLS 和 PoC 验证权将仅限于上一个纪元已确认的算力，而新的算力仍能立即获得奖励。治理可为每个模型设置目标份额和系数范围。升级时这些范围保持关闭，因此系数和奖励不会变动。新增了消息组和汽油规则，每个组默认禁用，因此协议仍不收取费用。获准的挑战者可将一个活跃主机从推理任务移至全容量 PoC，直至下一次常规 PoC。

**关键变更**

1. **可信权重。** 在 v0.2.16 之前，声称的算力突然增加会立即提升参与者在治理、BLS 和 PoC 验证中的权力。这使得未经确认的算力对共识和其他参与者的验证产生影响。本次升级将该权力限制为上一个纪元已确认的算力。新加入或回归的参与者起始为 0，若确认失败，则下一个基线设为 0。新的算力仍能立即获得奖励。只有对信任敏感的权力需等待一个已确认的纪元。
2. **动态系数 v1。** 网络需要为每个模型提供可预测的吞吐量，以便用户依赖其可用性。每个模型的目标吞吐量应遵循预期需求。使用固定系数时，整个硬件类别往往会转向同一模型，这使得每模型的容量难以平衡和预测。本次升级允许治理为每个模型设置网络算力的目标百分比和系数范围。协议会动态调整该范围内的系数，使算力趋向目标。超过目标的算力按最小系数评分，以抑制过度供给。治理最初根据需求估计和数据源（如 OpenRouter）设置目标。后续版本可聚合主机估计，最终使用链上模型使用数据。此版本仅改变激励机制，不会自动切换主机部署的模型。升级时，最小值和最大值均等于当前比例，因此系数和奖励在治理打开范围前保持不变。
3. **费用。** 历史上，协议从未收取交易费用。因此攻击者可以以极低成本提交大量消息，而每个验证者却需承担处理和存储成本。本次升级对交易类型进行分组，并添加每消息汽油规则，使每组可单独定价。所有组默认禁用且不收费。治理稍后可启用并定价。升级提案信息也可在升级高度启用这些组。
4. **PoC 挑战。** PoC 和随机确认 PoC 用于证明主机声称的算力。在纪元其余时间，推理统计数据用于检查该硬件是否用于协议分配的工作。高频率的遗漏或无效推理可导致主机被移除。这提供了强大的持续检查，但其敏感性取决于推理量，可能无法揭示声称与可用算力之间的所有差距。PoC 挑战为此类情况增加了一层额外安全。获准的挑战者可要求一个活跃主机离开推理，以全容量运行 PoC，直至下一次常规 PoC。为发起挑战，挑战者需锁定目标剩余纪元奖励的一部分作为付款。若目标通过，将获得该付款；若失败，挑战者获得退款，目标将受到与确认 PoC 失败相同的惩罚。挑战期间的推理遗漏不计入目标。仅允许名单中的 devshard 托管创建者可发起挑战。若允许名单为空，则任何人皆可发起挑战。
更多详细信息及其他变更请见：[https://github.com/gonka-ai/gonka/pull/1535 ](https://github.com/gonka-ai/gonka/pull/1535 )

**行动项**

1. **所有人。** 现在 —— 审查拉取请求。阅读 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)，对任何发现、疑问、改进建议、边界情况或漏洞留下评论。
2. **主机。** 投票前无需其他操作。不要更新容器、重新授权密钥或为此升级迁移模型。
...
该提案以 123,754 票赞成、反对票数为零获得通过。

**变更内容**

DevShard v5 已添加至链上批准的 devshard 版本列表中：
...
主要协议新增功能是高度同步（height-sync），为 DevShard 会话提供已签名、可重放的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立协议级逻辑时间。该版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机所需操作**

无需操作。`versiond` 将下载新运行时，验证其 sha256，并与已运行的版本并行启动。`v3`、`v4` 和 `v4.1` 仍为批准版本，因此现有会话和路由前缀不受影响。这是运行时版本，非网关发布：任何网关迁移将另行公告并附有独立指南。
...
这是仅针对 devshard 的变更。Cosmovisor 不涉及，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储值一致，并启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能是高度同步（height-sync），为 devshard 会话提供已签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级 QoS 建立协议级逻辑时间。该版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。

该版本已由核心测试团队和外部测试实验室完成测试。
...
收款人为活跃于多个开发周期和发布版本的 Gonka 核心贡献者：@akup / @a-kuprin, @qdanik, @Ryanchen911, @aikuznetsov, @snevolin, @shd 和 @redstartechno。
...
投票将于 **2026 年 9 月 18 日 16:47 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成票，且反对票（含否决）低于 33.4%。

请投票。上一次尝试因参与率不足而失败，无人反对，因此参与是此提案获得批准的唯一障碍。
...
```
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。

## 提案被拒绝：批准 devshard v5 及赏金支付

提案 ID 为 104 的链上治理投票已结束。该提案因 `failed_reason`：`proposal did not get enough votes to pass` 被拒绝。

最终统计：赞成 25291，反对 0，否决 0，弃权 0。

无人投反对票。参与率约为网络权重的 6%，低于 25% 的法定人数，因此该提案仅因参与不足而失败。
...
自第 394 个纪元起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供服务、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的 15%，且该惩罚将在您做出选择前持续生效。
...
提案编号104做两件事：将 `v5` 添加到批准的 devshard 版本列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖金。

**Devshard v5**

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，然后启动运行时。
...
该提案还发放 91,300 USDT 的贡献者奖金，分配给七位接收方：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重占比25%的投票参与率、多数赞成票，且反对票（含否决）低于33.4%。
...
**提案通过：补偿 Daniil 和 David Liberman 参加 All-In Summit 2026 的费用**

提案编号103的链上治理投票已结束。

该提案已**通过**。社区金库已拨款15,500 USDT，用于补偿 Gonka 协议的联合创建者 Daniil 和 David Liberman 参加 All-In Summit 2026 的两张参会通行证费用。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人7,500 USDT，门票已先行支付。本次补偿涵盖两张门票及提取资金的交易费用。本提案未申请补偿机票、住宿及其他费用。
...
提案已获批准。Quant Mesh Limited被正式批准为Gonka-Ledger集成的法律合同合作方，从社区资金池中拨付350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费用75,000 USDT以及350 USDT的处理成本。授权与资金拨付严格限于完整提案中描述的Ledger集成。
...
提案已获批准。`zai-org/GLM-5.3-Flash`现已被批准为PoC模型集的一部分，而`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`已从`poc_params.models`中移除。这两个ID仍保留在`GET /v1/governance/models`（推理目录）中，但不再作为PoC模型。

`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：其PoC参数和权重保持不变。

该模型及其参数由kaitaku.ai团队提出，经vbgd0独立验证，并联合提交。相关测量数据及阈值设定的依据见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash的激活纪元为**394**。从该纪元起，`penalty_start_epoch`生效，因此未明确选择的主机将面临不参与罚金。如您计划提供该模型，尽早声明意向将最大程度帮助组形成。主机指南：[GLM-5.3-Flash启动指南](./host/glm-bootstrap.md)。
...
提案已获批准，`v4.1`已被加入批准的devshard版本列表。`versiond`将下载二进制文件，验证其SHA-256值是否与链上存储的值一致，并启动运行时。主机无需手动操作。

v4.1将与现有的v3和v4运行时并行运行，因此当前流量仍可正常服务，经纪人可逐步迁移其网关。
...
`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：其PoC参数和权重保持不变。所有其他链参数保持不变。

该模型及其参数由kaitaku.ai团队提出，经vbgd0独立验证，现共同提出。发布输入、测量值及阈值背后的推理详见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
投票将于**2026年9月10日23:17 UTC**结束。提案通过需满足：权重25%的法定人数、多数赞成，且反对票低于33.4%且无否决权。
...
如果您没有直接访问持有投票权密钥的权限，或希望使用其他密钥代为投票，请参阅有关从冷密钥向热密钥授予治理投票权限的指南。

## 2026年9月9日

**devshard v4.1运行时提案已进入治理流程**

提案ID 100将`v4.1`添加到批准的devshard版本列表中。

这仅是devshard的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及Cosmovisor，链不会停止，推理服务继续运行。若获批准，`versiond`将下载二进制文件，验证其SHA-256值是否与链上存储的值一致，并启动运行时。

v4.1与现有的v3和v4运行时并行运行。提案通过本身不会迁移任何流量，因此经纪人可在之后逐步迁移其网关。
...
投票将于**2026年9月10日22:36 UTC**结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票（含否决票）低于33.4%。
...
该提案已获批准。它将从社区金库拨出100,000 USDT，用于支持Gonka参与2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金用于覆盖黄金赞助套餐及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
该提案已获批准。它将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提升至0.246。所有其他模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为`PROPOSAL_STATUS_REJECTED`，结果为：`failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终投票结果：赞成15452，弃权3093，反对0，否决0。
...
2. 差异高度标记为协议增加了逻辑时间。`observed_height`/`observed_block_hash`标记已存在于所有先前包含时间戳字段的消息中。后续版本可将`confirmed_at`和`started_at`等壁钟字段替换为高度标记。这解决了未来时间攻击问题，并为协议层处理流超时打开了可能性。本版本包含这些标记，但尚未将超时和密封决策切换至它们。

3. 新增了通过所有插槽往返的心跳机制，因此即使会话静默，仍能定期同步高度；而繁忙会话则可通过常规推理标记实现相同的同步频率。
...
投票将于 **UTC 时间 8 月 27 日 03:47** 结束。如能，请参与投票。
...
该提案已获批准。它调整了在周期早期数据不足时无效推断检查（SPRT）的行为，避免因样本过小而误触发。主机无需执行任何操作。
...
在周期初期，数据量尚不足，因此无效推断检查（SPRT）可能在样本足够大之前，仅因少量无效结果就触发。本提案使其更稳定：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本情况下，阈值接近 10% 的无效率，因此低于此阈值的诚实参与者不会被惩罚。真正的违规者仍会被触发（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效结果，且在 500 个样本时，观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如能，请参与投票。若未能在时限内达到加速阈值，提案不会被拒绝：它将转为普通提案，投票将继续进行更长的正常周期。若转为普通提案，您需重新投出选票。
...
一个稳定的网关版本目前正在一个网关上运行，并将用于首个结算阶段。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses和Invalid指标将用于链上。根据当前主网阈值配置，错误率低于10%的主机不会触发任何处罚或罚没，但作为协议开发过程的一部分，仍应进行调查。

错误率较高的主机应在结算开始前解决这些问题。无显著问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这些信息有助于排查主机端问题。超调度和隔离部分旨在为网关开发提供洞察，主机无需对此采取行动。

**下一步：****1) 接下来的24小时。** 主机应检查其统计数据并解决任何关键错误或配置问题。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，首先从较小模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi将稍后跟进，因为其数据在负载下目前可靠性较低。
...
针对DeepSeek的每模型参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需再做任何操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。

## 2026年8月14日

**MLNode 3.0.16发布**

如果您的所有节点均已运行新版本（3.0.16），则无需操作。

如果仍有节点运行旧版MLNode（即正在服务Kimi或MiniMax的节点，因为已运行DeepSeek的节点均已升级至3.0.16），请在方便时将这些节点升级至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。

MLNode镜像在`docker-compose.mlnode.yml`分支的[`vllm-0.25.1-upgrade`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)中已固定为3.0.16。

默认MLNode镜像（3.0.16）基于CUDA 13.0构建。如果您的主机仍使用CUDA 12.9，请改用`3.0.16-cu129`标签。这是同一MLNode 3.0.16，但专为CUDA 12.9构建。两个标签均在同一个compose文件中。
...
将实时社区销售合约和所有包装代币实例迁移至新存储的CosmWasm代码，并为未来实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查为 epoch 360 启动快照——在此前请保持委托多样化，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始（区块 **5,552,115**）前约 500 个区块的部署窗口内部署 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。
...
委托主机：选择**非守护者**的 DeepSeek 意向主机，并将权重分散至独立目标——参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
DeepSeek 已通过投票进入启动阶段——如果您计划提供服务，请验证您的设置并立即提交意向。
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则表示您已达标。

如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请勿忘记委托，以免遭受不参与罚金。
...
1. 声明意向。在提案94通过后、快照块5,536,224（约2026年8月13日02:40 UTC / 8月12日19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359启动前约500块的安全窗口内（块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。此窗口内切换是安全的，因为其中没有cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿将委托指向守护节点；请将委托分散至独立的DeepSeek主机。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 若要保留当前模型。在第360轮之前无需任何操作。从第360轮起，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的不参与罚金。

区块高度为精确值；时间仅为估算，将在接近轮次时确认。[更多关于DeepSeek启动的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。现有参数或模型均未修改，唯一变更即新增该模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 要求MLNode运行vLLM 0.25.1并使用发布候选节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留当前模型：在第360个周期前无需任何操作；从第360个周期起，如果您未提供该模型，则需为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
该提案已获通过，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换二进制文件——无需重启主网或手动操作主机。

**API 二进制更新：`v0.2.15-post3`**

新的 API 二进制文件 `v0.2.15-post3` 已可用。

此版本提高了PoC种子的可靠性，并包含若干API和部署修复。
...
这只是一个性能和资源管理补丁，除此之外无其他改动。协议、推理、验证和经济行为均保持不变。变更内容：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。
...
该提案已获通过，并于区块 5316315 成功在主网上执行。同一提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
2. 一个**v0.2.15 主网软件升级**，移除大量过时代码、遗留交易处理路径及相关组件，这些组件已不再符合当前架构的需求。
...
v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个面向多实例高可用的版本：在共享 Postgres 后端部署 N 个 versiond / devshardd 副本，由 versiond-router 使用粘性会话路由和验证租约排他性进行管理。网关仅通过 gRPC 与链交互。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
更多详情请查看拉取请求：[https://github.com/gonka-ai/gonka/pull/1497 ](https://github.com/gonka-ai/gonka/pull/1497 )
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并会自动清理（保留最近 3 个周期）。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和网桥。为确保主网上线升级期间以太坊网桥稳定，请提前按照 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。若您拥有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。若您的仪表板从链上读取并显示数据，则仍可正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 `application.db` 的 Cosmovisor 备份通常为数十 GB，请提前确认）。有关如何安全清理 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 仍将作为默认选项，并自动清理（保留最近 3 个周期）。
...
- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是将单体架构重构为高可用、容错、可扩展架构的一系列 devshard 和网络节点变更中的首次更新。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的RAM增长问题 —— 即7月15日事件中网络节点OOM的根本原因。
...
2. 提供Kimi服务的主机：在投票结束后、区块 **5,105,276**（2026年7月17日约12:05 UTC）前立即声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**请勿委托给守护者节点**；将委托分散到独立的Kimi主机上。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取更新的委托指引。
4. 经纪人 / devshard创建者：如果您的任何网关流量仍使用v1/v2或`/v1/devshard` —— 请在投票结束前切换至`/devshard/v3`。
...
- 意向截止时间（第330轮）：区块 **5,105,276** — 2026年7月17日约12:05 UTC（约05:05 PDT）。
...
`moonshotai/Kimi-K2.6`在第328-329轮中失去了PoC验证多数支持（详见[事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是最快恢复服务且最小化停机时间的方式 —— 与6月提案78的恢复路径相同。
...
2. 提供Kimi服务的主机：请保持Kimi设置就绪，并准备好在引导PoC时切换您的MLNode。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（紧急，0.667 是票阈值；参与率重要，请尽快投票）。
* 引导意图截止日期和纪元时间将在引导说明中跟进。

## 2026年7月15日

**Kimi-K2.6 事件（纪元328–329）：发生情况、恢复计划及委托指导变更**

在纪元328中，`moonshotai/Kimi-K2.6`失去了其PoC验证的多数票。在纪元329开始时，运行Kimi的主机被移出该组。网络其余部分正常运行。

**发生情况**

两台由守护者运营的运行Kimi的服务器同时故障：

* 其中一台服务器上，运行Kimi的MLNode因提供商端问题而崩溃。大量Kimi委托集中在这台主机上，因此其故障导致这些投票立即消失。
* 第二台服务器的网络节点因内存不足而失效。

Kimi的验证投票一直接近2/3阈值，因此这次故障足以使其低于多数。守护者投票权未被触发：目前没有每模型投票权重的守护者会被从投票权中过滤掉。这是一个已知问题，将在即将发布的v0.2.14升级中修复。

这种情况与纪元306–307事件类似。纪元329开始时的投票详细信息仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复Kimi的最安全方式是通过全新引导（约1.5个纪元）重新添加：

1. [今天] 一项紧急提案将在下一次PoC之前移除Kimi，并在之后立即重新添加。随后将发布包含提案ID、投票命令和截止日期的独立公告。
2. [明天] Kimi将再次通过标准引导流程：声明意图、部署窗口、PoC存储提交。将在意图截止日期前发布说明。

**委托指导变更——请阅读**

之前的指导（包括2026年6月27日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤销：

* **请勿委托给守护者节点。** 守护者是PoC验证的后备机制。将委托集中于它们会将主机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。目前正在讨论协议级限制。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重，且无单点故障。

**已在进行中的修复*** PoC验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce重复——v0.2.14（当前由早期API更新处理）。
* 网络节点的RAM增长——已在devshard v3中修复；需要从v1/v2迁移。完全迁移后，v1/v2将被移除且`/v1/devshard`将关闭。为`api`和`versiond`容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急Kimi提案上链，请投票（公告将随后发布）。
2. 如果您计划提供Kimi：关注引导说明，并准备好声明意图。
3. 如果您为Kimi委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即把devshard流量迁移到`/devshard/v3`。这将解决RAM增长问题，并在移除v1/v2前必须完成。
...
devshard v3运行时已通过链上批准并添加至`DevshardEscrowParams.approved_versions`。

此提案涵盖[devshard v3发布。](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.14/proposals/governance-artifacts/update-v0.2.13-devshard-v3)

这是一个仅限devshard的运行时升级。它独立于完整链软件升级运行，无需链二进制升级。

提案批准后，v3现在与现有devshard运行时并行运行。新进程在`/devshard/v3`前缀下提供服务，而现有devshard流量可继续在早期运行时前缀上运行，直到经纪人切换流量至v3。

此发布将`devshardd`二进制文件作为Gonka发布工件发布。`versiond`会自动下载该二进制文件，验证sha256哈希值，并在现有`versiond`容器内启动额外的`devshardd`进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

已获白名单的 devshard 创建者应在主网 v0.2.14 链升级前，将推理流量切换至 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，且不依赖已弃用的经典 API 路径。同时提升了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换至 v3。在此类升级期间，无需主网重启或手动主机操作。
...
投票截止：2026 年 7 月 11 日，UTC 时间 06:41:34
...
devshard 部分准备了 v3 运行时，使代理能够在链升级期间无需依赖已弃用的经典 API 路径提供推理服务，同时提升 RAM 利用率并启用 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
在此分支上将单独发布 devshard v3 版本，并在主网链升级前进行部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供推理服务。

**提议流程**

1. 活跃主机请在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查本提案。
...
本次仅呼吁审查 Pull Request，不会启动正式投票。治理投票流程将在审查期结束后开始。

**Devshard v3 治理投票**

devshard v3 发布版本将在主网链升级之前提出并部署，以便经纪人能够在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作事项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定，请提前根据指南将 `api` 二进制文件和桥接器镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接器容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复补丁 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机需在第310轮期间声明其意向，并在新轮次开始前准备好节点。一个守护节点也将运行 Kimi，因此不愿运行它的主机可以选择委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换至 Kimi（PoC 开始时间约为6月28日 ~12:47 UTC）。
...
常规引导流程从第309轮开始。由于投票将在 PoC 前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于引导的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的非参与惩罚）。该系数的选择使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。此设计并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
...
1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 服务的主机需在 `start_poc - deploy_window` 的引导快照前为对应模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 和 >2/3 可达性）。这使操作员可在配置硬件前确认模型是否可行。
...
从第309轮开始（PoC 开始时间约6月26日 15:25 UTC；生效时间约16:00 UTC），前提是每个模型均满足引导资格：

* `moonshotai/Kimi-K2.6` 再次成为活跃的 PoC 模型。
* `zai-org/GLM-5.2-FP8` 作为可选 PoC 模型可用，且无非参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计将支持该提案。

**主机需采取的行动**

1. **要提供 Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口期，此期间切换是安全的，因为此期间无 cPoC。

2. **要提供GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意向并配置您的MLNode。提供该模型是可选的——未选择参与的主机不会受到任何惩罚。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换前都不会获得本周期的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308周期将进行一次投票以恢复Kimi。

**即将进行**

在第308周期将进行一次加急投票，合并两项变更，均于第309周期生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导程序。
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可选择参与并提前衡量对该模型的需求，不提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以实现快速重新引导；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前没有足够的投票数通过PoC验证。因此，提供Kimi的参与者已被移出组别，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快且停机时间最短的恢复方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi没有足够的投票通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的路径。下一周期将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi情况无关，仅因同样是PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 已升级为基础模型。**

由于重置必须在epoch 307结束前完成，因此这些变更合并为一次紧急投票。每个epoch约22.9小时，标准的48小时投票周期无法容纳在一个epoch内；而12小时的紧急周期可以。Genesis守护者应支持该提案。
...
3. 计划再次提供Kimi服务的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——epoch 308将进行恢复Kimi的投票。


**下一步计划*** **恢复Kimi K2.6** —— 在epoch 308中进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，且**不设置不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不参与的主机也不会受到惩罚。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。`versiond`自动下载二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动额外的`devshardd`进程。

对于这种仅devshard的运行时升级，无需重启节点容器或手动执行主机步骤。
...
- 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每个槽的总数。
...
4. 将每个推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每个槽的总数
...
有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期获得社区奖励。
...
MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
[升级提案 v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **批准**。

升级将在主网上于 **区块高度 4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）自动执行。

**提醒**

1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT 代币元数据以及 CW20 `wrapped_token` 代码 ID `105` 均通过升级处理器注册，因此桥接将在升级高度在主网上激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在预下载约 230 GB 的 FP8 权重。在引导窗口期间，Hugging Face 的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需对其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会遭受每纪元 15% 的全权重惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基准。此调整**仅适用于您共识权重中来自 Kimi 的部分**；您的 Qwen 来源权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，现已提交若干更新。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约 ⅓ 的参与率才能满足法定人数（参见 inference-chain 部分）。
- 必要准备：桥接容器检查、MiniMax 决策、仪表板更新、投出选票。
- 除非提案获得批准，否则链上无任何变化。
...
- 将 `MiniMaxAI/MiniMax-M2.7` 作为治理批准的模型和 PoC 模型添加。
- 更新推理验证阈值：
    - Qwen 235B：`0.940`
    - Kimi K2.6：`0.900`
    - MiniMax-M2.7：`0.922`
...
- 将 devshard 非法次数上限从 `20_000` 提高至 `1_000_000`。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，这使得剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
- 将 4 个早期主机和经纪商添加至 `allowed_creator_addresses`。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再执行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

请主机更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先遵循加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余阶段跳过确认 PoC（≤ 10000 块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约 43 分钟前）→ 首次 MiniMax PoC 阶段在升级后的下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。

**注意**

- 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅[文档。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
...
- 该提案将有意在升级高度到升级纪元结束（10000 块宽限期）期间跳过确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近 3 个纪元）。
...
第266轮的问题是由针对运行Kimi模型的主机的攻击引起的。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入第266轮。

在应用额外防护措施期间，推理服务可能暂时不可用。访问预计会逐步恢复，首先通过若干社区代理和中介端点。

**发生了什么**

在第266轮中，网络的活跃权重出现显著下降。

该问题已追溯至使用非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。
...
在相关防护措施实施期间，推理服务可能暂时不可用。访问预计会逐步恢复，首先通过若干社区代理和中介端点。

**第267轮中Kimi的权重**

由于现有协议规则：单一模型的总权重不得超过前一轮所有模型总权重的75%，Kimi在第267轮中的活跃权重较低。
...
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方恢复，预计会通过社区代理和中介端点逐步恢复。
...
- 包括：对Kimi（`0.78`）的权重重新校准、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复、以太坊桥接主网激活。
...
- 在 vLLM 0.20.1 发布后，基于 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`
    - Kimi K2.6：`0.78`
    - MiniMax-M2.7：`0.3024`
    - 参考数据：[https://docs.google.com/spreadsheets/d/1dHHlbhW1_hVgd7Q6MtmcVSOpmnl7NnynoTzPHJ1oU-g/edit?gid=0#gid=0](https://docs.google.com/spreadsheets/d/1dHHlbhW1_hVgd7Q6MtmcVSOpmnl7NnynoTzPHJ1oU-g/edit?gid=0#gid=0)
...
- 将 devshard 的 nonce 限制从 `20_000` 提高到 `1_000_000`。
...
- 将创世守护者调整投票权减少至约 25%，并将链上治理门槛设为 `0.25`。在守护者不参与投票的情况下，剩余 75% 投票权的有效门槛为 1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正常。部分主机可能已部署桥接容器。若如此，请先确认您正在运行当前版本，再进行任何后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个周期）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作（可能包括以太坊端合约激活），所有主机需验证其桥接容器已部署、运行最新版本且同步正确。
...
[此拉取请求](https://github.com/gonka-ai/gonka/pull/1143) 针对下一个链上软件升级 v0.2.13，现开放审查。

请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一次升级周期中符合社区奖励资格。

本次仅为拉取请求审查请求，不启动正式投票。治理投票流程将在审查期结束后开始，很可能明天开始。

**关键变更****inference-chain**

- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和未合格模型的诚实矿工被削减。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断会立即再次使其失效。计数器现在在重新激活和即将晋升时重置。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权授予中缺少 `MsgRespondDealerComplaints`。升级将补全该权限，以便它们能够响应交易商投诉。
...
- Devshard 需要为更大规模部署提供共享存储选项。现在可以使用 Postgres 作为主存储，SQLite 仍作为本地备用。
...
在上一个轮次中，发现Kimi-K2.6解析某些响应时存在一个轻微的bug。
...
在Kimi-K2.6引导过程中，主机观察到30%的最低直接参与阈值在实际中难以达成。为避免Kimi-K2.6在未来轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
对于多模型PoC，这意味着Kimi现在作为独立的模型组拥有自己的参与和奖励追踪机制。
...
提交了PoCIntent的主机应在PoC于块 `3874496` 开始前，至少将一个MLNode从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换到 `moonshotai/Kimi-K2.6`。

预评估与PoC开始之间有500个区块的窗口期。在此期间无CPoC任务，因此声明意图的主机可安全地将其模型节点切换至 `Kimi K2.6`。

请遵循指南完成所需部署步骤：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年5月4日

传输代理 `node1`、`node2` 和 `node3` 已被禁用。所有主网推理现在均通过 `node4` 路由，该代理采用新的 `devshard` 基础计费方式。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保持您的MLNode使用 `Qwen235B`，并于明天为下一轮次提交您的意图。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个轮次激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
版本 0.2.11 通过将运行时视图截断为强制模型来掩盖了此问题，因此即使持久化配置仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种截断，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送带有清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。其余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将随机采样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、将遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
部署、委托或明确拒绝新的治理批准模型（包含的模型将在升级后两个周期内激活）。请参阅[指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
此更新修复了 PoC 开始附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
这些性能改进可根据工作负载和网络条件，使每个区块的推理次数最多提升 100 倍。
...
下一版链上软件 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改进 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过每个推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行与验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本并非因其已完备，而是因为此类系统需要尽早暴露于真实网络环境中，某些问题仅靠本地测试难以发现。当前实现已设计为不会对矿工奖励造成负面影响。

**[`StartInference` 和 `FinishInference` 性能改进](https://github.com/gonka-ai/gonka/pull/812)**

- 减少 `MsgStartInference` 和 `MsgFinishInference` 的不必要的状态写入和查询开销。
- 简化统计处理，降低推理生命周期中的工作量，以提升区块执行的稳定性。

在类似主网的条件下，这使得每个区块可容纳最多 100 倍的推理量，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中将同时启动大量剪枝活动，造成可避免的操作压力。
...
如果您没有直接访问具有投票权的密钥，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
目前，通过每项推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间启动剪枝活动，造成不必要的操作压力。
...
在 `v0.2.10` 中发现了一个潜在漏洞。为降低当前升级前阶段的风险，建议在下次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
对于已使用代理层的团队，这可能是简化架构并依赖原生行为的好机会。实际上，这应能带来更清晰的集成模式和更易维护的系统。
...
2. 之前的请求重试系统即使因处理超时（而非 TLS 超时）失败，也会重试推理。
...
现在提供一个新的vLLM / mlnode构建，可在PoC开始时中断正在进行的推理请求，以降低PoC开始时仍处于活动状态的请求可能导致权重下降的风险。
...
- 确保[抵押](https://gonka.ai/host/collateral/)正确设置，否则从第180个周期起，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为了使PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
由于有效归一化窗口比之前的名义模型长约5个区块，每个周期重新计算的权重成比例降低。
...
针对升级提案v0.2.10的链上治理投票已结束。该提案已获批准，升级已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **当前生效的关键变更****PoC验证采样优化**
    ...
    此升级引入了一种新的PoC验证机制，通过为每个参与者分配一组固定的采样验证者，将复杂度从O(N^2)降低到O(N x N_SLOTS)。

    **按实时时间标准化PoC权重**

    此升级根据实际PoC耗时对PoC参与者的权重进行标准化，以减少区块时间漂移效应，并使权重结果与实际执行持续时间保持一致。

    **启用Qwen235B工具**

    此升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 添加了工具调用参数（ `--enable-auto-tool-choice` 、 `--tool-call-parser hermes` ），并将验证阈值设为0.958。
    要启用工具，必须重启MLNode容器内的vLLM。

    **其他协议更新**

- 修复：PoC和CPoC交集错误（PR #752）。
- IBC升级：将IBC堆栈升级到v8.7.0。
...
**宽限期：** 升级后引入3000个区块的宽限期，期间不进行确认PoC，并对升级所在周期的漏报率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工最多只会损失其抵押品的0.5%。而所需的抵押品仅占每日奖励的约24%。

**警告：** 抵押参数将不受投票结果影响而生效。如果该提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。

投票结束后、第180周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其奖励将减少5倍。
...
下一个链上软件版本v0.2.10的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的PoC验证优化（默认关闭），并实现基于实时的权重标准化，以提升网络公平性与可扩展性。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并且在升级时段内放宽了漏报率和无效率阈值。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押物的0.5%。所需抵押仅占日奖励的约24%。

提案提交投票后将另行公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，将逐步增加抵押品至代币经济白皮书中描述的水平（例如，每块H100约100 GNK），以支持网络的长期成功。
...
请审阅GitHub上的v0.2.10升级PR并留下您的反馈。有意义的审查贡献可能在下一次升级中获得奖励。

[https://github.com/gonka-ai/gonka/pull/695](https://github.com/gonka-ai/gonka/pull/695)

## 2026年2月13日

如果您的节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止。这是因为节点运行的是不再与网络兼容的旧版二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
此升级通过实际 PoC 持续时间对 PoC 参与者权重进行标准化，以减少区块时间漂移影响，并确保权重结果与真实执行时长一致。
...
2. 在社区审查该PR后，预计会从此分支创建v0.2.10版本，并提交该版本的链上升级提案，以启动正式的治理投票流程。
3. 如果链上提案通过，此PR预计将在链上升级执行后合并。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此升级后重新检查。由于ibc-go升级到v8.7.0，枚举现在以字符串形式编码，而非数字，且int64/uint64值现在也以字符串形式编码。
...
- 在当前模型配置下，B200 GPU的推理性能并未高于H100级GPU，基于观测到的推理轨迹。
- 如果未来通过治理引入更大或更苛刻的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与PoC v2中观测到的GPU类型间相对性能差异一致的结果。

**跟踪器（仪表板）维护者操作**

在更新权重分配生效后，跟踪器（仪表板）维护者可能希望审查其第159期及之后的系数，以确保与当前PoC v2权重分配保持一致。

## 2026年2月2日

**网络更新——补丁可用**

现已提供补丁以解决导致PoC周期内区块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为并安全恢复区块生产。
...
根据当前评估，旨在处理此情况的机制可能无法按预期运行。为防止在不确定或不安全的条件下完成验证者最终确认，网络在验证者选择之前已停止运行。

**下一步**

以下操作目前正在执行中：

- 验证没有任何验证者集能够达到所需的验证阈值
- 确认验证者最终确认前的网络状态
- 准备修复已识别问题的补丁
...
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入下一纪元的参与节点集。
...
在因 cPoC 处罚而减少或排除奖励的情况下，未计入部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
!!! note 
    未能同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 的权重分配仍需满足观察到的采用阈值和后续治理决策。

    **维护与运营**

- Cosmovisor：节点和 API 二进制文件的更新将自动处理。现有主机无需对运行中的容器执行手动更新。
- 磁盘空间：Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份。请确保有 250 GB 以上的可用空间。
- Postgres：升级后现在可以通过 Postgres 配置本地负载存储。
...
本指南说明主机应如何根据 v0.2.8 模型可用性变化及即将推出的 PoC v2 更新来升级其 ML 节点。ML 节点配置对 PoC v2 的合规性从第 155 个纪元开始生效。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个纪元后安排。迁移阶段结束后，不符合配置要求的 ML 节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8` 在迁移期间受支持，但不参与 PoC v2 准备或权重分配。参与 PoC v2 需要提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
成功参与 PoC v2 过渡需满足以下两项条件：

- 您的所有 ML 节点均提供 Qwen/Qwen3-235B-A22B-Instruct-2507-FP8。这是唯一贡献 PoC v2 权重的模型。
- 您的所有 ML 节点均已升级至 PoC v2 兼容镜像：
    - ghcr.io/product-science/mlnode:3.0.12
    - ghcr.io/product-science/mlnode:3.0.12-blackwell

!!! note "重要"

    - 仅提供正确模型而不升级 ML 节点是不够的。
    - 不满足两项条件的节点在网络切换为单模型配置后将不再有资格。
...
- 这有助于在 PoC v2 行为仍被观察期间降低风险
- 鼓励在多个纪元中逐步 rollout

**4. 更新 ML 节点模型：仅保留受支持的模型**
...
```
使用 ML 节点管理 API 将 ML 节点切换至受支持模型（`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`）。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不具备参与 PoC v2 的资格。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设备以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个周期内逐步进行更新。
...
当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将`allowlist`的过期时间与区块2,222,222的参与者注册截止时间对齐。
所有参与逻辑保持不变。此提案仅解决现有过滤逻辑中的一个小问题。

**可复现性与方法论**

`allowlist`是通过预定义的硬件配置模式从公开可观察的链上数据中推导得出的。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
1) 网络将使用一个 `allowlist` 运行，其参与者需在多个周期中满足：
...
- 激活发生在第135周期的首次PoC中
- 无需软件升级
- 从此时起，`allowlist` 一直有效，直至并包括区块2,222,222

**可复现性与方法论**

- `allowlist` 仅来源于公开可观察的链上数据
...
**现在开始：稳定期临时参与者 `Allowlist`**

在成功采用解决PoC相关共识故障的补丁后，一项新的治理投票现已生效。
...
若获得批准，网络将临时使用一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：
...
`allowlist` 基于公开可观察的链上数据，使用预定义的硬件配置模式得出。这些模式通过此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
此升级正式将此前通过手动API更新分发且已在网络中使用的关键修复纳入协议。这些修复包括：
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备变更，为未来通过治理机制控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外 payouts，金额为第117轮奖励的1.083倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意破坏或压垮系统的行为引起的。本提案旨在通过一系列临时措施增强网络在高负载和不利条件下的韧性。
...
此次升级将此前通过手动 API 更新分发并已在网络上使用的关键修复正式化。这些修复包括：
...
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续有效至区块 2,294,222（约 19 天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理机制控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案本身不启用任何此类治理激活的限制，需经额外治理投票决定。

**第 117 个周期奖励分配**

本提案涵盖与链暂停（第 117 周期）相关的两项奖励分配：

- 在第 117 周期活跃但未收到周期奖励的节点，将获得该周期的补发奖励。
- 所有在第 117 周期活跃的节点，将获得额外奖励，金额为第 117 周期奖励的 1.083 倍，均匀应用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于常态的推理遗漏率。
在许多情况下，这是由于一个缺陷：未标记为已完成的不支持格式的推理请求，尽管请求本身已被处理。以下更新将解决此行为。
