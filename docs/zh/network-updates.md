...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
...
主网链/API 工作重点包括：PoC 重复工件保护、早期份额检测、经典推理 API 废弃（在主网上禁用 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收者路由以及升级时的安全修复。

devshard 部分准备了 v3 运行时，以便代理在链升级期间无需依赖已弃用的经典 API 路径即可提供推理服务，同时改进 RAM 利用率并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
在此分支上将单独提出并部署 devshard v3 发布版本，然后再进行主网链升级。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级运行期间继续提供服务。

**建议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查此提案。
2. 在主网链升级之前，提出并部署 devshard v3 发布版本。
3. 代理将推理流量切换到 `/devshard/v3`。
4. 如果链上提案获得批准，此 PR 将在链上升级执行后立即合并。
5. 请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。

有意义的评审贡献，包括重要评论、漏洞发现和安全问题，可能在下一次升级周期中获得社区奖励。

本次仅为对拉取请求的评审邀请，不启动正式投票。治理投票流程将在评审期结束后开始。

**Devshard v3 治理投票**

devshard v3 发布版本将在主网链升级前提出并部署，以便经纪人能够在升级运行前将推理流量迁移到 `/devshard/v3`。

**主机操作事项**

1. 现在——评审 PR。请阅读 GitHub 上的 PR #1267，并对任何发现、疑问、改进建议、边界情况或漏洞留下评论。
2. 现在 / 主网升级前——更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据指南将 `ap`i 二进制文件和网桥镜像更新至 0.2.14。如果您已更新 `api` 二进制文件，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。若您拥有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人——将推理流量切换至 `/devshard/v3`。一旦 devshard v3 发布，将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
...
针对即将发布的 v0.2.14 链升级，已准备好永久修复该问题的补丁。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案——可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 将在第 311 纪元再次尝试引导。希望提供 Kimi 服务的主机需在第 310 纪元声明意向，并在新纪元开始前准备好节点。守护节点也将运行 Kimi，因此不希望运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 纪元、区块 **4,797,456** 之前（约 6 月 28 日 12:00 UTC）声明 PoC 意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若足够多的主机声明意向，则在第 311 纪元开始前约 500 个区块的窗口期内（PoC 开始于约 6 月 28 日 12:47 UTC）将您的节点切换至 Kimi。
...
**关键时间点*** 意向截止时间（第 310 纪元）：区块 **4,797,456** —— 约 6 月 28 日 12:00 UTC。
* 第 311 纪元开始：区块 **4,797,956** —— 约 6 月 28 日 12:47 UTC。

更多关于引导机制的说明：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026 年 6 月 26 日

**紧急治理投票 79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定为使 8xB300 上的 Kimi 获得与主模型 MiniMax 大致相同的共识权重。该设定并非偏向任何硬件——MiniMax 上已具备相同的权重。在 8xB300 上运行 Kimi 而非 MiniMax 的主机将获得约 1% 的增益。
...
常规引导流程从第 309 纪元开始。由于投票将在 PoC 前约 1 小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多引导信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（此举实际上取消了该模型的不参与惩罚）。该系数设定为使 8xB300 上的 GLM-5.2 获得与 MiniMax 大致相同的共识权重。该设定并非偏向任何硬件。在 8xB300 上运行 GLM-5.2 而非 MiniMax 的主机将获得约 2% 的增益。
...
1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 的主机需在 `start_poc - deploy_window` 的引导快照前提交 `MsgDeclarePoCIntent` 以声明该模型。
   
2. **资格预审快照。** 在 `start_poc - deploy_window`，链会快照意向和委托，并发出建议性的资格预审事件（治理批准、权重阈值 `W_threshold`、最少成员数 `V_min`、以及 >2/3 可达性）。这使运营商能够在 provisioning 前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置 MLNode。
   
4. **PoC 启动。** 成员资格由实际在 PoC 启动时提交 PoC 存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第 309 个周期开始（PoC 启动时间约为 6 月 26 日 15:25 UTC；生效时间约为 16:00 UTC），前提是每个模型均满足引导资格条件：

* `moonshotai/Kimi-K2.6` 将再次成为活跃的 PoC 模型。
* `zai-org/GLM-5.2-FP8` 将作为可选的 PoC 模型可用，且无未参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍将作为基础模型。

**为何加速**

该提案必须在第 308 个周期结束前完成，以便两个模型都能在第 309 个周期中引导。由于每个周期约 22.8 小时，标准的 48 小时投票周期无法容纳在一个周期内；而 12 小时的加速周期可以。创世守护者应支持该提案。

**主机所需操作**

1. **要提供 Kimi：** 在投票结束后**立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 启动前有约 500 个区块的安全窗口，期间切换是安全的，因为此期间无 cPoC。

2. **要提供 GLM-5.2：** 在部署窗口期间为 `zai-org/GLM-5.2-FP8` 声明意向并配置您的 MLNode。提供 GLM-5.2 是自愿的——未选择的主机不会受到惩罚。

3. **继续使用 MiniMax：** 继续提供 `MiniMaxAI/MiniMax-M2.7`——无需操作。

4. **在截止日期前对提案 79 进行投票。** 加速窗口很短。
...
* 计划再次提供Kimi的主机应保留Kimi的配置并准备在PoC 309时将MLNode切换回来——在第308纪元期间将进行一次恢复Kimi的投票。

**接下来的内容**

在第308纪元期间将进行一次紧急投票，合并两项变更，均在第309纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：
...
`moonshotai/Kimi-K2.6` 目前没有足够的投票支持以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快且停机时间最短的恢复方式。
...
**每项变更的目的*** **Kimi K2.6 —— 移除以便快速恢复。** 由于Kimi没有足够的投票支持通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的方式。下个纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B —— 退役（独立、无关变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi的情况无关，仅因同为PoC阵容变更而一并列出。
* **MiniMax-M2.7 —— 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307纪元结束前完成。每个纪元约22.9小时，标准的48小时投票周期无法容纳在一个纪元内；而12小时的紧急周期可以。创世守护者应支持此提案。

**若获批准的效果**

自第308纪元开始（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如尚未准备好），以避免在纪元边界遭遇Hugging Face速率限制。
2. **在截止日期前对提案78投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备在PoC 309时切换回来——在第308纪元期间将进行一次恢复Kimi的投票。
...
* 第308个纪元将很快形成：PoC开始约16:50 UTC，生效约17:25 UTC。
...
该发布将`devshardd`二进制文件作为Gonka发布工件发布。`versiond`会自动下载二进制文件，验证sha256哈希值，并在现有`versiond`容器内启动额外的`devshardd`进程。

对于此类仅devshard的运行时升级，无需重启节点容器或手动执行主机步骤。

**关键变更**

1) 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长期运行的会话在RAM或状态中保留所有已服务的推理。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长期运行的会话在RAM或状态中保留所有已服务的推理
...
该发布将`devshardd`二进制文件作为Gonka发布工件发布。若链上提案获得批准，`versiond`将自动下载二进制文件，验证sha256哈希值，并在现有`versiond`容器内启动额外的`devshardd`进程。
...
这是首次仅针对devshard的升级，因此流程不同于标准链升级。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且不会导致推理服务停机。
...
MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需采取任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获得**批准**，升级已在主网区块 `4267300` 成功执行。
...
1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT代币元数据以及CW20 `wrapped_token` 代码ID `105` 均通过升级处理程序注册，因此桥接将在升级高度在主网上激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在预下载约230 GB的FP8权重。在引导窗口期间，Hugging Face的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为**每个**经治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 的截止日期为**链纪元 `278`**。不采取任何操作的主机将从纪元278起，对其全部权重施加每纪元15%的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基准。该调整**仅适用于您共识权重中的 Kimi 部分**；您的 Qwen 部分权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新投票，现已提交多项更新。
...
- 治理：将创世守护者投票权降低至约25%，并将链上法定人数设置为0.25。如果守护者弃权，非守护者需在剩余的75%中实现约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 将创世守护者调整后的投票权降低至约25%，并将链上治理法定人数设置为`0.25`。在守护者不投票的情况下，这使得剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将启用Gonka主网与Ethereum之间的跨链转移（包括在Ethereum上包装GNK以及桥接USDC/USDT）。包装/解包脚本和操作员工作流程将另行文档化。
...
**`MiniMaxAI/MiniMax-M2.7` 在第278个周期前选择参与方式（从此时开始处罚）**
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正常。部分主机可能已部署桥接容器。在这种情况下，请先检查您运行的是当前版本，再采取进一步操作。
...
- 运营商时间线：投票于5月22日22:12 UTC结束 → 升级高度约为5月26日14:42 UTC → 升级周期其余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax引导快照在start_poc − 500块时（约提前43分钟）→ 升级后下一个周期边界开始第一阶段MiniMax PoC → MiniMax惩罚在链周期278时执行。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获得批准，此跳过是预期行为而非故障；新快照逻辑将在下一个周期开始。
- 若获得批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认设置并自动清理（保留最近3个周期）。
- 若提案失败（未达到法定人数或 `no_with_veto` 超过⅓），链上无任何变化，升级将不会发生。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
问题已追踪至具有非标准语义的推理请求。该攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
5. 在社区渠道或此线程中分享相关日志或观察结果。如果您的主机在第266轮受到影响，或在接下来的轮次中行为异常，这一点尤为重要。

## 2026年5月16日

**第266轮：PoC权重下降调查**

在当前轮次（#266）中，网络观察到活跃权重大幅下降。
似乎PoC投票未能收集到本轮所需的投票。确切原因尚未确认，社区正在积极调查此情况。
...
- 包括：为Kimi（`0.78`）重新校准权重，新增模型 `MiniMaxAI/MiniMax-M2.7`，更新验证阈值，重构devshard存储，以及多项PoC/奖励修复和以太坊桥接主网激活。
...
- 修复新模型启动期间确认PoC奖励核算。
- 在本升级轮次剩余时间内禁用确认PoC，以便新快照逻辑从下一轮开始干净启动。
- 当参与者再次激活时重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问。
- 将创世守护者的调整投票权降低至约25%，并将链上治理的法定人数设定为 `0.25`。在守护者不投票的情况下，这使得剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
要求所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意跳过从升级高度到升级纪元结束（3000区块宽限期）的确认 PoC。若获得批准，此跳过属预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
...
作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊端合约激活，所有主机需验证其桥接容器已部署、运行最新版本并同步正确。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推理即可立即再次使其失效。该计数器现在在重新激活和即将晋升时重置。
...
- 有效载荷查找固定于托管纪元，对纪元边界和遗留纪元0请求提供回退支持。
...
审查者可在此处查看完整的升级提案、迁移细节、测试摘要和建议流程：
...
docker start api
```
部署后，请再次检查9100和9200端口的服务器是否正在运行：
```
...
对于多模型 PoC，这意味着 Kimi 现在作为活跃模型组拥有自己的参与和奖励追踪机制。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，其权重极低。请保持您的 MLNode 使用 `Qwen235B`，并于明天提交下一个周期的意向。
...
- **协议加固**。保留节点（`POC_SLOT=true`）将随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。

**对节点的指导**

- 部署、委托或明确拒绝新治理批准的模型（所包含模型将在升级后两个周期激活）。请参阅[指南。](https://gonka.ai/docs/host/multi_model_poc/)
...
- **Devshard 独立运行时**（[#1045](https://github.com/gonka-ai/gonka/pull/1045)）。将 devshard 发布与 DAPI/主网发布周期解耦。
- **Certik 审计修复**（[#1020](https://github.com/gonka-ai/gonka/pull/1020)、[#1021](https://github.com/gonka-ai/gonka/pull/1021)、[#1022](https://github.com/gonka-ai/gonka/pull/1022)、[#987](https://github.com/gonka-ai/gonka/pull/987)、[#949](https://github.com/gonka-ai/gonka/pull/949)、[#988](https://github.com/gonka-ai/gonka/pull/988)、[#825](https://github.com/gonka-ai/gonka/pull/825)、[#1011](https://github.com/gonka-ai/gonka/pull/1011)、[#1029](https://github.com/gonka-ai/gonka/pull/1029)、[#789](https://github.com/gonka-ai/gonka/pull/789)）。已解决审计发现的问题。
- **协议加固**。对保留节点（`POC_SLOT=true`）随机采样进行单 PoC/CPoC 时间验证。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
- 一旦治理批准新模型，需部署、委托或明确拒绝每个经治理批准的模型（包含的模型将在升级后3个周期内激活）
...
本次更新修复了在PoC启动附近的一个边缘情况，该情况过去可能导致在特定条件下性能下降。
...
这些性能提升可根据工作负载和网络条件，使每个区块的推理次数最多提升100倍。
...
请主机们审阅[GitHub](https://github.com/gonka-ai/gonka/pull/813)上的提案并投票。
...
下一版本v0.2.11的链上升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每项推理交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅处理会话创建和最终结算。
...
在类主网条件下，这使得每个区块可容纳多达 100 倍的推理量，具体取决于工作负载和网络条件。  ￼
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时启动剪枝活动，造成可避免的操作压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完备，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
强烈建议主机在升级前按照提供的说明剪枝 `application.db`。
提前执行此操作至关重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时启动剪枝活动，造成不必要的操作压力。
...
如果您无权访问持有投票权的冷钱包密钥，建议提前安排投票委托。请联系该密钥的所有者，请求授权您代为投票。若无此授权，其他账户无法提交投票。
...
现在提供一个新的 vLLM / mlnode 构建，它在PoC开始时中断正在进行的推理请求，以降低PoC开始时仍处于活动状态的请求可能导致的权重下降风险。
...
抵押参数更新提案未达到法定人数，因此根据当前治理规则被拒绝。这意味着更新后的参数将不会被激活。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能发生波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平仍相对较低时存入最高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际的 PoC 经过时间对 PoC 参与者权重进行归一化，以减少区块时间漂移影响，并使权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入了3000个区块的宽限期，在此期间无确认 PoC，并且在升级当期的 epoch 中，对错过率和无效率的阈值要求更为宽松。
...
这意味着，在单个 epoch 内，即使被惩罚，矿工最多也只能损失其抵押品的0.5%。而所需抵押品仅占日收益的约24%。

**警告：** 抵押参数将无视投票结果生效。如果该提案未通过，创世时定义的抵押参数将在第180个epoch自动激活，而非上述参数。

投票结束后、第180个epoch之前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180个epoch起，其收益将被削减5倍。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并且在升级时段内放宽了漏报率和无效率阈值。
...
- 如果 `application.db` 占用大量磁盘空间，可应用此处描述的清理技术 [https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
- 升级后，Postgres 可作为本地负载存储的选项。
...
正如AMA期间讨论并基于社区成员提出的论点，建议从较小的抵押要求和最低惩罚开始。
...
这意味着即使在一个周期内被惩罚，矿工最多仅会损失其抵押的0.5%。所需抵押仅占每日奖励的约24%。

提案提交投票后，将另行发布公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而不是上述列出的参数。
...
请在GitHub上审阅v0.2.10升级PR并提供您的反馈。有意义的审阅贡献可能会在下一次升级中获得奖励。
...
此升级通过实际PoC耗时归一化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
2. 社区审核该PR后，预计将从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，以启动正式的治理投票流程。
...
- 在 PoC 外部使用标准 vLLM 基于相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与 PoC v2 中观察到的相同 GPU 类型之间的相对性能差异。
...
针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并已在主网第2451000区块成功执行升级。此次升级实现了PoC v2权重分配机制，并完成了向传统PoC机制的过渡。

**注意**

- 下一个PoC周期（从第158纪元过渡到第159纪元）至关重要。请确保在线，以便在需要时能够及时执行后续步骤或缓解措施。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的ML节点有资格进入下一个（159）纪元并参与PoC v2权重分配。运行其他模型的ML节点将不会被纳入下一纪元的参与节点集合。
...
下一版本v0.2.9的升级提案现已上链并开放投票。若获批准，该提案将启用PoC v2用于权重分配，并通过链上治理完成对传统PoC机制的过渡。
...
- MMR 承诺：工件存储通过默克尔山峰承诺移至链下；仅 `root_hash` 和 `count` 记录在链上。
...
!!! note 
    未能同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去 PoC v2 参与资格。PoC v2 的权重分配仍受观测采用阈值和后续治理的约束。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML 节点版本，可评估是否具备过渡到 PoC v2 的条件。一旦观察到活跃主机中约 50% 的足够采用率，可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML 节点不足以参与 PoC v2。一旦网络切换到单模型配置，未满足两项条件的节点将不被视为有资格参与 PoC v2。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机准备新的 PoC 要求提供了时间，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个周期内逐步执行更新。
...
`Allowlist` 过期时间：在区块 2443558 自动过期。
...
2) 此前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块2,222,222结束。
...
- 报告的硬件特征已根据预定义的常见硬件配置模式进行评估，以识别偏离和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
此前表现出持续偏离这些模式的参与者，在稳定窗口于区块2222222结束前，不包含在 `allowlist` 中。
...
- 临时重新激活创世验证者增强——此前曾使用过的、为期有限的防御机制，现提议重新启用。
...
本次升级中包含的预备变更，为未来通过治理机制控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的 payout，均匀适用于所有符合条件的节点，包括已领取原始奖励的节点。
...
近期网络增长带来了若干挑战。在过去几天中，网络经历了多次问题，其中一些似乎是由故意破坏或压力测试引起的。本提案旨在通过一系列临时措施增强网络在高负载和不利条件下的弹性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并激活一些额外的保护措施。
...
- 开发者地址注册：在稳定期间暂停新开发者地址注册。一个预定义的 `allowlist` 开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续有效至区块2,294,222（约19天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的约束，需另行投票决定。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由一个缺陷导致的：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
