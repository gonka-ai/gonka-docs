...
!!! note "关于本页面"
    ...
    本页面不保证内容完整。如需获取最新信息，包括治理投票的启动及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。

## 2026年9月22日

**DevShard 版本：仅保留 v4.1 和 v5**

v3 和纯 v4 将从链上批准的版本列表中退役。将随后提出治理提案，我们将在投票窗口开放时发布相关信息。

如果您仍在使用 v3 或 v4，请拉取这些镜像并重启：
...
您应看到 v4.1 和 v5。如果您只看到 v3 或 v4，则镜像未更新。
...
该提案以 123,754 票赞成、反对票数较少的结果获得批准。
...
主要协议新增功能是高度同步（height-sync），它为 DevShard 会话提供了一个经过签名、可重放的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立了协议层的逻辑时间。此版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机所需操作**

无需操作。`versiond` 将下载新的运行时，验证其 sha256，并在现有运行版本旁启动。`v3`、`v4` 和 `v4.1` 仍为批准版本，因此现有会话和路由前缀不受影响。这是一个运行时版本，而非网关发布：任何网关变动将另行公告并附带独立指南。

**奖励发放**

该提案还从社区销售合约中向多个开发周期中做出贡献的核心贡献者发放 91,300 USDT：@a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。所有奖励均已链上执行。
...
提案 ID 105 将 `v5` 添加至批准的 devshard 版本列表，并从社区销售合约中分配 91,300 USDT 作为贡献者奖励。

**devshard v5**

这仅是 devshard 的变更。Cosmovisor 不参与，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，将其 SHA-256 与链上存储的值进行验证，并启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能是高度同步（height-sync），它为 devshard 会话提供了一个经过签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议层 QoS 建立了逻辑时间。此版本还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。
...
投票将于 **2026年9月18日16:47 UTC** 结束。提案通过需满足：权重达到25%的出席率、多数赞成票，且反对并行使否决权的票数少于33.4%。

请投票。此前的尝试因参与率不足而失败，无人反对，因此参与是唯一阻碍本提案通过的因素。
...
最终计票结果：赞成 25291，反对 0，反对并否决 0，弃权 0。

无人投反对票。参与率约为网络权重的6%，低于25%的出席门槛，因此提案仅因参与不足而失败。
...
自第394个周期起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供、委托或拒绝。不采取任何操作将导致每周期扣除您全部权重的15%，并持续累积直至您做出选择。
...
提案 ID 104 有两个作用：将 `v5` 添加到批准的 devshard 版本列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖励。

**Devshard v5**

这是一项仅限 devshard 的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会停止，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。提案通过本身不会迁移任何流量，因此经纪人可在之后逐步迁移其网关。
...
该提案还支付91,300 USDT作为贡献者赏金，分配给七位接收方：
...
投票将于**2026年9月16日01:23 UTC**结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票且具有否决权的比例低于33.4%。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代您投票，请参阅有关将冷密钥的治理投票权限授予热密钥的指南。
...
该提案已获批准。从社区资金池中拨款15,500 USDT，用于报销Daniil和David Liberman（Gonka协议的联合创建者）参加All-In Summit 2026的两张参会通行证。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人7,500 USDT，门票已预先支付。报销金额涵盖两张门票及提取资金的交易费用。本提案未申请从社区资金池报销机票、住宿及其他费用。

最终投票结果：赞成184762，反对0，否决0，弃权0。
...
该提案已获批准。Quant Mesh Limited被正式批准为Gonka-Ledger集成的法律签约方，并从社区资金池中拨款350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该笔拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费75,000 USDT以及350 USDT的处理费用。授权和资金拨付严格限于完整提案中描述的Ledger集成。
...
该提案已获批准。`zai-org/GLM-5.3-Flash`现已成为经治理批准的模型，并纳入PoC模型集合，而`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`已从`poc_params.models`中移除。这两个ID仍保留在`GET /v1/governance/models`（推理目录）中，但不再是PoC模型。
...
GLM-5.3-Flash的激活纪元为**394**。从该纪元起，`penalty_start_epoch`生效，因此未明确选择的主办方将面临不参与罚金。如果您计划提供该模型，尽早声明意向将有助于团队更好地组建。主办方指南：[GLM-5.3-Flash启动指南](./host/glm-bootstrap.md)。
...
该提案已获批准，`v4.1`已被添加到批准的devshard版本列表中。`versiond`会下载二进制文件，并将其SHA-256与链上存储的值进行验证，然后启动运行时。无需手动操作主机。

v4.1现在与现有的v3和v4运行时并行运行，因此当前流量继续被服务，经纪人可以逐步迁移其网关。
...
提案ID 101将`zai-org/GLM-5.3-Flash`注册为治理批准的模型，并将其添加到PoC模型集中。同时将其从`poc_params.models`中移除`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：它们的PoC参数和权重保持不变。所有其他链上参数均保持不变。

该模型及其参数由kaitaku.ai团队提出，并由vbgd0独立验证，现共同提出。发布输入、测量结果及阈值设定的推理详见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash的激活纪元为**394**。从该纪元起，`penalty_start_epoch`生效，因此未明确选择的主机将面临不参与罚金。
...
投票将于**2026年9月10日23:17 UTC**结束。提案通过需满足：权重25%的参与率、多数赞成、且否决票少于33.4%。
...
提案编号 100 将 `v4.1` 添加到已批准的 devshard 版本列表中。

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：Cosmovisor 不参与，链不会暂停，推理服务持续运行。若获通过，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。提案通过本身不会移动任何流量，因此经纪人可在之后逐步迁移其网关。
...
提案已获**批准**。它从社区金库中拨出 100,000 USDT，用于支持 Gonka 参与 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金涵盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
`v4.0.2` devshard 二进制文件现已可用。主机可使用以下说明手动更新 v4 二进制文件。尚未安排自动更新。

**变更内容**

重启时，`v4.0.1` 会从 nonce 1 恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 随后在 60 秒就绪超时后终止子进程，导致客户端看到 HTTP 502 错误。

`v4.0.2` 加载最新快照，仅恢复其后的差异。此版本与之前的 v4 发布版本完全兼容。
...
提案已获**批准**。它将 `weight_scale_factor` 的 `deepseek-ai/DeepSeek-V4-Flash-0731` 从 0.214 提高至 0.246。其他所有模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，原因是 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的法定人数）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 因子源于被高估的 nonce/min 数值。测量修正见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需采取任何操作。

感谢所有参与投票的人。

## 2026年8月25日

**用于仅 devshard 升级的 PR 现已开放评审**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调全节点升级，不影响主网行为，也不预期会导致推理服务停机。如果通过治理流程批准，新的 devshard v5 运行时将与现有的 v4 运行时并行运行。

**关键变更**

1. v5 的主要目标是高度同步：每个主机和排序器在保证金日志中维护一个已签名、可重放的主网高度视图，即使在没有推理流量时也是如此。借助该时钟，devshard 现在可以明确知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度戳为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 戳已存在于所有之前携带时间戳字段的消息中。后续版本可以将 `confirmed_at` 和 `started_at` 等墙钟字段替换为高度戳。这解决了时间未来攻击问题，并为在协议层面处理流超时打开了道路。本次发布包含这些戳，但尚未将超时和密封决策切换到它们。

3. 通过所有插槽的往返心跳已添加，因此安静会话仍能定期同步高度，而繁忙会话则可通过常规推理戳实现相同的节奏。

**操作项**

请评审 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并在任何发现、问题、改进建议、边缘情况或潜在漏洞上留下评论。

有意义的评审贡献，包括重要评论、漏洞发现和安全问题，可能有资格获得下一个升级周期的社区奖励。
...
投票将于 **UTC 时间 8 月 27 日 03:47** 结束。请尽可能投票。
...
该提案已获批准。它调整了在样本量较小时无效推断检查（SPRT）的行为，避免在周期早期数据不足时误触发。主机无需采取任何操作。
...
在周期早期，数据尚不足，因此无效推断检查（SPRT）可能在样本量足够大之前，仅因少数无效结果就触发。本提案使其更加稳定：

- 将 `invalidation_h_threshold` 提高至40，并将 `bad_participant_invalidation_rate` 设为0.18。
- 在大样本量下，阈值接近10%的无效率，因此低于该阈值的诚实参与者不会被惩罚。真正的违规者仍会触发它（约在360个样本时为18%，在1650个样本时为12%）。
- 小样本无法触发：至少需要32个被标记的无效结果，且在500个样本时，观察到的比率必须超过16%。
- 未使用的停机时间SPRT已被关闭。周期末统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于**2026年8月21日06:04 UTC**（约12小时后）结束。请尽可能投票。如果未能及时达到紧急阈值，提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。如果转为常规提案，您需要重新投票。
...
一个稳定版本的网关当前正在一个网关上运行，并将用于第一阶段的结算。其统计信息，代表了部分网络流量，可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses和Invalid指标将用于链上。根据当前主网阈值配置，错误率低于10%的主机不会触发任何处罚或罚没，但应作为协议开发过程的一部分进行调查。

具有严重错误率的主机应在结算开始前解决这些问题。没有显著问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需采取行动。

**下一步：****1）未来24小时。** 主机应检查其统计信息并解决任何关键错误或配置问题。如有需要，贡献者可协助调查问题。

**2）在此24小时窗口后的第一个周期。** 将在当前网关上手动启动结算，首先从较小模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi将稍后加入，因为其数据在负载下目前可靠性较低。

**3）接下来1-3天。** 将监控结算结果

**4）一旦此阶段稳定。** 其他经纪人可以部署稳定版本的网关并以受控模式开始结算。

**5）在所有经纪人成功验证后。** 结算覆盖率将逐步向100%推进。

此部署故意逐步进行，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash现已在Gonka网络上激活**

在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的启动阶段已完成。自链周期360起，`deepseek-ai/DeepSeek-V4-Flash-0731`与MiniMax-M2.7和Kimi K2.6一同成为活跃模型组，DeepSeek组中获得的PoC权重现在以校准系数0.214（后由[提案98](#august-29-2026)提升至0.246）转换为共识权重。

DeepSeek的模型级参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需采取任何其他操作——现有设置将继续有效。尚未做出选择的主机请尽快确定，以避免每轮惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（例如正在服务Kimi或MiniMax的节点，因为任何已运行DeepSeek的节点均已升级至3.0.16），请在方便时将这些节点升级至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。

MLNode镜像已在[`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)的`vllm-0.25.1-upgrade`分支中固定为3.0.16。

默认MLNode镜像（3.0.16）基于CUDA 13.0构建。如果您的主机仍在使用CUDA 12.9，请改用`3.0.16-cu129`标签。这是同一MLNode 3.0.16，仅针对CUDA 12.9构建。两个标签均位于同一compose文件中。
...
将活跃的社区销售合约及所有包装代币实例迁移至新存储的CosmWasm代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中识别出的理论风险，预计不会影响正常运行。
创世守护者预计会支持该提案。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
**DeepSeek V4 Flash引导：意向阈值已达成——请勿忘记在第360轮快照前委托**

`deepseek-ai/DeepSeek-V4-Flash-0731`在第359轮快照中未满足预合格条件。此后，实时意向和委托权重已满足`V_min`、`W_threshold`以及**>2/3可达性**（DeepSeek意向权重加上委托给这些主机的权重）。下一次**官方**检查为第360轮引导快照——请在此前保持多样化的委托配置，以确保快照有效。

**将运行DeepSeek的主机**

1. 请在区块**5,551,615**之前保持或提交`MsgDeclarePoCIntent`：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 如果模型已预合格，请在PoC开始前约500个区块的部署窗口内（区块**5,552,115**）部署DeepSeek。请使用vLLM 0.25.1及发布候选节点配置。

**继续运行其他模型的主机**

在区块**5,551,615**之前，请提交委托（有助于引导可达性）或拒绝。从第360轮起，DeepSeek`penalty_start_epoch`生效——未明确选择的主机将面临不参与惩罚。
...
委托主机：请选择**非守护者**的DeepSeek意向主机，并将权重分散至独立目标——参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
DeepSeek已通过投票进入引导阶段——如果您计划提供服务，请验证您的设置并立即提交意向。
...
建议逐步将节点切换到新 API，而不是一次性切换整个集群。

**3) 使用 mlnode-validate 验证**

该技能现已支持 DeepSeek。它位于独立的 `./skills` 文件夹中，因此无需任何代理特定设置——将其符号链接到 `.claude/skills` 并运行 [`mlnode-validate` 技能](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/skills/mlnode-validate)：
```
/mlnode-validate how to use
/mlnode-validate <your-node-ip:port> @deploy/join/node-config-deepseekv4flash0731-B300.json
```
该技能部署模型，测量 PoC 吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行对比。如果通过，则表示一切正常。

如果您希望提供 DeepSeek 服务，请使用上述步骤进行设置。否则，请不要忘记委托，以避免非参与惩罚。

注意：明天将发布另一个带有更好日志的 MLNode 构建版本，因此请期待后续简短更新。
...
1. 声明意向。在提案 94 通过后、启动快照前（区块 5,536,224，约 2026 年 8 月 13 日 02:40 UTC / 8 月 12 日 19:40 PDT），请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的 MLNode。在 PoC 359 启动前约 500 个区块的安全窗口内（区块 5,536,724，约 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），配置并切换您的 MLNode 至 DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口内切换是安全的，因为其中没有 cPoC。
...
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的 DeepSeek 主机上。参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第 360 个周期前无需任何操作。从第 360 个周期开始，若您未提供 DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用 15% 的非参与惩罚。
...
将 `deepseek-ai/DeepSeek-V4-Flash-0731` 添加到 PoC 模型列表的提案现已上链并开放投票。

**提案内容**

本提案将 DeepSeek V4 Flash 作为新的 PoC 模型添加到网络中。不修改任何现有参数或模型；唯一更改是新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要运行 vLLM 0.25.1 并使用发布候选节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）的 MLNode
...
模型通过标准引导流程进入，因此主机可以在承诺硬件前查看其可行性：

1. 声明意向：在 `start_poc - deploy_window` 快照前提交 `MsgDeclarePoCIntent`。
2. 预合格快照：链对意向/委托进行快照，并发出建议性的预合格事件。
3. 部署窗口：声明意向的主机部署其 MLNode。
4. PoC 开始：成员资格由在 PoC 开始时提交 PoC 存储提交的节点决定，而非由声明意向的节点决定。
...
1. 若要提供服务：投票结束后声明意向，然后部署您的 MLNode（vLLM 0.25.1 + release-candidate 配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
...
关于 `v0.2.15-devshard-v4.0.1` 运行时升级提案（提案编号 93）的链上治理投票已结束。
...
- API。
    ...
    - 默认的 Node Manager gRPC 端口现为 9400，确保与自定义配置的兼容性
...
请更新 api 容器使用的二进制文件。
在开始更新前，请确保当前未运行 CPoC 或 PoC。
每个 PoC 开始前有 500 个区块的 CPoC 空窗期，请尽可能利用此窗口进行更新。
请逐台机器部署更新，以降低操作风险。
...
这只是一个性能和资源管理补丁，仅此而已。协议、推理、验证和经济行为均保持不变。变更包括：时段追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与时段修剪对齐。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整链软件升级。若获批准，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动操作主机。

**如何投票**

如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅有关将冷密钥的治理投票权限授予热密钥的指南。
...
该提案已获批准，并于主网区块 5316315 成功执行升级。该提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
尽管这两项变更在一次投票中提交审议，但它们在操作上仍相互独立：
...
v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个支持多实例高可用的版本：在共享的 Postgres 上，通过 versiond-router 后部署 N 个版本化的 /devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；仅由托管人通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，以确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。通过移除过时行为并将交易处理集中于当前支持的架构，也为未来的开发和升级提供了便利。
...
投出您的投票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 或更高版本（v0.2.12）。
```
export NODE_URL=https://node3.gonka.ai/
...
- 投票结束：2026 年 7 月 30 日 07:08 UTC / 2026 年 7 月 30 日 12:08 PDT
- 提议的升级高度：5316315
- 预计升级时间：2026 年 7 月 30 日 15:28 UTC / 2026 年 7 月 30 日 08:28 PDT

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 现在/主网升级前——更新您的API和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将`api`二进制文件和网桥镜像更新至0.2.14-post3。如果您的`api`二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在PoC或cPoC之外执行此步骤。

- 仪表板维护人员——此次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上Cosmovisor对`application.db`的备份通常为数十GB，需提前确认）。有关如何安全清理`.inference`目录中的旧备份，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认，且会自动清理（保留最近3个周期）。
...
- v4的主要目标是在主机故障和升级时实现devshard主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器会逐台替换版本。这是系列更新中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4是首个面向多实例高可用的版本：在共享Postgres后部署N个versiond/devshardd副本，通过versiond-router实现粘性会话路由和验证租约独占性。网关仅通过gRPC与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管人通过签名聊天绑定。当治理发布同名但sha256不同的新二进制文件时，versiond可执行蓝绿切换并排空，以确保正在进行的工作（包括SSE）在旧版本上完成。
- v4还包含错误修复和安全补丁。

**操作项**

请审核PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现任何问题、疑问、改进建议、边缘情况或潜在漏洞时留下评论。

有意义的审核贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一个升级周期的社区奖励。
...
`moonshotai/Kimi-K2.6`重新进入治理模型列表，并进入标准引导流程。

* 在区块**5,105,276**（2026年7月17日，约12:05 UTC）前声明意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第331周期PoC开始前约500个区块窗口内（区块**5,105,776**——2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换为Kimi。
* 委托主机：选择非守护者Kimi主机，并在独立目标间分散权重——请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册到治理模型列表中，使其从第331周期开始进入标准引导流程。

同一提案从`devshard_escrow_params`中移除`approved_versions`的v1和v2。一旦批准，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的RAM增长问题——该问题导致了7月15日事件中的网络节点OOM。

**这标志着v3切换最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票——紧急窗口很短。
...
`moonshotai/Kimi-K2.6`在epoch 328–329中失去了PoC验证多数（详见[此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其在最小停机时间内恢复的最快方式——与6月提案78的恢复路径相同。

该提案将在下一次PoC之前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将在epoch 331开始标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票——紧急窗口很短。
2. 为Kimi提供服务的主机：保持您的Kimi设置就绪，并准备好在引导PoC时重新启用您的MLNode。
3. 在引导期间进行委托时：不要委托给守护节点；将委托分散到独立的Kimi主机上。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（紧急投票，0.667 赞成阈值；出勤率重要，请尽快投票）。
...
在纪元 328 中，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数票。在纪元 329 开始时，运行 Kimi 的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台运行 Kimi 的守护者操作服务器同时失效：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其失效一次性移除了这些投票。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次失效足以使该组低于多数。守护者投票权平局机制未启动：当前未赋予模型投票权重的守护者被排除在平局机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况类似于纪元 306–307 事件。纪元 329 开始时的投票确切细节仍在确认中；此摘要可能存在小幅修正。
...
先前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现被撤销：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主要机制与备用机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请向您自己运行模型的节点委托。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重，且无单点故障。

**已在进行中的修复*** 守护者参与 PoC 验证（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请立即投票。
...
该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 自动下载二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

白名单中的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这将使他们能够在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换到 v3。在此类升级期间，无需进行主网重启或手动主机操作。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
投票截止：2026 年 7 月 11 日 06:41:34（UTC）
...
主网链/API 工作重点包括：PoC 重复工件保护、早期共享检测、经典推理 API 废弃（禁用主网上的 `/v1/chat/completions` 计费并从 API 二进制文件中移除嵌入的 `/v1/devshard`）、奖励接收者路由以及升级时的安全修复。
...
在此分支上将单独发布 devshard v3 版本，并在主网链升级前进行部署。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级期间继续提供推理服务。

**建议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查本提案。
2. devshard v3 发布版本将在主网链升级前被提议并部署。
3. 经纪人将推理流量切换至 `/devshard/v3`。
4. 如果链上提案获得批准，此 PR 将在链上升级执行后立即合并。
5. 请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能在下一个升级周期中获得社区奖励。

本次仅为邀请审查 Pull Request，不启动正式投票。治理投票流程将在审查期结束后开始。

**Devshard v3 治理投票**

Devshard v3 发布版本将在主网链升级前提出并部署，以便经纪人能在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和桥接器。为保持主网升级期间以太坊桥接器的稳定，请提前根据指南将 `api` 二进制文件和桥接器镜像更新至 0.2.14。如果您已更新 `api` 二进制文件，则只需更新桥接器镜像并重启桥接容器。如果您已完成以上两步，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人 —— 将推理流量切换至 `/devshard/v3`。一旦 devshard v3 发布版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 —— 准备好调整指标统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近结束之后发布。
...
根据历史数据，此前未观察到此类攻击。但在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 正在使用此攻击。

针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供Kimi服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行Kimi，因此不愿运行它的主机可将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换至Kimi（PoC开始约在6月28日 ~12:47 UTC）。
...
> **更新（2026年7月15日）：此建议已撤回 —— 请勿委托给守护节点。** 守护节点是PoC验证的备用机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)获取最新的委托指导。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此举并非偏向任何硬件——MiniMax已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
常规启动流程从第309轮开始。由于投票将在PoC开始前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多启动信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（此举实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此举并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2服务的主机需在 `start_poc - deploy_window` 的启动快照前提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3可达性）。这使操作者可在配置硬件前确认模型是否可行。
...
从第309轮开始（PoC开始约6月26日 15:25 UTC；生效约16:00 UTC），前提是各模型满足启动资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能启动进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时紧急周期可以。创世守护节点预计将支持此提案。

**主机所需操作**

1. **要提供Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，期间切换是安全的，因为此期间无cPoC。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍使用Qwen或Kimi的主机，除非切换，否则本纪元将无法获得cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308个纪元将进行一次投票以恢复Kimi。

**接下来的内容**

在第308个纪元将进行一次加急投票，合并两项变更，均于第309个纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导过程。
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可自愿选择参与，提前衡量模型需求，不提供服务的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一纪元。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式。

一项加急提案现已上链，以便在下一纪元开始前生效。
链上提案在一次投票中执行以下操作：
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复服务且最小化停机时间的路径。下一纪元将进行投票以恢复Kimi。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的模型阵容调整。此变更与Kimi情况无关，仅因同属PoC阵容调整而一并包含在此处。
* **MiniMax-M2.7 — 提升为基础模型。**

由于重置必须在epoch 307结束前完成，因此这些更改被合并为一次紧急投票。每个epoch约22.9小时，标准的48小时投票周期无法容纳在一个epoch内；而12小时的紧急周期可以。Genesis守护者应支持该提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如尚未就位），以避免在epoch边界时遭遇Hugging Face速率限制。
...
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。
...
* 第308个纪元很快形成：PoC开始约16:50 UTC，生效约17:25 UTC。
* 紧急提案需要0.667的通过阈值；参与率很重要，请尽快投票。
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。`versiond`会自动下载二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动额外的`devshardd`进程。

对于这种仅针对devshard的运行时升级，无需重启节点容器或手动执行主机步骤。

**关键变更**

- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长会话在RAM或状态中保留所有已服务的推理。
- 通过OpenTelemetry和Prometheus添加了内部devshard跟踪和指标。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以避免长会话在RAM或状态中保留所有已服务的推理
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。如果链上提案获得批准，`versiond`将自动下载二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动额外的`devshardd`进程。
...
v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax 组中获得的 PoC 权重现在以校准系数 0.3024 转换为共识权重。

MiniMax 的按模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续正常工作。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已被批准，并已在区块 `4267300` 成功执行主网升级。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会对其全部权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基准。此调整**仅适用于您共识权重中源自 Kimi 的部分**；您的 Qwen 源权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，现提交了若干更新。

- 包括：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下文专用部分）。
- 该提案将升级后的宽限期延长至 3000 个区块，以便主机在新快照逻辑稳定期间不被惩罚。
- 治理：将创世守护者投票权降低至约 25%，并设置链上法定人数为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 在本次升级纪元剩余期间禁用确认 PoC，以便新快照逻辑从下一个纪元干净启动。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不参与投票的情况下，剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
**在第 278 个周期（处罚由此开始）前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。若如此，请先确认您运行的是当前版本，再执行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

主机需更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。若您尚未本地克隆 `gonka` 仓库，请先遵循加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
若您无权直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期其余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。

**注意**

- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关安全删除 `.inference` 目录中旧备份的指导，请参阅 [文档。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
...
- 该提案将有意跳过从升级高度到升级周期结束（10000区块宽限期）的确认 PoC。若获批准，此跳过为预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最后3个周期）。
...
第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者未能进入第266轮。
...
2. 监控下一阶段的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重被正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方可用，预计会通过社区代理和经纪人端点逐步恢复。
5. 在社区频道或此线程中分享相关日志或观察结果。如果您的主机在第266轮受到影响，或在后续轮次中行为异常，这一点尤为重要。
...
社区成员目前正在审查第266轮期间发生的情况。一旦对根本原因有更清晰的了解，将发布更新。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理门槛设为 `0.25`。在守护者不投票的情况下，其余 75% 投票权的有效门槛为 1/3（`0.25 / 0.75 = 0.334`）。
...
- 该提案将有意跳过从升级高度到升级纪元结束（3000区块宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近 3 个纪元）。
...
[此拉取请求](https://github.com/gonka-ai/gonka/pull/1143) 针对下一次链上软件升级 v0.2.13，现开放审查。

请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格在下一次升级周期中获得社区奖励。

本次仅为拉取请求的审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天启动。
...
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新出现的错误推断会立即再次使其失效。现在在重新激活和即将晋升时重置计数器。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级将回填该权限，以便它们能够响应交易商投诉。
- Devshard 结算使用了硬编码的 `20_000` 非 nonce 限制。现在限制为 `DevshardEscrowParams.MaxNonce`，v0.2.13 升级将其设置为 `1_000_000`。升级还将 `MaxEscrowsPerEpoch` 提高到 `500_000`。
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束，确认 PoC 触发器将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用了 v0.2.10 的宽限期原语。
...
- 由于旧的托管数据保留在一个 SQLite 存储中，devshard 存储可能无限增长。现在存储按纪元范围划分，并在后台清理旧纪元，仅保留最近的 3 个纪元。
- Devshard 现在支持更大规模部署的共享存储选项。它现在可以使用 Postgres 作为主存储，SQLite 仍作为本地备用存储。
- Postgres 数据按 `epoch_id` 对会话、差异和签名进行分区，以便清理可以干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
- 负载查找固定于托管纪元，同时为纪元边界和遗留纪元-0 请求提供回退机制。
- 当前纪元分片统计信息暴露了 nonce、版本、组和每主机计数器。

**bridge**

- 桥接工具处理 Sepolia 标志，并将 Gonka BLS 密钥/签名转换为以太坊合约所需的 EIP-2537 格式。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能延迟启动。这延迟了 api 激活，导致部分矿工跳过了 Confirmation PoC。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来的轮次中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。

安全模型保持不变：PoC验证本身未更改，仍需超级多数验证算力才能接受结果。

该提案已加速，以便在下一轮PoC之前生效。投票将持续12小时。
...
`moonshotai/Kimi-K2.6`已完成启动并加入Gonka网络的PoC参与。
...
已提交PoCIntent的主机应在PoC于区块`3874496`启动前，至少将一个MLNode从`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`切换至`moonshotai/Kimi-K2.6`。
...
这标志着网络的一个里程碑：`devshard`已上线并具备生产就绪能力。`node4`是今后推荐的公共网关。

**需执行操作**：请将您的端点更新为`node4`。

## 2026年5月2日

今天的预资格验证未通过，对于`PoCIntent`权重低于30%的主机。请保留您的MLNode使用`Qwen235B`，并于明天提交下一轮的意向。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 押金资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
每个 DAPI 会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种裁剪，直接加载持久化配置。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，然后通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、将遗留验证者 slashing 与所需抵押品语义对齐、确保 devshard 保证金基金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成其自身的本地 PoC 权重，并通过模型特定系数聚合为总共识权重
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI / 主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。所有已知的审计问题均已解决。
- **协议强化**。实现更强的 PoC v2 RNG（256 位完整熵，此前为 32 位），将通过独立治理投票激活。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG dealer 共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
更新修复了 PoC 开始附近的一个边界情况，此前在某些条件下可能导致性能下降。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改进 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过每笔推理的链上交易处理推理限制了吞吐量。此设计将推理执行与验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因其已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为不会对矿工奖励产生负面影响。
...
在类似主网的条件下，这可使每个区块容纳的推理数量提升高达 100 倍，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作非常重要。若大量节点推迟到升级后才进行剪枝，网络中将同时出现大量剪枝活动，造成不必要的运营压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，不是因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将几乎同时开始剪枝活动，造成不必要的运营压力。
...
对于已使用代理层的团队，这可能是简化架构、依赖原生行为的好机会。实际上，这将带来更清晰的集成模式和更易维护的系统。
...
如果您无权访问持有投票权的冷钱包密钥，建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代为投票。未经该授权，其他账户无法提交投票。
...
服务器端对长请求的重试通常无效，因为会导致相同的超时场景。同时，客户端可能会收到不一致的输出。新版本的 API 在此类情况下不再重试推理。
...
现在提供一个新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍处于活跃状态的请求导致权重可能下降的风险。
...
- 此构建版本旨在与前一版本保持向后兼容。
- 它已为少量节点启用，但仍建议在部署前审查变更。

## 2026年2月19日

**抵押参数更新提案——投票结果**

抵押参数更新提案未能达到法定人数，因此根据当前治理规则被拒绝。这意味着更新的参数将不会被激活。
...
抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍然是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能会波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时，存入高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为将PoC权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选定。如实施所示，有效的PoC参考窗口比先前名义假设长约5个区块。
...
由于有效归一化窗口比先前名义模型长约5个区块，每个周期重新计算的权重相应降低。
...
**宽限期：** 升级后引入了3000个区块的宽限期，期间不进行确认 PoC，并对升级当期的错过率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工最多损失其抵押品的0.5%。所需抵押品仅占每日奖励的约24%。

**警告：** 抵押参数将无视投票结果生效。若该提案未通过，创世时定义的抵押参数将在第180周期自动生效，而非上述参数。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并对升级所在纪元的错过率和无效率阈值采取较宽松的标准。
...
- 投票结束：2026年2月18日，UTC时间09:26:26
- 升级高度：2712600
- 预计升级时间：2026年2月18日，UTC时间15:30:00

**注意**

- 请检查任何解析`inferenced` CLI输出的脚本。由于IBC堆栈升级至v8.7.0，枚举和int64/uint64值现在以字符串形式编码。
- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中旧备份的指南，请参见[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用大量磁盘空间，可应用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理技术。
...
根据AMA期间的讨论及社区成员提出的论点，建议初始抵押要求较低，惩罚也最小化。
...
这意味着，在单个纪元内，即使被惩罚，矿工最多损失其抵押品的0.5%。所需抵押品仅占日奖励的约24%。

提案提交投票后将另行发布公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在有正当理由时适用。如果证明稳定性良好，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请保持在线，以便在需要时迅速应用补丁。更多细节和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已开放**

链上推理访问当前已开放，不限于开发者。推理请求可通过上一更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一次链上软件升级 v0.2.10 已开放审阅。欢迎提供反馈和改进建议。当前计划是将审阅窗口保持约2天。

对于对此次PR审阅有实质性贡献的，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审阅请求，并非正式投票的开始。治理投票流程将在审阅期结束后启动。
...
此升级根据实际PoC经过时间标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
- **[PR #716](https://github.com/gonka-ai/gonka/pull/716)** 重命名PoC权重函数以提高清晰度与正确性。
...
在更新的权重分配生效后，追踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新 — 补丁可用**

现已提供补丁以解决导致PoC周期内块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为并安全恢复块生产。
...
- 下一个 PoC 周期（从纪元 158 到 159 的过渡）至关重要。请确保在线，以便在需要时及时应用后续步骤或缓解指令。
...
在因 cPoC 惩罚而导致奖励减少或排除的情况下，未计入部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
这些变更的更多详细信息可在治理工件中查阅：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9 ](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9 )
...
在因cPoC处罚而减少或排除奖励的情况下，未计入部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
在最近的周期中，大量 ML 节点未获得 PoC 权重。
分析表明，这是由于 PoC 验证参与不足所致。在多个案例中，参与者发布了 nonce，但未执行验证，或验证水平远低于协议要求。
下表显示了上一周期拥有权重、在当前周期提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
!!! note 
    未能同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 的权重分配仍需满足观察性采用阈值和后续治理决策。
...
本指南说明主机应如何根据v0.2.8模型可用性变更及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
!!! note 
    ...
    `node-config.json` 仅在 Network Node API 首次启动或本地状态/数据库被删除时使用。若要全新重启，请编辑它。对于现有节点，模型更新应通过管理 API 进行。
...
!!! note "治理与 PoC v2 激活说明"
    ...
    **第一阶段：观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML Node 切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为兼容 PoC v2 的版本，才能参与 PoC v2 权重贡献。
...
PoC v2 仅在链上治理提案获得批准后，才成为权重分配的激活方法。
    在此提案获得批准之前：
...
作为 v0.2.8 升级的一部分，支持的模型集已更新。所有此前支持的模型均从活跃集中移除，除：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估是否具备向 PoC v2 过渡的条件。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
该提案将当前的稳定期延长约两周。延长的期限旨在为即将进行的 PoC 变更及相关网络升级提供额外的测试时间。有关新 PoC 开发进展的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个周期内逐步进行更新。

**投票范围**

如果获得批准，网络将暂时继续在现有的 `allowlist`（包括未表现出非标准硬件行为的主机）下运行。

开发者 `allowlist` 将以相同的偏移量延长，并将持续有效至区块 2459375。
...
`Allowlist`过期：自动在区块 2443558 处过期。
...
`Allowlist` 过期时间：在区块 2,222,222 自动过期。
...
该提案已获得批准。此提案定义了一个临时 `allowlist`，涵盖在最近多个周期中行为保持一致的参与者。
...
2) 此前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直至区块 2,222,222 的稳定窗口结束。

**执行特性**

- `allowlist` 将从下一个周期（第135周期）开始生效
- 激活发生在第135周期的首次PoC中
- 无需软件升级
...
- 生成的 `allowlist` 发布于此处：[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)
...
- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，且
- 观察到的PoC权重低于使用同类硬件的其他参与者所表现权重的150%。
此前表现出持续偏离这些模式的参与者在稳定窗口结束于区块2222222之前不属于 `allowlist`。
...
为恢复可靠的共识进展，需要至少67%的网络活跃算力安装该补丁。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将被暂停。一个预定义的`allowlist`开发者地址白名单立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续生效至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理机制控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票方可启用。
...
近期网络增长带来了若干挑战。在过去几天中，网络经历了多次问题，其中一些似乎是故意试图破坏或压垮系统所致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内处于激活状态。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。
...
本次升级将此前通过手动API更新分发并已在网络上使用的重大修复正式化。这些修复包括：
...
- 主机级别注册：在区块2,222,222之前（约两周后）暂停新主机注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。一个预定义的 `allowlist` 开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续至区块2,294,222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外奖励，金额为第117轮奖励的1.083倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个错误：未标记为已完成的不支持格式的推理请求，尽管请求本身已被处理。以下更新将解决此行为。
