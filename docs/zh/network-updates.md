...
如果您希望托管它，欢迎加入。在部署窗口期间声明对该模型的意向并配置您的MLNode，该窗口在下一个PoC开始前500个区块开启。

**别忘了您的参与选择**

自第394个纪元起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行服务、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的15%，并持续至您做出选择为止。
...
该提案还向七名受益人发放总计91,300 USDT的贡献者奖金：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重达到25%的最低参与门槛、多数赞成票，且反对票（含否决）低于33.4%。
...
该提案已获批准。从社区池中拨出 15,500 USDT，用于补偿 Daniil 和 David Liberman（Gonka 协议的联合创建者）参加 All-In Summit 2026 的两张参会通行证费用。

峰会于 2026 年 9 月 13-15 日在洛杉矶举行。标准参会通行证每人 7,500 USDT，门票已支付。本次补偿涵盖两张门票及提取资金的交易费用。本提案未申请从社区池中报销机票、住宿及其他费用。
...
该提案已获批准。Quant Mesh Limited 被正式批准为 Gonka-Ledger 集成的法定合同合作方，并从社区池中拨付 350,350 USDT 至 `gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用 275,000 USDT、第一年年度激活费 75,000 USDT 及 350 USDT 的处理费用。授权与资金拨付严格限于完整提案中描述的 Ledger 集成。
...
该提案已获批准。`zai-org/GLM-5.3-Flash` 现已成为经治理批准的模型并纳入 PoC 模型集，而 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8` 已从 `poc_params.models` 中移除。这两个 ID 仍保留在 `GET /v1/governance/models`（推理目录）中，但不再属于 PoC 模型。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：其 PoC 参数和权重保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，并联合提出。测量数据及阈值设定的推理详见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起 `penalty_start_epoch` 生效，因此未明确选择的宿主将面临不参与惩罚。如果您计划提供该模型，尽早声明意向将使团队有最佳机会组建。宿主指南：[GLM-5.3-Flash 启动](./host/glm-bootstrap.md)。
...
该提案已获批准，`v4.1` 已加入批准的 devshard 版本列表。`versiond` 将下载二进制文件，将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。无需手动宿主操作。
...
提案 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加到 PoC 模型集中。同时将其从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票门槛、多数赞成，且带否决的反对票少于 33.4%。
...
如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅有关将冷密钥的治理投票权限授予热密钥的指南。
...
这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会停止，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。提案通过本身不会迁移任何流量，因此经纪商可在之后逐步迁移其网关。
...
投票将于 **2026年9月10日 22:36 UTC** 结束。提案通过需满足：权重占比 25% 的参与率、多数赞成票，且反对并行使否决权的票数少于 33.4%。
...
提案已获批准。它将从社区金库拨款 100,000 USDT，用于支持 Gonka 参与 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金涵盖黄金赞助套餐及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
重启时，`v4.0.1` 从 nonce 1 恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 随后在 60 秒就绪超时后终止子进程，客户端因此看到 HTTP 502 错误。
...
提案已获批准。它将 `deepseek-ai/DeepSeek-V4-Flash-0731` 的 `weight_scale_factor` 从 0.214 提高至 0.246。所有其他模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，结果为 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的参与率）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 因子源于被高估的 nonce/min 数值。测量修复见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需执行任何操作。
...
Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，也不预期会导致推理服务停机。如果通过治理流程批准，新的 devshard v5 运行时将与现有的 v4 运行时并行运行。

**主要变更**

1. v5 的主要目的是高度同步：每个主机和序列器在保证金日志中维护一个已签名、可重放的主网高度视图，即使在没有推理流量时也是如此。借助该时钟，devshard 现在可以证明地知道 cPoC 正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前携带时间戳字段的消息中。后续版本可以将诸如 `confirmed_at` 和 `started_at` 等墙钟字段替换为高度标记。这消除了未来时间攻击问题，并为在协议级别处理流超时打开了道路。本次发布包含这些标记，但尚未将超时和封印决策切换到它们。

3. 通过所有插槽的往返心跳已添加，因此安静的会话仍能定期同步高度，而繁忙的会话则可通过常规推理标记实现相同的节奏。

**操作项**

请审阅 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并在任何发现、问题、改进建议、边缘情况或潜在漏洞上留下评论。

有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一升级周期的社区奖金。

本次仅为 PR 评审呼吁。不启动正式投票。
...
**提案已通过：稳定小样本下 epoch 内无效 SPRT**

提案编号 96 的链上治理投票已结束。

该提案已获批准。它调整了在样本量较小时无效推理检查的行为，避免在 epoch 初期数据不足时误触发。主机无需采取任何操作。
...
在 epoch 初期，数据尚不足，因此无效推理检查（SPRT）可能在样本量足够大之前仅因少量无效项而触发。本提案将其稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本量时，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真正的违规者仍会触发它（约 360 个样本时为 18%，1650 个样本时为 12%）。
- 小样本无法触发它：至少需要 32 个被标记的无效项，且在 500 个样本时，观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。epoch 结束时的统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于 **2026年8月21日 06:04 UTC**（约12小时后）结束。如能投票，请务必参与。若未能在时限内达到加速阈值，该提案不会被拒绝：它将转为普通提案，投票将继续进行更长的正常周期。若转换为普通提案，您需重新投票。
...
作为协议开发的重要里程碑，链已准备好逐步启用 devshard 结算，以提升网络的安全性与可靠性。

一个稳定的网关版本当前已在单个网关上运行，并将用于第一阶段结算。其统计数据（代表部分网络流量）可在此查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关视角展示每个主机执行的工作。Misses 和 Invalid 指标将在结算启用后上链使用。在当前主网阈值配置下，错误率低于10%不会触发任何惩罚或罚没，但仍应作为协议开发过程的一部分进行调查。
...
**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，先从较小模型开始。此过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 随后1-3天。** 将监控结算结果。

**4) 一旦此阶段稳定。** 其他经纪人可部署稳定版网关，并以受控模式启动结算。
...
在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在正以校准系数 0.214（后由 [提案98](#august-29-2026) 提升至 0.246）转换为共识权重。

针对 DeepSeek 的按模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需再做任何操作——现有设置将继续生效。尚未做出选择的主机建议尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版 MLNode（即仍在服务 Kimi 或 MiniMax 的节点，因为所有已部署 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。
...
默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。若您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。该标签同样是 MLNode 3.0.16，仅针对 CUDA 12.9 构建。两个标签均在同一个 compose 文件中。
...
将活跃的社区销售合约及所有封装代币实例迁移至新存储的 CosmWasm 代码，并为未来实例化注册该封装代币代码。合约地址与余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。

**如何投票**

如果您无权直接访问拥有投票权的密钥，或希望使用其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 启动快照——在此前请保持委托分散，以确保快照有效。
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则一切正常。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500个区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口期内切换是安全的，因为该区间内无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用以fp8 + nvfp4重新打包的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。为部署此模型，将在启动前发布并共享更新的API二进制文件。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。现有参数或模型均未修改，唯一变化是新增了该模型条目。
...
根据模型使用情况，社区后续可决定逐步用DeepSeek替代Kimi或MiniMax（例如，提高系数以激励所有B系列GPU转向DeepSeek）。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第360个纪元之前无需操作；从第360个纪元开始，如果您未提供该模型，请为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
这只是一个性能和资源管理补丁，除此之外无其他变更。协议、推理、验证和经济行为均保持不变。变更内容：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪保持一致。
...
2. 一个 **v0.2.15 主网软件升级**，移除了大量过时代码、遗留的交易处理路径及相关组件，这些已不再符合当前架构需求。
...
v4 的主要目标是在主机故障和升级时实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond / devshardd 副本，使用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可以执行蓝绿切换并排水，以确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。同时，通过移除过时行为并围绕当前支持的架构整合交易处理，使未来的开发和升级更加容易。
...
如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的 Cosmovisor 备份 `application.db` 通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
若提案获得批准，建议提前完成以下准备工作。

- 现在 / 主网上线升级前 —— 更新您的 API 和桥接器。为确保主网上线升级期间以太坊桥接器稳定，请提前按照 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和桥接器镜像升级至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
- v4 的主要目标是提高 devshard 主机在主机故障和升级期间的高可用性。当一台机器宕机或重启时，它仍可继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名但 sha256 不同的新二进制文件时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的内存增长问题——即7月15日网络节点OOM事件的根本原因。
...
2. 提供Kimi服务的主机：在投票结束**后**、区块 **5,105,276**（2026年7月17日约12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**不要委托给守护者节点**；将委托分散到独立的Kimi主机上。有关更新的委托指南，请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 经纪人 / devshard创建者：如果您的任何网关流量仍在使用v1/v2或`/v1/devshard`——请在投票结束前切换至`/devshard/v3`。
...
`moonshotai/Kimi-K2.6`在第328–329轮中失去了PoC验证多数（详见[事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其以最小停机时间恢复的最快方式——与6月提案78采用的恢复路径相同。
...
3. 在委托引导时：请勿委托给守护节点；将委托分散到独立的Kimi主机上。
...
投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 版本 v0.2.13 或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（紧急投票，0.667 是票阈值；出勤率重要，请及时投票）。
...
* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。监护人决胜机制未触发：目前没有每模型投票权重的监护人会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。
...
* **请勿委托给监护人节点。** 监护人是 PoC 验证的后备机制。将委托集中于它们会将主要机制和后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请向您自己运行模型的节点委托。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在进行中的修复*** PoC 验证中的监护人投票（监护人始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完成迁移后，v1/v2 将被移除并关闭 `/v1/devshard`。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非监护人委托方；尽可能将权重分散到多个独立主机上。
...
这是一个仅针对 devshard 的运行时升级。它独立于全链软件升级运行，不需要链二进制升级。

提案获批后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人切换流量至 v3。

此版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们能够在链升级运行期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期运行时前缀，直到代理将流量切换至 v3。在此类升级期间，无需进行主网重启或手动主机操作。
...
节点二进制文件通过链上软件升级提案进行升级。在升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
在此分支上将单独发布 devshard v3 版本，并在主网链升级前进行提案和部署。提前将推理流量切换至 `/devshard/v3` 的代理可以在链升级运行期间继续提供推理服务。
...
本次仅为请求审查 Pull Request，不启动正式投票。治理投票流程将在审查期结束后开始。
...
2. 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。如果您已更新 `api` 二进制文件，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
根据历史数据，此前未发现此类攻击。但在当前周期中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。

针对即将发布的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案——可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可将其权重委托给该节点。
...
> **更新（2026年7月15日）：此建议已撤回——请勿委托给守护节点。** 守护节点是 PoC 验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。有关更新的委托指南，请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。

**关键时间点*** 意向截止时间（第310轮）：区块 **4,797,456** —— 约6月28日，12:00 UTC。
* 第311轮开始：区块 **4,797,956** —— 约6月28日，12:47 UTC。

有关启动机制的更多信息：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****主要变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数的选择使得在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相同。此举并非偏向任何硬件——MiniMax 上已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相同。此举并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 以及 >2/3 可达性）。这使操作员能在配置资源前确认模型是否可行。
...
从第309轮开始（PoC 开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是各模型满足启动资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能启动至第309轮。由于每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计会支持该提案。

**主机需采取的行动**

1. **要提供 Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口期，期间切换是安全的，因为此期间无 cPoC。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，在切换前将无法获得本纪元的cPoC奖励。
* 打算再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308个纪元将进行一次投票以恢复Kimi。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可选择参与，提前衡量对该模型的需求，不提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——被移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是使其尽快回归且停机时间最短的路径。下一纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的模型阵容调整。此变更与Kimi情况无关，仅因同属PoC阵容调整而一并包含。
* **MiniMax-M2.7 — 已升级为基础模型。**

由于重置必须在epoch 307结束前完成，因此将所有变更合并为一次紧急投票。每个epoch约为22.9小时，标准的48小时投票周期无法容纳在一个epoch内；而12小时的紧急周期可以。Genesis守护者应支持该提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如尚未预载），以避免在epoch边界时遭遇Hugging Face速率限制。
2. **请在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需为PoC 309时切换回Kimi做好准备——epoch 308中将进行恢复Kimi的投票。


**下一步计划*** **恢复Kimi K2.6** —— 在epoch 308中进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，且**不设置不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供该模型的主机不会受到任何惩罚。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情及投票可通过`inferenced`访问：任何活跃节点均可使用：
...
提案通过后，v2现在可以与现有的v1 devshard运行时并行运行。新进程在`/devshard/v2`前缀下提供服务，而现有的v1流量继续在`/devshard/v1`和`/v1/devshard`上运行。
...
- 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以避免长时间运行的会话将所有已提供的推理保留在RAM或状态中。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以避免长时间运行的会话将所有已提供的推理保留在RAM或状态中
...
请直接审阅该PR，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。

有意义的审阅贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期中获得社区奖励。

本次仅为PR审阅请求，不启动正式投票。治理投票流程将在审阅期结束后开始。
...
关于 [Upgrade Proposal v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **批准**。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，也必须对其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机将对其全部权重遭受每纪元 15% 的惩罚。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新提交，此次提交包含若干更新。
...
- 治理：将创世守护者投票权降低至约 25%，并将链上法定人数设定为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 在本次升级纪元剩余期间禁用确认 PoC，以便新快照逻辑从下一纪元干净启动。
...
对于每个经治理批准的模型，多模型 PoC 要求每个主机明确选择参与（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 之后不采取任何操作将导致处罚。在此阶段，您应提前决定首选选项，以便在提案通过且升级成功应用于主网时能够迅速行动。

**桥接容器更新/验证**

请所有主机验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。如果是这种情况，请先确认您运行的是当前版本，再进行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约 43 分钟前）→ 第一阶段 MiniMax PoC 在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。
...
- 该提案将有意跳过从升级高度到升级周期结束（10000 块宽限期）的确认 PoC。若获批准，此跳过为预期行为，并非故障；新快照逻辑将在下一个周期开始。
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个周期）。
...
在应用额外防护措施期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪人端点。
...
2. 监控下几轮的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重正确反映。
...
似乎本轮的PoC投票未能收集到所需的票数。确切原因尚未确认，社区正在积极调查此事。
...
社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更多清晰信息，将及时更新。
...
- 包括：对Kimi（`0.78`）的权重重新校准、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复和以太坊桥主网激活。
...
- 将 `MiniMaxAI/MiniMax-M2.7` 作为治理批准的模型和概念验证模型添加。
...
- 将创世守护者调整投票权减少至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不投票的情况下，这使得其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您正在运行的是否为当前版本，然后再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意跳过从升级高度到升级周期结束（3000个区块的宽限期）期间的确认PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近3个周期）。
...
- 当参与者重新变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新恶意推断会立即再次使其失效。计数器现在在重新激活和即将晋升时重置。
- 在 v0.2.12 之前加入的 DAPIs 在冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级将回填该权限，以便它们能够响应交易商投诉。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能启动延迟。这延迟了 api 激活，导致一些矿工跳过了 Confirmation PoC。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来的轮次中失去资格的风险，并进一步简化新模型的接入，建议将该阈值降低至10%。
...
预评估与PoC开始之间有500个区块的窗口期。在此期间没有CPoC任务，因此已声明意向的主机可以安全地将模型节点切换至 `Kimi K2.6`。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 抵押资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易方共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位全熵，之前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 经销商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
更新修复了 PoC 开始附近的一个边缘情况，该情况过去可能导致在某些条件下性能下降。
...
v0.2.11 升级提案的链上治理投票已结束。该提案已获**批准**，升级已在主网上成功执行。
...
这些性能改进可使每个区块的推理量最多提升 100 倍，具体取决于工作负载和网络状况。
...
下一版本 v0.2.11 的链上升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理可扩展性。

目前，通过每条链上交易处理推理限制了吞吐量。此设计将推理执行与验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本并非因其已完成，而是因为此类系统需要尽早暴露于真实网络环境中，以发现某些仅靠本地测试难以暴露的问题。当前实现已设计为不会对矿工奖励造成负面影响。

**[`StartInference` 和 `FinishInference` 性能改进](https://github.com/gonka-ai/gonka/pull/812)**

- 减少 `MsgStartInference` 和 `MsgFinishInference` 的不必要状态写入和查询开销。
- 简化统计处理，减少推理生命周期中的工作量，提升区块执行稳定性。

在类似主网的条件下，这使得每个区块可容纳最多 100 倍的推理量，具体取决于工作负载和网络状况。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中可能同时出现大量剪枝活动，造成可避免的操作压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
如果您没有访问持有投票权的冷钱包密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求授权您代为投票。若无此授权，其他账户无法提交投票。
...
现已提供新的 vLLM / mlnode 构建，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍处于活跃状态的请求导致权重可能下降的风险。
...
抵押参数更新提案未达到法定人数，因此根据当前治理规则被否决。这意味着更新的参数将不会被激活。
...
- 审查创世时定义的抵押参数。
- 在第180轮次前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)正确设置，否则从第180轮次开始，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平仍相对较低时，存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
由于有效的归一化窗口比先前名义模型长约5个区块，每个轮次重新计算的权重成比例降低。

**观测到的GPU权重变化（第175轮次 → 第176轮次）**
...
升级提案v0.2.10的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提升网络公平性和可扩展性。
...
**宽限期：** 升级后引入了3000个区块的宽限期，期间无确认 PoC，并且对升级当期的错过率和无效率阈值放宽。
...
- 每1单位算力 0.032 GNK（约每块 H100 10 GNK）
- 错过率或封禁罚没 0.01%
- 无效推理罚没 0.5%

这意味着，在单个周期内，即使被惩罚，矿工最多只会损失其抵押物的 0.5%。而所需抵押物仅占日收益的约24%。

**警告：** 抵押参数将无视投票结果生效。如果此提案未通过，创世时定义的抵押参数将在第180周期自动激活，而非上述参数。

投票结束后、第180周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其收益将被削减5倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认关闭），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且对升级所在纪元的漏报率和无效率阈值放宽。
...
这意味着即使在一个纪元内被惩罚，矿工最多也只能损失其抵押物的0.5%。而所需抵押物仅占日奖励的约24%。

提案提交投票后将另行公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提案提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止运行。这是因为节点运行的是不再与网络兼容的旧版二进制文件。请遵循以下指南进行恢复：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
在当前纪元中观察到cPoC（确认PoC）与PoC的重叠。直到纪元最后一个区块，`is_confirmation_poc_active`被观察为`true`。

此重叠的影响正在评估中。初步观察表明，没有节点记录了PoC提交，导致本纪元累积权重为零。
...
此升级引入了一种新的PoC验证机制，通过为每位参与者分配固定的采样验证者集合，将复杂度从O(N^2)降低至O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际PoC时间标准化PoC权重**

此升级根据实际PoC耗时对参与者权重进行标准化，以减少区块时间漂移效应，使权重结果与实际执行时长保持一致。
...
现有主机无需升级其 `api` 和 `node` 容器。更新后的容器版本专为链上升级完成后加入的新主机设计。

**建议流程**

1. 活跃主机请在GitHub上审查本提案并留下反馈。
2. 社区审核该PR后，预计从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，启动正式的治理投票流程。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串而非数字编码，int64/uint64值也现在以字符串编码。
...
- 如果未来轮次通过治理引入更大或更苛刻的模型（例如 DeepSeek V3.2），可能会观察到不同的性能特征。
- 在 PoC 之外使用标准 vLLM 基于相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与 PoC v2 中观察到的 GPU 类型间相对性能差异一致的结果。

**跟踪器（仪表板）维护者操作**

在更新的权重分配生效后，跟踪器（仪表板）维护者可能希望审查其第 159 轮及之后的系数，以确保与当前 PoC v2 权重分配保持一致。
...
由于在当前的PoC周期中，验证阈值可能无法达成的高风险，主机们集体采取行动暂停了区块验证。
根据当前评估，用于处理此情况的机制可能无法按预期运行。为防止在不确定或不安全的条件下完成验证者最终确认，网络在验证者选择前已停止运行。
...
关于升级提案v0.2.9的链上治理投票已结束。该提案已获批准，并已在区块2451000处成功在主网上执行。此升级实现了PoC v2权重分配机制，并完成了向传统PoC机制的过渡。

**请注意**

- 下一个PoC周期（从第158纪元过渡到第159纪元）至关重要。请确保在线，以便在需要时能够及时应用后续步骤或缓解指令。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的ML节点有资格进入下一个（159）纪元并参与PoC v2权重分配。运行其他模型的ML节点将不会被纳入即将来临的纪元的参与集合中。
...
有关如何将ML节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、升级ML节点镜像以及移除其他模型的指南，请参阅[常见问题解答](https://gonka.ai/FAQ/#how-to-switch-to-qwenqwen3-235b-a22b-instruct-2507-fp8-upgrade-ml-nodes-and-remove-other-models)。
...
- 提供其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
下一代链上软件版本v0.2.9的升级提案现已上链并开放投票。若获批准，该提案将启用PoC v2用于权重分配，并通过链上治理完成向传统PoC机制的过渡。
...
其总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或无参与的总权重达到约 48%，这一比例过高。
...
Upgrade Proposal v0.2.8 的链上治理投票已结束。该提案已获批准并在主网上成功执行。
...
- 认证绕过：将 `epochId` 绑定到签名，以验证是否属于正确纪元。
...
本指南说明主机应如何根据 v0.2.8 模型可用性变化及即将推出的 PoC v2 更新来升级其 ML 节点。从第 155 个纪元开始，ML 节点配置需符合 PoC v2 要求。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个纪元后安排。迁移阶段结束后，不符合配置要求的 ML 节点的权重将不被计入。
...
- 您的所有 ML 节点均提供 Qwen/Qwen3-235B-A22B-Instruct-2507-FP8。这是唯一为 PoC v2 权重做贡献的模型。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估过渡到 PoC v2 的准备情况。一旦在活跃 Host 中观察到足够水平的采用率（约 50%），可能会提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[允许列表](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 的参与者注册截止时间对齐。
所有参与逻辑保持不变。本提案仅解决现有过滤逻辑中的一个小问题。

**可重复性与方法论**

`allowlist` 是通过预定义的一组硬件配置模式，从公开可观察的链上数据推导得出的。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
如果获得批准，网络将临时运行一个 `allowlist`，包含在之前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已与预定义的常见硬件配置模式进行比对，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
- 观测到的PoC权重低于使用同类硬件的其他参与者所表现权重的150%。
之前持续偏离这些模式的参与者在稳定窗口于区块 2222222 结束前不会被纳入 `allowlist`。
...
建议主机保持在线并监控更新，因补丁发布后可能需要采取后续操作。
...
- 临时重新激活创世验证者增强——一种此前曾使用过的、限时防御机制，现提议重新激活。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
创世验证者增强功能最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。

**主要变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用、有持续时间限制的防御机制，现提议重新激活。
- 在网络增长期间提供共识保护。在其先前运行期间：
    - 三位守护验证者共同持有约34%的共识投票权
    - 未向守护验证者授予额外奖励
    - 此配置有助于防止边缘情况下的共识停滞
...
此次升级将此前通过手动API更新分发并已在网络上使用的关键修复正式化。这些修复包括：
...
本升级中包含的预备性变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个错误，即以不支持格式发出的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
