...
!!! note "关于本页面"
    ...
    本页面内容不保证完整。如需获取最新信息（包括治理投票的启动及其当前状态），请参考链上数据或查看可用的浏览器和仪表板。

## 2026年6月6日

**[仅针对开发分片（devshard）升级的PR现已开放审查](https://github.com/gonka-ai/gonka/pull/1289)。**

这是首次仅涉及开发分片的升级，因此流程与标准链升级不同。开发分片升级会独立于主区块链更新其运行时，无需通过Cosmovisor协调全节点升级，不会影响主网行为，也不预期导致推理服务中断。

若该升级通过治理流程获得批准，新的开发分片版本将与现有的 v1 运行时并行运行。

请直接审查该PR，并对发现的问题、疑问、改进建议、边界情况或潜在漏洞留下评论。

在本次PR审查中有实质贡献者（如提出重要意见、发现漏洞或安全问题），可能有资格在下一次升级周期中获得社区赏金。

本次公告仅为征集PR审查意见，**不启动正式投票**。治理投票流程将在审查期结束后开始。

## 2026年5月28日

**MiniMax-M2.7 现已在 Gonka 网络上线**

此前在 v0.2.13 版本中宣布的引导阶段已完成。自链纪元 278 起，MiniMaxAI/MiniMax-M2.7 正式加入 Qwen3-235B 和 Kimi K2.6，成为活跃的模型组之一，MiniMax 组中获得的 PoC 权重现将按校准系数 0.3024 转换为共识权重。

针对 MiniMax 的按模型参与执行机制现已生效。已为 MiniMax 选择 DIRECT（直连）、DELEGATE（委托）或 REFUSE（拒绝）的节点无需额外操作——原有设置继续有效。尚未做出选择的节点建议立即完成选择，以避免遭受每纪元的惩罚机制影响（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。

## 2026年5月26日

**升级已执行：v0.2.13 现已在主网上线**

关于升级提案 v0.2.13（提案编号 54）的链上治理投票已结束。  
该提案已获**通过**，升级已于区块高度 `4267300` 在主网上成功执行。

## 2026年5月25日

**升级 v0.2.13：请提前下载二进制文件和 MiniMax-M2.7 模型权重**

v0.2.13 升级提案（提案编号 54，PR地址：[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)）已通过链上治理，升级时间已确定。
...
1. 请确保您的桥接容器已更新并完成同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT 代币元数据以及 CW20 `wrapped_token` 的 code id `105` 将通过升级处理器本身注册，因此桥接功能将在升级时自动在主网上激活。验证说明请参见：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划运行 `MiniMaxAI/MiniMax-M2.7`，请立即预先下载约 230 GB 的 FP8 权重文件。在启动窗口期间，Hugging Face 的速率限制和带宽饱和可能导致您错过首次资格检查。
3. 升级完成后，每个节点必须为**每一个**经治理批准的模型——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`——明确声明参与模式。即使只运行其中一个或两个模型，也必须对其他模型选择 **DELEGATE（委托）** 或 **REFUSE（拒绝）**。MiniMax 的截止时间为 **链纪元 `278`**。未采取行动的节点将从纪元 278 起，对其全部权重每纪元承受 15% 的惩罚。
4. 请计划在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解措施。确保 `.inference/data` 目录有足够空闲空间用于 cosmovisor 状态备份；如果 `application.db` 文件较大，请在升级前参考 [清理技术指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 中的方法进行处理。
5. v0.2.13 的校准将 Kimi K2.6 的 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 的吞吐量基准。此调整**仅影响您共识权重中来自 Kimi 的部分**；您来自 Qwen 的权重以及 Kimi 内部的 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 的收益将与 Qwen 相当，高于 Kimi。

- 提案链接：[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新提交至链上并开放投票。这是此前未通过的一项提案的重新提交版本，现已包含多项更新。

- 包含内容：Kimi 权重重新校准（`0.78`）、新增模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及多项 PoC/奖励修复。
- 在主网上激活以太坊桥接功能（详见下文专项说明）。
- 提案将升级后的宽限期延长至 3000 个区块，以便在新快照逻辑稳定期间避免对节点施加惩罚。
- 治理机制：将创世守护者（genesis-guardian）投票权降至约 25%，并将全链治理法定人数（quorum）设为 0.25。若守护者弃权，则非守护者需在剩余 75% 的投票权中达到约 1/3 的参与率才能满足法定人数（见 inference-chain 部分）。
- 所需准备：检查桥接容器、决定 MiniMax 参与方式、更新仪表板、参与投票。
...
- 更新推理验证阈值：
    ...
    - Kimi K2.6：`0.900`
    - MiniMax-M2.7：`0.922`
- 基于 vLLM 0.20.1 发布后 Qwen-on-B200 的基准重新校准 `WeightScaleFactor`：
    - Qwen 235B：`0.359`（保持不变）
    - Kimi K2.6：`0.78`（从 1.26 下调，相同 PoC 权重下，Kimi 每纪元共识权重约减少 38%）
    - MiniMax-M2.7：`0.3024`
...
- 将 devshard nonce 上限从 `20_000` 提升至 `1_000_000`。
- 将每纪元最大 devshard 数量从 `100` 提升至 `500_000`。
- 修复新模型启动期间确认 PoC 奖励计算问题。
- 在升级纪元的剩余时间内禁用确认 PoC，以便新快照逻辑从下一纪元干净启动。
- 当参与者重新激活时，重置 `ConsecutiveInvalidInferences`。
- 为在 v0.2.12 之前加入的 DAPI 补充缺失的 `MsgRespondDealerComplaints` 授权（authz grants）。
- 修复可能导致桥接和流动性池合约调用中出现间歇性权限错误的连接问题。
- 将创世守护者的调整后投票权降至约 25%，并将全链治理法定人数设为 `0.25`。若守护者不投票，则剩余 75% 投票权中需达到约 1/3 的参与率（`0.25 / 0.75 = 0.334`）。
- 将 4 个早期节点和经纪商地址添加至 `allowed_creator_addresses`。

**以太坊桥接主网激活**

- 通过升级处理器激活以太坊主网桥接设置。
- 注册以太坊桥接合约地址 `0x972a7a92d92796a98801a8818bcf91f1648f2f68`、USDC 和 USDT 代币元数据、桥接交易授权，以及 CW20 `wrapped_token` 的 code ID `105`。
- 激活后，桥接将支持 Gonka 主网与以太坊之间的跨链转账（包括在以太坊上封装 GNK 以及桥接 USDC/USDT）。封装/解封装脚本和运营商工作流程将另行文档说明。
...
- 修复启动耗时过长及 devshard 无效化流程中的边界情况。

**升级计划**

若提案通过，将通过链上升级提案更新二进制版本。有关升级流程的更多信息，请参见 [/docs/upgrades.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.13/docs/upgrades.md)。

**升级前建议的准备工作**

若提案通过，请提前完成以下准备。

**在纪元 278 前决定 `MiniMaxAI/MiniMax-M2.7` 的参与方式（惩罚将从此开始）**

对于每个经治理批准的模型，多模型 PoC 要求每个节点必须显式选择参与模式（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 之后若未采取行动，将导致惩罚。建议您提前决定首选选项，以便在提案通过且主网成功升级后能迅速执行。

**桥接容器更新 / 验证**

所有节点需确认其桥接容器已部署、运行最新版本并正确同步。部分节点可能已部署桥接容器，在此情况下，请先确认运行的是当前版本再进行其他操作。  
请遵循以下说明操作：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板 / 浏览器更新（升级前后均可）**

节点需更新仪表板/浏览器。请在 `gonka/deploy/join` 目录下运行以下命令。若您本地尚未克隆 `gonka` 仓库，请先参考加入网络指南。此次仪表板更新仅为容器拉取，安全无害，可在投票结束前后任意时间执行，不受结果影响。
```
...
- 运维人员时间线：投票截止（5月22日 22:12 UTC）→ 升级高度（约5月26日 14:42 UTC）→ 升级纪元剩余阶段跳过确认性PoC（≤10000个区块宽限期）→ 在 start_poc − 500 区块处生成MiniMax引导快照（约提前43分钟）→ 下一个纪元边界启动首个MiniMax PoC阶段 → 在链纪元278开始执行MiniMax惩罚机制  

**注意事项**

- 请计划在升级窗口期间保持在线，以便及时执行任何后续步骤或应对措施。  
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上 `application.db` 的 Cosmovisor 备份通常为数十GB，请提前确认）。有关安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。  
...
- 该提案将从升级高度开始，至升级纪元结束（10000个区块宽限期）期间，有意跳过确认性PoC。若提案通过，此跳过行为属预期设计，非系统故障；新的快照逻辑将从下一纪元开始生效。  
- 若提案通过，升级后 devshard 存储可选择使用共享的 Postgres 实例（与 payload 存储使用相同的环境变量）。本地 SQLite 仍为默认存储，并会自动清理（保留最近3个纪元的数据）。  
- 若提案失败（未达到法定人数，或 `no_with_veto` 超过三分之一），链上无任何变更，升级不会发生。运维人员可能会看到 `PROPOSAL_FAILED` 状态，此为正常现象，无需采取行动。  

## 2026年5月18日  

代理容器可能将对 devshard 的并行连接数限制从按客户端限制改为全局限制  

修复的 PR：[https://github.com/gonka-ai/gonka/pull/1183](https://github.com/gonka-ai/gonka/pull/1183)  
...
纪元 #266 的问题是由于一次攻击导致的，该攻击影响了运行 Kimi 模型的主机。尽管网络仍在运行，但此次攻击结合若干相关条件，导致许多参与者无法进入纪元 #266。

在应用额外防护措施期间，推理服务可能会暂时不可用。访问将逐步恢复，初期将通过若干社区代理和经纪端点提供。

**事件回顾**

在纪元 #266 中，网络的活跃权重出现了显著下降。

该问题已被追溯至具有非标准语义的推理请求。这一攻击向量影响了运行 Kimi 模型的主机，并导致其中许多主机无法参与 PoC。

在纪元 #267 中，主机已恢复正常，PoC 验证也已成功完成。

**推理服务可用性**

网络将不再接受使用受影响的非标准语义的请求。

在相关防护措施部署期间，推理服务可能会暂时中断。服务预计会逐步恢复，初期将通过社区代理和经纪端点提供访问。

**纪元 #267 中 Kimi 的权重**

由于协议的一项既有规则，Kimi 在纪元 #267 的活跃权重较低：任一模型的总权重不得超过上一纪元所有模型总权重的 75%。

由于纪元 #266 的总活跃权重显著降低，此规则也限制了 Kimi 在纪元 #267 的权重。

随着未来纪元中 PoC 参与逐步恢复正常，权重可能需要数日才能稳定。

**主机需采取的措施**

1. 尽可能保持 API 节点在线并可访问。这有助于维护主机参与情况的可见性，并支持后续审查。
2. 监控未来纪元中的 PoC 参与情况。确保您的节点按预期进入 PoC，且活跃权重被正确反映。
3. 仅使用受支持的推理请求格式。请勿发送或路由使用非标准请求语义的推理流量。
4. 预计推理服务将出现临时中断。服务不会立即在所有地方恢复，预计将通过社区代理和经纪端点逐步恢复。
5. 在社区频道或本帖中分享相关日志或观察结果。如果您的主机在纪元 #266 中受到影响，或在后续纪元中行为异常，此操作尤为重要。
...
在当前纪元（#266）期间，网络的活跃权重出现了显著下降。  
目前看来，PoC 投票未能收集到本纪元所需的足够票数。确切原因尚未确认，社区正在积极调查此事。

**对受影响参与者的建议**

如果您的节点未能进入本纪元，请尽可能保持 API 节点在线并可访问。  
这可能有助于补偿委员会收集 PoC 参与证据，从而更准确地核算受影响的贡献。

**调查进展**

社区成员目前正在审查纪元 #266 期间发生的情况。一旦明确根本原因，将及时发布更新。  
如果您有相关的观察记录、日志、假设或其他技术背景信息，欢迎在此分享。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 现已上链，进入投票阶段。
...
- 需提前准备：桥接容器检查、MiniMax 参与决策、仪表板更新、参与投票。
- 在提案未获批准前，链上不会发生任何变更。

PR 链接：[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)

**主要变更**

**模型**

- 添加 `MiniMaxAI/MiniMax-M2.7` 作为治理批准的模型及 PoC 模型。
...
- 将 devshard 的 nonce 上限从 `20,000` 提高至 `1,000,000`。
- 将每纪元最大 devshard 数量从 `100` 提高至 `500,000`。
- 修复新模型启动期间确认 PoC 奖励的记账问题。
- 在整个升级纪元中禁用确认 PoC，以便新快照逻辑从下一纪元干净启动。
- 当参与者重新激活时，重置 `ConsecutiveInvalidInferences`。
- 为在 v0.2.12 之前加入的 DAPI 补充缺失的 `MsgRespondDealerComplaints` 授权。
- 修复 Wasm keeper 对桥接和流动性池合约的权限检查。
- 将创世守护者调整后的投票权降至约 25%，并将全链治理法定人数设为 `0.25`。在守护者不投票的情况下，剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。

**以太坊桥主网上线**

- 通过升级处理器激活以太坊主网桥接配置。
- 注册以太坊桥合约地址 `0x972a7a92d92796a98801a8818bcf91f1648f2f68`、USDC 和 USDT 代币元数据、桥接交易授权，以及 CW20 `wrapped_token` 的代码 ID `105`。
...
- 添加状态快照以加快 devshard 启动和恢复速度。
- 修复 OpenAI 兼容 API 响应的解析问题。
- 修复启动耗时过长及 devshard 无效化流程中的边界情况。

**升级计划**

若提案获批，将通过链上升级提案更新二进制版本。有关升级流程的更多信息，请参阅 [/docs/upgrades.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.13/docs/upgrades.md)。

**升级前需准备的事项**

若提案获批，建议提前完成以下准备工作。

**`MiniMaxAI/MiniMax-M2.7` 参与选择（需在纪元 278 前完成）**

对于每个治理批准的模型，多模型 PoC 要求每个主机明确选择参与方式（DIRECT / DELEGATE / REFUSE）。在模型的 `PenaltyStartEpoch` 后未采取行动将导致处罚。请提前决定您的首选选项，以便在提案通过且主网成功升级后能迅速响应。

**桥接容器更新 / 验证**

所有主机需确认其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器，请先确认当前版本是否为最新，再决定是否采取进一步操作。  
请遵循以下指引：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
如果您无法直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予温密钥。
...
- 请计划在升级窗口期间保持在线，以便能够及时执行任何后续步骤或缓解措施。  
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份，请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。  
- 如果 `application.db` 占用大量磁盘空间，可参考 cosmovisor 备份[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中的清理方法进行处理。  
- 该提案将从升级高度开始，到升级周期结束（3000个区块的宽限期）期间，有意跳过确认 PoC（Proof of Confirmation）。若提案通过，此跳过行为属于预期设计，并非系统故障；新的快照逻辑将从下一个周期开始生效。  
- 若提案通过，升级后 devshard 存储可选择使用共享的 Postgres 实例（环境变量与 payload 存储相同）。本地 SQLite 仍为默认存储方式，并将自动清理（保留最近3个周期的数据）。

## 2026年5月7日

**需要更新/验证桥接容器**

为准备即将发布的 v0.2.13 版本升级（可能包含以太坊侧合约激活），所有节点运营者需确认其桥接容器已部署、运行最新版本并正确同步。
...
请直接审查 PR 中的代码，并就您发现的问题、疑问、改进建议、边界情况或潜在漏洞留下评论。

在下一轮升级周期中，有意义的审查贡献（包括重要评论、缺陷发现和安全问题）可能有资格获得社区奖励。

本次为**仅针对拉取请求的审查征集**，不启动正式投票。治理投票流程将在审查期结束后启动，预计为明天。

**主要变更**

**inference-chain**

- 确认 PoC（Proof of Confirmation）此前对“测量权重”、“保留权重”和“奖励重缩放”使用了不同的模型集合。在新模型引导期间，这可能导致同时服务了有效模型和尚未生效模型的诚实矿工被惩罚。修复方案为：存储一个确认模型及其权重缩放因子的单个纪元快照，并在所有确认和奖励权重计算中使用该快照。
- 当参与者重新变为 ACTIVE 状态时，`ConsecutiveInvalidInferences` 计数器未被重置，导致一次新的错误推理即可立即再次使其失效。现在该计数器在重新激活及即将晋升时会被重置。
- 在 v0.2.12 之前加入的 DAPI 节点，其从冷态到温态的 authz 授权中缺少 `MsgRespondDealerComplaints` 权限。本次升级将为这些节点补全该权限，使其能够响应经销商投诉。
- Devshard 结算曾使用硬编码的 `20_000` nonce 上限。现改为使用 `DevshardEscrowParams.MaxNonce`，并在 v0.2.13 升级中将其设置为 `1_000_000`。同时，`MaxEscrowsPerEpoch` 也提升至 `500_000`。
- 本次升级将为当前纪元安装一个“宽限期”条目，其中 `UpgradeProtectionWindow` 扩展为 3000 个区块。从升级高度到宽限期结束期间，确认 PoC 触发将被跳过，确保新的快照逻辑从下一个纪元才开始运行。此机制复用了 v0.2.10 中引入的宽限期原语。
- Wasm keeper 的访问权限在应用布线完成后才解析，以确保桥接和流动性池操作中的合约权限检查正常工作。

**decentralized-api**

- 某些兼容 OpenAI 的上游服务会返回数值型的 `stop_reason`。现在 `Choice.StopReason` 支持任意 JSON 类型，避免此类响应因反序列化失败而中断。
- 内部 devshard 存储迁移不再阻塞 dapi 启动。devshard 路由将在迁移和恢复完成前保持不可用状态。

**devshard**

- 此前 devshard 存储可能无限增长，因为旧的托管数据一直保留在单个 SQLite 数据库中。现存储按纪元划分，并在后台自动清理旧纪元数据，仅保留最近 3 个纪元。
- 为支持更大规模部署，devshard 新增了共享存储选项，现可使用 Postgres 作为主存储，SQLite 作为本地备用。
- Postgres 数据按 `epoch_id` 对会话、差异和签名进行分区，便于清理时直接删除旧纪元数据。
- 状态快照减少了长时运行会话的恢复工作量。
- 载荷查询固定绑定至托管所在的纪元，并对纪元边界和旧版 epoch-0 请求提供回退支持。
- 当前纪元的分片统计信息暴露了 nonce、版本、组别及每主机计数器。

**bridge**

- 桥接工具现已支持 Sepolia 标志，并能将 Gonka 的 BLS 密钥/签名转换为以太坊合约所期望的 EIP-2537 格式。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题：容器重启后，运行在 9100、9200 和 9400 端口的服务可能启动延迟较长。这会导致 api 激活延迟，部分矿工因此跳过了 Confirmation PoC。

修复方案通过并行加载 devshard 并从快照恢复现有 devshard 会话，消除了该阻塞问题。

[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)

请更新 api 容器的二进制文件。每个 PoC 开始前都有一个 500 个区块的无 CPoC 窗口（`confirmation_poc_safety_window`），因此当前可能是最安全的部署时机。

**更新前请确保没有正在进行的 CPoC 或 PoC。**
...
docker start api
```
部署后，请仔细检查 9100 和 9200 端口上的服务器是否正在运行：
```
...
在上一个周期中，发现了解析 Kimi-K2.6 某些响应时存在一个次要漏洞。
...
我们建议替换 api 容器的二进制文件。除修复外，新版本还为 devshard 数据库启用了修剪功能，并为 devshard 状态添加了对 Postgres 的支持。
...
docker start api
```
此外，如果你使用 MLNode 托管 Kimi-K2.6，请在部署参数中添加 `--enable-auto-tool-choice` 参数。为此，你可以重复执行部署命令（以 B200 为例）：
```
...
当 PoC / 确认 PoC 未正常执行时，应采取上述操作。

## 2026年5月5日

在 Kimi-K2.6 引导过程中观察到，实际中很难达到 30% 的最低直接参与阈值。为避免 Kimi-K2.6 在未来某个纪元中失去资格，并进一步简化模型的接入流程，提议将该阈值降低至 10%。

安全模型保持不变：PoC 验证机制本身未改变，仍然需要绝大多数验证算力来接受结果。

该提案将加速推进，以便在下一次 PoC 前生效。投票将持续 12 小时。
...
`moonshotai/Kimi-K2.6` 已通过引导流程，并加入 Gonka 网络的 PoC 参与。

该过程由全网多个节点协调完成：基础设施已准备就绪，意图已提交，委托与拒绝设置已完成，部署也已测试通过。

对于多模型 PoC 而言，这意味着 Kimi 现在作为独立的模型组拥有自己的参与记录和奖励追踪机制。

运行 Kimi 的节点运营者应继续像往常一样监控其 MLNode 及 PoC 参与状态。

## 2026年5月4日

**需采取行动：已提交 PoCIntent 的节点运营者请部署 `Kimi K2.6`**

`moonshotai/Kimi-K2.6` 今日通过了预评估检查。

已提交 PoCIntent 的节点运营者，需在 PoC 启动前（区块高度 `3874496`）将至少一个 MLNode 从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。

预评估与 PoC 启动之间有 500 个区块的窗口期。在此期间无 CPoC 任务，因此已声明意图的节点可安全切换其模型节点至 `Kimi K2.6`。

请遵循以下指南完成必要的部署步骤：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年5月4日

中转代理 `node1`、`node2` 和 `node3` 已停用。所有主网推理请求现将通过 `node4` 路由，其采用基于 `devshard` 的新型计费机制。

这标志着 `devshard` 已正式上线并具备生产就绪能力。后续推荐使用的公共网关为 `node4`。

**需采取行动**：请将您的终端节点更新为 `node4`。

## 2026年5月2日

今日的预资格验证未通过，对于 `PoCIntent` 小于 30% 的节点，权重极低。请保持您的 MLNode 使用 `Qwen235B`，并于明日提交下一周期的参与意图。

## 2026年4月30日

**升级已执行：v0.2.12 现已在主网上线**

关于升级提案 v0.2.12 的链上治理投票已结束。该提案已获**批准**，升级已在主网上成功执行。

**当前生效的主要变更**

- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明（Proof of Compute）从单一固定模型转变为按模型分组的 PoC 机制。每个经治理批准的模型将生成独立的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个节点必须参与每个模型组（可通过直接参与或委托 PoC 投票权重实现）。
- **引入第二个模型 `moonshotai/Kimi-K2.6`**：该模型组将在升级后两个周期激活。其权重系数为 Qwen235B 的 3.51 倍，基于相同硬件（8xH200，8xB200）下模型计算复杂度测算。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))：使 devshard 的发布与 DAPI / 主网发布周期解耦。
...
- **协议强化**：保留节点（`POC_SLOT=true`）将在单次 PoC / CPoC 中被随机抽样；其他更新包括将 `mlnode` 版本号传播至链上 `HardwareNode`、修复 DKG dealer 共识、对齐旧版验证者惩罚机制与抵押要求语义、确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中增加零时间戳容错。

**对节点运营者的建议**

- 部署、委托或明确拒绝新治理批准的模型（所包含的模型将在升级后两个周期激活）。请参考[操作指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
- 迁移：测试和迁移详细信息记录在 [v0.2.12 文档](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/upgrades.md) 中。
...
建议节点运营方在 [GitHub](https://github.com/gonka-ai/gonka/pull/948) 上审阅该提案并参与投票。

建议提前预先下载二进制文件，以避免在升级期间依赖 GitHub 的可用性。
...
建议节点运营方在 [GitHub](https://github.com/gonka-ai/gonka/pull/948) 上查看该提案并参与投票。

**升级前需采取的操作**

随着网络接近升级窗口期，各节点运营方应提前准备节点，以防提案通过。

此清理过程**必须在升级发生前完成**。如果在升级时，你的节点配置中包含不被支持的模型，**该节点将被拒绝并下线**。

v0.2.12 版本将移除所有不在升级后批准列表中的治理模型。在主网上，仅之前强制执行的模型和 Kimi 模型将被保留。  
每个 DAPI 会将 MLNode 配置本地持久化。启动时，系统会针对链上治理列表验证每一个已配置的模型。如果配置中包含至少一个不被支持的模型，整个节点将被拒绝，导致主机下线。

v0.2.11 版本通过将运行时视图裁剪为仅保留强制模型，掩盖了这一问题，因此即使持久化配置中仍包含多余模型，`/admin/v1/nodes` 接口仍显示正常。而 v0.2.12 将停止这种裁剪行为，改为直接加载持久化配置。

为解决此问题，以下脚本会查找 `/admin/v1/config` 中包含多余模型的节点，并向 `/admin/v1/nodes/<id>` 发送 `PUT` 请求，提交清理后的配置。这些更改将在60秒内完成持久化。保留的模型其参数、硬件和端口设置将完全保持不变。对于未列出强制模型的节点，脚本将跳过处理，需手动修复。

请将以下脚本粘贴到主机的 shell 中执行。默认情况下，脚本将应用更改。如需预览更改内容而不实际执行，请设置 `APPLY=dry`（或任何非 `--apply` 的值）。

仓库中的脚本地址：

- [Bash 脚本](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/proposals/governance-artifacts/update-v0.2.12/cleanup/cleanup_models.sh)
- [Python 脚本](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/proposals/governance-artifacts/update-v0.2.12/cleanup/cleanup_models.py)
...
下一版本链上软件 v0.2.12 的[升级提案](https://github.com/gonka-ai/gonka/pull/948)现已发布上链，进入投票阶段。

**主要变更**

- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算量证明（Proof of Compute, PoC）从单一固定模型转变为按模型划分的 PoC 组。每个经治理批准的模型生成其独立的本地 PoC 权重，并通过特定于模型的系数聚合为总共识权重。每个节点必须参与每个模型组（可直接参与或通过委托 PoC 投票权重实现）。
- **引入第二个模型 `moonshotai/Kimi-K2.6`**：该模型组将在升级后两个 epoch 激活。该模型的系数为 Qwen235B 模型系数的 3.51 倍，依据是在相同硬件（8xH200, 8xB200）上模型的计算复杂度确定。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 的发布与 DAPI / 主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已修复所有审计中发现的问题。
- **协议强化**。保留节点（`POC_SLOT=true`）将被随机抽样执行单次 PoC / CPoC 任务。其他更新包括：将 `mlnode` 版本信息同步至链上 `HardwareNode`，修复 DKG（分布式密钥生成）dealer 共识问题，统一旧版验证者惩罚机制与抵押品要求语义，确保 devshard 托管资金操作的原子性，并在解析 `inference_finished` 事件时增加对零时间戳的容错处理。
...
请从 `docker-compose.yml` 中部署 `versiond` 和 `proxy` 服务的最新版本（使用仓库标签 release/v0.2.12）：
```
...
部署、委托或明确拒绝新的经治理批准的模型（包含的模型将在升级后 2 个 epoch 激活）。请参考[指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
进行投票（可选 `yes`、`no`、`abstain`、`no_with_veto`）：
```
export NODE_URL=https://node3.gonka.ai/
...
- 请计划在升级窗口期间保持在线，以便及时执行后续步骤或应急处理指令。  
...
请直接审查 PR 代码，并对发现的问题、疑问、改进建议、边界情况或安全漏洞留下评论。

在下一轮升级周期中，有意义的审查贡献（包括重要评论、缺陷发现和安全问题）可能有资格获得社区赏金。

本次为 PR 审查呼吁，**不启动正式投票**。治理投票流程将在审查期结束后开始。

**主要变更**

- **多模型 PoC 概念验证（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))：将计算证明（Proof of Compute）从单一固定模型迁移至按模型划分的 PoC 组。每个经治理批准的模型生成其自身的本地 PoC 权重，再通过特定模型系数聚合为总共识权重。  
- **共识层交易费用并支持自动迁移** ([#937](https://github.com/gonka-ai/gonka/pull/937), [#981](https://github.com/gonka-ai/gonka/pull/981))：引入由治理控制的 gas 价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免除费用。`MsgPoCV2StoreCommit` 收取双组分费用（基础验证 + 数量线性增长），作为主要的 Sybil 防御机制。详情见 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。  
...
- **协议强化**：实现更强的 PoC v2 随机数生成器（完整 256 位熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG dealer 共识，对齐旧版验证者惩罚机制与抵押要求语义，确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中增加零时间戳容差。
...
- 确保冷账户持有足够代币（例如 100 GNK），以覆盖自动授予的费用额度上限。  
- 在治理批准新模型后，部署、委托或明确拒绝每个经治理批准的模型（包含的模型将在升级后 3 个 epoch 激活）。  
...
该版本已在最近多个 epoch 被部分矿工使用。初步观察表明，对于接近 PoC 启动时间运行的节点，稳定性有所提升。

此次更新修复了 PoC 启动附近的一个边界情况，此前在特定条件下可能导致性能下降。
...
v0.2.11 升级提案的链上治理投票已结束。提案已获**通过**，升级已在主网上成功执行。
...
性能优化使每区块推理数量最多可提升 100 倍，具体取决于工作负载和网络状况。  
其他变更的详细信息见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
更多变更细节请参阅治理提案文件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/)

## 2026年3月19日

**v0.2.11 升级：提前下载二进制文件**
...
建议提前下载二进制文件，以避免升级期间依赖 GitHub 的可用性。
...
下一次链上软件版本 v0.2.11 的升级提案现已发布到链上，进入投票阶段。如果提案获得通过，将引入基于 `devshards` 的推理会话初始版本，旨在提升推理的可扩展性，并显著优化 `StartInference` 和 `FinishInference` 的性能。
...
目前，通过每个推理任务对应一条链上交易的方式来处理推理请求，限制了系统的吞吐量。新设计将推理的执行和验证过程转移至一个指定的链下子组中进行，而区块链仅负责会话的创建和最终结算。

此版本是有意设计为早期且功能受限的实现。提出该方案供主网审查和有限生产环境测试，并非因为其已完全成熟，而是因为此类系统需要尽早暴露在真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为不会对矿工的奖励产生负面影响。

**[`StartInference` 与 `FinishInference` 性能优化](https://github.com/gonka-ai/gonka/pull/812)**

- 减少 `MsgStartInference` 和 `MsgFinishInference` 中不必要的状态写入和查询开销。
- 简化统计信息处理，降低推理生命周期中的计算负担，从而提升区块执行的稳定性。

在类似主网的条件下，该优化还可使每区块容纳的推理任务数量最多提升至原来的 100 倍，具体提升幅度取决于工作负载和网络状况。  
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)

**升级前建议操作**

**`application.db` 数据库修剪**

强烈建议节点运营者在升级前按照说明对 `application.db` 进行修剪。

提前执行此操作非常重要。如果大量节点将修剪操作推迟至升级之后，可能导致全网节点几乎同时开始修剪，造成不必要的运维压力。  
修剪操作说明详见：[https://gonka.ai/FAQ/#__tabbed_7_4](https://gonka.ai/FAQ/#__tabbed_7_4)

**浏览器（Explorer）更新**

请节点运营者更新仪表盘/浏览器。请在 `gonka/deploy/join` 目录下执行以下命令：
```
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml pull explorer
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
...
如果你无法直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[相关指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予温密钥。
...
下一版本链上软件升级 v0.2.11 的[拉取请求（Pull Request）](https://github.com/gonka-ai/gonka/pull/813)现已开放审查。欢迎提供反馈和改进建议。

对本次 PR 审查作出有意义贡献的参与者，可能在下一次升级中获得奖励金。
...
目前，通过每次推理生成链上交易的方式限制了吞吐量。新设计将推理执行与验证转移至指定的链下子组，而区块链仅负责会话创建和最终结算。

此版本是有意设计为早期且功能受限的原型。之所以将其提交主网进行审查和有限生产测试，并非因其已完全成熟，而是因为此类系统需尽早暴露于真实网络环境中。某些问题仅通过本地测试难以发现。当前实现已设计为不会对矿工奖励造成负面影响。

[`StartInference` 与 `FinishInference` 性能优化](https://github.com/gonka-ai/gonka/pull/812)

- 减少 `MsgStartInference` 和 `MsgFinishInference` 的不必要状态写入和查询开销。  
- 简化统计处理，降低推理生命周期中的计算负担，提升区块执行的稳定性。  

在类似主网的条件下，此优化还可使每区块容纳的推理请求提升至多达 100 倍，具体取决于工作负载和网络状况。

**升级前建议操作**

**`application.db` 裁剪（Pruning）**

强烈建议节点运营者在升级前按照说明对 `application.db` 进行裁剪。  
提前执行此操作至关重要。若大量节点在升级后才开始裁剪，可能导致全网裁剪操作集中发生，造成不必要的运维压力。  
裁剪操作说明详见[此处](https://gonka.ai/FAQ/#__tabbed_7_4)。
...
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
评审人员可以[在此处](https://github.com/gonka-ai/gonka/pull/813)查看完整的升级提案、迁移详情、测试摘要以及建议流程。

## 2026年3月16日

**API 二进制版本 `v0.2.10-post7` 已发布**

在 `v0.2.10` 版本中已发现一个潜在漏洞。为降低当前预升级阶段的风险，建议在下一轮 PoC 开始前将 API 二进制文件升级至 `v0.2.10-post7`。
...
现在可通过标准的函数调用模式（`type: "function"`）使用[工具调用](https://gonka.ai/developer/quickstart/#4-tool-calling)。
...
对于已在使用代理层的团队，这可能是一个简化技术栈的好机会，转而依赖原生功能。实际上，这将带来更清晰的集成模式和更易于维护的系统。

## 2026年3月6日

**提醒：v0.2.11版本升级预计将于下周初进入审查和治理投票阶段。**

请密切关注并计划参与投票。投票是支持网络发展并确保升级符合参与者实际需求的最简单方式之一。  
如果您无法访问持有投票权的冷钱包密钥，建议提前安排投票委托。请联系该密钥的所有者，请求授权您代表他们投票。若无此授权，无法从其他账户提交投票。
...
已发布新版 API 二进制文件。该版本更新了连接超时处理机制，并在 PoC 验证流程中引入了额外的检查。

1. v0.2.10 版本对 Executor → MLNode 连接引入了严格的 5 分钟超时限制，但某些请求可能需要更长时间。新 API 版本取消了该严格限制，恢复为原有超时设置。
2. 此前的请求重试系统会在推理因处理超时（非 TLS 超时）失败时仍尝试重试。对于长时间请求，服务端重试通常无效，容易导致相同的超时情况，同时客户端可能收到不一致的输出结果。新 API 版本在这些情况下不再重试推理请求。
3. 此前用于 PoC 验证时，即使 MLNode 处于保留状态且未参与 PoC 生成，仍会被纳入验证流程，可能导致推理遗漏。新版本已将此类节点从 PoC 验证中排除。
...
**建议（可选）：vLLM / mlnode 构建版本在 PoC 开始时中断进行中的请求**

一个新的 vLLM / mlnode 构建版本现已发布，该版本会在 PoC 开始时中断正在进行中的推理请求，以降低因请求在 PoC 启动时仍处于活动状态而导致潜在权重下降的风险。
...
- 该构建版本旨在与前一版本保持向后兼容。
- 已在少量节点上启用，但仍建议在部署前审阅变更内容。

## 2026年2月19日

**抵押参数更新提案 — 投票结果**

抵押参数更新提案已结束，但未达到法定投票人数（quorum）。根据当前治理规则，该提案被否决，因此更新后的参数将不会生效。
...
- 审查 Genesis 中定义的抵押参数；
- 在 Epoch 180 之前存入所需的 GNK；
- 确保正确设置[抵押](https://gonka.ai/host/collateral/)，否则自 Epoch 180 起，PoC 衍生的奖励将被减少 5 倍。

抵押机制的激活是协议从“宽限期”过渡到完全抵押型 PoC 权重模型的一部分。治理仍是调整参数的机制，但如果未批准替代方案，则默认规则将适用。

!!! note "重要提示：请预留缓冲额度存款"

    强烈建议参与者**不要**仅存入最低精确金额。由于归一化效应和网络级调整，PoC 权重可能在不同 epoch 之间波动。权重较小的节点可能经历相对更大的波动。为避免在 epoch 边界出现临时抵押不足的情况，建议存入最高达计算最低要求 2 倍的金额，尤其是在当前抵押水平仍较低的情况下。这能提供操作安全性，并防止因微小参数变化导致意外的权重降低。协议不会自动补充抵押。

若社区希望再次修改参数，未来可能会提出新的提案。

## 2026年2月19日

**PoC 权重归一化更新**

在最近的升级之后，由于 PoC 持续时间归一化，节点权重已进行调整。  
为了将 PoC 权重与实际区块生成时间对齐，校准参数是基于观察到的区块间隔选定的。实际实施中，有效的 PoC 参考窗口比之前的名义假设长约 5 个区块。
...
此前，PoC 权重计算依赖于名义上的 epoch 持续时间假设。在引入实时归一化后：
...
PoC 持续时间归一化现已生效，有效参考窗口比之前的名义假设长约 5 个区块，因此从 Epoch 176 开始的权重值已采用更新后的计算模型。  
所有基于 PoC 权重推导 H100 等效算力或奖励预测的追踪器和仪表板，应从 Epoch 176 起验证其转换系数。  
如果仍在使用归一化前的假设，则显示的硬件等效值和预测奖励可能会被高估。

## 2026年2月18日

**升级已执行：v0.2.10 现已在主网上线**

针对升级提案 v0.2.10 的链上治理投票已结束。该提案已获**批准**，升级已在主网上成功执行。此次升级引入了对 PoC 验证的重大优化，并实施了实时权重归一化，以提升网络的公平性和可扩展性。

**注意**

必须重启 ML Node 容器以触发模型的重新部署。请运行：
```
docker restart join-mlnode-1
```
向 `mlnode:3.0.12-post4-*` 的过渡应在升级引入的 3000 个区块宽限期结束前完成。

!!! note "兼容性说明"
    本次升级包含对 IBC 协议栈的升级至 v8.7.0 版本。请检查所有解析 `inferenced` CLI 输出的脚本。枚举类型和 int64/uint64 值现在将以字符串形式编码。
...
本次升级根据 PoC 实际经过的时间对参与者权重进行归一化处理，以减少区块时间漂移的影响，使权重结果与实际执行时长保持一致。

**启用 Qwen235B 工具调用功能**

本次升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型增加了工具调用参数（`--enable-auto-tool-choice`、`--tool-call-parser hermes`），并设置验证阈值为 0.958。  
要启用工具调用功能，必须重启 MLNode 容器内的 vLLM 服务。

**其他协议更新**

- 修复：PoC 与 CPoC 交集逻辑错误（PR #752）。
- IBC 升级：IBC 协议栈升级至 v8.7.0。
- 惩罚机制：惩罚阈值现由链上数据动态生成（PR #688）。
- 解锁机制：支持在解锁过程中进行流式解锁转账（streamvesting transfers with active vesting）（PR #641）。
- MLNode：发布更稳定的 MLNode 容器版本 ghcr.io/product-science/mlnode:3.0.12-post4 / ghcr.io/product-science/mlnode:3.0.12-post4-blackwell。

**宽限期说明**：本次升级引入了 3000 个区块的宽限期，在此期间无需执行确认型 PoC（Confirmation PoC），且在升级所在 epoch 内，对未命中率和无效率的判定标准将更为宽松。

更多变更详情请参阅治理文档：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md)

## 2026 年 2 月 18 日

**抵押参数更新提案现已开放投票**

关于更新抵押参数的提案已发布，现面向社区开放投票。

拟议参数如下：

- 每 1 单位算力需抵押 0.032 GNK（约每块 H100 需 10 GNK）
- 因未命中或被封禁触发的罚金比例为 0.01%
- 因无效推理触发的罚金比例为 0.5%

这意味着，在单个 epoch 内，即使受到惩罚，矿工最多仅会损失其抵押金的 0.5%。所需抵押金额仅约占日均收益的 24%。

**警告**：无论投票结果如何，抵押机制都将生效。如果该提案未获通过，则创世文件中定义的抵押参数将在第 180 个 epoch 自动激活，而非上述提议参数。

投票结束后至第 180 个 epoch 开始前，所有矿工必须按照[相关指引](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押账户，否则自第 180 个 epoch 起，其奖励将被削减为原来的 1/5。
...
下一次链上软件版本 v0.2.10 的升级提案现已发布至链上，进入投票阶段。若提案获得通过，将引入 PoC 验证的重大优化（默认关闭）并实现基于实时数据的权重归一化，以提升网络的公平性与可扩展性。
...
本次升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型添加了工具调用参数（`--enable-auto-tool-choice`、`--tool-call-parser hermes`），并将验证阈值设为 `0.958`。  
要启用工具功能，需重启 MLNode 容器内的 vLLM。升级后将设置一个 3000 个区块的宽限期，在此期间不执行确认型 PoC（Confirmation PoC），且在升级所在周期内采用更宽松的丢失率和无效率阈值。
...
若提案通过并完成升级，必须重启 ML Node 容器以触发模型的重新部署。请执行以下命令：
```
docker restart join-mlnode-1
```
应在此升级引入的 3000 个区块宽限期内完成向 `mlnode:3.0.12-post4-*` 的过渡。
...
- 请检查所有解析 `inferenced` CLI 输出的脚本。由于 IBC 栈升级至 v8.7.0，枚举和 int64/uint64 值现在以字符串形式编码。  
- 请计划在升级窗口期间保持在线，以便及时执行任何后续步骤或应急指令。  
...
- 如果 `application.db` 占用大量磁盘空间，可参考[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的方法进行清理。  
- 升级后，Postgres 将可作为本地负载存储的可选方案。
...
距离 Epoch 180 不足7天——是时候准备了。

根据 AMA 中的讨论以及社区成员提出的论点，提议以较低的抵押要求和最小化的惩罚机制启动。

拟提交社区投票的参数如下：

- 每1单位算力需抵押 0.032 GNK（约每 H100 需 10 GNK）  
- 因未出块或被隔离（jail）触发 0.01% 的惩罚  
- 因无效推理触发 0.5% 的惩罚  

这意味着，即使受到惩罚，矿工在一个 epoch 内最多仅损失其抵押金额的 0.5%。而所需抵押金额仅约为日收益的 24%。

一旦提案提交投票，将另行发布公告。

**警告**：无论提案投票结果如何，抵押机制都将在 Epoch 180 激活。若该提案未通过，Genesis 中定义的抵押参数将自动生效，而非上述参数。

未来任何抵押额度的上调都将通过单独的投票提出。目标是先观察网络稳定性，确保不合理的惩罚极少发生且仅在合理情况下触发。若网络表现出足够稳定性，将逐步提高抵押额度至《通证经济白皮书》中描述的水平（例如每 H100 约 100 GNK），以支持网络的长期成功。

## 2026年2月13日

**即将进行的 v0.2.10 版本升级投票与执行时间表**

即将进行的软件升级 v0.2.10 的链上投票预计将于周日晚上（洛杉矶时间）/ 周一上午（UTC 时间）开始。  
若提案经治理通过，升级计划将于周二执行。
...
请查看 GitHub 上 v0.2.10 升级的 PR 并提交您的反馈。有意义的审查贡献可能在下一次升级中获得赏金奖励。

[https://github.com/gonka-ai/gonka/pull/695](https://github.com/gonka-ai/gonka/pull/695)

## 2026年2月13日

如果您的节点未能及时应用最近的升级，可能在区块 2628371 因共识失败而停止运行。这是因为节点运行的二进制文件已过时，不再与网络兼容。恢复方法请参考本指南：  
[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)

## 2026年2月12日

**网络更新：补丁已发布（PoC / cPoC 重叠问题）**

现已发布补丁，用于解决当前 epoch（169/170）中观察到的问题。
...
当前周期中已观察到 cPoC（确认 PoC）与 PoC 出现重叠。直到该周期的最后一个区块，`is_confirmation_poc_active` 一直显示为 `true`。

目前正评估此重叠造成的影响。初步观察表明，没有任何节点记录 PoC 提交，导致本周期累积权重为零。
...
链上推理访问目前已开放，且不限于开发者使用。推理请求可通过此前更新中引入的“允许的传输代理”（Allowed Transfer Agents）发送。当前白名单可直接通过链上查询获取：
```
...
```
新版本库已上线：[https://gonka.ai/developer/quickstart/#3-inference-using-modified-openai-sdk](https://gonka.ai/developer/quickstart/#3-inference-using-modified-openai-sdk)

**注意**：若地址未包含在允许列表中，当前配置下通过该地址发起的推理请求将不会被接受。

---

## 2026年2月10日

**v0.2.10 升级版本 PR 审核通知**

下一次链上软件升级 v0.2.10 的 [Pull Request](https://github.com/gonka-ai/gonka/pull/695) 现已开放审查。欢迎提供反馈意见和改进建议。预计审查窗口将持续约 2 天。

对本次 PR 审查作出实质性贡献者，可能在下次升级中获得奖励。

本次仅为 PR 审查呼吁，**不**代表正式投票开始。治理投票流程将在审查期结束后启动。

### 主要变更

**[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC 验证采样优化**

本次升级引入了一种新的 PoC 验证机制，通过为每个参与者分配一组固定的采样验证节点，将复杂度从 O(N²) 降低至 O(N × N_SLOTS)。参考设计与分析文档：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 根据实际 PoC 时间进行 PoC 权重归一化**

该升级通过实际 PoC 耗时对参与者权重进行归一化处理，以减少区块时间漂移的影响，使权重结果更真实反映执行持续时间。
...
- **[PR #697](https://github.com/gonka-ai/gonka/pull/697)** 支持 macOS 的便携式 BLST 构建
...
- **[PR #634](https://github.com/gonka-ai/gonka/pull/634)**：添加请求体大小限制，降低 DoS 风险
- **[PR #727](https://github.com/gonka-ai/gonka/pull/727)**：#634 的后续，将响应写入器传递给 `http.MaxBytesReader` 并统一测试
...
- **[PR #639](https://github.com/gonka-ai/gonka/pull/639)**：修复任务申领路径中缺少错误返回的问题
- **[PR #643](https://github.com/gonka-ai/gonka/pull/643)**：在执行者选择中处理空参与者情况
...
1. 活跃主机在 GitHub 上审查此提案并留下反馈。
2. 社区审查 PR 后，将从该分支创建 v0.2.10 发布版本，并提交链上升级提案，启动正式的治理投票流程。
3. 若链上提案通过，则在链上执行升级后合并该 PR。

从 [upgrade-v0.2.10](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10) 分支（而非 `main` 分支）创建发布版本，可最小化 `/deploy/join/` 目录中容器版本与链上二进制版本不一致的时间，从而为新主机提供更顺畅的接入体验。

### 测试与迁移

v0.2.10 的测试指南和迁移细节记录在此：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10)，请仔细查阅。

### 兼容性说明

如果您有脚本用于解析 `inferenced` CLI 的 JSON 输出，请在本次升级后重新检查。由于 ibc-go 升级至 v8.7.0，枚举现在以字符串形式编码而非数字，int64/uint64 值也已改为字符串编码。
...
对于 v0.2.9 升级后创建的 warm key 授权操作，应使用 [CLI v0.2.9 版本](https://github.com/gonka-ai/gonka/releases/tag/release%2Fv0.2.9)。
...
随着 PoC v2 启用，权重分配现基于当前模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的实测推理性能。因此，各类 GPU 的中位权重及相对权重比例均已调整。

**GPU 权重变化情况（Epoch 158 → 159）**
...
- 观察到的变化表明，GPU 权重差异现在反映的是特定模型的推理吞吐能力，而非名义硬件规格。例如，H100 PCIe 权重下降幅度大于 H100 HBM3，这与 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的实测推理表现一致。
- 在当前模型配置下，根据实测推理轨迹，B200 GPU 的推理性能并未优于 H100 级 GPU。
- 若未来通过治理引入更大或更高要求的模型（如 DeepSeek V3.2），可能会观察到不同的性能特征。
- 在 PoC 外部进行的控制推理基准测试（使用相同模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 和标准 vLLM 推理）显示，各类 GPU 之间的相对性能差异与 PoC v2 中观察到的结果一致。

### 对追踪器（仪表板）维护者的建议

鉴于权重分配已更新，建议追踪器（仪表板）维护者检查第 159 轮及后续周期的系数设置，确保其与当前 PoC v2 的权重分配保持一致。
...
请各主机尽快应用补丁，以确保 PoC 验证正确运行并安全恢复区块生产。
```
# Download Binary
...
由于当前证明（PoC）周期内存在无法满足验证阈值的高风险，各节点运营方已集体采取行动，暂时停止了区块验证。  
根据目前评估，用于处理此情况的机制可能无法按预期运行。为防止在不确定或不安全条件下完成验证者最终确定（validator finalization），网络在验证者选择前已被主动暂停。
...
所有节点运营方必须做好准备，随时安装紧急补丁。  
请保持在线并密切关注公告。补丁就绪后将立即发布进一步指示。

## 2026年2月1日

**升级已执行：v0.2.9 已在主网上线**

针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获**通过**，升级已于主网区块高度 2451000 成功执行。本次升级启用了 PoC v2 的权重分配机制，并完成了从旧版 PoC 机制的全面过渡。

**注意事项**

- 下一个 PoC 周期（从第158轮过渡到第159轮）至关重要。请确保在线，以便在需要时及时执行后续步骤或应对措施。  
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型的 ML 节点，才有资格进入下一个（第159轮）周期并参与 PoC v2 的权重分配。运行其他模型的 ML 节点将不会被纳入下一轮周期的参与者集合中。
...
- PoC v2 已作为权重分配的现行机制启用  
- 确认型 PoC（V2 跟踪）被用作结果的权威来源  
- 旧版 PoC 逻辑不再用于权重计算  
...
ML 节点要具备参与 PoC v2 权重分配的资格，必须同时满足以下两个条件：

- 节点提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型  
- 节点运行的镜像为 PoC v2 兼容版本：
    - ghcr.io/product-science/mlnode:3.0.12-post1  
    - ghcr.io/product-science/mlnode:3.0.12-post1-blackwell  

**针对 cPoC 情况的奖励分配修正**

在因 cPoC 惩罚导致奖励被减少或排除的情况下，未分配的部分将转入社区资金池（Community pool）。此前，此类奖励会被重新分配给其他参与者。

**其他协议更新**

- 转账代理（Transfer Agent）角色在初始阶段仅限于[指定名单](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9#transfer-agent-whitelist)内的地址  
- 曾参与 PoC 生成但忽略 PoC 验证的节点，已被从[参与者名单](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9#suspicious-participant-removal)中移除  
...
建议各节点运营方提前在 [GitHub](https://github.com/gonka-ai/gonka/pull/668) 上审阅提案内容并参与投票。  
建议提前下载升级所需二进制文件，以避免升级期间因 GitHub 服务不可用而影响升级进程。
```
# 1. Create Directories
...
下一版本链上软件 v0.2.9 的升级提案现已发布上链，进入投票阶段。若提案获得通过，将通过链上治理机制启用 PoC v2 进行权重分配，并完成从旧版 PoC 机制的全面过渡。
...
- 网络将以单模型模式运行
- 用于 PoC v2 和权重分配的模型为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- 提供其他模型的 ML 节点将不参与 PoC v2 的权重分配。在支持的情况下，系统可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型
...
在因 cPoC 惩罚导致奖励被减少或排除的情况下，未分配的奖励部分将转入社区资金池（Community pool）。此前，此类奖励会被重新分配给其他参与者。

**其他协议更新**

- 转账代理（Transfer Agent）角色在初始阶段仅限于[指定白名单](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9#transfer-agent-whitelist)
- 在忽略 PoC 验证的情况下仍参与 PoC 生成的节点，已被移出[参与者白名单](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9#suspicious-participant-removal)
- 当 PoC v2 验证投票未达到阈值时，[守护者权重](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9#guardian-tiebreaker-for-poc-v2-voting) 将作为确定性备用机制启用

上述变更的更多细节，请参见治理文件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9)
...
提案详情及投票可通过 `inferenced` 进行。任意活跃节点均可使用。可用节点包括：
...
请投出您的票（`yes`、`no`、`abstain`、`no_with_veto`）：
```
...
建议节点运营方在 [GitHub](https://github.com/gonka-ai/gonka/pull/668) 上查看该提案并参与投票。

**注意事项**

- 请计划在升级窗口期间保持在线，以便在需要时能够及时执行后续步骤或应对措施。  
- 升级期间，Cosmovisor 将在 `.inference/data` 目录中创建完整的状态备份。请确保升级前有充足的磁盘空间。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。  
- 如果 `application.db` 占用大量磁盘空间，可参考[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的清理方法进行处理。  
...
在最近一个纪元（epoch）中，大量机器学习节点（ML Node）未获得 PoC 权重。  
分析表明，这是由于 PoC 验证参与度不足所致。在多个案例中，参与者发布了 nonce，但未执行验证，或验证行为远低于协议要求。  
以下表格列出了在上一个纪元拥有权重、在当前纪元提交了 PoC nonce，但在 PoC 验证阶段未参与或参与不足的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)

这些节点的总权重约为 36%。加上完全未参与 PoC 的参与者，无参与或低参与度的总权重达到约 48%，处于极高水平。  
如果您的节点在该表格中 `validated` 列显示为 0，请检查您的 PoC 验证日志和配置，确保验证功能正常运行。

用于生成上述表格的分析过程详见此笔记本：[https://github.com/gonka-ai/gonka/blob/gm/debug-155-1/debug-validation.ipynb](https://github.com/gonka-ai/gonka/blob/gm/debug-155-1/debug-validation.ipynb)

## 2026年1月29日

**升级已完成：v0.2.8 已在主网上线**

关于升级提案 v0.2.8 的链上治理投票已结束。该提案已获**通过**，并在主网上成功执行。  
...
- **vLLM 集成**：PoC 已直接集成至 vLLM，实现从推理到 PoC 的即时切换，无需卸载模型。  
- **MMR 承诺**：通过 Merkle Mountain Range（MMR）承诺将产物存储移至链下；仅将 `root_hash` 和 `count` 记录在链上。  
- **双模式迁移**：同时支持 V1（常规 PoC）和 V2（确认型 PoC）的权重追踪。

**模型可用性更新**

支持的模型集现已受限。除以下两个模型外，所有先前支持的模型均已从活跃集合中移除：
...
- **SSRF 与 DoS 防护**：验证 `InferenceUrl`，拒绝内网 IP，并增加超时机制以防止请求挂起。  
- **投票翻转防护**：拒绝重复的 PoC 验证，防止权重被覆盖。  
- **认证绕过修复**：将 `epochId` 绑定至签名，确保验证针对正确的纪元。

**参与 PoC v2 的节点运营方要求**

要具备参与 PoC v2 的资格，节点运营方必须完成以下两项操作：

- **模型配置**：将 ML 节点配置为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型服务  
- **ML 节点升级**：使用支持 PoC v2 的版本：
    - ghcr.io/product-science/mlnode:3.0.12  
    - ghcr.io/product-science/mlnode:3.0.12-blackwell  

!!! note  
    未能同时满足以上两个条件的节点，在网络切换至单模型配置后，将不具备参与 PoC v2 的资格。PoC v2 的权重分配过渡仍取决于观察到的采用率及后续治理提案。

**维护与运维**

- **Cosmovisor**：节点和 API 二进制文件的更新将自动处理。现有节点运营方无需手动更新正在运行的容器。  
- **磁盘空间**：Cosmovisor 将在 `.inference/data` 目录中创建完整状态备份，请确保至少有 250 GB 以上的可用空间。  
- **Postgres**：升级后可配置使用 Postgres 进行本地负载存储。

建议在升级后持续监控节点状态并通过 Discord 保持沟通，以确保系统稳定。

## 2026年1月28日

**如何切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、升级 ML 节点并移除其他模型？**

本指南说明节点运营方应如何根据 v0.2.8 版本的模型可用性变更及即将到来的 PoC v2 升级更新其 ML 节点。自第 155 纪元起，系统将开始检查 ML 节点配置是否符合 PoC v2 要求。建议运营方在此前完成配置审查与准备。PoC v2 的迁移可在第 155 纪元后安排。迁移阶段结束后，不符合配置要求的 ML 节点可能不再计入权重。

**1. 背景：模型可用性变更（v0.2.8 升级）**

作为 v0.2.8 升级的一部分，活跃模型集合已更新。
...
`Qwen/Qwen3-32B-FP8` 在迁移期间仍受支持，但不参与 PoC v2 的准备或权重计算。参与 PoC v2 必须提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型服务。

**已移除的模型**

所有先前支持的模型均已从活跃集合中移除，不得继续提供服务。

**3. PoC v2 准备条件（重要）**

成功参与 PoC v2 过渡需同时满足以下两点：

- 所有 ML 节点必须提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 服务。这是唯一对 PoC v2 权重有贡献的模型。  
...
!!! note "重要"
    ...
    - 未同时满足两个条件的节点，在网络切换至单模型配置后将不具备资格。  
    - ML 节点升级必须在迁移完成前完成，PoC v2 将通过 v0.2.8 升级后的单独治理提案激活。  
    - v0.2.8 升级本身**不会启用** PoC v2。

**3. 检查 ML 节点分配状态（推荐的安全步骤）**

在更改模型前，建议先检查当前 ML 节点的分配状态。请查询您的网络节点（Network Node）管理 API：
```
...
- 第一个布尔值：表示该节点在当前 epoch 是否正在提供推理服务  
- 第二个布尔值：表示该节点是否计划在下一轮 PoC 中提供推理服务  

**推荐行为**

- 优先仅在第二个值为 `false` 的节点上更改模型  
- 在 PoC v2 行为仍在观察阶段时，此举可降低风险  
- 建议跨多个 epoch 逐步推进模型更新  

**4. 更新 ML 节点的模型：仅保留支持的模型**

预下载模型权重（推荐）。为避免启动延迟，请提前将权重下载到 `HF_HOME` 目录中：
```
mkdir -p $HF_HOME
...
```
通过管理 API 进行的更改将在下一个训练周期（epoch）生效。[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)

!!! note

    `node-config.json` 仅在首次启动网络节点 API 或本地状态/数据库被清除时使用。如需重新初始化启动，请编辑此文件。对于已运行的节点，应通过管理 API 进行模型更新。

**5. 升级 ML 节点镜像（PoC v2 所需）**

编辑 `docker-compose.mlnode.yml` 文件，并更新 ML 节点镜像：
...
确认ML节点仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型服务，该模型是唯一用于PoC v2权重及未来权重分配的模型：
```
...
!!! note "治理与 PoC v2 激活说明"
    ...
    **阶段 1：观察期（v0.2.8 升级后的当前状态）**
    ...
    在 v0.2.8 升级后，PoC v2 的逻辑已存在，但尚未用于权重分配。

    此阶段期间：

    - 节点运营商（Host）可以提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 或 `Qwen/Qwen3-32B-FP8` 服务
    - 节点运营商必须将其 ML 节点切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 服务，并升级至兼容 PoC v2 的版本，才能参与 PoC v2 的权重贡献
    - 网络将观察模型采用情况，以评估节点对过渡到 PoC v2 权重的准备程度
    ...
    **阶段 2：治理提案（可选，未来阶段）**
    ...
    - 可能提交一项独立的治理提案
    - 该提案将请求批准激活 PoC v2，并将其用于权重分配

    采用率阈值仅为观察参考，不会触发任何自动变更。

    **阶段 3：激活（仅在治理批准后）**

    PoC v2 只有在链上治理提案获得通过后，才会成为正式的权重分配方式。在提案获批之前：
...
- ML 节点正在提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- 配置中已移除所有其他模型
...
建议提前预下载升级二进制文件，以避免升级期间因 GitHub 服务不可用而影响升级。
...
下一次链上软件版本 v0.2.8 的升级提案现已发布上链，进入投票阶段！  
您的审阅与投票对确保网络稳定性及未来能力至关重要。
...
- 使用 MMR（Merkle Mountain Range）承诺将产物存储迁移至链下——仅将 root_hash 和 count 记录在链上。
- 包含双模式迁移策略：V1 用于常规 PoC，V2 跟踪用于上线期间的确认型 PoC。

**模型可用性变更**

作为 v0.2.8 升级的一部分，支持的模型集合将进行调整。除以下两个模型外，所有此前支持的模型将从活跃集合中移除：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML 节点版本，将用于评估向 PoC v2 迁移的准备情况。当活跃 Host 中达到一定采用率（约 50%）后，可能会提交另一项治理提案，以批准并激活 PoC v2 用于权重分配。该阈值为观察性指标，不会触发任何自动的网络变更。

在下一次网络升级通过治理批准后，网络将暂时仅支持 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。

**安全性、正确性与可靠性改进**

- SSRF 与 DoS 防护：校验 InferenceUrl，拒绝内网 IP，并添加超时机制防止请求挂起。
- 投票翻转防护：通过拒绝重复提交防止 PoC 验证被覆盖。
- PoC 排除机制修复：修复 getInferenceServingNodeIds，确保正确排除参与推理服务的节点。
- 认证绕过与重放攻击防护：将 epochId 绑定至签名，并针对正确的 epoch 验证授权。

由于变更内容较多，此处仅列出部分重点。更多更新与修复的完整列表请参见 [GitHub 拉取请求](https://github.com/gonka-ai/gonka/pull/539)。

**Host 需采取的行动**

为参与 PoC v2 迁移，Host 必须完成以下两个步骤：

- 确保您的 ML 节点已配置为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 服务
...
您可使用 `inferenced` 命令获取提案详情并提交投票。请注意，任何活跃节点均可用于查询或投票。当前可用节点包括：
...
- 升级计划在区块 2387000 进行。该区块预计时间为 2026 年 1 月 29 日 06:30:00 UTC。

如果您是节点运营者（Host），请查看并参与投票。

**注意 1：** 请计划在升级窗口期间保持在线，以便在需要时能够及时执行后续步骤或缓解措施。

**注意 2：** 升级期间，Cosmovisor 将在 `.inference/data` 目录中创建完整的状态备份。请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中旧备份的说明，请参见[此处](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。如果 `application.db` 占用大量磁盘空间，可使用[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)描述的方法进行清理。

**备注：** 升级后，Postgres 可配置为本地负载的存储。
...
随着延期确认，请利用此时间准备您的节点以满足新的 PoC 要求。

- **模型更新**：请将您的 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。
- **逐步 rollout**：如果您运行多个 ML 节点，建议在多个 epoch 中逐步完成更新。

**如何更新**

更新现有 ML 节点的说明请见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)
...
该提案将当前稳定期延长约两周。延长期间旨在为即将到来的 PoC 变更及相关网络升级提供额外测试时间。有关 PoC 新进展的更多细节，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)

此次延期还为节点运营者提供了准备时间，以适配新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新说明请见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。对于运营多个 ML 节点的用户，建议在多个 epoch 中逐步完成更新。

**投票范围**

若提案通过，网络将继续临时运行于现有的 `allowlist`（包含未表现出非标准硬件行为的节点运营者）之下。

开发者 `allowlist` 也将同步延长，持续生效至区块 2459375。

未被列入 `allowlist` 的节点运营者在延长的稳定期内仍无法参与 PoC，此期间将在区块 2443558 结束。

**可复现性与方法论**

`allowlist`：

- 可在此处查看：[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)
- 基于链上公开数据，使用预定义的一组硬件配置模式生成。这些模式通过开源脚本评估，脚本位于：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
`allowlist` 具有固定过期时间，不会在延长的稳定期后继续存在。一旦 `allowlist` 在区块 2443558 失效：

- 网络将恢复至稳定期之前的常规参与规则，或
...
`白名单（allowlist）` 失效时间：在区块 2443558 自动失效。

## 2026年1月10日

**临时参与者白名单修正**

一项新的治理投票当前处于活跃状态。该提案通过将若干地址重新加入[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况——这些地址此前因硬件名称为空且ML节点权重为零而被错误排除。此外，该提案还将少量开发者账户添加至允许的开发者列表中，并将 `白名单` 的过期时间与参与者注册截止区块 2,222,222 保持一致。  
所有参与逻辑保持不变，此提案仅修复现有过滤逻辑中的一个次要问题。

**可复现性与方法论**

`白名单` 基于链上公开可查数据，通过一组预定义的硬件配置模式生成。这些模式使用以下开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)

`白名单` 文件可在以下地址查看：[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)

**如何投票**

您可以使用 `inferenced` 命令获取提案详情并提交投票。

请注意，任何活跃的节点均可用于查询或提交投票。当前可用的节点包括：
...
`白名单（allowlist）` 失效时间：在区块高度 2,222,222 自动失效。

## 2026年1月10日

**临时参与者 `白名单` 已获批准，将在第135轮（Epoch 135）生效**

针对稳定期的临时参与者 `白名单` 链上治理投票已结束。

该提案已获得通过。此提案定义了一个临时 `白名单`，包含那些在多个轮次中行为保持一致的参与者。

**当前生效的关键变更**

1) 网络将仅允许满足以下条件的参与者组成的 `白名单` 运行：
   
- 报告的硬件特征符合常见配置模式（被过滤的非标准配置字符串列表详见：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)）
- 展示的 PoC 权重未超过同类硬件所观测权重的 150%

2) 此前持续偏离上述模式的参与者，将在稳定期结束（区块 2,222,222）前被排除在 `白名单` 之外。

**执行特性**

- `白名单` 将从下一轮（第135轮，Epoch 135）开始生效
- 生效时间为第135轮的首次 PoC 阶段
- 无需软件升级
- 自此之后，`白名单` 将持续有效，直至并包括区块 2,222,222

**可复现性与方法论**

- `白名单` 完全基于公开可观察的链上数据生成
- 硬件描述符通过开源脚本，依据预定义的配置模式集合进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
- 生成的 `白名单` 已发布于此：[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)
...
**时机已至：稳定期临时参与者 `白名单` 投票**

在成功部署修复 PoC 相关共识故障的补丁后，一项新的治理投票现已启动。

随着正常出块功能恢复，网络即将进入一个短暂的稳定期，为后续增长做准备。

本次投票将定义稳定期期间的参与者 `白名单`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），该名单反映的是行为持续符合网络预期的参与者集合。

**投票范围**

若提案通过，网络将在短期内仅允许符合以下条件的参与者加入 `白名单`：

- 在此前多个轮次中，其报告的硬件特征与预定义的常见硬件配置模式一致，用于识别异常与不一致行为（具体非标准配置字符串列表见：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)）
- 观测到的 PoC 权重低于同类硬件其他参与者所展示权重的 150%

此前持续偏离上述模式的参与者，将在稳定期结束（区块 2222222）前不被纳入 `白名单`。

**可复现性与方法论**

`白名单` 基于公开可观察的链上数据，使用预定义的硬件配置模式集生成，并通过以下开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)

`白名单` 文件地址：[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)

**执行特性**

- 若提案通过，`白名单` 将自动生效
- 无需软件升级
- `白名单` 预计在投票通过后的下一次 PoC 阶段激活，对应区块：2089140
- 自此之后，`白名单` 将持续有效，直至并包括区块 2222222
- 后续如需调整，仍需通过治理流程决定

**稳定期结束后**

`白名单` 设有固定失效区块，不会在稳定期后继续存在。一旦在区块 2222222 失效：

- 网络将恢复至稳定期前的原有参与规则，或
- 任何替代配置需通过独立的治理决策另行定义
...
- `Allowlist` 激活时间：在区块 2089140 的下一次 PoC 执行后。
- `Allowlist` 过期时间：在区块 2222222 自动失效。

请查看并投票，若您是节点运营者（Host）。
...
解决近期共识问题的补丁已准备就绪，详细操作说明即将发布。  
每位活跃节点的参与对网络恢复正常运行至关重要。请保持在线，并在说明发布后及时准备应用更新。

## 2026年1月8日

**网络更新 — PoC 期间出现共识故障**

在“计算证明”（Proof-of-Compute, PoC）过程中，网络上观察到共识故障。  
问题已被定位，目前正在准备补丁以解决根本原因。详细技术说明与操作指引将很快发布。  
建议节点保持在线并关注更新，补丁发布后可能需要采取后续操作。

## 2026年1月8日

**v0.2.7 升级提案：创世验证者增强功能已在主网上线**

关于 **v0.2.7 升级提案：创世验证者增强功能** 的链上治理投票已结束，提案已获得 **通过**，并成功在主网上部署。

**现已生效的关键变更：**

**创世验证者增强功能（临时）**

- 临时重新启用“创世验证者增强功能”——此前曾使用过的、有时限的防御机制，现被提议重新激活。
- 在网络增长期间提供共识保护。在之前运行期间：
    - 三个守护者（Guardian）验证者共同持有约 34% 的共识投票权
    - 守护者验证者未获得额外奖励
    - 此配置有助于防止极端情况下的共识停滞
- 当满足以下两个条件时，创世验证者增强功能将自动停用：
    - 网络总算力达到 15,000,000
    - 达到区块 3,000,000

**协议稳定性修复（全网范围）**

此次升级将此前通过手动 API 更新分发、且已在网络中使用的若干关键修复正式纳入协议。这些修复包括：

- 修正了失败推理请求的会计核算问题（包括对不支持格式的请求被处理但未标记为完成的情况）
- 提升了推理失败处理的容错能力
...
- **节点注册限制**：在达到区块 2,222,222 之前（约两周后），将暂停新节点（Host）的注册。此措施旨在稳定网络，为后续增长做好准备。
- **开发者注册限制**：在稳定期间，新开发者地址的注册将暂停。一个预定义的开发者地址 `allowlist` 立即生效。列入白名单的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制（包括注册与推理执行）将持续有效，直至区块 2,294,222（约19天后）。

**治理可控机制**

本次升级包含的预备性变更，使未来可通过治理机制控制参与者准入和推理执行，而无需再次进行软件升级。但本次提案**不启用**任何此类由治理触发的限制，相关功能需经后续治理投票决定。

**第117轮奖励发放**

本提案涵盖与链暂停相关的两次奖励发放（第117轮）：

- 在第117轮期间活跃但未收到该轮奖励的节点，将补发其错过的奖励。
- 所有在第117轮期间活跃的节点，将获得一笔额外支付，金额为第117轮奖励的 **1.083倍**，该支付将统一适用于所有符合条件的节点（包括已收到原始奖励的节点）。

**关于持续时间与执行的说明**

本次升级重新激活或引入的所有保护机制均为临时性，无需手动治理干预即可自动移除。

**下一步操作：**

- 节点运营者无需采取进一步操作。
- Cosmovisor 在每次更新时都会在 `.inference` 状态文件夹中创建完整备份。为安全执行更新，建议预留 **250GB 以上**的磁盘空间。[点击此处](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory) 了解如何安全删除 `.inference` 目录中的旧备份。
...
```
当所有命令成功执行且显示确认消息时，即可认为二进制文件已成功安装。
```
...
- Cosmovisor 在升级期间会自动对 `.inference/data` 目录进行完整备份。请确保有足够的磁盘空间。如果磁盘使用率较高，可安全删除 `.inference` 目录中的旧备份。[详见说明。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
- 若 `application.db` 文件过大，可使用[以下方法](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)进行缩减。

**可选：跳过 Cosmovisor 自动备份**

Cosmovisor 支持通过为 `node` 容器设置环境变量 `UNSAFE_SKIP_BACKUP=true` 来跳过升级时的自动状态备份。

此操作可减少磁盘占用并加快升级速度，但若升级失败，将无法恢复至先前状态，风险较高。

## 2026年1月7日

**致所有节点运营者的重要提示**

Cosmovisor 升级过程中可通过为 `node` 容器设置环境变量 `UNSAFE_SKIP_BACKUP=true` 来跳过自动备份。

该选项存在风险——一旦升级失败，将无备份可用以恢复状态。

## 2026年1月6日

**v0.2.7 升级提案：创世验证者增强机制进入治理投票阶段**

一项与“创世验证者增强机制”相关的链上治理提案已发布，现开放投票。

近期网络快速增长引发了一系列挑战。过去几天中，网络经历了多次异常情况，部分问题疑似由蓄意干扰或压力攻击所致。本提案旨在通过一系列临时措施，提升网络在高负载和异常条件下的稳定性。

“创世验证者增强机制”最初在网络启动初期作为临时防御机制引入，并在前两个月运行。本次提案旨在根据当前网络状况，临时重新激活该已有机制，并启用部分新增保护措施。

**主要变更**

**创世验证者增强机制（临时）**

- 临时重新激活“创世验证者增强机制”——此前已使用过的限时防御机制，现提议重新启用。
...
本次升级将此前通过手动 API 更新部署并已在网络中运行的关键修复正式纳入协议层，确保全网行为一致。这些修复包括：

- 修正失败推理请求的会计核算问题（包括对不支持格式的请求被处理但未标记为完成的情况）
- 提升失败推理处理的容错能力
- 引入 `PoCBatch` 和 `PoCValidation` 交易的批量处理机制

通过将其纳入协议，这些行为将成为全网统一的协议规则。

**临时参与与执行限制**

- **节点注册限制**：新节点（Host）注册将在区块 2,222,222 前暂停（约两周后）。此举旨在稳定网络，为后续增长做准备。
- **开发者注册限制**：在稳定期间，新开发者地址注册将暂停。立即生效一个预定义的开发者地址**白名单**。仅白名单内的开发者地址可在该期间执行推理任务。所有针对开发者地址的限制（包括注册与推理执行）将持续至区块 2,294,222（约19天后）。

**由治理控制的机制**

本次升级包含的预备性变更，将使未来可通过治理投票控制参与者准入和推理执行，而无需再次进行软件升级。但本次提案**不启用**任何此类治理控制约束，需后续另行投票决定。

**第117轮奖励发放**

本提案涉及因链暂停导致的第117轮奖励补发：

- 在第117轮中活跃但未收到奖励的节点，将补发该轮应得奖励。
- 所有在第117轮中活跃的节点，将额外获得相当于 **1.083 倍** 第117轮奖励的统一补贴，适用于所有符合条件的节点（包括已领取原奖励的节点）。

**关于持续时间与执行说明**

本次升级重新激活或引入的所有保护机制均为临时措施，**无需**手动治理干预即可自动解除。
...
如果您是主机，请审阅提案并进行投票。  
在升级窗口期间保持在线，以便在出现问题时及时跟进操作指引。

**注意 2**

Cosmovisor 每次执行更新时，都会在 `.inference/data` 目录中创建一个完整的状态备份，请确保您的磁盘有足够的可用空间。  
...
如果您的 `application.db` 占用大量空间，可参考[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)的方法进行清理。
...
在许多情况下，这是由于一个漏洞导致的：格式不支持的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将修复此问题。

参考链接：[https://github.com/gonka-ai/gonka/pull/517](https://github.com/gonka-ai/gonka/pull/517)

此 `API` 版本增强了对失败推理处理的容错能力，并减少了推理遗漏计数问题。同时引入了 PoCBatch 和 PoCValidation 交易的批量处理功能。
