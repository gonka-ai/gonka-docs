...
!!! note "关于本页面"
    ...
    本页面不保证内容完整。如需获取最新信息，包括治理投票的发布及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。

## 2026年9月25日

**提案 108 已开放投票：DevShard v5.0.2**

修复 DevShard 节点故障的重要更新。同时退役 v3 和 v4。

投票将于 **2026年9月27日 00:03 UTC** 结束。

**修复内容**

每个运行的 DevShard 版本都会打开一个大小等于主机 CPU 数量的 Postgres 连接池。在多核机器上运行多个版本的主机可能会耗尽所有可用连接，导致节点失败。v5.0.2 将负载连接池限制为会话限制（`PG_POOL_MAX_CONNS`，默认值为 4）。

此外，它停止了在每次心跳时叠加推理超时计时器，在轮换后立即开启下一次心跳，并记录持有验证租约的进程，仅允许该进程完成或释放租约。

**链上变更**

`approved_versions` 变更为：
...
v3 和普通 v4 已被移除。主机无需任何操作：`versiond` 会自动获取新二进制文件，验证其 sha256 并运行。
...
完整技术范围：[PR 1840](https://github.com/gonka-ai/gonka/pull/1840)

## 2026年9月22日

**升级 v0.2.16 的拉取请求评审**

下一个链上软件升级 v0.2.16 的拉取请求已开放评审：[https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)。有意义的评审贡献，包括重要评论、错误发现和安全问题，可能在下一个升级周期中获得社区奖金。

主网链/API 工作重点包括可信权重、动态系数、交易费用组和 PoC 挑战。若提案获批，治理、BLS 和 PoC 验证权将仅限于上一个纪元确认的算力，而新的算力仍立即获得奖励。治理可为每个模型设置目标份额和系数范围。升级时这些范围保持关闭，因此系数和奖励不变。新增消息组和气体规则，所有组默认禁用，协议仍不收取费用。获准的挑战者可将一个活跃主机从推理任务中移出，转为全容量 PoC，直至下一次常规 PoC。

**关键变更**

1. **可信权重。** 在 v0.2.16 之前，声明算力的突然增加会立即提升参与者在治理、BLS 和 PoC 验证中的权力。这使得未经确认的算力影响共识和其他参与者的验证。本升级将该权力限制为上一个纪元已确认的算力。新参与者或回归参与者从 0 开始，若确认失败，则下一个基准设为 0。新的算力仍立即获得奖励。仅对信任敏感的权力需等待一个确认纪元。
2. **动态系数 v1。** 网络需要每个模型的可预测吞吐量，以便用户依赖其可用性。每个模型的目标吞吐量应遵循预期需求。使用固定系数时，整个硬件类别往往转向同一模型，导致每模型容量难以平衡和预测。本升级允许治理为每个模型设置网络算力的目标百分比和系数范围。协议将动态调整该范围内的系数，使算力趋向目标。超过目标的算力按最小系数评分，以抑制过度供给。治理初始目标基于需求估算和数据源（如 OpenRouter）。后续版本可聚合主机估算，最终使用链上模型使用数据。本版本仅改变激励机制，不会自动切换主机部署的模型。升级时，最小值和最大值均等于当前比例，因此系数和奖励在治理打开范围前保持不变。
3. **费用。** 历史上，协议从未收取交易费用。因此攻击者可以以极低成本提交大量消息，而每个验证者却需承担处理和存储成本。本升级将交易类型分组，并添加每消息气体规则，使每组可单独定价。所有组默认禁用且不收费。治理可稍后启用并定价。升级提案信息也可在升级高度启用组。
4. **PoC 挑战。** PoC 和随机确认 PoC 证明主机声明的算力。在纪元其余时间，推理统计检查该硬件是否用于协议分配的工作。高频率的遗漏或无效推理可移除主机。这提供了强大的持续检查，但其敏感性取决于推理量，可能无法揭示声明算力与可用算力之间的所有差距。PoC 挑战为此类情况增加了额外的安全层。获准的挑战者可要求一个活跃主机停止推理，转为全容量 PoC，直至下一次常规 PoC。为发起挑战，挑战者需锁定目标剩余纪元奖励的一部分作为保证金。若目标通过，获得保证金；若失败，挑战者退还保证金，目标遭受与失败确认 PoC 相同的惩罚。挑战期间的推理遗漏不计入目标。仅允许列表中的 devshard 托管创建者可发起挑战。若允许列表为空，则任何人都可发起挑战。
更多详细信息及其他变更见：[https://github.com/gonka-ai/gonka/pull/1535 ](https://github.com/gonka-ai/gonka/pull/1535 )

**行动项**

1. **所有人。** 现在 —— 评审 PR。阅读 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)，对任何发现、问题、改进建议、边缘情况或漏洞留下评论。
2. **主机。** 投票前无需其他操作。请勿更新容器、重新授权密钥或为此升级移动模型。
...
主要协议新增功能是高度同步，为 DevShard 会话提供已签名、可重播的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立协议级逻辑时间。该版本还包括高可用性改进、热切换和会话恢复、Gateway 更新、验证与结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机需要做什么**

无需操作。`versiond`会下载新的运行时，验证其sha256，并与已运行的版本并行提供服务。`v3`、`v4`和`v4.1`仍保持批准状态，因此现有会话和路由前缀不受影响。这是一个运行时版本，而非网关发布：任何网关变动将另行公告并提供独立指南。

**奖励**

该提案还从社区销售合约中向多个开发周期中作出贡献的核心贡献者分配91,300 USDT：@a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。所有 payouts 已在链上执行。

技术范围、个人贡献、PR 引用及完整奖励明细：[PR 1584](https://github.com/gonka-ai/gonka/pull/1584)

## 2026年9月16日

**devshard v5 提案重新进入治理（提案104的重新提交）**

提案ID 105 将 `v5` 添加到已批准的 devshard 版本列表中，并从社区销售合约中分配91,300 USDT作为贡献者奖励。

**Devshard v5**

这仅是 devshard 的变更。Cosmovisor 未涉及，链不会停止，推理服务持续运行。若获批准，`versiond`将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，并启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能是高度同步（height-sync），它为 devshard 会话提供了一个经过签名、可重放的主网高度视图，并建立了协议级别的逻辑时间，用于未来的 cPoC、超时和争议机制以及协议级 QoS。本次发布还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。
...
投票将于 **2026年9月18日16:47 UTC** 结束。提案通过需满足：权重25%的参与率、多数赞成、反对票低于33.4%且无否决权。

请投票。上一次尝试因参与率不足而失败，无人反对，因此参与是唯一阻碍其通过的障碍。

提案详情和投票可通过 `inferenced` 进行：
...
```
如果您没有直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。

## 2026年9月18日

**提案被拒绝：批准 devshard v5 及奖励发放**

提案ID为104的链上治理投票已结束。该提案因 `failed_reason`：`proposal did not get enough votes to pass` 而被拒绝。

最终计票：赞成 25291，反对 0，否决 0，弃权 0。

无人投反对票。参与率约为网络权重的6%，而法定人数为25%，因此该提案因参与度不足而失败。

## 2026年9月16日

**GLM-5.3-Flash 已上线**

自第395个纪元起，`zai-org/GLM-5.3-Flash` 已成为活跃模型组，并在主网上提供推理服务。

如果您希望托管它，欢迎加入。在部署窗口期间声明对该模型的意向并配置您的MLNode，该窗口在下一次PoC开始前500个区块开启。

**别忘了您的参与选择**

GLM-5.3-Flash 的不参与惩罚自第394个纪元起生效。对于每个经治理批准的模型，您必须明确选择：自行提供、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的15%，且该惩罚将持续至您做出选择为止。
...
提案ID 104 有两项内容：将 `v5` 添加到已批准的 devshard 版本列表中，并从社区销售合约分配91,300 USDT作为贡献者奖励。

**Devshard v5**

这是一项仅限 devshard 的变更。它通过链上参数提案进行，而非完整的链软件升级：无需 Cosmovisor，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。提案通过本身不会迁移任何流量，因此经纪人可随后逐步迁移其网关。
...
该提案还向七位受赠方发放总计91,300 USDT的贡献者奖励：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重占比25%的出席率、多数赞成票，且反对票（含否决）少于33.4%。
...
**提案通过：报销 Daniil 和 David Liberman 参加 All-In Summit 2026 的费用**

提案 ID 103 的链上治理投票已结束。

该提案已获批准。从社区资金池中拨款 15,500 USDT，用于报销 Daniil 和 David Liberman（Gonka 协议的联合创始人）参加 All-In Summit 2026 的两张参会通行证。

峰会于 2026 年 9 月 13-15 日在洛杉矶举行。标准参会通行证每人 7,500 USDT，门票已支付。本次报销涵盖两张门票及提款交易费用。本提案未申请报销机票、住宿及其他费用。
...
**提案通过：批准 Quant Mesh Limited 为 Ledger 集成合作方并拨付初始 Ledger 集成款项**

提案 ID 102 的链上治理投票已结束。

该提案已获批准。Quant Mesh Limited 被正式批准为 Gonka-Ledger 集成的法律签约方，且从社区资金池中拨款 350,350 USDT 至 `gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

拨款涵盖第一阶段启动费用 275,000 USDT、首年年度激活费 75,000 USDT 以及 350 USDT 的处理费用。授权与资金拨付严格限于完整提案中描述的 Ledger 集成。
...
提案 ID 101 的链上治理投票已结束。

该提案已获批准。`zai-org/GLM-5.3-Flash` 现已作为治理批准的模型纳入 PoC 模型集合，而 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8` 已从 `poc_params.models` 中移除。这两个 ID 仍保留在 `GET /v1/governance/models`（推理目录）中，但不再是 PoC 模型。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：其 PoC 参数和权重保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，并联合提交。测量数据及阈值设定的依据详见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起，`penalty_start_epoch` 生效，因此未明确选择的主机将面临不参与惩罚。如果您计划提供该模型，尽早声明意向将使团队有最佳机会组建。主机指南：[GLM-5.3-Flash 启动](./host/glm-bootstrap.md)。
...
提案 ID 100 的链上治理投票已结束。

该提案已获批准，`v4.1` 已添加至批准的 devshard 版本列表中。`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，并启动运行时。无需手动主机操作。

v4.1 将与现有的 v3 和 v4 运行时并行运行，因此当前流量将继续被服务，代理可逐步迁移其网关。
...
提案 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加至 PoC 模型集合中。同时将其从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。
...
投票将于 **2026 年 9 月 10 日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票参与率、多数赞成、且带否决的反对票少于 33.4%。
...
**devshard v4.1 运行时提案已进入治理阶段**

提案编号 100 将 `v4.1` 添加到 devshard 版本的批准列表中。

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：Cosmovisor 不参与，链不会停止，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。提案通过本身不会迁移任何流量，因此经纪人可在之后逐步迁移其网关。
...
投票将于 **2026年9月10日 22:36 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成票，且反对并行使否决权的票数少于 33.4%。
...
**提案通过：支持 Gonka 参加 TOKEN2049 新加坡**

提案编号 99 的链上治理投票已结束。

该提案已获批准。它将从社区池中拨出 100,000 USDT，用于支持 Gonka 参加 2026 年 10 月 7-8 日在滨海湾金沙举行的 TOKEN2049 新加坡活动。资金涵盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
重启时，`v4.0.1` 从 nonce 1 恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 随后在 60 秒就绪超时后终止子进程，导致客户端看到 HTTP 502 错误。

`v4.0.2` 加载最新快照，并仅恢复其后的差异。此版本与之前的 v4 版本完全兼容。

发布：[devshard/v4.0.2](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v4.0.2)

**更新说明**

一次更新一个节点，并在继续下一个节点前确认当前节点已恢复正常。
...
该提案已获批准。它将 `deepseek-ai/DeepSeek-V4-Flash-0731` 的 `weight_scale_factor` 从 0.214 提高至 0.246。所有其他模型和链参数保持不变。这是提案 97 的重新提交。链上提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，原因是 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的法定人数）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 因子源于被高估的 nonce/min 数值。测量修正见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需执行任何操作。

## 2026年8月25日

**用于仅 devshard 升级的 PR 现已开放供审查**
...
1. v5 的主要目的是高度同步：每个主机和排序器在保证金日志中维护一个已签名、可重放的主网高度视图，即使在没有推理流量时也是如此。通过该时钟，devshard 现在可以证明地知道 cPoC 是否正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议添加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前携带时间戳字段的消息中。后续版本可以将诸如 `confirmed_at` 和 `started_at` 之类的壁钟字段替换为高度标记。这解决了时间提前攻击问题，并为在协议层面处理流超时打开了道路。本版本包含这些标记；尚未将超时和封印决策切换到它们。

3. 通过所有插槽的往返心跳已添加，因此安静的会话仍能定期同步高度，而繁忙的会话则可通过常规推理标记实现相同的节奏。
...
**提案 97 已开放投票：将 DeepSeek V4 Flash 的 weight_scale_factor 提高到 0.246**

一个新的链上治理提案（提案编号 97）已开放投票。

**作用**

将 `weight_scale_factor` 的 `deepseek-ai/DeepSeek-V4-Flash-0731` 从 0.214 提高到 0.246。所有其他模型和链参数保持不变。

**投票**

投票将于 **UTC 时间 8 月 27 日 03:47** 结束。如有可能，请投票。
...
**提案通过：稳定小样本中周期内无效 SPRT**

提案编号 96 的链上治理投票已结束。

该提案已获批准。它调整了小样本中无效推理检查的行为，避免在周期初期数据不足时误触发。主机无需采取任何操作。

## 2026 年 8 月 20 日

**提案 96 已开放投票：稳定小样本中周期内无效 SPRT**

一个新的链上治理提案（提案编号 96）已开放投票。

**作用**

在周期初期，数据尚不足，因此无效推理检查（SPRT）可能在样本足够大之前仅因少数无效项而触发。本提案使其稳定：

- 将 `invalidation_h_threshold` 提高到 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会受到惩罚。真实违规者仍会触发它（约 360 个样本时为 18%，1650 个样本时为 12%）。
- 小样本无法触发它：至少需要 32 个被标记的无效项，且在 500 个样本时观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个加速提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时）结束。如有可能，请投票。如果未能在时限内达到加速阈值，不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。如果转为常规提案，您需要重新投出您的选票。
...
作为协议开发的重要里程碑，链已准备好逐步启用 devshard 结算，以提升网络的安全性和可靠性。

一个稳定的网关版本目前正运行在一个网关上，并将用于第一阶段的结算。其统计信息（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将被链上使用。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何处罚或罚没，但仍应作为协议开发过程的一部分进行调查。
...
**下一步：****1) 未来 24 小时。** 主机应检查其统计数据并解决任何关键错误或配置问题。如有需要，贡献者可协助调查问题。

**2) 在此 24 小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，先从小模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 接下来的 1-3 天。** 将监控结算结果

**4) 一旦该阶段稳定。** 其他经纪人可以部署稳定的网关版本并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至 100%。

该 rollout 有意逐步进行，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 已在 Gonka 网络上激活**

在 [提案 94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元 360 起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一起作为活跃模型组加入，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后由 [提案 98](#august-29-2026) 提高至 0.246）转换为共识权重。

针对 DeepSeek 的按模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需再做任何操作——现有设置将继续有效。尚未做出选择的主机应尽快选择，以避免每纪元的惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。

## 2026年8月14日

**MLNode 3.0.16 发布**

如果您的所有节点都已使用新版本（3.0.16），则无需操作。

如果仍有节点运行旧版 MLNode（即正在服务 Kimi 或 MiniMax 的节点，因为任何已运行 DeepSeek 的节点均已使用 3.0.16），请在方便时将这些节点升级到 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新几个节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。

MLNode 镜像已在 [`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 的 `vllm-0.25.1-upgrade` 分支上固定为 3.0.16。

默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是同一 MLNode 3.0.16，只是为 CUDA 12.9 构建。两个标签均在同一个 compose 文件中。
...
将正在运行的社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并为未来的实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常操作。
创世守护者预计会支持该提案。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望使用其他密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查为 epoch 360 启动快照——在此前请保持委托分散，以确保快照有效。
...
在区块 **5,551,615** 前，提交委托（有助于启动可达性）或拒绝。从 epoch **360** 开始，DeepSeek `penalty_start_epoch` 将激活——未明确选择的主机将面临不参与惩罚。
...
委托主机：选择**非守护者**的 DeepSeek 意向主机，并将权重分散到独立目标——参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请勿忘记委托，以避免不参与罚金。

提醒：明天将上线另一个带有更好日志的MLNode构建版本，敬请留意后续简短通知。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359开始前约500个区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。此窗口内切换是安全的，因为其中无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可使用以fp8 + nvfp4重新打包的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署该模型前，将发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；请将委托分散至独立的DeepSeek主机。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第360轮之前无需操作。从第360轮起，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的不参与罚金。

区块高度是精确的；时间仅为估算值，将在接近轮次时进一步确认。[更多关于DeepSeek启动的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案在网络上新增DeepSeek V4 Flash作为新的PoC模型。现有参数或模型均未修改，仅新增该模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 要求：MLNode运行vLLM 0.25.1并使用发布候选节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
基于模型使用情况，社区未来可决定逐步用DeepSeek替代Kimi或MiniMax（例如，提高系数以激励所有B系列GPU转向DeepSeek）。
...
2. 资格预快照：链对意向/委托进行快照并发出建议性预资格事件。
3. 部署窗口：已声明意向的主机配置其MLNode。
4. PoC开始：成员资格由在PoC开始时提交PoC存储提交的人决定，而不是由声明意向的人决定。

最终的MLNode版本和设置说明将在投票结束后发布。

有关引导过程的更多信息：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留当前模型：在第360个纪元之前无需任何操作；从第360个纪元起，如果您未提供该模型，请为DeepSeek提交`PoCDelegation`或`PoCRefusal`。
3. 在截止日期前对提案94进行投票。
...
- PoC 种子稳定性。在纪元边界延迟后恢复遗漏的种子提交，并在暂时性故障（如签名者不可用）后重试本地种子恢复，以防止验证者被归零。
- API。
    - `/v1/versions` 现在返回增强的 DAPI 版本信息，包括 ML 节点详情
    - 默认的 Node Manager gRPC 端口现已设为 9400，确保与自定义配置兼容
...
这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更内容：纪元追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪保持一致。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整链软件升级。若获通过，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动操作主机。

**如何投票**

如果您没有直接访问具有投票权密钥的权限，或希望由其他密钥代为投票，请参阅有关将冷密钥的治理投票权限授予热密钥的指南。
...
该提案已获通过，并于主网区块 5316315 成功执行。同一提案也批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。

## 2026年7月29日

**升级 v0.2.15 和 devshard v4：预下载二进制文件****devshard v4 和 v0.2.15** 升级提案的链上治理流程即将结束。
...
2. 一个 **v0.2.15 主网软件升级**，移除了大量过时代码、遗留的交易处理路径及相关组件，这些组件已不再符合当前架构的需求。
...
v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个面向多实例高可用（HA）的版本：在共享 Postgres 上，通过 versiond-router 后部署 N 个 versioned / devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，以确保正在进行的工作（包括 SSE）在旧版本上完成。
...
该提案同时在 `DevshardEscrowParams.approved_versions` 中注册 devshard v4。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上的 Cosmovisor 备份 `application.db` 通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
devshard 部分正在为 v3 运行时做准备，以便在链升级期间，代理无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器稳定，请提前按照[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成上述两步，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者——本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 如果获得批准，升级后 devshard 存储可选地由共享的 Postgres 实例支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放评审**

devshard 升级独立于主区块链进行。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上，通过 versiond-router 后部署 N 个 versioned/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，使正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。

**行动项**

请审阅 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现任何问题、疑问、改进建议、边缘情况或潜在漏洞时留下评论。

有意义的评审贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一升级周期的社区奖励。

本次仅为 PR 审阅请求，不启动正式投票。
...
```
* 在第331个周期PoC开始前的约500个区块窗口内（区块 **5,105,776** — 2026年7月17日12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
* 委托主机：选择非守护者Kimi主机，并将权重分散到独立目标上 — 请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。

**devshard v1/v2已移除**

`versiond`已停止v1和v2的`devshardd`进程。所有网关流量必须使用`/devshard/v3`。如果您的网关在此次变更后停止提供推理服务，说明您仍在通过已移除的路径路由 — 请遵循v3迁移指南。
...
2. 提供Kimi服务的主机：在投票结束**后**、区块 **5,105,276**（2026年7月17日，约12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331个周期PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间委托时：**不要委托给守护者节点**；将委托分散到独立的Kimi主机上。请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取更新的委托指导。
4. 经纪人 / devshard创建者：如果您的任何网关流量仍在v1/v2或`/v1/devshard`上 —— 请在投票结束前切换到`/devshard/v3`。
...
- 第331周期PoC开始：区块 **5,105,776** — 2026年7月17日12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6`在第328至329周期失去了PoC验证多数（详见[事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式 —— 与6月提案78采用的恢复路径相同。

该提案将在下一次PoC前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将从第331周期开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票 —— 紧急窗口时间很短。
2. 提供Kimi服务的主机：保持您的Kimi设置就绪，并准备好在引导PoC时切换您的MLNode。
...
投出您的选票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced`从v0.2.13或更高版本起支持。
```
export NODE_URL=https://node3.gonka.ai/
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（紧急，0.667 赞成阈值；出勤率重要，请尽快投票）。
* 引导意图截止日期和纪元时间详见引导说明。
...
两台运行 Kimi 的监护人操作服务器同时失效：

* 其中一台服务器上运行 Kimi 的 MLNode 因供应商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其失效一次性移除了这些投票。
* 第二台服务器的网络节点因内存不足而断开。

Kimi 的验证投票一直接近 2/3 阈值，因此此次失效足以使该组低于多数。监护人票决机制未启动：当前未计入投票权重的监护人会被排除在票决机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况类似于纪元 306–307 的事件。纪元 329 开始时的投票确切细节仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个纪元）重新添加：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的独立公告。
2. [明天] Kimi 将再次通过标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布说明。

**委托指导变更——请阅读**

先前的指导（包括 2026 年 6 月 27 日的引导公告）建议将监护人节点作为委托目标。此建议现已撤销：

* **请勿委托给监护人节点。** 监护人是 PoC 验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请向您自己运行模型的节点委托。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在进行中的修复*** PoC 验证中的监护人投票（监护人始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完成迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非监护人委托方；尽可能将权重分散到多个独立主机。
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这将修复内存增长问题，并在移除 v1/v2 前必须完成。
...
这是一个仅限 devshard 的运行时升级。它独立于完整链软件升级，无需链二进制升级。

提案批准后，v3 现在与现有 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换至 v3。
...
获准的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。同时改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。如果链上提案获批，`versiond` 将自动下载二进制文件，验证 sha256 哈希值，并在现有 `versiond` 容器内启动一个额外的 `devshardd` 进程。
...
devshard 部分准备了 v3 运行时，使代理能够在链升级期间无需依赖已弃用的经典 API 路径提供推理服务，同时提升 RAM 利用率并支持 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
在此分支上将单独发布 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供服务。

**提议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查此提案。
2. devshard v3 发布版本将在主网链升级前提出并部署。
...
2. 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据指南将 `api` 二进制文件和桥接器镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，您只需更新桥接器镜像并重启桥接容器。如果您已完成这两步，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复版本 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可以选择委托给该节点。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定使得在8xB300硬件上，Kimi 的共识权重与主模型 MiniMax 大致相当。该设定并非偏向任何硬件——MiniMax 上已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定使得在8xB300硬件上，GLM-5.2 的共识权重与 MiniMax 大致相当。该设定并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的增益。
...
1. **声明意向。** 计划提供 Kimi 或 GLM-5.2 服务的主机需在 `start_poc - deploy_window` 的引导快照前为该模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作者可在配置资源前确认模型的可行性。
...
从第309轮开始（PoC 开始时间约为6月26日15:25 UTC；生效时间约为16:00 UTC），前提是各模型满足引导资格条件：
...
该提案必须在第308轮结束前完成，以便两个模型都能在第309轮中引导。由于每轮约22.8小时，标准的48小时投票周期无法容纳在单轮内；而12小时的紧急投票周期可以。创世守护节点预计支持本提案。

**主机需采取的行动**

1. **要提供 Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 中将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口期，此期间切换是安全的，因为该窗口期内无 cPoC。

2. **要部署GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意图并 provision 您的MLNode。部署是可选的——未选择的主机不会受到任何惩罚。
...
如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`查看：任何活跃节点均可使用：
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换前都不会获得本纪元的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi配置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308纪元期间将进行一次投票以恢复Kimi。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，以便主机可自愿选择参与，提前衡量模型需求，不提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票以通过PoC验证。因此，提供Kimi的参与者被移出群体，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快恢复服务且最小化停机时间的途径。下一纪元将进行一次投票以恢复Kimi。
* **Qwen3-235B——退役（独立且无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已提出的阵容调整。它与Kimi的情况无关，仅因同属PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在epoch 307结束前完成，因此这些更改被合并为一次紧急投票。每个epoch约为22.9小时，标准的48小时投票周期无法容纳在一个epoch内；而12小时的紧急周期可以。Genesis守护者应支持该提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如果尚未就绪），以避免在epoch边界时遭遇Hugging Face速率限制。
2. **请在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——epoch 308将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— epoch 308将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，且**不设置不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：
...
- 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每插槽总数。
...
4. 将每个推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每插槽总数
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。若链上提案获得批准，`versiond`将自动下载二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。
...
MiniMax 的按模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会对其全部权重遭受每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅适用于您共识权重中由 Kimi 导出的部分**；您的 Qwen 导出权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降至约 25%，并将链上投票门槛设为 0.25。如果守护者弃权，非守护者需在剩余 75% 中达到约 1/3 的参与率才能满足门槛（参见 inference-chain 部分）。
...
- 将 `MiniMaxAI/MiniMax-M2.7` 添加为治理批准模型和 PoC 模型。
- 更新推理验证阈值：
    - Qwen 235B：`0.940`
    - Kimi K2.6：`0.900`
    - MiniMax-M2.7：`0.922`
- 在 vLLM 0.20.1 发布后，根据 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`（不变）
    - Kimi K2.6：`0.78`（从 1.26 下调，与相同 PoC 权重相比，Kimi 的每纪元共识权重约减少 38%）
    - MiniMax-M2.7：`0.3024`
...
- 将 devshard 随机数上限从 `20_000` 提高至 `1_000_000`。
- 将每纪元最大 devshards 数量从 `100` 提高至 `500_000`。
- 修复新模型引导期间确认 PoC 奖励核算。
- 在本次升级纪元的剩余时间内禁用确认 PoC，以便新快照逻辑从下一个纪元干净启动。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

主机需更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先遵循加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期其余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最后3个周期）。
...
第266轮的问题是由攻击导致的，该攻击影响了运行Kimi模型的主机。网络继续运行，但该攻击与若干相关条件共同导致许多参与者未能进入第266轮。

在应用额外防护措施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。
...
在相关防护措施应用期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。
...
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方恢复，预计将通过社区代理和中介端点逐步恢复。
...
似乎PoC投票未能收集到本轮所需的足够票数。确切原因尚未确认，社区正在积极调查此情况。
...
社区成员目前正在审查第266轮期间发生的情况。一旦对根本原因有更多明确信息，将及时更新。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 现已在链上开放投票。

- 包括：Kimi（`0.78`）权重重新校准、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及若干PoC/奖励修复和以太坊桥接主网激活。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不投票的情况下，其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
作为即将进行的 v0.2.13 升级的准备工作，该升级可能包括以太坊侧合约激活，所有主机均需验证其桥接容器已部署、运行最新版本并同步正确。

部分主机可能已部署桥接容器。在这种情况下，请先检查您是否运行的是当前版本，然后再采取任何进一步操作。
...
本次仅为拉取请求的审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始，很可能在明天。
...
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束期间，确认 PoC 触发器将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用了 v0.2.10 的宽限期原语。
...
- Devshard 现在支持更大部署的共享存储选项。它可以使用 Postgres 作为主存储，SQLite 仍作为本地备用。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能启动延迟。这延迟了 api 激活，导致部分矿工因延迟跳过确认 PoC。

修复方案通过并行加载 devshards 并从快照恢复现有 devshard 会话来移除该阻塞。

[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)

请更新 api 容器的二进制文件。每个 PoC 开始前有 500 个区块的无 CPoC（`confirmation_poc_safety_window`）窗口，因此这可能是部署的最安全版本。
...
在上一个纪元中，发现Kimi-K2.6在解析某些响应时存在一个轻微的bug。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来的纪元中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
对于多模型PoC，这意味着Kimi现在作为独立的模型组拥有自己的参与和奖励追踪。
...
提交了PoCIntent的主机应在PoC于区块`3874496`开始前，将至少一个MLNode从`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`切换至`moonshotai/Kimi-K2.6`。

预评估与PoC开始之间有500个区块的窗口期。在此期间无CPoC任务，因此声明了意向的主机可以安全地将模型节点切换至`Kimi K2.6`。
...
这标志着网络的一个重要里程碑：`devshard`已上线并具备生产就绪能力。`node4`是今后推荐的公共网关。

**需执行操作**：请将您的端点更新为`node4`。

## 2026年5月2日

今日的预资格验证未通过，对于`PoCIntent`权重低于30%的主机。请保持您的MLNode使用`Qwen235B`，并于明天为下一个纪元提交意向。
...
- **`moonshotai/Kimi-K2.6`作为第二个模型引入**：该模型组将在升级后两个纪元激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200，8xB200）上的模型计算复杂度。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
有关这些更改的更多详细信息，请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
为解决此问题，以下脚本会查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个包含清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。其余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
- **协议加固**。对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间验证。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、将遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
更新修复了 PoC 开始附近的一个边缘情况，此前在特定条件下可能导致性能下降。
...
此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理可扩展性。
...
建议主机审阅 [GitHub](https://github.com/gonka-ai/gonka/pull/813) 上的提案并参与投票。
提前下载二进制文件有助于避免在升级窗口期间依赖 GitHub 的可用性。
...
目前，通过每笔推理的链上交易处理推理会限制吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本供主网审查和有限生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，这使得每个区块可容纳多达 100 倍的推理次数，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中可能同时启动剪枝活动，造成可避免的操作压力。
...
这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，不是因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在 `v0.2.10` 中发现潜在漏洞。为降低当前升级前时期的潜在风险，建议在下一次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
如果您无权访问持有投票权的冷钱包密钥，则建议提前安排投票委托。请与该密钥的所有者联系，请求授权您代表其投票。若无此授权，其他账户无法提交投票。
...
现已提供新的 vLLM / mlnode 构建，可在PoC开始时中断正在进行的推理请求，以降低PoC开始时仍处于活跃状态的请求可能导致权重下降的风险。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选定。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
针对升级提案v0.2.10的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提高网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **按实际时间归一化 PoC 权重**
    ...
    此升级通过实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并使权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入了一个为期 3000 个区块的宽限期，期间不进行确认 PoC，并对升级时期的错过率和无效率阈值采取较宽松的标准。
...
这意味着，在单个周期内，即使被惩罚，矿工的抵押品损失也不会超过 0.5%。所需抵押品仅占日奖励的约 24%。

**警告：** 抵押参数将无视投票结果生效。如果本提案未通过，创世时定义的抵押参数将在第 180 个周期自动激活，而非上述参数。

投票结束后、第 180 个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 个周期开始，其奖励将被削减 5 倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性与可扩展性。
...
此升级通过实际PoC耗时标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。

**启用Qwen235B工具**

此升级为`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`添加了工具调用参数（`--enable-auto-tool-choice`、`--tool-call-parser hermes`），并设置验证阈值`0.958`。
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级时段内放宽了漏报率和无效率阈值。
...
如AMA期间讨论并基于社区成员提出的论点，建议初始抵押要求较低， slashing 也最小化。

将提交给社区投票的参数：

- 每1单位算力需0.032 GNK（约每H100需10 GNK）
- 漏报率或封禁的slashing为0.01%
- 无效推理的slashing为0.5%

这意味着，在单个周期内，即使被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押品仅占每日奖励的约24%。

提案提交投票后将另行公告。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下适用。如果稳定性得到验证，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您的节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止运行。这是因为节点运行的是不再与网络兼容的旧版二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
在当前周期中观察到cPoC（确认PoC）与PoC之间的重叠。在该周期的最后一个区块之前，`is_confirmation_poc_active`被观测为`true`。
...
链上推理访问现已开放，不再仅限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
本次仅为呼吁审查拉取请求，并非正式投票的开始。治理投票流程将在审查期结束后启动。
...
此升级通过实际 PoC 已用时间对参与者权重进行标准化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
为授予在v0.2.9升级后创建的暖密钥权限，应使用[CLI版本v0.2.9](https://github.com/gonka-ai/gonka/releases/tag/release%2Fv0.2.9)。
...
- 观测到的变化表明，GPU权重差异现在反映的是模型特定的推理吞吐量，而非名义上的硬件规格。例如，H100 PCIe权重的下降幅度大于H100 HBM3权重，这与`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`的观测推理行为一致。
- 在当前模型配置下，基于观测到的推理轨迹，B200 GPU的推理性能并未高于H100类GPU。
- 如果未来轮次通过治理引入更大或更复杂的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准vLLM推理对同一模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`进行的控制推理基准测试，显示了与PoC v2中观察到的相同GPU类型间的相对性能差异。

**跟踪器（仪表板）维护者操作**

在更新的权重分配生效后，跟踪器（仪表板）维护者可能希望审查第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。
...
- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时应用任何后续步骤或缓解指令。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不包含在下一纪元的参与者集合中。
...
在因cPoC惩罚而减少或排除奖励的情况下，未计入部分将转移至社区池。此前，此类奖励会重新分配给其他参与者。
...
在上一个纪元中，大量ML节点未获得PoC权重。
分析表明，这是由于PoC验证参与不足所致。在多个案例中，参与者发布了nonce，但未执行验证，或验证级别远低于协议要求。
下表列出了上一纪元具有权重、在当前纪元提交了PoC nonce，但未参与或未充分参与PoC验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)

他们的总权重约为36%。加上完全未参与PoC的参与者，PoC验证中无或低参与度的总权重达到约48%，这一比例过高。
如果您的节点在 `validated` 列中显示为0，请检查您的PoC验证日志和配置，确保验证正常运行。
...
此升级实现了PoC v2架构，简化了模型支持，并应用了关键的安全性和可靠性修复。

**当前生效的关键变更****PoC v2核心集成**

- vLLM集成：PoC直接集成到vLLM中，无需卸载模型即可立即从推理切换到PoC。
- MMR承诺：使用Merkle Mountain Range承诺将工件存储移至链下；仅 `root_hash` 和 `count` 记录在链上。
- 双模式迁移：支持V1（常规PoC）和V2（确认PoC）跟踪。
...
- SSRF & DoS：验证 `InferenceUrl` 以拒绝内部IP，并添加超时以防止请求挂起。
- 投票翻转：拒绝重复的PoC验证以防止覆盖。
- 身份验证绕过：将 `epochId` 绑定到签名，以验证是否属于正确的纪元。

**PoC v2参与资格的主机要求**

参与PoC v2的资格要求主机完成以下事项：

- 模型配置：配置ML节点以提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- ML节点升级：使用支持PoC v2的版本：
    - ghcr.io/product-science/mlnode:3.0.12
    - ghcr.io/product-science/mlnode:3.0.12-blackwell

!!! note 
    一旦网络过渡到单模型配置，未能满足上述两项条件的节点将失去参与PoC v2的资格。PoC v2的权重分配过渡仍受观察性采用阈值和后续治理的约束。

    **维护与操作**

- Cosmovisor：节点和API二进制文件的更新将自动处理。现有主机无需对正在运行的容器执行手动更新。
- 磁盘空间：Cosmovisor会在`.inference/data`目录中创建完整的状态备份。请确保有250 GB以上的可用空间。
- Postgres：现在可以通过Postgres配置升级后本地有效负载存储。
...
本指南说明主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来更新其ML节点。ML节点配置需符合PoC v2要求，自第155个周期起生效。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估是否具备向 PoC v2 过渡的条件。一旦观察到约 50% 的活跃 Host 达成足够采用率，可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
- 认证绕过与重放：将 epochId 绑定到签名，并验证授权是否针对正确的 epoch。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不再被视为 PoC v2 参与资格有效。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
在延长确认后，请利用这段时间为新的 PoC 要求做好您的设置准备。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步进行更新。

**投票范围**

若获得批准，网络将暂时继续在现有的 `allowlist`（包括未表现出非标准硬件行为的主机）下运行。

开发者 `allowlist` 将以相同的偏移量延长，并持续生效至区块 2459375。

未包含在 `allowlist` 中的主机在延长的稳定期内仍将无法参与 PoC，该阶段将在区块 2443558 结束。
...
- 通过预定义的硬件配置模式，从公开可观察的链上数据中推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
2) 之前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直到在区块 2,222,222 结束的稳定窗口期结束。
...
**现在是关键时刻：稳定期临时参与者 `Allowlist`**

在成功采用解决PoC相关共识故障的补丁后，新的治理投票当前正在生效。

随着正常区块生产恢复，网络即将进入一个短暂的稳定期，为后续增长做准备。

此投票定义了稳定窗口期内的参与者 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为始终符合网络预期的参与者集合。

**投票范围**

若获批准，网络将临时运行一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），并且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
之前表现出这些模式持续偏离的参与者在稳定窗口期于区块 2222222 结束前不属于 `allowlist`。

**可重复性与方法论**

`allowlist` 使用预定义的硬件配置模式从公开可观察的链上数据推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
- 从那时起，`allowlist`将持续生效，直至并包括区块2222222。
- 如有进一步调整，仍需通过治理决定。
...
链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强方案已获得批准，并成功在主网上部署。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用过的、有限时长的防御机制，现提议重新激活。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单将立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备变更，为未来通过治理机制控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票方可启用。
...
本升级重新激活或引入的所有保护措施均为临时性质，无需手动治理干预即可移除。
...
创世验证者增强功能最初在网路早期阶段作为临时防御机制引入，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。
...
- 在网络增长期间提供共识保护。在其先前运行期间：
    ...
    - 未向守护验证者发放额外奖励
    - 此配置有助于防止边缘情况下的共识停滞
- 当以下两个条件同时满足时，创世验证者增强将自动停用：
    - 网络总权力达到 15,000,000。
    - 区块达到 3,000,000。

**协议稳定性修复（全网）**

本次升级正式将此前通过手动 API 更新分发并已在网络中使用的关键修复纳入协议。这些修复包括：

- 纠正失败推理请求的计数错误（包括以不支持格式处理但未标记为完成的请求）
- 改进对失败推理的处理韧性
- 为 `PoCBatch` 和 `PoCValidation` 交易引入批处理。

通过将其纳入协议，该行为将成为全网一致的协议级规则。

**临时参与和执行限制**

- 主机级注册：在区块 2,222,222（约两周后）之前暂停新主机注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期间暂停新开发者地址注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续至区块 2,294,222（约 19 天）。
...
本提案涵盖与链暂停（第 117 期）相关的两项奖励分发：

- 在第 117 期活跃但未收到该期奖励的节点，将补发该期遗漏的奖励。
- 所有在第 117 期活跃的节点，将额外获得相当于第 117 期奖励 1.083 倍的 payouts，均匀分配给所有符合条件的节点，包括已收到原始奖励的节点。
...
如果您的 `application.db` 占用大量空间，可参考[此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中的方法进行清理。
...
## 2026年1月5日
当前网络上观察到高于常态的推理遗漏率。
在许多情况下，这是由于一个bug导致以不支持格式提交的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。

参考：[https://github.com/gonka-ai/gonka/pull/517](https://github.com/gonka-ai/gonka/pull/517)

此 `API` 版本改进了对失败推理的处理韧性，并减少了推理遗漏计数问题。同时引入了对PoCBatch和PoCValidation交易的批处理功能。
