...
!!! note "关于此页面"
    ...
    本页面不保证内容完整。如需获取最新信息，包括治理投票的发布及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。

## 2026年9月18日

**提案通过：批准 devshard v5 和奖励发放**

提案 ID 为 105 的链上治理投票已结束。

该提案以 123,754 票赞成、反对票数较少的结果获得通过。

**变更内容**

DevShard v5 已添加至链上批准的 devshard 版本列表：
...
主要协议新增功能是 height-sync，它为 DevShard 会话提供了一个经过签名、可重放的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立了协议级别的逻辑时间。此次发布还包括高可用性改进、热切换和会话恢复、网关更新、验证与结算修复、安全加固、可观测性提升以及扩展的端到端测试。

**主机所需操作**

无需操作。`versiond` 将下载新的运行时，验证其 sha256，并在现有运行版本旁启动服务。`v3`、`v4` 和 `v4.1` 仍为已批准版本，因此现有会话和路由前缀不受影响。这是一个运行时版本，而非网关发布：任何网关变动将另行公告并提供独立指南。

**奖励**

该提案还从社区销售合约中向多个开发周期中作出贡献的核心贡献者发放 91,300 USDT 奖励：@a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。所有奖励均已链上执行。

技术范围、个人贡献、PR 引用及完整奖励明细：[PR 1584](https://github.com/gonka-ai/gonka/pull/1584)

## 2026年9月16日

**devshard v5 提案重返治理（提案 104 的重新提交）**

提案 ID 105 将 `v5` 添加至批准的 devshard 版本列表，并从社区销售合约中分配 91,300 USDT 给贡献者奖励。

**Devshard v5**

这是一次仅针对 devshard 的变更。Cosmovisor 不涉及，链不会停止，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，将其 SHA-256 与链上存储的值进行验证，然后启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能是 height-sync，它为 devshard 会话提供了一个经过签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级别的 QoS 建立了协议级逻辑时间。此次发布还包括高可用性改进、热切换和会话恢复、网关更新、验证与结算修复、安全加固、可观测性提升以及扩展的端到端测试。

该版本已由核心测试团队和外部测试实验室进行了测试。
...
技术范围、个人贡献、PR 引用及完整奖励明细：[gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)

完整描述：[update-v0.2.15-devshard-v5/README.md](https://github.com/gonka-ai/gonka/blob/devshard-0.2.15-v5/proposals/governance-artifacts/update-v0.2.15-devshard-v5/README.md) (sha256 `7bba17188e4c3d15476f2c6330281b3908fc124e9eb26950acd6f9f927d89dc4`)

**奖励发放**

受奖者为在多个开发周期和版本中积极参与的 Gonka 核心贡献者：@akup / @a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。
...
投票将于 **2026年9月18日16:47 UTC** 结束。为通过，该提案需达到25%的加权法定人数、多数赞成票，且反对票（含否决）少于33.4%。

请投票。此前的尝试因参与率不足而失败，无人反对，因此参与是该提案获得批准的唯一障碍。
...
```
如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。

## 提案被拒绝：批准 devshard v5 及奖励发放

提案ID为104的链上治理投票已结束。该提案因 `failed_reason`：`proposal did not get enough votes to pass` 被拒绝。

最终票数：赞成 25291，反对 0，反对（含否决）0，弃权 0。

无人投反对票。参与率约为网络权重的6%，低于25%的法定人数，因此该提案仅因参与不足而失败。

## 2026年9月16日

**GLM-5.3-Flash 已上线**

自第395个周期起，`zai-org/GLM-5.3-Flash` 已成为活跃模型组，并在主网上提供推理服务。

如果您希望托管它，欢迎加入。在部署窗口期内声明对该模型的意向并配置您的MLNode，该窗口在下一次PoC开始前500个区块开启。

**请勿忘记您的参与选择**

GLM-5.3-Flash 的不参与惩罚自第394个周期起生效。对于每个经治理批准的模型，您必须明确选择：自行提供、委托或拒绝。不采取任何行动将导致每周期扣除您全部权重的15%，且该惩罚将持续至您做出选择为止。
...
提案ID 104 有两项内容：将 `v5` 添加到已批准的 devshard 版本列表，并从社区销售合约中分配91,300 USDT作为贡献者奖励。

**Devshard v5**

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 与链上存储的值进行验证后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。通过该提案本身不会转移任何流量，因此经纪人可随后逐步迁移其网关。
...
该提案还向七名受益人发放共计91,300 USDT的贡献者奖励：
...
投票将于 **2026年9月16日01:23 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成票，且反对票（含否决）少于33.4%。

提案详情及投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
**提案通过：补偿Daniil和David Liberman参加All-In Summit 2026的费用**

提案ID为103的链上治理投票已结束。

该提案已获批准。从社区资金池中拨款15,500 USDT，用于补偿Gonka协议的联合创建者Daniil和David Liberman参加All-In Summit 2026的两张参会通行证费用。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人7,500 USDT，门票已支付。本次补偿涵盖两张门票及提取资金的交易费用。飞行、住宿及其他费用未在本提案中申请社区资金池支付。

最终投票结果：赞成184762，反对0，含否决反对0，弃权0。
...
该提案已获批准。Quant Mesh Limited被正式批准为Gonka与Ledger集成的法律签约方，并从社区资金池拨款350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

拨款涵盖第一阶段启动费275,000 USDT、第一年年度激活费75,000 USDT以及350 USDT处理费用。授权与资金拨付严格限于完整提案中所述的Ledger集成。

链上转账已完成。

最终投票结果：赞成146903，反对0，含否决反对0，弃权5568。
...
该提案已获批准。`zai-org/GLM-5.3-Flash`现为经治理批准的模型，并纳入PoC模型集，而`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`已从`poc_params.models`中移除。这两个ID仍保留在`GET /v1/governance/models`（推理目录）中，但不再作为PoC模型。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：其 PoC 参数和权重保持不变。

该模型及其参数由 kaitaku.ai 团队提出，由 vbgd0 独立验证，并共同提出。测量数据及阈值设定的推理过程见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起 `penalty_start_epoch` 生效，因此未明确选择的宿主将面临不参与惩罚。如果您计划提供该模型，尽早声明意向将使团队有最佳机会形成。宿主指南：[GLM-5.3-Flash 启动](./host/glm-bootstrap.md)。

最终投票结果：是 309308，否 0，带否决的否 0，弃权 3168。
...
提案 ID 为 100 的链上治理投票已结束。

该提案已获批准，`v4.1` 已添加至 approved 的 devshard 版本列表中。`versiond` 将下载二进制文件，并将其 SHA-256 与链上存储的值进行验证，然后启动运行时。无需手动宿主操作。

v4.1 现在与现有的 v3 和 v4 运行时并行运行，因此当前流量仍正常服务，经纪人可逐步迁移其网关。
...
最终统计：赞成 312476，反对 0，否决 0，弃权 0。

感谢所有参与投票的人。

## 2026年9月9日

**提案 101：添加 GLM-5.3-Flash，从 PoC 模型中移除 Kimi-K2.6 和 GLM-5.2-FP8**

提案 id 101 将 `zai-org/GLM-5.3-Flash` 注册为治理批准的模型，并将其添加到 PoC 模型集中。同时将其从 `poc_params.models` 中移除 `moonshotai/Kimi-K2.6` 和 `zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7` 和 `deepseek-ai/DeepSeek-V4-Flash-0731` 不受影响：它们的 PoC 参数和权重保持不变。所有其他链参数保持不变。

该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，现共同提出。发布输入、测量数据及阈值设定的推理依据见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起，`penalty_start_epoch` 生效，因此未明确选择的宿主将面临不参与惩罚。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票率、多数赞成，且反对票（含否决）少于 33.4%。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。

## 2026年9月9日

**devshard v4.1 运行时提案已进入治理流程**

提案 id 100 将 `v4.1` 添加到批准的 devshard 版本列表中。

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并验证其 SHA-256 值是否与链上存储的值一致，然后启动运行时。

v4.1 将与现有的 v3 和 v4 运行时并行运行。通过提案本身不会移动任何流量，因此经纪人可在之后逐步迁移其网关。
...
投票将于 **2026年9月10日22:36 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票（含否决）低于33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅有关从冷密钥向热密钥授予治理投票权限的指南。

## 2026年9月4日

**提案通过：支持Gonka出席TOKEN2049新加坡**

提案ID为99的链上治理投票已结束。

提案已获批准。将从社区资金池拨款100,000 USDT，用于支持Gonka参与2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金涵盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支援材料。
...
`v4.0.2` devshard二进制文件已发布。主机可使用以下说明手动更新v4二进制文件。尚未安排自动更新。

**变更内容**

重启时，`v4.0.1`会从nonce 1恢复所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond`随后在60秒就绪超时后终止子进程，客户端出现HTTP 502错误。
...
**提案通过：将DeepSeek V4 Flash的weight_scale_factor提高至0.246**

提案ID为98的链上治理投票已结束。

提案已获批准。将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提高至0.246。所有其他模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为`PROPOSAL_STATUS_REJECTED`，原因是`failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终投票结果：赞成15452，弃权3093，反对0，否决0。

原始的0.214系数源于被高估的nonce/min数值。测量修正见[gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需任何操作。

感谢所有参与投票的人。

## 2026年8月25日

**仅devshard升级的PR现已开放审核**

Devshard升级独立于主区块链更新devshard运行时。无需通过Cosmovisor协调全节点升级，不影响主网行为，预计不会造成推理服务的停机。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**主要变更**

1. v5的主要目标是高度同步：每个主机和序列器在保证金日志中维护一个已签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard现在可以确凿地知道cPoC何时运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height`/`observed_block_hash`标记已存在于所有先前携带时间戳字段的消息中。后续版本可将`confirmed_at`和`started_at`等墙钟字段替换为高度标记。这解决了未来时间攻击问题，并为协议层面处理流超时铺平了道路。本版本包含这些标记，但尚未将超时和封印决策切换至其上。

3. 已添加通过所有插槽往返的心跳机制，因此安静会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同节奏。
...
投票将于 **UTC 时间 8 月 27 日 03:47** 结束。如能请投票。
...
该提案已获批准。它调整了在周期早期数据不足时无效推断检查（SPRT）的行为，避免因样本量不足而过早触发。主机无需采取任何操作。
...
一个新的链上治理提案（提案 ID 96）已开放投票。

**功能说明**

在周期初期，数据尚不足，因此无效推断检查（SPRT）可能在样本量足够前仅因少量无效样本而触发。本提案对其进行稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本量下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会被惩罚。真实违规者仍会被触发（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效样本，且在 500 个样本时观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如能请投票。如果未能及时达到紧急阈值，提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。如转为常规提案，您需要重新投出选票。
...
一个稳定的网关版本当前已在单个网关上运行，并将用于第一阶段结算。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于10%的主机不会触发任何惩罚或罚没，但应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。没有显著问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端问题。
超调度和隔离部分旨在为网关开发提供洞察，主机无需采取行动。

**下一步：****1) 接下来的24小时。** 主机应检查其统计数据并解决任何关键错误或配置错误。如有需要，贡献者可协助调查问题。

**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，首先从较小模型开始。该过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其数据在负载下目前可靠性较低。

**3) 接下来的1-3天。** 将监控结算结果

**4) 一旦此阶段稳定。** 其他经纪人可部署稳定的网关版本并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至100%。

该发布过程故意缓慢进行，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在 [提案94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同作为活跃模型组加入，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后经 [提案98](#august-29-2026) 提高至 0.246）转换为共识权重。

对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行其他操作——现有设置将继续生效。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点使用旧版 MLNode（即仍在服务 Kimi 或 MiniMax 的节点，因为任何已运行 DeepSeek 的节点均已使用 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。

MLNode 镜像在 [`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 的 `vllm-0.25.1-upgrade` 分支上已固定为 3.0.16。

默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍在使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是相同的 MLNode 3.0.16，仅针对 CUDA 12.9 构建。两个标签均在同一个 compose 文件中。
...
将活跃的社区销售合约和所有包装代币实例迁移到新存储的 CosmWasm 代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常操作。
创世守护者预计会支持该提案。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
- 投票截止：2026年8月14日，12:01 UTC
- 预计迁移时间：2026年8月14日，12:01 UTC

## 2026年8月13日

**DeepSeek V4 Flash 引导：意图阈值已满足 —— 请在 epoch 360 快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意图和委托权重已满足 `V_min`、`W_threshold` 和 **>2/3 可达性**（DeepSeek 意图权重加上委托给这些主机的权重）。下一次**官方**检查为 epoch 360 引导快照——在此前请保持委托多样化，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始（区块 **5,552,115**）前约 500 区块的部署窗口内部署 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。

**保留其他模型的主机**

在区块 **5,551,615** 前，提交委托（有助于引导可达性）或拒绝。从 epoch **360** 起，DeepSeek `penalty_start_epoch` 将激活——未明确选择的主机将面临不参与惩罚。
...
委托主机：选择**非守护者** DeepSeek 意图主机，并将权重分散到独立目标上——参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
更多关于引导机制的信息：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)

## 2026年8月13日

**测试 DeepSeek V4 Flash**

DeepSeek 已通过投票进入引导阶段——如果您计划提供服务，请验证您的设置并立即提交意图。
...
从该分支获取 MLNode 栈——默认镜像已在 [compose 文件](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 中锁定。
...
为在 B* 卡上获得最佳性能，可使用从 fp8 + fp4 重新打包为 fp8 + nvfp4 的模型：
...
建议逐步将节点切换至新 API，而非一次性全量更新。

**3) 使用 mlnode-validate 验证**

该技能现已支持 DeepSeek。它位于独立的 `./skills` 文件夹中，无需任何代理特定配置——将其符号链接至 `.claude/skills` 并运行 [`mlnode-validate` 技能](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/skills/mlnode-validate)：
```
/mlnode-validate how to use
/mlnode-validate <your-node-ip:port> @deploy/join/node-config-deepseekv4flash0731-B300.json
```
该技能将部署模型，测量 PoC 吞吐量，并将输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则说明准备就绪。

如果您计划提供 DeepSeek，请按上述步骤设置。若不提供，请勿忘记委托，以免遭受不参与惩罚。

提醒：明天将发布带有更佳日志的另一个 MLNode 构建版本，请留意后续更新。

## 2026年8月12日

**DeepSeek V4 Flash 启动（第 359 轮）**

提案 94 已通过——DeepSeek V4 Flash 已获批准。计划提供该模型的主机应立即声明意向，以便模型在第 359 轮启动。

[包含系数的完整表格](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)

**效果**

从第 359 轮开始（PoC 启动时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），前提是模型满足启动资格：

- `deepseek-ai/DeepSeek-V4-Flash-0731` 变为可用的 PoC 模型。
- `MiniMaxAI/MiniMax-M2.7` 保持为基础模型。
- `moonshotai/Kimi-K2.6` 保持可用且不变。

**主机所需操作**

1. 声明意向。在提案 94 通过后、启动快照区块 5,536,224（约 8 月 13 日 02:40 UTC / 8 月 12 日 19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 在PoC 359开始前的约500个区块安全窗口内切换您的MLNode，将其配置为DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate配置），该窗口对应区块5,536,724（约8月13日，03:24 UTC / 8月12日 20:24 PDT）。在此窗口内切换是安全的，因为其中没有cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在引导前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留您当前的模型。在epoch 360之前无需操作。从epoch 360开始，如果您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的未参与惩罚。

区块高度是精确的；时间是估算值，将在接近epoch时确认。[更多关于DeepSeek引导的详情](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。不修改任何现有参数或模型；唯一的变化是新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在vLLM 0.25.1上运行的MLNode，并使用release-candidate节点配置（`node-config-release-candidate-*.json` 适用于 B300 / B200 / H200 / H100）
...
根据模型使用情况，社区之后可决定逐步用DeepSeek替代Kimi或MiniMax（例如，提高系数以激励所有B系列GPU转向DeepSeek）。

完整系数表：[Google Sheet](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)

**引导机制说明**

模型通过标准引导流程进入，因此主机可在投入硬件前评估其可行性：

1. 声明意向：在 `start_poc - deploy_window` 截止快照前提交 `MsgDeclarePoCIntent`。
2. 预资格快照：链对意向和委托进行快照并发出建议性预资格事件。
3. 部署窗口：声明意向的主机为其MLNode进行配置。
4. PoC开始：成员资格由在PoC开始时提交PoC存储提交的主机决定，而非仅由声明意向的主机决定。

最终MLNode版本和设置说明将在投票结束后发布。

更多关于引导机制的说明：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)

**若获批准的效果**

在符合引导资格的前提下，从epoch 359（PoC开始时间约为2026年8月13日 03:24 UTC / 8月12日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与惩罚从epoch `360`（`penalty_start_epoch = 360`）开始生效。

**主机所需操作**

1. 若要提供服务：投票结束后声明意向，然后配置您的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在epoch 360之前无需操作；从epoch 360起，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`。
3. 在截止日期前对提案94进行投票。

**如何投票**

如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
- PoC 种子稳定性。在纪元边界延迟后恢复遗漏的种子提交，并在瞬时故障（如签名者不可用）后重试本地种子恢复，以防止验证者被归零。
- API。
...
请更新 api 容器使用的二进制文件。
在开始更新前，请确保 CPoC 和 PoC 均未在运行。
每次 PoC 开始前有 500 个区块的 CPoC 空窗期。请尽可能利用此窗口进行更新。
每次仅在一台机器上部署更新，以降低操作风险。
...
这只是一个性能和资源管理补丁，除此之外无其他改动。协议、推理、验证和经济行为均保持不变。变更内容：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作线程的内存清理现在与纪元修剪保持一致。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅将治理投票权限从冷密钥授予热密钥的指南。
...
该提案已获批准，并于主网第 5316315 块成功执行。该提案还批准了 devshard v4 运行时，其现在与现有的 v3 运行时并行运行。

## 2026年7月29日

**升级 v0.2.15 和 devshard v4：预先下载二进制文件**

关于 **devshard v4 和 v0.2.15** 升级提案的链上治理流程即将结束。

- 投票截止：UTC 时间 2026 年 7 月 30 日上午 07:08 / PDT 时间 2026 年 7 月 30 日凌晨 12:08
- 提议的升级高度：5316315
- 预计升级时间：UTC 时间 2026 年 7 月 30 日下午 03:28 / PDT 时间 2026 年 7 月 30 日上午 8:28

鼓励主机所有者在 [GitHub](https://github.com/gonka-ai/gonka/pull/1497) 上审阅提案并参与投票。

提前预下载二进制文件有助于在升级窗口期内避免依赖 GitHub 的可用性。
...
2. 一个 **v0.2.15 主网软件升级**，移除大量过时代码、遗留的交易处理路径及相关组件，这些组件已不再符合当前架构的需求。

尽管这两项变更以单次投票提交审议，但它们在操作上仍相互独立：
...
v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列性 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个专为多实例高可用设计的版本：在共享的 Postgres 上，由 versiond-router 负责将 N 个版本化的 /devshardd 副本进行粘性会话路由和验证租约独占。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并引流，以确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。同时，通过移除过时行为并集中交易处理流程至当前支持的架构，使未来的开发和升级更加便捷。
...
若获得批准，二进制版本将通过链上升级提案更新。有关升级流程的更多信息，请参阅 [/docs/upgrades.md.](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.15/docs/upgrades.md)。

**Devshard v4**

同一提案还将在 `DevshardEscrowParams.approved_versions` 中注册 devshard v4。
...
提案获批后，`versiond` 可自动执行：

1. 下载已批准的二进制文件。
2. 将其 SHA-256 哈希值与链上存储的值进行校验。
...
投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 版本 v0.2.12 或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 提议升级高度：5316315
- 预计升级时间：2026年7月30日 03:28 UTC / 2026年7月30日 08:28 PDT

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的Cosmovisor备份`application.db`通常为数十GB，需提前确认）。有关如何安全删除`.inference`目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用大量磁盘空间，可应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的Cosmovisor备份清理技术。
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例提供支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认，并自动清理（保留最近3个周期）。

## 2026年7月23日

**升级已执行：v0.2.14现已在主网上线**

针对升级提案v0.2.14（提案ID 89）的链上治理投票已结束。

该提案已获批准，并于主网区块5195700处成功执行升级。

## 2026年7月22日

**升级v0.2.14：提前下载二进制文件**

v0.2.14升级提案的链上治理流程即将结束。

- 投票结束：2026年7月23日 00:02 UTC
- 升级高度：5195700
- 预计升级时间：2026年7月23日 ~03:45 UTC
...
[v0.2.14提案](https://github.com/gonka-ai/gonka/pull/1267)现已上链并开放投票。

主网链/API工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（在主网上禁用`/v1/chat/completions`计费并从API二进制中移除嵌入的`/v1/devshard`）、奖励接收者路由以及升级时的安全修复。

devshard部分为v3运行时做准备，以便代理在链升级期间无需依赖已弃用的经典API路径即可提供推理服务，从而提高RAM利用率并支持SQLite与Postgres存储之间的安全切换。
...
如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参考[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
投出您的选票（`yes`、`no`、`abstain`、`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced` v0.2.12或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
- 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护者 — 此次升级中未移除或重塑任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）

**截止日期**

- 投票结束：2026 年 7 月 23 日 00:02 UTC / 2026 年 7 月 22 日 17:02 PDT
- 提议的升级高度：5195700
- 预计升级时间：2026 年 7 月 23 日 03:45 UTC / 2026 年 7 月 22 日 20:45 PDT

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关安全清理 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个纪元）。

## 2026 年 7 月 20 日

**[仅 devshard 升级的 PR 现已开放审查](https://github.com/gonka-ai/gonka/pull/1482)**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，使用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。

**操作事项**

请审查 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现任何问题、疑问、改进建议、边缘情况或潜在漏洞时留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一升级周期的社区奖励。
...
* 在区块 **5,105,276**（2026 年 7 月 17 日 ~12:05 UTC）之前声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在纪元 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776** — 2026 年 7 月 17 日 12:50:53 UTC / 05:50 PDT），将您的 MLNode 切换为 Kimi。
* 委托主机：选择非守护者 Kimi 主机，并在独立目标间分散权重 —— 请参阅[多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。

**devshard v1/v2 已移除**

`versiond` 已停止 v1 和 v2 的 `devshardd` 进程。所有网关流量必须使用 `/devshard/v3`。如果您的网关在此次变更后停止提供推理服务，说明您仍在通过已被移除的路径路由 —— 请遵循 v3 迁移指南。

## 2026 年 7 月 16 日

**紧急治理投票（提案 88）：重新注册 Kimi-K2.6 并移除 devshard v1/v2 运行时**

这是 7 月 15 日宣布的 Kimi 恢复计划的第二步。提案 87 已将 `moonshotai/Kimi-K2.6` 从活跃集合中移除；提案 88 将其重新注册到治理模型列表中，使其从纪元 331 开始进入标准引导流程。

同一提案还从 `devshard_escrow_params` 中移除了 `approved_versions` v1 和 v2。一旦批准，`versiond` 将停止 v1 和 v2 的 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。这消除了旧运行时容器长期存在的 RAM 增长问题 —— 即 7 月 15 日事件中网络节点 OOM 的根源。

**这标志着 v3 切换最终完成。** 仍通过 v1/v2 前缀或经典 `/v1/devshard` 路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 进行投票 —— 紧急窗口较短。
2. 提供 Kimi 服务的主机：在投票结束后、区块 **5,105,276**（2026 年 7 月 17 日 ~12:05 UTC）之前立即声明 `moonshotai/Kimi-K2.6` 意向，然后在纪元 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776**，2026 年 7 月 17 日 ~12:50 UTC）切换您的 MLNode。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在进行引导委托时：**不要委托给守护节点**；将委托分散到独立的 Kimi 主机上。请参阅 [多模型概念验证指南](https://gonka.ai/docs/host/multi_model_poc/) 以获取最新的委托指引。
4. 经纪人 / devshard 创建者：如果您的网关流量仍使用 v1/v2 或 `/v1/devshard` —— 请在投票结束前切换到 `/devshard/v3`。
...
- 投票时间：2026-07-16 11:09 ~ 2026-07-16 23:09（太平洋时间）/ 2026-07-16 18:09 ~ 2026-07-17 06:09（UTC）（紧急，0.667 赞成阈值；出席率重要，请尽快投票）。
- 意向截止时间（第330个周期）：区块 **5,105,276** —— 7月17日，约12:05 UTC（约05:05 PDT）。
- 第331个周期PoC开始：区块 **5,105,776** —— 7月17日 12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6` 在第328-329周期失去了PoC验证多数支持（详见[此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其以最小停机时间恢复的最快方式——与六月提案78的恢复路径相同。

该提案将在下一个PoC前将 `moonshotai/Kimi-K2.6` 从PoC参数中移除，随后立即重新添加，Kimi 将从第331周期开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票——紧急窗口很短。
2. 运行Kimi的主机：请保持您的Kimi设置就绪，并准备好在引导PoC时切换回您的MLNode。
3. 在进行引导委托时：不要委托给守护节点；将委托分散到独立的 Kimi 主机上。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（太平洋时间）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（紧急，0.667 赞成阈值；出席率重要，请尽快投票）。
* 引导意向截止时间和周期时间详见引导说明。

## 2026年7月15日

**Kimi-K2.6事件（第328-329周期）：事件经过、恢复计划及委托指引变更**

在第328周期，`moonshotai/Kimi-K2.6` 失去了PoC验证多数支持。在第329周期开始时，运行Kimi的主机被移出组。网络其余部分正常运行。

**事件经过**

两台由守护者运营的运行Kimi的服务器同时发生故障：

* 其中一台服务器上，运行Kimi的MLNode因提供商端问题而崩溃。大量Kimi委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 的阈值，因此这足以使该组低于多数。守护者决胜机制未启用：目前没有每模型投票权重的守护者已被从决胜机制中过滤掉。这是一个已知问题，已在即将发布的 v0.2.14 升级中修复。

这种情况类似于第 306–307 个周期的事件。第 329 个周期开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 的最安全方法是通过全新的引导流程（约 1.5 个周期）重新添加它：

1. [今天] 一项加速提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意向、部署窗口、PoC 存储提交。意向截止日期前将发布操作说明。

**委托指导变更——请阅读**

先前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已撤销：

* **请勿将委托指向守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已在进行中的修复*** PoC 验证中的守护者投票（守护者始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期的 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完全迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦加速的 Kimi 提案上链，立即对其投票（公告将随后发布）。
2. 如果您计划提供 Kimi：关注引导说明，并在意向截止前准备好声明意向。
3. 如果您为 Kimi 委托：选择非守护者的委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这将解决内存增长问题，并在移除 v1/v2 前必需完成。
...
提案获批后，v3 现在与现有的 devshard 运行时并行运行。新进程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人切换流量至 v3。

该版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

此类仅限 devshard 的运行时升级无需重启主网或手动主机操作。

**操作项**

已获白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而不依赖已弃用的经典 API 路径。

**关键变更**

- 使经纪人能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
- 改进了内存利用率。
- 修复了网关运行时行为。
...
**v0.2.13-devshard-v3 运行时升级提案已进入治理**

此提案涵盖 [devshard v3 发布。](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.14/proposals/governance-artifacts/update-v0.2.13-devshard-v3)

这是一个仅限 devshard 的升级。它独立于完整链软件升级运行。一旦批准，v3 将与现有的 devshard 运行时并行运行。

v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。它还改进了 RAM 利用率，修复了网关运行时行为，并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整链软件升级。
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获得批准，`versiond` 将自动下载二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用早期运行时前缀，直到代理将流量切换到 v3。此类升级期间无需重启主网或手动执行主机步骤。

**如何投票**

如果您没有直接访问拥有投票权密钥的权限，或希望使用其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
[此拉取请求](https://github.com/gonka-ai/gonka/pull/PR) 针对下一个链上软件升级 v0.2.14，现开放审查。
...
节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从本分支单独发布并部署 devshard v3 版本，然后再进行主网链升级。提前将推理流量切换到 `/devshard/v3` 的代理可在链升级期间继续提供推理服务。

**建议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查本提案。
2. 在主网链升级前，提出并部署 devshard v3 版本。
3. 代理将推理流量切换到 `/devshard/v3`。
...
devshard v3 版本将在主网链升级前提出并部署，以便代理在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作事项**

1. 现在 — 审查 PR。阅读 GitHub 上的 PR #1267，并对任何发现、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。若已完成以上两步，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 代理 — 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制的计算无法通过采样检查。
...
**Kimi-K2.6 引导：第 311 个纪元的再次尝试**

`moonshotai/Kimi-K2.6` 在第 311 个纪元再次尝试引导。希望提供 Kimi 服务的主机需在第 310 个纪元声明意图，并在新纪元开始前准备好节点。一个守护节点也将运行 Kimi，因此不希望运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 个纪元，于区块 **4,797,456** 之前（约 6 月 28 日 12:00 UTC）声明 PoC 意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若足够多的主机声明意图，请在第 311 个纪元开始前约 500 个区块的窗口期内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
> **更新（2026年7月15日）：此建议已撤销——请勿委托给守护节点。** 守护节点是 PoC 验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/) 以获取最新的委托指导。

**关键时间点*** 意向截止时间（第310轮）：区块 **4,797,456** —— 约6月28日，12:00 UTC。
* 第311轮开始：区块 **4,797,956** —— 约6月28日，12:47 UTC。

更多关于引导流程的信息：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。该设置并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
更多引导信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。该设置并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使运营商可在配置资源前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由PoC开始时实际提交PoC存储提交的节点决定——而非仅凭声明意向。只有满足资格条件的模型才会被激活。

**若获批准的效果**

从第309轮开始（PoC开始时间约为6月26日15:25 UTC；生效时间约为16:00 UTC），前提是每个模型均满足引导资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中完成引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计会支持本提案。

**主机所需操作**

1. **若要提供Kimi：** 在投票结束后立即为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。

2. **若要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口期间配置您的MLNode。提供该模型是可选的——未选择的主机不会受到任何惩罚。

3. **若要继续使用MiniMax：** 继续提供 `MiniMaxAI/MiniMax-M2.7` —— 无需任何操作。

4. **在截止日期前对提案79进行投票。** 紧急窗口时间很短。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 紧急提案需要0.667的通过门槛；参与率很重要，请尽快投票。

## 2026年6月25日

**提案78通过：`MiniMaxAI/MiniMax-M2.7` 现为唯一的PoC模型；Kimi K2.6和Qwen3-235B已被移除**
...
自第308个周期起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。

**主机所需操作*** 请确保您的MLNode正在提供 `MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，在切换前将无法获得本周期的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——在第308周期将进行一次恢复Kimi的投票。
...
* **添加GLM-5.2** —— 添加GLM-5.2，**不设不参与惩罚**，以便主机可选择参与，并提前衡量对该模型的需求，不提供该模型的主机不会受到惩罚。

## 2026年6月25日

**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，正在提供Kimi的参与者已被移出组，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以便快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的方式。下个周期将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立且无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容调整。它与Kimi情况无关，仅因同为PoC阵容变更而被一并列入。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307周期结束前完成。每个周期约22.9小时，标准48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创世守护者应支持该提案。

**若获批准的效果**

自第308周期开始（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个周期的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可安全切换，因为此期间无cPoC。请现在预下载FP8权重（如尚未就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——在第308周期将进行一次恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前测试对该模型的需求，而选择不提供该模型的主机不会受到惩罚。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过`inferenced`获取。任何活跃节点均可使用：
...
* 第308纪元将在稍后形成：PoC开始约16:50 UTC，生效约17:25 UTC。
* 紧急提案要求0.667的通过阈值；参与率至关重要，请尽快投票。

## 2026年6月22日

**v0.2.13-devshard-v2运行时升级提案已通过治理**

devshard v2运行时已获链上批准并添加至`DevshardEscrowParams.approved_versions`。
...
提案通过后，v2可与现有的v1 devshard运行时并行运行。新进程在`/devshard/v2`前缀下提供服务，而现有的v1流量继续在`/devshard/v1`和`/v1/devshard`上运行。

发布版本将`devshardd`二进制文件作为Gonka发布工件。`versiond`会自动下载该二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。

对于此类仅devshard的运行时升级，无需重启节点容器或手动执行主机步骤。

**关键变更**

- 移除了种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中。
...
本提案涵盖devshard v2发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对devshard的升级。它独立于完整链软件升级。若获批准，v2将与现有的v1 devshard运行时并行运行。
...
1. 移除种子揭示轮次，封存已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。若链上提案获得批准，`versiond`将自动下载该二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。

新进程在 `/devshard/v2` 前缀下提供服务。现有的 v1 devshard 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。此类升级期间无需重启节点容器或手动执行主机步骤。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望使用其他密钥代您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将冷密钥的治理投票权限授予热密钥。
提案详情和投票可通过 `inferenced` 查看。可使用任何活跃节点。可用节点包括：
...
这是首次仅针对devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且不会导致推理服务中断。

如果通过治理流程批准，新版本的devshard将与现有的v1运行时并行运行。

请直接审查该PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格在下一轮升级周期中获得社区奖励。

本次仅为PR审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始。

## 2026年5月28日

**MiniMax-M2.7 现已在 Gonka 网络上激活**

v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax 组中获得的 PoC 权重现在将以校准系数 0.3024 转换为共识权重。

MiniMax 的每模型参与强制执行现已生效。已为 MiniMax 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续正常工作。尚未做出选择的主机应尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。

## 2026年5月26日

**升级已执行：v0.2.13 现已在主网上线**

升级提案 v0.2.13（提案ID 54）的链上治理投票已结束。
该提案已获得**批准**，升级已在主网块 `4267300` 成功执行。

## 2026年5月25日

**升级 v0.2.13：提前下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13 升级提案（提案ID [54](https://github.com/gonka-ai/gonka/pull/1143)）已通过链上治理，升级现已排期。

- 升级高度：4267300
- 预计升级时间：2026年5月26日 14:42 UTC（07:42 PDT）

提前下载二进制文件和权重有助于避免在升级窗口期间依赖 GitHub / Hugging Face 的可用性。
```
# 1. Create Directories
...
**v0.2.13 投票结束 — 准备在高度 4267300 执行升级**

对 [v0.2.13 升级提案](https://github.com/gonka-ai/gonka/pull/1143)（提案ID `54`）的链上治理投票已结束。该提案已**批准**。

升级将在主网的**块高度 4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）自动执行。

**提醒**

1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT 代币元数据和 CW20 `wrapped_token` 代码 ID `105` 均通过升级处理程序注册，因此桥接将在升级高度在主网上激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在提前下载约 230 GB 的 FP8 权重。在引导窗口期间，Hugging Face 的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为**每个**经治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 的截止时间为**链纪元 `278`**。未采取任何行动的主机将从纪元 278 起，对其全部权重施加每纪元 15% 的惩罚。
4. 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。请确保 `.inference/data` 有足够的空闲空间用于 cosmovisor 状态备份；如果 `application.db` 较大，建议在升级前应用 [cosmovisor 备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基准。此调整**仅适用于您共识权重中的 Kimi 部分**；您的 Qwen 部分权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新投票，现已提交若干更新。

- 包括：Kimi 的重新校准权重（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard 存储重构，以及若干 PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专用部分）。
- 该提案将升级后的宽限期延长至3000个区块，以便在新快照逻辑稳定期间主机不被惩罚。
- 治理：将创世守护者投票权降低至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%投票权中实现约三分之一的参与率才能满足法定人数（参见inference-chain部分）。
- 所需准备：桥接容器检查、MiniMax决策、仪表板更新、投票。
- 除非提案获得批准，否则链上无任何更改。

该PR：[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)
...
- 更新推理验证阈值：
    ...
    - Kimi K2.6：`0.900`
    - MiniMax-M2.7：`0.922`
- 在vLLM 0.20.1发布后，基于Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B：`0.359`（不变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下，Kimi每轮共识权重降低约38%）
    - MiniMax-M2.7：`0.3024`
...
- 将devshard nonce上限从`20_000`提升至`1_000_000`。
- 将每轮最大devshard数量从`100`提升至`500_000`。
- 修复新模型引导期间确认PoC奖励核算问题。
...
- 将创世守护者的调整投票权降低至约25%，并将链上治理法定人数设置为`0.25`。若守护者不投票，剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将启用Gonka主网与Ethereum之间的跨链转账（包括在Ethereum上封装GNK以及桥接USDC/USDT）。封装/解封装脚本和操作员工作流程将另行文档化。
...
**在第278个周期前做出`MiniMaxAI/MiniMax-M2.7`参与选择（届时开始处罚）**
...
所有主机需验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您是否运行的是当前版本，再采取进一步操作。
请按照以下说明操作：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

主机需更新仪表板/浏览器。请从`gonka/deploy/join`目录运行以下命令。如果您尚未在本地克隆`gonka`仓库，请先参考加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 投票结束：2026年5月22日 22:12:25 UTC
- 提议的升级高度：4267300
- 预计升级时间：2026年5月26日 14:42:02 UTC
- 运营商时间表：投票于5月22日22:12 UTC结束 → 升级高度约在5月26日14:42 UTC → 升级周期剩余阶段跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在start_poc − 500块时（约提前43分钟）→ 升级后下一个周期边界开始首个MiniMax PoC阶段 → MiniMax惩罚在链周期278执行。

**注意**

- 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中Cosmovisor对 `application.db` 的备份通常为数十GB，需提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。如果获得批准，此跳过是预期行为而非故障；新快照逻辑将在下一个周期开始。
- 如果获得批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认设置，并自动清理（保留最近3个周期）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过⅓），链上不会发生任何变化，升级将不会执行。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何操作。
...
在周期 #267 中，PoC验证已成功完成，受影响的主机已能够重新加入活跃集合。

周期 #266 的问题由针对运行Kimi模型的主机的攻击引起。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入周期 #266。

在应用额外保护措施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。

**发生了什么**

在周期 #266 中，网络的活跃权重大幅下降。

问题已追溯至具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。

在周期 #267 中，主机得以恢复，PoC验证成功完成。

**推理可用性**

网络将不再接受使用受影响的非标准语义的请求。

在相关保护措施实施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和中介端点。

**周期 #267 中Kimi的权重**

由于现有协议规则，Kimi在周期 #267 中的活跃权重较低：单一模型的总权重不得超过上一周期所有模型总权重的75%。

由于第266个周期的总活跃权重显著降低，此规则也限制了Kimi在第267个周期的权重。

随着未来周期中正常的PoC参与持续进行，权重可能需要数天才能稳定。

**主机所需操作**

1. 尽可能保持您的API节点在线并可访问。这有助于保留对主机参与情况的可见性，并支持后续审查。
2. 监控接下来周期的PoC参与情况。确保您的节点按预期进入PoC，且活跃权重正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方可用，预计会通过社区代理和经纪人端点逐步恢复。
5. 在社区渠道或本帖中分享相关日志或观察结果。如果您的主机在第266个周期受到影响，或在后续周期中行为异常，这一点尤为重要。

## 2026年5月16日

**第266个周期：PoC权重下降调查**

在当前周期（#266）中，网络的活跃权重出现了显著下降。
似乎PoC投票未能收集到本周期所需的投票。确切原因尚未确认，社区正在积极调查此事。

**受影响的参与者**

如果您的节点未能进入本周期，请尽可能保持您的API节点在线并可访问。
这有助于赔偿委员会收集PoC参与的证据，并更准确地核算受影响的贡献。

**调查进行中**

社区成员目前正在审查第266个周期发生的情况。一旦对根本原因有更清晰的了解，将及时分享更新。
如果您有能帮助调查的相关观察、日志、假设或其他技术背景，请在此处分享。
...
- 包括：对Kimi（`0.78`）重新校准权重、新增模型 `MiniMaxAI/MiniMax-M2.7`、更新验证阈值、devshard存储重构，以及多项PoC/奖励修复和以太坊桥接主网激活。
- 提案在升级后延长了宽限期，以避免在升级发生后3000个区块内惩罚主机。
- 所需准备：桥接容器检查、MiniMax决策、仪表板更新、投票。
- 除非提案获得批准，否则链上无任何变更。

PR链接：[https://github.com/gonka-ai/gonka/pull/1143](https://github.com/gonka-ai/gonka/pull/1143)
...
- 将devshard的nonce上限从 `20_000` 提高到 `1_000_000`。
- 将每个周期的最大devshards数量从 `100` 提高到 `500_000`。
- 修复新模型启动期间确认PoC奖励核算问题。
- 在本次升级周期的剩余时间内禁用确认PoC，以便新快照逻辑从下一周期开始干净启动。
...
**在第 278 个周期前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
要求所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

要求主机更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令：
```
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml pull explorer
docker compose -f docker-compose.mlnode.yml -f docker-compose.yml up -d explorer
```
...
- 该提案将有意从升级高度到升级周期结束（3000区块宽限期）跳过确认 PoC。如果获得批准，此跳过是预期行为而非故障；新的快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效载荷存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊端合约激活，所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。

部分主机可能已部署桥接容器。在这种情况下，请先检查您是否正在运行当前版本，再采取进一步操作。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/1143) 针对下一次链上软件升级 v0.2.13 已开放供审查。

请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一次升级周期中获得社区奖励。

本次仅为拉取请求的审查请求，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天启动。

**主要变更****inference-chain**

- 确认PoC在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被削减。修复方案是存储一个可确认模型和权重缩放因子的纪元快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一个新产生的错误推断会立即再次使其失效。该计数器现在在重新激活和即将晋升时重置。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授予中未包含`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应交易商投诉。
- Devshard结算使用了硬编码的`20_000` nonce限制。现在限制为`DevshardEscrowParams.MaxNonce`，v0.2.13升级将其设为`1_000_000`。升级还将`MaxEscrowsPerEpoch`提升至`500_000`。
- 升级为当前纪元安装了一个宽限期条目，延长了`UpgradeProtectionWindow`（3000个区块）。从升级高度到升级纪元结束期间，确认PoC触发器将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用了v0.2.10的宽限期原语。
- Wasm守护进程访问在应用连接后解析，因此合约权限检查适用于桥接和流动性池操作。

**decentralized-api**

- 某些OpenAI兼容的上游返回数值型`stop_reason`值。`Choice.StopReason`现在接受任何JSON类型，因此这些响应不再因反序列化失败。
- 内部devshard存储迁移不再阻塞dapi启动。在迁移和恢复完成前，devshard路由仍不可用。

**devshard**

- 由于旧的托管数据保留在一个SQLite存储中，devshard存储可能无限增长。现在存储按纪元范围划分，并在后台清理旧纪元，仅保留最新的3个纪元。
- Devshard需要为大规模部署提供共享存储选项。现在可以使用Postgres作为主存储，SQLite仍作为本地备用。
- Postgres数据按`epoch_id`对会话、差异和签名进行分区，以便清理可干净地删除旧纪元数据。
- 状态快照减少了长期会话的恢复工作量。
- 负载查找被固定到托管纪元，并为纪元边界和遗留纪元0请求提供回退机制。
- 当前纪元分片统计信息公开了nonce、版本、组和每个主机的计数器。

**bridge**

- 桥接工具处理Sepolia标志，并将Gonka BLS密钥/签名转换为以太坊合约期望的EIP-2537格式。
- 添加了用于GNK和封装代币桥接操作的脚本。

评审者可在此处查看完整的升级提案、迁移细节、测试摘要和建议流程：
...
在api容器版本v0.2.11、v0.2.12和v0.2.12-api-post2中存在一个潜在问题。在容器重启后，9100、9200和9400端口上的服务器可能启动延迟。这延迟了api激活，导致部分矿工跳过确认PoC。
...
docker start api
```
部署后，请再次检查9100和9200端口上的服务器是否正在运行：
```
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实践中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化后续模型的接入，建议将该阈值降低至10%。

安全模型保持不变：PoC验证本身未更改，仍需超多数验证算力才能接受结果。

该提案被加速，以便在下一次PoC之前生效。投票将持续12小时。
...
`moonshotai/Kimi-K2.6` 已完成引导并加入 Gonka 网络的 PoC 参与。

该过程由网络各节点协调完成：基础设施已准备就绪，意向已提交，委托与拒绝已设置，部署已测试。

对于多模型 PoC，这意味着 Kimi 现在作为活跃模型组拥有独立的参与和奖励跟踪。

运行 Kimi 的节点应继续像往常一样监控其 MLNode 和 PoC 参与情况。

## 2026年5月4日

**对已提交 PoCIntent 的节点：部署 `Kimi K2.6`**

今日 `moonshotai/Kimi-K2.6` 的预评估检查已通过。

已提交 PoCIntent 的节点应在 PoC 于区块 `3874496` 开始前，将至少一个 MLNode 从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。

预评估与 PoC 开始之间有 500 个区块的窗口期。在此期间无 CPoC 任务，因此已声明意向的节点可安全地将模型节点切换至 `Kimi K2.6`。
...
这标志着网络的一个里程碑：`devshard` 已上线并具备生产就绪能力。`node4` 将作为今后推荐的公共网关。

**需采取操作**：将您的端点更新为 `node4`。

## 2026年5月2日

今日的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，其权重极低。请保留您的 MLNode 使用 `Qwen235B`，并于明天提交下一周期的意向。

## 2026年4月30日

**升级已执行：v0.2.12 现已在主网上线**

关于升级提案 v0.2.12 的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个周期激活。其系数为 Qwen235B 的 3.51 倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议加固**。保留节点（`POC_SLOT=true`）被随机抽选用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易方共识，使旧版验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。

**对节点的指导**

- 部署、委托或明确拒绝新的治理批准模型（所包含的模型将在升级后两个周期激活）。请参阅[指南。](https://gonka.ai/docs/host/multi_model_poc/)
...
- 迁移：测试与迁移详情见[v0.2.12 文档。](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/upgrades.md)

这些变更的更多详情请参阅治理文档：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)

## 2026年4月29日

**升级 v0.2.12：预下载二进制文件**

关于 v0.2.12 升级提案的链上治理流程即将结束。
...
版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此即使持久化配置仍包含额外模型，`/admin/v1/nodes` 也显示为干净。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个带有清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。其余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需要手动修复。

将以下脚本粘贴到主机的 shell 中。默认情况下，它将应用更改。若要预览更改而不应用，请将 `APPLY=dry` 设置为任意非 `--apply` 的值。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接或通过委托 PoC 投票权重）。
...
- **协议加固。** 保留的节点（`POC_SLOT=true`）将被随机采样用于单个 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
从 `docker-compose.yml` 部署 `versiond` 和 `proxy` 服务的最新版本（使用 repo 的 release/v0.2.12 标签）：
```
...
如果您没有直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。
...
- 投票截止：2026 年 4 月 30 日 00:12（UTC）
- 升级高度：3834200
- 预计升级时间：2026 年 4 月 30 日 6:00（UTC）

**注意**

- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用大量磁盘空间，可应用 cosmovisor 备份[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
...
下一个链上软件升级 v0.2.12 的[拉取请求](https://github.com/gonka-ai/gonka/pull/948)已开放供审查。

请直接审查 PR 代码，并对您发现的任何问题、疑问、改进建议、边界情况或漏洞留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一个升级周期中获得社区奖励。

本次仅为拉取请求审查请求，不启动正式投票。治理投票流程将在审查期结束后开始。
...
- **共识层交易费用与自动迁移**（[#937](https://github.com/gonka-ai/gonka/pull/937)，[#981](https://github.com/gonka-ai/gonka/pull/981)）。引入治理控制的 Gas 价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免费。`MsgPoCV2StoreCommit` 采用两部分费用（基础验证 + 计数线性）作为主要 Sybil 防御机制。详情请参阅 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard 独立运行时**（[#1045](https://github.com/gonka-ai/gonka/pull/1045)）。将 devshard 发布与 DAPI / 主网发布周期解耦。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
- 确保冷账户持有足够的资金（例如 100 GNK），以覆盖自动授予的费用配额支出限额。
- 在治理批准新模型后，部署、委托或明确拒绝每个治理批准的模型（包含的模型将在升级后3个周期内激活）
- 从 `docker-compose.yml` 部署 `versiond` 服务（使用主分支中的最后一次提交）
...
新版本的 mlnode 现已可用：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
此更新修复了在PoC启动附近的一个边缘情况，该情况此前可能导致在特定条件下性能下降。
...
针对升级提案 v0.2.11 的链上治理投票已结束。该提案已获批准，并已在主网上成功执行升级。
...
这些性能改进可根据工作负载和网络条件，使每个区块的推理次数最多提升100倍。
...
鼓励主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/813) 上审阅提案并投票。
提前下载二进制文件有助于在升级窗口期间避免依赖 GitHub 的可用性。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理扩展性，并显著改善 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每个推理交易处理推理操作限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
有关这些及其他更改的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)

**升级前的建议操作****`application.db` 剪枝**

强烈建议主机在升级前按照提供的说明对 `application.db` 进行剪枝。

提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时启动剪枝活动，从而造成不必要的运营压力。
...
对于本 PR 审查的有意义贡献，可在下一次升级中提出奖励。

本次仅为拉取请求的审查邀请，并非正式投票的开始。治理投票流程将在审查期结束后启动。
...
目前，通过每笔链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完备，而是因为此类系统需要尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
在类似主网的条件下，这使得每个区块可容纳的推理数量最多提升100倍，具体取决于工作负载和网络条件。  ￼

**升级前的建议操作****`application.db` 剪枝**

强烈建议主机在升级前按照提供的说明剪枝 `application.db`。
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时开始剪枝活动，造成不必要的运营压力。
...
已发现 `v0.2.10` 中存在潜在漏洞。为降低当前升级前阶段的风险，建议在下次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
如果您没有访问持有投票权的冷密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求他们授予您代为投票的权限。若无此授权，其他账户无法提交投票。

在此设置中：

- 授予方 = 拥有投票权的账户（冷密钥）
- 被授予方 = 代表授予方提交投票的账户（热密钥）

被授予方仍可为其自身账户投票。授予方可随时撤销此权限。
...
2. 之前的请求重试系统在因处理超时（而非TLS超时）失败时仍会重试推理。
服务器端对长请求的重试通常无效，因为它会导致相同的超时场景。同时，客户端可能收到不一致的输出。新版本的API在这些情况下不再重试推理。
3. 当前被保留且不参与PoC生成的MLNode仍被用于PoC验证。这可能导致推理遗漏。新版本将此类节点排除在PoC验证之外。
4. PoC验证管道中增加了额外的安全措施。
...
现在提供一个新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍处于活动状态的请求导致潜在权重下降的风险。
...
如前所述，第180个周期的抵押激活独立于本次投票。

由于提案未通过，创世区块中定义的抵押参数将在第180个周期自动生效。
...
抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若无其他方案获批，则默认规则适用。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能会波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选择。如实施所示，有效的PoC参考窗口比先前名义假设长约5个区块。
...
此前，PoC权重计算依赖于名义周期持续时间假设。引入实时归一化后：
...
由于采用了PoC持续时间归一化，且有效的参考窗口现在比之前的名义假设长约5个区块，从第176轮开始的权重值反映了更新的计算模型。
从PoC权重推导H100等效容量或奖励预测的追踪器和仪表板应从第176轮开始验证其转换系数。
...
!!! note "兼容性说明"
    此升级包括迁移到 IBC 栈 v8.7.0。请检查任何解析 `inferenced` CLI 输出的脚本。枚举和 int64/uint64 值现在以字符串形式编码。
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际的 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。
...
**宽限期：** 升级引入了一个宽限期，在升级后 3000 个区块内无 Confirmation PoC，并且在升级周期内对错过率和无效率阈值的要求更为宽松。

有关这些变更的更多详细信息，请参阅治理工件：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md)
...
这意味着，在单个周期内，即使被惩罚，矿工最多只能损失其抵押品的 0.5%。而所需的抵押品仅占日收益的约 24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果该提案未通过，创世时定义的抵押参数将在第 180 周期自动激活，而非上述参数。

投票结束后、第 180 周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 周期起，其收益将被削减至五分之一。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项对 PoC 验证的重大优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性和可扩展性。
...
此升级为`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`添加了工具调用参数（`--enable-auto-tool-choice`、`--tool-call-parser hermes`），并设置了验证阈值`0.958`。
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个为期3000个区块的宽限期，在此期间无确认PoC，并且在升级当期降低了误报率和失效率的阈值。
...
根据AMA期间的讨论以及社区成员提出的论点，建议初始抵押要求较低，并且 slashing 也最小化。

将提交给社区投票的参数：

- 每1单位算力需0.032 GNK（约每H100为10 GNK）
- 误报率或封禁的 slashing 为0.01%
- 无效推理的 slashing 为0.5%

这意味着即使在一个周期内被惩罚，矿工的抵押品损失也不会超过0.5%。所需抵押品仅占日收益的约24%。

提案提交投票后将另行公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世时定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
在当前纪元中观察到cPoC（确认PoC）与PoC之间的重叠。直到纪元的最后一个区块，`is_confirmation_poc_active`被观察为`true`。

此重叠的影响正在评估中。初步观察表明，没有节点记录PoC提交，导致该纪元累积权重为零。

作为预防措施，矿工通过协调行动暂时暂停了区块生产。

问题正在定位中。

请保持在线，以便在补丁发布时能及时应用。更多细节和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已开放**

链上推理访问目前开放，不限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一次链上软件升级 v0.2.10 已开放审查。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约2天。

对于对本次PR审查有实质性贡献的，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审查请求，并非正式投票的开始。治理投票流程将在审查期结束后启动。

**关键变更****[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC验证采样优化**

此升级引入了一种新的PoC验证机制，通过为每个参与者分配固定的采样验证者集合，将复杂度从O(N^2)降低至O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际PoC时间标准化PoC权重**

此升级根据实际PoC耗时标准化参与者的权重，以减少区块时间漂移效应，使权重结果与实际执行时长保持一致。
...
- **[PR #712](https://github.com/gonka-ai/gonka/pull/712)** 要求proto-go生成代码与提交代码一致
- **[PR #711](https://github.com/gonka-ai/gonka/pull/711)** 从链状态获取PoC测试参数
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式资金转移
- **[PR #659](https://github.com/gonka-ai/gonka/pull/659)** 模型分配检查上个纪元奖励
...
- **[PR #727](https://github.com/gonka-ai/gonka/pull/727)**：对#634的后续更新，将响应写入器传递给 `http.MaxBytesReader` 并对齐测试。
...
- **[PR #639](https://github.com/gonka-ai/gonka/pull/639)**：在任务申领路径中添加缺失的return语句。
...
现有主机无需升级其 `api` 和 `node` 容器。更新后的容器版本仅适用于在链上升级完成后加入的新主机。
...
v0.2.10的测试指南和迁移详情请参见[此处](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10)。请仔细审阅。

**兼容性说明**

如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串形式编码而非数字，int64/uint64值现在也以字符串形式编码。
...
在PoC v2激活后，权重分配现在基于当前模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`的实测推理性能。因此，中位GPU权重以及GPU类型间的相对权重比例均已调整。
...
- 如果未来轮次通过治理引入更大或更苛刻的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示出与PoC v2中观察到的GPU类型间相对性能差异一致的结果。

**跟踪器（仪表板）维护者操作**

在更新权重分配生效后，跟踪器（仪表板）维护者可能希望审查第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。

## 2026年2月2日

**网络更新 — 补丁可用**

现已提供补丁以解决导致PoC周期内块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为并安全恢复块生成。
...
- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时应用后续步骤或缓解指令。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入下一纪元的参与者集合中。
...
- 提供其他模型的 ML 节点不参与 PoC v2 权重分配。在支持的情况下，系统可能自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在因 cPoC 处罚而导致奖励减少或排除的情况下，未计入的部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
- 运行其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可能会自动切换至`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在最近的纪元中，大量 ML 节点未获得 PoC 权重。
分析表明，这是由于 PoC 验证参与不足所致。在多个案例中，参与者发布了 nonce，但未执行验证，或验证级别远低于协议要求。
下表列出了上一纪元拥有权重、在当前纪元提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
支持的模型集现已受限。除以下模型外，所有此前支持的模型均已从活跃集中移除：
...
- 认证绕过：将 `epochId` 绑定到签名，以验证是否属于正确的纪元。
...
- 磁盘空间：Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份。请确保有 250 GB 以上的可用空间。
- Postgres：现在可通过 Postgres 配置本地有效负载存储，升级后可用。
...
本指南说明了主机应如何根据 v0.2.8 模型可用性变更及即将推出的 PoC v2 更新来升级其 ML 节点。ML 节点配置在第 155 个周期开始符合 PoC v2 要求。建议主机在该时间点前审查并准备其 ML 节点配置。迁移至 PoC v2 可在第 155 个周期后安排。迁移阶段结束后，未满足配置要求的 ML 节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8` 在迁移期间受支持，但不贡献于 PoC v2 就绪状态或权重分配。参与 PoC v2 需要提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
!!! note "重要"
    ...
    - ML 节点升级必须在迁移完成且 PoC v2 通过 v0.2.8 升级后独立治理提案激活之前完成。
    - v0.2.8 升级本身不会启用 PoC v2。

**3. 检查 ML 节点分配状态（推荐的安全步骤）**

更改模型前，您应检查当前的 ML 节点分配情况。查询您的网络节点管理 API：
```
...
- 第二个布尔值：节点是否计划在下一个 PoC 中提供推理服务

**推荐行为**

- 优先仅在第二个值为 `false` 的节点上更改模型
- 这可降低在 PoC v2 行为仍被观察期间的风险
- 鼓励跨周期逐步 rollout

**4. 更新 ML 节点的模型：仅保留受支持的模型**

建议预先下载模型权重。为避免启动延迟，请将权重预先下载至 `HF_HOME`：
```
mkdir -p $HF_HOME
...
!!! note "治理与 PoC v2 激活说明"
    ...
    **第一阶段：观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML 节点切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为兼容 PoC v2 的版本，才能为 PoC v2 权重做出贡献。
...
PoC v2 仅在链上治理提案获得批准时，才成为权重分配的活跃方法。
    在此提案获得批准之前：
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估向 PoC v2 过渡的准备情况。一旦在活跃 Host 中观察到足够比例的采用率（约 50%），可提交独立的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。

在下一次网络步骤经治理批准后，网络将暂时仅支持 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。

**安全性、正确性与可靠性改进**

- SSRF & DoS：验证 InferenceUrl 以拒绝内部 IP，并添加超时以防止请求挂起。
- 投票翻转：通过拒绝重复项，防止覆盖 PoC 验证结果。
- PoC 排除：修复 getInferenceServingNodeIds，正确排除推理服务节点。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不再被视为 PoC v2 参与资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
- 升级提案于区块 2387000 提出，该区块预计时间为 2026 年 1 月 29 日 06:30:00（UTC）。

请查看并投票，若您为 Host。

**注意事项 1：** 请确保在升级窗口期间保持在线，以便在需要时及时执行后续步骤或缓解措施。
...
**注意：** 升级后，可以将 Postgres 配置为本地负载的存储。
...
- 逐步部署：如果您运行多个 ML 节点，建议您在多个纪元中逐步执行这些更新。
...
该提案将当前的稳定期延长约两周，以便为即将进行的 PoC 变更及相关网络升级进行额外测试。有关新 PoC 开发进展的更多详情，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机建议在多个纪元中逐步执行更新。

**投票范围**

如果获得批准，网络将暂时继续在现有的 `allowlist`（由未表现出非标准硬件行为的主机组成）下运行。

开发人员 `allowlist` 将以相同的偏移量延长，并将持续有效至区块 2459375。
...
- 通过一组预定义的硬件配置模式，从公开可观察的链上数据中推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
`allowlist` 具有固定的过期时间，不会在延长的稳定期后继续生效。一旦 `allowlist` 在区块 2443558 过期：

- 网络将恢复为稳定期之前生效的标准参与规则，或
...
投票结束：2026年1月18日 05:28:01（UTC）。

`Allowlist`过期时间：自动在区块 2443558 处过期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[允许列表](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 处的参与者注册截止时间对齐。
...
该提案已获得批准。此提案定义了一个临时 `allowlist`，反映那些在最近多个轮次中行为保持一致的参与者。

**当前生效的关键变更**

1) 网络将运行一个由以下参与者组成的 `allowlist`：

- 报告的硬件特征与常见观测到的配置模式匹配（过滤后的非标准配置字符串列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))
- 展示的PoC权重不超过同类硬件观测到的权重的150%

2) 此前表现出偏离这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块2,222,222结束。
...
如果获得批准，网络将临时运行一个 `allowlist`，包含在之前轮次中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个轮次中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致性（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，以及
- 观测到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
...
针对近期共识问题的补丁已准备就绪，详细说明将很快发布。
每位活跃主机的参与对网络推进和恢复正常运行至关重要。请保持在线，待说明发布后立即应用更新。
...
问题已定位，正在准备补丁以解决根本原因。后续说明和技术细节将很快发布。
建议主机保持在线并关注更新，补丁发布后可能需要采取后续行动。

## 2026年1月8日

**v0.2.7升级提案：主网上线的创世验证者增强功能**

链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强方案已获批准，并在主网上成功部署。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用过的、持续时间有限的防御机制，现提议重新启用。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理机制控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行通过治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的 payout，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意破坏或压力测试引起的。本提案旨在通过一系列临时措施增强网络在高负载和不利条件下的韧性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内生效。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。

**主要变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——此前曾使用过的、有时间限制的防御机制，现提议重新启用。
- 在网络增长期间提供共识保护。在其先前运行期间：
    - 三位守护验证者共同持有约 34% 的共识投票权
    - 未向守护验证者授予额外奖励
    - 此配置有助于防止边缘情况下的共识停滞
...
此升级正式将此前通过手动 API 更新分发并已在网络上使用的关键修复纳入协议。这些修复包括：

- 纠正失败推理请求的计数错误（包括以不支持格式处理但未标记为完成的请求）
- 改进对失败推理的处理韧性
- 为 `PoCBatch` 和 `PoCValidation` 交易引入批处理。

通过将其纳入协议，该行为将成为全网一致应用的协议级规则。

**临时参与和执行限制**

- 主机级别注册：在区块 2,222,222（约两周后）之前，暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续到区块 2,294,222（约19天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理控制参与者入驻和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。
...
Cosmovisor 在执行更新时会在 `.inference/data` 状态文件夹中创建完整备份，请确保您的磁盘有足够的空间。阅读 [此处](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory) 了解如何安全删除 `.inference` 目录中的旧备份。
如果您的 `application.db` 占用大量空间，可使用 [此处](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it) 提供的技术进行清理。

**参考**

Genesis 验证者增强功能的完整技术细节请参见：
[https://github.com/gonka-ai/gonka/tree/main/proposals/early-network-protection](https://github.com/gonka-ai/gonka/tree/main/proposals/early-network-protection)
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个错误，即不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
