...
!!! note "关于此页面"
    ...
    此页面不保证内容完整。如需获取最新信息，包括治理投票的发布及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。
...
以该格式运行模型需要新版本 API，v0.2.15-post5。安装方法如下：
```bash
...
```
该技能会部署模型，测量 PoC 吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则说明您的设置正确。

如果您希望提供 DeepSeek，请使用上述步骤进行设置。否则，请不要忘记委托，以避免因未参与而受到惩罚。

提醒：明天将发布带有更好日志的另一个 MLNode 构建版本，因此请留意后续更新。

## 2026年8月12日

**DeepSeek V4 Flash 引导（第 359 个周期）**

提案 94 已通过——DeepSeek V4 Flash 已获批准。计划提供该模型的主机应立即声明意向，以便模型能在第 359 个周期启动引导。

[完整系数表](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)

**效果**

从第 359 个周期开始（PoC 启动时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），前提是模型满足引导资格：
...
1. 声明意向。在提案 94 通过后、引导快照（区块 5,536,224，约 2026 年 8 月 13 日 02:40 UTC / 8 月 12 日 19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 在PoC 359开始前的约500个区块安全窗口内（区块5,536,724，约8月13日，03:24 UTC / 8月12日约20:24 PDT），切换您的MLNode。将您的MLNode部署并切换至DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate配置）。在此窗口期内切换是安全的，因为其中没有cPoC。
...
4. 要保留当前模型。在epoch 360之前无需任何操作。从epoch 360开始，如果您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的未参与惩罚。

区块高度是精确的；时间是估算值，将在接近epoch时确认。[更多关于DeepSeek引导的细节](https://gonka.ai/docs/host/deepseek-bootstrap/)。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。现有参数或模型均未修改，唯一的变化是新增模型条目。
...
1. 声明意向：在 `start_poc - deploy_window` 截图前提交 `MsgDeclarePoCIntent`。
2. 预资格快照：链对意向和委托进行快照，并发出建议性预资格事件。
3. 部署窗口：声明意向的主机部署其MLNode。
...
在符合引导资格的前提下，从epoch 359（PoC启动时间约为2026年8月13日03:24 UTC / 8月12日20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731`将成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与惩罚从epoch `360`（`penalty_start_epoch = 360`）开始。

**主机所需操作**

1. 要提供服务：投票结束后声明意向，然后部署您的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留当前模型：在epoch 360之前无需操作；从epoch 360开始，若您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`。
3. 在截止日期前对提案94进行投票。

**如何投票**

如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
**`v0.2.15-devshard-v4.0.1` 运行时升级提案已进入治理**

这是一个非关键性的、仅限 devshard 的升级，独立于完整的链软件升级运行。它将现有的 devshard v4 运行时替换为 v4.0.1。

这只是一个性能和资源管理补丁，仅此而已。协议、推理、验证和经济行为均保持不变。更改包括：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代表您投票，请参阅有关将冷密钥的治理投票权限授予热密钥的指南。
...
鼓励主机查阅 [GitHub](https://github.com/gonka-ai/gonka/pull/1497) 上的提案并参与投票。
...
组合的 **devshard v4 和 v0.2.15 主网升级提案** 现已上链并开放投票。
...
- **devshard v4 运行时** 通过 `versiond` 的 devshard 版本管理机制分发并启动。
- **主网 v0.2.15 二进制文件** 通过标准的链上软件升级流程和 Cosmovisor 激活。

**Devshard v4 变更**

v4 的主要目标是在主机故障和升级时实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个面向多实例高可用的版本：在共享 Postgres 上，通过 versiond-router 后部署 N 个版本化的 /devshardd 副本，使用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，使正在进行的工作（包括 SSE）在旧版本上完成。

v4 还修复了错误和安全问题。

**主网 v0.2.15 变更**

升级的主网部分移除了大量过时代码，包括不再符合当前架构要求的遗留交易处理路径及相关组件。
此清理工作简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。通过移除过时行为并围绕当前支持的架构整合交易处理流程，也为未来的开发和升级提供了便利。


更多详情，请查阅拉取请求：[https://github.com/gonka-ai/gonka/pull/1497 ](https://github.com/gonka-ai/gonka/pull/1497 )
...
同一提案还将 `DevshardEscrowParams.approved_versions` 注册为 devshard v4。
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件提供。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
升级提案 v0.2.14（提案 ID 89）的链上治理投票已结束。

该提案已获批准，并在主网第 5195700 块成功执行升级。
...
devshard 部分准备了 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率并实现 SQLite 与 Postgres 存储之间的安全切换。
...
- 现在/主网升级前——更新您的API和桥接器。为保持主网升级期间以太坊桥接的稳定，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将`api`二进制文件和桥接镜像更新至0.2.14-post3。如果您的`api`二进制文件已更新，您只需更新桥接镜像并重启桥接容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在PoC或cPoC之外执行此步骤。
...
- 投票结束：2026年7月23日00:02 UTC / 2026年7月22日17:02 PDT
- 提议的升级高度：5195700
- 预计升级时间：2026年7月23日03:45 UTC / 2026年7月22日20:45 PDT

**注意**

- 请在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上Cosmovisor对`application.db`的备份通常为数十GB，需提前确认）。有关安全清理`.inference`目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例提供支持（与负载存储相同的环境变量）。本地SQLite将保持默认，并自动清理（保留最近3个周期）。

## 2026年7月20日

**[仅devshard升级的PR现已开放评审](https://github.com/gonka-ai/gonka/pull/1482)**

Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调完整的节点升级，不影响主网行为，也不预期导致推理服务中断。如果通过治理流程批准，新devshard版本将与现有的v3运行时并行运行。

**关键变更**

- v4的主要目标是在主机故障和升级时实现devshard主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐个替换版本。这是系列更新中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4是首个面向多实例高可用的版本：在共享Postgres上通过versiond-router部署N个versioned/devshardd副本，采用粘性会话路由和验证租约独占性。网关仅通过gRPC与链交互。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有保证金所有者通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅sha256变化）时，versiond可执行蓝绿切换并排水，使正在进行的工作（包括SSE）在旧版本上完成。
- v4还包含错误修复和安全补丁。

**操作项**

请评审PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并在发现的问题、疑问、改进建议、边缘情况或潜在漏洞上留下评论。
...
* 在区块**5,105,276**之前（2026年7月17日约12:05 UTC）声明意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第331周期PoC开始前约500个区块的窗口期内（区块**5,105,776**——2026年7月17日12:50:53 UTC / 05:50 PDT），将您的MLNode切换为Kimi。
...
同一提案还从`devshard_escrow_params`中移除`approved_versions`的v1和v2版本。一旦批准，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）。此举消除了旧运行时容器长期存在的内存增长问题——这也是7月15日网络节点OOM事件的根源。
...
2. 为Kimi提供服务的主机：在投票结束**后**、区块**5,105,276**（7月17日，约12:05 UTC）前声明对`moonshotai/Kimi-K2.6`的意向，然后在epoch 331 PoC开始前约500个区块的窗口内（区块**5,105,776**，7月17日，约12:50 UTC）重新启用您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间委托时：**不要将委托给守护节点**；将委托分散到独立的Kimi主机上。有关最新委托指南，请参阅[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`moonshotai/Kimi-K2.6`在epoch 328–329中失去了PoC验证多数（详见[此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将Kimi从活跃集合中移除并重新引导，是使其以最小停机时间恢复的最快方式——与6月提案78的恢复路径相同。

该提案将在下一次PoC前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将在epoch 331开始标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票——紧急窗口很短。
2. 为Kimi提供服务的主机：保持您的Kimi设置就绪，并准备在引导PoC时重新启用您的MLNode。
3. 在引导期间委托时：不要将委托给守护节点；将委托分散到独立的Kimi主机上。
...
两台运行 Kimi 的托管服务器同时失效：

* 其中一台服务器上运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其失效一次性移除了这些投票。
* 第二台服务器因内存不足导致网络节点断开。

Kimi 的验证投票一直接近 2/3 阈值，因此这次失效足以使该组低于多数。托管方的平票决胜机制未启动：当前没有每模型投票权重的托管方会被排除在平票决胜之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况与纪元 306–307 的事件类似。纪元 329 开始时的投票确切细节仍在确认中；本摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导（约 1.5 个纪元）重新添加：

1. [今天] 一项紧急提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。随后将发布单独公告，包含提案 ID、投票命令和截止日期。
2. [明天] Kimi 将再次经历标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布说明。

**委托指导变更——请阅读**

之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将托管节点作为委托目标。此建议现已撤销：

* **请勿委托给托管节点。** 托管节点是 PoC 验证的后备机制。将委托集中于它们会将主机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运行多个节点，请向您自己运行模型的节点委托。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在进行中的修复*** PoC 验证中的托管投票（托管方始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
...
3. 如果您为 Kimi 委托：选择非托管委托方；尽可能将权重分散到多个独立主机上。
...
白名单上的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
v3 运行时准备代理在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。它还改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
将从该分支发布一个独立的 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供推理服务。
...
2. 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成以上两步，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供Kimi服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。一个守护节点也将运行Kimi，因此不希望运行它的主机可以将其权重委托给该节点。
...
> **更新（2026年7月15日）：此建议已撤回——请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。有关更新的委托指南，请参阅 [Multi-Model PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该因子设定为：在8xB300硬件上，Kimi的共识权重约等于主模型MiniMax。此设定并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该因子设定为：在8xB300硬件上，GLM-5.2的共识权重约等于MiniMax。此设定并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2服务的主机，需在 `start_poc - deploy_window` 的启动快照前为相应模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、以及 >2/3 可达性）。这使操作员可在配置硬件前确认模型的可行性。
...
从第309轮开始（PoC开始约6月26日15:25 UTC；生效约16:00 UTC），前提是每个模型均满足启动资格：
...
提案必须在第308轮结束前完成，以便两个模型都能启动进入第309轮。由于每轮约22.8小时，标准48小时投票周期无法适应单轮；而12小时的紧急周期可以。创世守护节点预计支持该提案。

**主机所需操作**

1. **要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，期间切换是安全的，因为该窗口内无cPoC。

2. **要部署GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意图并 provision 您的MLNode。部署是可选的——未选择参与的主机不会受到任何惩罚。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，将无法在本周期获得cPoC，直到切换为止。
* 计划再次提供Kimi的主机应保留其Kimi部署配置，并准备好在PoC 309时将MLNode切换回Kimi——将在第308周期内进行一次投票以恢复Kimi。
...
* **添加GLM-5.2**——添加GLM-5.2，**且不设不参与惩罚**，以便主机可选择性参与，提前衡量模型需求，未选择提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前投票不足，无法通过PoC验证。因此，正在提供Kimi的参与者被移出组，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的最快方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi投票不足无法通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的途径。下一周期将进行投票以恢复Kimi。
* **Qwen3-235B——退役（独立、无关变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的模型阵容调整。此变更与Kimi情况无关，仅因同属PoC阵容调整而一并包含在此处。
...
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设置不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。
...
* 第308个周期将在不久后形成：PoC开始约16:50 UTC，生效约17:25 UTC。
...
- 在周期变更时修剪旧周期存储，将SQLite/Postgres模式设置移出热路径，并强制每个进程仅选择一个存储后端。
...
4. 将每个推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每槽总计数
...
这是首次仅针对devshard的升级，因此流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响mainnet行为，且预计不会导致推理服务中断。
...
MiniMax的按模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
1. 请确保您的桥接容器已更新并同步。以太坊主网桥接合约（`0x972a7a92d92796a98801a8818bcf91f1648f2f68`）、USDC/USDT 代币元数据以及 CW20 `wrapped_token` 代码 ID `105` 均通过升级处理器注册，因此桥接将在升级高度于主网激活。验证说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 的权重从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整 **仅适用于您共识权重中源自 Kimi 的部分**；您源自 Qwen 的权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降低至约 25%，并设置链上最低通过率阈值为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约 ⅓ 的参与率才能满足最低通过率（参见 inference-chain 部分）。
...
- 将 devshard 非负数上限从 `20_000` 提升至 `1_000_000`。
...
**在第 278 个周期（届时开始处罚）前选择 `MiniMaxAI/MiniMax-M2.7` 参与方式**
...
请所有主机验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。如果是这种情况，请先确认您运行的是当前版本，再进行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个纪元）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上不会发生任何变化，升级将不会执行。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何措施。
...
该问题已被追踪至使用非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
4. 预期可能出现临时推理中断。访问可能不会立即在所有地方恢复，预计会通过社区代理和中介端点逐步恢复。
...
社区成员目前正在审查第266轮期间发生的情况。一旦对根本原因有更多清晰信息，将及时分享更新。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不投票的情况下，这使得其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。在这种情况下，请先检查您是否运行的是当前版本，再采取进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并会自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作，该升级可能包括以太坊端合约激活，所有主机需验证其桥接容器已部署、运行最新版本并同步正确。
...
有意义的审阅贡献，包括重要评论、错误发现和安全问题，可能在下一次升级周期中符合社区奖励资格。
...
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级会回填该权限，以便它们能响应交易商投诉。
...
今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保持您的MLNode使用 `Qwen235B`，并于明天为下一轮次提交意向。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 抵押资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
此清理过程**必须在升级前完成**。如果在升级时，您的节点配置包含不受支持的模型，**节点将被拒绝并离线。**

版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留之前强制执行的模型和 Kimi。
每个 DAPI 都会在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置中包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图截断为强制模型来掩盖了此问题，因此即使持久化配置中仍包含额外模型，`/admin/v1/nodes` 仍显示为干净。版本 0.2.12 停止了这种截断，意味着直接加载持久化配置。

为解决此问题，以下脚本会查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个包含清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。剩余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG dealer 共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 保证金资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
新版本的 mlnode 现已发布：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
此更新修复了在 PoC 开始附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
v0.2.11 升级提案的链上治理投票已结束。该提案已获**批准**，并在主网上成功执行升级。

**当前已生效的关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**
...
- **二进制版本**：通过链上升级流程更新。
- **迁移**：测试和迁移细节请参见 [v0.2.11 文档](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.11/docs/upgrades.md)。
...
下一版链上软件 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著提升 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过每个推理的链上交易处理推理会限制吞吐量。此设计将推理执行与验证移至分配的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露于真实网络环境中。某些问题仅靠本地测试难以发现。当前实现已设计为不影响矿工奖励。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中将同时启动大量剪枝活动，造成可避免的操作压力。
...
这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会在同一时间启动大量剪枝活动，造成不必要的运营压力。
...
2. 之前的请求重试系统即使因处理超时（而非TLS超时）失败，也会重试推理。
服务器端对长请求的重试通常无效，因为它会导致相同的超时场景。同时，客户端可能会收到不一致的输出。新版本的API在这种情况下不再重试推理。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC开始时仍有活动请求而导致的潜在权重下降风险。
...
- 确保[抵押](https://gonka.ai/host/collateral/)已正确设置，否则从第180个周期起，PoC所得奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若无替代方案获批，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能发生波动。较小的权重可能经历更大比例的相对波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这可提供操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
在启用PoC持续时间归一化且有效参考窗口比先前名义假设长约5个区块的情况下，自第176轮起的权重值反映了更新的计算模型。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此升级通过实际 PoC 持续时间对 PoC 参与者权重进行标准化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
**宽限期：** 升级后引入了一个3000个区块的宽限期，在此期间无确认 PoC，并且对升级所在纪元的错过率和无效率阈值放宽。
...
这意味着，在单个纪元内，即使被惩罚，矿工最多损失其抵押品的0.5%。而所需的抵押品仅占日奖励的约24%。

**警告：** 抵押参数将无视投票结果而生效。如果此提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。

投票结束后、第180纪元前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180纪元起，其奖励将减少5倍。
...
要投出您的选票（`yes`, `no` , `abstain` , `no_with_veto`）：
```
export NODE_URL=https://node3.gonka.ai/
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级时段内放宽了错过率和无效率阈值。
...
- 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 在升级期间，Cosmovisor会在`.inference/data`目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除`.inference`目录中旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
根据AMA期间的讨论以及社区成员提出的论点，建议初始抵押要求较低，并且惩罚轻微。
...
任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请保持在线，以便在需要时能立即应用补丁。更多细节和补丁说明将在准备就绪后发布。

## 2026年2月12日

**推理功能现已开放**

链上推理访问当前已开放，不限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一次链上软件升级v0.2.10已开放审查。欢迎提供反馈和改进建议。当前计划是将审查窗口保持约2天。

对于对本次PR审查做出有意义贡献的，可能在下一次升级中提出奖励。
...
- 在PoC之外使用标准的vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与PoC v2中观察到的相同GPU类型间的相对性能差异。

**跟踪器（仪表板）维护人员的操作**

在更新权重分配生效后，跟踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保与当前PoC v2的权重分配保持一致。

## 2026年2月2日

**网络更新——补丁可用**

现已提供补丁以解决导致PoC周期内区块验证暂停的问题。建议主机尽快应用补丁，以确保正确的PoC验证行为，并安全地恢复区块生成。

**需要采取的操作**

请主机尽快应用补丁，以确保正确的PoC验证行为，并安全地恢复区块生成。
```
# Download Binary
...
针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并于区块 2451000 成功在主网上执行。此次升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**注意**

- 下一个 PoC 周期（从纪元 158 到 159 的过渡）至关重要。请确保在线，以便在需要时及时应用后续步骤或缓解指令。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不包含在即将到来的纪元的参与者集合中。
...
- 身份验证绕过：将 `epochId` 绑定到签名，以验证是否属于正确周期。
...
本指南说明主机应如何根据v0.2.8模型可用性变化和即将推出的PoC v2更新来更新其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估是否具备向 PoC v2 过渡的条件。一旦在活跃主机中观察到足够水平的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足上述两项条件的节点将不再被视为 PoC v2 参与资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
`allowlist` 具有固定过期时间，不会延续至延长的稳定窗口之外。一旦 `allowlist` 在区块 2443558 过期：
...
2) 此前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直至区块 2,222,222 的稳定窗口结束。
...
此次投票定义了稳定窗口期间参与者的 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为一直符合网络预期的参与者集合。

**投票范围**

如果获得批准，网络将暂时使用一个 `allowlist`，包含在之前周期中未表现出非常规硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏差和不一致（非常规配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))，且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
此前持续偏离这些模式的参与者在稳定窗口结束于区块 2222222 之前不属于 `allowlist`。
...
针对近期共识问题的补丁已就绪，详细说明将 shortly 发布。
每位活跃主机的参与对网络推进和恢复正常运行至关重要。请保持在线，并在指令发布后准备应用更新。
...
问题已识别，正在准备补丁以解决根本原因。后续指令和技术细节将 shortly 发布。
...
- 临时重新激活创世验证者增强——一种此前曾使用、具有时限的防御机制，现提议重新启用。
...
本次升级正式将此前通过手动API更新分发并已在网络中使用的关键修复纳入协议。这些修复包括：
...
- 开发者地址注册：在稳定期间，将暂停新开发者地址的注册。一个预定义的`allowlist`开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理方式控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将获得额外 payout，金额为第117轮奖励的1.083倍，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内生效。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
本次升级将此前通过手动 API 更新分发并已在网络中使用的关键修复正式化。这些修复包括：
...
- 主机级注册：在区块 2,222,222（约两周后）之前，暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块 2,294,222（约19天）。
...
- 所有在第117轮期间活跃的节点，将额外获得等于1.083倍第117轮奖励的支付，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由于一个缺陷：未被支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
