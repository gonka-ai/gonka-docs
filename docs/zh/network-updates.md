...
本提案将 DeepSeek V4 Flash 作为新的 PoC 模型添加到网络中。不修改任何现有参数或模型；唯一的变化是新增该模型条目。
...
模型通过标准引导流程进入，因此托管者可在投入硬件前评估其可行性：

1. 声明意向：在 `start_poc - deploy_window` 截止快照前提交 `MsgDeclarePoCIntent`。
2. 预资格快照：链对意向/委托进行快照并发出建议性预资格事件。
3. 部署窗口：声明意向的托管者部署其 MLNode。
4. PoC 启动：成员资格由在 PoC 启动时提交 PoC 存储提交的人决定，而非仅由声明意向者决定。
...
1. 若要提供该模型：投票结束后声明意向，然后配置您的 MLNode（vLLM 0.25.1 + 发布候选配置）：
```
...
投下您的投票（`yes`，`no`，`abstain`，`no_with_veto`）：
```
...
这只是一个性能和资源管理补丁，除此之外无其他变更。协议、推理、验证和经济行为均保持不变。变更内容：纪元追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪对齐。
...
v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个面向多实例高可用的版本：在共享 Postgres 后端部署 N 个带版本的 /devshardd 副本，由 versiond-router 使用粘性会话路由和验证租约排他性进行管理。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
若获批准，二进制版本将通过链上升级提案进行更新。有关升级流程的更多信息，请参阅 [/docs/upgrades.md.](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.15/docs/upgrades.md)。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的Cosmovisor备份`application.db`通常为数十GB，需提前确认）。有关如何安全删除`.inference`目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例支持（与有效载荷存储使用相同的环境变量）。本地SQLite将保持默认，并自动清理（保留最近3个周期）。
...
主网链/API工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（禁用主网上的`/v1/chat/completions`计费并从API二进制文件中移除嵌入的`/v1/devshard`）、奖励接收方路由以及升级时的安全修复。
...
- 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接镜像升级到 0.2.14-post3。如果您的 `api` 二进制文件已更新，您只需更新桥接镜像并重启桥接容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- v4 的主要目标是在主机故障和升级时实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能处理新请求；在版本升级时，多台机器逐台替换版本。这是系列更新中的首次，该系列更新将重构单体架构，使其具备高可用性、容错性和可扩展性。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上，通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅受信方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可进行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
* 在区块 **5,105,276**（2026 年 7 月 17 日约 12:05 UTC）之前声明意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在周期 331 PoC 开始前约 500 个区块的窗口内（区块 **5,105,776** — 2026 年 7 月 17 日 12:50:53 UTC / 05:50 PDT），将您的 MLNode 切换为 Kimi。
...
同一提案从 `devshard_escrow_params` 中移除 `approved_versions` 的 v1 和 v2。一旦批准，`versiond` 将停止 v1 和 v2 的 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。这消除了旧运行时容器长期存在的 RAM 增长问题 —— 即 7 月 15 日事件中网络节点 OOM 的根源。

**这标志着 v3 切换的最终完成。** 仍在通过 v1/v2 前缀或经典 `/v1/devshard` 路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 进行投票 —— 紧急窗口较短。
2. 提供 Kimi 服务的主机：在投票结束后立即、在区块 **5,105,276**（2026 年 7 月 17 日约 12:05 UTC）之前声明对 `moonshotai/Kimi-K2.6` 的意图，然后在周期 331 PoC 开始前约 500 个区块的窗口内（区块 **5,105,776**，2026 年 7 月 17 日约 12:50 UTC）将您的 MLNode 切换回来。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
...
`moonshotai/Kimi-K2.6` 在第328至329周期失去了PoC验证多数票（详见[此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的方式——与六月提案78的恢复路径相同。

该提案将在下一个PoC之前从PoC参数中移除 `moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi 将从第331周期开始进入标准引导流程。

**主机所需操作**

1. 在截止时间前对提案87进行投票——紧急窗口很短。
2. 运行Kimi的主机：请保持您的Kimi设置处于就绪状态，并准备好在引导PoC时切换回您的MLNode。
...
**截止时间*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01（PDT）/ 2026-07-16 00:01 ~ 2026-07-16 12:01（UTC）（紧急，0.667 赞成阈值；出席率重要，请尽快投票）。
* 引导意向截止时间和周期时间详见引导说明。

## 2026年7月15日

**Kimi-K2.6事件（第328–329周期）：事件经过、恢复计划及委托指南变更**

在第328周期，`moonshotai/Kimi-K2.6` 失去了PoC验证多数票。在第329周期开始时，运行Kimi的主机被移出组。网络其余部分正常运行。

**事件经过**

两台由守护者运营的运行Kimi的服务器同时发生故障：

* 其中一台服务器上运行Kimi的MLNode因提供商端问题而崩溃。大量Kimi委托集中在这台主机上，因此其故障导致这些投票立即消失。
* 第二台服务器因内存耗尽而失去了其网络节点。

Kimi 的验证投票一直接近 2/3 的阈值，因此这足以使该组低于多数。守护者投票的平局解决机制未启动：目前未分配每模型投票权重的守护者已被排除在平局解决机制之外。这是一个已知问题，已在即将发布的 v0.2.14 升级中修复。

这种情况类似于第 306–307 个纪元的事件。第 329 个纪元开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 的最安全方式是通过全新引导程序重新添加它（约 1.5 个纪元）：

1. [今天] 一个加速提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。后续将发布包含提案 ID、投票命令和截止日期的独立公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意向、部署窗口、PoC 存储提交。在意向截止日期前将发布相关说明。

**委托指导变更——请阅读**

之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤销：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主机制与备用机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行模型的主机上。如果您运行多个节点，请将委托分配给运行模型的您自己的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重，且无单点故障。

**已列入计划的修复*** PoC 验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除，并关闭 `/v1/devshard`。为 `api` 和 `versiond` 容器设置独立的内存限制。
...
提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人将流量切换至 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于此类仅限 devshard 的运行时升级，无需重启主网或手动执行主机操作。

**操作事项**

已获白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这使他们能在链升级运行期间继续提供推理服务，而不依赖已弃用的经典 API 路径。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用较早的运行时前缀，直到代理将流量切换到 v3。此类升级期间无需重启主网或手动执行主机步骤。
...
[此拉取请求](https://github.com/gonka-ai/gonka/pull/PR) 针对下一阶段链上软件升级 v0.2.14，现开放审查。
...
devshard 部分准备 v3 运行时，使代理可在链升级期间不依赖已废弃的经典 API 路径提供推理服务，从而提升 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布并部署 devshard v3 版本，然后再进行主网链升级。提前将推理流量切换至 `/devshard/v3` 的代理，可在链升级运行时继续提供推理服务。
...
根据历史数据，此前未发现此类攻击。但在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 的使用行为。

针对即将到来的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复版本 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制的计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 将在第 311 纪元再次尝试引导。希望提供 Kimi 服务的主机需在第 310 纪元声明意向，并在新纪元开始前准备好节点。一个守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 纪元声明 PoC 意向，于区块 **4,797,456** 之前（约 6 月 28 日 12:00 UTC）：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第 311 纪元开始前约 500 个区块的窗口期内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
> **更新（2026年7月15日）：此建议已撤销——请勿向守护节点委托。** 守护节点是 PoC 验证的备用机制，必须与委托保持独立。请选择运行模型的非守护主机，并避免选择已是主要委托目标的主机。有关最新委托指南，请参阅 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一并恢复。该系数设定为使8xB300运行Kimi时的共识权重与主模型MiniMax大致相当。此设定并非偏向任何硬件——MiniMax已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的增益。
...
常规引导流程从第309轮开始。由于投票将在PoC前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
更多关于引导的信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一并添加（此举将有效取消该模型的不参与惩罚）。该系数设定为使8xB300运行GLM-5.2时的共识权重与MiniMax大致相当。此设定并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的增益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 以及 >2/3 可达性）。这使操作者可在配置资源前确认模型的可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由PoC开始时实际提交PoC存储提交的主机决定——而非仅凭意向声明。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮开始（PoC开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是每个模型均满足引导资格：

* `moonshotai/Kimi-K2.6` 将再次成为活跃的PoC模型。
* `zai-org/GLM-5.2-FP8` 将作为可选的PoC模型提供，且无非参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍将作为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型能顺利引导进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在单轮内；而12小时的紧急周期可以。创世守护节点预计会支持本提案。

**主机所需操作**

1. **若要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。

2. **若要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口期间配置您的MLNode。提供GLM-5.2是自愿的——未选择的主机不会受到任何惩罚。
...
如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 获取：任何活跃节点均可使用：
...
* 计划再次提供Kimi的主机应保留Kimi的配置暂存，并准备在PoC 309时将MLNode切换回Kimi——将在第308周期内进行恢复Kimi的投票。

**接下来的内容**

在第308周期内将进行一次紧急投票，合并两项变更，均将于第309周期生效（PoC开始时间约为2026年6月26日15:25 UTC）：
...
`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出群体，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6 —— 移除以便快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的路径。下一周期将进行恢复Kimi的投票。
* **Qwen3-235B —— 退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的模型调整。它与Kimi的情况无关，仅因同为PoC模型调整而一并包含在此处。
* **MiniMax-M2.7 —— 升级为基础模型。**

这些变更合并为一次紧急投票，因为重置必须在第307周期结束前完成。每个周期约22.9小时，标准的48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创世守护者应支持此提案。

**若获通过的效果**

从第308周期开始（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B 和 Kimi K2.6 不再活跃。Kimi的恢复将在下一周期的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前约500个区块的窗口期内切换是安全的，因为此时无cPoC。请现在预先下载FP8权重（如尚未暂存），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时切换回Kimi——将在第308周期内进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，因此主机可选择加入并提前测试对该模型的需求，选择不提供该模型的主机不会受到惩罚。
...
这是首次仅针对devshard的运行时升级。它独立于全链软件升级，无需升级链二进制文件。

提案通过后，v2可与现有的v1 devshard运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有v1流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

该发布将 `devshardd` 二进制文件作为Gonka发布工件发布。`versiond` 将自动下载该二进制文件，验证sha256哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。
...
- 将每次推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每槽总计数。
...
4. 将每次推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每槽总计数
...
该发布将 `devshardd` 二进制文件作为Gonka发布工件发布。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证sha256哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。
...
如果通过治理流程批准，新devshard版本将与现有v1运行时并行运行。

请直接审核该PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。

有意义的审核贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期获得社区奖励。
...
链上治理投票已结束，[v0.2.13升级提案](https://github.com/gonka-ai/gonka/pull/1143)（提案ID `54`）已获**批准**。

升级将在主网**区块高度4267300**（≈ **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）自动执行。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6`和`MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一两个模型的主机，也必须为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元 `278`**。未采取任何操作的主机将从纪元278起，对其全部权重施加15%的每纪元惩罚。
4. 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。请确保 `.inference/data` 有足够的空闲空间用于cosmovisor状态备份；如果 `application.db` 较大，请在升级前应用[cosmovisor备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对之前发布但未通过提案的重新投票，现已提交多项更新。
...
- 治理：将创世守护者投票权降至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%的投票权中达到约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 在vLLM 0.20.1发布后，根据Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B：`0.359`（未变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下Kimi的每轮共识权重减少了约38%）
    - MiniMax-M2.7：`0.3024`
...
- 将创世守护者调整后的投票权降至约25%，并将链上治理法定人数设为`0.25`。若守护者不投票，则剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其网桥容器已部署、运行最新版本且同步正常。部分主机可能已部署网桥容器。在这种情况下，请先确认您运行的是当前版本，再采取进一步操作。
请按照以下说明操作：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

主机需更新仪表板/浏览器。请从`gonka/deploy/join`目录运行以下命令。如果您尚未本地克隆`gonka`仓库，请先参考加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 运营商时间线：投票于5月22日22:12 UTC结束 → 升级高度约为5月26日14:42 UTC → 升级周期剩余阶段跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在 start_poc − 500 块时（约提前43分钟）→ 首个MiniMax PoC阶段在升级后下一个纪元边界开始 → MiniMax惩罚在链纪元278执行。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获得批准，此跳过是预期行为而非故障；新快照逻辑将在下一个纪元开始。
- 若获得批准，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite将保持默认设置并自动清理（保留最近3个纪元）。
- 如果提案失败（未达到法定人数或 `no_with_veto` 超过⅓），链上不会发生任何变化，升级将不会进行。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
纪元 #267 中PoC验证已成功完成，受影响的主机已能重新加入活跃集合。

纪元 #266 的问题由针对运行Kimi模型主机的攻击引起。网络继续运行，但该攻击与若干相关条件共同导致许多参与者无法进入纪元 #266。

在应用额外防护措施期间，推理服务可能暂时不可用。访问预计将逐步恢复，首先通过多个社区代理和经纪人端点。

**发生了什么**

在纪元 #266 中，网络的活跃权重显著下降。

该问题已追踪至具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并干扰了它们的PoC参与。
...
在相关防护措施应用期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过多个社区代理和经纪人端点。
...
2. 监控接下来几个周期的PoC参与情况。确保您的节点如预期般进入PoC，且活跃权重正确反映。
...
- 包括：为Kimi（`0.78`）重新校准权重，新增模型 `MiniMaxAI/MiniMax-M2.7`，更新验证阈值，重构devshard存储，以及多项PoC/奖励修复，以太坊桥接主网激活。
...
- 在本次升级周期剩余时间内禁用确认PoC，以便新快照逻辑从下一个周期干净启动。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再采取进一步操作。
请按照以下说明操作：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意从升级高度到升级周期结束（3000区块宽限期）跳过确认 PoC。若获得批准，此跳过属预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 仍将为默认选项，并会自动清理（保留最近3个周期）。
...
部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再采取进一步操作。
...
本次仅为拉取请求的审查呼吁，不启动正式投票。治理投票流程将在审查期结束后开始，很可能于明天开始。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推理即可再次使其失效。现在在重新激活和即将升级时会重置该计数器。
- 在v0.2.12之前加入的DAPI在其冷到暖的授权中未包含`MsgRespondDealerComplaints`。升级会补全该权限，以便它们能够响应经销商投诉。
...
- 由于旧的托管数据长期保留在一个SQLite存储中，devshard存储可能无限增长。现在存储按纪元范围管理，并在后台清理旧纪元，仅保留最近3个纪元。
...
评审者可在此处查阅完整的升级提案、迁移详情、测试摘要和建议流程：
...
在api容器版本v0.2.11、v0.2.12和v0.2.12-api-post2中存在潜在问题。容器重启后，9100、9200和9400端口上的服务器可能延迟启动，导致api激活延迟，部分矿工因此跳过确认PoC。
...
在Kimi-K2.6引导期间，主机观察到30%的最低直接参与阈值在实践中难以达成。为避免Kimi-K2.6在未来纪元中失去资格的风险，并简化更多模型的接入，建议将该阈值降低至10%。
...
**对已提交 PoCIntent 的节点需采取操作：部署 `Kimi K2.6`**
...
预评估与 PoC 开始之间有 500 个区块的窗口期。在此期间无 CPoC 任务，因此已声明意向的节点可安全地将其模型节点切换至 `Kimi K2.6`。
...
- **协议加固**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 经销商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
每个 DAPI 都会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机采样用于单个 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 交易方共识，使遗留验证者 slashing 与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
如果您没有直接访问持有投票权的密钥，或希望使用其他密钥代您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位熵，此前为 32 位），将通过独立治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
此更新修复了在PoC开始附近的一个边缘情况，此前在特定条件下可能导致性能下降。
...
这些性能提升使每个区块的推理次数最多可提高100倍，具体取决于工作负载和网络条件。
...
目前，通过每个推理的链上交易处理推理限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时开始剪枝活动，从而造成可避免的操作压力。
...
目前，通过每项推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中，某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
强烈建议主机在升级前按照提供的说明剪枝 `application.db`。
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时开始剪枝活动，造成不必要的运营压力。
...
如果您无法访问持有投票权的冷钱包密钥，则建议提前安排投票委托。请联系该密钥的所有者，请求授权您代表其投票。若无此授权，其他账户无法提交投票。
...
2. 以前的请求重试系统即使因处理超时（而非 TLS 超时）失败也会重试推理。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低PoC启动时仍处于活跃状态的请求导致权重可能下降的风险。
...
如前所述，Epoch 180的抵押激活独立于本次投票。

由于提案未通过，创世时定义的抵押参数将在Epoch 180自动生效。

参与者应：

- 审查创世时定义的抵押参数。
- 在Epoch 180之前准备并存入所需的GNK。
- 确保[抵押](https://gonka.ai/host/collateral/)设置正确，否则从Epoch 180起，PoC衍生奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押化PoC权重模型的一部分。治理仍是调整参数的机制，但若无其他方案获批准，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各纪元间可能波动。较小的权重可能经历相对更大的波动。为避免在纪元边界出现临时抵押不足，建议在抵押水平仍相对较低时，存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
由于有效归一化窗口比早期名义模型长约5个区块，每个纪元重新计算的权重相应降低。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移影响，并使权重结果与实际执行时长保持一致。
...
**宽限期：** 升级后引入了一个3000区块的宽限期，在此期间不进行确认 PoC，并对升级所在纪元的错过率和无效率阈值放宽要求。
...
这意味着，在单个纪元内，即使被惩罚，矿工的抵押品损失也不会超过0.5%。所需抵押品仅占日收益的约24%。

**警告：** 抵押参数将无视投票结果而生效。如果此提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。

投票结束后、第180纪元之前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180纪元起，其收益将被削减5倍。
...
下一版链上软件 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级时段放宽了错过率和无效率的阈值。
...
根据AMA期间的讨论以及社区成员提出的论点，建议初始抵押要求较低， slashing也最小化。

将提交供社区投票的参数：

- 每1单位算力需0.032 GNK（约每H100为10 GNK）
- 错过率或封禁的slashing为0.01%
- 无效推理的slashing为0.5%

这意味着，在单个周期内，即使被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押仅占日奖励的约24%。

提案提交投票后，将另行发布通知。

警告：抵押品将无论提案投票结果如何均生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提案。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在有充分理由时适用。如果证明稳定性良好，逐步将抵押品增加至代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请在GitHub上审阅v0.2.10升级PR并提供反馈。有意义的审阅贡献可能会在下一次升级中获得奖励。
...
在当前纪元中观察到cPoC（确认PoC）与PoC的重叠。在纪元最后一个区块之前，`is_confirmation_poc_active`被观察为`true`。
...
此升级根据实际PoC耗时标准化参与者的权重，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
2. 在社区审查该PR后，预计将从该分支创建v0.2.10版本，并可提交该版本的链上升级提案，从而启动正式的治理投票流程。
...
在更新的权重分配生效后，跟踪器（仪表板）维护人员可能希望审查第159期及之后的系数，以确保与当前PoC v2的权重分配保持一致。
...
由于在当前的PoC周期中，验证阈值可能无法达成的高风险，主机们集体采取了预防性措施，暂停了区块验证。
...
- 下一个 PoC 周期（从纪元 158 到 159 的过渡）至关重要。请确保在线，以便在需要时能够及时应用后续步骤或缓解措施。
...
- 提供其他模型的 ML 节点不参与 PoC v2 权重分配。在支持的情况下，系统可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型
...
下一个链上软件版本v0.2.9的升级提案现已上链并开放投票。若获批准，该提案将启用PoC v2用于权重分配，并通过链上治理完成向传统PoC机制的过渡。
...
- 认证绕过：将 `epochId` 绑定到签名，以验证是否属于正确周期。
...
本指南说明了主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估过渡到 PoC v2 的准备情况。一旦在活跃 Host 中观察到足够水平的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
- 升级提案于区块 2387000 提出。该区块的预计时间是 2026 年 1 月 29 日 06:30:00 UTC。
...
该提案已获批准。此提案定义了一个临时 `allowlist`，反映那些在最近多个轮次中行为保持一致的参与者。

**当前生效的关键变更**

1) 网络将使用一个 `allowlist` 运行，该 `allowlist` 由以下参与者组成：
...
2) 之前表现出偏离这些模式的参与者将被排除在 `allowlist` 之外，直到区块2,222,222的稳定窗口结束。
...
此投票定义了稳定窗口期间的参与者 `allowlist` ([https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv))，反映那些行为一直符合网络预期的参与者集合。

**投票范围**

若获批准，网络将暂时使用一个 `allowlist`，包含在先前轮次中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个轮次中满足以下条件的参与者：
...
- 临时重新激活创世验证者增强——一种此前曾使用过的、有限时长的防御机制，现提议重新启用。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。白名单内的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是由故意破坏或压测系统的行为引起的。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强最初是在网络早期阶段作为临时防御机制引入的，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
- 主机级别注册：在区块2.222.222（约两周后）之前暂停新主机注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者地址注册和推理执行，将持续到区块2.294.222（约19天）。
...
此 `API` 版本改进了对失败推理的处理韧性，并减少了推理遗漏统计问题。它还为 PoCBatch 和 PoCValidation 交易引入了批处理功能。
