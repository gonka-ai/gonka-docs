...
!!! note "关于本页面"
    ...
    本页面的内容不保证完整。如需获取最新信息，包括治理投票的发布及其当前状态，请参阅链上数据或查看可用的浏览器和仪表板。
...
该提案已被批准。它将 `weight_scale_factor` 的 `deepseek-ai/DeepSeek-V4-Flash-0731` 从 0.214 提高至 0.246。所有其他模型和链参数保持不变。这是提案 97 的重新提交。链上，提案 97 最终结果为 `PROPOSAL_STATUS_REJECTED`，且 `failed_reason`：`proposal did not get enough votes to pass`（未达到 25% 的最低投票门槛）。最终投票结果：赞成 15452，弃权 3093，反对 0，否决 0。

原始的 0.214 因子来源于被高估的 nonce/min 数值。测量修复见 [gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需执行任何操作。

感谢所有参与投票的人员。

## 2026年8月25日

**仅用于 devshard 的升级 PR 现已开放供审查**
...
1. v5 的主要目标是高度同步：每个主机和排序器在保证金日志中维护一个已签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard 现在可以明确知道 cPoC 正在运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前携带时间戳字段的消息中。后续版本可将诸如 `confirmed_at` 和 `started_at` 等壁钟字段替换为高度标记。这解决了时间提前攻击问题，并为在协议层面处理流超时打开了通道。本次发布包含这些标记，但尚未将超时和密封决策切换至它们。

3. 增加了通过所有插槽的往返心跳机制，因此安静会话仍能定期同步高度，而繁忙会话则可通过常规推理标记实现相同的周期。

**行动项**

请审查 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并在发现任何问题、疑问、改进建议、边缘情况或潜在漏洞时留下评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格在下一次升级周期中获得社区奖励。

本次仅为请求 PR 审查，不启动正式投票。
...
该提案已被批准。它调整了在小样本上无效推理检查的行为，以避免在 Epoch 初期数据不足时误触发。主机无需执行任何操作。

感谢所有参与投票的人员。

## 2026年8月20日

**提案 96 正在开放投票：稳定小样本下同周期内无效 SPRT**

一个新的链上治理提案（提案编号 96）正在开放投票。

**其作用**

在周期早期，数据量尚不足，因此无效推断检查（SPRT）可能在样本足够大之前，仅因少量无效数据就被触发。本提案将其稳定化：

- 将 `invalidation_h_threshold` 提升至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本情况下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会被惩罚。真实违规者仍会触发它（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效项，且在 500 个样本时，观察到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个加速提案，投票将于 **2026年8月21日 06:04 UTC**（约12小时后）结束。如能投票，请务必参与。若未能在时限内达到加速阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的常规周期。若转换为常规提案，您需重新投票。
...
此仪表板从网关视角展示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何惩罚或罚没，但仍应作为协议开发过程的一部分进行调查。

具有严重错误率的主机应在结算开始前解决这些问题。无显著问题的主机无需采取任何操作。
...
**2) 在此 24 小时窗口后的第一个周期。** 将在当前网关上手动启动结算，首先针对较小模型。此过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 随后的 1-3 天内。** 将监控结算结果。

**4) 一旦此阶段稳定。** 其他经纪人可部署稳定版网关，并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至 100%。

该部署过程故意缓慢进行，以便在结算扩展到整个网络之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在 [提案 94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链周期 360 起，`deepseek-ai/DeepSeek-V4-Flash-0731` 与 MiniMax-M2.7 和 Kimi K2.6 一同作为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后由 [提案 98](#august-29-2026) 提升至 0.246）转换为共识权重。

针对 DeepSeek 的每模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需额外操作——现有设置继续有效。尚未做出选择的主机请尽快选择，以避免每周期惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
一个请求：请逐步部署。请勿一次性切换所有节点。先更新少数节点，确认其稳定后，再逐步更新其余节点。在验证新版本的同时保持其余节点正常运行，是最安全的方式。

MLNode 镜像在 [`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 的 `vllm-0.25.1-upgrade` 分支上固定为 3.0.16。

默认的 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是相同的 MLNode 3.0.16，只是为 CUDA 12.9 构建。两个标签都在同一个 compose 文件中。
...
将正在运行的社区销售合约和所有包装代币实例迁移到新存储的 CosmWasm 代码，并为未来的实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举缓解了安全报告中发现的理论风险，预计不会影响正常操作。
创世守护者预计会支持该提案。
...
`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时错过预资格。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 和 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查是 epoch 360 启动快照——在此前请保持委托的多样性，以确保快照有效。
...
提醒：明天将发布带有更好日志的另一个MLNode构建版本，因此请期待后续简短通知。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359启动前约500区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口期内切换是安全的，因为期间无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4而非fp8 + fp4的模型，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署时，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；请将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
2. 预资格快照：链对意向/委托进行快照，并发出建议性预资格事件。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 若要保留当前模型：在第360个纪元前无需操作；从第360个纪元起，若您未提供该模型，请提交`PoCDelegation`或`PoCRefusal`以用于DeepSeek。
3. 在截止日期前对提案94进行投票。
...
这只是一个性能和资源管理补丁，除此之外无其他变更。协议、推理、验证和经济行为均保持不变。变更内容：纪元追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪保持一致。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参考从冷密钥向热密钥授予治理投票权限的指南。
...
v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond / devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
devshard 部分准备 v3 运行时，使代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。
...
- 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者——本次升级未移除或重构任何现有查询端点。若您的仪表板从链上读取并显示数据，则将继续正常运行。您需要理解一项语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 请确保在升级窗口期间保持在线，以便及时执行任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网 `application.db` 的 Cosmovisor 备份通常为数十 GB，建议提前确认）。有关如何安全清理 `.inference` 目录中旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 若获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，且会自动清理（保留最近 3 个周期）。

## 2026年7月20日

**[仅 devshard 的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放评审**

devshard 升级独立于主区块链进行。它们无需通过 Cosmovisor 协调完整节点升级，不影响主网行为，且预计不会导致推理服务中断。若通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍可继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是系列 devshard 和网络节点变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个专为多实例高可用设计的版本：在共享 Postgres 上，通过 versiond-router 部署 N 个 versioned/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
* 在区块 **5,105,276** 之前声明意向（7月17日，约12:05 UTC）：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在纪元331的PoC开始前约500个区块的窗口期内（区块 **5,105,776** — 7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
...
2. 运行Kimi的主机：在投票结束后、区块 **5,105,276**（7月17日，约12:05 UTC）之前立即声明对`moonshotai/Kimi-K2.6`的意向，然后在纪元331的PoC开始前约500个区块的窗口期内（区块 **5,105,776**，7月17日，约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
...
- 意向截止时间（纪元330）：区块 **5,105,276** — 7月17日，约12:05 UTC（约05:05 PDT）。
- 纪元331 PoC开始：区块 **5,105,776** — 7月17日 12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6`在纪元328–329中失去了PoC验证多数（详见[事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式 —— 与6月提案78采用的恢复路径相同。
...
2. 运行Kimi的主机：保持Kimi设置就绪，随时准备在引导PoC时切换您的MLNode。
...
投出您的选票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered` 和 `--timeout-duration` 标志需要 `inferenced` 或更高版本（v0.2.13+）。
```
export NODE_URL=https://node3.gonka.ai/
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（加速，0.667 赞成阈值；出勤率重要，请尽快投票）。
* 引导意图截止日期和时段详情见引导说明。

## 2026年7月15日

**Kimi-K2.6 事件（时段 328–329）：发生了什么、恢复计划以及委托指导的变更**
...
Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者决胜机制未启动：目前没有模型投票权重的守护者被排除在决胜机制之外。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。
...
1. [今天] 一个加速提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。随后将发布包含提案 ID、投票命令和截止日期的单独公告。
...
* 网络节点的内存增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦加速的 Kimi 提案上链，请投票（公告随后发布）。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即迁移 devshard 流量到 `/devshard/v3`。这将修复内存增长问题，并在移除 v1/v2 前必需完成。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可以继续使用较早的运行时前缀，直到代理将流量切换到 v3。在此类升级期间，无需进行主网重启或手动主机操作。
...
节点二进制文件通过链上软件升级提案进行升级。作为升级的一部分，现有主机无需手动更新其 `api` 或 `node` 容器。
将从该分支发布一个独立的 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级运行期间继续提供推理服务。
...
2. 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，您只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人 —— 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 发布版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 —— 准备好调整指标的统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近结束时另行发布。
...
根据历史数据，此前未观察到此类攻击。但在当前周期中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 正在使用此攻击。

针对即将发布的 v0.2.14 链升级，已准备修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证以阻止复制计算通过采样检查，从而阻断攻击。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将节点切换至 Kimi（PoC 开始于约6月28日 ~12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数设定使得在8xB300硬件上运行 Kimi 时，其共识权重与主模型 MiniMax 大致相当。此设定并非为了偏向任何硬件——MiniMax 上已具备相同的权重。在8xB300主机上运行 Kimi 而非 MiniMax 可获得约1%的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数设定使得在8xB300硬件上运行 GLM-5.2 时，其共识权重与 MiniMax 大致相当。此设定并非为了偏向任何硬件。在8xB300主机上运行 GLM-5.2 而非 MiniMax 可获得约2%的增益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3 可达性）。这使操作员可在配置硬件前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在 PoC 开始时实际提交 PoC 存储提交的主机决定——而非仅凭声明意向。只有满足资格条件的模型才会被激活。
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中引导。每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计将支持该提案。

**主机所需操作**

1. **要提供 Kimi：** 在投票结束**后立即**声明 `moonshotai/Kimi-K2.6` 意向。然后在 PoC 309 中将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，期间切换是安全的，因为该窗口内无 cPoC。
...
* 计划重新使用Kimi的主机应保留Kimi的部署环境，并在PoC 309时准备好将MLNode切换回Kimi——将在第308周期内进行一次恢复Kimi的投票。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，以便主机可选择性参与，提前衡量模型需求，未选择部署的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`当前投票不足，无法通过PoC验证。因此，提供Kimi的参与者被移出组别，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是最快恢复且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi的投票不足，无法通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的方式。下一周期将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立无关变更）。** 退役较旧的Qwen3-235B是独立的、此前已请求的模型阵容调整，与Kimi情况无关，仅因同属PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 提升为基础模型。**

由于重置必须在epoch 307结束前完成，因此将所有变更合并为一次紧急投票。每个epoch约为22.9小时，标准的48小时投票周期无法容纳在一个epoch内；而12小时的紧急周期可以。Genesis守护者应支持本提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请提前下载FP8权重（如尚未准备就绪），以避免在epoch边界时遭遇Hugging Face速率限制。
2. **请在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——epoch 308中将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 在epoch 308中进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不对选择不提供该模型的主机施加任何惩罚。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：
...
有意义的审核贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期中获得社区奖励。
...
MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需任何额外操作——现有设置将继续有效。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
3. 升级完成后，每个主机都需要为**每个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，也必须为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会遭受每纪元 15% 的全权重惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。请确保 `.inference/data` 有足够的空闲空间用于 cosmovisor 状态备份；如果 `application.db` 较大，建议在升级前应用 [cosmovisor 备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 的值从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅影响您共识权重中 Kimi 相关的部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。如果守护者弃权，非守护者需在剩余 75% 中实现约 1/3 的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将 devshard 非 nonce 限制从 `20_000` 提升至 `1_000_000`。
...
请所有主机验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。此时，请先确认您运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

请主机更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先参考加入网络指南。此仪表板更新仅涉及容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 区块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后的下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最后 3 个周期）。
...
在第267轮中，PoC验证成功完成，受影响的主机已能重新加入活跃集合。
...
2. 监控下一阶段的PoC参与情况。确保您的节点如预期进入PoC，且活跃权重正确反映。
...
社区成员正在审查第266轮期间发生的情况。一旦对根本原因有更清晰的了解，将分享更新。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理门槛设为 `0.25`。在守护者不参与投票的情况下，剩余 75% 投票权的有效门槛为 1/3（`0.25 / 0.75 = 0.334`）。
...
**在第 278 周期前的 `MiniMaxAI/MiniMax-M2.7` 参与选择**
...
所有主机需验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。若如此，请先确认您运行的是当前版本，再执行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级将补全该权限，以便它们能够响应交易商投诉。
...
- Devshard 现在支持更大部署的共享存储选项。它可以使用 Postgres 作为主存储，SQLite 仍作为本地备用存储。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能启动延迟。这延迟了 api 激活，导致一些矿工因延迟跳过确认 PoC。
...
在上一个周期中，发现Kimi-K2.6在解析某些响应时存在一个小bug。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实践中难以满足。为避免Kimi-K2.6在未来周期中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。

安全模型保持不变：PoC验证本身未更改，仍需超多数验证算力才能接受结果。

该提案被加速，以便在下一个PoC之前生效。投票将持续12小时。
...
该过程由网络各主机协调完成：基础设施已准备就绪，意图已提交，委托与拒绝已设置，部署已测试。
...
转移代理 `node1`、`node2` 和 `node3` 已被禁用。所有主网推理现在均通过 `node4` 路由，该代理采用新的 `devshard` 计费方式。

这标志着网络的重要里程碑：`devshard` 已上线并具备生产就绪能力。`node4` 将作为今后推荐的公共网关。

**需执行操作**：请将您的端点更新为 `node4`。

## 2026年5月2日

今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机。请保留您的MLNode使用 `Qwen235B`，并于明天为下一个周期提交您的意图。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 抵押资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
有关这些更改的更多详情，请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
- **协议加固**。对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间验证。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 经销商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 确保冷钱包账户持有足够数量（例如 100 GNK）以覆盖自动授予的费用限额支出。
...
更新修复了在 PoC 开始附近的一个边缘情况，此前在某些条件下可能导致性能下降。
...
这些性能改进可使每个区块的推理数量提升高达 100 倍，具体取决于工作负载和网络条件。
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著改善 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。提出此版本并非因其已完备，而是因为此类系统需要尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中可能同时出现大规模剪枝活动，造成不必要的运营压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露于真实网络环境中，部分问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作至关重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时启动大量剪枝活动，造成不必要的运营压力。
...
1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格 5 分钟超时，而某些请求可能需要更长的时间。新版本的 API 将此值恢复，而非强制执行严格限制。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低PoC开始时仍处于活跃状态的请求可能导致的权重下降风险。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际的 PoC 经过时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。

    **启用 Qwen235B 工具**

    此升级为 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 添加了工具调用参数（ `--enable-auto-tool-choice` , `--tool-call-parser hermes` ），并将验证阈值设为 0.958。
...
**宽限期：** 升级后引入了 3000 个区块的宽限期，在此期间不进行确认 PoC，并对升级当期的错过率和无效率阈值放宽要求。
...
这意味着，在单个周期内，即使被惩罚，矿工最多只能损失其抵押物的 0.5%。而所需的抵押物仅占日收益的约 24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果该提案未通过，创世时定义的抵押参数将在第 180 个周期自动生效，而非上述参数。

投票结束后、第 180 个周期前，每位矿工必须遵循[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 个周期起，其收益将被削减至 1/5。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个为期3000个区块的宽限期，在此期间无确认PoC，并降低了升级所在纪元的错过率和无效率阈值。
...
根据AMA期间的讨论以及社区成员提出的论点，建议初始抵押要求较低，并削减幅度最小。
...
这意味着，在单个纪元内，即使被惩罚，矿工最多损失其抵押物的0.5%。所需抵押物仅占每日奖励的约24%。

提案提交投票后将另行发布公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世时定义的抵押参数将在第180个纪元自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在合理理由下适用。如果证明稳定性良好，逐步将抵押品提高到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
链上推理访问现已开放，不限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
此升级通过实际 PoC 持续时间对 PoC 参与者权重进行标准化，以减少区块时间漂移效应，并使权重结果与真实执行时长保持一致。
...
1. 活跃主机请在 GitHub 上审查此提案并留下反馈。
2. 在社区审查该PR后，预计会从该分支创建v0.2.10版本，并提交该版本的链上升级提案，以启动正式的治理投票流程。
...
- 如果未来通过治理引入更大或更苛刻的模型（例如 DeepSeek V3.2），可能会观察到不同的性能特征。
- 在 PoC 外部使用标准的 vLLM 基于 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型执行的控制推理基准测试，显示出与 PoC v2 中观察到的 GPU 类型之间相同的相对性能差异。

**跟踪器（仪表板）维护者操作**

在更新权重分配生效后，跟踪器（仪表板）维护者可能希望审查其在第 159 期及之后的系数，以确保与当前 PoC v2 权重分配保持一致。
...
所有主机必须随时准备安装补丁。
请保持在线并密切关注公告。一旦补丁准备好，将立即分享进一步的说明。
...
- 下一个 PoC 周期（从第 158 轮到第 159 轮的过渡）至关重要。请确保在线，以便在需要时及时应用后续步骤或缓解指令。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）轮次并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将到来的轮次的参与者集合中。
...
有关如何将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、升级 ML 节点镜像以及移除其他模型的指南，请参阅 [常见问题解答](https://gonka.ai/FAQ/#how-to-switch-to-qwenqwen3-235b-a22b-instruct-2507-fp8-upgrade-ml-nodes-and-remove-other-models)。
...
- 提供其他模型的 ML 节点不参与 PoC v2 权重分配。在支持的情况下，可能会自动切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
!!! note 
    无法同时满足上述两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置在第155个周期开始符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
!!! note "治理与 PoC v2 激活说明"
    ...
    **阶段 1. 观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML 节点切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为兼容 PoC v2 的版本，才能参与 PoC v2 权重贡献。
...
作为 v0.2.8 升级的一部分，支持的模型集已更新。所有先前支持的模型均从活动集中移除，但以下模型除外：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML 节点版本，可用于评估过渡到 PoC v2 的准备情况。一旦观察到活跃主机中达到足够的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
- 身份验证绕过与重放：将 epochId 绑定到签名，并验证授权是否针对正确的纪元。

由于变更量较大，此处仅突出显示部分项目。更多更新和修复的完整列表请参见 [GitHub 拉取请求](https://github.com/gonka-ai/gonka/pull/539)。
...
- 将 ML 节点升级到支持 PoC v2 的版本：
    - ghcr.io/product-science/mlnode:3.0.12
    - ghcr.io/product-science/mlnode:3.0.12-blackwell

仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML 节点不足以参与 PoC v2。一旦网络切换到单模型配置，未满足上述两项条件的节点将不具备参与 PoC v2 的资格。ML 节点升级必须在 PoC v2 通过治理完全启用前完成。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个周期内逐步执行更新。
...
本次投票定义了稳定期内参与者的 `allowlist` ([https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv))，反映那些行为持续符合网络预期的参与者集合。

**投票范围**

若获得批准，网络将临时使用一个 `allowlist`，包含在之前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：
...
- 观测到的PoC权重低于使用同类硬件的其他参与者所展示的权重的150%。
...
- 临时重新激活创世验证者增强——这是一种此前曾使用过的、限时防御机制，现提议重新激活。
...
- 开发者地址注册：在稳定期间，新开发者地址的注册将暂停。预定义的`allowlist`开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本次升级中包含的预备变更，为未来通过治理机制控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在前两个月运行。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。

**关键变更****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前使用过的、有时间限制的防御机制，现提议重新启用。
- 在网络增长期间提供共识保护。在其先前运行期间：
...
本次升级将此前通过手动 API 更新分发并已在网络中使用的重大修复正式纳入协议。这些修复包括：
...
本次升级包含的预备性变更，为未来通过治理机制控制参与者入驻和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需经额外治理投票决定。
