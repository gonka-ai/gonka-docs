...
如果仍有节点使用旧版 MLNode（服务于 Kimi 或 MiniMax 的节点，因为任何已运行 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。先更新少量节点，确认其稳定性后，再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。

MLNode 镜像已在 `docker-compose.mlnode.yml` 分支的 [`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 中锁定为 3.0.16。
...
将在线的社区销售合约和所有包装代币实例迁移至新存储的 CosmWasm 代码，并注册该包装代币代码以供未来实例化。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中识别出的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。
...
**DeepSeek V4 Flash 启动：意向阈值已达成——请在第360个周期快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在第359个周期快照时未满足预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次 **官方** 检查是第360个周期启动快照——在此前请保持分散的委托，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始前约 500 个区块的部署窗口内（区块 **5,552,115**）部署 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。

**保留其他模型的主机**

在区块 **5,551,615** 之前，提交委托（有助于启动可达性）或拒绝。从第 **360** 个周期起，DeepSeek `penalty_start_epoch` 将生效——未明确选择的主机将面临不参与处罚。
...
**关键时间点*** 第360个周期快照（`start_poc − deploy_window`）：区块 **5,551,615** —— 约为 2026年8月14日 01:24 UTC
* 第360个周期 PoC 开始：区块 **5,552,115** —— 约为 2026年8月14日 02:09 UTC
* DeepSeek 不参与处罚从第 **360** 个周期开始

有关引导工作原理的更多信息：[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)
...
以该格式运行模型需要新的 API 版本 v0.2.15-post5。安装方法如下：
```bash
...
建议逐步将节点切换到新 API，而不是一次性全 fleet 切换。
...
```
该技能会部署模型，测量 PoC 吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。如果通过，即可正常运行。

如果您打算提供 DeepSeek，请使用上述步骤进行设置。如不打算提供，请不要忘记委托，以免遭受未参与惩罚。

提醒：明天将发布带有更好日志的另一个 MLNode 构建版本，敬请留意后续更新。

## 2026年8月12日

**DeepSeek V4 Flash 引导（第 359 轮）**

提案 94 已通过——DeepSeek V4 Flash 已获批准。计划提供该模型的主机应立即声明意向，以便模型在第 359 轮启动引导。

[包含系数的完整表格](https://docs.google.com/spreadsheets/d/1Tw4V7xEXR2p5MbCHqzqjS9vHXQ0eI1IHVXC6guEHnio/edit?gid=0#gid=0)

**效果**

从第 359 轮开始（PoC 启动时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），前提是模型满足引导资格：

- `deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的 PoC 模型。
- `MiniMaxAI/MiniMax-M2.7` 仍为基线模型。
- `moonshotai/Kimi-K2.6` 仍保持可用且不变。

**主机所需操作**

1. 声明意向。在提案 94 通过后、引导快照（区块 5,536,224，约 8 月 13 日 02:40 UTC / 8 月 12 日 19:40 PDT）之前，请提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的 MLNode。在 PoC 359 启动前约 500 区块的安全窗口内（区块 5,536,724，约 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT），配置并切换您的 MLNode 至 DeepSeek V4 Flash（vLLM 0.25.1 + release-candidate 配置）。在此窗口内切换是安全的，因为其中没有 cPoC。
注意：在 Blackwell GPU 上，为获得最佳性能，您可以使用重新打包为 fp8 + nvfp4 而非 fp8 + fp4 的模型，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。部署该模型需等待引导前发布的更新版 API 二进制文件。
3. 如果您委托。请勿委托给守护节点；请将委托分散至独立的 DeepSeek 主机。参见 [多模型 PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 如需保留当前模型。在第 360 轮前无需任何操作。从第 360 轮起，若您未提供 DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用 15% 的未参与惩罚。

区块高度是精确的；时间是估算值，将在接近纪元时进一步确定。[有关 DeepSeek 引导的更多详细信息](https://gonka.ai/docs/host/deepseek-bootstrap/).
...
本提案将 DeepSeek V4 Flash 作为新的 PoC 模型添加到网络中。未修改任何现有参数或模型；唯一的变化是新增了该模型条目。
...
该模型通过标准引导流程进入，因此主机可在投入硬件前查看其可行性：

1. 声明意向：在 `start_poc - deploy_window` 截止快照前提交 `MsgDeclarePoCIntent`。
2. 预资格快照：链对意向/委托进行快照并发出建议性预资格事件。
3. 部署窗口：声明意向的主机配置其 MLNode。
4. PoC 启动：成员资格由在 PoC 启动时提交 PoC 存储提交的节点决定，而非由声明意向的节点决定。
...
在符合引导资格的前提下，从第 359 个纪元（PoC 启动时间约为 2026 年 8 月 13 日 03:24 UTC / 8 月 12 日 20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的 PoC 模型。现有模型保持不变。DeepSeek 的不参与惩罚从第 `360` 个纪元（`penalty_start_epoch = 360`）开始。

**主机所需操作**

1. 为提供该模型：投票结束后声明意向，然后配置您的 MLNode（vLLM 0.25.1 + 发布候选配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留您的当前模型：在第360轮之前无需任何操作；从第360轮开始，如果您未提供DeepSeek服务，请提交`PoCDelegation`或`PoCRefusal`。
3. 请在截止日期前对提案94进行投票。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。
...
**升级已执行：devshard运行时v4.0.1现已上线**

针对`v0.2.15-devshard-v4.0.1`运行时升级提案（提案ID 93）的链上治理投票已结束。

该提案已获批准，devshard v4运行时已升级至v4.0.1。`versiond`自动下载并验证了替换的二进制文件——无需重启主网或手动操作主机。

**API二进制文件更新：`v0.2.15-post3`**
...
- PoC种子稳定性。在区块边界延迟后恢复遗漏的种子提交，并在临时故障（如签名者不可用）后重试本地种子恢复，以防止验证者被归零。
- API。
...
请更新api容器使用的二进制文件。
在开始更新前，请确保CPoC或PoC当前未在运行。
每次PoC开始前有500个区块的CPoC空窗期。请尽可能利用此窗口期进行更新。
...
这是一个非关键性的、仅限devshard的升级，独立于完整链软件升级运行。它将现有的devshard v4运行时替换为v4.0.1。

这只是一个性能和资源管理补丁，除此之外无其他改动。协议、推理、验证和经济行为均保持不变。变更包括：区块轮次追踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与区块轮次修剪对齐。

**升级计划**

devshard运行时通过链上参数提案升级，而非完整链软件升级。若获批准，`versiond`将自动下载并验证替换的二进制文件。无需重启主网或手动操作主机。

**如何投票**

如果您没有直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅指南了解如何将冷密钥的治理投票权限授予热密钥。
...
投下您的选票（`yes`，`no`，`abstain`，`no_with_veto`）：
...
v4 是首个支持多实例高可用的版本：在共享 Postgres 后端部署 N 个 versiond / devshardd 副本，由 versiond-router 使用粘性会话路由和验证租约独占机制进行调度。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布新二进制文件时（名称不变，仅 sha256 变化），versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。通过移除过时行为并集中交易处理于当前支持的架构，也为未来的开发和升级提供了便利。
...
v4 的批准本身并不要求所有流量立即切换。在运行时启动并验证后，可逐步执行迁移。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十 GB，需提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个纪元）。
...
- 现在 / 主网升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器稳定，请提前按照 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和桥接器镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成上述两步，则无需重复操作。如果您拥有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
devshard 升级独立于主区块链进行。它们无需通过 Cosmovisor 协调全节点升级，不影响主网行为，预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是提高 devshard 主机在主机故障和升级时的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐个替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还修复了漏洞和安全问题。

**行动项**

请审查 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并对发现的问题、疑问、改进建议、边界情况或潜在漏洞留下评论。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。此举消除了旧运行时容器长期存在的内存增长问题 —— 这正是7月15日网络节点OOM事件的根源。

**这标志着v3切换的最终完成。** 仍通过v1/v2前缀或经典`/v1/devshard`路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案88进行投票 —— 紧急窗口时间很短。
2. 提供Kimi服务的主机：在投票结束后立即、并在区块 **5,105,276**（2026年7月17日，约12:05 UTC）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在引导期间委托时：**不要委托给守护者节点**；将委托分散至独立的Kimi主机。请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取最新的委托指导。
...
`moonshotai/Kimi-K2.6`在第328-329轮失去了PoC验证多数（详见事件[详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是最快恢复其服务且最小化停机时间的方式 —— 与6月提案78采用的恢复路径相同。
...
投票（`yes`，`no`，`abstain`，`no_with_veto`）：`--unordered`和`--timeout-duration`标志需要`inferenced`版本v0.2.13或更高版本。
```
export NODE_URL=https://node3.gonka.ai/
...
在纪元 328 中，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数支持。在纪元 329 开始时，运行 Kimi 的主机被移出该组。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行 Kimi 的服务器同时失败：

* 其中一台服务器上，运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障立即移除了这些投票。
* 第二台服务器因内存不足而失去其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者决胜机制未启动：目前没有模型投票权重的守护者已被从决胜机制中过滤。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况类似于纪元 306–307 事件。纪元 329 开始时的投票确切细节仍在确认中；本摘要可能有轻微修正。
...
早期指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤销：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主要机制与后备机制绑定到同一硬件——本次事件正是这种故障模式。目前正在讨论协议级限制。
* **避免将委托集中于任何单个主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在开发中的修复*** PoC 验证中的守护者投票（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点的内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完全迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（公告将随后发布），请对其投票。
...
此发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 自动下载该二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。
...
v3 运行时准备代理，在 v0.2.14 链升级期间无需依赖已弃用的经典 API 路径即可继续提供推理服务。同时改进了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用早期运行时前缀，直到代理切换至 v3。在此类升级期间，无需主网重启或手动主机操作。
...
本次仅为对拉取请求的审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始。
...
4. 经纪人 —— 将推理流量切换至 `/devshard/v3`。一旦 devshard v3 发布版本上线，请将推理流量移至 `/devshard/v3`，以便在链升级期间持续提供推理服务。
...
根据历史数据，此前未观察到此类攻击。但在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 正在使用该漏洞。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供 Kimi 服务的主机必须在第310轮期间声明其意向，并在新轮次开始前准备好节点。一个守护节点也将运行 Kimi，因此不希望运行它的主机可以将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换至 Kimi（PoC 开始于约6月28日 ~12:47 UTC）。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行 Kimi 时，其共识权重与主模型 MiniMax 大致相当。此举并非偏向任何硬件——MiniMax 上已具备相同的权重。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。此系数的选择使得在8xB300硬件上运行 GLM-5.2 时，其共识权重与 MiniMax 大致相当。此举并非偏向任何硬件。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3 可达性）。这使操作者能在配置硬件前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在 PoC 开始时实际提交 PoC 存储提交的主机决定——而非仅由声明意向的主机决定。只有满足资格条件的模型才会被激活。

**若获批准的效果**

从第309轮开始（PoC 开始约6月26日 15:25 UTC；生效约16:00 UTC），前提是各模型满足引导资格：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中引导。由于每轮约22.8小时，标准48小时投票周期无法适应单轮；而12小时紧急周期可以。创世守护节点预计支持本提案。

**主机所需操作**

1. **要提供 Kimi：** 在投票**结束后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 中将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口期，期间切换是安全的，因为该窗口期内没有 cPoC。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍使用Qwen或Kimi的主机，在切换前将无法获得本周期的cPoC奖励。
* 计划再次提供Kimi的主机应保留Kimi的部署环境，并在PoC 309周期随时准备将MLNode切换回Kimi——在第308周期将进行一次恢复Kimi的投票。
...
`moonshotai/Kimi-K2.6`目前投票数不足，无法通过PoC验证。因此，提供Kimi的参与者被移出组别，Kimi无法进入下一周期。现在将其从活跃列表中移除并重新引导，是最快恢复其服务且最小化停机时间的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi的投票数不足，无法通过PoC验证，将其移除并重新引导是最快恢复服务且最小化停机时间的方式。下一周期将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的模型调整。它与Kimi的情况无关，仅因同属PoC模型列表变更而一并纳入。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在第307个周期结束前完成，因此这些更改合并为一次紧急投票。每个周期约为22.9小时，标准的48小时投票周期无法容纳在一个周期内；而12小时的紧急周期可以。创始守护者应支持该提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预先下载FP8权重（如尚未准备就绪），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308周期将进行恢复Kimi的投票。


**下一步计划*** **恢复Kimi K2.6** —— 第308周期将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择提供该模型的主机不会受到惩罚。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：任何活跃节点均可使用：
...
- 将每个推理的验证计数器移出状态根，存入SQLite/Postgres，并在推理修剪后通过devshard统计端点暴露每插槽总计数。
...
4. 将每个推理的验证计数器存储在状态根之外的SQLite/Postgres中，并在推理修剪后通过devshard统计端点暴露每插槽总计数
...
升级将在主网的 **区块高度 4267300** 自动执行（约 **2026年5月26日星期二 14:42 UTC** / **07:42 PDT**）。
...
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在预先下载约 230 GB 的 FP8 权重。在引导窗口期间，Hugging Face 的速率限制和带宽饱和可能导致您错过首次资格检查。
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从第 278 个纪元开始，未采取任何操作的主机会对其全部权重施加每纪元 15% 的惩罚。
...
- 治理：将创世守护者投票权降至约 25%，并将链上法定人数设为 0.25。若守护者弃权，非守护者需在剩余 75% 中达到约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 在 vLLM 0.20.1 发布后，根据 Qwen-on-B200 参考重新校准 `WeightScaleFactor` 值：
    - Qwen 235B：`0.359`（不变）
    - Kimi K2.6：`0.78`（从 1.26 下调，与相同 PoC 权重相比，Kimi 的每纪元共识权重减少约 38%）
    - MiniMax-M2.7：`0.3024`
...
- 将 devshard 非法数限制从 `20_000` 提高至 `1_000_000`。
...
所有主机请验证其桥接容器是否已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。如果是，请先确认您正在运行当前版本，再进行任何进一步操作。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约 43 分钟前）→ 第一阶段 MiniMax PoC 在升级后的下一个周期边界启动 → MiniMax 处罚在链周期 278 时执行。
...
- 该提案将有意在升级高度到升级纪元结束（10000 块宽限期）期间跳过 Confirmation PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个纪元）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上不会发生任何变化，升级将不会进行。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何操作。
...
2. 监控下一几轮的PoC参与情况。确保您的节点如预期进入PoC，且活跃权重被正确反映。
...
似乎本轮的PoC投票未收集到所需的票数。确切原因尚未确认，社区正在积极调查此事。
...
如果您有有助于调查的相关观察、日志、假设或其他技术背景，请在此处分享。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，这使得其余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再执行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个周期）。

## 2026年5月7日

**桥接容器更新/验证必需**

作为即将进行的 v0.2.13 升级的准备工作（可能包括以太坊端合约激活），所有主机需验证其桥接容器已部署、运行最新版本并同步正确。
...
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断可能立即再次使其失效。现在在重新激活和即将晋升时重置计数器。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权中未包含 `MsgRespondDealerComplaints`。升级将补全该权限，以便它们能响应交易商投诉。
...
- Devshard 需要为大规模部署提供共享存储选项。现在可以使用 Postgres 作为主存储，SQLite 仍作为本地备用存储。
...
审查者可在以下地址查看完整的升级提案、迁移细节、测试摘要和建议流程：
...
请更新 api 容器的二进制文件。在每个 PoC 开始前有 500 个区块的无 CPoC（`confirmation_poc_safety_window`）窗口，因此这可能是部署最安全的版本。
...
在Kimi-K2.6引导过程中，主机观察到30%的最低直接参与阈值在实际中难以满足。为避免Kimi-K2.6在未来纪元中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
提交了PoCIntent的主机应在PoC于区块`3874496`启动前，将至少一个MLNode从`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`切换至`moonshotai/Kimi-K2.6`。

预评估与PoC启动之间有500个区块的窗口期。在此期间，没有CPoC任务，因此已声明意图的主机可以安全地将其模型节点切换至`Kimi K2.6`。
...
今天的预资格验证未通过，对于`PoCIntent`权重低于30%的主机。请保持您的MLNode使用`Qwen235B`，并于明天为下一个纪元提交您的意图。

## 2026年4月30日

**升级已执行：v0.2.12现已在主网上线**

针对升级提案v0.2.12的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。
...
- **协议强化**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发货人共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
有关这些变更的更多详情，请参见治理工件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
- **协议加固。** 保留的节点（`POC_SLOT=true` 将随机采样用于单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，之前为 32 位），将通过独立的治理投票激活。其他更新包括：将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 经销商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露于真实网络环境中，某些问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中将同时出现大量剪枝活动，造成不必要的运营压力。
...
目前，通过每次推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅负责会话创建和最终结算。

这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中将同时开始剪枝活动，造成可避免的操作压力。
...
对于已使用代理层的团队，这可能是简化堆栈、依赖原生行为的好机会。实际上，这应能带来更清晰的集成模式和更易维护的系统。
...
1. 升级 v0.2.10 引入了 Executor → MLNode 连接的严格 5 分钟超时，但部分请求可能需要更长时间。新版本将恢复该值，而非强制执行严格限制。
2. 之前的请求重试系统在因处理超时（而非 TLS 超时）导致失败时仍会重试推理。
服务器端对长请求的重试通常无效，因为会导致相同的超时场景，同时客户端可能收到不一致的输出。新版本在这些情况下不再重试推理。
3. 当前被保留且未参与 PoC 生成的 MLNode 仍被用于 PoC 验证，这可能导致推理遗漏。新版本将此类节点排除在 PoC 验证之外。
4. PoC 验证流水线中增加了额外的安全防护措施。
...
- 确保[抵押](https://gonka.ai/host/collateral/)已正确设置，否则从第180个周期开始，PoC产生的奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍是调整参数的机制，但若未批准替代方案，则默认规则生效。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能发生波动。较小的权重可能经历相对更大的波动。为避免在周期边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
由于有效归一化窗口比早期名义模型长约5个区块，每个周期重新计算的权重相应降低。
...
!!! note "兼容性说明"
    ...
    **按实际时间标准化 PoC 权重**
    ...
    此升级根据实际 PoC 持续时间标准化 PoC 参与者权重，以减少区块时间漂移效应，并确保权重结果与实际执行时长保持一致。
...
**宽限期：** 升级引入了一个宽限期，在升级后 3000 个区块内不进行确认 PoC，并对升级当期的错过率和无效率阈值采取较宽松的标准。
...
这意味着，在单个周期内，即使被惩罚，矿工最多只会损失其抵押物的 0.5%。而所需抵押物仅占日收益的约 24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果该提案未通过，创世时定义的抵押参数将在第 180 周期自动激活，而非上述参数。

投票结束后、第 180 周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 周期起，其收益将被削减至五分之一。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获批准，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于真实时间的权重标准化，以提升网络公平性和可扩展性。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了3000个区块的宽限期，期间无确认PoC，并在升级当期放宽了错过率和无效率阈值。
...
有关这些及其他变更的更多详情，请参见治理文档 [https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md ](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/governance-artifacts/update-v0.2.10/README.md)

**升级执行后所需的主机操作**

若提案获批准并执行升级，必须重启ML Node容器以触发模型重新部署。运行：
```
docker restart join-mlnode-1
```
...
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参见[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果 `application.db` 占用大量磁盘空间，可应用此处描述的清理技术 [https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it]。
- 升级后，Postgres 可作为本地负载存储的选项。
...
这意味着即使在一个周期内被惩罚，矿工最多也只能损失其抵押品的0.5%。所需抵押仅占日奖励的约24%。

提案提交投票后将另行公告。

警告：无论提案投票结果如何，抵押品都将生效。如果本提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而不是上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到验证，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止。这是因为节点运行的是与网络不再兼容的旧版本二进制文件。要恢复，请遵循本指南：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
对于对此次 PR 审查的有意义贡献，可能在下一次升级中提出奖励。

本次仅为拉取请求的审查呼吁，并非正式投票的开始。治理投票流程将在审查期结束后开始。
...
此升级通过实际 PoC 已用时间对参与者权重进行标准化，以减少区块时间漂移效应，并使权重结果与真实执行时长保持一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带有锁仓的流式转让
...
- **[PR #727](https://github.com/gonka-ai/gonka/pull/727)**：对 #634 的后续更新，将响应写入器传递给 `http.MaxBytesReader` 并对齐测试。
...
- 在PoC之外使用标准vLLM推理对同一模型 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 进行的控制推理基准测试，显示了与PoC v2中观测到的相同GPU类型间的相对性能差异。

**追踪器（仪表板）维护者操作**

在更新后的权重分配生效后，追踪器（仪表板）维护者可能希望审查第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。

## 2026年2月2日

**网络更新 — 补丁可用**

现已提供补丁以解决导致PoC周期内区块验证暂停的问题。建议主机尽快应用该补丁，以确保正确的PoC验证行为并安全恢复区块生产。
...
升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并于区块 2451000 成功在主网上执行。此升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**注意**

- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时能够及时执行后续步骤或缓解措施。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不被纳入下一纪元的参与者集合中。
...
!!! note 
    未能满足上述两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 的权重分配过渡仍受观测采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来更新其ML节点。从第155个纪元开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未同时满足上述两项条件的节点，一旦网络切换为单模型配置，将不再具备资格。
...
作为 v0.2.8 升级的一部分，支持的模型集合已更新。所有先前支持的模型均从活跃集合中移除，除：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦观察到活跃 Host 中约 50% 的足够采用率，可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不被视为 PoC v2 参与资格合格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
- 升级提案于区块 2387000 提出。该区块的预计时间为 2026 年 1 月 29 日 06:30:00 UTC。
...
**注意：** 升级后，可将 Postgres 配置为本地有效载荷的存储。
...
该提案将当前稳定期延长约两周，以便为即将进行的 PoC 变更及相关网络升级进行额外测试。有关新 PoC 开发进展的更多详情，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。
...
开发者 `allowlist` 将以相同偏移量延长，并持续有效至区块 2459375。

未包含在 `allowlist` 中的主机在延长的稳定期内将无法参与 PoC，该延长期将在区块 2443558 结束。
...
当前有一个新的治理投票正在进行。它通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的过期时间与区块 2,222,222 处的参与者注册截止时间对齐。
...
若获批准，网络将临时使用一个 `allowlist`，包含在之前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：
...
每位活跃主机的参与对网络推进和恢复正常运行至关重要。请保持在线，并在指令发布后立即准备应用更新。
...
问题已定位，正在准备补丁以解决根本原因。后续指令和技术细节将很快发布。
...
本次升级包含的预备性变更，为未来通过治理机制控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的 payouts，均匀应用于所有符合条件的节点，包括已领取原始奖励的节点。
...
这可能会减少磁盘使用量和升级时间。但是，如果升级失败，将无法恢复到之前的状态。
...
与创世验证者增强功能相关的链上治理提案已发布，目前开放投票。

近期网络增长带来了若干挑战。在过去几天中，网络经历了多次问题，其中一些似乎是由故意破坏或压力测试引起的。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。

创世验证者增强功能最初是在网络早期阶段作为临时防御机制引入的，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并启用一些额外的保护措施。
...
- 开发者地址注册：在稳定期间暂停新开发者地址的注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续有效至区块 2,294,222（约19天）。

**治理控制机制**

本升级包含的预备性变更，为未来通过治理控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第 117 期奖励分配**

本提案涵盖与链暂停（第 117 期）相关的两项奖励分配：

- 在第 117 期活跃但未收到奖励的节点，将补发该期的遗漏奖励。
- 所有在第 117 期活跃的节点，将额外获得等于 1.083 倍第 117 期奖励的支付，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
