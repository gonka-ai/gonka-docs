...
**devshard v5 提案重返治理（提案104的重新提交）**

提案105将 `v5` 添加到已批准的 devshard 版本列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖金。

**Devshard v5**

这是一个仅限 devshard 的更改。Cosmovisor 不参与，链不会暂停，推理服务继续运行。若获批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储的值一致，然后启动运行时。v5 将与现有的 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可以逐步迁移其网关。

主要协议新增功能是高度同步（height-sync），它为 devshard 会话提供了一个经过签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级 QoS 建立了协议级逻辑时间。本次发布还包括高可用性改进、热切换和会话恢复、网关更新、验证和结算修复、安全加固、可观测性改进以及扩展的端到端测试。
...
技术范围、个人贡献、PR 引用和完整的奖金明细：[gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)
...
投票将于 **2026年9月18日 16:47 UTC** 结束。提案通过需满足：权重达到25%的法定人数、多数赞成票，且反对票（含否决）低于33.4%。

请参与投票。此前的尝试因参与率不足而失败，无人反对，因此参与是唯一阻碍其通过的因素。
...
如果您希望托管它，欢迎加入。在部署窗口期间声明对该模型的意向并配置您的 MLNode，该窗口在下一次 PoC 开始前 500 个区块开启。

**别忘了您的参与选择**

自第 394 个纪元起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的 15%，并持续适用于每个纪元，直到您做出选择。
...
提案 ID 104 做了两件事：将 `v5` 添加到批准的 devshard 版本列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖励。

**Devshard v5**

这是一个仅限 devshard 的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会停止，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。通过该提案本身并不会转移任何流量，因此经纪人可以在之后逐步迁移其网关。
...
该提案还发放 91,300 USDT 的贡献者奖励，分配给七位受领人：
...
投票将于**2026年9月16日01:23（UTC）**结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票（含否决）低于33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅有关将冷密钥的治理投票权限授予热密钥的指南。

## 2026年9月12日

**提案通过：为Daniil和David Liberman参加All-In峰会2026的费用报销**

提案ID为103的链上治理投票已结束。

提案已获批准。从社区资金池拨款15,500 USDT，用于报销Daniil和David Liberman（Gonka协议的联合创始人）参加All-In峰会2026的两张参会通行证。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人7,500 USDT，门票已支付。本次报销涵盖两张门票及提现交易费用。本提案未申请报销航班、住宿及其他费用。

最终票数：赞成184762，反对0，含否决反对0，弃权0。
...
提案已获批准。Quant Mesh Limited被正式批准为Gonka与Ledger集成的法律签约方，并从社区资金池拨款350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用275,000 USDT、首年年度激活费75,000 USDT及350 USDT处理费。授权与资金拨付严格限于完整提案中描述的Ledger集成。

链上转账已完成。

最终票数：赞成146903，反对0，含否决反对0，弃权5568。
...
提案已获批准。`zai-org/GLM-5.3-Flash`现为治理批准的模型并纳入PoC模型集，而`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`已从`poc_params.models`中移除。这两个ID仍保留在`GET /v1/governance/models`（推理目录）中，但不再是PoC模型。

`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：其PoC参数和权重保持不变。

该模型及其参数由kaitaku.ai团队提出，经vbgd0独立验证，并联合提出。测量数据及阈值设定依据详见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元起，`penalty_start_epoch` 生效，因此未明确选择的宿主将面临不参与惩罚。如果您计划提供服务，尽早声明意向将使团队最有希望组建。宿主指南：[GLM-5.3-Flash 启动](./host/glm-bootstrap.md)。
...
该提案已获批准，`v4.1` 已被添加到批准的 devshard 版本列表中。`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。无需手动宿主操作。

v4.1 现在与现有的 v3 和 v4 运行时并行运行，因此当前流量继续被服务，代理可以逐步迁移其网关。
...
该模型及其参数由 kaitaku.ai 团队提出，经 vbgd0 独立验证，现共同提出。发布输入、测量结果及阈值设定的推理依据见 [gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash 的激活纪元为 **394**。从该纪元开始，`penalty_start_epoch` 生效，因此未明确选择的主机将面临不参与惩罚。
...
投票将于 **2026年9月10日 23:17 UTC** 结束。提案通过需满足：权重占比 25% 的投票率、多数赞成，且带否决权的反对票少于 33.4%。
...
如果您没有直接访问持有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。
...
这是一项仅限 devshard 的变更。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond` 将下载二进制文件，并将其 SHA-256 与链上存储的值进行验证后启动运行时。
...
投票将于**2026年9月10日22:36 UTC**结束。提案通过需满足：权重达到25%的法定人数、多数赞成，且反对票（含否决）少于33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参阅有关从冷密钥向热密钥授予治理投票权限的指南。
...
提案已获批准。将从社区资金池拨出100,000 USDT，用于支持Gonka参加2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金用于覆盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
提案已获批准。将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提升至0.246。其他所有模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为`PROPOSAL_STATUS_REJECTED`，原因是`failed_reason`：`proposal did not get enough votes to pass`（未达到25%法定人数）。最终票数：赞成15452，弃权3093，反对0，否决0。

原始的0.214系数源于被高估的nonce/min数值。测量修复见[gonka-ai/gonka#1640](https://github.com/gonka-ai/gonka/pull/1640)。主机无需任何操作。

感谢所有参与投票的人。

## 2026年8月25日

**仅devshard升级的PR现已开放审查**

Devshard升级独立于主区块链更新devshard运行时。它们无需通过Cosmovisor协调全节点升级，不影响主网行为，预计不会导致推理服务中断。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**关键变更**

1. v5的主要目标是高度同步：每个主机和排序器在托管日志中维护一个已签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard现在可以明确知道cPoC何时运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差分高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash`标记已存在于所有之前包含时间戳字段的消息中。后续版本可将`confirmed_at`和`started_at`等墙钟字段替换为高度标记。这解决了时间未来攻击问题，并为协议层面处理流超时打开了通道。本版本包含这些标记，但尚未将超时和密封决策切换至它们。
...
在周期初期，数据尚不足，因此无效推断检查（SPRT）可能在样本足够大之前，仅因少量无效结果而触发。本提案使其更稳定：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设为 0.18。
- 在大样本情况下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会被惩罚。真实违规者仍会触发该机制（约在 360 个样本时为 18%，1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效结果，且在 500 个样本时，观察到的无效率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期结束时的统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如有可能，请参与投票。如果未能在时限内达到紧急阈值，提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。若转为常规提案，您需要重新投票。
...
此仪表板从网关的角度显示每个主机执行的工作。在结算启用后，Misses 和 Invalid 指标将用于链上。根据当前主网阈值配置，错误率低于 10% 的主机不会触发任何惩罚或罚没，但仍应作为协议开发过程的一部分进行调查。

错误率较高的主机应在结算开始前解决这些问题。没有显著问题的主机无需采取任何操作。

仪表板还显示了网关的内部细节，例如特定主机的请求失败原因或无效上下文长度，这有助于排查主机端问题。超调度和隔离部分旨在为网关开发提供洞察，主机无需对此采取行动。

**下一步：****1) 接下来的 24 小时。** 主机应检查其统计数据并解决任何关键错误或配置错误。如有需要，贡献者可协助调查问题。

**2) 在此 24 小时窗口之后的第一个纪元。** 将在当前网关上手动开始结算，首先从较小的模型开始。此过程将被密切监控，以尽量减少误报记录。Kimi 将稍后加入，因为其在负载下的数据目前可靠性较低。

**3) 接下来的 1-3 天。** 将监控结算结果。

**4) 一旦此阶段稳定。** 其他经纪人可部署已稳定的网关版本，并以受控模式开始结算。

**5) 在所有经纪人成功验证后。** 结算覆盖率将逐步提升至 100%。

此次发布故意采取渐进方式，以便在结算扩展到整个网络之前发现并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash 现已在 Gonka 网络上激活**

在 [提案 94](https://gonka.ai/docs/network-updates/#august-10-2026) 中宣布的启动阶段已完成。自链纪元 360 起，`deepseek-ai/DeepSeek-V4-Flash-0731` 加入 MiniMax-M2.7 和 Kimi K2.6，成为活跃模型组，DeepSeek 组中获得的 PoC 权重现在以校准系数 0.214（后由 [提案 98](#august-29-2026) 提高至 0.246）转换为共识权重。

对 DeepSeek 的按模型参与强制执行现已生效。已为 DeepSeek 选择 DIRECT、DELEGATE 或 REFUSE 的主机无需执行任何其他操作——现有设置将继续有效。尚未做出选择的主机应尽快做出选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版 MLNode（正在服务 Kimi 或 MiniMax 的节点，因为已运行 DeepSeek 的节点均已升级至 3.0.16），请在方便时将这些节点升级至 3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少量节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点继续运行是最安全的方式。

MLNode 镜像在 [`docker-compose.mlnode.yml`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml) 的 `vllm-0.25.1-upgrade` 分支中已固定为 3.0.16。

默认 MLNode 镜像（3.0.16）基于 CUDA 13.0 构建。如果您的主机仍使用 CUDA 12.9，请改用 `3.0.16-cu129` 标签。这是相同的 MLNode 3.0.16，仅针对 CUDA 12.9 构建。两个标签均在同一 compose 文件中。
...
将正在运行的社区销售合约和所有封装代币实例迁移到新存储的 CosmWasm 代码，并为未来的实例化注册该封装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。
...
**DeepSeek V4 Flash 引导：意向阈值已满足 — 别忘了在 epoch 360 快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次 **官方** 检查是 epoch 360 引导快照 — 在此之前请保持委托多样化，以确保快照有效。
...
```
该技能会部署模型，测量 PoC 吞吐量，并将输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。若通过，则说明准备就绪。

如果您打算提供 DeepSeek，请按上述步骤设置。如不打算提供，请勿忘记委托，以免遭受不参与惩罚。

提醒：明天将发布日志更完善的 MLNode 新版本，敬请关注后续更新。
...
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4的模型而非fp8 + fp4，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型的精度相同。部署时，将在引导前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留您当前的模型。在epoch 360之前无需任何操作。从epoch 360开始，如果您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的未参与罚金。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。未修改任何现有参数或模型；唯一的变化是新增了该模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（锁定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在vLLM 0.25.1上运行的MLNode，并使用release-candidate节点配置（`node-config-release-candidate-*.json`适用于B300 / B200 / H200 / H100）
...
2. 预资格快照：链对意向和委托进行快照，并发出建议性的预资格事件。
...
在符合引导资格的前提下，从epoch 359（PoC开始时间约为2026年8月13日03:24 UTC / 8月12日20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731` 将成为可用的PoC模型。现有模型保持不变。DeepSeek的未参与罚金从epoch `360`（`penalty_start_epoch = 360`）开始生效。

**主机所需操作**

1. 要提供该模型：投票结束后声明意向，然后配置您的MLNode（vLLM 0.25.1 + release-candidate配置）：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
...
该提案已获批准，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换的二进制文件——无需重启主网或手动干预主机。
...
- API。
    ...
    - 默认的 Node Manager gRPC 端口现已改为 9400，以确保与自定义配置的兼容性
...
这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更包括：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪保持一致。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整的链软件升级。若获批准，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动干预主机。

**如何投票**

如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参考将治理投票权限从冷密钥授予热密钥的指南。
...
该提案已获批准，并于主网第 5316315 块成功执行。该提案同时批准了 devshard v4 运行时，其现在与现有的 v3 运行时并行运行。

## 2026年7月29日

**升级 v0.2.15 和 devshard v4：提前下载二进制文件**
...
v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是将单体架构重构为高可用、容错、可扩展架构系列变更中的首次更新。

v4 是首个面向多实例高可用的版本：在共享 Postgres 上，通过 versiond-router 部署 N 个 versioned/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布新二进制文件（名称不变，仅 sha256 变化）时，versiond 可进行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
同一提案还将 devshard v4 注册到 `DevshardEscrowParams.approved_versions` 中。
...
- 升级期间，Cosmovisor会在`.inference/data`目录中创建完整的状态备份；请确保有足够的磁盘空间（主网上的Cosmovisor备份`application.db`通常为数十GB，建议提前确认）。有关如何安全删除`.inference`目录中的旧备份的指南，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
- 如果`application.db`占用了大量磁盘空间，可以应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的Cosmovisor备份清理技术。
- 如果获得批准，升级后devshard存储可选择由共享的Postgres实例支持（与有效负载存储使用相同的环境变量）。本地SQLite将保持默认设置，并会自动清理（保留最近3个周期）。
...
主网链/API工作重点包括：PoC重复工件保护、早期共享检测、经典推理API弃用（禁用主网上的`/v1/chat/completions`计费并从API二进制中移除嵌入的`/v1/devshard`）、奖励接收方路由以及升级时的安全修复。

devshard部分为v3运行时做准备，以便代理在链升级期间无需依赖已弃用的经典API路径即可提供推理服务，从而提升RAM利用率并实现SQLite与Postgres存储之间的安全切换。
...
- 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥的稳定性，请提前按照[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和网桥镜像升级到 0.2.14-post3。如果您的 `api` 二进制文件已更新，您只需更新网桥镜像并重启网桥容器。如果您已完成这两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护人员 — 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上的 Cosmovisor 备份 `application.db` 通常为数十 GB，建议提前确认）。有关安全清理 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近 3 个周期）。

## 2026 年 7 月 20 日

**[仅 devshard 升级的 PR](https://github.com/gonka-ai/gonka/pull/1482) 现已开放供审查**

Devshard 升级独立于主区块链更新 devshard 运行时。它们不需要通过 Cosmovisor 协调完整节点升级，不影响主网行为，且预计不会导致推理服务中断。如果通过治理流程批准，新 devshard 版本将与现有的 v3 运行时并行运行。

**关键变更**

- v4 的主要目标是在主机故障和升级时实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐个替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管人通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可进行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
```
* 在周期 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776** — 2026 年 7 月 17 日 12:50:53 UTC / 05:50 PDT），将您的 MLNode 切换为 Kimi。
...
这是 7 月 15 日宣布的 Kimi 恢复计划的第二步。提案 87 已将 `moonshotai/Kimi-K2.6` 从活跃集合中移除；提案 88 将其重新注册到治理模型列表中，使其从周期 331 开始进入标准引导流程。

同一提案从 `devshard_escrow_params` 中移除了 `approved_versions` v1 和 v2。一旦获批，`versiond` 将停止 v1 和 v2 `devshardd` 进程，仅保留 v3 运行时（`/devshard/v3`）。此举消除了旧运行时容器长期存在的内存增长问题 —— 这正是 7 月 15 日事件中网络节点 OOM 的根源。

**这标志着 v3 切换最终完成。** 仍通过 v1/v2 前缀或经典 `/v1/devshard` 路径路由的网关，在提案通过后将停止提供推理服务。

**主机所需操作**

1. 在截止日期前对提案 88 进行投票 —— 紧急窗口时间很短。
2. 提供 Kimi 服务的主机：在投票结束后立即、在区块 **5,105,276**（2026 年 7 月 17 日 ~12:05 UTC）之前声明对 `moonshotai/Kimi-K2.6` 的意向，然后在周期 331 PoC 开始前约 500 个区块窗口内（区块 **5,105,776**，2026 年 7 月 17 日 ~12:50 UTC）切换您的 MLNode。
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 在进行引导委托时：**不要委托给守护节点**；将委托分散到独立的 Kimi 主机上。有关最新的委托指南，请参阅 [多模型概念验证指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
- 第 331 个周期 PoC 开始：区块 **5,105,776** —— 7 月 17 日 12:50 UTC（05:50 PDT）。

有关引导工作原理的更多信息：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026 年 7 月 15 日

**紧急治理投票（提案 87）：移除 Kimi-K2.6 以实现快速重新引导**

`moonshotai/Kimi-K2.6` 在第 328–329 周期失去了 PoC 验证多数票（详见 [此处事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其在最小停机时间内恢复的最快方式——与六月提案 78 的恢复路径相同。

该提案将在下一个 PoC 之前将 `moonshotai/Kimi-K2.6` 从 PoC 参数中移除，随后立即重新添加，Kimi 将从第 331 周期开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案 87 投票——紧急窗口时间很短。
...
**Kimi-K2.6 事件（第 328–329 周期）：发生了什么、恢复计划及委托指南变更**

在第 328 周期，`moonshotai/Kimi-K2.6` 失去了 PoC 验证多数票。在第 329 周期开始时，运行 Kimi 的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台由守护节点运营的运行 Kimi 的服务器同时出现故障：

* 其中一台服务器上运行 Kimi 的 MLNode 因供应商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障导致这些投票瞬间消失。
* 第二台服务器因内存耗尽而失去了其网络节点。

Kimi 的验证投票一直接近 2/3 阈值，因此这足以使该组低于多数。守护者投票决胜机制未启动：目前未分配每模型投票权重的守护者会被从决胜机制中过滤掉。这是一个已知问题，已在即将发布的 v0.2.14 升级中修复。

这种情况与第 306–307 个周期的事件类似。第 329 个周期开始时的投票详细信息仍在确认中；此摘要可能存在小幅修正。

**恢复计划**

恢复 Kimi 最安全的方式是通过全新引导重新添加它（约 1.5 个周期）：

1. [今天] 一项加速提案将在下一次 PoC 之前移除 Kimi，并在之后立即重新添加。随后将发布包含提案 ID、投票命令和截止日期的单独公告。
2. [明天] Kimi 将再次经历标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布相关说明。

**委托指南变更——请阅读**

之前的指南（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已撤回：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的后备机制。将委托集中于它们会将主要机制和后备机制绑定到同一硬件上——本次事件正是这种故障模式。目前正在讨论协议层面的限制。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行模型的主机上。如果您运营多个节点，请向您自己运行模型的节点进行委托。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多的验证权重且无单点故障。

**已列入计划的修复*** PoC 验证中的守护者投票（守护者始终可以投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期的 API 更新处理）。
* 网络节点的 RAM 增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立的内存限制。

**主机所需操作**

1. 一旦加速的 Kimi 提案上链，立即对其投票（公告将随后发布）。
2. 如果您计划提供 Kimi：关注引导说明，并在意图截止前做好声明准备。
3. 如果您为 Kimi 委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这将修复 RAM 增长问题，并在移除 v1/v2 之前必需完成。
...
提案获批后，v3 现在与现有的 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到经纪人将流量切换至 v3。

此版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

此类仅限 devshard 的运行时升级无需主网重启或手动主机操作。

**操作项**

白名单 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们在链升级运行期间仍能继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
这是一个仅限 devshard 的升级。它独立于完整链软件升级运行。一旦获得批准，v3 将与现有的 devshard 运行时并行运行。
...
devshard 部分准备 v3 运行时，以便代理在链升级期间无需依赖已废弃的经典 API 路径即可提供推理服务，从而提升 RAM 利用率，并支持在 SQLite 和 Postgres 存储之间安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从本分支单独发布并部署 devshard v3 版本，且在主网链升级前完成。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级运行期间继续提供推理服务。
...
2. 现在 / 主网升级前 — 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据指南将 `api` 二进制文件和网桥镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则仅需更新网桥镜像并重启网桥容器。若您已完成上述两项操作，则无需重复执行。若您拥有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
根据历史数据，此前未观察到此类攻击。然而，在当前纪元中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 的使用行为。

针对即将到来的 v0.2.14 链升级，已准备好永久修复此问题的补丁。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制的计算无法通过采样检查。
...
**Kimi-K2.6 引导：第 311 纪元的再次尝试**

`moonshotai/Kimi-K2.6` 在第 311 纪元再次尝试引导。希望提供 Kimi 服务的主机需在第 310 纪元声明其意图，并在新纪元开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 纪元，于区块 **4,797,456** 之前（约 6 月 28 日 12:00 UTC）声明 PoC 意图：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若足够多的主机声明意图，请在第 311 纪元开始前约 500 个区块的窗口期内将节点切换至 Kimi（PoC 开始于约 6 月 28 日 12:47 UTC）。
...
有关引导机制的更多内容：[https://gonka.ai/docs/host/kimi-bootstrap/](https://gonka.ai/docs/host/kimi-bootstrap/)

## 2026年6月26日

**紧急治理投票79：恢复 `moonshotai/Kimi-K2.6` 并添加 `GLM-5.2`****关键变更****1. 恢复 `Kimi-K2.6`**

`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数设定为：在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此设定并非偏向任何硬件——MiniMax已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的增益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（此举将取消该模型的未参与惩罚）。此系数设定为：在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此设定并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的增益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最少成员数 `V_min`、以及 >2/3 可达性）。这使运营商可在配置前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口内为其模型配置MLNode。
   
4. **PoC启动。** 成员资格由在PoC启动时实际提交PoC存储提交的主机决定——而非仅凭意向声明。只有满足资格条件的模型才会激活。

**若获批准的效果**

从第309轮起（PoC启动约6月26日15:25 UTC；生效约16:00 UTC），前提是各模型满足引导资格：

* `moonshotai/Kimi-K2.6` 再次成为活跃的PoC模型。
* `zai-org/GLM-5.2-FP8` 作为可选的PoC模型提供，且无未参与惩罚。
* `MiniMaxAI/MiniMax-M2.7` 仍为基础模型。

**为何紧急**

提案必须在第308轮结束前完成，以便两个模型都能在第309轮中完成引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计支持本提案。

**主机需采取的操作**

1. **若要提供Kimi：** 在投票**结束后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309中将您的MLNode切换至该模型。在PoC启动前有约500区块的安全窗口，期间切换是安全的，因为该窗口内无cPoC。

2. **若要提供GLM-5.2：** 为 `zai-org/GLM-5.2-FP8` 声明意向，并在部署窗口内配置您的MLNode。提供该模型是自愿的——未选择的主机不会受到惩罚。
...
**主机所需操作*** 请确保您的MLNode正在提供 `MiniMaxAI/MiniMax-M2.7`。任何仍使用Qwen或Kimi的主机，在切换前将无法获得本纪元的cPoC。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并准备好在PoC 309时将MLNode切换回来——将在第308个纪元进行一次恢复Kimi的投票。
...
**紧急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6` 目前尚未获得足够的投票以通过PoC验证。因此，提供Kimi的参与者已被移出组别，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快恢复且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以快速恢复。** 由于Kimi未获得足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的方式。下一个纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立、无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前要求的阵容变更。它与Kimi的情况无关，仅因同样是PoC阵容变更而被一并列入。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307个纪元结束前完成。每个纪元约22.9小时，标准48小时投票期无法容纳在一个纪元内；而12小时的紧急投票期可以。创世守护者应支持该提案。

**若获通过的效果**

自第308个纪元（约2026年6月25日17:25 UTC）起，`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前，将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可安全切换，因为此时无cPoC。请现在预先下载FP8权重（如尚未就绪），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时切换回来——第308个纪元将进行一次恢复Kimi的投票。


**即将进行*** **恢复 Kimi K2.6** — 在第308个纪元中进行的后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加GLM-5.2，**不设不参与惩罚**，以便主机可选择加入并提前测试模型需求，选择不提供该模型的主机不会受到惩罚。

**如何投票**

如果您没有直接访问持有投票权的密钥，或希望使用另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 查看。任何活跃节点均可使用：
...
**截止时间*** 提案78投票结束：**2026年6月25日 15:39:53 UTC**（加速处理）。
* 第308个纪元将在稍后形成：PoC开始约16:50 UTC，生效约17:25 UTC。
* 加速提案要求0.667的赞成阈值；参与率至关重要，请尽快投票。
...
本提案涵盖 devshard v2 发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0)

这是首次仅针对 devshard 的运行时升级。它独立于完整链软件升级，无需链二进制升级。

提案获批后，v2 现在可与现有的 v1 devshard 运行时并行运行。新进程通过 `/devshard/v2` 前缀提供服务，而现有的 v1 流量继续在 `/devshard/v1` 和 `/v1/devshard` 上运行。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。

对于此类仅 devshard 的运行时升级，无需重启节点容器或手动执行主机步骤。

**主要变更**

- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在内存或状态中。
...
- 将每个推理的验证计数器移出状态根，存入 SQLite/Postgres，并在推理修剪后通过 devshard 统计端点暴露每个插槽的总计数。
...
本提案涵盖 devshard v2 发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对 devshard 的升级。它独立于完整链软件升级。若获批，v2 将与现有的 v1 devshard 运行时并行运行。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在内存或状态中
...
4. 将每个推理的验证计数器存储在状态根之外的 SQLite/Postgres 中，并在推理修剪后通过 devshard 统计端点暴露每个插槽的总计数
...
该发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。若链上提案获批，`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动额外的 `devshardd` 进程。
...
提案详情和投票可通过 `inferenced` 查看。可使用任何活动节点。可用节点包括：
...
请直接审核该PR，并对任何发现、问题、改进建议、边缘情况或潜在漏洞留下评论。
...
v0.2.13中宣布的引导阶段已完成。自链纪元278起，MiniMaxAI/MiniMax-M2.7 与 Qwen3-235B 和 Kimi K2.6 一同成为活跃模型组，MiniMax 组内获得的PoC权重现在以校准系数0.3024转换为共识权重。

MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需额外操作——现有设置将继续有效。尚未做出选择的主机应尽快完成选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获**批准**，升级已在主网`4267300`区块成功执行。

## 2026年5月25日

**升级 v0.2.13：提前下载二进制文件和 MiniMax-M2.7 权重**

v0.2.13升级提案（提案ID [54](https://github.com/gonka-ai/gonka/pull/1143)）已通过链上治理，升级现已排期。
...
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在提前下载约230 GB的FP8权重。引导窗口期间Hugging Face的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都必须为**每个**治理批准的模型声明参与模式——`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6`和`MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一个或两个模型的主机，也必须为其他模型选择DELEGATE或REFUSE。MiniMax的截止时间为**链纪元 `278`**。未采取任何操作的主机将从纪元278起，对其全部权重施加15%的每纪元惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。确保 `.inference/data` 有足够空间用于cosmovisor状态备份；如果 `application.db` 较大，请在升级前应用[cosmovisor备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13校准将Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映Qwen-on-B200参考的vLLM-0.20.1吞吐量基准。此调整**仅适用于您共识权重中Kimi部分**；您的Qwen部分权重和Kimi内部PoC分配保持不变。在B200/B300上，Kimi仍是收益最高的选项；在H100/H200上，MiniMax-M2.7成为与Qwen相当、高于Kimi的选项。
...
[v0.2.13提案](https://github.com/gonka-ai/gonka/pull/1143)已重新上链并开放投票。这是对先前发布但未通过提案的重新投票，现重新提交并包含多项更新。

- 包括：Kimi权重重新校准（`0.78`）、新模型 `MiniMaxAI/MiniMax-M2.7`、验证阈值更新、devshard存储重构，以及多项PoC/奖励修复。
- 在主网上激活以太坊桥接（详见下方专节）。
- 该提案将升级后的宽限期延长至3000个区块，以便在新快照逻辑稳定期间主机不被惩罚。
- 治理：将创世守护者投票权降至约25%，并设置链上法定人数为0.25。若守护者弃权，非守护者需在剩余75%的投票权中实现约1/3的参与率才能满足法定人数（参见inference-chain部分）。
...
- 在vLLM 0.20.1发布后，基于Qwen-on-B200参考重新校准`WeightScaleFactor`值：
    - Qwen 235B：`0.359`（未变）
    - Kimi K2.6：`0.78`（从1.26下调，相当于在相同PoC权重下Kimi的每轮共识权重降低约38%）
    - MiniMax-M2.7：`0.3024`
...
- 将创世守护者的调整投票权降低至约25%，并将链上治理法定人数设置为`0.25`。若守护者不投票，则剩余75%投票权的有效法定人数为1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将支持Gonka主网与Ethereum之间的跨链转账（包括在Ethereum上封装GNK以及桥接USDC/USDT）。封装/解封装脚本和操作员工作流程将另行文档化。
...
所有主机需验证其网桥容器已部署、运行最新版本且同步正常。部分主机可能已部署网桥容器。在这种情况下，请先检查您是否运行的是当前版本，再进行任何进一步操作。
...
- 运营商时间线：投票于5月22日22:12 UTC结束 → 升级高度约在5月26日14:42 UTC → 升级周期剩余部分跳过确认PoC（≤10000区块宽限期）→ MiniMax启动快照在start_poc − 500区块时（约43分钟前）→ 升级后下一个周期边界开始第一阶段MiniMax PoC → MiniMax惩罚在链周期278时执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网中Cosmovisor对 `application.db` 的备份通常为数十GB，请提前确认）。有关安全删除 `.inference` 目录中旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 该提案将有意从升级高度到升级周期结束（10000区块宽限期）跳过确认PoC。若获得批准，此跳过属预期行为，并非故障；新快照逻辑将在下一个周期开始。
- 若提案获批，升级后devshard存储可选择由共享Postgres实例支持（与负载存储使用相同的环境变量）。本地SQLite仍将为默认选项，并自动清理（保留最近3个周期）。
- 若提案未通过（未达到法定人数，或 `no_with_veto` 超过⅓），链上无任何变更，升级不会发生。运营商可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需操作。
...
周期 #267 中PoC验证已成功完成，受影响的主机已能重新加入活跃集合。

周期 #266 的问题是由针对运行Kimi模型的主机的攻击引起的。网络继续运行，但该攻击与若干相关条件结合，导致许多参与者无法进入周期 #266。

在应用额外保护措施期间，推理可能暂时不可用。访问预计会逐步恢复，首先通过多个社区代理和经纪人端点。

**发生了什么**

在周期 #266 中，网络的活跃权重大幅下降。

问题已追溯到具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。
...
在相关保护措施实施期间，推理可能暂时不可用。访问预计会逐步恢复，首先通过多个社区代理和经纪人端点。
...
2. 监控接下来几个周期的PoC参与情况。确保您的节点按预期进入PoC，且活跃权重正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期会出现临时的推理中断。访问可能不会立即在所有地方可用，预计会通过社区代理和经纪人端点逐步恢复。
5. 在社区渠道或此线程中分享相关日志或观察结果。如果您的主机在第266个周期受到影响，或在后续周期中行为异常，这一点尤为重要。

## 2026年5月16日

**第266个周期：PoC权重下降调查**

在当前周期（#266）中，网络观察到活跃权重大幅下降。
似乎PoC投票未能收集到本周期所需的投票数。确切原因尚未确认，社区正在积极调查此情况。
...
社区成员目前正在审查第266个周期发生的情况。一旦对根本原因有更清晰的了解，将分享更新。
...
- 包括：为Kimi（`0.78`）重新校准权重，新增模型 `MiniMaxAI/MiniMax-M2.7`，更新验证阈值，devshard存储重构，以及若干PoC/奖励修复，以太坊桥接主网激活。
- 提案在升级后延长了宽限期，确保升级后3000个区块内不惩罚主机。
...
- 将 `MiniMaxAI/MiniMax-M2.7` 作为治理批准的模型和PoC模型添加。
...
- 修复新模型启动期间确认PoC奖励核算。
- 在本升级周期剩余时间内禁用确认PoC，以便新快照逻辑从下一个周期开始干净启动。
- 当参与者再次激活时，重置 `ConsecutiveInvalidInferences`。
- 为在v0.2.12之前加入的DAPIs回填缺失的 `MsgRespondDealerComplaints` 授权。
- 修复桥接和流动性池合约权限检查的Wasm keeper访问。
- 将创世守护者调整后的投票权降低至约25%，并将链上治理通过门槛设置为 `0.25`。在守护者不参与投票的情况下，这使得剩余75%投票权的有效通过门槛为1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再进行任何进一步操作。
...
- 该提案将有意从升级高度到升级纪元结束（3000区块宽限期）跳过确认PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效载荷存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个纪元）。

## 2026年5月7日

**桥接容器更新/验证要求**

作为即将进行的 v0.2.13 升级准备工作的一部分，该升级可能包括以太坊端合约激活，所有主机需验证其桥接容器已部署、运行最新版本并正确同步。
...
有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能在下一轮升级周期中获得社区奖励。
...
- 当参与者再次变为ACTIVE时，`ConsecutiveInvalidInferences`未被重置。一次新的错误推断即可再次使其失效。现在在重新激活和即将晋升时会重置该计数器。
- 在v0.2.12之前加入的DAPIs在其冷到暖的授权授予中未包含`MsgRespondDealerComplaints`。升级会回填该权限，以便它们能够响应交易商投诉。
- Devshard结算使用了硬编码的`20_000` nonce上限。现在上限为`DevshardEscrowParams.MaxNonce`，v0.2.13升级将其设置为`1_000_000`。升级还将`MaxEscrowsPerEpoch`提升至`500_000`。
...
- 由于旧的托管数据保留在一个SQLite存储中，devshard存储可能无限增长。现在存储按纪元范围管理，并在后台清理旧纪元，仅保留最新的3个纪元。
- Devshard需要为大规模部署提供共享存储选项。现在可使用Postgres作为主存储，SQLite仍作为本地备用选项。
...
审查者可在此处找到完整的升级提案、迁移详情、测试摘要和建议流程：
...
docker start api
```
部署后，请再次检查9100和9200端口上的服务器是否正常运行：
```
...
安全模型得以保留：PoC验证本身未改变，仍需超级多数的验证算力才能接受结果。
...
预评估与 PoC 开始之间有 500 个区块的窗口期。在此期间没有 CPoC 任务，因此已声明意向的节点可以安全地将模型节点切换至 `Kimi K2.6`。
...
这标志着网络的一个里程碑：`devshard` 已上线并具备生产就绪能力。`node4` 将作为未来推荐的公共网关。

**操作要求**：将您的端点更新为 `node4`。

## 2026年5月2日

今日的预资格验证未通过，对于 `PoCIntent` 权重低于 30% 的节点，权重极低。请保持您的 MLNode 使用 `Qwen235B`，并于明天提交下一周期的意向。
...
- **协议加固**。保留节点（`POC_SLOT=true`）随机采样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
每个 DAPI 会本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此 `/admin/v1/nodes` 看起来是干净的，即使持久化配置中仍包含额外模型。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。

为解决此问题，以下脚本将查找 `/admin/v1/config` 中包含额外模型的每个节点，并向 `/admin/v1/nodes/<id>` 发送一个包含清理后配置的 `PUT` 请求。这些更改将在 60 秒内持久化。剩余模型的参数、硬件和端口将完全保留。未列出强制模型的节点将被跳过，需手动修复。
...
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI / 主网发布周期解耦。
- **Certik 审计修复** ([#1020](https://github.com/gonka-ai/gonka/pull/1020), [#1021](https://github.com/gonka-ai/gonka/pull/1021), [#1022](https://github.com/gonka-ai/gonka/pull/1022), [#987](https://github.com/gonka-ai/gonka/pull/987), [#949](https://github.com/gonka-ai/gonka/pull/949), [#988](https://github.com/gonka-ai/gonka/pull/988), [#825](https://github.com/gonka-ai/gonka/pull/825), [#1011](https://github.com/gonka-ai/gonka/pull/1011), [#1029](https://github.com/gonka-ai/gonka/pull/1029), [#789](https://github.com/gonka-ai/gonka/pull/789))。已解决审计发现的问题。
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机采样用于单次 PoC / CPoC 时间。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 交易商共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
如果您没有直接访问拥有投票权的密钥，或希望使用另一个密钥代表您投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何从冷密钥向热密钥授予治理投票权限。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡到按模型分组的 PoC。每个治理批准的模型生成自己的本地 PoC 权重，然后通过模型特定系数聚合为总共识权重。
- **共识层交易费用与自动迁移** ([#937](https://github.com/gonka-ai/gonka/pull/937), [#981](https://github.com/gonka-ai/gonka/pull/981))。引入由治理控制的汽油价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免费。`MsgPoCV2StoreCommit` 采用双组件费用（基础验证 + 计数线性）作为主要的 Sybil 防御机制。详情请参见 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 托管资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
现在有新的mlnode版本可用：`ghcr.io/gonka-ai/mlnode:3.0.12-post6`
...
此次更新修复了在PoC启动附近的一个边缘情况，该情况过去可能导致在特定条件下性能下降。
...
**升级已执行：v0.2.11现已在主网上线**

针对升级提案v0.2.11的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。
...
- **二进制版本**：通过链上升级流程更新。
- **迁移**：测试和迁移详情请参见[v0.2.11文档](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.11/docs/upgrades.md)。
...
- 投票截止：2026年3月20日05:59:52（UTC）
- 升级高度：3186100
- 预计升级时间：2026年3月20日14:30（UTC）
...
下一个链上软件版本v0.2.11的升级提案现已上链并开放投票。若获得批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理扩展性，并显著改善 `Start`/`FinishInference` 性能。

**关键变更****[初始扩展架构：基于`devshards`的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于`devshards`的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过链上每笔推理交易处理推理操作限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅负责会话创建和最终结算。

这故意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅通过本地测试难以暴露。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳的推理数量最多提升100倍，具体取决于工作负载和网络状况。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会同时启动剪枝活动，从而造成可避免的操作压力。
...
本次仅为拉取请求的审查请求，而非正式投票的开始。治理投票流程将在审查期结束后启动。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限生产测试，并非因为其已被视为完成，而是因为此类系统需要尽早暴露在真实网络条件下。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，此改进可使每个区块容纳多达 100 倍的推理量，具体取决于工作负载和网络状况。  ￼

**升级前建议操作****`application.db` 剪枝**

强烈建议主机在升级前按照提供的说明剪枝 `application.db`。
提前执行此操作至关重要。如果大量节点推迟到升级后才进行剪枝，网络中可能会在同一时间启动剪枝活动，造成不必要的运营压力。
...
在 `v0.2.10` 中发现潜在漏洞。为降低当前升级前阶段的风险，建议在下一次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
如果您没有访问持有投票权的冷密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求他们授予您代表其投票的权限。若无此授权，其他账户无法提交投票。
...
1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格 5 分钟超时，而某些请求可能需要更长的时间。新版本的 API 将此值恢复，而不是强制执行严格限制。
2. 之前的请求重试系统在因处理超时（而非 TLS 超时）导致失败时仍会重试推理。
服务器端对长请求的重试通常无效，因为它会导致相同的超时场景。同时，客户端可能收到不一致的输出。新版本的 API 在此类情况下不再重试推理。
3. 当前被保留且不参与 PoC 生成的 MLNode 仍被用于 PoC 验证。这可能导致推理遗漏。新版本将这些节点从 PoC 验证中排除。
...
现已提供新的vLLM / mlnode构建版本，可在PoC开始时中断正在进行的推理请求，以降低因PoC启动时仍有活动请求而导致潜在权重下降的风险。
...
抵押参数更新提案未能达到法定人数，因此根据当前治理规则被拒绝。这意味着更新后的参数将不会激活。
...
!!! note "重要：存入时预留缓冲**
    ...
    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各周期间可能波动。较小的权重可能会经历更大比例的相对波动。为避免在周期边界时出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为使PoC权重与实际区块生成时间对齐，校准参数基于观测到的区块间隔选择。如实施所示，有效的PoC参考窗口比先前名义假设长约5个区块。
...
由于有效归一化窗口比之前的名义模型长约5个区块，因此每个周期重新计算的权重成比例降低。

**观测到的GPU权重变化（第175周期→第176周期）**
...
升级提案v0.2.10的链上治理投票已结束。该提案已获批准，并已在主网上成功执行。此升级对PoC验证进行了重大优化，并实现了实时权重归一化，以提高网络的公平性和可扩展性。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此升级通过实际 PoC 持续时间对 PoC 参与者权重进行标准化，以减少区块时间漂移影响，并使权重结果与实际执行时长保持一致。
...
**宽限期：** 升级引入了一个宽限期，在升级后的3000个区块内不进行确认 PoC，并且对升级时期的错过率和无效率阈值要求更宽松。
...
这意味着，在单个周期内，即使被惩罚，矿工最多只会损失其抵押物的0.5%。而所需的抵押物仅占日收益的约24%。

**警告：** 抵押参数将无论投票结果如何均生效。如果此提案未通过，创世时定义的抵押参数将在第180个周期自动激活，而非上述参数。

投票结束后、第180个周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180个周期起，其收益将被削减5倍。
...
为启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间不进行确认PoC，并且对升级所在纪元的错过率和无效率阈值放宽。
...
距离第180个纪元不足7天，现在是准备的时候了。

根据AMA期间的讨论以及社区成员提出的论点，建议从较小的抵押要求和最低惩罚开始。
...
这意味着在单个纪元内，即使被惩罚，矿工最多只会损失其抵押品的0.5%。所需抵押品仅占日收益的约24%。

一旦提案提交投票，将另行发布公告。

警告：抵押品将无论提案投票结果如何都生效。如果此提案未通过，创世中定义的抵押参数将在第180个周期自动激活，而不是上面列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果证明稳定性良好，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。

## 2026年2月13日

**即将推出的v0.2.10升级投票与执行时间表**

即将推出的软件升级v0.2.10的链上投票期预计将于周日晚上（洛杉矶时间）/ 周一早晨（UTC）开始。
...
请在GitHub上审阅v0.2.10升级PR并提供反馈。有意义的审阅贡献可能在下一次升级中获得奖励。
...
在当前周期中观察到cPoC（确认PoC）与PoC之间的重叠。直到该周期的最后一个区块，`is_confirmation_poc_active`被观察为`true`。

目前正评估此重叠的影响。初步观察表明，没有节点记录PoC提交，导致该周期累积权重为零。

作为预防措施，矿工通过协调行动暂时停止了区块生成。
...
链上推理访问当前已开放，不再仅限于开发者。推理请求可通过上一次更新中引入的允许传输代理发送。当前允许列表可在链上查询：
```
...
对于对本次PR审查的有意义贡献，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审查呼吁，并非正式投票的开始。治理投票流程将在审查期结束后开始。
...
此升级根据实际PoC耗时标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与真实执行时长一致。
...
- **[PR #727](https://github.com/gonka-ai/gonka/pull/727)**：对#634的后续改进，将响应写入器传递给 `http.MaxBytesReader` 并对齐测试。
...
2. 社区审查该PR后，预计从该分支创建v0.2.10版本，并提交该版本的链上升级提案，以启动正式的治理投票流程。
...
针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并于区块 2451000 成功在主网上执行升级。此次升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**请注意**

- 下一个 PoC 周期（从纪元 158 到 159 的过渡）至关重要。请确保在线，以便在需要时及时执行后续步骤或缓解措施。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型的 ML 节点才有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将来临的纪元的参与者集合中。
...
- 提供其他模型的 ML 节点不会被纳入 PoC v2 权重分配。在支持的情况下，可能会自动切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型
...
在因 cPoC 惩罚而导致奖励减少或排除的情况下，未计入的部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。

**其他协议更新**

- 转移代理角色在初始阶段仅限于[定义](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.9/proposals/governance-artifacts/update-v0.2.9#transfer-agent-whitelist)的`allowlist`
...
下表显示了上一周期具有权重、在当前周期提交了 PoC nonce，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
升级提案 v0.2.8 的链上治理投票已结束。该提案已获批准并在主网上成功执行。
...
- 身份验证绕过：将 `epochId` 绑定到签名，以验证对应正确周期。
...
!!! note 
    未能同时满足以上两项条件的节点，在网络过渡到单模型配置后将失去参与 PoC v2 的资格。PoC v2 的权重分配过渡仍受观察性采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个周期开始，ML节点配置需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个周期后安排。迁移阶段结束后，未满足配置要求的ML节点的权重将不被计入。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未同时满足两项条件的节点在网络切换为单模型配置后将不再符合条件。
...
!!! note "治理和 PoC v2 激活说明"
    ...
    **阶段 1. 观察（v0.2.8 之后的当前状态）**
    ...
    - 主机必须将其 ML 节点切换为提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 并升级为与 PoC v2 兼容的版本，才能为 PoC v2 权重做出贡献。
...
PoC v2 仅在链上治理提案获得批准时才成为权重分配的活跃方法。
    在此提案获得批准之前：
...
作为 v0.2.8 升级的一部分，支持的模型集已更新。所有先前支持的模型均从活跃集中移除，除以下模型外：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过成功参与使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 PoC v2，并配合所需的 ML Node 版本，可评估是否具备向 PoC v2 过渡的条件。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
- 身份验证绕过与重放：将 epochId 绑定到签名，并验证授权是否针对正确的 epoch。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未同时满足两项条件的节点将不再具备参与 PoC v2 的资格。ML Node 升级必须在 PoC v2 通过治理完全启用前完成。
...
在延长已确认的情况下，请利用这段时间为新的 PoC 要求准备您的系统。
...
该延长也为主机提供了时间，以准备其系统以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。有关更新现有 ML 节点的说明，请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个纪元中逐步执行更新。

**投票范围**

如果获得批准，网络将暂时在现有的 `allowlist`（包含未表现出非标准硬件行为的主机）下继续运行。
...
- 通过预定义的硬件配置模式，从公开可观察的链上数据推导得出。这些模式使用此处提供的开源脚本进行评估：[https://github.com/product-science/filter](https://github.com/product-science/filter)
...
`allowlist` 具有固定的过期时间，不会在延长的稳定窗口后继续存在。一旦 `allowlist` 在区块 2443558 过期：
...
2) 此前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块 2,222,222 结束。
...
- 生成的 `allowlist` 发布在此处：[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)
...
- 观察到的PoC权重低于使用同类硬件的其他参与者所表现权重的150%。
此前持续偏离这些模式的参与者在稳定窗口于区块 2222222 结束前不包含在 `allowlist` 中。
...
**v0.2.7升级提案：主网上线Genesis验证器增强**

链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强的提案已获得批准，并已在主网上成功部署。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——一种此前曾使用、持续时间有限的防御机制，现提议重新启用。
...
此次升级将此前通过手动API更新分发并已在网络中使用的关键修复正式化。这些修复包括：
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单将立即生效。白名单中的开发者地址在此期间可执行推理任务。所有适用于开发者地址的限制，包括开发者级注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理机制控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需经额外治理投票决定。
...
```
当所有命令均无错误完成且显示确认消息时，即可认为二进制文件已成功安装。
```
...
这可以减少磁盘使用量和升级时间。但是，如果升级失败，将没有备份可用于恢复之前的状态。
...
近期网络增长带来了若干挑战。过去几天，网络经历了多次问题，其中一些似乎是故意试图破坏或压垮系统所致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的韧性。
...
本次升级正式将此前通过手动 API 更新分发且已在网络中使用的关键修复纳入协议。这些修复包括：
...
- 主机级注册：新主机的注册将在区块 2,222,222 之前暂停（约两周后）。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期间，新开发者地址的注册将被暂停。一个预定义的 `allowlist` 开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块 2,294,222（约19天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理控制参与者加入和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。

**第 117 期奖励分配**

本提案涵盖与链暂停（第 117 期）相关的两项奖励分配：

- 在第 117 期活跃但未收到该期奖励的节点，将补发该期遗漏的奖励。
- 所有在第 117 期活跃的节点，将额外获得相当于第 117 期奖励 1.083 倍的 payout，均匀分配给所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由一个漏洞导致的：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。

参考：[https://github.com/gonka-ai/gonka/pull/517](https://github.com/gonka-ai/gonka/pull/517)

此 `API` 版本改进了对失败推理的处理弹性，并减少了推理遗漏计数问题。它还为 PoCBatch 和 PoCValidation 交易引入了批处理功能。
