...
提案通过后，v3 现在与现有的 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换至 v3。

该发布将 `devshardd` 二进制文件作为 Gonka 发布工件。`versiond` 会自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。

对于此类仅限 devshard 的运行时升级，无需重启主网或执行手动主机步骤。

**代理的操作事项**

代理应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这使它们能够在链升级运行时继续提供推理服务，而无需依赖已弃用的经典 API 路径。

**关键变更**

- 使代理能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
- 提高了 RAM 利用率。
- 修复了网关运行时行为。
...
这是一个仅限 devshard 的升级。它独立于全链软件升级。一旦获得批准，v3 将与现有的 devshard 运行时并行运行。

v3 运行时使代理能够在 v0.2.14 链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。它还提高了 RAM 利用率，修复了网关运行时行为，并启用了 SQLite 和 Postgres 存储之间的安全切换。
...
该发布将 `devshardd` 二进制文件作为 Gonka 发布工件。如果链上提案获得批准，`versiond` 将自动下载该二进制文件，验证 sha256 哈希值，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
新进程在 `/devshard/v3` 前缀下提供服务。现有 devshard 流量可继续使用早期运行时前缀，直到代理将流量切换至 v3。在此类升级期间，无需重启主网或执行手动主机步骤。
...
**Upgrade v0.2.14 的 PR 审查**

[此拉取请求](https://github.com/gonka-ai/gonka/pull/PR) 针对下一次链上软件升级 v0.2.14，现开放审查。
...
将从本分支发布一个独立的 devshard v3 版本，并在主网链升级前推出。提前将推理流量切换至 `/devshard/v3` 的代理，可在链升级运行时继续提供推理服务。
...
devshard v3 版本将在主网链升级前提议并推出，以便代理可在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作项**

1. 现在 — 审查 PR。请阅读 GitHub 上的 PR #1267，并对您发现的任何问题、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器稳定，请提前根据指南将 `ap`i 二进制文件和桥接器镜像更新至 0.2.14。若您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。若您已完成上述两步，则无需重复操作。若您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备好永久修复此问题的补丁。由于攻击已在使用中，因此在升级前提供仅 API 的热修复程序 —— 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制的计算无法通过采样检查。
...
**Kimi-K2.6 启动：第 311 个纪元的再次尝试**

`moonshotai/Kimi-K2.6` 在第 311 个纪元再次尝试启动。希望提供 Kimi 服务的主机需在第 310 个纪元声明意向，并在新纪元开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行 Kimi 的主机可将其权重委托给该节点。

**将运行 Kimi 的主机**

1. 在第 310 个纪元，于区块 **4,797,456** 之前（约 6 月 28 日 12:00 UTC）声明 PoC 意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 若足够多的主机声明意向，请在第 311 个纪元开始前约 500 个区块的窗口期内（PoC 开始于约 6 月 28 日 12:47 UTC）将您的节点切换至 Kimi。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一起重新添加。此系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此举并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
常规引导流程从第309轮开始。由于投票在PoC前约1小时结束，请在投票结束后立即提交 `MsgDeclarePoCIntent`。
有关引导的更多信息：https://gonka.ai/docs/host/kimi-bootstrap/

**2. 添加 `GLM-5.2`**

`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一起添加（这实际上取消了该模型的非参与惩罚）。此系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此举并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min`、>2/3可达性）。这使操作员可在配置硬件前确认模型是否可行。
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮中完成引导。由于每轮约22.8小时，标准的48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护者应支持本提案。

**主机所需操作**

1. **要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间无cPoC。
...
如果您无法直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。提案详情和投票可通过 `inferenced` 访问：任何活跃节点均可使用：
...
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并准备好在PoC 309时将MLNode切换回来——在第308纪元将进行一次恢复Kimi的投票。
...
**每项变更的目的*** **Kimi K2.6 — 被移除以便快速恢复。** 因为Kimi未获得足够的PoC验证投票，将其移除并重新引导是最快恢复且停机时间最短的途径。下一个纪元将进行一次恢复Kimi的投票。
* **Qwen3-235B — 退役（独立、无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已请求的阵容变更。它与Kimi情况无关，仅因同为PoC阵容变更而一并列入。
* **MiniMax-M2.7 — 升级为基础模型。**

这些变更被合并为一次紧急投票，因为重置必须在第307纪元结束前完成。每个纪元约22.9小时，标准的48小时投票期无法容纳在一个纪元内；而12小时的紧急投票期可以。创世守护者应支持该提案。

**若获批准的效果**

从第308纪元开始（约2026年6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7` 是唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6不再活跃。Kimi的恢复将在下一个纪元的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前将您的MLNode切换至 `MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前约500个区块的窗口期内切换是安全的，因为此时没有cPoC。请现在预先下载FP8权重（如尚未就绪），以避免在纪元边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划再次提供Kimi的主机现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308纪元将进行一次恢复Kimi的投票。


**接下来的内容*** **恢复 Kimi K2.6** — 在第308个周期中进行的后续投票，以重新添加 Kimi 并重启其引导过程。
* **GLM-5.2** — 一项后续提案将添加 GLM-5.2 **而不施加不参与惩罚**，以便主机可选择加入并提前测试对该模型的需求，选择不提供该模型的主机不会受到任何惩罚。
...
1) 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪负载，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中。
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。如果链上提案获批，`versiond`将自动下载该二进制文件，验证sha256哈希值，并在现有的`versiond`容器内启动额外的`devshardd`进程。

新进程在`/devshard/v2`前缀下提供服务。现有v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。在此类升级期间，无需重启节点容器或手动执行主机步骤。
...
提案详情和投票可通过`inferenced`获取。可使用任何活跃节点。可用节点包括：
...
这是首个仅限devshard的升级，因此其流程与标准链升级不同。Devshard升级独立于主区块链更新devshard运行时，无需通过Cosmovisor协调全节点升级，不影响主网行为，且不会导致推理服务停机。
...
此仅为PR审查请求，不启动正式投票。治理投票流程将在审查期结束后开始。
...
针对 [Upgrade Proposal v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **获得批准**。
...
3. 升级完成后，每个主机都需要为**每一个**治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需对其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从第 278 个纪元开始，未采取任何操作的主机会对其全部权重施加每纪元 15% 的惩罚。
4. 请确保在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。确保 `.inference/data` 有足够的空闲空间用于 cosmovisor 状态备份；如果 `application.db` 较大，请在升级前应用 [cosmovisor 备份指南中的清理技术](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)。
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基线。此调整**仅适用于您共识权重中由 Kimi 导出的部分**；您的 Qwen 导出权重和 Kimi 内部 PoC 分配保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 治理：将创世守护者投票权降低至约 25%，并设置链上法定人数为 0.25。如果守护者弃权，非守护者需在剩余 75% 中达到约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理法定人数设置为 `0.25`。在守护者不投票的情况下，这使剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
如果您无权直接访问持有投票权的密钥，或希望使用其他密钥代为投票，请参阅 [指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf) 了解如何将治理投票权限从冷密钥授予热密钥。
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约在 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。
...
2. 监控接下来几轮的PoC参与情况。确保您的节点如预期进入PoC，且活跃权重正确反映。
...
- 将创世守护者调整投票权降低至约 25%，并将链上治理法定人数设为 `0.25`。在守护者不投票的情况下，剩余 75% 投票权的有效法定人数为 1/3（`0.25 / 0.75 = 0.334`）。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。此时，请先确认您运行的是当前版本，再进行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 该提案将有意跳过从升级高度到升级纪元结束（3000个区块的宽限期）之间的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个纪元开始。
...
本次仅为拉取请求审查邀请，不启动正式投票。治理投票流程将在审查期结束后开始，很可能在明天。

**关键变更****inference-chain**

- 确认 PoC 在测量权重、保留权重和奖励重缩放时使用了不同的模型集。在新模型引导期间，这可能导致同时服务了合格模型和尚未合格模型的诚实矿工被惩罚。修复方案存储了一个纪元的可确认模型和权重缩放因子快照，然后在所有确认和奖励权重计算中使用该快照。
- 当参与者再次变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断会立即再次使其失效。现在在重新激活和即将晋升时重置计数器。
...
在 api 容器版本 v0.2.11、v0.2.12 和 v0.2.12-api-post2 中存在潜在问题。容器重启后，9100、9200 和 9400 端口上的服务器可能启动延迟。这延迟了 api 激活，导致部分矿工因延迟而跳过 Confirmation PoC。
...
在上一个周期中，发现Kimi-K2.6解析某些响应时存在一个小bug。
...
在Kimi-K2.6启动过程中，主机观察到30%的最低直接参与阈值在实际中难以达成。为避免Kimi-K2.6在未来的周期中失去资格的风险，并进一步简化新模型的接入，建议将阈值降低至10%。
...
今天的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机，请保留您的MLNode使用 `Qwen235B`，并于明天提交下一个周期的意图。
...
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个周期激活。该模型的系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的计算复杂度。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单次 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 押金资金的原子性，并在 `inference_finished` 事件解析中添加零时间戳容差。
...
- **多模型 PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）。将计算证明从单一固定模型过渡为每模型 PoC 组。每个经治理批准的模型生成其自身的本地 PoC 权重，然后通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固**。保留的节点（`POC_SLOT=true`）将被随机抽样用于单次 PoC/CPoC 时间。其他更新包括：将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位完整熵，此前为 32 位），将通过独立的治理投票激活。其他更新包括：将`mlnode`版本传播至链上`HardwareNode`、修复 DKG 经销商共识、使遗留验证者 slashing 与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及在`inference_finished`事件解析中添加零时间戳容差。
...
下一版本 v0.2.11 的升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中，某些问题仅靠本地测试难以发现。当前实现已设计为不会对矿工奖励产生负面影响。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中将同时启动剪枝活动，造成可避免的操作压力。
...
这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
对于已使用代理层的团队，这可能是简化架构、转而依赖原生行为的好机会。实践中，这将带来更清晰的集成模式和更易维护的系统。

## 2026年3月6日

**请注意：v0.2.11 升级预计将于下周初进入审核和治理投票阶段。**

请留意相关信息并计划参与。投票是支持网络发展的最简单方式之一，也能确保升级与参与者实际需求保持一致。
如果您无权访问持有投票权的冷钱包密钥，建议提前安排投票委托。请联系该密钥的所有者，请求其授权您代为投票。若无此授权，其他账户将无法提交投票。
...
3. 当前被保留且不参与 PoC 生成的 MLNodes 仍被用于 PoC 验证。这可能导致推理丢失。新版本将这些节点排除在 PoC 验证之外。
...
现在提供一个新的vLLM / mlnode构建，可在PoC开始时中断正在进行的推理请求，以降低在PoC启动时仍处于活动状态的请求导致潜在权重下降的风险。
...
为将PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选定。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
**宽限期：** 升级后引入了3000个区块的宽限期，期间不进行确认 PoC，并对升级当期的错过率和无效率阈值采取较宽松的标准。
...
这意味着在单个周期内，即使被惩罚，矿工损失的抵押金也不会超过其抵押金的 0.5%。且所需抵押金仅占日奖励的约24%。

**警告：** 抵押参数将无视投票结果而生效。若本提案未通过，创世时定义的抵押参数将在第180周期自动生效，而非上述参数。

投票结束后、第180周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第180周期起，其奖励将被削减5倍。
...
下一个链上软件版本 v0.2.10 的升级提案现已上链并开放投票。若获通过，该提案将引入一项重要的 PoC 验证优化（默认禁用），并实现基于实时的权重归一化，以提升网络公平性与可扩展性。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了3000个区块的宽限期，期间无确认PoC，并对升级当期的错过率和无效率阈值进行了放宽。

**其他协议更新**

- 修复PoC与CPoC交集错误（PR #752）
- 将IBC堆栈升级至v8.7.0。
- 惩罚阈值现由链上数据推导得出（PR #688）
...
如果提案获得通过并执行升级，必须重启ML Node容器以触发模型重新部署。运行：
```
docker restart join-mlnode-1
```
...
根据AMA期间的讨论以及社区成员提出的论点，建议初始抵押要求较低，惩罚轻微。
...
这意味着即使在一个周期内被惩罚，矿工损失的抵押品也不会超过0.5%。所需抵押品仅占日奖励的约24%。

提案提交投票后将另行公告。

警告：无论提案投票结果如何，抵押品都将生效。如果本提案未通过，创世中定义的抵押品参数将在第180个周期自动激活，而非上述列出的参数。
...
如果您节点未能及时应用最新升级，可能在区块2628371处因共识失败而停止运行。这是由于节点运行的是与网络不再兼容的旧版二进制文件。请按照本指南恢复：[https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch](https://gonka.ai/FAQ/#recovery-guide-consensus-failure-after-missing-patch)
...
2. 社区审查该PR后，预计将从该分支创建v0.2.10版本，并提交该版本的链上升级提案，以启动正式的治理投票流程。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此次升级后重新检查。由于ibc-go升级到v8.7.0，枚举现在以字符串形式编码而非数字，int64/uint64值现在也以字符串形式编码。
...
在更新后的权重分配生效后，追踪器（仪表板）维护者可能希望审查第159轮及之后的系数，以确保其与当前PoC v2的权重分配保持一致。
...
- 下一个 PoC 周期（从第 158 轮到第 159 轮的过渡）至关重要。请确保在线，以便在需要时及时执行后续步骤或缓解措施。
- 只有运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型的 ML 节点才有资格进入下一个（159）轮次并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入下一周期的参与者集合中。
...
- 提供其他模型的 ML 节点不参与 PoC v2 权重分配。在支持的情况下，系统可能会自动将模型切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
!!! note 
    一旦网络过渡到单模型配置，未能满足上述两项条件的节点将失去参与 PoC v2 的资格。PoC v2 的权重分配过渡仍受观测采用阈值和后续治理的约束。
...
本指南说明主机应如何根据v0.2.8模型可用性变化及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 未同时满足两项条件的节点，在网络切换为单模型配置后将不再符合条件。
...
通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估向 PoC v2 过渡的准备情况。一旦在活跃 Host 中观察到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。该阈值为观察性指标，不会触发任何自动网络变更。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机被鼓励在多个周期内逐步执行更新。
...
- 报告的硬件特征已与预定义的常见硬件配置模式进行评估，用于识别偏差和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），且
...
为恢复可靠的共识进展，需要**至少67%**的网络活跃算力安装该补丁。
...
- 临时重新激活创世验证者增强——这是一种此前曾使用过的、有限时长的防御机制，现提议重新启用。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。白名单中的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。
...
近期网络增长带来了若干挑战。在过去几天中，网络经历了多次问题，其中一些似乎是故意试图破坏或施压系统所致。本提案旨在通过一系列临时措施，增强网络在高负载和不利条件下的弹性。
...
此升级将此前通过手动 API 更新分发并已在网络上使用的关键修复正式化。这些修复包括：
...
本升级包含的预备变更，为未来通过治理控制参与者入驻和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的约束，需另行投票决定。
