...
!!! note "关于本页面"
    ...
    本页面内容不保证完整。如需获取最新信息，包括治理投票的发布及其当前状态，请参考链上数据或查看可用的浏览器和仪表板。

## 2026年9月22日

**升级 v0.2.16 的拉取请求审查**

下一个链上软件升级 v0.2.16 的拉取请求现已开放审查 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535)。有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一轮升级周期中获得社区奖金。

主网链/API 工作重点在于可信权重、动态系数、交易费用组和 PoC 挑战。如果提案获得批准，治理、BLS 和 PoC 验证权将仅限于上一个纪元确认的算力，而新的算力仍能立即获得奖励。治理可以为每个模型设置目标份额和系数范围。升级时这些范围保持关闭，因此系数和奖励不会变动。新增了消息组和 Gas 规则，所有组默认禁用，因此协议仍不收取费用。获准的挑战者可将一个活跃主机从推理任务中移出，转为全容量 PoC，直至下一次常规 PoC。

**关键变更**

1. **可信权重。** 在 v0.2.16 之前，突然增加的已声明算力会立即提升参与者在治理、BLS 和 PoC 验证中的权力。这使得未经确认的算力对共识和其他参与者的验证产生影响。此次升级将该权力限制为上一个纪元已确认的算力。新加入或回归的参与者从 0 开始，若确认失败，则下一个基准设为 0。新的算力仍能立即获得奖励。只有对信任敏感的权力需等待一个确认纪元。
2. **动态系数 v1。** 网络需要为每个模型提供可预测的吞吐量，以便用户能够依赖其可用性。每个模型的目标吞吐量应符合预期需求。使用固定系数时，整个硬件类别往往集中于同一模型，导致各模型的容量难以平衡和预测。此次升级允许治理为每个模型设置网络算力的目标百分比和系数范围。协议将动态调整该范围内的系数，以将算力引导至目标。超出目标的算力按最小系数评分，从而抑制过度供给。治理初始目标基于需求估算和 OpenRouter 等数据源设定。后续版本可聚合主机估算，最终使用链上模型使用数据。本版本仅改变激励机制，不会自动切换主机部署的模型。升级时，最小值和最大值均等于当前比例，因此系数和奖励在治理开放范围前保持不变。
3. **费用。** 历史上，协议一直不收取交易费用。因此攻击者可以以极低成本提交大量消息，而每个验证者却需承担处理和存储成本。此次升级对交易类型进行分组，并添加每条消息的 Gas 规则，以便为每组单独定价。所有组默认禁用且不收费。治理可稍后启用并定价。升级提案信息也可在升级高度启用这些组。
4. **PoC 挑战。** PoC 和随机确认 PoC 证明主机声明的算力。在纪元其余时间，推理统计检查该硬件是否用于协议分配的工作。高频率的遗漏或无效推理可能导致主机被移除。这提供了强有力的持续检查，但其敏感性依赖于推理量，可能无法揭示声明算力与可用算力之间的所有差距。PoC 挑战为此类情况增加了额外的安全层。获准的挑战者可要求一个活跃主机暂停推理，转为全容量 PoC，直至下一次常规 PoC。为发起挑战，挑战者需锁定目标主机剩余纪元奖励的一部分作为押金。若目标通过，将获得押金；若失败，挑战者将获得退款，目标将受到与确认 PoC 失败相同的惩罚。挑战期间遗漏的推理不计入目标。仅允许名单中的 devshard 托管创建者可发起挑战。若允许名单为空，则任何人都可发起挑战。
更多详情及其他变更见：[https://github.com/gonka-ai/gonka/pull/1535 ](https://github.com/gonka-ai/gonka/pull/1535 )

**行动项**

1. **所有人。** 现在 —— 审查拉取请求。阅读 [https://github.com/gonka-ai/gonka/pull/1535](https://github.com/gonka-ai/gonka/pull/1535) 并对任何发现、问题、改进建议、边缘情况或漏洞留下评论。
2. **主机。** 投票前无需任何操作。不要更新容器、重新授权密钥或为此升级移动模型。
3. **仪表板维护者。** 若治理投票通过，治理、BLS 和验证投票将遵循上限权重。奖励仍基于实际权重。


## 2026年9月18日

**提案已通过：批准 devshard v5 及奖金发放**

提案 ID 为 105 的链上治理投票已结束。

该提案以 123,754 票赞成、反对票数未达阈值的结果获得批准。
...
主要协议新增功能为高度同步（height-sync），为 DevShard 会话提供已签名、可重放的主网高度视图，并为未来的 cPoC、超时、争议和 QoS 机制建立协议级逻辑时间。本次发布还包括高可用性改进、热切换和会话恢复、网关更新、验证与结算修复、安全加固、可观测性改进以及扩展的端到端测试。

**主机所需操作**

无需操作。`versiond` 将下载新运行时，验证其 sha256，并与已运行的版本并行启动。`v3`、`v4` 和 `v4.1` 仍为批准版本，因此现有会话和路由前缀不受影响。这是运行时版本，而非网关发布：任何网关迁移将另行公告并提供独立指南。

**奖励发放**

该提案还从社区销售合约向多个开发周期中做出贡献的核心贡献者分配了 91,300 USDT：@a-kuprin、@qdanik、@Ryanchen911、@aikuznetsov、@snevolin、@shd 和 @redstartechno。所有奖金均已链上执行。

技术范围、个人贡献、PR 引用及完整奖金明细：[PR 1584](https://github.com/gonka-ai/gonka/pull/1584)

## 2026年9月16日

**devshard v5 提案重返治理（提案 104 的重新提交）**

提案 ID 105 将 `v5` 添加至批准的 devshard 版本列表，并从社区销售合约分配 91,300 USDT 给贡献者奖金。

**devshard v5**

这是仅针对 devshard 的变更。Cosmovisor 不涉及，链不会暂停，推理服务持续运行。若获得批准，`versiond` 将下载二进制文件，验证其 SHA-256 值是否与链上存储值一致，并启动运行时。v5 将与现有 v3、v4 和 v4.1 运行时并行运行，因此不会自动迁移，经纪人可逐步迁移其网关。

主要协议新增功能为高度同步（height-sync），为 devshard 会话提供已签名、可重放的主网高度视图，并为未来的 cPoC、超时和争议机制以及协议级 QoS 建立协议级逻辑时间。本次发布还包括高可用性改进、热切换和会话恢复、网关更新、验证与结算修复、安全加固、可观测性改进以及扩展的端到端测试。

该版本已由核心测试团队和外部测试实验室测试。
...
技术范围、个人贡献、PR 引用及完整奖金明细：[gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)
...
受益人为在多个开发周期和版本中活跃的 Gonka 核心贡献者：@akup / @a-kuprin, @qdanik, @Ryanchen911, @aikuznetsov, @snevolin, @shd 和 @redstartechno。
...
投票将于 **2026 年 9 月 18 日 16:47 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成，且反对票（含否决）低于 33.4%。

请投票。上一次尝试因参与率不足而失败，无人反对，因此参与是唯一阻碍本提案通过的障碍。
...
## 提案被拒绝：批准 devshard v5 及赏金发放

提案 ID 为 104 的链上治理投票已结束。该提案因 `failed_reason`：`proposal did not get enough votes to pass` 被拒绝。

最终计票：赞成 25291，反对 0，否决 0，弃权 0。

无人投反对票。参与率约为网络权重的 6%，低于 25% 的法定人数，因此提案仅因参与不足而失败。

## 2026 年 9 月 16 日

**GLM-5.3-Flash 已上线**

自第 395 个纪元起，`zai-org/GLM-5.3-Flash` 成为活跃模型组，并在主网上提供推理服务。

如果您希望托管它，欢迎加入。在部署窗口期间声明对该模型的意向并配置您的 MLNode，该窗口在下一个 PoC 开始前 500 个区块开启。

**请勿忽略您的参与选择**

自第 394 个纪元起，GLM-5.3-Flash 的不参与惩罚已生效。对于每个经治理批准的模型，您必须明确选择：自行提供、委托或拒绝。不采取任何行动将导致每纪元扣除您全部权重的 15%，且该惩罚将在您做出选择前持续生效。
...
提案编号104做了两件事：将 `v5` 添加到批准的 devshard 版本列表中，并从社区销售合约中分配 91,300 USDT 作为贡献者奖励。

**Devshard v5**

这是一个仅限 devshard 的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及 Cosmovisor，链不会停止，推理服务持续运行。如果获得批准，`versiond` 将下载二进制文件，并将其 SHA-256 值与链上存储的值进行验证，然后启动运行时。

v5 将与现有的 v3、v4 和 v4.1 运行时并行运行。提案通过本身不会迁移任何流量，因此经纪人可以在之后逐步迁移其网关。
...
该提案还发放 91,300 USDT 的贡献者奖励，分配给七名接收方：
...
投票将于 **2026年9月16日 01:23 UTC** 结束。提案通过需满足：权重达到 25% 的法定人数、多数赞成，且反对票（含否决）少于 33.4%。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由另一个密钥代为投票，请参阅有关将治理投票权限从冷钱包授予热钱包的指南。
...
该提案已获**批准**。从社区金库中拨出 15,500 USDT，用于补偿 Gonka 协议的联合创建者 Daniil 和 David Liberman 参加 All-In Summit 2026 的两张参会通行证费用。

峰会于2026年9月13日至15日在洛杉矶举行。标准参会通行证每人售价 7,500 USDT，门票已预先支付。本次补偿涵盖两张门票及提现交易费用。飞行、住宿及其他费用未在本提案中向社区金库申请。
...
**提案通过：批准Quant Mesh Limited作为Ledger集成合作方并拨付初始Ledger集成款项**

提案ID为102的链上治理投票已结束。

提案已获批准。Quant Mesh Limited被正式批准为Gonka-Ledger集成的法律签约方，并从社区池中拨付350,350 USDT至`gonka1yqj5xf0wtqgpdmv5v68cus0tp2j5fv7lzcfd6g`。

该拨款涵盖第一阶段启动费用275,000 USDT、第一年年度激活费用75,000 USDT以及350 USDT的处理成本。授权和资金用途严格限于完整提案中描述的Ledger集成。

转账已在链上执行。

最终投票结果：赞成146903，反对0，否决0，弃权5568。
...
GLM-5.3-Flash的激活纪元为**394**。从该纪元起，`penalty_start_epoch`生效，因此未明确选择的主机将面临不参与罚金。如果您计划提供该模型，尽早声明意向将有助于团队最佳组建。主机指南：[GLM-5.3-Flash启动指南](./host/glm-bootstrap.md)。

最终投票结果：赞成309308，反对0，否决0，弃权3168。
...
提案已获批准，`v4.1`已被添加至批准的devshard版本列表中。`versiond`将下载二进制文件，并将其SHA-256与链上存储的值进行校验后启动运行时。主机无需手动操作。

v4.1将与现有的v3和v4运行时并行运行，因此当前流量将继续被服务，经纪人可逐步迁移其网关。
...
提案ID 101将`zai-org/GLM-5.3-Flash`注册为治理批准的模型，并将其添加到PoC模型集中。同时，它从`poc_params.models`中移除`moonshotai/Kimi-K2.6`和`zai-org/GLM-5.2-FP8`。

`MiniMaxAI/MiniMax-M2.7`和`deepseek-ai/DeepSeek-V4-Flash-0731`不受影响：它们的PoC参数和权重保持不变。所有其他链上参数均保持不变。

该模型及其参数由kaitaku.ai团队提出，经vbgd0独立验证，现共同提出。发布输入、测量值及阈值设定的推理依据见[gonka-ai/gonka#1734](https://github.com/gonka-ai/gonka/pull/1734)。
...
GLM-5.3-Flash的激活纪元为**394**。从该纪元起，`penalty_start_epoch`生效，因此未明确选择的主机将面临不参与惩罚。
...
投票将于**2026年9月10日23:17 UTC**结束。提案通过需满足：权重25%的最低投票率、多数赞成票，且反对票低于33.4%且无否决权。
...
如果您没有直接访问持有投票权密钥的权限，或希望使用其他密钥代为投票，请参阅有关将治理投票权限从冷密钥授予热密钥的指南。

## 2026年9月9日

**devshard v4.1运行时提案已进入治理流程**

提案ID 100将`v4.1`添加到已批准的devshard版本列表中。

这仅是devshard的更改。它通过链上参数提案进行，而非完整的链软件升级：不涉及Cosmovisor，链不会暂停，推理服务持续运行。若获批准，`versiond`将下载二进制文件，验证其SHA-256值是否与链上存储的值一致，然后启动运行时。

v4.1将与现有的v3和v4运行时并行运行。提案通过本身不会转移任何流量，因此经纪人可在之后逐步迁移其网关。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅有关从冷密钥向热密钥授予治理投票权限的指南。
...
该提案已获批准。它将从社区池中拨出100,000 USDT，用于支持Gonka参与2026年10月7日至8日在滨海湾金沙举行的TOKEN2049新加坡活动。资金涵盖黄金赞助包及相关后勤：差旅、住宿、本地协调、嘉宾接待、活动运营和支持材料。
...
`v4.0.2` devshard二进制文件现已可用。主机可使用以下说明手动更新v4二进制文件。尚未安排自动更新。

**变更内容**

重启时，`v4.0.1` 从nonce 1恢复了所有会话。拥有大量存储会话的节点需要数小时才能就绪，`versiond` 在60秒就绪超时后终止子进程，客户端因此看到HTTP 502错误。
...
该提案已获批准。它将`weight_scale_factor`对`deepseek-ai/DeepSeek-V4-Flash-0731`的权重系数从0.214提升至0.246。所有其他模型和链参数保持不变。这是提案97的重新提交。链上提案97最终结果为`PROPOSAL_STATUS_REJECTED`，原因是`failed_reason`：`proposal did not get enough votes to pass`（未达到25%的出席率）。最终票数：赞成15452，弃权3093，反对0，否决0。
...
Devshard升级独立于主区块链更新devshard运行时。它们不需要通过Cosmovisor协调全节点升级，不影响主网行为，且预计不会造成推理服务的停机。若通过治理流程批准，新的devshard v5运行时将与现有的v4运行时并行运行。

**关键变更**

1. v5的主要目标是高度同步：每个主机和序列器在保证金日志中维护一个已签名、可重放的主网高度视图，即使在无推理流量时也是如此。借助该时钟，devshard现在可以明确知道cPoC何时运行，并将参与者跳过的推理视为合法跳过。高度同步协议设计：[HEIGHT_SYNC_PROTOCOL_PROPOSAL.md](https://github.com/gonka-ai/gonka/blob/ak/height-sync-protocol/devshard/docs/proposals/HEIGHT_SYNC_PROTOCOL_PROPOSAL.md)

2. 差异高度标记为协议增加了逻辑时间。`observed_height` / `observed_block_hash` 标记已存在于所有先前包含时间戳字段的消息中。后续版本可将诸如`confirmed_at`和`started_at`等壁钟字段替换为高度标记。这解决了未来时间攻击问题，并为在协议层面处理流超时开辟了道路。本版本包含这些标记，但尚未将超时和封印决策切换至它们。

3. 已添加通过所有插槽往返的心跳机制，因此即使会话安静，仍能定期同步高度；而繁忙会话则可通过常规推理标记实现相同的节奏。

**行动项**

请审查 PR [gonka-ai/gonka#1584](https://github.com/gonka-ai/gonka/pull/1584)，并对任何发现、问题、改进建议、边界情况或潜在漏洞留下评论。
...
该提案已被批准。它调整了在周期早期数据不足时无效推断检查（SPRT）的行为，避免过早触发。主机无需采取任何操作。
...
在周期早期，数据尚不足，因此无效推断检查（SPRT）可能在样本足够大之前仅因少量无效结果而触发。本提案对其进行稳定化：

- 将 `invalidation_h_threshold` 提高至 40，并将 `bad_participant_invalidation_rate` 设置为 0.18。
- 在大样本情况下，阈值接近 10% 的无效率，因此低于该阈值的诚实参与者不会被惩罚。真实违规者仍会被触发（约在 360 个样本时为 18%，在 1650 个样本时为 12%）。
- 小样本无法触发：至少需要 32 个被标记的无效结果，且在 500 个样本时观测到的比率必须超过 16%。
- 未使用的停机时间 SPRT 已关闭。周期末统计测试不受影响。

**投票**

这是一个紧急提案，因此投票将于 **UTC 时间 8 月 21 日 06:04**（约 12 小时后）结束。如有可能，请参与投票。如果未能在时限内达到紧急阈值，该提案不会被拒绝：它将转为常规提案，投票将继续进行更长的正常周期。若转换，您需要在常规提案中重新投票。
...
一个稳定的网关版本目前正运行在一个网关上，并将用于首个结算阶段。其统计数据（代表部分网络流量）可在此处查看：[https://tracker.gonka.pro/?view=gateway&epoch=364](https://tracker.gonka.pro/?view=gateway&epoch=364)

**如何阅读仪表板**

此仪表板从网关的角度展示了每个主机执行的工作。Misses和Invalid指标在结算启用后将被链上使用。根据当前主网阈值配置，错误率低于10%的主机不会触发任何处罚或罚没，但仍应作为协议开发过程的一部分进行调查。
...
**2) 在此24小时窗口之后的第一个纪元。** 将在当前网关上手动启动结算，首先从较小模型开始。此过程将被密切监控，以尽量减少误报记录。Kimi将稍后跟进，因为其在负载下的数据目前可靠性较低。

**3）接下来1-3天。** 将监控结算结果

**4) 一旦此阶段稳定。** 其他经纪人可部署已稳定的网关版本并以受控模式开始结算。

**5）在所有经纪人成功验证后。** 结算覆盖率将逐步向100%推进。

此次发布故意采取渐进方式，以便在结算扩展到全网之前识别并解决任何意外问题。

## 2026年8月14日

**DeepSeek V4 Flash现已在Gonka网络上激活**

在[提案94](https://gonka.ai/docs/network-updates/#august-10-2026)中宣布的启动阶段已完成。自链纪元360起，`deepseek-ai/DeepSeek-V4-Flash-0731`与MiniMax-M2.7和Kimi K2.6一同成为活跃模型组，DeepSeek组中获得的PoC权重现已按校准系数0.214（后由[提案98](#august-29-2026)提升至0.246）转换为共识权重。

针对DeepSeek的按模型参与强制执行现已生效。已为DeepSeek选择DIRECT、DELEGATE或REFUSE的主机无需再做任何操作——现有设置将继续有效。尚未做出选择的主机建议尽快选择，以避免每纪元处罚（[https://gonka.ai/docs/host/deepseek-bootstrap/](https://gonka.ai/docs/host/deepseek-bootstrap/)）。
...
如果仍有节点运行旧版MLNode（即正在服务Kimi或MiniMax的节点，因为已运行DeepSeek的节点均已升级至3.0.16），请在方便时将这些节点迁移至3.0.16。

一个请求：请逐步部署。请勿一次性切换所有节点。请先更新少数节点，确认其稳定后再更新其余节点。在验证新版本的同时保持其余节点正常运行是最安全的方式。

MLNode镜像在`docker-compose.mlnode.yml`分支的[`vllm-0.25.1-upgrade`](https://github.com/gonka-ai/gonka/blob/vllm-0.25.1-upgrade/deploy/join/docker-compose.mlnode.yml)中已锁定为3.0.16。

默认MLNode镜像（3.0.16）基于CUDA 13.0构建。如果您主机仍使用CUDA 12.9，请改用`3.0.16-cu129`标签。这是同一MLNode 3.0.16，仅针对CUDA 12.9构建。两个标签均在同一个compose文件中。
...
将在线社区销售合约及所有包装代币实例迁移至新存储的CosmWasm代码，并为未来实例化注册该包装代币代码。合约地址和余额保持不变。无需链二进制升级。此举旨在缓解安全报告中发现的理论风险，预计不会影响正常运行。
创世守护者预计将支持该提案。

**如何投票**

如果您无法直接访问拥有投票权的密钥，或希望由其他密钥代表您投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。
...
**DeepSeek V4 Flash 引导：意向阈值已达成——请在 epoch 360 快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在 epoch 359 快照时未达到预合格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查为 epoch 360 引导快照——在此前请保持委托的多样化，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始（区块 **5,552,115**）前约 500 区块的部署窗口内配置 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。

**继续运行其他模型的主机**

在区块 **5,551,615** 之前，提交委托（有助于引导可达性）或拒绝。从 epoch **360** 起，DeepSeek `penalty_start_epoch` 将激活——未明确选择的主机将面临不参与惩罚。
...
建议逐步将节点切换到新 API，而非一次性全量升级。

**3) 使用 mlnode-validate 验证**

该技能现已支持 DeepSeek。它位于独立的 `./skills` 文件夹中，无需任何代理特定配置——将其符号链接到 `.claude/skills` 并运行 [`mlnode-validate` 技能](https://github.com/gonka-ai/gonka/tree/vllm-0.25.1-upgrade/skills/mlnode-validate)。
```
...
如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请勿忘记委托，以避免非参与罚金。

提醒：明天将发布另一个带有更好日志的MLNode构建版本，因此请留意后续简短通知。
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359启动前约500个区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口内切换是安全的，因为该窗口内无cPoC。
注意：在Blackwell GPU上，为获得最佳性能，您可以使用重新打包为fp8 + nvfp4而非fp8 + fp4的模型，例如 `MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型精度相同。为部署此模型，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给守护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第360轮之前无需操作。从第360轮起，若您未提供DeepSeek，请提交 `PoCDelegation` 或 `PoCRefusal`，否则将适用15%的非参与罚金。
...
本提案将DeepSeek V4 Flash作为新的PoC模型添加到网络中。现有参数或模型均未修改，唯一变更仅为新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本 `7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在vLLM 0.25.1上运行MLNode，并使用发布候选节点配置（`node-config-release-candidate-*.json`适用于B300 / B200 / H200 / H100）
...
4. PoC开始：成员资格由在PoC开始时提交PoC存储提交的人决定，而非由声明意向的人决定。
...
在符合引导资格的前提下，从第359个纪元（PoC开始时间约为2026年8月13日03:24 UTC / 8月12日20:24 PDT）起，`deepseek-ai/DeepSeek-V4-Flash-0731`将成为可用的PoC模型。现有模型保持不变。DeepSeek的不参与惩罚从第`360`个纪元（`penalty_start_epoch = 360`）开始。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代表您投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
该提案已获批准，devshard v4 运行时已升级至 v4.0.1。`versiond` 自动下载并验证了替换的二进制文件——无需重启主网或手动执行主机操作。
...
**`v0.2.15-devshard-v4.0.1` 运行时升级提案已进入治理流程**

这是一个非关键性的、仅限 devshard 的升级，独立于完整链软件升级运行。它将现有的 devshard v4 运行时替换为 v4.0.1。

这只是一个性能和资源管理补丁，仅此而已。协议、推理、验证和经济行为均保持不变。变更内容：纪元跟踪从每区块链查询改为事件驱动更新，主机会话和验证工作进程的内存清理现在与纪元修剪保持一致。

**升级计划**

devshard 运行时通过链上参数提案升级，而非完整链软件升级。若获批准，`versiond` 将自动下载并验证替换的二进制文件。无需重启主网或手动执行主机操作。
...
该提案已获批准，并于主网区块 5316315 成功执行。同一提案还批准了 devshard v4 运行时，该运行时现在与现有的 v3 运行时并行运行。
...
v4 的主要目标是在主机故障和升级期间实现高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐台替换版本。这是系列变更中的首次更新，旨在将单体架构重构为高可用、容错、可扩展的架构。

v4 是首个为多实例高可用设计的版本：在共享 Postgres 后端部署 N 个 versiond / devshardd 副本，通过 versiond-router 实现粘性会话路由和验证租约独占性。网关仅通过 gRPC 与链通信。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，提升了系统的整体稳定性和可维护性，并通过移除过时行为、将交易处理集中于当前支持的架构，使未来的开发和升级更加容易。
...
Devshard v4 将与现有的 v3 运行时并行运行。在经纪人和主机操作员准备和验证其 v4 部署期间，现有的 v3 流量可继续运行。
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整的状态备份；请确保有足够的磁盘空间（主网中 Cosmovisor 对 `application.db` 的备份通常为数十GB，建议提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅[文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并会自动清理（保留最近3个周期）。
...
该提案已获批准，并已在主网区块 5195700 成功执行升级。
...
建议主机查阅 [GitHub](https://github.com/gonka-ai/gonka/pull/1267) 上的提案并参与投票。
...
- 现在 / 主网升级前 —— 更新您的 API 和网桥。为确保主网升级期间以太坊网桥稳定，请提前根据 [指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update) 将 `api` 二进制文件和网桥镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新网桥镜像并重启网桥容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在 PoC 或 cPoC 之外执行此步骤。

- 仪表板维护者——本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取数据并显示，它将继续正常工作。您需要了解一项语义变更（委托），以及三个小注意事项（请阅读完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上的 Cosmovisor `application.db` 备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中的旧备份的指导，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- v4 的主要目标是在主机故障和升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器逐个替换版本。这是系列更新中的首次变更，旨在将单体架构重构为高可用、容错、可扩展的架构。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；仅托管所有者通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件时（名称不变，仅 sha256 变化），versiond 可执行蓝绿切换并排空，以确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还修复了错误和安全问题。

**操作项**

请审查 PR [https://github.com/gonka-ai/gonka/pull/1482](https://github.com/gonka-ai/gonka/pull/1482)，并对任何发现、问题、改进建议、边界情况或潜在漏洞发表评论。

有意义的审查贡献，包括重要评论、错误发现和安全问题，可能有资格获得下一个升级周期的社区奖励。
...
* 在区块 **5,105,276** 之前（2026年7月17日，约12:05 UTC）声明意向：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
* 在第331个周期PoC开始前的约500个区块窗口内（区块 **5,105,776** — 2026年7月17日 12:50:53 UTC / 05:50 PDT），将您的MLNode切换至Kimi。
* 委托主机：选择非守护者Kimi主机，并将权重分散到独立目标上 — 请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。

**Devshard v1/v2被移除**

`versiond`已停止v1和v2的`devshardd`进程。所有网关流量必须使用`/devshard/v3`。如果您的网关在此次变更后停止提供推理服务，说明您仍在通过已被移除的路径路由 — 请遵循v3迁移指南。

## 2026年7月16日

**紧急治理投票（提案88）：重新注册Kimi-K2.6并移除devshard v1/v2运行时**

这是7月15日宣布的Kimi恢复计划的第二步。提案87已将`moonshotai/Kimi-K2.6`从活跃集合中移除；提案88将其重新注册到治理模型列表中，使其从第331个周期开始进入标准引导流程。

同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦通过，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）继续运行。此举消除了旧运行时容器长期存在的内存增长问题 —— 这正是7月15日事件中网络节点OOM的根本原因。
...
2. 提供Kimi服务的主机：在投票结束后、区块 **5,105,276**（2026年7月17日，约12:05 UTC）之前立即声明对`moonshotai/Kimi-K2.6`的意向，然后在第331个周期PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日，约12:50 UTC）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
3. 委托引导时：**请勿委托给守护者节点**；将委托分散到独立的Kimi主机上。请参阅 [多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/) 获取更新的委托指导。
4. 经纪人 / devshard创建者：如果您的任何网关流量仍在v1/v2或`/v1/devshard`上 —— 请在投票结束前切换至`/devshard/v3`。
...
- 意向截止时间（第330周期）：区块 **5,105,276** — 2026年7月17日，约12:05 UTC（约05:05 PDT）。
- 第331周期PoC开始：区块 **5,105,776** — 2026年7月17日 12:50 UTC（05:50 PDT）。
...
`moonshotai/Kimi-K2.6`在第328–329周期失去了PoC验证多数（详情见 [此处](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是最快恢复服务且最小化停机时间的方式 —— 与6月提案78采用的恢复路径相同。

该提案将在下一个PoC前从PoC参数中移除`moonshotai/Kimi-K2.6`，随后立即重新添加，Kimi将从第331周期开始进入标准引导流程。

**主机所需操作**

1. 在截止日期前对提案87进行投票 —— 紧急窗口时间很短。
2. 提供Kimi服务的主机：请保持Kimi配置就绪，并准备好在引导PoC时切换您的MLNode。
3. 在委托引导时：不要委托给守护节点；将委托分散到独立的Kimi主机上。
...
**截止日期*** 投票时间：2026-07-15 17:01 ~ 2026-07-16 05:01 (PDT) / 2026-07-16 00:01 ~ 2026-07-16 12:01 (UTC)（紧急，0.667 是票阈值；出勤率重要，请尽快投票）。
* 引导意图截止日期和纪元时间详见引导说明。

## 2026年7月15日

**Kimi-K2.6 事件（纪元 328–329）：发生了什么、恢复计划以及委托指导的变更**

在纪元 328 中，`moonshotai/Kimi-K2.6` 失去了其 PoC 验证的多数支持。在纪元 329 开始时，运行 Kimi 的主机被移出组。网络其余部分正常运行。

**发生了什么**

两台由守护者运营的运行 Kimi 的服务器同时失败：

* 其中一台服务器上，运行 Kimi 的 MLNode 因提供商端问题而崩溃。大量 Kimi 委托集中在这台主机上，因此其故障一次性移除了这些投票。
* 第二台服务器因内存不足导致网络节点中断。

Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。守护者决胜机制未启动：目前没有模型投票权重的守护者会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。

这种情况类似于纪元 306–307 事件。纪元 329 开始时的投票具体细节仍在确认中；本摘要可能存在小幅修正。
...
2. [明天] Kimi 将再次通过标准引导流程：声明意图、部署窗口、PoC 存储提交。在意图截止日期前将发布说明。

**委托指导变更——请阅读**

之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将守护者节点作为委托目标。此建议现已被撤回：

* **请勿委托给守护者节点。** 守护者是 PoC 验证的备用机制。将委托集中于它们会将主要机制和备用机制绑定到同一硬件——本次事件正是这种故障模式。协议层面的限制正在讨论中。
* **避免将委托集中于任何单一主机。** 在支持基于百分比的多主机委托之前，请将委托分散到多个独立运行该模型的主机上。如果您运营多个节点，请将委托指向您自己运行模型的节点。
* **如果您能直接运行 Kimi——这是最大的帮助。** 更多直接主机意味着更多验证权重且无单点故障。

**已在进行中的修复*** 守护者参与 PoC 验证（守护者始终可投票）——v0.2.14。
* 自委托问题——v0.2.14。
* Nonce 重复——v0.2.14（当前由早期 API 更新处理）。
* 网络节点内存增长——已在 devshard v3 中修复；需从 v1/v2 迁移。完整迁移后，v1/v2 将被移除，`/v1/devshard` 将关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（将发布公告），请对该提案进行投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非守护者委托目标；尽可能将权重分散到多个独立主机。
4. 经纪人：立即迁移 devshard 流量至 `/devshard/v3`。这将解决内存增长问题，并在移除 v1/v2 前必须完成。
5. 检查网络节点的内存并设置容器内存限制（`api`、`versiond`）。
...
这是一个仅限 devshard 的运行时升级。它独立于全链软件升级运行，无需链二进制升级。

提案通过后，v3 现在与现有 devshard 运行时并行运行。新进程在 `/devshard/v3` 前缀下提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人切换流量至 v3。

该版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 会自动下载该二进制文件，验证 sha256 哈希值，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

白名单上的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换到 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
发布版本将 `devshardd` 二进制文件作为 Gonka 发布工件发布。如果链上提案获得批准，`versiond` 将自动下载二进制文件，验证 sha256 哈希，并在现有的 `versiond` 容器内启动一个额外的 `devshardd` 进程。
...
devshard 部分准备了 v3 运行时，使代理能够在链升级期间不依赖已弃用的经典 API 路径提供推理服务，同时改进 RAM 利用率并启用 SQLite 和 Postgres 存储之间的安全切换。

**升级计划**

节点二进制文件通过链上软件升级提案进行升级。升级过程中，现有主机无需手动更新其 `api` 或 `node` 容器。
将从该分支发布一个独立的 devshard v3 版本，并在主网链升级前提案并部署。提前将推理流量切换到 `/devshard/v3` 的代理可以在链升级期间继续提供推理服务。

**建议流程**

1. 活跃主机在 [GitHub](https://github.com/gonka-ai/gonka/pull/PR) 上审查此提案。
2. devshard v3 发布版本将在主网链升级前提案并部署。
3. 经纪人将推理流量切换到 `/devshard/v3`。
...
Devshard v3 发布版本将在主网链升级前提出并部署，以便经纪人可以在升级运行前将推理流量移至 `/devshard/v3`。

**主机操作项**

1. 现在 —— 审查 PR。请阅读 GitHub 上的 PR #1267，并对任何发现、疑问、改进建议、边缘情况或漏洞留下评论。
2. 现在 / 主网升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据指南将 `api` 二进制文件和桥接器镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成上述两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
3. 对 devshard v3 进行投票。
4. 经纪人 —— 将推理流量切换到 `/devshard/v3`。一旦 devshard v3 发布版本部署完成，请将推理流量移至 `/devshard/v3`，以便在链升级期间继续提供推理服务。
5. 仪表板维护者 —— 准备好调整指标的统计方式。详细说明将在 `devshard v3` 投票启动并顺利接近结束之后发布。

## 2026 年 7 月 6 日

**安全更新：PoC-v2 权重验证加固 —— 更新您的 API 容器**

上周，HackerOne 上报告了 PoC-v2 权重路径中的一个漏洞。参与者可能夸大其计算权重，从而影响区块奖励、共识投票权和治理投票权。

根据历史数据，此前未观察到此类攻击。但在当前周期中，已检测到主机 `gonka1w7s4pharl5qs2lupxkuw2c0gzcls8chehwafg3` 使用了该攻击。

针对即将发布的 v0.2.14 链升级，已准备好修复方案，将永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复方案——可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
**Kimi-K2.6 引导：第311轮的又一次尝试**

`moonshotai/Kimi-K2.6` 在第311轮再次尝试引导。希望提供Kimi服务的主机必须在第310轮期间声明其意向，并在新轮次开始前准备好节点。一个守护节点也将运行Kimi，因此不希望运行它的主机可以将其权重委托给该节点。
...
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
2. 如果足够多的主机声明意向，请在第311轮开始前约500个区块的窗口期内将您的节点切换至Kimi（PoC开始时间约为6月28日 ~12:47 UTC）。
...
```
~~守护节点 `gonka1kx9mca3xm8u8ypzfuhmxey66u0ufxhs7nm6wc5` 将运行Kimi，可作为委托目标。~~

> **更新（2026年7月15日）：此建议已撤回——请勿委托给守护节点。** 守护节点是PoC验证的后备机制，必须与委托保持独立。请选择一个非守护节点且运行该模型的主机，并避免选择已是主要委托目标的主机。有关更新的委托指南，请参阅 [Multi-Model PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。该系数的选择使得在8xB300硬件上运行Kimi时，其共识权重与主模型MiniMax大致相当。此设计并非偏向任何硬件——MiniMax上已具备相同的权重。对于运行Kimi而非MiniMax的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的非参与惩罚）。该系数的选择使得在8xB300硬件上运行GLM-5.2时，其共识权重与MiniMax大致相当。此设计并非偏向任何硬件。对于运行GLM-5.2而非MiniMax的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
1. **声明意向。** 计划提供Kimi或GLM-5.2服务的主机，需在 `start_poc - deploy_window` 的引导快照前为该模型提交 `MsgDeclarePoCIntent`。
   
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 以及 >2/3 可达性）。这使操作员能够在配置硬件前确认模型是否具备可行性。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期内为其模型配置MLNode。
   
4. **PoC开始。** 成员资格由PoC开始时实际提交PoC存储提交的主机决定——而非仅由声明意向的主机决定。只有满足资格条件的模型才会被激活。

**若获批准的效果**

从第309轮开始（PoC开始时间约为6月26日 15:25 UTC；生效时间约为16:00 UTC），前提是每个模型均满足引导资格条件：
...
提案必须在第308轮结束前完成，以便两个模型都能在第309轮启动引导。由于每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时的紧急周期可以。创世守护节点预计将支持此提案。

**主机所需操作**

1. **要提供Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在PoC 309期间将您的MLNode切换至该模型。在PoC开始前有约500个区块的安全窗口，在此期间切换是安全的，因为该窗口内没有cPoC。

2. **要部署GLM-5.2：** 在部署窗口期间为`zai-org/GLM-5.2-FP8`声明意向并配置您的MLNode。部署是可选的——未选择的主机不会受到任何惩罚。
...
如果您没有直接访问拥有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`查看：任何活跃节点均可使用：
...
**提案78已通过：`MiniMaxAI/MiniMax-M2.7`成为唯一的PoC模型；Kimi K2.6和Qwen3-235B已被移除**
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍运行Qwen或Kimi的主机，在切换前将无法获得本纪元的cPoC奖励。
* 计划再次提供Kimi的主机应保持Kimi设置就绪，并在PoC 309时准备好将MLNode切换回Kimi——将在第308纪元内进行一次恢复Kimi的投票。

**接下来的内容**

在第308纪元内将进行一次加急投票，合并两项变更，均于第309纪元生效（PoC开始时间约为2026年6月26日15:25 UTC）：

* **恢复Kimi K2.6**——重新添加Kimi并重启其引导流程。
* **添加GLM-5.2**——添加GLM-5.2，**不设置不参与惩罚**，因此主机可选择性加入，提前衡量对该模型的需求，未选择部署的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一的PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前投票数不足，无法通过PoC验证。因此，运行Kimi的参与者被移出小组，Kimi无法进入下一个纪元。现在将其从活跃集合中移除并重新引导，是最快恢复其运行且停机时间最短的方式。
...
**每项变更的目的*** **Kimi K2.6——移除以便快速恢复。** 由于Kimi投票数不足，无法通过PoC验证，将其移除并重新引导是最快恢复且停机时间最短的路径。下一个纪元将进行恢复Kimi的投票。
* **Qwen3-235B——退役（独立、无关的变更）。** 退役较旧的Qwen3-235B是一项独立的、此前已提出的阵容调整。它与Kimi的情况无关，仅因同属PoC阵容变更而一并包含在此处。
* **MiniMax-M2.7 — 已提升为基础模型。**

由于重置必须在第307个周期结束前完成，因此这些变更被合并为一次紧急投票。每个周期约22.9小时，标准的48小时投票周期无法容纳在一个周期内，而12小时的紧急周期可以。创世守护者应支持本提案。
...
1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请提前下载FP8权重（若尚未预加载），以避免在周期边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划未来再次服务Kimi的主机仍应现在将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——第308周期将进行恢复Kimi的投票。


**接下来的内容*** **恢复Kimi K2.6** —— 第308周期将进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设置不参与惩罚**，因此主机可选择参与以提前测试模型需求，而不选择服务该模型的主机也不会受到惩罚。

**如何投票**

如果您无法直接访问持有投票权的密钥，或希望由另一个密钥代为投票，请参阅[指南](https://gonka.ai/docs/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将冷密钥的治理投票权限授予热密钥。提案详情和投票可通过`inferenced`访问：
...
* 第308个纪元将在不久后形成：PoC开始约16:50 UTC，生效约17:25 UTC。
...
对于此类仅devshard的运行时升级，无需重启节点容器或执行手动主机步骤。

**主要变更**

- 移除了种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中。
...
1. 移除种子揭示轮次，密封已完成的推理统计信息，并修剪有效载荷，以防止长时间运行的会话将所有已服务的推理保留在RAM或状态中
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。若链上提案获批准，`versiond`将自动下载二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。

新进程通过`/devshard/v2`前缀提供服务。现有的v1 devshard流量继续在`/devshard/v1`和`/v1/devshard`上运行。在此类升级期间，无需重启节点容器或执行手动主机步骤。
...
有意义的审阅贡献，包括重要评论、错误发现和安全问题，可能有资格在下一个升级周期中获得社区奖励。

本次仅为PR审阅邀请，不启动正式投票。治理投票流程将在审阅期结束后开始。
...
MiniMax的每模型参与强制执行现已生效。已为MiniMax选择DIRECT、DELEGATE或REFUSE的主机无需执行任何其他操作——现有设置将继续正常工作。尚未做出选择的主机请尽快选择，以避免每纪元惩罚（[https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal](https://gonka.ai/docs/host/quickstart/#optional-poc-delegation-and-refusal)）。
...
该提案已获批准，并于区块`4267300`成功在主网上执行升级。
...
**v0.2.13 投票结束 — 准备在高度 4267300 升级**

关于 [Upgrade Proposal v0.2.13](https://github.com/gonka-ai/gonka/pull/1143)（提案 ID `54`）的链上治理投票已结束。该提案已 **通过**。

升级将在主网 **区块高度 4267300** 自动执行（≈ **2026年5月26日 星期二 14:42 UTC** / **07:42 PDT**）。
...
2. 如果您计划提供 `MiniMaxAI/MiniMax-M2.7`，请现在预先下载约 230 GB 的 FP8 权重。在启动窗口期间，Hugging Face 的速率限制和带宽饱和可能导致错过首次资格检查。
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。仅运行其中一个或两个模型的主机仍需为其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会对其完整权重施加每纪元 15% 的惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 Qwen-on-B200 参考的 vLLM-0.20.1 吞吐量基线。此调整 **仅适用于您共识权重中 Kimi 相关部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
[v0.2.13 提案](https://github.com/gonka-ai/gonka/pull/1143) 已重新上链并开放投票。这是对先前发布但未通过提案的重新提交，现已更新若干内容。
...
- 治理：将创世守护者投票权降至约 25%，并设定链上法定人数为 0.25。若守护者弃权，非守护者需在剩余 75% 中实现约三分之一的参与率才能满足法定人数（参见 inference-chain 部分）。
...
- 将 devshard 随机数限制从 `20_000` 提升至 `1_000_000`。
- 将每纪元最大 devshards 数量从 `100` 提升至 `500_000`。
- 修复新模型启动期间确认 PoC 奖励核算。
...
- 将创世守护者调整后的投票权降低至约 25%，并将链上治理门槛设为 `0.25`。在守护者不参与投票的情况下，剩余 75% 投票权的有效门槛为 1/3（`0.25 / 0.75 = 0.334`）。
...
- 激活后，桥接将启用 Gonka 主网与 Ethereum 之间的跨链传输（包括在 Ethereum 上封装 GNK 以及桥接 USDC/USDT）。封装/解封装脚本和操作员工作流程将另行文档化。
...
请所有主机验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您正在运行当前版本，再执行任何进一步操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

请主机更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。如果您尚未本地克隆 `gonka` 仓库，请先遵循加入网络指南。此仪表板更新仅涉及容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约为 5 月 26 日 14:42 UTC → 升级周期剩余部分跳过确认 PoC（≤ 10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 第一个 MiniMax PoC 阶段在升级后下一个周期边界开始 → MiniMax 处罚在链周期 278 执行。

**注意**

- 请在升级窗口期间保持在线，以便及时应用任何后续步骤或缓解指令。
- 升级期间，Cosmovisor 会在 `.inference/data` 目录中创建完整状态备份；请确保有足够的磁盘空间（主网上 Cosmovisor 对 `application.db` 的备份通常为数十 GB，建议提前确认）。有关如何安全删除 `.inference` 目录中旧备份的指南，请参阅 [文档](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)。
...
- 该提案将有意在升级高度到升级周期结束（10000区块宽限期）之间跳过确认 PoC。若获批准，此跳过为预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。
- 如果提案未通过（未达到法定人数，或 `no_with_veto` 超过 ⅓），链上无任何变更，升级将不会发生。操作员可能会看到 `PROPOSAL_FAILED` 状态；这是预期行为，无需采取任何操作。
...
在第267轮中，PoC验证已成功完成，受影响的主机已能够重新加入活跃集合。
...
该问题已追溯至具有非标准语义的推理请求。此攻击向量影响了运行Kimi模型的主机，并扰乱了它们的PoC参与。
...
在相关保护措施实施期间，推理可能暂时不可用。访问预计将逐步恢复，首先通过若干社区代理和经纪人端点。
...
2. 监控下一几轮的PoC参与情况。检查您的节点是否如预期进入PoC，且活跃权重是否正确反映。
3. 仅使用受支持的推理请求格式。不要发送或路由具有非标准请求语义的推理流量。
4. 预期可能出现临时推理中断。访问可能不会立即在所有地方恢复，预计将通过社区代理和经纪人端点逐步恢复。
...
似乎PoC投票未能收集到本轮所需的投票数。确切原因尚未确认，社区正在积极调查此事。
...
社区成员目前正在审查第266轮发生的情况。一旦对根本原因有更清晰的了解，将分享更新。
...
- 必要准备：桥接容器检查、MiniMax决策、仪表板更新、投出选票。
- 除非提案获得批准，否则链上无任何变更。
...
请所有主机验证其桥接容器是否已部署、运行最新版本并同步正确。部分主机可能已部署桥接容器。在这种情况下，请先确认您运行的是当前版本，再执行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
如果您没有直接访问持有投票权密钥的权限，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何从冷密钥向热密钥授予治理投票权限。

提案详情与投票可通过 `inferenced` 查看。任何活跃节点均可使用。可用节点包括：
...
- 该提案将有意跳过从升级高度到升级周期结束（3000个区块的宽限期）的确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与有效负载存储使用相同的环境变量）。本地 SQLite 将保持默认设置，并自动清理（保留最近3个周期）。
...
有意义的审查贡献，包括重要评论、错误发现和安全问题，可能在下一个升级周期中符合社区奖励资格。
...
- 当参与者重新变为 ACTIVE 时，`ConsecutiveInvalidInferences` 未被重置。一个新错误推断会立即再次使其失效。计数器现在在重新激活和即将晋升时重置。
- 在 v0.2.12 之前加入的 DAPIs 在其冷到暖的授权授予中未包含 `MsgRespondDealerComplaints`。升级将回填该权限，以便它们可以响应交易商投诉。
...
- 内部 devshard 存储迁移不再阻塞 dapi 启动。在迁移和恢复完成前，devshard 路由保持不可用。

**devshard**

- 由于旧的托管数据保留在一个 SQLite 存储中，devshard 存储可能无限增长。现在存储按纪元范围管理，并在后台清理旧纪元，仅保留最近 3 个纪元。
- Devshard 需要为大规模部署提供共享存储选项。现在可以使用 Postgres 作为主存储，SQLite 仍作为本地备用。
...
我们建议替换api容器的二进制文件。除修复外，新版本还启用了devshard数据库的修剪功能，并为devshard状态增加了Postgres支持。
...
`moonshotai/Kimi-K2.6`已通过引导并加入Gonka网络的PoC参与。

该过程由网络中的主机协同完成：基础设施已准备就绪，意向已提交，委托与拒绝已设置，部署已测试。

对于多模型PoC，这意味着Kimi现在作为独立模型组拥有自己的参与和奖励追踪机制。
...
已提交PoCIntent的主机应在PoC于区块 `3874496` 开始前，将至少一个MLNode从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。
...
传输代理 `node1`、`node2` 和 `node3` 已被禁用。所有主网推理现在均通过 `node4` 路由，该代理采用新的 `devshard` 计费方式。

这标志着网络的一个重要里程碑：`devshard` 已上线并具备生产就绪能力。`node4` 将作为今后推荐的公共网关。

**需执行操作**：请将您的端点更新为 `node4`。

## 2026年5月2日

今日的预资格验证未通过，对于 `PoCIntent` 权重低于30% 的主机，请保留您的MLNode使用 `Qwen235B`，并于明天提交下一轮的意向。

## 2026年4月30日

**升级已执行：v0.2.12现已在主网上线**

针对升级提案v0.2.12的链上治理投票已结束。该提案已获批准，升级已在主网上成功执行。

**当前生效的关键变更**

- **多模型PoC（最大变更）**（[#1039](https://github.com/gonka-ai/gonka/pull/1039)）：将计算证明从单一固定模型过渡为按模型分组的PoC。每个经治理批准的模型生成自己的本地PoC权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托PoC投票权重）。
- **`moonshotai/Kimi-K2.6` 作为第二个模型引入**：该模型组将在升级后两个轮次激活。其系数为Qwen235B的3.51倍，基于相同硬件（8xH200、8xB200）上的模型计算复杂度。
...
- **协议加固**。保留节点（`POC_SLOT=true` 在单个 PoC / CPoC 时间内随机采样。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`，修复 DKG 发牌者共识，使遗留验证者惩罚与所需抵押品语义对齐，确保 devshard 保证金资金的原子性，并为 `inference_finished` 事件解析添加零时间戳容差。
...
有关这些变更的更多详情请参见治理文件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
此清理过程**必须在升级发生前完成**。如果在升级时您的节点配置包含不受支持的模型，**节点将被拒绝并下线**。

版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留此前强制执行的模型和 Kimi。
每个 DAPI 在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。如果配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将下线。

版本 0.2.11 通过将运行时视图裁剪为强制模型来掩盖了此问题，因此 `/admin/v1/nodes` 看起来是干净的，即使持久化配置仍包含额外模型。版本 0.2.12 停止了这种裁剪，意味着直接加载持久化配置。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固。** 对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间。其他更新包括将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及在 `inference_finished` 事件解析中添加零时间戳容差。
...
从 `docker-compose.yml` 部署 `versiond` 和 `proxy` 服务的最新版本（使用标签 release/v0.2.12 的仓库）：
```
...
如果您没有直接访问具有投票权的密钥，或希望使用其他密钥代为投票，请参阅[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)了解如何将治理投票权限从冷密钥授予热密钥。
...
有意义的审查贡献，包括重要评论、漏洞发现和安全问题，可能有资格在下一轮升级周期中获得社区奖励。

本次仅为拉取请求的审查请求，不启动正式投票。治理投票流程将在审查期结束后开始。

**关键变更**

- **多模型概念验证（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡到按模型分组的 PoC。每个经治理批准的模型生成自己的本地 PoC 权重，并通过模型特定系数聚合为总共识权重
- **共识层交易费用与自动迁移** ([#937](https://github.com/gonka-ai/gonka/pull/937), [#981](https://github.com/gonka-ai/gonka/pull/981))。引入由治理控制的汽油价格。协议职责消息（PoC、验证、推理、BLS DKG）通过 `NetworkDutyFeeBypassDecorator` 免费。`MsgPoCV2StoreCommit` 采用两部分费用（基础验证 + 计数线性）作为主要 Sybil 防御机制。详情请见 [docs/host_onboarding.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.12/docs/host_onboarding.md)。
- **Devshard 独立运行时** ([#1045](https://github.com/gonka-ai/gonka/pull/1045))。将 devshard 发布与 DAPI/主网发布周期解耦。
...
- **协议加固**。实现更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过独立治理投票激活。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 经销商共识、使旧验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- 确保冷钱包账户持有足够的金额（例如 100 GNK），以覆盖自动授予的费用限额支出。
...
此版本现已设为主分支默认版本：[https://github.com/gonka-ai/gonka/commit/ec8f45573149ce5686e8e5fc29f1a8f49a295689](https://github.com/gonka-ai/gonka/commit/ec8f45573149ce5686e8e5fc29f1a8f49a295689)

**变更内容**

此版本已在最近几个周期中被部分矿工使用。
初步观察表明，对于靠近 PoC 开始运行的节点，稳定性有所提升。
...
这些性能改进可根据工作负载和网络条件，使每个区块的推理数量提升高达 100 倍。
...
下一版本 v0.2.11 的链上软件升级提案现已上链并开放投票。若获批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改善 `Start`/`FinishInference` 的性能。

**关键变更****[初始扩展架构：基于 `devshards` 的推理会话](https://github.com/gonka-ai/gonka/pull/877)**

此升级引入了基于 `devshards` 的推理会话的初始版本，旨在提升推理的可扩展性。

目前，通过每项推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至分配的链下子组，而链上仅处理会话创建和最终结算。

这有意是一个早期且受限的版本。提出此版本用于主网审查和有限生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
在类似主网的条件下，这使得每个区块可容纳的推理数量最多提升 100 倍，具体取决于工作负载和网络条件。  ￼
有关这些及其他变更的更多详细信息，请参见：[https://github.com/gonka-ai/gonka/pull/813](https://github.com/gonka-ai/gonka/pull/813)
...
提前执行此操作至关重要。若大量节点推迟到升级后才进行剪枝，网络中将同时出现大量剪枝活动，造成可避免的操作压力。
...
目前，通过每个推理的链上交易处理推理会话限制了吞吐量。此设计将推理执行和验证移至指定的链下子组，而链仅处理会话创建和最终结算。

这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其被认为已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果许多节点推迟到升级后才进行剪枝，网络中可能会同时开始大量剪枝活动，造成不必要的操作压力。
...
在 `v0.2.10` 中发现了一个潜在漏洞。为降低当前升级前时期的潜在风险，建议在下一次 PoC 开始前将 api 二进制文件升级至 `v0.2.10-post7`。
...
对于已使用代理层的团队，这可能是简化堆栈并依赖原生行为的好机会。实际上，这将带来更清晰的集成模式和更易维护的系统。
...
如果您没有访问持有投票权的冷密钥的权限，建议提前安排投票委托。请联系该密钥的所有者，请求授予您代表其投票的权限。没有该授权，其他账户无法提交投票。
...
API 二进制文件的新版本已发布。该版本更新了连接超时处理，并在 PoC 验证管道中引入了额外的检查。

1. 升级 v0.2.10 引入了 Executor → MLNode 连接的严格 5 分钟超时，而某些请求可能需要更长时间。新版本的 API 将此值恢复，而非强制执行严格限制。
2. 之前的请求重试系统在因处理超时（而非 TLS 超时）失败时仍会重试推理。
服务器端对长请求的重试通常无效，因为会导致相同的超时场景。同时，客户端可能收到不一致的输出。新版本的 API 在此类情况下不再重试推理。
3. 当前被保留且不参与 PoC 生成的 MLNode 仍被用于 PoC 验证。这可能导致推理遗漏。新版本将此类节点排除在 PoC 验证之外。
4. PoC 验证管道中增加了额外的安全保障。
...
现已提供新的 vLLM / mlnode 构建，可在 PoC 开始时中断正在进行的推理请求，以降低因 PoC 开始时仍活跃的请求导致潜在权重下降的风险。
...
- 此构建旨在与前一版本向后兼容。
- 它已针对少量节点启用，但仍建议在部署前审查更改。

## 2026年2月19日

**抵押参数更新提案——投票结果**

抵押参数更新提案未达到法定人数，因此根据当前治理规则被否决。这意味着更新的参数将不会激活。
...
- 审查创世时定义的抵押参数。
- 在第180个周期前准备并存入所需的 GNK。
- 确保 [抵押](https://gonka.ai/host/collateral/) 设置正确，否则从第180个周期开始，PoC 奖励将减少5倍。

抵押激活是协议从宽限期过渡到完全抵押的 PoC 权重模型的一部分。治理仍然是调整参数的机制，但若无其他方案获批，则默认规则适用。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC 权重在各周期间可能波动。较小的权重可能经历相对更大的波动。为避免在周期边界处出现临时抵押不足，建议在抵押水平相对较低时存入最高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外的权重降低。协议不会自动补充抵押。
...
为将 PoC 权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选定。如实施所示，有效的 PoC 参考窗口比先前名义假设长约5个区块。
...
此前，PoC 权重计算依赖于名义周期持续时间假设。引入实时归一化后：

- PoC 持续时间与实际区块生成时间对齐
- 权重更准确地反映实际计算时间

由于有效归一化窗口比之前的名义模型长约5个区块，每个周期重新计算的权重相应降低。

**观察到的 GPU 权重变化（第175周期 → 第176周期）**
...
由于启用了PoC持续时间归一化，有效参考窗口现在比之前的名义假设长约5个区块，从第176轮开始的权重值反映了更新的计算模型。
...
!!! note "兼容性说明"
    ...
    **按实时时间标准化 PoC 权重**
    ...
    此次升级根据实际 PoC 经过时间对 PoC 参与者权重进行标准化，以减少区块时间漂移效应，并使权重结果与实际执行持续时间保持一致。
...
**宽限期：** 升级后引入了3000个区块的宽限期，期间无确认 PoC，并对升级所在纪元的错过率和无效率阈值采取较宽松的标准。
...
这意味着，在单个纪元内，即使被惩罚，矿工最多只会损失其抵押品的0.5%。所需抵押品仅占日奖励的约24%。

**警告：** 抵押参数将不受投票结果影响而生效。若本提案未通过，创世时定义的抵押参数将在第180纪元自动激活，而非上述参数。
...
要启用工具，必须重启MLNode容器内的vLLM。升级引入了一个3000个区块的宽限期，在此期间无确认PoC，并且在升级当期降低了误报率和无效率的阈值。
...
这意味着即使在一个周期内被惩罚，矿工最多也只能损失其抵押物的0.5%。而所需抵押物仅占日奖励的约24%。

提案提交投票后将另行发布公告。

警告：无论提案投票结果如何，抵押品都将生效。如果此提案未通过，创世中定义的抵押参数将在第180个纪元自动激活，而非上述列出的参数。

任何未来的抵押品增加都将通过单独的投票提出。目标是观察网络稳定性，并确保不公正的惩罚罕见且仅在有正当理由时适用。如果稳定性得到证实，逐步将抵押品增加到代币经济白皮书中描述的水平（例如，每块H100约100 GNK）将支持网络的长期成功。
...
请在GitHub上审阅v0.2.10升级PR并留下您的反馈。有意义的审查贡献可能在下一次升级中获得奖励。
...
请主机尽快应用此补丁，以确保正确的PoC验证行为并安全恢复区块生成。
```
# Download Binary
...
在当前纪元中观察到cPoC（确认PoC）与PoC之间的重叠。在纪元最后一个区块之前，`is_confirmation_poc_active`被观察为`true`。
...
链上推理访问当前已开放，不再仅限于开发者。推理请求可通过上一更新中引入的允许传输代理发送。当前允许列表可通过链上查询：
```
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一个链上软件升级 v0.2.10 已开放审阅。欢迎提供反馈和改进建议。当前计划是将审阅窗口保持约2天。

对于对本次PR审阅的有意义贡献，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审阅呼吁，并非正式投票的开始。治理投票流程将在审阅期结束后启动。

**关键变更****[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC验证采样优化**

此升级引入了一种新的PoC验证机制，通过为每个参与者分配固定的采样验证者集合，将复杂度从O(N²)降低至O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际PoC时间标准化PoC权重**

此升级通过实际PoC耗时标准化参与者权重，以减少区块时间漂移影响，并确保权重结果与真实执行时长一致。
...
- **[PR #641](https://github.com/gonka-ai/gonka/pull/641)** 带锁仓的流式分配转移
...
- **[PR #727](https://github.com/gonka-ai/gonka/pull/727)**：对#634的后续，将响应写入器传递给 `http.MaxBytesReader` 并对齐测试。
...
- **[PR #661](https://github.com/gonka-ai/gonka/pull/661)**：防止在网络短暂中断时API崩溃。
...
- **[PR #643](https://github.com/gonka-ai/gonka/pull/643)**：清理执行器选择中的空参与者。
- **[PR #545](https://github.com/gonka-ai/gonka/pull/545)**：API流程中的小bug修复。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此升级后重新检查。由于ibc-go升级到v8.7.0，枚举现在以字符串而非数字编码，int64/uint64值现在也以字符串编码。
...
- 观察到的变化表明，GPU权重差异现在反映的是模型特定的推理吞吐量，而非名义上的硬件规格。例如，H100 PCIe权重的下降幅度大于H100 HBM3权重，这与`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`的观察到的推理行为一致。
- 在当前模型配置下，基于观察到的推理轨迹，B200 GPU的推理性能并未高于H100类GPU。
- 如果未来通过治理引入更大或更复杂的模型（例如DeepSeek V3.2），可能会观察到不同的性能特征。
- 在PoC之外使用标准的vLLM推理对同一模型`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`进行的控制推理基准测试，显示了与PoC v2中观察到的相同GPU类型之间的相对性能差异。

**追踪器（仪表板）维护人员的操作**

在更新后的权重分配生效后，追踪器（仪表板）维护人员可能希望审查第159轮及之后的系数，以确保与当前PoC v2权重分配保持一致。
...
由于在当前的PoC周期中验证阈值可能无法达成的高风险，主机们集体采取行动暂停了区块验证。
...
所有主机必须随时准备安装补丁。
请保持在线并密切监控公告。一旦补丁准备就绪，将立即发布进一步的说明。

## 2026年2月1日

**升级已执行：v0.2.9 现已在主网上线**

针对升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并已在区块 2451000 成功在主网上执行升级。此次升级实现了 PoC v2 权重分配机制，并完成了向传统 PoC 机制的过渡。

**注意**

- 下一个 PoC 周期（从第 158 个纪元过渡到第 159 个纪元）至关重要。请确保在线，以便在需要时及时应用后续步骤或缓解指令。
- 仅运行 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 的 ML 节点有资格进入下一个（159）纪元并参与 PoC v2 权重分配。运行其他模型的 ML 节点将不会被纳入即将到来的纪元的参与集合中。
...
在因 cPoC 惩罚而减少或排除奖励的情况下，未计入的部分将转移至社区池。此前，此类奖励会在其他参与者之间重新分配。
...
- 运行其他模型的ML节点不包含在PoC v2权重分配中。在支持的情况下，可自动切换至`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
...
在因cPoC处罚而减少或排除奖励的情况下，未计入部分将转入社区池。此前，此类奖励会在其他参与者间重新分配。
...
分析表明，这是由于 PoC 验证参与不足所致。在多个案例中，参与者发布了非ces，但未执行验证，或验证水平远低于协议要求。
下表列出了上一纪元拥有权重、在当前纪元提交了 PoC 非ces，但未参与或未充分参与 PoC 验证阶段的参与者：[https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/](https://docs.google.com/spreadsheets/d/17agQXP77lATT2bNK12OEOzek5wNSptN2ktiSag3QXB0/)
...
本指南说明主机应如何根据v0.2.8模型可用性变更及即将推出的PoC v2更新来升级其ML节点。从第155个纪元开始，ML节点配置必须符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
`Qwen/Qwen3-32B-FP8`在迁移期间受支持，但不参与PoC v2就绪性或权重分配。参与PoC v2需要提供`Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`。
...
!!! note "重要"
    ...
    - 仅提供正确模型而不升级ML节点是不够的。
    - 不满足这两项条件的节点在网络切换为单模型配置后将不再符合资格。
...
!!! note "治理和 PoC v2 激活说明"
    ...
    **阶段 1. 观察（v0.2.8 之后的当前状态）**
    ...
    - 网络将观察采用情况，以评估主机为切换到 PoC v2 权重做好准备的程度。
...
- 将 PoC 直接集成到 vLLM 中，无需卸载模型或加载独立的 PoC 模型，即可立即从推理切换至 PoC。
- 使用 MMR（Merkle 山脉范围）承诺将工件存储移至链下——仅在链上记录 root_hash 和 count。
- 包含双模式迁移策略：V1 用于常规 PoC，V2 用于部署期间的 Confirmation PoC 跟踪。

**模型可用性变更**

作为 v0.2.8 升级的一部分，支持的模型集已更新。所有先前支持的模型均从活跃集中移除，但以下模型除外：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可评估是否具备过渡到 PoC v2 的条件。一旦在活跃 Host 中观察到足够水平的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
仅提供 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 而未升级 ML Node 不足以参与 PoC v2。一旦网络切换至单模型配置，未满足两项条件的节点将不被视为符合 PoC v2 参与资格。ML Node 的升级必须在 PoC v2 通过治理完全启用前完成。
...
**注意 1：** 请确保在升级窗口期间保持在线，以便在需要时及时执行后续步骤或缓解措施。
...
该提案将当前稳定期延长约两周，以便为即将到来的 PoC 变更及相关网络升级进行额外测试。有关新 PoC 开发进展的更多详情，请参见：[https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md](https://github.com/gonka-ai/gonka/blob/gm/poc-status/proposals/governance-artifacts/poc-update-status.md)。

延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换至 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。建议运行多个 ML 节点的主机在多个纪元中逐步进行更新。

**投票范围**

如果获得批准，网络将暂时继续在现有的 `allowlist`（包含未表现出非标准硬件行为的主机）下运行。

开发者 `allowlist` 将以相同偏移量延长，并将持续生效至区块 2459375。

未包含在 `allowlist` 中的主机在延长的稳定期内将无法参与 PoC，该阶段将在区块 2443558 结束。
...
`allowlist` 具有固定的过期时间，不会延续至延长的稳定期之外。一旦 `allowlist` 在区块 2443558 过期：
...
`Allowlist`到期时间：自动在区块 2443558 到期。

## 2026年1月10日

**临时参与者 `allowlist` 修正**

当前有一个新的治理投票正在进行。该投票通过将若干地址添加到[白名单](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)来修正一个过滤边缘情况，这些地址此前因硬件名称为空但ML节点权重为零而被过滤掉。该提案还向允许的开发者列表中添加了少量开发者账户，并将 `allowlist` 的到期时间与区块 2,222,222 的参与者注册截止时间对齐。
...
`Allowlist`过期：自动在区块2,222,222处过期。
...
该提案已获得批准。此提案定义了一个临时 `allowlist`，反映在最近多个纪元中行为保持一致的参与者。

**当前生效的关键变更**

1) 网络将使用由以下参与者组成的 `allowlist`：

- 报告的硬件特征与常见观测到的配置模式匹配（过滤后的非常规配置字符串列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt))
- 证明的PoC权重未超过同类硬件观测到的权重的150%

2) 之前表现出不符合这些模式的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块2,222,222处结束。

**执行特性**

- `allowlist` 将从下一个纪元（第135个纪元）开始生效
- 激活发生在第135个纪元的首次PoC期间
- 无需软件升级
...
在成功采用解决PoC相关共识故障的补丁后，一项新的治理投票当前正在生效。

随着区块生产恢复正常，网络即将进入一个短暂的稳定期，为后续增长做准备。

此次投票定义了稳定窗口期间的参与者 `allowlist`（[https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv](https://github.com/product-science/filter/blob/main/artifacts_end2end/allowlist.csv)），反映那些行为始终符合网络预期的参与者集合。

**投票范围**

若获得批准，网络将暂时使用一个 `allowlist`，包含在先前周期中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个周期中满足以下条件的参与者：

- 报告的硬件特征已与预定义的常见硬件配置模式进行评估，以识别偏差和不一致（确切的非常规配置字符串列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），以及
- 观测到的PoC权重低于使用同类硬件的其他参与者所表现权重的150%。
之前持续偏离这些模式的参与者在稳定窗口于区块2222222结束前不包含在 `allowlist` 中。
...
- `Allowlist`激活：在下一个PoC执行区块2089140后。
- `Allowlist`过期：自动在区块2222222。
...
为恢复可靠的共识进展，至少需要**67%**的网络活跃算力安装该补丁。
...
建议主机保持在线并关注更新，因为补丁发布后可能需要采取后续操作。

## 2026年1月8日

**v0.2.7升级提案：主网上线的创世验证者增强**

链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强的提案已获得批准，并在主网上成功部署。

**当前生效的关键变更：****创世验证者增强（临时）**

- 临时重新激活创世验证者增强——此前曾使用过的、限时防御机制，现提议重新启用。
...
本次升级正式将此前通过手动API更新分发且已在网络中使用的关键修复纳入协议。这些修复包括：
...
- 主机级别注册：在区块2.222.222之前（约两周后），将暂停新主机的注册。此措施旨在稳定网络并为其进一步增长做准备。
- 开发者地址注册：在稳定期内将暂停新开发者地址的注册。一个预定义的`allowlist`开发者地址白名单立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续至区块2.294.222（约19天）。

**治理控制机制**

本次升级包含的预备性变更，为未来通过治理控制参与者入驻和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
- 所有在第117轮期间活跃的节点，将额外获得等于第117轮奖励1.083倍的发放，统一适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
- Cosmovisor 在升级期间会为 `.inference/data` 目录创建完整备份。请确保有足够的磁盘空间。如果磁盘使用率较高，`.inference` 中的旧备份[可以安全删除。](https://gonka.ai/FAQ/#how-much-free-disk-space-is-required-for-a-cosmovisor-update-and-how-can-i-safely-remove-old-backups-from-the-inference-directory)
...
这可能会减少磁盘使用量和升级时间。但是，如果升级失败，将没有备份可用于恢复先前状态。
...
创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内激活。当前治理提案旨在根据当前网络状况，临时重新激活这一现有机制，并激活一些额外的保护措施。
...
- 主机级注册：在区块2,222,222之前（约两周后）暂停新主机注册。此措施旨在稳定网络并为其进一步增长做好准备。
- 开发者地址注册：在稳定期间暂停新开发者地址注册。预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续至区块2,294,222（约19天）。

**治理控制机制**

本升级包含的预备变更，为未来通过治理控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案不启用任何此类治理激活的限制，需另行投票决定。
...
- 在第117轮期间活跃但未收到该轮奖励的节点，将补发该轮遗漏的奖励。
- 所有在第117轮期间活跃的节点，将额外获得相当于第117轮奖励1.083倍的支付，均匀适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于平常的推理遗漏率。
在许多情况下，这是由一个漏洞引起的：未被标记为已完成的不支持格式的推理请求，尽管请求本身已被处理。以下更新将解决此行为。

参考：[https://github.com/gonka-ai/gonka/pull/517](https://github.com/gonka-ai/gonka/pull/517)

此 `API` 版本改进了对失败推理的处理弹性，并减少了推理遗漏统计问题。它还为PoCBatch和PoCValidation交易引入了批处理功能。
