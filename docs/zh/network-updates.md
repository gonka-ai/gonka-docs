...
**DeepSeek V4 Flash 启动：意向阈值已达成——请在第360个纪元快照前完成委托**

`deepseek-ai/DeepSeek-V4-Flash-0731` 在第359个纪元快照时未达到预资格条件。此后，实时意向和委托权重已满足 `V_min`、`W_threshold` 以及 **>2/3 可达性**（DeepSeek 意向权重加上委托给这些主机的权重）。下一次**官方**检查为第360个纪元启动快照——在此前请保持委托分散，以确保快照有效。
...
```
2. 如果模型已预合格，请在 PoC 开始前约500个区块的部署窗口内（区块 **5,552,115**）部署 DeepSeek。使用 vLLM 0.25.1 + 发布候选节点配置。

**继续运行其他模型的主机**

在区块 **5,551,615** 之前，提交委托（有助于启动可达性）或拒绝。从第360个纪元起，DeepSeek `penalty_start_epoch` 将生效——未明确选择的主机将面临不参与惩罚。
...
**关键时间点*** 第360个纪元快照（`start_poc − deploy_window`）：区块 **5,551,615** —— 约为 2026年8月14日 01:24 UTC
* 第360个纪元 PoC 开始：区块 **5,552,115** —— 约为 2026年8月14日 02:09 UTC
* DeepSeek 不参与惩罚从第360个纪元开始
...
```
该技能部署模型，测量PoC吞吐量，并将您的输出向量与黄金参考（`deepseek-ai-deepseek-v4-flash-0731.json`）进行比对。如果通过，您就没问题了。

如果您希望提供DeepSeek服务，请使用上述步骤进行设置。否则，请勿忘记委托，以避免非参与惩罚。

提醒：明天将发布带有更好日志的另一个MLNode构建版本，因此请期待后续简短更新。
...
1. 声明意向。在提案94通过后、在区块5,536,224（约2026年8月13日02:40 UTC / 8月12日19:40 PDT）的启动快照之前，提交：
```
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 切换您的MLNode。在PoC 359启动前约500区块的安全窗口内（区块5,536,724，约2026年8月13日03:24 UTC / 8月12日20:24 PDT），配置并切换您的MLNode至DeepSeek V4 Flash（vLLM 0.25.1 + 发布候选配置）。在此窗口期内切换是安全的，因为其中没有cPoC。

注意：在Blackwell GPU上，为获得最佳性能，您可以使用以fp8 + nvfp4重打包的模型而非fp8 + fp4，例如`MJPansa/DeepSeek-V4-Flash-0731-NVFP4`。模型的精度相同。部署时，将在启动前发布并共享更新的API二进制文件。
3. 如果您委托。请勿委托给监护节点；将委托分散到独立的DeepSeek主机上。参见[多模型PoC指南](https://gonka.ai/docs/host/multi_model_poc/)。
4. 要保留当前模型。在第360轮之前无需操作。从第360轮起，若您未提供DeepSeek服务，请提交`PoCDelegation`或`PoCRefusal`，否则将适用15%的非参与惩罚。
...
本提案在网络上新增DeepSeek V4 Flash作为新的PoC模型。未修改任何现有参数或模型，唯一更改是新增模型条目。

详情：

- 模型：`deepseek-ai/DeepSeek-V4-Flash-0731`（固定版本`7872f01b1d1fe23eabc4c98b48bffcef5a386062`）
- 需要在vLLM 0.25.1上运行的MLNode，并使用发布候选节点配置（`node-config-release-candidate-*.json`适用于B300 / B200 / H200 / H100）
...
./inferenced tx inference declare-poc-intent deepseek-ai/DeepSeek-V4-Flash-0731
```
2. 要保留您的当前模型：在第360个周期前无需任何操作；从第360个周期开始，如果您未提供DeepSeek服务，请提交 `PoCDelegation` 或 `PoCRefusal`。
3. 请在截止日期前对提案94进行投票。
...
此版本提高了PoC种子的可靠性，并修复了多个API和部署问题。

**变更内容**

- PoC 种子稳定性。在周期边界延迟后恢复遗漏的种子提交，并在瞬时故障（如签名者不可用）后重试本地种子恢复，以防止验证者被归零。
- API。
...
这是一个非关键性、仅限devshard的升级，独立于完整链软件升级运行。它将现有的devshard v4运行时替换为v4.0.1。

这只是一个性能和资源管理补丁，别无其他。协议、推理、验证和经济行为均保持不变。变更包括：周期跟踪从每个区块的链上查询改为事件驱动更新，主机会话和验证工作线程的内存清理现在与周期修剪对齐。

**升级计划**

devshard运行时通过链上参数提案升级，而非完整链软件升级。若获得批准，`versiond` 将自动下载并验证替换二进制文件。无需重启主网或手动操作主机。
...
该提案已获批准，并于主网第 5316315 块成功执行升级。该提案同时批准了 devshard v4 运行时，该运行时将与现有的 v3 运行时并行运行。
...
- 投票截止时间：UTC 时间 2026 年 7 月 30 日上午 07:08 / PDT 时间 2026 年 7 月 30 日凌晨 12:08
- 提议的升级高度：5316315
...
2. **v0.2.15 主网软件升级**，移除大量过时代码、遗留的交易处理路径及相关组件，这些组件已不再符合当前架构的需求。
...
v4 是首个支持多实例高可用的版本：在共享 Postgres 上，由 versiond-router 负责路由，N 个版本化的 devshardd 副本采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链通信。公共可观测性是无版本的（/devshard/sessions|stats|metrics）；只有托管方通过签名聊天绑定。当治理发布同名新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排水，确保正在进行的工作（包括 SSE）在旧版本上完成。
...
此清理简化了代码库，减少了不必要的复杂性，并提升了系统的整体稳定性和可维护性。同时，通过移除过时行为并围绕当前支持的架构整合交易处理流程，使未来的开发和升级更加容易。
...
如果您没有直接访问拥有投票权密钥的权限，或希望由其他密钥代为投票，请参考[指南](https://gonka.ai/FAQ/#what-should-i-do-if-i-cannot-vote-because-i-do-not-have-access-to-the-cold-key-or-if-i-want-another-key-to-vote-on-my-behalf)，了解如何将治理投票权限从冷密钥授予热密钥。
...
- 现在 / 主网上线升级前 —— 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器稳定，请提前根据[指南](https://gonka.ai/docs/FAQ/#upgrade-v0214-pre-upgrade-api-and-bridge-update)将 `api` 二进制文件和桥接器镜像更新至 0.2.14-post3。如果您的 `api` 二进制文件已更新，则只需更新桥接器镜像并重启桥接容器。如果您已完成上述两步，则无需重复操作。若您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。

- 仪表板维护者 —— 本次升级未移除或重构任何现有查询端点。如果您的仪表板从链上读取并显示数据，它将继续正常工作。您需要了解一个语义变更（委托），以及三个小注意事项（详见完整指南：[https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/](https://gonka.ai/docs/dashboard-maintainer-memo-v0.2.14/)）
...
- 如果 `application.db` 占用大量磁盘空间，可应用[指南](https://gonka.ai/FAQ/#why-is-my-applicationdb-growing-so-large-and-how-do-i-fix-it)中描述的清理技术。
- 若获批准，升级后 devshard 存储可选择由共享的 Postgres 实例提供支持（与负载存储使用相同的环境变量）。本地 SQLite 将保持默认，且会自动清理（保留最近 3 个周期）。
...
- v4 的主要目标是在主机故障或升级期间实现 devshard 主机的高可用性。当一台机器宕机或重启时，它仍能继续处理新请求；在版本升级时，多台机器可逐台替换版本。这是将单体架构重构为高可用、容错、可扩展架构系列变更中的首次更新。
- v4 是首个面向多实例高可用的版本：在共享 Postgres 上，通过 versiond-router 部署 N 个 versiond/devshardd 副本，采用粘性会话路由和验证租约排他性。网关仅通过 gRPC 与链交互。公共可观测性为无版本化（/devshard/sessions|stats|metrics）；仅托管方通过签名聊天绑定。当治理发布具有相同版本名称的新二进制文件（名称不变，仅 sha256 变化）时，versiond 可执行蓝绿切换并排空，以确保正在进行的工作（包括 SSE）在旧版本上完成。
- v4 还包含错误修复和安全补丁。
...
同一提案还从`devshard_escrow_params`中移除了`approved_versions`的v1和v2版本。一旦获批，`versiond`将停止v1和v2的`devshardd`进程，仅保留v3运行时（`/devshard/v3`）运行。这消除了旧运行时容器长期存在的内存增长问题 —— 也就是7月15日事件中网络节点OOM的根本原因。
...
2. 提供Kimi服务的主机：在投票结束**后**、区块 **5,105,276**（2026年7月17日，约UTC 12:05）之前声明对`moonshotai/Kimi-K2.6`的意向，然后在第331轮PoC开始前的约500个区块窗口内（区块 **5,105,776**，2026年7月17日，约UTC 12:50）切换您的MLNode：
```
./inferenced tx inference declare-poc-intent moonshotai/Kimi-K2.6
```
...
- 意向截止（第330轮）：区块 **5,105,276** — 2026年7月17日，约UTC 12:05（约05:05 PDT）。
...
`moonshotai/Kimi-K2.6`在第328–329轮中失去了PoC验证多数（详见[事件详情](https://gonka.ai/docs/network-updates/#july-15-2026)）。现在将其从活跃集合中移除并重新引导，是使其尽快恢复且停机时间最短的方式 —— 与6月提案78所用的恢复路径相同。
...
2. 提供Kimi服务的主机：请保持Kimi设置就绪，随时准备在引导PoC时切换您的MLNode。
...
Kimi 的验证投票一直接近 2/3 阈值，因此这次故障足以使该组低于多数。监护人投票决胜机制未启动：目前，没有模型级投票权重的监护人会被从决胜机制中过滤掉。这是一个已知问题，将在即将发布的 v0.2.14 升级中修复。
...
之前的指导（包括 2026 年 6 月 27 日的引导公告）建议将监护人节点作为委托目标。此建议现已撤销：

* **请勿委托给监护人节点。** 监护人是 PoC 验证的后备机制。将委托集中于它们会将主机制和后备机制绑定到同一硬件上——本次事件正是这种故障模式。协议层面的限制正在讨论中。
...
* 网络节点的内存增长——已在 devshard v3 中修复；需要从 v1/v2 迁移。完全迁移后，v1/v2 将被移除且 `/v1/devshard` 关闭。为 `api` 和 `versiond` 容器设置独立内存限制。

**主机所需操作**

1. 一旦紧急 Kimi 提案上链（后续公告），请对该提案进行投票。
2. 如果您计划提供 Kimi：关注引导说明，并准备好声明意图。
3. 如果您为 Kimi 委托：选择非监护人委托目标；尽可能将权重分散到多个独立主机上。
4. 经纪人：立即把 devshard 流量迁移到 `/devshard/v3`。这将解决内存增长问题，并在移除 v1/v2 之前必须完成。
...
提案批准后，v3 现在与现有 devshard 运行时并行运行。新流程通过 `/devshard/v3` 前缀提供服务，而现有 devshard 流量可继续在早期运行时前缀上运行，直到经纪人将流量切换到 v3。

此发布将 `devshardd` 二进制文件作为 Gonka 发布工件发布。`versiond` 将自动下载该二进制文件，验证 sha256 哈希，并在现有 `versiond` 容器内启动额外的 `devshardd` 进程。

对于这种仅限 devshard 的运行时升级，无需进行主网重启或手动主机操作。

**操作项**

已列入白名单的 devshard 创建者应在主网 v0.2.14 链升级前将推理流量切换至 `/devshard/v3`。这将使他们在链升级期间继续提供推理服务，而无需依赖已弃用的经典 API 路径。
...
新进程通过 `/devshard/v3` 前缀提供服务。现有的 devshard 流量可继续使用较早的运行时前缀，直到代理将流量切换至 v3。在此类升级期间，无需进行主网重启或手动主机操作。
...
投票结束：2026 年 7 月 11 日 06:41:34（UTC）
...
节点二进制文件通过链上软件升级提案进行升级。现有主机无需在升级过程中手动更新其 `api` 或 `node` 容器。
将从该分支单独发布 devshard v3 版本，并在主网链升级前提出并部署。提前将推理流量切换至 `/devshard/v3` 的代理可在链升级期间继续提供服务。
...
2. 现在 / 主网升级前 — 更新您的 API 和桥接器。为确保主网升级期间以太坊桥接器的稳定性，请提前根据指南将 `api` 二进制文件和桥接镜像更新至 0.2.14。如果您的 `api` 二进制文件已更新，则只需更新桥接镜像并重启桥接容器。如果您已完成以上两个步骤，则无需重复操作。如果您有多个节点，请逐个更新，并在非 PoC 或 cPoC 期间执行此步骤。
...
针对即将发布的 v0.2.14 链升级，已准备修复方案以永久解决此问题。由于攻击已在使用中，因此在升级前提供仅 API 的热修复版本 — 可异步部署，通过强化 PoC-v2 验证来阻止攻击，使复制计算无法通过采样检查。
...
`moonshotai/Kimi-K2.6` 在第311轮再次尝试启动。希望提供 Kimi 服务的主机需在第310轮期间声明意向，并在新轮次开始前准备好节点。守护节点也将运行 Kimi，因此不愿运行它的主机可以将权重委托给它。
...
> **更新（2026年7月15日）：此建议已撤回 —— 请勿委托给守护节点。** 守护节点是 PoC 验证的后备机制，必须与委托保持独立。请选择运行该模型的非守护节点，并避免选择已是主要委托目标的节点。有关更新的委托指南，请参阅 [Multi-Model PoC 指南](https://gonka.ai/docs/host/multi_model_poc/)。
...
`Kimi-K2.6` 将与 `weight_scale_factor=0.9` 一同恢复。此系数的选择使得在8xB300硬件上运行 Kimi 时，其共识权重与主模型 MiniMax 大致相当。该设置并非为任何硬件偏袒——相同权重在 MiniMax 上已可用。对于运行 Kimi 而非 MiniMax 的8xB300主机，将获得约1%的收益。
...
`GLM-5.2` 将与 `weight_scale_factor=2.47` 和 `penalty_start_epoch=500` 一同添加（这实际上取消了该模型的不参与惩罚）。该系数的选择使得在8xB300硬件上运行 GLM-5.2 时，其共识权重与 MiniMax 大致相当。该设置并非为任何硬件偏袒。对于运行 GLM-5.2 而非 MiniMax 的8xB300主机，将获得约2%的收益。
此方案使其非常适合8xB300服务器。
...
2. **预资格快照。** 在 `start_poc - deploy_window`，链将快照意向和委托，并发出建议性的预资格事件（治理批准、权重阈值 `W_threshold`、最低成员数 `V_min` 和 >2/3 可达性）。这使操作者能在配置资源前确认模型是否可行。
   
3. **部署窗口。** 已声明意向的主机在部署窗口期间为其模型配置 MLNode。
   
4. **PoC 开始。** 成员资格由在 PoC 开始时实际提交 PoC 存储提交的节点决定——而非仅由声明意向的节点决定。只有满足资格条件的模型才会激活。

**若获批准的影响**

从第309轮开始（PoC 开始时间约6月26日15:25 UTC；生效时间约16:00 UTC），前提是每个模型均满足启动资格：
...
该提案必须在第308轮结束前完成，以便两个模型都能启动进入第309轮。每轮约22.8小时，标准48小时投票周期无法容纳在一个轮次内；而12小时紧急周期可以。创世守护节点预计支持本提案。

**主机需采取的操作**

1. **要提供 Kimi：** 在投票结束**后立即**为 `moonshotai/Kimi-K2.6` 声明意向。然后，在 PoC 309 期间将您的 MLNode 切换至该模型。在 PoC 开始前有约500个区块的安全窗口，在此期间切换是安全的，因为此期间无 cPoC。
...
**主机所需操作*** 请确保您的MLNode正在提供`MiniMaxAI/MiniMax-M2.7`。任何仍在使用Qwen或Kimi的主机，直到切换为止，将无法获得本周期的cPoC奖励。
...
* **添加GLM-5.2**——添加GLM-5.2，**不设不参与惩罚**，因此主机可自愿选择参与，以便提前衡量模型需求，不提供该模型的主机不会受到任何惩罚。

## 2026年6月25日

**加急治理投票（提案78）：MiniMax-M2.7成为唯一PoC模型；Kimi K2.6被移除以便快速重启；Qwen3-235B退役**

`moonshotai/Kimi-K2.6`目前缺乏足够的投票支持以通过PoC验证。因此，提供Kimi的参与者已被移出组别，Kimi无法进入下一个周期。现在将其从活跃集合中移除并重新引导，是使其尽快回归且停机时间最短的最快方式。
...
**每项变更的目的*** **Kimi K2.6——被移除以便快速恢复。** 由于Kimi缺乏足够的PoC验证投票，将其移除并重新引导是最快且停机时间最短的回归方式。下一个周期将进行一次投票以恢复Kimi。
* **Qwen3-235B——已退役（独立、无关的变更）。** 退役旧版Qwen3-235B是一项独立的、此前已请求的阵容调整。此变更与Kimi情况无关，仅因同属PoC阵容调整而一并包含在此处。
...
从epoch 308开始（~6月25日17:25 UTC），`MiniMaxAI/MiniMax-M2.7`将成为唯一的活跃PoC模型。Qwen3-235B和Kimi K2.6将不再活跃。恢复Kimi将在下一个epoch的后续投票中处理。

**主机所需操作**

1. **在PoC 308开始前，将您的MLNode切换至`MiniMaxAI/MiniMax-M2.7`。** 所有主机——包括当前使用Qwen或Kimi的主机——都必须将MLNode切换至MiniMax。在PoC开始前有约500个区块的窗口期可以安全切换，因为此时没有cPoC。请现在预下载FP8权重（如尚未就绪），以避免在epoch边界时遭遇Hugging Face速率限制。
2. **在截止日期前对提案78进行投票。** 紧急窗口期很短。
3. 计划未来再次提供Kimi的主机，现在仍应将MLNode切换至MiniMax，但需准备好在PoC 309时将其切换回来——epoch 308将进行恢复Kimi的投票。


**下一步计划*** **恢复Kimi K2.6** —— 在epoch 308进行后续投票，以重新添加Kimi并重启其引导过程。
* **GLM-5.2** —— 后续提案将添加GLM-5.2，**且不设不参与惩罚**，因此主机可选择加入，提前测试模型需求，而不参与的主机不会受到任何惩罚。
...
发布版本将`devshardd`二进制文件作为Gonka发布工件。`versiond`会自动下载二进制文件，验证sha256哈希，并在现有的`versiond`容器内启动额外的`devshardd`进程。
...
本提案涵盖devshard v2发布：[https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0](https://github.com/gonka-ai/gonka/releases/tag/release/devshard/v2.0.0) 
这是首次仅针对devshard的升级。它独立于全链软件升级。若获批，v2将与现有的v1 devshard运行时并行运行。
...
升级将在主网 **区块高度 4267300** 自动执行（约 **2026年5月26日 星期二 14:42 UTC** / **07:42 PDT**）。
...
3. 升级完成后，每个主机都需要为 **每个** 经治理批准的模型声明参与模式 — `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`、`moonshotai/Kimi-K2.6` 和 `MiniMaxAI/MiniMax-M2.7`。即使仅运行其中一两个模型的主机，也必须对其他模型选择 DELEGATE 或 REFUSE。MiniMax 截止时间为 **链纪元 `278`**。从纪元 278 开始，未采取任何操作的主机会遭受每纪元 15% 的全权重惩罚。
...
5. v0.2.13 校准将 Kimi K2.6 `WeightScaleFactor` 从 `1.26` 调整为 `0.78`，以反映 vLLM-0.20.1 发布后 Qwen-on-B200 参考的吞吐量基准。该调整 **仅适用于您共识权重中 Kimi 相关部分**；您的 Qwen 相关权重和 Kimi 内部 PoC 分布保持不变。在 B200/B300 上，Kimi 仍是收益最高的选项；在 H100/H200 上，MiniMax-M2.7 成为与 Qwen 相当、高于 Kimi 的选项。
...
- 将 devshard 非负数上限从 `20_000` 提高至 `1_000_000`。
...
请所有主机验证其桥接容器已部署、运行最新版本并正确同步。部分主机可能已部署桥接容器。若如此，请先确认您运行的是当前版本，再进行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)

**仪表板/浏览器更新（升级前或升级后）**

请主机更新仪表板/浏览器。请从 `gonka/deploy/join` 目录运行以下命令。若您尚未本地克隆 `gonka` 仓库，请先遵循加入网络指南。此仪表板更新仅为容器拉取，无论投票结果如何，均可在投票结束前或结束后安全运行。
```
...
- 操作员时间线：投票于 5 月 22 日 22:12 UTC 结束 → 升级高度约 5 月 26 日 14:42 UTC → 升级周期剩余阶段跳过确认 PoC（≤10000 区块宽限期）→ MiniMax 启动快照在 start_poc − 500 块时（约提前 43 分钟）→ 首次 MiniMax PoC 阶段在升级后的下一个周期边界开始 → MiniMax 处罚在链周期 278 时执行。
...
- 该提案将有意在升级高度到升级周期结束（10000 区块宽限期）之间跳过确认 PoC。如果获得批准，此跳过是预期行为，并非故障；新的快照逻辑将在下一个周期开始。
...
4. 预期会出现临时推理中断。访问可能不会立即在所有地方可用，预计将通过社区代理和经纪人端点逐步恢复。
...
社区成员目前正在审查第266轮期间发生的情况。一旦对根本原因有更清晰的认识，将分享更新。
...
- 将创世守护者调整投票权减少至约 25%，并将链上治理门槛设为 `0.25`。在守护者不参与投票的情况下，其余 75% 投票权的有效门槛为 1/3（`0.25 / 0.75 = 0.334`）。
...
所有主机需验证其桥接容器已部署、运行最新版本且同步正确。部分主机可能已部署桥接容器。若如此，请先确认您运行的是当前版本，再执行后续操作。
请遵循以下说明：[https://gonka.ai/docs/network-updates/#may-7-2026](https://gonka.ai/docs/network-updates/#may-7-2026)
...
- 如果获得批准，升级后 devshard 存储可选择由共享的 Postgres 实例支持（与有效载荷存储使用相同的环境变量）。本地 SQLite 将保持默认，并自动清理（保留最近 3 个周期）。
...
- 升级为当前纪元安装了一个宽限期条目，延长了 `UpgradeProtectionWindow`（3000 个区块）。从升级高度到升级纪元结束期间，确认 PoC 触发将被跳过，因此新的快照逻辑仅从下一个纪元开始运行。复用 v0.2.10 的宽限期原语。
...
提交了PoCIntent的主机应在PoC于块 `3874496` 开始前，将至少一个MLNode从 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 切换至 `moonshotai/Kimi-K2.6`。
...
这些变更的更多详情请参见治理文件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.12/proposals/)
...
此清理过程**必须在升级前完成**。若升级时节点配置包含不受支持的模型，**节点将被拒绝并离线**。

版本 0.2.12 将移除所有不在升级后批准列表中的治理模型。在主网上，仅保留此前强制实施的模型和 Kimi。
每个 DAPI 会在本地持久化其 MLNode 配置。启动时，它会将每个配置的模型与链上治理列表进行验证。若配置包含至少一个不受支持的模型，整个节点将被拒绝，主机将离线。
...
- **多模型 PoC（最大变更）** ([#1039](https://github.com/gonka-ai/gonka/pull/1039))。将计算证明从单一固定模型过渡为按模型分组的 PoC。每个经治理批准的模型生成其自身的本地 PoC 权重，再通过模型特定系数聚合为总共识权重。每个主机必须参与每个模型组（直接参与或委托 PoC 投票权重）。
...
- **协议加固**。对保留节点（`POC_SLOT=true`）随机采样进行单次 PoC/CPoC 时间验证。其他更新包括：将 `mlnode` 版本传播至链上 `HardwareNode`、修复 DKG 交易方共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
- **协议加固**。实施更强的 PoC v2 RNG（256 位全熵，此前为 32 位），将通过单独的治理投票激活。其他更新包括：将 `mlnode` 版本传播到链上 `HardwareNode`、修复 DKG 交易商共识、使遗留验证者惩罚与所需抵押品语义对齐、确保 devshard 托管资金的原子性，以及为 `inference_finished` 事件解析添加零时间戳容差。
...
有关这些变更的更多详细信息，请参阅治理文件：[https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/](https://github.com/gonka-ai/gonka/tree/upgrade-v0.2.11/proposals/)
...
下一个链上软件版本 v0.2.11 的升级提案现已上链并开放投票。若获得批准，该提案将引入基于 `devshards` 的推理会话的初始版本，以提升推理可扩展性，并显著改进 `Start`/`FinishInference` 性能。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完成，而是因为此类系统需要尽早暴露于真实网络环境中。某些类型的问题仅通过本地测试难以发现。当前实现已设计为避免对矿工奖励造成负面影响。
...
提前执行此操作非常重要。如果大量节点推迟到升级后才进行剪枝，网络中将同时出现大量剪枝活动，造成不必要的运维压力。
...
这有意是一个早期且受限的版本。它被提议用于主网审查和有限的生产测试，并非因为其已完善，而是因为此类系统需要尽早暴露于真实网络条件中。某些类型的问题仅靠本地测试难以发现。当前实现已设计为避免对矿工奖励产生负面影响。
...
- 授权方 = 拥有投票权的账户（冷钱包密钥）
- 被授权方 = 代表授权方提交投票的账户（热钱包密钥）

被授权方仍可为自己账户投票。授权方可随时撤销此权限。
...
1. 升级 v0.2.10 引入了对 Executor → MLNode 连接的严格5分钟超时，而某些请求可能需要更长时间。新版本的API将恢复该值，而不是强制执行严格限制。
2. 之前的请求重试系统即使因处理超时（而非TLS超时）失败，仍会重试推理。
服务器端对长请求的重试通常无效，因为这会导致相同的超时场景。同时，客户端可能收到不一致的输出。新版本的API在这种情况下不再重试推理。
...
现在提供一个新的 vLLM / mlnode 构建，可在PoC开始时中断正在进行的推理请求，以降低PoC开始时仍处于活动状态的请求导致潜在权重下降的风险。
...
抵押激活是协议从宽限期过渡到完全抵押的PoC权重模型的一部分。治理仍然是调整参数的机制，但若未批准替代方案，则默认规则适用。

!!! note "重要：存入时预留缓冲**

    强烈建议参与者**不要**存入精确的最低金额。由于归一化效应和网络级调整，PoC权重在各轮次间可能波动。较小的权重可能经历相对更大的波动。为避免在轮次边界出现临时抵押不足，建议在抵押水平相对较低时存入高达计算最低要求2倍的金额。这提供了操作安全性，防止因微小参数变动导致意外权重降低。协议不会自动补充抵押。
...
为了使PoC权重与实际区块生成时间对齐，校准参数基于观察到的区块间隔选定。如实施所示，有效的PoC参考窗口比先前的名义假设长约5个区块。
...
!!! note "兼容性说明"
    ...
    **基于实时的 PoC 权重归一化**
    ...
    此升级根据实际 PoC 持续时间对 PoC 参与者权重进行归一化，以减少区块时间漂移效应，并确保权重结果与实际执行时长一致。
...
**宽限期：** 升级后引入了一个 3000 个区块的宽限期，在此期间不进行确认 PoC，并且对升级时期的错过率和无效率阈值要求更为宽松。
...
- 每单位算力 0.032 GNK（约每 H100 10 GNK）
- 错过率或封禁惩罚为 0.01%
- 无效推理惩罚为 0.5%
...
投票结束后、第 180 个周期前，每位矿工必须按照[说明](https://gonka.ai/host/collateral/#slashing)将所需资金转入抵押。否则，从第 180 个周期开始，其奖励将减少 5 倍。
...
如AMA期间讨论并根据社区成员提出的论点，建议从较小的抵押要求和最低的惩罚开始。
...
[拉取请求](https://github.com/gonka-ai/gonka/pull/695) 针对下一次链上软件升级 v0.2.10 已开放审阅。欢迎提供反馈和改进建议。当前计划是将审阅窗口保持约两天。

对于对本次 PR 审阅做出有意义贡献的，可能在下一次升级中提出奖励。

本次仅为对拉取请求的审阅呼吁，并非正式投票的开始。治理投票流程将在审阅期结束后启动。

**关键变更****[PR #710](https://github.com/gonka-ai/gonka/pull/710) PoC 验证采样优化**

此升级引入了一种新的 PoC 验证机制，通过为每个参与者分配固定的采样验证者集合，将复杂度从 O(N²) 降低至 O(N x N_SLOTS)。参考设计与分析：[https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md](https://github.com/gonka-ai/gonka/blob/upgrade-v0.2.10/proposals/poc/optimize.md)

**[PR #725](https://github.com/gonka-ai/gonka/pull/725) 按实际 PoC 时间标准化 PoC 权重**

此升级通过实际 PoC 持续时间对参与者权重进行标准化，以减少区块时间漂移影响，并确保权重结果与实际执行时长一致。
...
如果您有任何解析`inferenced` CLI JSON输出的脚本，请在此升级后重新检查。由于ibc-go升级至v8.7.0，枚举现在以字符串形式编码，而非数字，int64/uint64值现在也以字符串形式编码。
...
- 在当前模型配置下，基于观察到的推理轨迹，B200 GPU的推理性能并未高于H100级GPU。
...
在更新后的权重分配生效后，跟踪器（仪表板）维护人员可能希望审查其第159期及之后的系数，以确保与当前PoC v2的权重分配保持一致。
...
所有主机必须随时准备安装补丁。
请保持在线并密切关注公告。一旦补丁准备就绪，将立即分享进一步说明。

## 2026年2月1日

**升级已执行：v0.2.9 现已在主网上线**

关于升级提案 v0.2.9 的链上治理投票已结束。该提案已获批准，并于区块 2451000 成功在主网上执行了升级。此次升级实现了 PoC v2 的权重分配，并完成了向传统 PoC 机制的过渡。
...
他们的总权重约为 36%。加上完全未参与 PoC 的参与者，PoC 验证参与不足或极低的总权重达到约 48%，这属于临界水平。
...
本指南说明主机应如何根据v0.2.8版本模型可用性变化及即将推出的PoC v2更新来升级其ML节点。ML节点配置在第155个纪元起需符合PoC v2要求。建议主机在该时间点前审查并准备其ML节点配置。迁移至PoC v2可在第155个纪元后安排。迁移阶段结束后，不符合配置要求的ML节点的权重将不被计入。
...
作为 v0.2.8 升级的一部分，支持的模型集已更新。所有先前支持的模型均从活跃集中移除，除了：

- `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8`
- `Qwen/Qwen3-32B-FP8`

通过使用 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 成功参与 PoC v2，并配合所需的 ML Node 版本，可用于评估向 PoC v2 过渡的准备情况。一旦观察到活跃 Host 中达到足够程度的采用率（约 50%），可提交单独的治理提案以批准并激活 PoC v2 用于分配权重。此阈值为观察性指标，不会触发任何自动网络变更。
...
- 授权绕过与重放：将 epochId 绑定至签名，并验证授权是否与正确的 epoch 匹配。
...
该延长也为主机提供了时间，以准备其设置以满足新的 PoC 要求，包括将 ML 节点切换到 `Qwen/Qwen3-235B-A22B-Instruct-2507-FP8` 模型。更新现有 ML 节点的说明请参见：[https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode](https://gonka.ai/host/mlnode-management/#updating-an-existing-mlnode)。运行多个 ML 节点的主机建议在多个纪元中逐步进行更新。
...
2) 此前表现出与这些模式持续偏离的参与者将被排除在 `allowlist` 之外，直至稳定窗口在区块 2,222,222 结束。
...
如果获得批准，网络将暂时使用 `allowlist`，包含在之前纪元中未表现出非标准硬件行为的参与者。实际上，`allowlist` 对应于在多个纪元中满足以下条件的参与者：

- 报告的硬件特征已使用预定义的常见硬件配置模式进行评估，以识别偏离和不一致（非标准配置字符串的确切列表见此处：[https://github.com/product-science/filter/blob/main/filter_strings.txt](https://github.com/product-science/filter/blob/main/filter_strings.txt)），且
- 观察到的PoC权重低于使用同类硬件的其他参与者所展示权重的150%。
此前表现出这些模式持续偏离的参与者在稳定窗口于区块 2222222 结束前不属于 `allowlist`。
...
链上治理投票已结束，关于v0.2.7升级提案：创世验证者增强方案已获得批准，并在主网上成功部署。
...
- 开发者地址注册：在稳定期内，新开发者地址的注册将暂停。一个预定义的`allowlist`开发者地址白名单立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者级别注册和推理执行，将持续有效至区块2.294.222（约19天）。

**治理控制机制**

本升级中包含的预备性变更，为未来通过治理方式控制参与者入网和推理执行提供了支持，无需额外软件升级。本提案未启用任何此类治理激活的限制，需另行投票决定。
...
通过为 `node` 容器设置环境变量 `UNSAFE_SKIP_BACKUP=true`，可以在 Cosmovisor 升级期间跳过自动备份。
...
创世验证者增强最初在网络早期阶段作为临时防御机制引入，并在运行的前两个月内处于激活状态。当前治理提案旨在根据当前网络状况，临时重新激活此现有机制，并启用一些额外的保护措施。
...
本次升级将此前通过手动 API 更新分发并已在网络上使用的重大修复正式化。这些修复包括：
...
- 开发者地址注册：在稳定期内暂停新开发者地址的注册。一个预定义的 `allowlist` 开发者地址列表立即生效。列入白名单的开发者地址在此期间可执行推理。所有适用于开发者地址的限制，包括开发者注册和推理执行，将持续有效至区块 2,294,222（约 19 天）。

**治理控制机制**

本次升级包含的预备变更，为未来通过治理机制控制参与者准入和推理执行提供了支持，无需额外软件升级。本提案中未启用任何此类治理激活的限制，需经额外治理投票决定。
...
- 所有在第 117 期活跃的节点，将获得额外支付，金额为第 117 期奖励的 1.083 倍，统一适用于所有符合条件的节点，包括已收到原始奖励的节点。
...
## 2026年1月5日
当前网络上观察到高于常态的推理遗漏率。
在许多情况下，这是由于一个bug：不支持格式的推理请求未被标记为已完成，尽管请求本身已被处理。以下更新将解决此行为。
