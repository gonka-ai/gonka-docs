...
!!! note "关于钱包桥接兼容性的重要说明。如果您打算通过以太坊桥出售 Gonka 代币，请仔细阅读"
    当前桥接功能对账户设置有特定要求。某些钱包可能允许您创建 Gonka 账户甚至导出私钥，但这并不总意味着该账户能与桥接功能正常工作。如需使用桥接功能，请通过以下方式之一创建您的 Gonka 账户：
...
=== "外部钱包"
    ...
    === "我没有外部钱包"
        ...
        === "Keplr 移动应用"
            ...
            点击“通过 Google 登录”，按照指示使用 Gmail 登录。
            ...
            安全备份您的私钥。任何人拥有您的私钥都可以访问您的资产。如果您丢失了对 Gmail 账户的访问权限，恢复钱包的唯一方法就是使用私钥。请将私钥存放在安全的地方，切勿与任何人分享。
            ...
            您已成功复制您的 Gonka 账户地址。您可以将其分享给任何要向您付款的人。分享地址是安全的。
        ...
        === "Keplr 浏览器插件"
            ...
            此时，扩展程序已安装，但您的钱包和 Gonka 账户尚未创建。请继续下一步进行设置。
            ...
            您已成功复制您的 Gonka 账户地址。您可以将其分享给任何要向您付款的人。分享地址是安全的。  
            要在移动设备上访问您的钱包，请下载 Keplr 应用并使用注册时相同的登录方式登录。您的 Gonka 网络账户将自动出现在移动钱包应用中。
        ...
        === "Cosmostation 浏览器插件"
            ...
            !!! note "重要提示：功能受限"
                此选项使用助记词创建账户，**不支持通过桥接进行交易**。如果您希望通过桥接进行交易，请改用 `inferenced` CLI 工具或 Keplr（选择“通过 Google 登录”路径）创建 Gonka 账户。
            ...
            按顺序完成测试题。核对已备份的助记词，并为每个编号选择正确的词语。
            ...
            在右上角点击“所有网络”，选择 Gonka 链以将其添加到您的钱包中。
            ...
            完成！您的 Gonka 账户已成功创建。要复制可用于收款的地址（通常以 `gonka...` 开头），请点击余额上方的地址。
            ...
            点击“Gonka”以查看私钥。复制您的私钥或助记词，并妥善保存（建议使用纸质备份）。
    ...
    === "我已有外部钱包"
        ...
        === "Keplr 移动应用"
            ...
            ??? note "关于钱包桥接兼容性的重要说明。如果您计划通过以太坊桥出售 Gonka 代币，请仔细阅读"
                目前，桥接服务对账户设置有特定要求。某些钱包可能允许您创建 Gonka 账户并导出私钥，但这并不总意味着该账户能与桥接服务正常协作。如需使用桥接功能，请通过以下任一方式创建您的 Gonka 账户：
                ...
                这些是为需要以太坊桥兼容性的用户推荐且受支持的方式。

            打开 Keplr 移动应用并登录您的钱包。点击左上角的菜单按钮。
            
            <a href="/images/keplr_mobile_start_screen.PNG" target="_blank"><img src="/images/keplr_mobile_start_screen.PNG" style="width:auto; height:337.5px;"></a>

            点击“添加/移除链 (Add/Remove Chains)”。
            ...
            您已成功复制 Gonka 账户地址。您可以将其分享给任何要向您付款的人。此地址可安全分享。
        ...
        === "Keplr 浏览器插件"
            ...
            为您的浏览器安装扩展程序（如果已安装，请跳转至步骤 [“将 Gonka 网络添加到您的钱包”](#add-gonka-network-to-your-wallet)）。
            
            前往 [Keplr 官方网站](https://www.keplr.app/){target=_blank}，点击“获取 Keplr 钱包 (Get Keplr wallet)”。
            ...
            安装完成后，您应在浏览器的右上角看到该扩展图标。
                
            <a href="/images/keplr_extension.png" target="_blank"><img src="/images/keplr_extension.png" style="width:500px; height:auto;"></a>

            此时扩展已安装，但尚未连接到您的钱包。  
            接下来，请打开扩展并登录您的钱包。登录后，请按照以下步骤继续完成设置。

 
            #### 将 Gonka 网络添加到您的钱包

            本指南介绍如何将 Gonka 网络添加到您的钱包，以及您的 Gonka 账户将如何被创建。  
            打开 Keplr 浏览器扩展，点击左上角的菜单按钮。
            ...
            打开 Keplr，点击余额上方的“复制地址 (Copy Address)”按钮。
            ...
            您已成功复制 Gonka 账户地址。您可以将其分享给任何要向您付款的人。此地址可安全分享。  
            若要在移动设备上访问您的钱包，请下载 Keplr 应用，并使用注册时相同的登录方式登录。您的 Gonka 网络账户将自动出现在手机钱包应用中。

=== "通过 `inferenced` CLI 工具"
    
    本指南介绍如何使用 `inferenced` CLI 工具创建 Gonka 网络账户。请下载 `inferenced` CLI 工具（适用于您系统的最新二进制文件请见 [此处](https://github.com/gonka-ai/gonka/releases)）。

    !!! note "什么是 inferenced CLI 工具？"
        `inferenced` CLI 工具是一个命令行接口工具，用于与 Gonka 网络进行交互。它是一个独立的可执行二进制文件，允许用户创建和管理 Gonka 账户、执行推理任务、上传模型，并通过脚本命令自动化各种操作。
    ...
    ??? note "关于账户名称的注意事项"
        ...
        名称的唯一性是本地的：使用相同名称创建两个密钥会覆盖已存在的密钥（CLI 会发出警告）。若继续操作，原始密钥将永久丢失。强烈建议在执行此操作前备份你的公钥和私钥。
    ...
    ??? note "为什么要使用随机节点？"
        为了避免过度依赖创世节点并促进网络去中心化，Gonka 推荐从当前纪元中选择一个随机的活跃节点。这有助于改善网络负载分布，并增强对节点故障的容错能力。

    ??? note "如何选择节点 URL？"
        你可以随机选择任意节点——**无需**考虑该节点运行的是哪个模型。在此阶段，节点仅作为获取网络状态和广播交易的网关使用。所有节点都暴露相同的公共 API。
    ...
    下载 `inferenced` CLI 工具（适用于您系统的最新 `inferenced` 二进制文件请见[此处](https://github.com/gonka-ai/gonka/releases)）。

    ??? note "在 Mac OS 上启用执行权限"
        在 macOS 上，下载 `inferenced` 二进制文件后，您可能需要手动启用执行权限。请按照以下步骤操作：
         
 
        1.	打开终端，导航到二进制文件所在的目录。
        ...
        chmod +x inferenced
        ```
        3.	运行 `./inferenced --help` 以确认其正常工作。

        4.	如果尝试运行 `inferenced` 时出现安全警告，请前往“系统设置”→“隐私与安全”。
...
要访问您的 Gonka 私钥，请导出私钥并妥善保存。以下命令将输出明文格式的私钥。私钥是一段保密的代码，拥有它就等于完全控制了您的钱包及其内的资金。它用于确认（签名）交易，证明您是该钱包的所有者。

    - 谁拥有私钥，谁就控制钱包。
    - 如果丢失私钥，您将失去对钱包的访问权限。
    - 如果他人获取了您的私钥，他们便可转移您的资金。
