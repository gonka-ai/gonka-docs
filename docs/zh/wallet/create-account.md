...
!!! note "关于钱包桥接兼容性的重要提示。如果您打算通过以太坊桥出售 Gonka 币，请仔细阅读"
    当前桥接功能对账户设置有特定要求。某些钱包可能允许您创建 Gonka 账户甚至导出私钥，但这并不总意味着该账户能与桥接功能正常协作。如需使用桥接功能，请通过以下方式之一创建您的 Gonka 账户：
...
=== "外部钱包"
    ...
    === "我没有外部钱包"
        ...
        === "Keplr 移动应用"
            ...
            点击“使用 Google 登录”，按照提示通过 Gmail 登录。
            ...
            安全备份您的私钥。任何人拥有您的私钥都可以访问您的资产。如果您丢失了 Gmail 账户的访问权限，恢复钱包的唯一方法就是使用私钥。请将私钥存放在安全的地方，切勿与他人分享。
            ...
            在搜索栏中输入“Gonka”，选择 Gonka 链以将其添加至您的钱包。
            ...
            您已成功复制您的 Gonka 账户地址。您可以将其分享给向您付款的人。分享地址是安全的。
        ...
        === "Keplr 浏览器扩展"
            ...
            此时，扩展已安装，但您的钱包和 Gonka 账户尚未创建。请继续下一步进行设置。
            ...
            您已成功复制您的 Gonka 账户地址。您可以将其分享给向您付款的人。分享地址是安全的。  
            要在移动设备上访问您的钱包，请下载 Keplr 应用并使用注册时相同的方法登录。您的 Gonka 网络账户将自动出现在移动钱包应用中。
        ...
        === "Cosmostation 浏览器扩展"
            ...
            !!! note "重要提示：功能受限"
                此选项使用助记词创建账户，**不支持通过桥接进行交易**。如果您希望使用桥接功能，请改用 `inferenced` CLI 工具或 Keplr（选择“使用 Google 登录”路径）创建 Gonka 账户。
            ...
            记下您的助记词。**切勿**将恢复短语分享给任何人。任何人拥有您的恢复短语都可以完全控制您的资产。请时刻警惕钓鱼攻击，并安全备份该短语。
            ...
            按顺序完成测试题。确认已备份的助记词，并为每个编号选择正确的短语。
            ...
            在右上角点击“所有网络”，选择 Gonka 链以将其添加至您的钱包。
            ...
            完成！您的 Gonka 账户已成功创建。要复制可用于收款的地址，请点击余额上方的地址。地址通常以 `gonka...` 开头。
    ...
    === "我已有外部钱包"
        ...
        === "Keplr 手机应用"
            ...
            ??? note "关于钱包桥接兼容性的重要说明。如您计划通过以太坊桥出售 Gonka 代币，请仔细阅读"
                目前桥接功能对账户设置有特定要求。某些钱包可能允许您创建 Gonka 账户甚至导出私钥，但这并不意味着该账户一定能与桥接功能正常配合使用。如需使用桥接功能，请通过以下方式之一创建您的 Gonka 账户：
        ...
        === "Keplr 浏览器插件"
            ...
            为您的浏览器安装扩展程序（如果您已安装，请跳转至[“将 Gonka 网络添加到钱包”](#add-gonka-network-to-your-wallet)步骤）。
            
            前往 [Keplr 官方网站](https://www.keplr.app/){target=_blank} 并点击“获取 Keplr 钱包 (Get Keplr wallet)”。
            ...
            安装扩展程序后，您将在浏览器的右上角看到它。
                
            <a href="/images/keplr_extension.png" target="_blank"><img src="/images/keplr_extension.png" style="width:500px; height:auto;"></a>

            此时扩展程序已安装，但尚未连接到您的钱包。接下来，请打开扩展程序并登录您的钱包。登录后，请按照以下步骤继续完成设置。

 
            #### 将 Gonka 网络添加到您的钱包

            本指南介绍如何将 Gonka 网络添加到您的钱包，以及您的 Gonka 账户将如何创建。打开 Keplr 浏览器扩展程序，点击左上角的菜单按钮。
...
=== "通过 `inferenced` CLI 工具"
    ...
    本指南介绍如何使用 inferenced CLI 工具创建 Gonka 网络账户。请下载 `inferenced` CLI 工具（适用于您系统的最新 `inferenced` 二进制文件位于[此处](https://github.com/gonka-ai/gonka/releases)）。

    !!! note "什么是 inferenced CLI 工具？"
        `inferenced` CLI 工具是一种命令行接口工具，用于与 Gonka 网络进行交互。它是一个独立的可执行二进制文件，允许用户创建和管理 Gonka 账户、执行推理任务、上传模型，并通过脚本命令自动化各种操作。
    ...
    ??? note "为什么要选择随机节点？"
        为了避免对创世节点的过度依赖并促进网络去中心化，Gonka 建议从当前纪元的活跃节点中随机选择一个节点。这有助于改善网络负载分布，并增强对节点故障的容错能力。

    ??? note "如何选择节点 URL？"
        你可以随机选择任意节点——**无需**考虑其运行的模型。在此阶段，节点仅用作获取网络状态和广播交易的网关。所有节点都暴露相同的公共 API。
    ...
    === "当前活跃参与者列表"
        或者，您可以从当前纪元中随机选择一位活跃参与者。打开以下链接或运行相应命令，以获取活跃参与者列表及用于验证的加密证明：
        === "链接"
    ...
    ??? note "在 Mac OS 上启用执行权限"
        ...
        4.	如果尝试运行 `inferenced` 时看到安全警告，请前往“系统设置” → “隐私与安全性”。
...
此命令将执行以下操作：
    ...
    - 生成密钥对
    - 将其保存到 `~/.inference`
    - 返回您的账户地址、公钥和助记词（请同时将助记词安全地抄写在纸质介质上保存！）
...
要访问您的 Gonka 私钥，请导出私钥并妥善保存。以下命令将输出明文形式的私钥。私钥是一段保密的代码，拥有它就等于完全控制了您的钱包及其内的资金。它用于确认（签名）交易，证明您是该钱包的所有者。

    - 谁拥有私钥，谁就控制钱包。
    - 一旦丢失，您将失去对钱包的访问权限。
    - 若被他人获取，您的资金可能会被全部盗取。

因此，私钥必须始终安全存储，绝不可与任何人共享。
