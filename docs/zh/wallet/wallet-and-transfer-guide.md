...
本指南介绍如何在网络中使用钱包和代币：如何获取钱包地址、查询余额、发送代币以及跟踪交易记录。  
在执行任何钱包操作之前，您需要先访问您的账户。请根据您在网络中的角色，按照以下说明操作。

**您是节点提供者（Host）吗？**

您提供计算资源，并获得代币作为奖励。  
在继续操作前，您需要访问您的钱包，该钱包会在 `chain-node` 容器首次运行时自动创建。

**您是开发者（Developer）吗？**

如果您仅需进行推理任务（发送提示并获取结果），则无需 Gonka 账户——请前往 [开发者快速入门](https://gonka.ai/developer/quickstart/) 使用社区代理。

如果您正在运行自己的网关，或需要在链上持有和转账 GNK 代币，请先 [创建一个 Gonka 账户](https://gonka.ai/wallet/create-account/)，然后返回此处。

在成功访问账户后，请返回本指南学习以下操作：
...
在链上，唯一有效的单位是 `ngonka`。所有余额、手续费和交易都必须使用 `ngonka`。  
Cosmos SDK 可能允许定义其他单位，但这些单位无效——SDK 不会自动在不同单位之间进行转换。  
`gonka` 仅作为链下的人类友好显示单位使用，代表 10 亿 `ngonka`，其本身并不存在于链上。

**有效单位**

| 单位     | 用途                         | 是否在链上 | 换算比例                             |
| --- | --- | --- | --- |
| `ngonka` | 网络中使用的基准单位         | 是         | 1                                    |
| `gonka`  | 便于阅读的显示单位           | 否         | 1 `gonka` = 1,000,000,000 `ngonka`   |

## 获取您的钱包地址

在查询余额或发送资金之前，您需要知道自己的钱包地址。
...
要检查您的余额，请使用以下命令：确保转账前余额充足，或验证转账是否成功。
...
=== "CLI"
    ...
    你可以使用 Cosmos SDK 提供的命令行工具（具体为 `inferenced` CLI）来执行转账操作。每笔转账都会被记录在区块链上，且必须包含有效的发送方、接收方、转账金额以及代币面额。

    在确认你的账户余额并获取接收方地址后，即可发送代币。
    ...
    !!! note
        ...
        `<sender-key-name>` 是密钥环中的**本地密钥名称**——即创建密钥时所选择的相同标签。可使用 `inferenced keys list` 查看本机上的密钥名称。
...
=== "Keplr（浏览器扩展）"
    ...
    使用 Keplr 钱包在 Gonka 链上的 Gonka 账户之间进行转账时，请先登录并打开您的 Keplr 钱包。
...
=== "Keplr（移动应用）"
    ...
    使用 Keplr 移动钱包在 Gonka 链上的 Gonka 账户之间进行转账时，请先登录并打开您的 Keplr 钱包。
    ...
    等待页面提示交易成功。由于 Gonka 是非原生链，您不会在“活动”标签页中看到该交易记录。
...
发送交易后，您可能需要确认该交易是否已成功处理并被打包进区块。每笔交易都会被分配一个唯一的交易哈希（`TXHASH`），您可以使用该哈希在区块链上查询其状态。  
要查看交易状态，请使用以下命令：
```bash
...
- `code: 0` — 表示成功  
- 区块 `height`（高度）— 交易被打包的区块高度  
- `timestamp`（时间戳）— 区块提交的时间  
- 交易消息的详细信息（例如 `sender`（发送方）、`receiver`（接收方）、`amount`（金额）、`module`（模块）、使用的 `gas`）
